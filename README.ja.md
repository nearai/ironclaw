...
- ソースビルドには Rust 1.96+
- `webui-v2-beta` を有効にするソースビルドには Node.js 22+ と Corepack/pnpm
- 選択した LLM プロバイダーの認証情報
- PostgreSQL 15+ はホスト／本番 profile でのみ任意。local-dev では不要
...
正式な `ironclaw` コマンドは Reborn を実行します。初期化では
`~/.ironclaw/reborn` に独立した設定を作成し、v1 のデータベース、設定、キーチェーンには触れません：
...
ローカル利用では PostgreSQL の手動作成は不要です。ホスト環境で PostgreSQL を使う場合も、接続 URL とマスターキーは既存の `IRONCLAW_REBORN_*` 環境変数から渡します。詳細は英語版 README と [Reborn onboarding](docs/reborn/onboarding.md) を参照してください。
...
CLI でプロバイダーとモデルを選択し、表示された認証情報の環境変数を設定します：
...
プロバイダーの検出と選択については、[canonical CLI モデルガイド](docs/reborn-binary.md#models)をご覧ください。
...
- Runtime 状態はデフォルトでローカルに保存され、ホスト／本番 profile では PostgreSQL を選択可能
...
現在のリリースは Reborn ランタイムを標準で使用し、正式な CLI コマンドは `ironclaw` です。`ENGINE_V2` の設定は不要です。

```bash
# Reborn home、設定ファイル、WebUI token を初期化
ironclaw onboard

# 1 件のメッセージを送信、または対話 REPL を起動
ironclaw run --message "こんにちは"
ironclaw repl
