...
This project has two machines:
- **GPU machine** (primary for heavy tasks): `root@116.62.144.47` at `/data/work/claw/ironclaw` (configured in `~/.ssh/config` as `gpu`)
- **Current machine**: Limited resources, avoid running `cargo check/build/test` here

**Before making code changes:**
1. On GPU machine: `cd /data/work/claw/ironclaw && git pull origin staging`
2. Verify with `cargo fmt` / `cargo check` on the GPU machine

**To connect to GPU machine:**
```bash
ssh gpu
cd /data/work/claw/ironclaw
```
...
```
Store token in `~/.git-credentials` or use `git remote add origin https://ghp_TOKEN@github.com/user/repo.git`

## PR Review Lessons (PR #1446 Case Study)

This section documents all mistakes made during PR #1446 (Aliyun Coding Plan support) to avoid repeating them.

### Common Mistakes & Fixes

1. **Always rebase, don't just merge**
   - Problem: `git merge upstream/staging` creates merge commits, making PR history messy
   - Fix: Use `git rebase upstream/staging` to keep linear history

2. **Resolve conflicts properly**
   - Problem: Leftover conflict markers (`<<<<<<< HEAD`) broke compilation
   - Fix: Always run `cargo fmt` / `cargo check` after resolving conflicts

3. **Check PR status before asking for review**
   - Problem: Multiple times PR was "out of date" or had merge conflicts
   - Fix: Before pushing, verify: `git fetch upstream && git rebase upstream/staging`

4. **Test locally before push**
   - Problem: Many issues caught by reviewer that could have been caught locally
   - Fix: Always run `cargo fmt && cargo check` before push (on GPU machine for this project)

5. **Don't break compilation**
   - Merge conflicts, syntax errors, or type mismatches must be resolved before push
   - Use GPU machine: `ssh gpu && cd /data/work/claw/ironclaw`

### Specific Code Issues Fixed

| Issue | Severity | Fix |
|-------|----------|-----|
| Text content overwrite bug | HIGH | Use `Vec<String>` for accumulation, then `join()` |
| Silent tool call loss | HIGH | Add `tracing::warn!` for each dropped call |
| HTTP 429 not detected | MEDIUM | Map to `LlmError::RateLimited` with Retry-After |
| Usage data missing warning | MEDIUM | Add `tracing::warn!` when usage is None |
| API key not validated | MEDIUM | Validate at config time, fail early with clear error |
| http1_only() undocumented | MEDIUM | Add comment explaining HTTP/2 compatibility issues |
| Model override not handled | MEDIUM | Use `take_model_override()` before API call |
| Tool results not merged | MEDIUM | Merge adjacent Role::Tool messages |
...
1. **Before any push:**
   ...
   cargo fmt
   cargo check -p ironclaw --lib  # Run on GPU machine
   ```

2. **After rebase fix conflicts:**
   ```bash
   git add -A
   git rebase --continue
   # Then verify again
...
3. **Check PR status via API:**
   ```bash
   curl -s "https://api.github.com/repos/owner/repo/pulls/N" | jq '.mergeable, .state'
   ```
