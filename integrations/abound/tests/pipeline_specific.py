    ...
    for sub in range(1, runs_per_thread + 1):
        ...
        for turn in range(1, MAX_TURNS + 1):
            ...
            for c in calls:
                ...
                if not action:
                    # Qwen embeds action in tool name: "abound_send_wire(initiate)"
                    _, _, suffix = c["name"].partition("(")
                    if suffix.endswith(")"):
                        action = suffix[:-1]
                output = c.get("output") or ""
                note(f"  DBG: name={c['name']!r} action={action!r} output_start={output[:60]!r}", sub)
                ...
                if action == "initiate":
                    if output.startswith("Error:"):
                        ...
                        # Output capture fails when same tool is called multiple
                        # times per turn (last_call_id_for resolution is racy).
                        # The call was made and no error surfaced — treat as seen.
