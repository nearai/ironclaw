"""Replay pipeline: exact conversation sequence from the 23:10:24.784 log that
caused action=send to be skipped (initiate → execute directly).

Observed turns:
  1. "Send money to indi"
  2. "Send money to <recipient> (account ending in <mask>)"
  3. "Use <bank> account ending in <mask>"
  4. "0.7$"
  5. "Let's do 15"
  6. "The payment reason is Investment"   ← triggers initiate (IR015)
  7. "send now"                           ← should trigger send, was execute in the log

Adapts recipient and funding source to the test account's real values.
Payment reason is always IR015 (Mutual Fund Investment) to match the original.
"""
