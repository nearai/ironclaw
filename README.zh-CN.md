...
- 从源码构建时需要 Rust 1.96+
- 启用 `webui-v2-beta` 的源码构建需要 Node.js 22+ 和 Corepack/pnpm
- 所选 LLM provider 的凭据
- PostgreSQL 15+ 仅用于可选的托管/生产 profile；local-dev 不需要
...
规范的 `ironclaw` 命令运行 Reborn。初始化会在
`~/.ironclaw/reborn` 下创建独立配置，不会读取或修改 v1 数据库、设置或钥匙串：
...
本地运行不需要手动创建 PostgreSQL 数据库。托管部署可以在
`config.toml` 中选择 PostgreSQL，但连接 URL 和主密钥必须通过现有的
`IRONCLAW_REBORN_*` 环境变量提供。详见英文 README 和
[Reborn onboarding 文档](docs/reborn/onboarding.md)。
...
使用 CLI 选择提供商和模型，再按提示设置对应的密钥环境变量：
...
Provider 发现和选择请参阅 [canonical CLI 模型指南](docs/reborn-binary.md#models)。
...
- Runtime 状态默认保存在本地；托管/生产 profile 可选择 PostgreSQL
- 密钥使用 AES-256-GCM 加密
- 无遥测、无分析、无数据共享
- 所有工具执行的完整审计日志
...
当前发行版默认使用 Reborn 运行时，规范的 CLI 命令是 `ironclaw`；不再需要设置 `ENGINE_V2`。

```bash
# 首次初始化 Reborn home、配置文件和 WebUI token
ironclaw onboard
