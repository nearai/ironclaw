...
- **Broad full-path extension↔provider mutation coverage is incremental**:
  the Reborn E2E job replays harvested live-QA model traces against Emulate's
  supported provider operations and runs serve-native provider paths. Equivalent
  mutation paths for provider operations outside that inventory remain
  follow-up work.
