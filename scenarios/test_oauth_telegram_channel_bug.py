...
async def test_oauth_url_parameter_naming_web_vs_telegram(ironclaw_server, installed_gmail):
    """Test that OAuth URLs have consistent parameter names across channels.

    Specifically validates that the 'client_id' parameter (with underscore)
    is present, not 'clientid' (without underscore), which was the symptom
    of bug #992.
    """
...
async def test_oauth_url_cannot_be_regenerated_symptom(ironclaw_server, installed_gmail):
    ...
    Each request should generate a new URL with a unique CSRF state.
    """
    # Get first OAuth URL
    r1 = await api_post(
        ironclaw_server,
...
async def test_oauth_url_parameter_structure(ironclaw_server, installed_gmail):
    ...
    for param_name, description in expected_params.items():
        assert param_name in params, f"Missing parameter: {param_name} - {description}"
...
async def test_oauth_url_matches_google_spec(ironclaw_server, installed_gmail):
...
async def test_oauth_extra_params_preserved(ironclaw_server, installed_gmail):
