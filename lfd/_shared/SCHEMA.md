...
runner (`tests/integration/lfd/`), the scorer (`lfd/_shared/scorer/`), and every
...
- `profile`: selects the runner profile (harness assembly). One profile per
  feature; registry in `tests/integration/lfd/profiles/`.
...
- `tests/integration/lfd/**` and `tests/integration/support/**` are hash-pinned in
...
- Exactly ONE agent-writable runner file per feature:
  `tests/integration/lfd/profiles/<feature>.rs` (harness assembly only). Outcome
