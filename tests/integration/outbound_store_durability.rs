...
use ironclaw_reborn_composition::{RebornHostBindings, RebornRuntimeInput, build_runtime};
...
async fn filesystem_outbound_state_store_persists_across_reopen() {
    ...
    let services = build_runtime(RebornRuntimeInput::from_build_input(
        RebornHostBindings::local_dev("w6-outbound-durability", dir.path().join("local-dev")),
    ))
    .await
