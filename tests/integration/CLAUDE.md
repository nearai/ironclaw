...
root, extension-host registry-backed planner, installation record, and
host-mediated network egress. Its URL must use a literal IPv4 loopback host. The
installed-local fixture declares no credential, so no authorization header is
injected; the test proves the exact manifest endpoint is the only private-range
exception.
