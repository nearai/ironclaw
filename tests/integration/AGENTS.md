...
> `tests/AGENTS.md`** — the repo-wide scenario coverage map (what each scenario
...
1. **Test-first & consolidate.** Follow `.claude/rules/testing.md` — write/update
   the test first and watch it fail for the right reason; extend an existing
   test rather than standing up a redundant one, and say why if you add a new one.
...
- `harness/` — the `HostRuntimeCapabilityHarness` split (was a single `harness.rs`):
  ...
  `coding_read`, `core_builtin`, `device_link`, `extension`, `file`, `github`,
  `mock_mcp`, `outbound`, `process`, `profile`, `project`, `qa_smoke`,
  `sandbox_shell`, `skill`, `trace_commons`, `trigger`, `web_access` — re-derive
  with `ls tests/integration/support/harness/profiles/`) — each returning a
  `ToolsProfile` via a constructor like `profiles::file::file_tools_requiring_approval()` or
...
`group_constructors.rs` defines ~30 capability-preset constructors (both bare
`RebornIntegrationGroup::xxx()` associated functions and
`RebornIntegrationGroupBuilder` chain methods). Re-derive the full, current
list before adding a new preset — extending an existing one is the rule (see
"Requirements & expectations" item 1 above):

```bash
rg -n "pub async fn " tests/integration/support/group_constructors.rs
```

The two anchors this guide's own examples and prose depend on:
