...
use reborn_support::scripted_provider::{CONTEXT_OVERFLOW_USED_TOKENS, ParkingModelGate};
...
async fn context_overflow_recovers_with_model_visible_observation() {
    // Forced compaction still preserves the configured 8K-token prompt tail.
    // Seed one oversized user message so this whole-turn test exercises the
    // real compactor instead of taking its safe "nothing eligible" skip path.
