...
//! IronClaw memory is scoped by the run's owner (`MemoryDocumentScope` keys the
...
//! #5460 targets.
//!
//! Seam: `IronClawIntegrationGroup::multiuser_memory_tools` builds the backend
...
//! one shared memory subtree — see `tests/ironclaw_group_memory/`).
