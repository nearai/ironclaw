...
//!
//! Before the fix, `IronClawIntegrationGroupBuilder::into_group`'s shared
...
//! (issue #5479). `with_actor_id` on `IronClawThreadBuilder` is the seam that
...
async fn multiuser_group_e2e() {
    ...
    let memory_group = IronClawIntegrationGroup::multiuser_memory_tools()
        .await
    ...
    let approvals_group = IronClawIntegrationGroup::multiuser_approvals()
        .await
