...
//!
//! Proves the model-visible `builtin.bash` dispatch path with the inert
//! `RecordingProcessPort` default: the command is recorded (real dispatch path
//! ran) and no real OS process was spawned (the safety invariant). No network,
//! services, keys,
...
/// Default build: a scripted `builtin.bash` call is recorded by the inert
/// `RecordingProcessPort` and no real OS process is spawned (slice 5 safety
/// invariant).
#[test]
fn shell_call_recorded_not_executed() {
    run_with_larger_stack(async {
        let h = RebornIntegrationHarness::test_default()
            .with_builtin_http_tools()
            .script([
                RebornScriptedReply::tool_call("builtin.bash", json!({"command": "echo s5-probe"})),
...
/// Error path — non-zero exit. A scripted `exit_code = 1` is a completed
/// `builtin.bash` result carrying the OMP exit-code notice, so the run
/// completes and the model can react.
#[test]
fn shell_non_zero_exit_surfaces_as_completed_result() {
    run_with_larger_stack(async {
        let h = RebornIntegrationHarness::test_default()
            .with_shell_exit_code(1)
            .script([
                RebornScriptedReply::tool_call("builtin.bash", json!({"command": "echo boom"})),
                RebornScriptedReply::text("done"),
            ])
            .build()
            .await
            .expect("harness builds");
        h.submit_turn("run shell").await.expect("turn completes");
        h.assert_tool_result_contains("Command exited with code 1")
            .await
...
/// Error path — command timeout. `builtin.bash` converts a process timeout
/// into the OMP timeout notice in a completed model-visible result, so the
/// run continues to completion rather than dying with `driver_unavailable`.
#[test]
fn shell_timeout_surfaces_completed_notice() {
    run_with_larger_stack(async {
        let h = RebornIntegrationHarness::test_default()
            .with_shell_timeout()
            .script([
                RebornScriptedReply::tool_call("builtin.bash", json!({"command": "sleep 999"})),
                RebornScriptedReply::text("done"),
            ])
            .build()
            .await
            .expect("harness builds");
        h.submit_turn("run shell").await.expect("turn completes");
        h.assert_tool_result_contains("[Command timed out after 300 seconds]")
            .await
            .expect("timeout surfaced in the completed model-visible bash result");
        h.assert_reply_contains("done")
            .await
...
/// Live-shell path (`.with_live_shell()`) — proves `builtin.bash` dispatches
