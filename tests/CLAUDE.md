...
| The user can… | Evidence |
|---|---|
| Create an automation through chat; run it now, rename, pause, resume, reload, and delete it through the UI with persisted API state; disable run-now while a fire is active or the scheduler is off; filter automations, retry failed runs, and dismiss error toasts | `test_reborn_webui_v2_smoke.py::test_reborn_v2_automation_lifecycle_persists_from_ui`, `…::test_reborn_v2_automation_run_now_respects_active_fire_and_scheduler`, other automation tests in that file, `test_reborn_webui_v2_automation_trace_outbound_api.py` (4) |
