...
use parity_qa_support::binary_e2e::{
    IronClawBinaryE2EHarness, IronClawHarnessSharedStorage, trace_tool_call_response,
};
...
async fn slack_shaped_harness(
    room: &str,
    model_gateway: IronClawTraceReplayModelGateway,
) -> IronClawBinaryE2EHarness {
    IronClawBinaryE2EHarness::with_model_gateway_scope_installation_shared_storage(
        room,
        model_gateway,
        ...
        IronClawHarnessSharedStorage::new().expect("shared storage"),
    )
    .await
...
#[tokio::test]
async fn ironclaw_qa_slack_dm_strategy_question_gets_reply_in_same_thread() {
...
#[tokio::test]
async fn ironclaw_qa_slack_bug_prefix_message_runs_logging_action() {
...
// (`ironclaw_qa_slack_outbound_reply_delivers_to_bound_reply_target`) was removed
...
// and `tests/ironclaw_adapter_installation_scope_isolation_parity.rs`.
