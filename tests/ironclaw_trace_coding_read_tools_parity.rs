...
use parity_qa_support::{
    binary_e2e::{IronClawBinaryE2EHarness, assert_milestone_order},
    model_replay::{
...
#[tokio::test]
async fn ironclaw_trace_coding_read_tools_parity() {
    ...
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![
                IronClawScriptedProviderToolCall::new(
                    list_dir.clone(),
                    "call_list_dir_notes",
                    serde_json::json!({
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_coding_read_capabilities(
...
fn seed_workspace(harness: &IronClawBinaryE2EHarness) {
