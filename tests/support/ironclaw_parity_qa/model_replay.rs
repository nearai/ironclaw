...
#[derive(Debug, Error)]
pub enum IronClawTraceReplayError {
    #[error("trace response variant cannot be replayed by the IronClaw model gateway")]
    UnsupportedResponse,
...
#[derive(Debug, Clone)]
pub struct IronClawTraceReplayModelGateway {
    inner: Arc<Mutex<ReplayState>>,
}

#[allow(dead_code)]
#[derive(Debug, Clone)]
pub enum IronClawModelReplayStep {
    Response {
    ...
    AssertProviderToolsThenProviderToolCalls {
        capability_ids: Vec<CapabilityId>,
        calls: Vec<IronClawScriptedProviderToolCall>,
        expected_tool_results: Vec<ExpectedToolResult>,
    },
    ResponseForRequest {
...
#[allow(dead_code)]
#[derive(Debug, Clone)]
pub struct IronClawScriptedProviderToolCall {
    pub capability_id: CapabilityId,
    pub call_id: String,
    pub arguments: serde_json::Value,
}

impl IronClawScriptedProviderToolCall {
    #[allow(dead_code)]
    pub fn new(
...
enum ReplayOutput {
    ...
    ProviderToolCalls(Vec<IronClawScriptedProviderToolCall>),
    ModelError {
        kind: HostManagedModelErrorKind,
        message: String,
    },
...
impl IronClawTraceReplayModelGateway {
    pub fn from_trace(trace: LlmTrace) -> Result<Self, IronClawTraceReplayError> {
    ...
    #[allow(dead_code)]
    pub fn with_scripted_steps(steps: impl IntoIterator<Item = IronClawModelReplayStep>) -> Self {
        Self::from_steps(
            steps
                .into_iter()
                .map(|step| match step {
                    IronClawModelReplayStep::Response {
                        response,
                    ...
                    IronClawModelReplayStep::ResponseForRequest {
                        request_contains,
                        response,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::AssertProviderToolsThenResponse {
                        capability_ids,
                        response,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::AssertProviderToolsThenProviderToolCalls {
                        capability_ids,
                        calls,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::DelayedResponse {
                        response,
                        delay,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::DelayedResponseForRequest {
                        request_contains,
                        response,
                        delay,
                        expected_tool_results,
                    ...
                    IronClawModelReplayStep::ProviderToolCalls {
                        calls,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::ProviderToolCallsForRequest {
                        request_contains,
                        calls,
                        expected_tool_results,
                    } => ReplayStep {
                    ...
                    IronClawModelReplayStep::ModelError { kind, message } => ReplayStep {
                        output: ReplayOutput::ModelError { kind, message },
                        request_contains: None,
                        expected_tool_results: Vec::new(),
                    },
                    IronClawModelReplayStep::ModelErrorForRequest {
                        request_contains,
                        kind,
                        message,
                    } => ReplayStep {
...
#[async_trait]
impl HostManagedModelGateway for IronClawTraceReplayModelGateway {
...
async fn provider_tool_calls_response_from_replayed_response(
    ...
    for call in calls {
        ...
        scripted_calls.push(IronClawScriptedProviderToolCall {
...
fn replay_step(step: TraceStep) -> Result<ReplayStep, IronClawTraceReplayError> {
    Ok(ReplayStep {
...
) -> Result<HostManagedModelResponse, IronClawTraceReplayError> {
    match response {
        ...
        TraceResponse::UserInput { .. } => Err(IronClawTraceReplayError::UnsupportedResponse),
    }
}

fn capability_call_from_trace(
    call: TraceToolCall,
) -> Result<CapabilityCallCandidate, IronClawTraceReplayError> {
...
pub(crate) fn capability_call_from_trace_with_surface(
    ...
    let surface_version = CapabilitySurfaceVersion::new(surface_version)
        .map_err(IronClawTraceReplayError::InvalidSurfaceVersion)?;
    ...
    let capability_id = CapabilityId::new(capability_name.clone()).map_err(|error| {
        IronClawTraceReplayError::InvalidCapabilityId {
    ...
    let input_ref =
        CapabilityInputRef::new(format!("input:trace-{}", call.id)).map_err(|reason| {
            IronClawTraceReplayError::InvalidInputRef {
                id: call.id.clone(),
                reason,
            }
        })?;
...
fn trace_provider_tool_name(name: &str) -> Result<ProviderToolName, IronClawTraceReplayError> {
    let provider_name = name.replace('.', "__");
    ProviderToolName::new(provider_name).map_err(|error| {
        IronClawTraceReplayError::InvalidProviderToolName {
            name: name.to_string(),
            reason: error.to_string(),
        }
    })
