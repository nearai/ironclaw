//! Record/replay support for QA-phrase traces against the IronClaw runtime.
//!
...
//! use — it sits at the `LlmProvider` seam, underneath IronClaw's
//! `LlmProviderModelGateway`, so it is runtime-agnostic) and drives a
//! local-dev IronClaw runtime with the production model-gateway conversion
//! layer. The flushed JSON is the recorded `LlmTrace` format that
//! `IronClawTraceReplayModelGateway::from_trace` replays deterministically.
//!
//! Tool names recorded at this seam are the model-facing names the IronClaw
...
use ironclaw_composition::{
    AssistantReply, IronClawCompositionProfile, IronClawProductAuthServices, IronClawRuntime,
    IronClawRuntimeIdentity, IronClawRuntimeInput, IronClawRuntimeProfileOptions,
    IronClawTurnDriveOutcome, PollSettings, TriggerPollerSettings, build_ironclaw_runtime,
    build_ironclaw_services, local_runtime_build_input_with_options,
};
use ironclaw_config::{IronClawConfigFile, IronClawHome};
...
const QA_GOOGLE_ACCESS_HANDLE: &str = "ironclaw_qa_google_access_token";
const QA_GOOGLE_REFRESH_HANDLE: &str = "ironclaw_qa_google_refresh_token";
const QA_GITHUB_ACCESS_HANDLE: &str = "ironclaw_qa_github_access_token";
...
/// The model profile id the composed IronClaw runtime routes turns through;
...
pub fn qa_fixture_path(fixture_name: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/llm_traces/ironclaw_qa")
...
/// (`prefix.tool`). IronClaw advertises capabilities to the model as `prefix__tool`
...
/// pairs. IronClaw advertises built-ins to the model as `builtin__foo`; the QA
...
/// Build the local-dev yolo IronClaw runtime the QA traces are recorded and
...
async fn build_qa_trace_runtime_with_http_interceptor_and_trigger_poller(
    ...
    let mut input = IronClawRuntimeInput::from_services(input)
...
async fn seed_qa_auto_approve(runtime: &IronClawRuntime) {
...
fn seed_static_outbound_delivery_targets(runtime: &IronClawRuntime) {
    runtime
        .register_static_outbound_delivery_target_for_test(
            "qa-trace-slack",
            IronClawOutboundDeliveryTargetId::new("slack:qa-trace-dm").expect("QA Slack target id"),
            "slack",
            "Slack DM",
    ...
    runtime
        .register_static_outbound_delivery_target_for_test(
            "qa-trace-email",
            IronClawOutboundDeliveryTargetId::new("email:qa-trace-inbox")
...
/// reply.
pub async fn send_qa_phrase(runtime: &IronClawRuntime, phrase: &str) -> AssistantReply {
...
async fn seed_live_credentials_for_fixture(
    ...
    let source = IronClawQaCredentialSource::resolve();
    eprintln!(
        "[IronClawQaTrace] importing {} credential account(s) from IronClaw source root {} \
         tenant={} user={} agent={}",
        seeds.len(),
    ...
    let source_product_auth = source_services
        .product_auth
        .as_ref()
        .expect("IronClaw source runtime exposes product auth services");
    ...
    for seed in seeds {
        ...
        let source_access = source_account.access_secret.as_ref().unwrap_or_else(|| {
            panic!(
                "configured IronClaw product-auth account {:?} for provider {:?} has no access secret",
        ...
        secret_store
            ...
            .await
            .expect("seed IronClaw access secret");
        ...
        let refresh_handle = match (source_account.refresh_secret.as_ref(), seed.refresh_handle) {
            (Some(source_refresh), Some(refresh_handle)) => {
                ...
                secret_store
                    ...
                    .await
                    .expect("seed IronClaw refresh secret");
                Some(handle)
            }
            (None, Some(_)) => {
                eprintln!(
                    "[IronClawQaTrace] source account {:?} for provider {:?} has no refresh \
        ...
        let created_account = product_auth
            ...
            .await
            .expect("seed IronClaw credential account");
...
async fn preflight_seeded_qa_credential(
    fixture_name: &str,
    product_auth: &IronClawProductAuthServices,
...
struct IronClawQaCredentialSource {
...
impl IronClawQaCredentialSource {
    fn resolve() -> Self {
        let home = IronClawHome::resolve_from_env()
            .unwrap_or_else(|error| panic!("resolve IronClaw QA credential source home: {error}"));
        let config_file = IronClawConfigFile::load(&home.config_file_path())
            .unwrap_or_else(|error| panic!("load IronClaw QA credential source config: {error}"));
    ...
    async fn build_services(&self) -> ironclaw_composition::IronClawServices {
        ...
        .expect("IronClaw QA credential source input")
        .with_local_runtime_identity(
            TenantId::new(&self.tenant).expect("source tenant id"),
            AgentId::new(&self.agent).expect("source agent id"),
        );
        build_ironclaw_services(input)
            .await
            .expect("build IronClaw QA credential source services")
    }
...
async fn select_source_credential_account(
    product_auth: &ironclaw_composition::IronClawProductAuthServices,
    source: &IronClawQaCredentialSource,
        ...
        Err(selection_error) => {
            let visible_accounts = record_source
                ...
                .unwrap_or_else(|accounts_error| {
                    panic!(
                        "recording fixture {fixture_name:?} could not list visible IronClaw \
            ...
            match scan_local_dev_db_for_source_account(source, &provider).await {
                Ok(Some(account)) => {
                    eprintln!(
                        "[IronClawQaTrace] product-auth record source did not select provider {} \
            ...
            panic!(
                "recording fixture {fixture_name:?} requires exactly one configured \
                 IronClaw product-auth account for provider {:?} in source root {} \
...
async fn resolve_source_secret_scope(
    ...
    eprintln!(
        "[IronClawQaTrace] could not find exact local-dev scope for {kind} secret {} on source \
...
fn read_local_dev_secret_master_key(source: &IronClawQaCredentialSource) -> Result<String, String> {
    let key_path = source
        .local_dev_root
...
async fn consume_source_secret(
    ...
    let lease = match store.lease_once(scope, handle).await {
        ...
        Err(lease_error) => {
            eprintln!(
                "[IronClawQaTrace] source secret store could not lease {kind} secret {} for \
            ...
            return read_local_dev_db_secret_material(source, handle)
                ...
                .unwrap_or_else(|fallback_error| {
                    panic!(
                        "lease {kind} secret for source IronClaw credential account {:?}: \
    ...
    match store.consume(scope, lease.id).await {
        ...
        Err(consume_error) => {
            eprintln!(
                "[IronClawQaTrace] source secret store could not consume {kind} secret {} for \
            ...
            read_local_dev_db_secret_material(source, handle)
                ...
                .unwrap_or_else(|fallback_error| {
                    panic!(
                        "consume {kind} secret for source IronClaw credential account {:?}: \
        ...
        .await
    {
        ...
        Err(error) => {
            ...
            match recorder.flush().await {
                Ok(()) => eprintln!(
                    "[IronClawQaTrace] drive failed ({error}); flushed PARTIAL trace to {} for \
                     inspection — not a valid fixture, do not commit",
                    fixture_path.display()
                ),
                Err(flush_error) => {
                    eprintln!("[IronClawQaTrace] partial-trace flush also failed: {flush_error}")
                }
            }
...
fn assert_recorded_fixture_matches_expected_result(
    ...
    if requires_terminal_reply {
        match outcome {
            IronClawTurnDriveOutcome::Terminal(reply) if reply.is_successful_final_reply() => {}
            IronClawTurnDriveOutcome::Terminal(reply) => {
                panic!(
...
mod tests {
    ...
    async fn local_dev_db_secret_material_reader_decrypts_record() {
        ...
        let source = IronClawQaCredentialSource {
    ...
    async fn ironclaw_qa_crm_google_credential_preflight_from_source_root() {
        let root = tempfile::tempdir().expect("tempdir");
