...
//! IronClaw planned agent loop -> model/capability/transcript evidence.
//!
...
use super::model_replay::IronClawTraceReplayModelGateway;
use crate::ironclaw_support::config::WaitConfig;
use crate::ironclaw_support::doubles::{
    EmptyIdentityContextSource, RecordingTestCapabilityPort, TEST_CAPABILITY_ID,
    TEST_CAPABILITY_SURFACE_VERSION,
};
use crate::ironclaw_support::filesystem::{BlockingTurnStatePutFilesystem, local_filesystem};
use crate::ironclaw_support::harness::profiles::core_builtin::{self, CoreBuiltinOptions};
use crate::ironclaw_support::harness::{
...
use crate::ironclaw_support::product_workflow::IronClawProductWorkflowHarness;
use crate::ironclaw_support::session_thread::IronClawThreadHarness;
use crate::ironclaw_support::test_adapter::IronClawTestIngress;
...
pub struct IronClawBinaryE2EHarness {
    ingress: IronClawTestIngress,
    ...
    _product_harness: IronClawProductWorkflowHarness,
    thread_harness: IronClawThreadHarness,
...
#[derive(Clone)]
pub struct IronClawHarnessSharedStorage {
...
impl IronClawHarnessSharedStorage {
...
impl IronClawBinaryE2EHarness {
    ...
    pub async fn with_model_gateway_scope_shared_storage(
        ...
        capability_port: RecordingTestCapabilityPort,
        scope: ResourceScope,
        shared_storage: IronClawHarnessSharedStorage,
    ) -> HarnessResult<Self> {
    ...
    pub async fn with_host_runtime_file_capabilities(
        ...
        let host_runtime =
            Arc::new(crate::ironclaw_support::harness::profiles::file::file_tools().await?);
    ...
    pub async fn with_host_runtime_file_capabilities_requiring_approval(
        ...
        // `"ironclaw-e2e-builtin-user"`), so the disabled global auto-approve
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::file::file_tools_requiring_approval_profile_for_user(
                subject_user.as_str(),
            )?
            .build()
            .await?,
    ...
    pub async fn with_host_runtime_write_only(
        ...
        let host_runtime =
            Arc::new(crate::ironclaw_support::harness::profiles::file::write_only().await?);
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
            HarnessCapabilityMode::HostRuntime(host_runtime),
    ...
    pub async fn with_host_runtime_coding_read_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::coding_read::coding_read_tools().await?,
        );
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
    ...
    pub async fn with_host_runtime_process_capabilities(
        ...
        let host_runtime =
            Arc::new(crate::ironclaw_support::harness::profiles::process::process_tools().await?);
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
            HarnessCapabilityMode::HostRuntime(host_runtime),
    ...
    pub async fn with_host_runtime_qa_smoke_capabilities(
        ...
        let host_runtime =
            Arc::new(crate::ironclaw_support::harness::profiles::qa_smoke::qa_smoke_tools().await?);
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
            HarnessCapabilityMode::HostRuntime(host_runtime),
    ...
    pub async fn with_host_runtime_extension_lifecycle_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::extension::extension_lifecycle_tools_profile_google_oauth_configured_for_user(
                subject_user.as_str(),
            )?
            .build()
            .await?,
    ...
    async fn resolve_default_binding_subject_user(
        ...
        let ingress = IronClawTestIngress::new("reborn-test", "install-1")?;
        ...
        let product_harness = IronClawProductWorkflowHarness::filesystem_temp(product_scope())?;
    ...
    pub async fn with_host_runtime_skill_management_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::skill::skill_management_tools().await?,
        );
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
    ...
    pub async fn with_host_runtime_trigger_management_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::trigger::trigger_management_tools().await?,
        );
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
    ...
    pub async fn with_host_runtime_trace_commons_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::trace_commons::trace_commons_tools()
                .await?,
        );
        Self::with_model_gateway_capability_mode(
            conversation_id,
    ...
    pub async fn with_host_runtime_github_issue_capabilities(
        ...
        let host_runtime = Arc::new(
            crate::ironclaw_support::harness::profiles::github::github_issue_tools().await?,
        );
        Self::with_model_gateway_capability_mode(
            conversation_id,
            model_gateway,
    ...
    async fn with_model_gateway_capability_mode_identity_source_trigger_storage_and_adapter(
        ...
        shared_storage: Option<IronClawHarnessSharedStorage>,
        adapter_id: &str,
        installation_id: &str,
        initial_actor_id: &str,
    ) -> HarnessResult<Self> {
        let ingress = IronClawTestIngress::new(adapter_id, installation_id)?;
        let product_harness = if let Some(storage) = shared_storage.as_ref() {
            IronClawProductWorkflowHarness::filesystem_shared_backend(
                product_scope.clone(),
                Arc::clone(&storage.product_backend),
                Arc::clone(&storage.product_root),
            )?
        } else {
            IronClawProductWorkflowHarness::filesystem_temp(product_scope)?
        };
        let binding = product_harness
            .binding_service()?
        ...
        let thread_harness = if let Some(storage) = shared_storage.as_ref() {
            IronClawThreadHarness::filesystem_shared_backend(
                thread_scope.clone(),
                Arc::clone(&storage.thread_backend),
            )?
        } else {
            IronClawThreadHarness::filesystem_temp(thread_scope.clone())?
        };
    ...
    fn from_composition(
        ...
        capability_recorder: HarnessCapabilityRecorder,
        milestone_sink: Arc<ironclaw_turns::run_profile::InMemoryLoopHostMilestoneSink>,
        composition: IronClawRuntimeLoopComposition<
...
impl Drop for IronClawBinaryE2EHarness {
    fn drop(&mut self) {
