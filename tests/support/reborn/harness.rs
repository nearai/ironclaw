...
impl HostRuntimeCapabilityHarness {
    ...
    /// `web-access.get_content` capabilities behind `WebAccessTestHandler`
    /// (`harness_web_access.rs`), which calls the same public
    /// `WebAccessExecutor::dispatch` production composition uses. Unlike
