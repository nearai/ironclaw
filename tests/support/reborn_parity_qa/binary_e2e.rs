...
impl RebornBinaryE2EHarness {
    ...
    pub async fn with_host_runtime_extension_lifecycle_capabilities(
        ...
        let subject_user = Self::resolve_default_binding_subject_user(conversation_id).await?;
        // Keep the boot fallback aligned with the seeded Google credential
        // account so callback/refresh paths in this parity tier remain fully
        // configured. Runtime admin-configuration precedence is covered by the
        // composed WebUI caller-path tests.
        let host_runtime = Arc::new(
