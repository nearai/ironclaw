...
fn user_echo_app(auth: MultiAuthState) -> Router {
    ...
    Router::new()
        ...
        .layer(middleware::from_fn_with_state(
            t3claw::channels::web::auth::CombinedAuthState::from(auth),
            auth_middleware,
        ))
}
...
async fn full_server_chat_send_accepts_document_attachment_for_alice() {
    ...
    assert_eq!(
        ...
        t3claw::channels::AttachmentKind::Document
    );
...
async fn start_multi_user_server_with_db() -> (
    ...
    let addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
    let bound =
        t3claw::channels::web::platform::router::start_server(addr, state.clone(), auth.into())
            .await
