//! Ignored live canary for IronClaw first-party GitHub API contracts.
//!
...
//!     cargo test --test ironclaw_live_github_pat_contract -- --ignored
...
async fn ironclaw_live_github_pat_accepts_read_only_schema_calls() {
    let token = std::env::var("LIVE_CANARY_GITHUB_TOKEN")
        .or_else(|_| std::env::var("GITHUB_TOKEN"))
        .expect("set LIVE_CANARY_GITHUB_TOKEN or GITHUB_TOKEN");
    let client = Client::builder()
        .user_agent("IronClaw-IronClaw-GitHub-Live-Contract-Test")
        .build()
