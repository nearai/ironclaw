...
use ironclaw_support::{config::WaitConfig, harness::RecordingTestCapabilityPort};
use ironclaw_turns::TurnStatus;
use parity_qa_support::binary_e2e::{IronClawBinaryE2EHarness, SubmittedTurn};
...
async fn blocking_spawn_waits_while_child_is_blocked_on_approval_then_resumes() {
    ...
    let mut harness = tokio::time::timeout(
        ...
        IronClawBinaryE2EHarness::with_harness_blocked_evidence(
...
fn spawn_call(
    ...
    IronClawScriptedProviderToolCall::new(spawn_capability_id(), call_id, arguments)
}
...
fn subagent_allowed_tool_call(call_id: impl Into<String>) -> IronClawScriptedProviderToolCall {
...
async fn await_single_child(
    harness: &IronClawBinaryE2EHarness,
