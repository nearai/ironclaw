...
#[tokio::test]
async fn ironclaw_trace_file_tools_parity() {
...
#[tokio::test]
async fn ironclaw_trace_file_write_local_dev_approval_gate_bubbles() {
    let write_file = CapabilityId::new(WRITE_FILE_CAPABILITY_ID).expect("valid capability id");
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
    ...
    let mut harness =
        IronClawBinaryE2EHarness::with_host_runtime_file_capabilities_requiring_approval(
