...
fn test_parser_thread_switch() {
    ...
    assert!(matches!(submission, Submission::SwitchThread { thread_id } 
        if thread_id.to_string() == uuid));
}
...
fn test_history_command_variations() {
    ...
    for variation in variations {
        ...
        assert!(
            matches!(submission, Submission::SystemCommand { ref command, ref args } 
                if command == "history" && args.is_empty()),
...
fn test_thread_list_alias_variations() {
    let variations = vec!["/thread list", "/thread LIST", "/thread List", "/THREAD LIST"];
