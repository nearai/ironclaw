...
Python/Playwright test suite that runs against a live t3claw instance. Added in PR #553 ("Trajectory benchmarks and e2e trace test rig").
...
|---------|-------------|
| `t3claw_binary` | Checks `target/debug/t3claw`; if absent, runs `cargo build --no-default-features --features libsql` (timeout 600s). |
...
| `t3claw_server` | Starts the t3claw binary with a minimal env (see below), waits for `/api/health` (timeout 60s). Yields the base URL. On teardown sends **SIGINT** (not SIGTERM) so the tokio ctrl_c handler triggers a graceful shutdown and LLVM coverage data is flushed. |
| `hosted_oauth_refresh_server` | Starts a second t3claw instance with a dedicated libSQL DB and `GOOGLE_OAUTH_CLIENT_ID=hosted-google-client-id`, while still pointing `T3CLAW_OAUTH_EXCHANGE_URL` at `mock_llm.py`. Yields a dict with `base_url`, `db_path`, `gateway_user_id`, and `mock_llm_url` for the hosted refresh regression scenario. |
| `extension_cleanup_server` | Starts an isolated t3claw instance with its own temp DB/home/WASM dirs, `SECRETS_MASTER_KEY`, and hosted-style OAuth env so uninstall-cleanup scenarios can inspect the `secrets` table without interfering with the shared E2E server state. |
...
The function-scoped `page` fixture means **each test gets a clean browser context** (cookies, storage, etc.) but reuses the same t3claw server and browser process. Tests that need the server URL directly (e.g., `test_auth_rejection`) accept `t3claw_server` as an additional parameter.

### Environment passed to t3claw in tests

The `t3claw_server` fixture injects a minimal, deterministic environment:
...
`conftest.py` handles all server startup automatically — you do not need to start t3claw manually before running `pytest`. The conftest builds the binary (libsql feature), starts the mock LLM, and starts t3claw with a fresh temp database on every `pytest` invocation.

If you need to test against a manually started t3claw, you can skip conftest by running pytest with `--co` (collect-only) to understand what would run, or by calling the httpx/REST helpers directly without the `page` fixture.
...
3. Use the `page` fixture for browser tests (function-scoped, fresh context each test). Use `t3claw_server` directly for pure HTTP tests.
...
async def test_my_endpoint(t3claw_server):
    headers = {"Authorization": f"Bearer {AUTH_TOKEN}"}
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{t3claw_server}/api/health", headers=headers)
...
E2E tests run in CI with `cargo-llvm-cov` for coverage collection. The CI workflow (`fix(ci): persist all cargo-llvm-cov env vars for E2E coverage` — PR #559) sets `LLVM_PROFILE_FILE` and related vars before spawning the t3claw binary so coverage from E2E runs is captured.
