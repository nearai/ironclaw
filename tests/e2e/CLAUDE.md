...
- the **legacy `ironclaw-v1` gateway** (`/`, `#auth-screen`, `/api/chat/*`) via the `ironclaw_binary` + `page` fixtures, and
- the **Reborn `ironclaw serve` WebChat v2 SPA** (`/v2/`, `/api/webchat/v2/*`) via the `ironclaw_reborn_binary` + `reborn_v2_server` / `reborn_v2_browser` / `reborn_v2_page` fixtures.
...
scenarios that start the canonical `ironclaw serve` binary and exercise
...
Do not add legacy `ironclaw-v1` gateway tests to that manifest, even if they run
...
| `ironclaw_reborn_binary` | Reborn v2 binary. Builds `target/debug/ironclaw` with `--features webui-v2-beta` (transitively `libsql`) when stale/missing. Used by the v2 SPA scenarios. |
| `reborn_v2_server` | Starts `ironclaw serve` (v2 SPA under `/v2/`, `local-dev` profile) against `mock_llm_server`; config written via `_write_config_toml` (selects the `openai` provider pointed at the mock). Waits for `/api/health`; SIGINT teardown. (Module-scoped, defined in `test_reborn_webui_v2_smoke.py`.) |
