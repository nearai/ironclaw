...
| `reborn_v2_server` | Starts `ironclaw serve` (v2 SPA at `/`, `local-dev` profile) against `mock_llm_server`; config written via `_write_config_toml` (selects the `openai` provider pointed at the mock). The shared launcher opts into the QA-only regression-artifact export routes required by served artifact assertions; production remains default-off. Waits for `/api/health`; SIGINT teardown. (Module-scoped, defined in `reborn_webui_harness.py`.) |
