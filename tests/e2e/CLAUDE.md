...
| `provider_fault_proxy_world` | Starts one transparent aiohttp proxy per resettable Emulate provider. Reborn traffic crosses these proxies; setup and provider readback continue to use direct Emulate URLs. Faults and the safe request ledger reset independently from provider state. |
