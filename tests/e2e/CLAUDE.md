...
| `test_recent_workflow_regressions.py` | User-reported workflows that regressed in the past two weeks: multi-step tool chain through chat UI (PR #3157/#3365), single-tool fetch-then-summarize (PR #3364 http default), mission/routine creation when the goal names an uninstalled tool (PR #3132/#3155/#3166), and approval-card clarity copy (PR #3364/#2991) |
| `test_ceo_onboarding_workflow.py` | Full executive-onboarding live workflow: CEO declaration triggers BOOTSTRAP + ceo-setup skill, meeting-reflection request drives Notion (MCP DCR) + Google Drive (WASM OAuth) install end-to-end, then scan + commitment capture under `projects/commitments/`. Uses the `ceo_workflow_live_server` fixture with `live_llm_proxy.py` record/replay (skipped if the fixture JSON is missing). |
| `test_assistant_executes_not_plans_2544.py` | Behaviour regression for nearai/ironclaw#2544 — "agent plans but never executes". Uses `intent_live_server` (live LLM record/replay) to give the agent a concrete two-step task ("get the time, save it to memory/now.md, tell me what you saved") and asserts the agent actually called the tools, persisted the file, and quoted a real timestamp back. Does NOT assert on the nudge mechanism — the fix may live in the system prompt instead. Re-record after any system-prompt change. |
