"""IronClaw OpenAI-compatible Responses API E2E coverage."""
...
from helpers import IRONCLAW_V2_AUTH_TOKEN, wait_for_ready
...
async def _enable_ironclaw_global_auto_approve(base_url: str) -> None:
...
[webui]
env_token_var = "IRONCLAW_WEBUI_TOKEN"
env_user_id_var = "IRONCLAW_WEBUI_USER_ID"
...
async def ironclaw_responses_server(
    ironclaw_openai_compat_binary, mock_llm_server, tmp_path_factory
):
    """Start `ironclaw serve` with `/v1/responses` mounted."""
    home_dir = tmp_path_factory.mktemp("ironclaw-responses-home")
    ironclaw_home = home_dir / "ironclaw-home"
    ironclaw_home.mkdir(parents=True, exist_ok=True)
    _write_config_toml(ironclaw_home / "config.toml", mock_llm_server)
    ...
    for attempt in range(1, 4):
        ...
        env = {
            ...
            "HOME": str(home_dir),
            "IRONCLAW_HOME": str(ironclaw_home),
            "IRONCLAW_PROFILE": PROFILE,
            "IRONCLAW_WEBUI_TOKEN": IRONCLAW_V2_AUTH_TOKEN,
            "IRONCLAW_WEBUI_USER_ID": USER_ID,
        ...
        try:
            ...
            await _enable_ironclaw_global_auto_approve(base_url)
            break
        ...
        pytest.fail(
            "IronClaw Responses API server failed to start after 3 attempts.\n"
...
@pytest.fixture()
async def ironclaw_responses_client(ironclaw_responses_server):
    async with httpx.AsyncClient(
        base_url=ironclaw_responses_server,
        headers={
            "Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}",
...
async def test_ironclaw_responses_lookup_and_cancel_missing_id_match_not_found_shape(
    ironclaw_responses_client,
):
    retrieve = await ironclaw_responses_client.get("/api/v1/responses/resp_missing")
    cancel = await ironclaw_responses_client.post("/api/v1/responses/resp_missing/cancel")
...
async def test_ironclaw_openai_compat_route_mounts_require_bearer_served(
    ironclaw_responses_server,
):
    routes = [
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        for method, path, body in routes:
            response = await client.request(
                method,
                f"{ironclaw_responses_server}{path}",
                json=body,
            )
            assert response.status_code == 401, (method, path, response.text)


async def test_ironclaw_openai_compat_route_mounts_authenticated_aliases_served(
    ironclaw_responses_client,
):
    ...
    for prefix, response_id in [
        ...
        retrieved = await ironclaw_responses_client.get(f"{prefix}/{response_id}")
        assert retrieved.status_code == 200, (prefix, retrieved.text)
        assert retrieved.json()["id"] == response_id

    models_v1 = await ironclaw_responses_client.get("/v1/models")
    models_api = await ironclaw_responses_client.get("/api/v1/models")
    ...
    cancel_v1 = await ironclaw_responses_client.post(
        "/v1/responses/resp_route_mount_missing/cancel"
    )
    cancel_api = await ironclaw_responses_client.post(
...
async def test_ironclaw_chat_completions_non_streaming_served(ironclaw_responses_client):
...
async def test_ironclaw_chat_completions_idempotency_replay_and_conflict_served(
    ironclaw_responses_client,
):
    payload = {
        "model": "mock-model",
    ...
    first = await ironclaw_responses_client.post(
        "/v1/chat/completions",
        json=payload,
        headers=headers,
    )
    replay = await ironclaw_responses_client.post(
        "/v1/chat/completions",
        json=payload,
        headers=headers,
    )
    conflict = await ironclaw_responses_client.post(
        "/v1/chat/completions",
        json={
            **payload,
...
async def test_ironclaw_chat_completions_streaming_raw_sse_served(
    ironclaw_responses_server,
):
    async with httpx.AsyncClient(timeout=45) as client:
        stream = client.stream(
            "POST",
            f"{ironclaw_responses_server}/v1/chat/completions",
            headers={
...
async def test_ironclaw_chat_completions_auth_and_validation_served(
    ironclaw_responses_server,
    ironclaw_responses_client,
):
    async with httpx.AsyncClient(timeout=10) as client:
    ...
    invalid = await ironclaw_responses_client.post(
        "/v1/chat/completions",
        json={
            "model": "mock-model",
            "messages": [],
...
async def test_ironclaw_models_v1_lists_configured_mock_model(ironclaw_responses_client):
    response = await ironclaw_responses_client.get("/v1/models")
    assert response.status_code == 200, response.text
...
async def test_ironclaw_models_api_v1_alias_matches_v1_models(ironclaw_responses_client):
    v1 = await ironclaw_responses_client.get("/v1/models")
    api_v1 = await ironclaw_responses_client.get("/api/v1/models")
...
async def test_ironclaw_models_requires_auth(ironclaw_responses_server):
    async with httpx.AsyncClient(timeout=10) as client:
        response = await client.get(f"{ironclaw_responses_server}/v1/models")
        alias = await client.get(f"{ironclaw_responses_server}/api/v1/models")

    assert response.status_code == 401
    assert alias.status_code == 401


async def test_ironclaw_responses_repeated_external_tools_round_trip(
    ironclaw_responses_client, mock_llm_server
):
    ...
    assert "IronClaw external tool loop complete" in rendered_output
...
async def test_ironclaw_responses_external_tool_failure_output_reaches_llm(
    ironclaw_responses_client, mock_llm_server
):
    await _reset_mock_chat_requests(mock_llm_server)
    ...
    assert "IronClaw external tool failure observed" in rendered_output
...
async def test_ironclaw_responses_rejects_wrong_external_tool_call_id(
    ironclaw_responses_client,
):
    response = await create_response(
        ironclaw_responses_client,
    ...
    rejected = await ironclaw_responses_client.post(
        "/v1/responses",
        json={
            "model": "mock-model",
            "previous_response_id": response["id"],
...
async def test_ironclaw_responses_mixed_internal_and_external_tools_same_assistant_response(
    ironclaw_responses_client,
    mock_llm_server,
):
    await _reset_mock_chat_requests(mock_llm_server)
