"""Legacy SSE reconnect and history persistence coverage ported to IronClaw v2."""
...
from helpers import IRONCLAW_V2_AUTH_TOKEN, SEL_V2, sse_stream, wait_for_sse_comment
...
async def test_ironclaw_legacy_message_persists_across_page_reload(
...
async def test_ironclaw_legacy_sse_resume_reuses_last_cursor_without_history_reload(
    ironclaw_v2_server, ironclaw_v2_browser
):
    ...
    try:
        ...
        assert query.get("token") == [IRONCLAW_V2_AUTH_TOKEN]
...
async def test_ironclaw_legacy_sse_error_reconnect_resumes_after_last_cursor(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy EventSource reconnect to IronClaw's after_cursor resume path."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
...
async def test_ironclaw_legacy_usage_event_does_not_render_message_badge(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port of legacy turn_cost-event rendering guard to IronClaw's event stream."""
...
async def test_ironclaw_legacy_stale_replay_timeline_refresh_dedupes_messages(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port stale Last-Event-ID duplicate protection to IronClaw timeline refresh."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    timeline_requests: list[str] = []
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{STALE_REPLAY_THREAD_ID}?token={IRONCLAW_V2_AUTH_TOKEN}"
        )
...
async def test_ironclaw_legacy_late_projection_text_dedupes_history_response(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy late response-event dedupe to IronClaw projection text items."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{LATE_TEXT_PROJECTION_THREAD_ID}"
...
async def test_ironclaw_legacy_sse_thread_switch_drops_prior_thread_cursor(
    ironclaw_v2_server, ironclaw_v2_browser
):
    ...
    try:
        await page.goto(f"{ironclaw_v2_server}/chat/{THREAD_A_ID}?token={IRONCLAW_V2_AUTH_TOKEN}")
...
async def test_ironclaw_legacy_multiple_tabs_receive_same_response(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port of legacy multi-tab SSE delivery to IronClaw's per-thread event stream."""
    ...
    try:
        url = f"{ironclaw_v2_server}/chat/{thread_id}?token={IRONCLAW_V2_AUTH_TOKEN}"
...
async def test_ironclaw_legacy_excess_sse_connections_are_rate_limited(
    ironclaw_v2_server,
):
    """Port the legacy SSE connection cap to IronClaw's per-caller stream limit."""
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await _create_thread(client, ironclaw_v2_server)
...
async def test_ironclaw_legacy_sse_keepalive_comments_arrive(ironclaw_v2_server):
    """Port the legacy idle SSE keepalive check to IronClaw's v2 event stream."""
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await _create_thread(client, ironclaw_v2_server)
