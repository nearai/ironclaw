"""Legacy pending-message behavior ported to IronClaw WebChat v2."""
...
async def _open_mocked_pending_page(
    ...
    *,
    ...
    await page.goto(
        f"{ironclaw_v2_server}/chat/{initial_thread_id}?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
...
async def test_ironclaw_legacy_pending_message_visible_while_send_is_in_flight(
...
async def test_ironclaw_legacy_empty_landing_hidden_when_message_pending(
    ironclaw_v2_server, ironclaw_v2_browser
):
    release_send = asyncio.Event()
...
async def test_ironclaw_legacy_send_failure_renders_inline_error_not_toast(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_pending_message_reconciles_with_confirmed_timeline(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_pending_message_survives_thread_reload(
    ironclaw_v2_server, ironclaw_v2_browser
):
    release_send = asyncio.Event()
...
async def test_ironclaw_legacy_pending_attachment_message_survives_thread_reload(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy in-progress attachment reload to IronClaw pending messages."""
    release_send = asyncio.Event()
            ...
            [
                {
                    ...
                    "mimeType": "text/plain",
                    "buffer": b"Attachment survives IronClaw pending reload.",
                }
            ]
        )
...
async def test_ironclaw_legacy_sidebar_cache_keeps_active_thread_outside_summary_window(
    ironclaw_v2_server, ironclaw_v2_browser
):
    async def handle_successful_send(route, _payload, fulfill_json):
        await fulfill_json(route, _submitted_response(), status=202)
    ...
    try:
        ...
        await composer.fill("Summary refresh should keep this IronClaw thread")
        await composer.press("Enter")

        await expect(
            page.locator(SEL_V2["msg_user"]).filter(
                has_text="Summary refresh should keep this IronClaw thread"
            )
        ).to_have_count(1, timeout=5000)
        ...
        assert (
            ...
            == "Summary refresh should keep this IronClaw thread"
        )
        assert await page.evaluate("() => location.pathname") == f"/chat/{THREAD_ID}"
...
async def test_ironclaw_legacy_sidebar_running_indicator_clears_on_terminal_run(
    ironclaw_v2_server, ironclaw_v2_browser
):
    async def handle_successful_send(route, _payload, fulfill_json):
        await fulfill_json(route, _submitted_response(), status=202)
...
async def test_ironclaw_legacy_background_thread_shows_processing_indicator(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port background-thread processing affordance to IronClaw's thread summary state."""
...
async def test_ironclaw_legacy_processing_indicator_does_not_leak_after_thread_switch(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port stale in-progress indicator coverage to IronClaw's thread switch path."""
...
async def test_ironclaw_legacy_failed_send_marks_single_error_message(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_failed_send_retry_resubmits_message(
    ironclaw_v2_server, ironclaw_v2_browser
):
