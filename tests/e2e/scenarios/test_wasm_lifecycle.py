...
async def extension_lifecycle_cleanup(t3claw_server):
    """Start and end the module with a clean extension set."""
    await _ensure_removed(t3claw_server, "web_search")
...
async def web_search_installed(t3claw_server, extension_lifecycle_cleanup):
    """Install web_search once for tests that require the pre-configure state."""
    data = await _install_extension(t3claw_server, "web_search")
    return {"name": "web_search", "install": data}


@pytest.fixture(scope="module")
async def web_search_configured(t3claw_server, web_search_installed):
...
@pytest.fixture(scope="module")
async def gmail_installed(t3claw_server, extension_lifecycle_cleanup):
    """Install gmail once for multi-extension and OAuth setup assertions."""
    data = await _install_extension(t3claw_server, "gmail")
    return {"name": "gmail", "install": data}


@pytest.fixture(scope="module")
async def web_search_removed(t3claw_server, web_search_configured):
    """Remove web_search once for post-uninstall assertions."""
    r = await api_post(
        t3claw_server, "/api/extensions/web_search/remove", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
@pytest.fixture(scope="module")
async def web_search_reinstalled(t3claw_server, web_search_removed):
...
async def test_registry_lists_extensions(t3claw_server):
    """Registry endpoint returns entries from the embedded catalog."""
    r = await api_get(t3claw_server, "/api/extensions/registry")
    assert r.status_code == 200
    data = r.json()
...
async def test_registry_entry_fields(t3claw_server):
...
async def test_registry_installed_flag_false_initially(t3claw_server):
...
async def test_registry_search_filters(t3claw_server):
    """Search query filters registry results."""
    r = await api_get(
        t3claw_server, "/api/extensions/registry", params={"query": "search"}
    )
    assert r.status_code == 200
    entries = r.json()["entries"]
...
async def test_registry_search_no_match(t3claw_server):
...
async def test_installed_extension_fields(t3claw_server, web_search_installed):
    """After install, extension list shows correct fields."""
    ext = await _get_extension(t3claw_server, "web_search")
...
async def test_installed_in_registry(t3claw_server, web_search_installed):
...
async def test_setup_schema_has_secrets(t3claw_server, web_search_installed):
    """Setup schema returns brave_api_key with correct field info."""
    r = await api_get(t3claw_server, "/api/extensions/web_search/setup")
    assert r.status_code == 200
    data = r.json()
...
async def test_extension_not_authenticated_before_configure(
    t3claw_server, web_search_installed
):
...
async def test_activate_before_configure_rejected(t3claw_server, web_search_installed):
    ...
    *some* actionable hint about how to provide the missing credential.
    ...
    """
    r = await api_post(
        t3claw_server, "/api/extensions/web_search/activate", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
async def test_configure_rejects_unknown_secret(t3claw_server, web_search_installed):
...
async def test_extension_active_after_configure(t3claw_server, web_search_configured):
...
async def test_setup_shows_provided(t3claw_server, web_search_configured):
...
async def test_tools_registered_after_activate(
    t3claw_server, web_search_configured
):
    """After activation, extension tools appear in the tools endpoint."""
    r = await api_get(t3claw_server, "/api/extensions/tools")
    assert r.status_code == 200
...
async def test_configure_empty_secret_skipped(t3claw_server, web_search_configured):
    ...
    r2 = await api_get(t3claw_server, "/api/extensions/web_search/setup")
...
async def test_gmail_fields(t3claw_server, gmail_installed):
...
async def test_both_extensions_listed(
    t3claw_server, web_search_configured, gmail_installed
):
...
async def test_gmail_setup_schema_auto_resolves(t3claw_server, gmail_installed):
    """Gmail setup schema returns empty secrets (builtin creds auto-resolve)."""
    r = await api_get(t3claw_server, "/api/extensions/gmail/setup")
    assert r.status_code == 200
    data = r.json()
...
async def test_removed_not_in_extensions(t3claw_server, web_search_removed):
...
async def test_removed_extension_not_listed(t3claw_server, web_search_removed):
...
async def test_removed_not_in_registry_installed(t3claw_server, web_search_removed):
...
async def test_activate_after_remove_uses_replacement_bytes_not_cached_module(
    t3claw_server, wasm_tools_dir, web_search_removed
):
...
async def test_reinstall_after_remove(t3claw_server, web_search_reinstalled):
...
async def test_install_nonexistent(t3claw_server):
...
async def test_install_empty_name(t3claw_server):
...
async def test_remove_noninstalled(t3claw_server):
    """Removing a non-installed extension returns an error."""
    r = await api_post(
        t3claw_server, "/api/extensions/nonexistent-xyz/remove", timeout=10
    )
    if r.status_code == 200:
        assert r.json().get("success") is False
    else:
...
async def test_activate_noninstalled(t3claw_server):
    """Activating a non-installed extension returns an error."""
    r = await api_post(
        t3claw_server, "/api/extensions/nonexistent-xyz/activate", timeout=10
    )
    if r.status_code == 200:
        assert r.json().get("success") is False
    else:
...
async def test_setup_noninstalled(t3claw_server):
    """Setup for non-installed extension returns an error."""
    r = await api_get(t3claw_server, "/api/extensions/nonexistent-xyz/setup")
    # May return 500 or a JSON error
    assert r.status_code >= 400 or r.json().get("success") is False


async def test_configure_noninstalled(t3claw_server):
