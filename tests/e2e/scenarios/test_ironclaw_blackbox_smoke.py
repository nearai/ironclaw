...
The in-process IronClaw integration harness (`tests/integration/`) is the
coverage workhorse for IronClaw behavior, but it cannot prove three things: real
...
functional/UI coverage belongs in `test_ironclaw_webui_v2_smoke.py` and its
...
`ironclaw_webui_harness.py`; see that module for the shared implementation.

Run directly:

    cd tests/e2e && source .venv/bin/activate
    pytest scenarios/test_ironclaw_blackbox_smoke.py -v

Wired into CI as the `blackbox-smoke` job in `.github/workflows/ironclaw-e2e.yml`.
"""
...
async def test_ironclaw_blackbox_boot_health_and_chat_roundtrip(ironclaw_v2_server):
    """`serve` boots, `/api/health` responds, and a scripted chat turn round-trips."""
    async with httpx.AsyncClient() as anon:
        health = await anon.get(f"{ironclaw_v2_server}/api/health", timeout=15)
    ...
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        ...
        reply = await wait_for_assistant_message(client, ironclaw_v2_server, thread_id)

    assert reply["content"] == "The answer is 4.", reply


async def test_ironclaw_blackbox_tool_call_turn_executes_and_replies(ironclaw_v2_yolo_server):
    ...
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        ...
        await send_message(
            client, ironclaw_v2_yolo_server, thread_id, f"reborn builtin echo {marker}"
        )
        ...
        await wait_for_assistant_message(client, ironclaw_v2_yolo_server, thread_id)

    assert preview["status"] == "completed", preview
    assert marker in (preview.get("output_preview") or ""), preview


async def test_ironclaw_blackbox_graceful_restart_preserves_thread_history(
...
async def test_ironclaw_blackbox_kill9_durability(ironclaw_v2_restartable_server):
...
async def test_ironclaw_blackbox_auth_boundary(ironclaw_v2_server):
    ...
    already covered by `test_ironclaw_v2_bearer_auth_and_token_shim_scope` in
    `test_ironclaw_webui_v2_smoke.py`.
    """
    async with httpx.AsyncClient() as anon:
        unauth = await anon.get(f"{ironclaw_v2_server}/api/webchat/v2/session", timeout=15)
    assert unauth.status_code == 401, unauth.text

    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        authed = await client.get(f"{ironclaw_v2_server}/api/webchat/v2/session", timeout=15)
