"""Legacy tool-execution API checks ported to standalone IronClaw WebUI v2."""
...
from ironclaw_webui_harness import (
    ...
    ironclaw_v2_loop_limited_yolo_server,  # noqa: F401 - imported fixture
    ironclaw_v2_server,  # noqa: F401 - imported fixture
    ironclaw_v2_yolo_server,  # noqa: F401 - imported fixture
    send_message,
...
async def _open_mocked_empty_reply_page(ironclaw_v2_server, ironclaw_v2_browser):
    ...
    await page.goto(
        f"{ironclaw_v2_server}/chat/{EMPTY_REPLY_THREAD_ID}?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
...
async def test_ironclaw_legacy_builtin_echo_tool_executes(ironclaw_v2_yolo_server):
    """Port of legacy builtin echo execution to IronClaw's namespaced capability."""
    marker = "reborn tool execution echo 1429"
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
...
async def test_ironclaw_legacy_builtin_time_tool_executes(ironclaw_v2_yolo_server):
    """Port of legacy builtin time execution to IronClaw's namespaced capability."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(
            client,
...
async def test_ironclaw_legacy_non_tool_message_still_works(ironclaw_v2_yolo_server):
    """Port of legacy non-tool chat regression to IronClaw's v2 timeline."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(client, ironclaw_v2_yolo_server, thread_id, "What is 2+2?")
        ...
        timeline = await fetch_timeline(client, ironclaw_v2_yolo_server, thread_id)
...
async def test_ironclaw_legacy_parallel_tool_calls_complete(ironclaw_v2_yolo_server):
    """Port legacy parallel echo+time dispatch to IronClaw capability previews."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(
            client,
...
async def test_ironclaw_legacy_multi_step_tool_chain_completes(ironclaw_v2_yolo_server):
    """Port legacy echo-then-time chain to IronClaw's planned loop path."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(
            client,
...
async def test_ironclaw_legacy_tool_failure_recovers_with_final_response(
    ironclaw_v2_yolo_server,
):
    """Port issue-1780 failed-tool recovery to IronClaw's v2 turn path."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(
            client,
...
async def test_ironclaw_legacy_truncated_tool_call_recovers_without_activity_card(
    ironclaw_v2_yolo_server,
):
    """Port issue-1780 truncated tool-call recovery to IronClaw's v2 turn path."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
        await send_message(
            client,
...
async def test_ironclaw_legacy_empty_reply_failure_projection_is_visible(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy empty-reply recovery to IronClaw's visible failed-run bubble."""
    harness = await _open_mocked_empty_reply_page(ironclaw_v2_server, ironclaw_v2_browser)
    try:
        page = harness["page"]
...
async def test_ironclaw_legacy_looping_tool_calls_stop_at_low_iteration_boundary(
    ironclaw_v2_loop_limited_yolo_server,
):
    """Port issue-1780 looping tool-call termination to IronClaw's v2 turn path."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as client:
        thread_id = await create_thread(client, ironclaw_v2_loop_limited_yolo_server)

        timeout = aiohttp.ClientTimeout(total=60, sock_read=60)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            events_url = (
                f"{ironclaw_v2_loop_limited_yolo_server}"
                f"/api/webchat/v2/threads/{thread_id}/events"
            )
            async with session.get(
                events_url,
                params={"token": IRONCLAW_V2_AUTH_TOKEN},
