...
The in-process Reborn integration harness (`tests/support/reborn/`) is the
...
async def test_reborn_blackbox_kill9_durability(reborn_v2_restartable_server):
    ...
    if shutil.which("pgrep") is not None:
        leaked = subprocess.run(["pgrep", "-P", str(pid)], capture_output=True, text=True)
        assert leaked.returncode != 0, (
            f"kill -9 of pid {pid} left child processes running: {leaked.stdout!r}"
        )
