"""Served IronClaw WebUI v2 extension lifecycle API tests.
...
async def test_ironclaw_v2_extension_lifecycle_served(ironclaw_v2_server):
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        await _remove_web_access_if_present(client, ironclaw_v2_server)

        registry = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/extensions/registry",
            timeout=15,
        )
        ...
        listed = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/extensions",
            timeout=15,
        )
        ...
        install = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/extensions/install",
        ...
        try:
            ...
            setup = await client.get(
                f"{ironclaw_v2_server}/api/webchat/v2/extensions/web-access/setup",
                timeout=15,
            )
            ...
            activate = await client.post(
                f"{ironclaw_v2_server}/api/webchat/v2/extensions/web-access/activate",
                timeout=15,
            )
...
async def test_ironclaw_v2_extension_routes_require_auth_served(ironclaw_v2_server):
...
async def test_ironclaw_v2_extension_routes_reject_invalid_input_served(
    ironclaw_v2_server,
):
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        malformed_package_id = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/extensions/bad%20id/activate",
            timeout=15,
        )
        assert malformed_package_id.status_code == 400

        missing_package = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/extensions/not-installed-extension/setup",
            timeout=15,
        )
