"""Legacy approval-card browser coverage ported to IronClaw WebChat v2."""
...
async def test_ironclaw_legacy_approval_card_renders_details_and_expands_payload(
...
async def test_ironclaw_legacy_approval_buttons_resolve_gate(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_approval_deny_shows_declined_activity(
    ironclaw_v2_server, ironclaw_v2_browser
):
    context, page, resolve_requests = await _open_stubbed_approval_thread(
        ironclaw_v2_server,
...
async def test_ironclaw_legacy_approval_actions_disable_while_resolving(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_pending_approval_blocks_send_without_error_message(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port pending-approval send rejection to IronClaw's local gate blocker."""
    context, page, _resolve_requests = await _open_stubbed_approval_thread(
        ironclaw_v2_server, ironclaw_v2_browser
    )
...
async def test_ironclaw_legacy_pending_approval_does_not_block_other_thread(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port other-thread approval isolation to IronClaw's per-thread gate state."""
    context, page, _resolve_requests = await _open_stubbed_approval_thread(
        ironclaw_v2_server,
        ironclaw_v2_browser,
        thread_records=[
...
async def test_ironclaw_legacy_bare_approval_keywords_send_as_chat_without_gate(
    ironclaw_v2_page,
):
    """Port of the no-pending-gate approval-keyword regression to IronClaw."""
    composer = ironclaw_v2_page.locator(SEL_V2["chat_composer"])

    for keyword in ("yes", "no", "always"):
        user_count = await ironclaw_v2_page.locator(SEL_V2["msg_user"]).count()
        assistant_count = await ironclaw_v2_page.locator(SEL_V2["msg_assistant"]).count()
        ...
        await expect(ironclaw_v2_page.locator(SEL_V2["msg_user"])).to_have_count(
            user_count + 1,
            timeout=10000,
        )
        await expect(ironclaw_v2_page.locator(SEL_V2["msg_user"]).last).to_contain_text(
            keyword
        )
        await expect(ironclaw_v2_page.locator(SEL_V2["msg_assistant"])).to_have_count(
            assistant_count + 1,
            timeout=15000,
        )
        await expect(ironclaw_v2_page.locator(SEL_V2["approval_card"])).to_have_count(0)
