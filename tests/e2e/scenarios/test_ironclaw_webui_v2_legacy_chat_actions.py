"""IronClaw WebChat v2 ports of legacy chat action coverage."""
...
async def test_ironclaw_legacy_message_copy_button_writes_raw_text(ironclaw_v2_page):
    """Port of legacy per-message Copy behavior to IronClaw's message actions."""
...
async def test_ironclaw_legacy_selection_copy_forces_plain_text(ironclaw_v2_page):
    """Port of legacy selected-chat copy behavior to IronClaw's message list."""
    page = ironclaw_v2_page
    composer = page.locator(SEL_V2["chat_composer"])
    await composer.fill("link test")
    await composer.press("Enter")
...
async def test_ironclaw_legacy_command_palette_filters_and_navigates(ironclaw_v2_page):
    """Port the command-discovery affordance to IronClaw's command palette."""
    page = ironclaw_v2_page
