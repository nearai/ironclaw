"""Legacy message-persistence activity-card checks ported to IronClaw WebUI v2."""
...
async def test_ironclaw_legacy_tool_activity_cards_expand_after_reload(
    ironclaw_v2_yolo_page,
):
    """Port of legacy tool-call history card rendering to IronClaw timeline records."""
    marker = "reborn activity persistence marker 8157"
    composer = ironclaw_v2_yolo_page.locator(SEL_V2["chat_composer"])

    await composer.fill(f"reborn builtin echo {marker}")
    await composer.press("Enter")

    await expect(
        ironclaw_v2_yolo_page.locator(SEL_V2["msg_assistant"]).filter(has_text=marker)
    ).to_be_visible(timeout=45000)
    await expect(
        ironclaw_v2_yolo_page.locator(SEL_V2["activity_run"]).first
    ).to_be_visible(timeout=15000)

    await ironclaw_v2_yolo_page.reload(wait_until="load")
    await expect(composer).to_be_visible(timeout=15000)
    await expect(
        ironclaw_v2_yolo_page.locator(SEL_V2["msg_assistant"]).filter(has_text=marker)
    ).to_be_visible(timeout=15000)

    activity_run = ironclaw_v2_yolo_page.locator(SEL_V2["activity_run"]).first
