"""Served IronClaw WebUI v2 project-file and filesystem-browser API tests.
...
from helpers import IRONCLAW_V2_AUTH_TOKEN
...
async def test_ironclaw_v2_project_file_routes_list_stat_and_read_served(
    ironclaw_v2_yolo_server,
):
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await _produce_report_files(client, ironclaw_v2_yolo_server)
        expected_stat = await _wait_for_project_file_stat(
            client, ironclaw_v2_yolo_server, thread_id, CSV_PATH
        )

        listed = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/threads/{thread_id}/files",
            timeout=15,
        )
        ...
        stat = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/threads/{thread_id}/files/stat",
        ...
        content = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/threads/{thread_id}/files/content",
            params={"path": CSV_PATH},
            timeout=15,
        )
...
async def test_ironclaw_v2_filesystem_browser_mounts_list_stat_and_read_served(
    ironclaw_v2_yolo_server,
):
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        mounts = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/fs/mounts",
            timeout=15,
        )
        ...
        listed = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/fs/list",
        ...
        stat = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/fs/stat",
        ...
        content = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/fs/content",
            params={"mount": "workspace", "path": CSV_RELATIVE_PATH},
            timeout=15,
        )
        content.raise_for_status()
...
async def test_ironclaw_v2_filesystem_routes_reject_unauthorized_and_invalid_paths(
    ironclaw_v2_yolo_server,
):
    async with httpx.AsyncClient() as anonymous:
        for path in (
            ...
            response = await anonymous.get(f"{ironclaw_v2_yolo_server}{path}", timeout=15)
            assert response.status_code == 401

        unauthorized_project = await anonymous.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/threads/thread/files",
            timeout=15,
        )
    ...
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await create_thread(client, ironclaw_v2_yolo_server)
