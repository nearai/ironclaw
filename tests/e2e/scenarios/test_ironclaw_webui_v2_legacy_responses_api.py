"""Legacy Responses API coverage ported to standalone IronClaw."""
...
async def ironclaw_openai_compat_server(
    ironclaw_openai_compat_binary,
    mock_llm_server,
    tmp_path_factory,
):
    home_dir = tmp_path_factory.mktemp("ironclaw-openai-compat-home")
    proc, base_url = await start_ironclaw_webui_v2_server(
        ironclaw_binary=ironclaw_openai_compat_binary,
...
@pytest.fixture()
async def ironclaw_responses_client(ironclaw_openai_compat_server):
    async with httpx.AsyncClient(
        base_url=ironclaw_openai_compat_server,
...
async def test_ironclaw_legacy_responses_non_streaming_text_input(
...
async def test_ironclaw_legacy_responses_untyped_message_input_alias(
    ironclaw_responses_client,
):
    response = await _create_response(
        ironclaw_responses_client,
...
async def test_ironclaw_legacy_responses_continue_and_retrieve(
    ironclaw_responses_client,
):
    first = await _create_response(ironclaw_responses_client, input="Say hello")
    ...
    retrieved = await ironclaw_responses_client.get(f"/api/v1/responses/{second['id']}")
...
async def test_ironclaw_legacy_responses_streaming_raw_sse(ironclaw_responses_client):
    async with ironclaw_responses_client.stream(
        "POST",
        "/v1/responses",
...
async def test_ironclaw_legacy_responses_context_injection_approval(
    ironclaw_responses_client,
):
    response = await _create_response(
        ironclaw_responses_client,
...
async def test_ironclaw_legacy_responses_context_injection_rejection(
    ironclaw_responses_client,
):
    response = await _create_response(
        ironclaw_responses_client,
...
async def test_ironclaw_legacy_responses_rejects_missing_auth(
    ironclaw_openai_compat_server,
):
    async with httpx.AsyncClient(timeout=10) as client:
        response = await client.post(
            f"{ironclaw_openai_compat_server}/v1/responses",
...
async def test_ironclaw_legacy_responses_rejects_empty_input_items(
    ironclaw_responses_client,
):
    response = await ironclaw_responses_client.post(
        "/v1/responses",
...
async def test_ironclaw_legacy_responses_rejects_empty_text_input(
    ironclaw_responses_client,
):
    response = await ironclaw_responses_client.post(
        "/v1/responses",
