"""Emulate-backed provider contract tests for IronClaw-capable integrations.

These tests keep the provider fixture layer honest for integrations that
IronClaw already exposes in the codebase. They assert seeded data and real
...
async def test_emulate_google_covers_ironclaw_gsuite_read_inputs(emulate_google_server):
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        assert any(event["summary"] == "IronClaw planning sync" for event in events)
        assert any(
        ...
        assert any(
            item["id"] == "drv_ironclaw_qa_brief"
            and item["name"] == "IronClaw QA Brief"
...
async def test_emulate_google_covers_ironclaw_gsuite_write_outputs(emulate_google_server):
    base_url = emulate_google_server["url"]
    async with httpx.AsyncClient(timeout=10) as client:
        subject = "IronClaw meeting prep summary"
        ...
        created_event_response = await client.post(
            ...
            headers=google_headers(),
            json={
                "summary": "IronClaw created prep follow-up",
                "description": "Created by the IronClaw Emulate provider contract.",
        ...
        assert created_event["summary"] == "IronClaw created prep follow-up"

        listed_events_response = await client.get(
            f"{base_url}/calendar/v3/calendars/primary/events",
            headers=google_headers(),
            params={"q": "IronClaw created prep follow-up"},
        )
        ...
        boundary = "ironclaw-e2e-drive-upload"
        drive_content = "CRM row export generated by the IronClaw Emulate provider contract."
        drive_metadata = {
            "name": "IronClaw CRM Export",
            "mimeType": "text/plain",
            "parents": ["root"],
        }
        multipart_body = (
        ...
        assert drive_file["name"] == "IronClaw CRM Export"
...
async def test_emulate_google_covers_ironclaw_docs_contract(emulate_google_server):
    """Cover create, insert, and read shapes used by harvested Docs traces."""
    base_url = emulate_google_server["url"]
    marker = "IronClaw harvested Google Docs contract"
    async with httpx.AsyncClient(timeout=10) as client:
...
async def test_emulate_google_covers_ironclaw_sheets_contract(emulate_google_server):
    """Cover create, metadata, rename, write, append, and read trace shapes."""
    base_url = emulate_google_server["url"]
    marker = "IronClaw harvested Google Sheets contract"
    async with httpx.AsyncClient(timeout=10) as client:
        created = await client.post(
...
async def test_emulate_slack_covers_ironclaw_delivery_surfaces(emulate_slack_server):
    base_url = emulate_slack_server["url"]
    async with httpx.AsyncClient(timeout=10) as client:
        auth = await slack_post(client, base_url, "auth.test")
        assert auth["team"] == "IronClaw E2E Workspace"
        ...
        text = "IronClaw Emulate Slack delivery contract"
        ...
        thread_reply_text = "Threaded IronClaw follow-up"
        ...
        dm_text = "IronClaw Emulate Slack DM delivery contract"
...
async def test_emulate_slack_covers_ironclaw_search_messages(emulate_slack_server):
...
async def test_emulate_github_covers_ironclaw_repo_surfaces(emulate_github_server):
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        fork = await github_json(
            ...
            payload={"name": "ironclaw-fork"},
            expected_status=202,
        )
        assert fork["full_name"] == "ironclaw-dev/ironclaw-fork"
        ...
        issue_title = "Emulate IronClaw provider contract issue"
        issue = await github_json(
            ...
            payload={
                "title": issue_title,
                "body": "Created by the IronClaw Emulate provider contract test.",
            },
            expected_status=201,
        )
        ...
        issue_search = await github_json(
            ...
            "GET",
            "/search/issues",
            params={"q": "repo:nearai/ironclaw Emulate IronClaw provider contract issue"},
        )
        ...
        content = "IronClaw provider contract git object payload."
        ...
        pr_title = "Emulate IronClaw provider contract PR"
        ...
        merge = await github_json(
            ...
            payload={
                "commit_title": "Merge IronClaw Emulate provider contract PR",
                "merge_method": "squash",
            },
        )
...
async def test_emulate_google_drive_update_roundtrips(emulate_google_server):
    ...
    base_url = emulate_google_server["url"]
    async with httpx.AsyncClient(timeout=10) as client:
        boundary = "ironclaw-e2e-drive-update"
        drive_metadata = {
            "name": "IronClaw QA Update Fixture",
            "mimeType": "text/plain",
            "parents": ["root"],
        }
        multipart_body = (
        ...
        renamed = await client.patch(
            ...
            headers=google_headers(),
            json={"name": "IronClaw QA Brief Updated"},
        )
        renamed.raise_for_status()
        assert renamed.json()["name"] == "IronClaw QA Brief Updated"
        ...
        assert readback.json()["name"] == "IronClaw QA Brief Updated"
