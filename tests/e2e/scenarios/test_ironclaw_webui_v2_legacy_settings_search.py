"""Legacy settings search coverage ported to IronClaw WebChat v2."""
...
async def _open_mocked_settings_page(
    ...
    *,
    ...
    await page.goto(f"{ironclaw_v2_server}/settings/{tab}?token={IRONCLAW_V2_AUTH_TOKEN}")
...
async def test_ironclaw_legacy_settings_tools_search_and_clear(
...
async def test_ironclaw_legacy_settings_skills_search_empty_state(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_settings_page(
        ironclaw_v2_server,
...
async def test_ironclaw_legacy_settings_channels_search(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_settings_page(
        ironclaw_v2_server,
...
async def test_ironclaw_legacy_settings_inference_add_test_and_activate_provider(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_settings_inference_edit_and_delete_custom_provider(
    ironclaw_v2_server, ironclaw_v2_browser
):
    llm_state = _mock_llm_state()
    llm_requests: list[dict] = []
