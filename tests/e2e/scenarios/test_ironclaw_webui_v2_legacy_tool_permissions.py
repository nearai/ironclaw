"""Legacy tool-permission coverage ported to IronClaw WebChat v2."""
...
from ironclaw_webui_harness import (
    ...
    ironclaw_v2_restartable_server,  # noqa: F401 - imported fixture
...
async def _open_mocked_tools_page(
    ...
    *,
    ...
    await page.goto(f"{ironclaw_v2_server}/settings/tools?token={IRONCLAW_V2_AUTH_TOKEN}")
    await expect(
...
@pytest.fixture
def ironclaw_approval_artifact_cleanup():
    yield
    for label in ("first", "second"):
        Path(f"reborn-approval-{label}.txt").unlink(missing_ok=True)


async def _set_real_auto_approve(ironclaw_v2_server: str, enabled: bool):
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        response = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/settings/tools",
...
async def _wait_for_gate_prompt_after_send(
    ...
    async with httpx.AsyncClient(timeout=timeout) as stream_client:
        async with stream_client.stream("GET", url) as response:
            response.raise_for_status()
            async with httpx.AsyncClient(headers=ironclaw_bearer_headers()) as action_client:
...
async def test_ironclaw_legacy_tool_permissions_tab_visible(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_tools_page(ironclaw_v2_server, ironclaw_v2_browser)
    try:
        page = harness["page"]
...
async def test_ironclaw_legacy_tool_permission_menu_persists_after_reload(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_tools_page(ironclaw_v2_server, ironclaw_v2_browser)
    try:
...
async def test_ironclaw_legacy_tool_permission_retains_selection_while_saving(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_legacy_tool_permission_save_failure_shows_error(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_tools_page(
        ironclaw_v2_server,
...
async def test_ironclaw_legacy_locked_tool_shows_badge_without_select(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_tools_page(ironclaw_v2_server, ironclaw_v2_browser)
    try:
...
async def test_ironclaw_legacy_auto_approve_switch_persists(
    ironclaw_v2_server, ironclaw_v2_browser
):
    harness = await _open_mocked_tools_page(ironclaw_v2_server, ironclaw_v2_browser)
    try:
...
async def test_ironclaw_legacy_auto_approve_real_api_persists_across_browser_contexts(
    ironclaw_v2_server,
    ironclaw_v2_browser,
):
    await _set_real_auto_approve(ironclaw_v2_server, False)
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()

    try:
        update = await _set_real_auto_approve(ironclaw_v2_server, True)
        assert update["entry"]["key"] == "agent.auto_approve_tools"
        assert update["entry"]["value"] is True

        await page.goto(
            f"{ironclaw_v2_server}/settings/tools?token={IRONCLAW_V2_AUTH_TOKEN}"
        )
        await expect(
            page.get_by_placeholder(SEL_V2["settings_search_placeholder"])
        ).to_be_visible(timeout=15000)
...
async def test_ironclaw_legacy_tool_permission_real_api_persists_and_rejects_locked(
    ironclaw_v2_server,
):
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        if mutable is None:
            pytest.skip("IronClaw test catalog has no mutable operator tool")

        capability_id = mutable["key"].removeprefix("tool.")
        update = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/settings/tools/{capability_id}",
            json={"state": "disabled"},
            timeout=15,
        )
        ...
        if locked is not None:
            ...
            rejected = await client.post(
                f"{ironclaw_v2_server}/api/webchat/v2/settings/tools/{locked_id}",
...
async def test_ironclaw_legacy_always_approve_survives_ironclaw_restart(
    ironclaw_v2_restartable_server,
    ironclaw_approval_artifact_cleanup,
):
    state, start_server, stop_server = ironclaw_v2_restartable_server
