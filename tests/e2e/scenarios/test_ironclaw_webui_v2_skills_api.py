"""Served IronClaw WebUI v2 skill-management API tests.
...
async def test_ironclaw_v2_skill_lifecycle_served(ironclaw_v2_yolo_server):
    skill_name = f"qa-served-skill-{uuid.uuid4().hex[:8]}"
    headers = ironclaw_bearer_headers()

    async with httpx.AsyncClient(headers=headers) as client:
        initial = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills",
            timeout=15,
        )
        ...
        install = await client.post(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills/install",
            json={
                "name": skill_name,
        ...
        read = await client.get(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills/{skill_name}",
            timeout=15,
        )
        ...
        search = await client.post(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills/search",
        ...
        disable = await client.post(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills/{skill_name}/auto-activate",
            json={"enabled": False},
            timeout=15,
        )
...
async def test_ironclaw_v2_skill_routes_require_auth_and_toggle_global_auto_activate(
    ...
    async with httpx.AsyncClient() as anonymous:
        for method, path in [
            ...
            response = await anonymous.request(
                method,
                f"{ironclaw_v2_yolo_server}{path}",
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        disabled = await client.post(
            f"{ironclaw_v2_yolo_server}/api/webchat/v2/skills/auto-activate-learned",
            json={"enabled": False},
            timeout=15,
        )
...
async def test_ironclaw_v2_skill_routes_reject_invalid_input_served(
    ironclaw_v2_yolo_server,
):
