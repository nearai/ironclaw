...
async def test_pairing_list_requires_auth(t3claw_server):
    """GET /api/pairing/{channel} rejects unauthenticated requests."""
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{t3claw_server}/api/pairing/ownership-test-channel",
            timeout=10,
        )
    assert response.status_code in (401, 403)


async def test_admin_can_access_pairing_list_endpoint(t3claw_server):
    """Admins can access the pairing list endpoint even when the channel has no requests."""
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{t3claw_server}/api/pairing/nonexistent-channel",
...
async def test_member_cannot_access_pairing_list_endpoint(t3claw_server):
    """Members receive 403 when attempting to enumerate pending pairing codes."""
    member = await create_member_user(t3claw_server)
...
async def test_pairing_approve_requires_auth(t3claw_server):
    """POST /api/pairing/{channel}/approve rejects unauthenticated requests."""
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{t3claw_server}/api/pairing/test-channel/approve",
            json={"code": "ABCD1234"},
            timeout=10,
        )
    assert response.status_code in (401, 403)
...
async def test_member_invalid_code_returns_expected_failure_shape(t3claw_server):
...
async def test_member_blank_code_rejected_cleanly(t3claw_server):
...
async def test_member_lowercase_invalid_code_does_not_500(t3claw_server):
