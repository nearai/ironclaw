"""Full-path IronClaw extension tests backed by Emulate provider state."""
...
from ironclaw_emulate_harness import (
...
async def test_ironclaw_google_calendar_lifecycle_mutates_emulate(
...
async def test_ironclaw_github_issue_lifecycle_mutates_emulate(
...
async def test_ironclaw_google_drive_lifecycle_mutates_emulate(
    hosted_google_emulate_server,
):
    server = hosted_google_emulate_server["base_url"]
    emulate_google_url = hosted_google_emulate_server["emulate_google_url"]
...
async def test_ironclaw_gmail_lifecycle_mutates_emulate(
    hosted_google_emulate_server,
):
...
async def test_ironclaw_slack_delivery_and_github_release_cross_provider_path(
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        await github_json(
            ...
            payload={
                ...
                "name": "IronClaw cross-provider canary",
