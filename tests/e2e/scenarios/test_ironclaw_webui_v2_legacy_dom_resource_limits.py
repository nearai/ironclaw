"""Legacy DOM resource-limit coverage ported to IronClaw WebUI v2 history paging."""
...
async def test_ironclaw_chat_history_dom_stays_page_bounded_until_user_loads_more(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """IronClaw replaces legacy DOM pruning with explicit 50-message timeline paging."""
...
async def test_ironclaw_response_projection_survives_full_history_page(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy near-DOM-cap response integrity to IronClaw projection updates."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    timeline_queries: list[dict[str, list[str]]] = []
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{FULL_PAGE_THREAD_ID}?token={IRONCLAW_V2_AUTH_TOKEN}"
        )
...
async def test_ironclaw_user_send_survives_full_history_page_without_loading_older(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy near-DOM-cap user-send preservation to IronClaw history paging."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    timeline_queries: list[dict[str, list[str]]] = []
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{FULL_PAGE_SEND_THREAD_ID}"
...
async def test_ironclaw_sse_reconnect_timer_clears_when_tab_hidden(
    ironclaw_v2_server, ironclaw_v2_browser
):
    """Port legacy reconnect timer cleanup to IronClaw's visibility pause path."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{RECONNECT_THREAD_ID}?token={IRONCLAW_V2_AUTH_TOKEN}"
        )
