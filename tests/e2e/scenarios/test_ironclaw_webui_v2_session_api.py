"""Served IronClaw WebUI v2 session/thread/message API matrix tests.
...
`test_ironclaw_webui_v2_smoke.py`, which is already owned by the normal IronClaw
...
async def test_ironclaw_v2_session_and_thread_lifecycle_served(ironclaw_v2_server):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        first_thread_id = await create_thread(client, ironclaw_v2_server)
        second_thread_id = await create_thread(client, ironclaw_v2_server)
        assert first_thread_id != second_thread_id

        first_page = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/threads?limit=1",
            timeout=15,
        )
        ...
        second_page = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/threads",
        ...
        delete_response = await client.delete(
            f"{ironclaw_v2_server}/api/webchat/v2/threads/{first_thread_id}",
            timeout=15,
        )
        delete_response.raise_for_status()

        after_delete = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/threads?limit=20",
            timeout=15,
        )
...
async def test_ironclaw_v2_message_submission_and_timeline_served(ironclaw_v2_server):
    headers = ironclaw_bearer_headers()
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await create_thread(client, ironclaw_v2_server)

        await send_message(client, ironclaw_v2_server, thread_id, "what is 2+2?")
        assistant = await wait_for_assistant_message(client, ironclaw_v2_server, thread_id)
        assert "4" in assistant.get("content", "")

        timeline = await fetch_timeline(client, ironclaw_v2_server, thread_id)
...
async def test_ironclaw_v2_session_thread_api_rejects_bad_requests(ironclaw_v2_server):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        unknown_timeline = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/threads/not-a-thread/timeline",
            timeout=15,
        )
        ...
        empty_message = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/threads/{thread_id}/messages",
