"""IronClaw WebChat v2 ports of legacy browser attachment coverage."""
...
from ironclaw_webui_harness import (
    ...
    ironclaw_v2_vision_page,  # noqa: F401 - imported fixture
    ironclaw_v2_vision_server,  # noqa: F401 - imported fixture dependency
)
...
async def test_ironclaw_legacy_attachment_flow_renders_and_reaches_model(
    ironclaw_v2_page,
):
...
async def test_ironclaw_legacy_attachment_document_extraction_reaches_model(
    ironclaw_v2_vision_page,
    mock_llm_server,
):
    """Port of legacy PDF/text/PPTX/image model-payload attachment assertions."""
    page = ironclaw_v2_vision_page
    await page.set_input_files(
        SEL_V2["attachment_file_input"],
        files=[
            {
            ...
            },
            {
                "name": "notes.txt",
                "mimeType": "text/plain",
                "buffer": b"Quarterly roadmap notes\nShip the IronClaw attachment flow.",
            },
            {
                "name": "roadmap.pptx",
                "mimeType": "application/vnd.openxmlformats-officedocument.presentationml.presentation",
                "buffer": _make_test_pptx("IronClaw attachment roadmap slide"),
            },
        ],
    )

    composer = page.locator(SEL_V2["chat_composer"])
    await composer.fill("Please review these IronClaw attachments.")
    await composer.press("Enter")

    user_message = page.locator(SEL_V2["msg_user"]).last
    await expect(user_message).to_contain_text(
        "Please review these IronClaw attachments.", timeout=15000
    )
    ...
    payload = await _wait_for_mock_llm_request_contains(
        mock_llm_server,
        [
            "Please review these IronClaw attachments.",
            "hello.pdf",
            "Quarterly roadmap notes",
            "Ship the IronClaw attachment flow.",
            "IronClaw attachment roadmap slide",
        ],
        timeout=45.0,
    )
...
async def test_ironclaw_legacy_unextractable_attachment_uses_placeholder(
    ironclaw_v2_page,
    mock_llm_server,
):
    """Port of corrupt-PDF extraction fallback to IronClaw's attachment marker."""
    page = ironclaw_v2_page
    ...
    composer = page.locator(SEL_V2["chat_composer"])
    await composer.fill("Please inspect this IronClaw binary attachment.")
    await composer.press("Enter")

    user_message = page.locator(SEL_V2["msg_user"]).last
    await expect(user_message).to_contain_text(
        "Please inspect this IronClaw binary attachment.", timeout=15000
    )
    ...
    payload = await _wait_for_mock_llm_request_contains(
        mock_llm_server,
        [
            "Please inspect this IronClaw binary attachment.",
            "mystery.pdf",
...
async def test_ironclaw_legacy_files_only_attachments_reload_from_history(
    ironclaw_v2_page,
):
...
async def test_ironclaw_legacy_attachment_count_limit_blocks_extra_files(
    ironclaw_v2_page,
):
    """Port of legacy batch count validation to IronClaw's staging alert UI."""
    page = ironclaw_v2_page
    await page.set_input_files(
        SEL_V2["attachment_file_input"],
        files=[
...
async def test_ironclaw_legacy_attachment_size_limits_block_invalid_files(
    ironclaw_v2_page,
):
...
async def test_ironclaw_legacy_attachment_unsupported_type_is_rejected(
    ironclaw_v2_page,
):
    """Port of legacy attachment type rejection through IronClaw's accept contract."""
    page = ironclaw_v2_page
    await page.set_input_files(
        SEL_V2["attachment_file_input"],
        files=[
