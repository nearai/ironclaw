"""Served IronClaw WebUI v2 product-auth API tests.
...
async def test_ironclaw_v2_product_auth_routes_require_auth_served(ironclaw_v2_server):
...
async def test_ironclaw_v2_product_auth_manual_token_setup_redacts_submit_error_served(
    ironclaw_v2_server,
):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        setup = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/manual-token/setup",
            json={
                "provider": "github",
        ...
        submit = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/manual-token/secret-submit",
            json={
...
async def test_ironclaw_v2_product_auth_accounts_and_recovery_served(
    ironclaw_v2_server,
):
    headers = ironclaw_bearer_headers()
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        listed = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/accounts/list",
        ...
        recovery = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/accounts/recovery",
            json={"provider": "github", "invocation_id": invocation_id},
            timeout=15,
        )
...
async def test_ironclaw_v2_product_auth_routes_reject_invalid_input_served(
    ironclaw_v2_server,
):
    headers = ironclaw_bearer_headers()
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        malformed_invocation = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/accounts/select",
            json={
                "provider": "github",
        ...
        malformed_account = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/accounts/refresh",
            json={
                "provider": "github",
        ...
        invalid_extension_id = await client.post(
            f"{ironclaw_v2_server}/api/reborn/product-auth/lifecycle/cleanup",
            json={
                "extension_id": "../bad",
                "action": "deactivate",
                "invocation_id": _invocation_id(),
