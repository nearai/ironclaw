"""IronClaw WebChat v2: agent-produced files are downloadable from the UI.

Drives the real `ironclaw serve` binary (v2 SPA) under the
...
turns the prompt into two IronClaw `builtin.write_file` capability calls via the
...
from ironclaw_webui_harness import (
    ...
    ironclaw_v2_yolo_page,  # noqa: F401 - imported fixture
    ironclaw_v2_yolo_server,  # noqa: F401 - imported fixture dependency
)
...
async def test_ironclaw_v2_agent_files_render_download_chips(ironclaw_v2_yolo_page):
    """Agent writes a CSV + PDF; the reply renders chips that download the bytes."""
    page = ironclaw_v2_yolo_page
...
async def test_workspace_viewer_reports_download_failure(ironclaw_v2_yolo_page):
