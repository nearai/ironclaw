"""IronClaw WebUI v2 notification center E2E coverage."""
...
async def test_ironclaw_v2_notification_popover_opens_automation_thread(
    ironclaw_v2_server,
    ironclaw_v2_browser,
):
    ...
    try:
        ...
        await _open_v2(page, ironclaw_v2_server)
...
async def test_ironclaw_v2_notification_open_marks_read_without_hiding_pending_message(
    ironclaw_v2_server,
    ironclaw_v2_browser,
):
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
...
async def test_ironclaw_v2_notification_drawer_and_header_actions_fit_mobile(
    ironclaw_v2_server,
    ironclaw_v2_browser,
):
    viewport = {"width": 390, "height": 740}
    context = await ironclaw_v2_browser.new_context(viewport=viewport)
    page = await context.new_page()
    try:
        await _route_notification_threads(page)
        await _open_v2(page, ironclaw_v2_server, "/settings/language")
...
async def test_ironclaw_v2_error_toast_pauses_dismisses_and_stays_above_notifications(
    ironclaw_v2_server,
    ironclaw_v2_browser,
):
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
