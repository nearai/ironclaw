"""Dedicated IronClaw WebChat v2 smoke E2E.

This proves the *new* IronClaw surface end-to-end: the `ironclaw serve`
...
async def test_ironclaw_v2_serves_shell_and_gates_auth(ironclaw_v2_server, ironclaw_v2_browser):
    ...
    authed_ctx = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    authed_page = await authed_ctx.new_page()
    try:
        await authed_page.goto(f"{ironclaw_v2_server}/?token={IRONCLAW_V2_AUTH_TOKEN}")
    ...
    anon_ctx = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    anon_page = await anon_ctx.new_page()
    try:
        await anon_page.goto(f"{ironclaw_v2_server}/")
...
async def test_ironclaw_v2_session_check_failure_blocks_app_and_retries(
    ironclaw_v2_page,
):
            ...
            body=json.dumps(
                {
                    ...
                    "user_id": USER_ID,
                    "capabilities": {},
                    "features": {"ironclaw_projects": False},
                    "attachments": {
                        "accept": ["text/plain"],
    ...
    await ironclaw_v2_page.route("**/api/webchat/v2/session", handle_session)
    await ironclaw_v2_page.reload()

    error = ironclaw_v2_page.locator(SEL_V2["session_check_error"])
    ...
    await expect(ironclaw_v2_page.locator(SEL_V2["chat_composer"])).to_have_count(0)
    await expect(ironclaw_v2_page.locator(SEL_V2["login_token"])).to_have_count(0)
    assert await ironclaw_v2_page.evaluate(
        "() => sessionStorage.getItem('ironclaw_token')"
    ) == IRONCLAW_V2_AUTH_TOKEN
    assert session_requests == 1

    await ironclaw_v2_page.locator(SEL_V2["session_check_retry"]).click()
    await expect(ironclaw_v2_page.locator(SEL_V2["chat_composer"])).to_be_visible(
        timeout=15000
    )
...
async def test_ironclaw_v2_session_check_failure_allows_sign_out(
    ironclaw_v2_page,
):
    ...
    await ironclaw_v2_page.route("**/api/webchat/v2/session", fail_session_check)
    await ironclaw_v2_page.route("**/auth/logout", handle_logout)
    await ironclaw_v2_page.reload()

    await expect(
        ironclaw_v2_page.locator(SEL_V2["session_check_error"])
    ).to_be_visible(timeout=15000)

    await ironclaw_v2_page.locator(SEL_V2["session_check_sign_out"]).click()

    await expect(ironclaw_v2_page.locator(SEL_V2["login_token"])).to_be_visible(
        timeout=15000
    )
    await ironclaw_v2_page.wait_for_url(
...
async def test_ironclaw_v2_legacy_paths_redirect_to_root(
    ironclaw_v2_server, ironclaw_v2_browser
):
    ...
    async with httpx.AsyncClient(follow_redirects=False) as client:
        for source, target in [
            ...
            response = await client.get(f"{ironclaw_v2_server}{source}")
    ...
    context = await ironclaw_v2_browser.new_context(
        viewport={"width": 1280, "height": 720}
    )
    page = await context.new_page()
    try:
        await page.goto(
            f"{ironclaw_v2_server}/v2/settings/skills"
            f"?token={IRONCLAW_V2_AUTH_TOKEN}&source=compat"
        )
...
async def test_ironclaw_v2_light_theme_semantic_colors_have_readable_contrast(
    ironclaw_v2_page,
):
    """Theme-aware controls, success states, and secondary text meet WCAG AA."""
    await ironclaw_v2_page.evaluate(
        """() => {
    ...
    assert await ironclaw_v2_page.locator("html").get_attribute("data-theme") == "light"

    # A slow turn exposes the real danger Button used to cancel an active run.
    composer = ironclaw_v2_page.locator(SEL_V2["chat_composer"])
    await composer.fill("editable composer slow response")
    await composer.press("Enter")
    user_message = ironclaw_v2_page.locator(SEL_V2["msg_user"]).last
    await expect(user_message).to_contain_text("editable composer slow response", timeout=15000)
    cancel_button = ironclaw_v2_page.get_by_role("button", name="Cancel").first
    ...
    origin = await ironclaw_v2_page.evaluate("location.origin")
    await ironclaw_v2_page.goto(
        f"{origin}/extensions/registry?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
    install_button = ironclaw_v2_page.get_by_role("button", name="Install").first
    ...
    await ironclaw_v2_page.mouse.down()
    try:
        ...
        await ironclaw_v2_page.mouse.move(0, 0)
        await ironclaw_v2_page.mouse.up()
    ...
    await ironclaw_v2_page.goto(
        f"{origin}/settings/skills?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
    toggle_name = re.compile(r"^Default: (On|Off)$")
    toggle = ironclaw_v2_page.get_by_role("button", name=toggle_name).first
    await expect(toggle).to_be_visible(timeout=15000)
    original_label = await toggle.inner_text()
    restore_label = "Default: Off" if original_label == "Default: On" else "Default: On"
    await toggle.click()
    try:
        restore_toggle = ironclaw_v2_page.get_by_role("button", name=restore_label).first
        await expect(restore_toggle).to_be_visible(timeout=15000)
        success_banner = ironclaw_v2_page.locator(SEL_V2["skill_action_result"])
        ...
        if await restore_toggle.count():
            ...
            await expect(
                ironclaw_v2_page.get_by_role("button", name=original_label).first
            ).to_be_visible(timeout=15000)


async def test_ironclaw_v2_appearance_theme_selection_persists(ironclaw_v2_page):
    """Appearance controls update the live theme and preserve it across reloads."""
    origin = await ironclaw_v2_page.evaluate("location.origin")
    await ironclaw_v2_page.goto(
        f"{origin}/v2/settings/appearance?token={IRONCLAW_V2_AUTH_TOKEN}"
    )

    light_option = ironclaw_v2_page.locator(SEL_V2["appearance_theme_light"])
    dark_option = ironclaw_v2_page.locator(SEL_V2["appearance_theme_dark"])
    ...
    await expect(ironclaw_v2_page.locator("html")).to_have_attribute(
        "data-theme", "dark"
    )
    await ironclaw_v2_page.wait_for_function(
...
async def test_ironclaw_v2_settings_import_rejects_unsupported_payloads(
    ironclaw_v2_page,
):
    ...
    ironclaw_v2_page.on("request", count_settings_reads)
    await ironclaw_v2_page.keyboard.press("Control+K")
    command_palette = ironclaw_v2_page.get_by_role(
    ...
    file_input = ironclaw_v2_page.locator(SEL_V2["settings_import_file"])
    await expect(file_input).to_have_count(1, timeout=15000)
    await ironclaw_v2_page.wait_for_timeout(250)
    ...
    for filename, settings in [
        ...
        status = ironclaw_v2_page.get_by_role("status").filter(
        ...
        await expect(
            ironclaw_v2_page.get_by_text("Settings imported", exact=True)
        ).to_have_count(0)
        await expect(
            ironclaw_v2_page.get_by_text(re.compile(r"^Import failed:"))
        ).to_have_count(0)
...
async def test_ironclaw_v2_text_turn_persists(ironclaw_v2_server):
    """A text turn over /api/webchat/v2/* completes and persists one assistant reply."""
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        thread_id = await _create_thread(client, ironclaw_v2_server)

        prompt = "what is 2+2?"
        await _send_message(client, ironclaw_v2_server, thread_id, prompt)
        assistant = await _wait_for_assistant_message(client, ironclaw_v2_server, thread_id)
...
async def test_ironclaw_v2_ui_enter_submits_initial_and_follow_up_messages(
    ironclaw_v2_page,
):
    ...
    user_messages = ironclaw_v2_page.locator(SEL_V2["msg_user"])
    assistant_messages = ironclaw_v2_page.locator(SEL_V2["msg_assistant"])
...
async def test_ironclaw_v2_automation_rename_persists_from_ui(
    ironclaw_v2_server, ironclaw_v2_browser
):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        await _wait_for_assistant_message(client, ironclaw_v2_server, thread_id)
        automation = await _wait_for_automation_named(
            client, ironclaw_v2_server, original_name
        )
        automation_id = automation["automation_id"]

    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    try:
        await page.goto(f"{ironclaw_v2_server}/automations?token={IRONCLAW_V2_AUTH_TOKEN}")
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        renamed = await _wait_for_automation_named(client, ironclaw_v2_server, renamed_name)
        assert renamed["automation_id"] == automation_id


async def test_ironclaw_v2_automation_action_error_toast_is_safe_dismissible_and_cleared_on_retry(
    ironclaw_v2_server, ironclaw_v2_page
):
...
async def test_ironclaw_v2_automation_failed_run_actions_are_clickable(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_v2_composer_accepts_draft_while_run_is_processing(ironclaw_v2_page):
    """The composer stays editable while the current assistant run is still active."""
    composer = ironclaw_v2_page.locator(SEL_V2["chat_composer"])
    await composer.fill("editable composer slow response")
    await composer.press("Enter")

    await expect(ironclaw_v2_page.locator(SEL_V2["msg_user"]).first).to_contain_text(
        "editable composer slow response", timeout=15000
    )
    await expect(
        ironclaw_v2_page.locator(SEL_V2["typing_indicator"])
    ).to_be_visible(timeout=15000)
    ...
    await composer.press("Enter")
    await expect(ironclaw_v2_page.locator(SEL_V2["msg_user"])).to_have_count(1, timeout=1000)


async def test_ironclaw_v2_disconnected_run_shows_status_and_stops_typing(
    ironclaw_v2_server, ironclaw_v2_browser
) -> None:
    ...
    try:
        await page.goto(f"{ironclaw_v2_server}/chat/{thread_id}?token={IRONCLAW_V2_AUTH_TOKEN}")
...
async def test_ironclaw_v2_approval_gate_blocks_composer_send(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_v2_unscoped_activity_stays_with_previous_reply(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_v2_desktop_sidebar_can_collapse_and_persist(ironclaw_v2_page):
    """Desktop users can collapse the sidebar, and the preference survives reload."""
    sidebar = ironclaw_v2_page.locator(SEL_V2["sidebar"])
    toggle = ironclaw_v2_page.locator(SEL_V2["sidebar_toggle"])
...
async def test_ironclaw_v2_messages_omit_identity_labels(ironclaw_v2_page):
    ...
    user_bubble = ironclaw_v2_page.locator(SEL_V2["msg_user"]).first
    await expect(user_bubble).to_contain_text("hello there", timeout=15000)
    await expect(user_bubble).not_to_contain_text("You")

    assistant_bubble = ironclaw_v2_page.locator(SEL_V2["msg_assistant"]).first
...
async def test_ironclaw_v2_response_links_open_in_new_tab(ironclaw_v2_page):
    ...
    link = (
        ironclaw_v2_page.locator(SEL_V2["msg_assistant"])
...
async def test_ironclaw_v2_logs_page_passes_scope_to_api_and_renders_context(
    ...
    await ironclaw_v2_page.route("**/api/webchat/v2/operator/logs**", handle_operator_logs)
    await ironclaw_v2_page.goto(
        f"{ironclaw_v2_server}/logs"
    ...
    await expect(
        ironclaw_v2_page.locator(SEL_V2["logs_scope_toolbar"])
    ).to_be_visible(timeout=10000)
    await expect(
        ironclaw_v2_page.locator(SEL_V2["logs_scope_chip"].format(key="thread_id"))
    ).to_contain_text("thread-ui")
    await expect(
        ironclaw_v2_page.locator(SEL_V2["logs_scope_chip"].format(key="run_id"))
    ).to_contain_text("run-ui")

    entry = ironclaw_v2_page.locator(SEL_V2["logs_entry"]).first
...
async def test_ironclaw_v2_logs_deep_link_loads_scoped_conversation_on_first_open(
    ironclaw_v2_server, ironclaw_v2_browser
):
...
async def test_ironclaw_v2_thread_list_and_delete(ironclaw_v2_server):
    """Threads are listed for the caller and deletion removes the thread and transcript."""
    headers = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=headers) as client:
        keep_id = await _create_thread(client, ironclaw_v2_server)
        drop_id = await _create_thread(client, ironclaw_v2_server)

        listed = await client.get(f"{ironclaw_v2_server}/api/webchat/v2/threads", timeout=15)
        ...
        deleted = await client.request(
            "DELETE", f"{ironclaw_v2_server}/api/webchat/v2/threads/{drop_id}", timeout=15
        )
        assert deleted.status_code == 200, deleted.text

        # Transcript is gone (404, not an empty timeline) and re-delete is idempotent-404.
        gone = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/threads/{drop_id}/timeline", timeout=15
        )
        ...
        relisted = await client.get(f"{ironclaw_v2_server}/api/webchat/v2/threads", timeout=15)
...
async def test_ironclaw_v2_sidebar_loads_older_thread_pages(ironclaw_v2_page):
...
async def test_ironclaw_v2_thread_delete_uses_shared_confirmation_dialog(
    ironclaw_v2_server, ironclaw_v2_page
):
    ...
    ironclaw_v2_page.on("dialog", dismiss_native_dialog)
    await ironclaw_v2_page.goto(
        f"{ironclaw_v2_server}/chat?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
    delete_button = ironclaw_v2_page.locator(
        SEL_V2["thread_delete_for"].format(id=thread_id)
    )
    await expect(delete_button).to_be_visible(timeout=15000)

    await delete_button.click()
    confirmation = ironclaw_v2_page.get_by_role("dialog", name="Delete chat")
...
async def test_ironclaw_v2_ui_delete_removes_sidebar_thread_without_refetch(
    ironclaw_v2_server, ironclaw_v2_page
):
    ...
    try:
        await page.goto(f"{ironclaw_v2_server}/?token={IRONCLAW_V2_AUTH_TOKEN}")
        await expect(page.locator(SEL_V2["chat_composer"])).to_be_visible(timeout=15000)
...
async def test_ironclaw_v2_timeline_pagination(ironclaw_v2_server):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        ...
        await _send_and_settle(client, ironclaw_v2_server, thread_id, "hello one", expected=1)
        await _send_and_settle(client, ironclaw_v2_server, thread_id, "hello two", expected=2)
...
async def test_ironclaw_v2_loading_older_messages_preserves_viewport(
    ironclaw_v2_server, ironclaw_v2_browser
):
    ...
    try:
        await page.goto(
            f"{ironclaw_v2_server}/chat/{thread_id}?token={IRONCLAW_V2_AUTH_TOKEN}"
        )
...
async def test_ironclaw_v2_sse_streams_run_lifecycle(ironclaw_v2_server):
    ...
    """
    bearer = {"Authorization": f"Bearer {IRONCLAW_V2_AUTH_TOKEN}"}
    async with httpx.AsyncClient(headers=bearer) as client:
        thread_id = await _create_thread(client, ironclaw_v2_server)

    events_url = (
        f"{ironclaw_v2_server}/api/webchat/v2/threads/{thread_id}/events"
        f"?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
    ...
    async with aiohttp.ClientSession(timeout=client_timeout) as session:
        ...
        async with session.get(
            ...
            async with httpx.AsyncClient(headers=bearer) as client:
                await _send_message(client, ironclaw_v2_server, thread_id, "hello sse")
...
async def test_ironclaw_v2_bearer_auth_and_token_shim_scope(ironclaw_v2_server):
