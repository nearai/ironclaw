"""Stubbed-browser WebUI v2 Settings provider-login matrix.

These tests drive the committed Settings -> Inference provider cards in
Chromium while stubbing the v2 provider-login endpoints. They cover the
browser-only REBCLI-097 provider-login rows without live NEAR AI or Codex
provider calls.
"""
...
class ProviderLoginStub:
    def __init__(
        self,
        *,
        nearai_status: int = 200,
        codex_status: int = 200,
    ):
        self.nearai_status = nearai_status
    ...
    async def _session(self, route) -> None:
        await self._fulfill(
            route,
            {
                ...
                "user_id": "provider-login-browser-user",
    ...
    async def _providers(self, route) -> None:
        ...
        await self._fulfill(
            route,
            {
                ...
                "active": {"provider_id": "openai", "model": "gpt-4o"},
            },
        )
    ...
    async def _nearai_login(self, route) -> None:
        ...
        if self.nearai_status >= 400:
            await self._fulfill(
                route,
                {"error": "nearai_login_unavailable"},
                status=self.nearai_status,
            )
            return
...
async def _open_public_settings_page(reborn_v2_server, stub: ProviderLoginStub):
    ...
    await page.goto(
        f"{public_base}/v2/settings/inference?token={REBORN_V2_AUTH_TOKEN}",
        wait_until="domcontentloaded",
    )
    await expect(page).to_have_url(
        re.compile(r"app\.example\.test:\d+/v2/settings/inference/?$"),
        timeout=15000,
    )
    await expect(
...
async def test_settings_provider_login_browser_starts_nearai_and_codex_flows(
    ...
    stub = ProviderLoginStub()
    playwright, browser, context, page, public_base = await _open_public_settings_page(
        reborn_v2_server,
        stub,
    )
    try:
        await page.locator(SEL_V2["llm_provider_nearai_github_login"]).click()
    ...
    stub = ProviderLoginStub()
    playwright, browser, context, page, _public_base = await _open_public_settings_page(
        reborn_v2_server,
        stub,
    )
    try:
...
async def test_settings_provider_login_browser_surfaces_codex_start_failure(
    reborn_v2_server,
):
    stub = ProviderLoginStub(codex_status=503)
    playwright, browser, context, page, _public_base = await _open_public_settings_page(
        reborn_v2_server,
        stub,
    )
    try:
        await page.locator(SEL_V2["llm_provider_codex_login"]).click()
        await expect(page.locator(SEL_V2["llm_provider_codex_error"])).to_contain_text(
            "ChatGPT sign-in failed. Please try again.",
            timeout=10000,
        )
        await expect(page.locator(SEL_V2["llm_provider_codex_code"])).to_have_count(0)
        await expect(page.locator(SEL_V2["llm_provider_codex_waiting"])).to_have_count(0)
        assert stub.codex_requests == [
            {
                "authorization": f"Bearer {REBORN_V2_AUTH_TOKEN}",
                "body": "",
...
async def test_settings_provider_login_browser_surfaces_nearai_start_failure(
    reborn_v2_server,
):
    stub = ProviderLoginStub(nearai_status=503)
    playwright, browser, context, page, public_base = await _open_public_settings_page(
        reborn_v2_server,
        stub,
    )
    try:
        await page.locator(SEL_V2["llm_provider_nearai_github_login"]).click()
        await expect(page.locator(SEL_V2["llm_provider_nearai_error"])).to_contain_text(
            "NEAR AI sign-in failed. Please try again.",
            timeout=10000,
        )
        await expect(page.locator(SEL_V2["llm_provider_nearai_waiting"])).to_have_count(0)
        assert stub.nearai_requests == [
            {
                "authorization": f"Bearer {REBORN_V2_AUTH_TOKEN}",
                "body": {"provider": "github", "origin": public_base},
