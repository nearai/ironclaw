"""IronClaw generic channel-stack Slack E2E (extension-runtime P6 §10).

ONE scenario over the existing harness, driving the GENERIC surfaces of the
`ironclaw serve` binary end to end — never the retiring per-vendor
...
Vendor egress reaches the fake through `IRONCLAW_TEST_HTTP_REWRITE_MAP`
...
async def ironclaw_slack_channel_server(
    ironclaw_binary,
    mock_llm_server,
    fake_slack_server,
    tmp_path_factory,
):
    """IronClaw serve with vendor egress rewritten onto the fake Slack API.
    ...
    home_dir = tmp_path_factory.mktemp("ironclaw-slack-channel-home")
...
async def test_ironclaw_slack_channel_configure_connect_roundtrip_remove(
    ironclaw_slack_channel_server, fake_slack_server
):
    base_url = ironclaw_slack_channel_server
