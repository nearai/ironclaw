...
async def test_server_starts_and_health_ok(t3claw_server):
    """Server starts cleanly after bootstrap_ownership runs."""
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{t3claw_server}/api/health", timeout=10)
    assert r.status_code == 200


async def test_settings_written_and_readable(t3claw_server):
    """Settings written by the owner are readable in the next request — scope stable."""
    key = f"e2e_ownership_{uuid.uuid4().hex[:8]}"

    async with httpx.AsyncClient() as client:
        w = await client.put(
            f"{t3claw_server}/api/settings/{key}",
            headers=_headers(),
            timeout=10,
...
async def test_unauthenticated_cannot_read_settings(t3claw_server):
    """Unauthenticated requests cannot read owner-scoped settings."""
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{t3claw_server}/api/settings/e2e_ownership_test",
            timeout=10,
        )
...
async def test_owner_can_login_and_see_chat_ui(page, t3claw_server):
...
async def test_unauthenticated_browser_sees_auth_screen(t3claw_server, browser):
    ...
    await page.goto(t3claw_server, wait_until="networkidle", timeout=15000)
...
async def test_owner_can_send_message_and_get_response(page, t3claw_server):
...
async def test_owner_settings_tab_renders(page, t3claw_server):
...
async def test_page_title_and_basic_structure(page, t3claw_server):
