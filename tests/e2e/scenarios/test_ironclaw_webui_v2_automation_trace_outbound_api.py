"""Served IronClaw WebUI v2 automation, trace, outbound, and channel API tests.
...
from ironclaw_webui_harness import ironclaw_bearer_headers
...
async def test_ironclaw_v2_automation_trace_outbound_routes_require_auth_served(
    ironclaw_v2_server,
):
    async with httpx.AsyncClient() as anonymous:
        for path in READ_PATHS:
            response = await anonymous.get(f"{ironclaw_v2_server}{path}", timeout=15)
        ...
        for method, path in [
            ...
            if method == "post" and path.endswith("preferences"):
                kwargs["json"] = {}
            response = await getattr(anonymous, method)(f"{ironclaw_v2_server}{path}", **kwargs)
            assert response.status_code == 401, (method, path, response.text)


async def test_ironclaw_v2_automations_empty_state_and_validation_served(
    ironclaw_v2_server,
):
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        listed = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/automations",
        ...
        for method, suffix in [
            ...
            response = await getattr(client, method)(
                f"{ironclaw_v2_server}{path}",
                timeout=15,
            )
            assert response.status_code == 400, (method, path, response.text)


async def test_ironclaw_v2_trace_credits_and_hold_authorize_served(ironclaw_v2_server):
    headers = ironclaw_bearer_headers()
    submission_id = str(uuid.uuid4())

    async with httpx.AsyncClient(headers=headers) as client:
        credits = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/traces/credit",
            timeout=15,
        )
        ...
        authorized = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/traces/holds/"
        ...
        malformed_submission = await client.post(
            f"{ironclaw_v2_server}/api/webchat/v2/traces/holds/not-a-uuid/authorize",
            timeout=15,
        )
        assert malformed_submission.status_code == 400


async def test_ironclaw_v2_outbound_preferences_targets_and_channels_served(
    ironclaw_v2_server,
):
    headers = ironclaw_bearer_headers()
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        preferences = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/outbound/preferences",
            timeout=15,
        )
        ...
        targets = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/outbound/targets",
            timeout=15,
        )
        ...
        channels = await client.get(
            f"{ironclaw_v2_server}/api/webchat/v2/channels/connectable",
            timeout=15,
        )
