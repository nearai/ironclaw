"""Shared IronClaw WebUI v2 E2E harness.

The legacy Playwright suite has mature shared fixtures in ``conftest.py`` for
the ``ironclaw`` gateway. IronClaw WebUI v2 is a different product surface: it
...
scenarios exercise the real IronClaw binary without duplicating process plumbing.
"""
...
from helpers import IRONCLAW_V2_AUTH_TOKEN, SEL_V2, wait_for_ready
...
# `IRONCLAW_DEV_SECRET__<handle>` is read once at `serve` boot, so it must
# be present in the process env before start — see
# ironclaw_v2_private_installs_yolo_server below.
...
def write_config_toml(
    ...
    """Seed a sparse IronClaw config that selects the mock OpenAI-compatible LLM."""
    path.write_text(
...
async def start_ironclaw_webui_v2_server(
    *,
    ironclaw_binary: str,
    mock_llm_server: str,
    home_dir: Path,
    ...
    pytest.fail(
        f"IronClaw WebUI v2 server failed to start after 3 attempts.\n"
        f"Last attempted port: {last_port}\n"
        f"stderr:\n{last_stderr}"
    )


async def close_ironclaw_server(proc) -> None:
...
async def kill_ironclaw_server(proc) -> None:
    ...
    unclean process death, as opposed to `close_ironclaw_server`'s SIGINT/SIGTERM path.
    """
    if proc is not None and proc.returncode is None:
        await stop_process(proc, sig=signal.SIGKILL, timeout=5)


async def enable_ironclaw_global_auto_approve(
    base_url: str, *, token: str = IRONCLAW_V2_AUTH_TOKEN
) -> None:
    """Enable the Tools settings global auto-approve switch for this test user."""
    async with httpx.AsyncClient(headers=ironclaw_bearer_headers(token)) as client:
...
async def ironclaw_v2_server(ironclaw_binary, mock_llm_server, tmp_path_factory):
    """Start ``ironclaw serve`` with the default local-dev profile."""
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-home")
...
@pytest.fixture(scope="module")
async def ironclaw_v2_yolo_server(ironclaw_binary, mock_llm_server, tmp_path_factory):
    """Start ``ironclaw serve`` with auto-approval local-dev-yolo profile."""
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-yolo-home")
    proc, base_url = await start_ironclaw_webui_v2_server(
        ironclaw_binary=ironclaw_binary,
        mock_llm_server=mock_llm_server,
        home_dir=home_dir,
...
@pytest.fixture(scope="module")
async def ironclaw_v2_private_installs_yolo_server(
    ironclaw_binary, mock_llm_server, tmp_path_factory
):
    ...
    """
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-private-installs-home")
    proc, base_url = await start_ironclaw_webui_v2_server(
        ironclaw_binary=ironclaw_binary,
        mock_llm_server=mock_llm_server,
        home_dir=home_dir,
        ...
        extra_env={"IRONCLAW_DEV_SECRET__market_data_api_key": MARKET_DATA_DEV_SECRET},
    )
    await enable_ironclaw_global_auto_approve(base_url)
    try:
        yield base_url
...
@pytest.fixture
async def ironclaw_v2_restartable_server(
    ironclaw_binary, mock_llm_server, tmp_path_factory
):
    """Start/stop IronClaw against one persistent home directory.
    ...
    """
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-restartable-home")
    ...
    async def stop(*, hard: bool = False) -> None:
        if hard:
            await kill_ironclaw_server(state["proc"])
        else:
            await close_ironclaw_server(state["proc"])
        state["proc"] = None
...
@pytest.fixture(scope="module")
async def ironclaw_v2_loop_limited_yolo_server(
    ironclaw_binary, mock_llm_server, tmp_path_factory
):
    """Start IronClaw yolo mode with a low planned-profile loop budget."""
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-loop-limited-home")
    proc, base_url = await start_ironclaw_webui_v2_server(
        ironclaw_binary=ironclaw_binary,
        mock_llm_server=mock_llm_server,
        home_dir=home_dir,
        ...
        extra_env={
            "IRONCLAW_PLANNED_DEFAULT_ITERATION_LIMIT": "1",
        },
    )
    await enable_ironclaw_global_auto_approve(base_url)
    try:
...
@pytest.fixture(scope="module")
async def ironclaw_v2_vision_server(ironclaw_binary, mock_llm_server, tmp_path_factory):
    """Start IronClaw with a vision-classified model id backed by the mock LLM."""
    home_dir = tmp_path_factory.mktemp("ironclaw-v2-vision-home")
    proc, base_url = await start_ironclaw_webui_v2_server(
        ironclaw_binary=ironclaw_binary,
        mock_llm_server=mock_llm_server,
        home_dir=home_dir,
...
@pytest.fixture(scope="module")
async def ironclaw_v2_browser():
    """Chromium instance for IronClaw v2 tests, independent of the legacy gateway."""
...
@pytest.fixture
async def ironclaw_v2_page(ironclaw_v2_server, ironclaw_v2_browser):
    """Fresh authenticated page on the IronClaw v2 SPA."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    await open_ironclaw_v2_page(page, ironclaw_v2_server)
    yield page
    await context.close()


@pytest.fixture
async def ironclaw_v2_yolo_page(ironclaw_v2_yolo_server, ironclaw_v2_browser):
    ...
    page = await context.new_page()
    await open_ironclaw_v2_page(page, ironclaw_v2_yolo_server)
    yield page
    await context.close()


@pytest.fixture
async def ironclaw_v2_vision_page(ironclaw_v2_vision_server, ironclaw_v2_browser):
    """Fresh authenticated page backed by a vision-classified mock model."""
    context = await ironclaw_v2_browser.new_context(viewport={"width": 1280, "height": 720})
    page = await context.new_page()
    await open_ironclaw_v2_page(page, ironclaw_v2_vision_server)
    yield page
    await context.close()


async def open_ironclaw_v2_page(page, base_url: str, path: str = "/") -> None:
    separator = "&" if "?" in path else "?"
    await page.goto(f"{base_url}{path}{separator}token={IRONCLAW_V2_AUTH_TOKEN}")
    await page.wait_for_selector(SEL_V2["chat_composer"], timeout=15000)


def ironclaw_bearer_headers(token: str = IRONCLAW_V2_AUTH_TOKEN) -> dict[str, str]:
