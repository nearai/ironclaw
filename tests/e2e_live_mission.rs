...
mod live_mission_tests {
    ...
    #[tokio::test]
    #[ignore] // Live tier: requires LLM API keys. Deterministic replay not supported (UUIDs non-deterministic in tool calls).
    async fn mission_daily_news_digest_with_followup() {
        init_tracing();

        // This test is live-only because deterministic replay cannot handle UUIDs
        // that are generated freshly on each run. The fixture records mission_id
        // parameters in tool calls (mission_fire(mission_id='...')), but those
        // UUIDs don't match the ones generated by fresh mission_create calls.
        // To run this test, set IRONCLAW_LIVE_TEST=1:
        //   IRONCLAW_LIVE_TEST=1 cargo test --features libsql --test e2e_live_mission -- --ignored
