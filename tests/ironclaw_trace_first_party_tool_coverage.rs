...
use parity_qa_support::{
    binary_e2e::{HarnessWaitConfig, IronClawBinaryE2EHarness, assert_milestone_order},
    model_replay::{
        IronClawModelReplayStep, IronClawScriptedProviderToolCall, IronClawTraceReplayModelGateway,
    },
};

const IRONCLAW_FIRST_PARTY_E2E_COVERED_CAPABILITIES: &[&str] = &[
    ECHO_CAPABILITY_ID,
    TIME_CAPABILITY_ID,
    JSON_CAPABILITY_ID,
    HTTP_CAPABILITY_ID,
...
#[test]
fn ironclaw_builtin_first_party_capability_e2e_coverage_is_complete() {
    let declared = builtin_first_party_package()
        ...
        .collect::<BTreeSet<_>>();
    let covered = IRONCLAW_FIRST_PARTY_E2E_COVERED_CAPABILITIES
        .iter()
    ...
    assert_eq!(
        declared, covered,
        "each built-in first-party capability must have IronClaw e2e coverage"
    );
}

#[tokio::test]
async fn ironclaw_trace_process_first_party_tools_parity() {
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_process_capabilities(
    ...
    let submitted = harness
        ...
        .expect("submit text");
    harness
        .wait_for_status_with_config(submitted.run_id, TurnStatus::Completed, ironclaw_e2e_wait())
        .await
    ...
    assert!(
        ...
        "shell must be advertised on the IronClaw model-facing first-party surface"
    );
...
async fn ironclaw_trace_spawn_subagent_is_surface_text_and_structured_tool() {
    let spawn_subagent =
        CapabilityId::new(SPAWN_SUBAGENT_CAPABILITY_ID).expect("valid capability id");
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
    ...
    assert!(
        requests[0]
            ...
            .iter()
            .any(|message| message.content.contains(spawn_subagent.as_str())),
        "spawn_subagent must be advertised in IronClaw model-facing surface text"
    );
    harness.assert_model_exhausted();
...
#[tokio::test]
async fn ironclaw_trace_http_save_first_party_tool_parity() {
...
#[tokio::test]
async fn ironclaw_trace_skill_management_first_party_tools_parity() {
    let skill_install =
    ...
    let skill_content = skill_md(SKILL_NAME, "IronClaw skill management e2e");
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
                skill_install.clone(),
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_skill_management_capabilities(
...
#[tokio::test]
async fn ironclaw_trace_trigger_management_first_party_tools_parity() {
    let trigger_create =
...
#[tokio::test]
async fn ironclaw_trace_trace_commons_first_party_tools_parity() {
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_trace_commons_capabilities(
...
#[tokio::test]
async fn ironclaw_trace_trace_commons_pilot_tools_are_model_visible() {
    let onboard = CapabilityId::new(TRACE_COMMONS_ONBOARD_CAPABILITY_ID).expect("capability id");
    let status = CapabilityId::new(TRACE_COMMONS_STATUS_CAPABILITY_ID).expect("capability id");
    let credits = CapabilityId::new(TRACE_COMMONS_CREDITS_CAPABILITY_ID).expect("capability id");
    let profile_token =
...
#[tokio::test]
async fn ironclaw_trace_memory_first_party_tools_parity() {
        ...
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
                ...
                serde_json::json!({
                    ...
                    "content": "IronClaw memory e2e marker for capability search.",
                    "append": false
                }),
            )],
            expected_tool_results: Vec::new(),
    ...
    assert!(
        results[1].output["content"]
            ...
            .contains("IronClaw memory e2e marker")
    );
    assert!(
...
#[tokio::test]
async fn ironclaw_trace_profile_set_first_party_tool_parity() {
...
fn ironclaw_e2e_wait() -> HarnessWaitConfig {
    HarnessWaitConfig {
