...
use parity_qa_support::{
    binary_e2e::{HarnessWaitConfig, IronClawBinaryE2EHarness},
    model_replay::{
...
#[tokio::test]
async fn ironclaw_trace_advertises_github_v2_wasm_capabilities() {
    let expected_capabilities =
        ironclaw_support::github::capability_ids().expect("valid GitHub capability ids");
...
#[tokio::test]
async fn ironclaw_trace_wasm_guest_operation_failure_recovers_with_changed_action() {
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
...
#[tokio::test]
async fn ironclaw_trace_executes_github_v2_wasm_capability_matrix() {
...
fn github_capability_calls() -> Vec<IronClawScriptedProviderToolCall> {
    vec![
        call(
            "github.get_repo",
            "get-repo",
