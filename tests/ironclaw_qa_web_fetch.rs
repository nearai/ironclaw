...
#[tokio::test]
async fn ironclaw_qa_endpoint_status_check_reports_http_200() {
    ...
    let mut harness =
        IronClawBinaryE2EHarness::with_host_runtime_core_builtin_capabilities_live_http_egress(
...
#[tokio::test]
async fn ironclaw_qa_latest_release_summary_from_github_api() {
    let http = CapabilityId::new(HTTP_CAPABILITY_ID).expect("valid capability id");
    ...
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        ...
        IronClawModelReplayStep::Response {
            response: HostManagedModelResponse::assistant_reply(
                "Latest release v0.9.0: adds IronClaw WebUI operator observability routes",
            ),
            expected_tool_results: Vec::new(),
        },
    ]);
...
#[tokio::test]
async fn ironclaw_qa_hacker_news_keyword_search_reports_matches() {
    let http = CapabilityId::new(HTTP_CAPABILITY_ID).expect("valid capability id");
    let server =
...
async fn latest_release(State(state): State<LiveLoopbackHttpState>, uri: Uri) -> impl IntoResponse {
    ...
    (
        ...
        Json(serde_json::json!({
            ...
            "body": "Adds IronClaw WebUI operator observability routes",
        })),
    )
        .into_response()
}
