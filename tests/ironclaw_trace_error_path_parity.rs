...
#[tokio::test]
async fn ironclaw_trace_error_path_parity() {
...
#[tokio::test]
async fn ironclaw_trace_invalid_input_recovers_with_changed_action() {
...
#[tokio::test]
async fn ironclaw_trace_unadvertised_capability_is_rejected() {
    let read_file = CapabilityId::new(READ_FILE_CAPABILITY_ID).expect("valid capability id");
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_write_only(
