...
//!
//! Creation runs through the IronClaw binary-E2E harness (chat turn →
...
use ironclaw_composition::{
    IronClawCompositionProfile, IronClawRuntime, IronClawRuntimeIdentity, IronClawRuntimeInput,
    IronClawRuntimeProfileOptions, TriggerPollerSettings, build_ironclaw_runtime,
...
async fn run_routine_creation(case: RoutineCreationCase) {
    ...
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_trigger_management_capabilities(
        case.room,
        model_gateway,
    )
    .await
...
#[tokio::test]
async fn ironclaw_qa_routine_created_for_meeting_prep_email_every_30_minutes() {
...
#[tokio::test]
async fn ironclaw_qa_routine_created_for_endpoint_health_ping_every_5_minutes() {
    run_routine_creation(RoutineCreationCase {
...
#[tokio::test]
async fn ironclaw_qa_routine_created_for_github_release_watch_every_5_minutes() {
    run_routine_creation(RoutineCreationCase {
...
#[tokio::test]
async fn ironclaw_qa_routine_created_for_crm_inbox_sweep_every_30_minutes() {
    run_routine_creation(RoutineCreationCase {
...
#[tokio::test]
async fn ironclaw_qa_routine_created_for_hacker_news_monitor_every_hour() {
    run_routine_creation(RoutineCreationCase {
...
/// Builds a local-dev yolo `IronClawRuntime` with the trigger poller enabled
...
async fn build_qa_fire_runtime(
    ...
    let input = IronClawRuntimeInput::from_services(input)
    ...
    let runtime = build_ironclaw_runtime(input).await.expect("runtime builds");
    seed_qa_fire_auto_approve(&runtime).await;
    runtime
}

async fn seed_qa_fire_auto_approve(runtime: &IronClawRuntime) {
...
#[tokio::test]
async fn ironclaw_qa_routine_created_by_tool_fires_and_runs_routine_prompt() {
...
#[tokio::test]
async fn ironclaw_qa_fired_routine_executes_action_and_finalizes_reply() {
    ...
    let gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
        IronClawModelReplayStep::ProviderToolCalls {
            calls: vec![IronClawScriptedProviderToolCall::new(
                echo.clone(),
...
async fn invoke_trigger_create(runtime: &IronClawRuntime, input: Value) -> Value {
