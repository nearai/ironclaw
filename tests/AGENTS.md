...
| Decode GitHub Contents API base64 in the producer before the shared durable writer, expose only UTF-8 semantic content to the next model call, and round-trip it through `result_read` | `tool_call.rs::github_file_content_decodes_before_the_durable_result_path` |
