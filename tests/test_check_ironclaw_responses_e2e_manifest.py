...
SCRIPT = ROOT / "scripts/ci/check-ironclaw-responses-e2e-manifest.py"
SPEC = importlib.util.spec_from_file_location(
    "check_ironclaw_responses_e2e_manifest", SCRIPT
)
...
class CheckIronClawResponsesE2EManifestTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.primary.write_text(
            "def test_ironclaw_responses_primary():\n    pass\n",
            encoding="utf-8",
        )
        self.legacy.write_text(
            "async def test_ironclaw_legacy():\n    pass\n",
            encoding="utf-8",
        )
        self.expected = [
            f"{self.primary.name}::test_ironclaw_responses_primary",
            f"{self.legacy.name}::test_ironclaw_legacy",
        ]
    ...
    def test_rejects_unexpected_selector(self) -> None:
        unexpected = f"{self.primary.name}::test_ironclaw_responses_unexpected"
