...
//! `tests/fixtures/llm_traces/ironclaw_qa/`:
//!
//! 1. **Recorder tests** (`#[ignore]`, run manually with `ANTHROPIC_API_KEY`
//!    set): drive each QA phrase through a local-dev IronClaw runtime backed by
...
//!      cargo test --test ironclaw_qa_recorded_behavior record_ \
...
//!      cargo test --test ironclaw_qa_recorded_behavior record_investigate_ci_job \
//!        -- --ignored --test-threads=1 --nocapture
//!    ```
//!
...
//!    configured Google product-auth account from the local IronClaw store.
...
//!
//!    ```bash
//!    scripts/ci/check-ironclaw-qa-fixtures.sh
//!    ```
//!
...
//!
//! 3. **Replay tests**: replay the fixture through a real IronClaw runtime via
//!    `IronClawTraceReplayModelGateway::from_trace` (with recorded
...
async fn replay_routine_phrase(case: &QaPhrase, cron_fragment: &str) {
    ...
    let gateway =
        IronClawTraceReplayModelGateway::from_trace(trace).expect("replay gateway from fixture");
