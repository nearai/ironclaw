...
SCRIPT = ROOT / "scripts/live-canary/validate_ironclaw_binary_artifact.py"
SPEC = importlib.util.spec_from_file_location("validate_ironclaw_binary_artifact", SCRIPT)
assert SPEC is not None and SPEC.loader is not None
VALIDATOR = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(VALIDATOR)


class ValidateIronClawBinaryArtifactTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = tempfile.TemporaryDirectory()
        self.artifact_dir = Path(self.temp_dir.name)
        self.archive = self.artifact_dir / VALIDATOR.ARCHIVE_NAME
        self.archive.write_bytes(b"tested IronClaw binary")
