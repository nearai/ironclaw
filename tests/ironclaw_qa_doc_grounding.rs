...
#[tokio::test]
async fn ironclaw_qa_strategy_doc_becomes_knowledge_base_for_answers() {
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_core_builtin_capabilities(
...
#[tokio::test]
async fn ironclaw_qa_meeting_prep_references_company_doc_and_latest_news() {
...
fn seed_drive_doc(harness: &IronClawBinaryE2EHarness, relative: &str, content: &str) {
    let path = harness
