...
use parity_qa_support::model_replay::{
    IronClawModelReplayStep, IronClawScriptedProviderToolCall, IronClawTraceReplayModelGateway,
};
...
#[test]
fn every_pasted_qa_scenario_has_ironclaw_e2e_coverage() {
...
async fn qa_trigger_automation_smokes_create_view_and_cleanup() {
    ...
    assert!(
        ...
        "current IronClaw triggers expose scheduled/removed state, not a paused state"
    );
...
async fn qa_subagent_capability_smoke_uses_child_run() {
    ...
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps([
    ...
    let mut harness = IronClawBinaryE2EHarness::with_harness_blocked_evidence(
        "room-qa-subagent",
        model_gateway,
...
async fn qa_extension_lifecycle_tools_search_install_activate_and_remove_e2e() {
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_extension_lifecycle_capabilities(
    ...
    let submitted = harness
        .submit_text(
            ...
            "search, install, activate, and remove a IronClaw extension",
        )
        .await
...
async fn qa_activating_bundled_extensions_exposes_complete_model_surface_e2e() {
    ...
    let submitted = harness
        .submit_text(
            ...
            "activate every bundled IronClaw extension and verify the model surface",
        )
        .await
...
async fn qa_plugin_browser_mcp_artifact_and_image_capability_surface_smokes() {
    ...
    let model_gateway =
        IronClawTraceReplayModelGateway::with_scripted_steps([IronClawModelReplayStep::Response {
            response: HostManagedModelResponse::assistant_reply(
                "qa unavailable plugin artifact surface smoke complete",
            ),
            expected_tool_results: Vec::new(),
        }]);
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_qa_smoke_capabilities(
    ...
    for unsupported in unsupported_capabilities {
        assert!(
            ...
            "{unsupported} should not be advertised by the IronClaw first-party QA surface"
        );
    }
    harness.assert_model_exhausted();
    harness.shutdown().await;
...
async fn qa_github_capability_smoke_discovers_actions_without_live_github() {
    ...
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_github_issue_capabilities(
        "room-qa-github",
        model_gateway,
    )
    .await
...
async fn run_qa_harness(
    ...
    event_id: &str,
    prompt: &str,
    steps: impl IntoIterator<Item = IronClawModelReplayStep>,
) -> IronClawBinaryE2EHarness {
    let model_gateway = IronClawTraceReplayModelGateway::with_scripted_steps(steps);
    let mut harness = IronClawBinaryE2EHarness::with_host_runtime_qa_smoke_capabilities(
        conversation_id,
        model_gateway,
    )
...
) -> IronClawScriptedProviderToolCall {
    IronClawScriptedProviderToolCall::new(capability_id.clone(), call_id, arguments)
}
