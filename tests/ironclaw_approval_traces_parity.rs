...
use ironclaw_support::harness::RecordingTestCapabilityPort;
use ironclaw_turns::{TurnStatus, run_profile::LoopHostMilestoneKind};
use parity_qa_support::{
    binary_e2e::{IronClawBinaryE2EHarness, assert_milestone_order, trace_tool_call_response},
    model_replay::IronClawTraceReplayModelGateway,
};

#[tokio::test]
async fn ironclaw_approval_traces_parity() {
    let model_gateway = IronClawTraceReplayModelGateway::with_responses([
...
#[tokio::test]
async fn ironclaw_approval_cancel_traces_parity() {
    let model_gateway = IronClawTraceReplayModelGateway::with_responses([
        trace_tool_call_response(),
