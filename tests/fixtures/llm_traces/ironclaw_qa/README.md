...
```bash
python3 scripts/import-ironclaw-run-artifact.py \
  ~/Downloads/ironclaw-run-<run-id>.json \
  tests/fixtures/llm_traces/ironclaw_qa/<scenario>.candidate.json
```
...
4. Run `scripts/ci/check-ironclaw-qa-fixtures.sh`.
