...
- Rust 1.96+ для сборки из исходников
- Node.js 22+ и Corepack/pnpm для сборки с `webui-v2-beta`
- Учётные данные выбранного LLM-провайдера
- PostgreSQL 15+ опционален для hosted/production profile; local-dev его не требует
...
Каноническая команда `ironclaw` запускает Reborn. Инициализация создаёт изолированную конфигурацию в `~/.ironclaw/reborn` и не изменяет базу данных, настройки или связку ключей v1:
...
Для локального запуска не нужно вручную создавать PostgreSQL. В хостинге можно выбрать PostgreSQL, но URL подключения и мастер-ключ передаются через существующие переменные `IRONCLAW_REBORN_*`. Подробности см. в английском README и [документации Reborn onboarding](docs/reborn/onboarding.md).
...
Выберите провайдера и модель через CLI, затем задайте указанную переменную окружения с ключом:
...
См. [руководство по моделям canonical CLI](docs/reborn-binary.md#models) для поиска и выбора провайдера.
...
- Состояние runtime по умолчанию хранится локально; hosted/production profile могут использовать PostgreSQL.
...
Текущий релиз по умолчанию использует Reborn; каноническая команда CLI — `ironclaw`. Переменная `ENGINE_V2` больше не нужна.

```bash
# Инициализация Reborn home, файлов конфигурации и WebUI token
ironclaw onboard

# Отправить одно сообщение или запустить интерактивный REPL
ironclaw run --message "Привет"
ironclaw repl
...
cargo test -p ironclaw_reborn_cli название_теста
```
