...
Read each module's `AGENTS.md` (every crate has one; they list the seams and the
...
- Keep substrate handles (secret store, raw stores) **private** to factories;
  ...
  composition and hand out only that. (See composition `AGENTS.md`.)
...
- Persist-then-reload must be atomic-ish: either pre-validate, or treat the
  ...
  failure. See `.Codex/rules/error-handling.md`.
