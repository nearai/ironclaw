...
Trace Commons hardening note: required DB mirror mode now avoids publishing local submission metadata/object state, delayed-credit ledger rows, benchmark/ranker artifact/provenance files, or service-local encrypted export artifacts when those mirror writes fail, and vector indexing now enforces tenant policy plus signed-claim allowed-use ABAC before indexing. The vector source set and nearest-neighbor pool are filtered to traces that permit a derived vector use; aggregate-only claims and sources are rejected/skipped because they cannot produce derived artifacts.
