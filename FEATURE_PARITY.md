...
| Hosted MCP extensions | ✅ | 🚧 | Reborn supports authenticated per-user HTTPS MCP register/unregister plus explicit live activation through host-owned `RuntimeHttpEgress`; registered `tools/list` skips invalid names per tool, quarantines unsafe description/schema metadata as `HostInternal` + `Deny` with inert host-authored replacements, forces `ExternalWrite`, disables instead of publishing stale zero-capability descriptors after restart, and uses ID-only cleanup inventory for exact policy/override revocation. T3 registration remains `auth = none`; bundled Notion/NEAR AI ProductAuth behavior remains supported, while registered API-key/OAuth setup is follow-up and must use encrypted `SecretStore` handles with just-in-time egress injection. |
...
| Tool policies (allow/deny) | ✅ | ✅ | Reborn stores scoped persistent `AlwaysAllow` approval policies for manifest-allow capabilities and replays them at the current sandbox scope; WebChat v2 exposes authenticated caller-scoped settings at `/api/webchat/v2/settings/tools`, and registered MCP unregister performs product-facing exact dispatch-policy revocation plus per-tool override clearing from its durable capability-id inventory without touching unrelated tools. |
