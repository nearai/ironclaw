...
| `traces` | ➖ | 🚧 | - | IronClaw-native Trace Commons MVP: opt-in local redaction, autonomous submission with scoped policy preflight, private ingest/review/export/credit/admin helpers, optional ingest tenant policies for allowed consent scopes and trace-card uses with a require-policy production gate, optional fail-closed export guardrails for explicit accepted/low-risk/consent-scoped replay, benchmark, and ranker-training corpus slices, hardened local Privacy Filter sidecar guardrails, governed delayed-credit mutation with typed DB audit metadata, hash-chained file-backed audit rows with maintenance verification and DB mirror hash fields, local credit/status summaries, optional DB dual-write/backfill with contributor, reviewer metadata, replay selection, policy-gated DB object-ref-backed replay envelope reads with tenant/object-ref/hash verification and an object-ref-required production gate, compact export-manifest metadata/invalidation/listing plus CLI listing, purpose-indexed reviewer trace listing, replay export item snapshots/invalidation with source object refs, benchmark/ranker provenance invalidation with derived and active vector artifact refs, audit reads/mirroring with reviewer queue/audit-log reads and retention/purge/revocation-tombstone invalidation reads, source-hashed exports, and per-trace replay content-read events including `object_ref_id` for DB object-ref reads, maintenance DB reconciliation with reader-projection parity for contributor credit, reviewer metadata, analytics, audit, and export manifests, retention expiry maintenance with expired-source export cache pruning and explicit expired-artifact purge, vector-entry metadata/indexing, encrypted artifact sidecar, initial PostgreSQL RLS policies with transaction-local tenant context, and delayed credit notices. Not an OpenClaw parity feature. |
