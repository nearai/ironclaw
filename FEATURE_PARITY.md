...
| Per-user/private MCP registration | ✅ | 🚧 | Internal framework only, not yet user-reachable: owner-scoped registered-MCP store (tenant + user path sharding), minted owner-unique registration ids (tenant + owner + URL + account label hashed into the id), and lifecycle chokepoints for install/search/list/remove/restore. No register verb, UI, CLI, or live discovery exists yet. Unblocks planned follow-on work: egress, then verb + discovery + UI, then credentials/multi-account, then re-discovery/drift detection |
