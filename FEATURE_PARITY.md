...
| `traces` | ➖ | 🚧 | - | IronClaw-native Trace Commons MVP: opt-in local redaction, autonomous submission, private ingest/review/export/credit/admin helpers, hardened local Privacy Filter sidecar guardrails, governed delayed-credit mutation, optional DB dual-write/backfill with contributor, reviewer metadata, replay selection, compact export-manifest metadata/invalidation/listing, replay export item snapshots/invalidation, audit reads/mirroring with source-hashed exports and per-trace replay content-read events, retention expiry maintenance with expired-source export cache pruning and explicit expired-artifact purge, vector-entry metadata/indexing, encrypted artifact sidecar, and delayed credit notices. Not an OpenClaw parity feature. |
