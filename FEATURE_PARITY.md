...
- 🚧 Slack channel (real implementation): Reborn host-beta route can be explicitly mounted by `ironclaw-reborn serve` with Slack Events API signing, extension-card personal OAuth setup that binds Slack `authed_user.id` to the authenticated Reborn user, DM/app-mention routing through Product Workflow/Reborn, final-reply delivery, admin-managed allowed-channel picker, durable WebUI channel-route assignment APIs, provider-side default outbound target inventory for shared channels and explicitly provisioned personal DMs, and one unified host-bundled `slack` extension manifest (provider `slack`) declaring tool, channel, and auth surfaces; the extension wire is `runtime + surfaces` with no `kind`. Persisted split-Slack installation/provider identities migrate forward with bounded CAS on CAS-capable backends; legacy non-CAS local-dev startup normalizes installation state only in memory, while explicitly requested lifecycle mutations remain serialized and durable. The provider migration has aggregate traversal bounds and requires a one-time cutover with pre-Train-A writers quiesced. Collision-safe actor keys retain safe legacy reads. DMs execute as the OAuth-bound actor, while shared channel turns route to allowed dynamic or static channel subjects and fail closed for unrouted channels in admin-managed mode. Train A recognizes only this composition-owned Slack connection and rejects every other inbound channel/auth combination across install, restore, and activation; broader typed generic connection ownership/ingress/delivery remains Train B work.
