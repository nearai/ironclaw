...
| Per-user/private MCP registration | ✅ | 🚧 | Owner-scoped registered-MCP store (tenant + user path sharding) with row-authoritative install/search/list/remove/restore resolution; single flat installation-row map keyed by bare extension id (no owner-unique id minting yet) is a known follow-up constraint |
