...
Trace Commons hardening note: required DB mirror mode now avoids publishing local submission metadata/object state, delayed-credit ledger rows, replay manifests, benchmark/ranker artifact/provenance files, benchmark lifecycle rewrites, or service-local encrypted export artifacts when those mirror writes fail, and rolls back replay/benchmark/ranker export DB/file publication if the final export audit mirror fails. Replay, benchmark, and ranker export DB mirrors now write manifest metadata, export artifact object refs, and item snapshots atomically across PostgreSQL/libSQL so item-reference failures do not leave partial DB exports, with PostgreSQL parity coverage for rollback on invalid item references. Retention purge now records `deleted_at` only on exact physically deleted submitted-envelope object refs, scoped by tenant, submission, object store, and object key, with PostgreSQL facade/RLS tests covering retention job and item tenant visibility. PostgreSQL facade/RLS tests now also cover tenant-policy row isolation and updates, review-lease typed audit metadata, and tenant/reviewer-scoped lease claim, renewal, release, and expired-handoff semantics. Manual reviewer/admin delayed utility credit for benchmark conversion, regression catches, model training, and ranking training now enforces the same source allowed-use plus tenant-policy/signed-claim ABAC as worker utility credit before ledger rows are written. Admin config-status now surfaces safe PostgreSQL RLS readiness diagnostics for policy coverage, expression drift, disabled RLS, force-RLS count/missing-table state, role bypass state, tenant-rollout gate counts, and a stricter production-ready boolean without exposing row data or tenant ids, and `TRACE_COMMONS_REQUIRE_POSTGRES_TRACE_RLS_READY` can fail startup unless PostgreSQL RLS is fully ready as a tenant boundary. DB read/object-ref/object-primary gates now have tenant allowlists for canary promotion, with caller tests proving tenant-scoped contributor DB reads and object-primary submit rollout while other tenants stay on file fallback. The local encrypted object store now implements a `TraceArtifactStore` provider trait for the service-owned object-store migration path. Vector indexing now enforces tenant policy plus signed-claim allowed-use ABAC before indexing. The vector source set and nearest-neighbor pool are filtered to traces that permit a derived vector use; aggregate-only claims and sources are rejected/skipped because they cannot produce derived artifacts. Review decisions now fail closed for non-quarantined, terminal, expired, and aggregate-only approval records before any trace-content read is emitted.

Trace Commons production-boundary note: PostgreSQL/libSQL now include a durable tenant-scoped revocation-propagation ledger for downstream invalidation and retry work across object refs, exports, vectors, derived artifacts, benchmark/ranker artifacts, credit settlements, and physical delete receipts. The revocation worker can now verify and physically delete exact service-local encrypted submitted/review envelope object payloads for tenant-scoped object-ref items, marking unsupported stores and artifact kinds as skipped. Export call sites for replay, benchmark, and ranker slices now create and validate short-lived tenant/principal/purpose/dataset-kind access grants before producing artifacts. `TRACE_COMMONS_OBJECT_STORE=remote_service` parses production remote object-store intent but deliberately fails closed behind a disabled service-owned provider instead of falling back to plaintext files. Local autonomous status sync now keeps append-only safe history events and sanitizes server-returned credit explanations before periodic credit notices persist them.

Trace Commons revocation-worker note: the ingest service now recognizes a scoped `revocation_worker` role and exposes `POST /v1/workers/revocation-propagation` for DB-backed propagation runs. The worker claims due tenant-scoped ledger items, performs idempotent metadata/vector/export invalidation actions, physically deletes hash-verified service-local submitted/review envelope payloads for exact object-ref targets, records unsupported physical-delete stores/artifact kinds and credit-settlement actions as explicit skipped items, preserves other tenants' due work, and emits safe audit counts.
...
Trace Commons issuer/TenantCtx note: this branch now includes a standalone EdDSA/Ed25519-only `trace_commons_upload_claim_issuer` binary that signs short-lived contributor upload claims, authenticates workload JWTs with EdDSA only, enforces workload issuer/audience/expiry plus consent/use narrowing, optionally requires DB-backed contributor tenant-access grants using the same signed-principal hash shape as ingest, rejects RSA key material, and publishes the ingest-compatible `kid`/`public_key_pem` keyset shape. The ingest compatibility layer also now fails closed when file-backed submission metadata or derived rows are read from one authenticated tenant directory but carry a different embedded tenant id or tenant storage ref, with caller tests for revoke and review side-effect paths.
