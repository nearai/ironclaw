...
Trace Commons issuer/TenantCtx note: this branch now includes a standalone EdDSA/Ed25519-only `trace_commons_upload_claim_issuer` binary that signs short-lived contributor upload claims, authenticates workload JWTs with EdDSA only, enforces workload issuer/audience/expiry plus consent/use narrowing, rejects RSA key material, and publishes the ingest-compatible `kid`/`public_key_pem` keyset shape. The ingest compatibility layer also now fails closed when file-backed submission metadata or derived rows are read from one authenticated tenant directory but carry a different embedded tenant id or tenant storage ref, with caller tests for revoke and review side-effect paths.
