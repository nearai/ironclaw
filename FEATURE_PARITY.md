...
| Hosted MCP extensions | ✅ | 🚧 | Reborn composes host-mediated MCP runtime, bundles discovery-only Notion and Xquik packages, wires DCR OAuth exchange and rotating refresh tokens through ProductAuth, and activates hosted MCP packages with live `tools/list` schema discovery through host-staged credentials |
