...
This push also adds local autonomous queue diagnostics/status surfaces: CLI `traces queue-status` reports readiness, bearer-token environment presence, queue/held counts, retry/manual-review/policy hold counts, next retry time, sanitized held-reason counts, and local credit summaries; authenticated web `/api/traces/queue-status` reports scoped queue/held diagnostics, and `/api/traces/credit-notice` marks due notices. Due credit notices now carry local acknowledge/snooze state: CLI `traces credit --notice --ack` and authenticated `POST /api/traces/credit-notice` acknowledgement suppress the current credit fingerprint until credit changes, while `--snooze-hours` and the matching web action suppress it until a bounded deadline without exposing trace bodies or explanation text in the fingerprint. The agent loop now runs a periodic Trace Commons queue worker for opted-in owner/active-user scopes, stores retryable submission failures as typed redacted sidecars with capped backoff, skips retry-held envelopes until due, and broadcasts returned credit notices. EdDSA/Ed25519 public-key verification is available through default or `kid`-selected key config and JSON/file keysets with optional activation windows, with safe total/EdDSA config-status counts; managed issuer distribution/governance remains planned work.
