...
impl SetupWizard {
    ...
    async fn step_inference_provider(&mut self) -> Result<(), SetupError> {
        ...
        if let Some(ref current) = self.settings.llm_backend {
            let display = match current.as_str() {
                "nearai" => "NEAR AI",
                "anthropic" => "Anthropic (Claude, API key/setup-token)",
                "openai" => "OpenAI (API key/Codex OAuth token)",
                "ollama" => "Ollama (local)",
        ...
        let options = &[
            ...
            "OpenAI           - GPT models (API key or Codex OAuth token)",
    ...
    ///
    /// Supports both API keys and Claude setup-tokens.
    async fn setup_anthropic(&mut self) -> Result<(), SetupError> {
        let options = &[
            "API key      - direct key from Anthropic console",
            "setup-token  - generated by `claude setup-token`",
        ];
        let choice = select_one("Anthropic auth method:", options).map_err(SetupError::Io)?;
        match choice {
            0 => {
            ...
            1 => self.setup_anthropic_setup_token().await,
            _ => Err(SetupError::Config(
                "Invalid Anthropic auth selection".to_string(),
            )),
        }
    }
    ...
    ///
    /// Supports API keys and Codex OAuth access tokens.
    async fn setup_openai(&mut self) -> Result<(), SetupError> {
        let options = &[
            "API key             - direct key from platform.openai.com",
            "Codex OAuth token   - from `codex login`",
        ];
        let choice = select_one("OpenAI auth method:", options).map_err(SetupError::Io)?;
        match choice {
            0 => {
                self.setup_api_key_provider(
                    "openai",
            ...
            1 => self.setup_openai_codex_token().await,
            _ => Err(SetupError::Config(
                "Invalid OpenAI auth selection".to_string(),
            )),
        }
    }

    /// Anthropic setup-token flow (`claude setup-token`).
    async fn setup_anthropic_setup_token(&mut self) -> Result<(), SetupError> {
        self.settings.llm_backend = Some("anthropic".to_string());
        if self.settings.selected_model.is_some() {
            self.settings.selected_model = None;
        }

        if let Ok(existing) = std::env::var("ANTHROPIC_OAUTH_TOKEN") {
            print_info(&format!(
                "ANTHROPIC_OAUTH_TOKEN found: {}",
    ...
    ///
    /// Reads from `OPENAI_CODEX_OAUTH_TOKEN`, then attempts to import the token
    /// from `$CODEX_HOME/auth.json` (or `~/.codex/auth.json`) before falling
    /// back to manual entry.
    async fn setup_openai_codex_token(&mut self) -> Result<(), SetupError> {
        self.settings.llm_backend = Some("openai".to_string());
        if self.settings.selected_model.is_some() {
            self.settings.selected_model = None;
        }

        if let Ok(existing) = std::env::var("OPENAI_CODEX_OAUTH_TOKEN") {
            print_info(&format!(
                "OPENAI_CODEX_OAUTH_TOKEN found: {}",
                mask_api_key(&existing)
            ));
            if confirm("Use this Codex OAuth token?", true).map_err(SetupError::Io)? {
                self.persist_openai_codex_token(existing.trim()).await?;
                print_success("OpenAI configured (Codex OAuth token from env)");
                return Ok(());
            }
        }

        if let Some((token, auth_path)) = read_codex_cli_access_token() {
            print_info(&format!("Found Codex auth at {}", auth_path.display()));
            if confirm("Import token from Codex CLI?", true).map_err(SetupError::Io)? {
                self.persist_openai_codex_token(&token).await?;
                print_success("OpenAI configured (imported Codex OAuth token)");
                return Ok(());
            }
        }

        print_info("Run `codex login` first, then paste the OAuth access token.");
        let token = secret_input("Codex OAuth access token").map_err(SetupError::Io)?;
        let trimmed = token.expose_secret().trim();
        if trimmed.is_empty() {
            return Err(SetupError::Config(
                "OAuth access token cannot be empty".to_string(),
            ));
        }
        self.persist_openai_codex_token(trimmed).await?;
        print_success("OpenAI configured (Codex OAuth token)");
        Ok(())
    }

    async fn persist_openai_codex_token(&mut self, token: &str) -> Result<(), SetupError> {
        ...
        if let Ok(ctx) = self.init_secrets_context().await {
            ctx.save_secret("llm_openai_codex_oauth_token", &token)
                .await
                .map_err(|e| {
                    SetupError::Config(format!("Failed to save Codex OAuth token: {e}"))
                })?;
...
async fn fetch_anthropic_models(cached_key: Option<&str>) -> Vec<(String, String)> {
    ...
    let is_setup_token = api_key.starts_with(ANTHROPIC_SETUP_TOKEN_PREFIX);
    let resp = match fetch_anthropic_models_response(&client, &api_key, is_setup_token).await {
        Some(resp) => resp,
        None => return static_defaults,
    };
...
async fn fetch_anthropic_models_response(
...
async fn fetch_openai_models(cached_key: Option<&str>) -> Vec<(String, String)> {
    ...
    let api_key = cached_key
        ...
        .or_else(|| std::env::var("OPENAI_CODEX_OAUTH_TOKEN").ok())
        .filter(|k| !k.is_empty());
...
fn validate_anthropic_setup_token(raw: &str) -> Result<(), String> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return Err("Setup-token cannot be empty".to_string());
    }
...
fn read_codex_cli_access_token() -> Option<(String, PathBuf)> {
...
mod tests {
    ...
    #[test]
    fn test_validate_anthropic_setup_token() {
        let valid = format!(
            "{}{}",
            ANTHROPIC_SETUP_TOKEN_PREFIX,
            "a".repeat(ANTHROPIC_SETUP_TOKEN_MIN_LENGTH)
        );
        assert!(validate_anthropic_setup_token(&valid).is_ok());
        assert!(validate_anthropic_setup_token("sk-ant-foo").is_err());
        assert!(validate_anthropic_setup_token("   ").is_err());
    }

    #[test]
    fn test_read_codex_cli_access_token_from_auth_file() {
        let dir = tempfile::tempdir().unwrap();
        let auth_path = dir.path().join("auth.json");
        std::fs::write(
            &auth_path,
            r#"{"tokens":{"access_token":"codex-access-token","refresh_token":"codex-refresh-token"}}"#,
        )
        .unwrap();
        let _guard = EnvGuard::set("CODEX_HOME", dir.path().to_string_lossy().as_ref());

        let loaded = read_codex_cli_access_token();
        assert!(loaded.is_some());
        let (token, path) = loaded.unwrap();
