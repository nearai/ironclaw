...
use crate::db::{LearningStore, SkillStatus, SynthesizedSkillRow};
    ...
    async fn update_synthesized_skill_status(
        ...
        let n = conn
            .execute(
                ...
                "#,
                libsql::params![id.to_string(), user_id, status.as_str(), now],
            )
            .await
    ...
    async fn list_synthesized_skills(
        ...
        let mut rows = if let Some(status) = status {
            conn.query(
                ...
                "#,
                libsql::params![user_id, agent_id, status.as_str()],
            )
            .await
        } else {
            conn.query(
...
fn parse_skill_row(row: &libsql::Row) -> Result<SynthesizedSkillRow, DatabaseError> {
    ...
    Ok(SynthesizedSkillRow {
        ...
        status: SkillStatus::from_str_opt(&get_text(row, 7)).unwrap_or_else(|| {
