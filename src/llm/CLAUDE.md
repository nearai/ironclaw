...
Configure via `LlmConfig` fields: `circuit_breaker_threshold` (env: `LLM_CIRCUIT_BREAKER_THRESHOLD`, legacy: `CIRCUIT_BREAKER_THRESHOLD`; None = disabled), `circuit_breaker_recovery_secs` (env: `LLM_CIRCUIT_BREAKER_RECOVERY_SECS`, legacy: `CIRCUIT_BREAKER_RECOVERY_SECS`; default: 30).
...
Configure via `LlmConfig.max_retries` (env: `LLM_MAX_RETRIES`, legacy: `NEARAI_MAX_RETRIES`; default: 3). Set to 0 to disable.
...
**Defaults:** TTL = 1 hour, max entries = 1000. Configure via `LlmConfig` fields: `response_cache_enabled` (env: `LLM_RESPONSE_CACHE_ENABLED`, legacy: `RESPONSE_CACHE_ENABLED`), `response_cache_ttl_secs` (env: `LLM_RESPONSE_CACHE_TTL_SECS`, legacy: `RESPONSE_CACHE_TTL_SECS`), `response_cache_max_entries` (env: `LLM_RESPONSE_CACHE_MAX_ENTRIES`, legacy: `RESPONSE_CACHE_MAX_ENTRIES`). Cache is in-memory only — evicted on restart.
...
`build_provider_chain()` in `mod.rs` is the single source of truth for assembling decorators. It creates the base provider (dispatching to `create_openai_codex_provider()` for codex, `create_llm_provider()` for everything else), then applies all decorators inline using the top-level `LlmConfig` shared settings:
...
Raw provider
  ...
  → CircuitBreakerProvider  (fast-fail; only when `LlmConfig.circuit_breaker_threshold` is set)
  → CachedProvider          (response cache; only when `LlmConfig.response_cache_enabled=true`)
