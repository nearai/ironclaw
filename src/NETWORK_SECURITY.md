# cLawyer Network Security Reference

This document catalogs every network-facing surface in cLawyer, its authentication mechanism, bind address, security controls, and known findings. Use this as the authoritative reference during code reviews that touch network-facing code.
...
cLawyer operates across four trust boundaries:
...
- Webhook senders must prove knowledge of the shared secret. The secret is never transmitted in the clear by cLawyer itself.
...
The listener is **ephemeral** — it is started only when an OAuth flow is initiated (e.g., `clawyer tool auth <name>`) and shut down after the callback is received or the timeout expires.
...
This is by design: MCP server URLs come from **operator-controlled configuration** (config files, environment variables, or the CLI `tool install` command), not from user input or LLM output. A compromised config file is outside cLawyer's threat model — it would imply the operator's machine is already compromised.
