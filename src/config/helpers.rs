...
pub(crate) fn validate_base_url(url: &str, field_name: &str) -> Result<(), ConfigError> {
    ...
    if let Ok(ip) = bare_host.parse::<IpAddr>() {
        ...
        // The URL format, scheme, and blocked-host checks above still apply.
        #[cfg(not(test))]
        {
...
mod tests {
    ...
    // DNS resolution is skipped in test builds (#[cfg(not(test))]) to avoid
    // failures in offline/sandboxed CI environments. This test verifies the
    // runtime-only DNS validation and must run with `--ignored`.
    #[test]
    #[ignore]
