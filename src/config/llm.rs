...
impl LlmConfig {
    ...
    pub(crate) fn resolve(settings: &Settings) -> Result<Self, ConfigError> {
        ...
        let nearai_auth_url = nearai_auth_url_env
            ...
            .to_string();
        // Only validate user-supplied URLs; the well-known default doesn't need DNS checks.
        ...
        let nearai_base_url_explicit = nearai_override
            ...
            .or(optional_env("NEARAI_BASE_URL")?);
        let nearai_base_url = if let Some(url) = &nearai_base_url_explicit {
            url.clone()
        ...
        // Only validate user-supplied URLs; the well-known defaults don't need DNS checks.
        ...
        let openai_codex = if is_openai_codex {
            ...
            let codex_auth_env = optional_env("OPENAI_CODEX_AUTH_URL")?;
            let auth_endpoint = codex_auth_env
                .as_deref()
                .unwrap_or("https://auth.openai.com")
                .to_string();
            if codex_auth_env.is_some() {
                validate_base_url(&auth_endpoint, "OPENAI_CODEX_AUTH_URL")?;
            }
            let codex_api_env = optional_env("OPENAI_CODEX_API_URL")?;
            let api_base_url = codex_api_env
                .as_deref()
                .unwrap_or("https://chatgpt.com/backend-api/codex")
                .to_string();
            if codex_api_env.is_some() {
