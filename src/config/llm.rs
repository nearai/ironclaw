...
mod tests {
    ...
    /// Stub NEARAI_AUTH_URL and NEARAI_BASE_URL to localhost so that
    /// `validate_base_url` does not attempt DNS resolution of
    /// `private.near.ai` / `cloud-api.near.ai`, which fails in
    /// sandboxed / offline environments.
    ///
    /// Must be called while holding `lock_env()`.
    fn stub_nearai_urls() {
        // SAFETY: Only called under ENV_MUTEX in tests.
        unsafe {
    ...
    /// Remove NEARAI_AUTH_URL and NEARAI_BASE_URL stubs.
    ///
    /// Must be called while holding `lock_env()`.
    #[allow(dead_code)]
    fn clear_nearai_url_stubs() {
        // SAFETY: Only called under ENV_MUTEX in tests.
        unsafe {
