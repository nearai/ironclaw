...
//!
//! ```bash
//! T3CLAW_PROFILE=server t3claw
//! ```
//!
//! **Lookup order** for `T3CLAW_PROFILE=foo`:
//! 1. `~/.t3claw/profiles/foo.toml`  (user-defined, highest priority)
...
/// Directory under the T3Claw base dir where user-defined profiles live.
...
/// Read `T3CLAW_PROFILE` and merge the profile onto `settings`.
///
...
pub fn apply_profile(settings: &mut Settings) -> Result<(), ConfigError> {
    let name = match optional_env("T3CLAW_PROFILE")? {
...
/// Path to the user-defined profiles directory: `~/.t3claw/profiles/`.
fn user_profiles_dir() -> PathBuf {
    t3claw_base_dir().join(PROFILES_DIR)
}
...
mod tests {
    ...
    fn no_profile_is_noop() {
        let _guard = lock_env();
        // Ensure T3CLAW_PROFILE is not set.
        unsafe { std::env::remove_var("T3CLAW_PROFILE") };
    ...
    fn unknown_profile_errors() {
        let _guard = lock_env();
        unsafe { std::env::set_var("T3CLAW_PROFILE", "nonexistent-xyz-987") };
    ...
    fn local_profile_disables_gateway_and_sandbox() {
        let _guard = lock_env();
        unsafe { std::env::set_var("T3CLAW_PROFILE", "local") };
    ...
    fn server_profile_enables_postgres() {
        let _guard = lock_env();
        unsafe { std::env::set_var("T3CLAW_PROFILE", "server") };
    ...
    fn server_multitenant_sets_higher_parallelism() {
        let _guard = lock_env();
        unsafe { std::env::set_var("T3CLAW_PROFILE", "server-multitenant") };
    ...
    fn local_sandbox_enables_sandbox() {
        let _guard = lock_env();
        unsafe { std::env::set_var("T3CLAW_PROFILE", "local-sandbox") };
    ...
    fn user_profile_overrides_builtin() {
        ...
        // `t3claw_base_dir()` is cached in a `LazyLock`. Instead we test the
