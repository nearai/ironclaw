...
/// targets when `T3CLAW_OAUTH_PROXY_ALLOW_LOOPBACK=1` is set in the
...
async fn validate_oauth_proxy_url(proxy_url: &str) -> Result<(), String> {
    let allow_loopback = if cfg!(any(test, debug_assertions)) {
        std::env::var("T3CLAW_OAUTH_PROXY_ALLOW_LOOPBACK")
    ...
    match validate_and_resolve_http_target(proxy_url).await {
        ...
        Err(error) => {
                ...
                && ip.is_loopback()
            {
                tracing::debug!(
                    proxy_url = %proxy_url,
                    "Loopback OAuth proxy permitted via T3CLAW_OAUTH_PROXY_ALLOW_LOOPBACK"
                );
                return Ok(());
            }
            Err(error)
...
pub async fn refresh_oauth_access_token(
    ...
    if let Some(proxy_url) = config.exchange_proxy_url.as_deref() {
        ...
        // the operator put in `T3CLAW_OAUTH_EXCHANGE_URL`. Without this,
        ...
        // mock proxy can set `T3CLAW_OAUTH_PROXY_ALLOW_LOOPBACK=1` to
        ...
        // by setting `T3CLAW_OAUTH_EXCHANGE_URL=http://localhost/...`.
