...
/// Build an A2A JSON-RPC 2.0 request body for `message/send`.
///
/// Part format follows A2A v1.0: `{"text": "..."}` without `kind` discriminator.
pub(crate) fn build_jsonrpc_request(
    query: &str,
    context: Option<&serde_json::Value>,
    thread_id: Option<&str>,
) -> serde_json::Value {
    // v1.0: Parts use flat type fields, no "kind" discriminator
    let mut parts = vec![serde_json::json!({ "text": query })];

    if let Some(ctx) = context {
        parts.push(serde_json::json!({ "data": ctx }));
    }
...
pub(crate) fn classify_event(event: &A2aStreamEvent) -> EventKind {
    ...
    // Check for result-level error (v0.x used `kind: "error"`, check both)
    ...
    // v1.0: check status.state for terminal states (replaces deprecated `final` field)
    let state = result
        .get("status")
    ...
    match state {
        Some("completed") => return EventKind::Final(result.clone()),
        Some("failed") | Some("canceled") | Some("rejected") => {
            let msg = result
                .get("status")
            ...
            return EventKind::Error(format!("A2A task {}: {}", state.unwrap_or("failed"), msg));
        }
        _ => {}
    }

    // Backward compat: v0.x `final` field (remove once all servers upgrade)
...
mod tests {
    ...
    fn build_request_with_context_and_thread() {
        ...
        // v1.0: no "kind" discriminator, data part has direct "data" field
    ...
    #[test]
    fn classify_final_event_v1_status_state() {
        // v1.0: uses status.state = "completed", no `final` field
        let event = A2aStreamEvent {
            event_type: None,
            raw: serde_json::json!({
                "result": {
    ...
    #[test]
    fn classify_final_event_legacy_final_field() {
        // Backward compat: v0.x `final: true`
        let event = A2aStreamEvent {
            event_type: None,
            raw: serde_json::json!({
                "result": {
        ...
        let event = A2aStreamEvent {
            ...
            raw: serde_json::json!({
                "result": {
                    "status": {
                        "state": "failed",
                        "message": {"parts": [{"text": "out of memory"}]}
                    }
                }
            }),
        };
    ...
    fn classify_canceled_task() {
        ...
        match classify_event(&event) {
            EventKind::Error(msg) => assert!(msg.contains("canceled")),
            _ => panic!("expected Error"),
        }
    }
