    ...
    let target_name = params
        ...
        .ok_or_else(|| {
            ToolError::InvalidParameters(
                "convert operation requires 'to_timezone'".to_string(),
            )
        })?;
...
fn execute_format(
    ...
    let source_tz = optional_timezone(params, &["from_timezone"])?
        .or_else(|| output_tz.as_ref().map(|(tz, _)| tz.clone()));
    let dt = parse_timestamp(input, source_tz.as_ref())?;
...
mod tests {
    ...
    async fn test_now_accepts_explicit_timezone() {
        ...
        assert!(output.result.get("utc_iso").is_some(), "should have utc_iso");
        assert!(
    ...
    #[tokio::test]
    async fn test_now_uses_context_timezone_fallback() {
        let tool = TimeTool;
