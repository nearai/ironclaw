...
mod tests {
    ...
    #[test]
    fn mcp_tool_annotations_deserialises_hints() {
        use crate::tools::mcp::protocol::McpToolAnnotations;

        let json = r#"{"destructiveHint": true, "readOnlyHint": false}"#;
        let annotations: McpToolAnnotations =
            serde_json::from_str(json).expect("deserialise annotations");

        assert!(
            annotations.destructive_hint,
            "destructive_hint must be true when destructiveHint: true is present"
        );
        assert!(
            !annotations.read_only_hint,
            "read_only_hint must be false when readOnlyHint: false is present"
        );
    }
    ...
    fn mcp_tool_annotations_defaults_when_field_absent() {
        ...
        let annotations: McpToolAnnotations =
            serde_json::from_str(json).expect("deserialise annotations with fields omitted");

        assert!(
            !annotations.read_only_hint,
            "read_only_hint must default to false when the field is absent"
        );
        assert!(
            !annotations.side_effects_hint,
            "side_effects_hint must default to false when the field is absent"
        );
    }
