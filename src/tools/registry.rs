...
impl ToolRegistry {
    ...
    /// Register permission management tool (`tool_permission_set`) and upgrade
    /// the `tool_list` tool to include built-in tool listings.
    ///
    /// Call this **after** `register_extension_tools()` so `tool_list` can be
    /// replaced with an enhanced version that includes the built-in registry.
    /// Requires the registry to be in an `Arc<Self>` so the tool can hold a
    /// weak reference.
    pub fn register_permission_tools(
        self: &Arc<Self>,
        settings_store: Option<Arc<dyn crate::db::SettingsStore + Send + Sync>>,
    ) {
        // Register tool_permission_set.
        self.register_sync(Arc::new(ToolPermissionSetTool::new(
            Arc::clone(self),
            settings_store.clone(),
        )));

        // Replace tool_list with an enhanced version that also lists built-ins.
        // We need to retrieve the existing extension manager from the registered tool.
        // Since we can't downcast `Arc<dyn Tool>` easily, we build the enhanced
        // tool_list by looking up the old one and extracting its manager.
        // Instead, callers that want the full builtin listing should call
        // `upgrade_tool_list_with_registry()` passing the extension manager.
