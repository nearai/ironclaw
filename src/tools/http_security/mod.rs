//! Shared HTTP SSRF defenses (ungated).
//!
//! These helpers are used by built-in HTTP tools, the auth/OAuth module,
//! and the WASM sandbox. They don't depend on wasmtime so they live here.
//! Wasm-specific helpers like `reject_private_ip` (which uses blocking DNS
//! inside WASM host functions) stay in `tools::wasm::http_security`.
...
pub(crate) struct ValidatedHttpTarget {
    ...
    pub(crate) pin_host_resolution: bool,
}

/// Build a reqwest client builder with the SSRF redirect policy applied.
