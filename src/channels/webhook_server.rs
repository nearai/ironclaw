...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Provoke a bind failure by re-binding the port already held by our
        // server (EADDRINUSE).  Unlike port 1, this reliably fails regardless
        // of privilege level — containers and CI often run as root where
        // privileged ports are bindable.
        let occupied_addr = addr1;
        ...
        let app = server
            .merged_router_clone()
            .expect("Router should exist after start()");
        let result = tokio::net::TcpListener::bind(occupied_addr).await;
