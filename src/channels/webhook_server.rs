...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Try to restart on an address that cannot be bound.
        // 0.0.0.0:0 would succeed (kernel picks a port), so use a non-local
        // address (192.0.2.1 from RFC 5737 TEST-NET-1) which is never assigned
        // to this host. Port 1 doesn't work when running as root.
