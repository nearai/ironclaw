...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Guarantee a bind failure by occupying a port, then trying to bind
        // the same port. Using a privileged port (1) doesn't work when running
        // as root in CI containers.
        let blocker = tokio::net::TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind blocker");
        let occupied_addr = blocker.local_addr().expect("blocker addr");
