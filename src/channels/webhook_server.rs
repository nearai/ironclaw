...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Bind to the SAME port the running server already holds — this will
        // fail with EADDRINUSE regardless of privilege level (port 1 succeeds
        // when running as root, which breaks CI containers).
