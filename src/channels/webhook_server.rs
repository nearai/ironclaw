...
mod tests {
    ...
    async fn test_restart_with_addr_rollback_on_bind_failure() {
        ...
        // Try to bind to the same address the server already holds — this
        // reliably produces "address already in use" regardless of privilege level
        // (the original port-1 trick fails when running as root).
        ...
        assert!(result.is_err(), "Bind to already-used address should fail");
