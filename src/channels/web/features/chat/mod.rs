...
mod tests {
    ...
    #[tokio::test]
    async fn test_chat_threads_handler_includes_pending_gates_without_db() {
        let session_manager = Arc::new(SessionManager::new());
        ...
        let thread_id = {
            ...
            thread.await_approval(crate::agent::session::PendingApproval {
                request_id,
        ...
        assert_eq!(response.pending_gates.len(), 1);
        assert_eq!(response.pending_gates[0].request_id, request_id.to_string());
        assert_eq!(response.pending_gates[0].thread_id, thread_id.to_string());
        assert_eq!(response.pending_gates[0].tool_name, "shell");
    }
