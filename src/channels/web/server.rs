...
pub async fn start_server(
    ...
    let same_host_origin: axum::http::HeaderValue = format!("http://{}:{}", addr.ip(), addr.port())
        .parse()
        .map_err(|e| crate::error::ChannelError::StartupFailed {
            name: "gateway".to_string(),
            reason: format!(
                "Failed to parse gateway origin {}:{}: {}",
                addr.ip(),
                addr.port(),
                e
            ),
        })?;
    let localhost_origin: axum::http::HeaderValue = format!("http://localhost:{}", addr.port())
        .parse()
        .map_err(|e| crate::error::ChannelError::StartupFailed {
            name: "gateway".to_string(),
            reason: format!(
                "Failed to parse localhost origin on port {}: {}",
                addr.port(),
                e
            ),
        })?;
    let cors = CorsLayer::new()
        .allow_origin([same_host_origin, localhost_origin])
        .allow_methods([
...
/// production code in this repo avoids panicking on request-path values.
