...
mod tests {
    ...
    async fn test_extensions_setup_submit_returns_failure_when_not_activated() {
        ...
        // Must use the canonical (underscore) form: staging PR #2129
        // changed `ExtensionManager::configure` to canonicalize the name
        // before any file I/O, so a hyphenated channel name no longer
        // matches the on-disk wasm/capabilities files and the call
        // returns Err *before* the activation attempt — defeating the
        // intent of this test (exercising the failure path INSIDE the
        // activation step).
