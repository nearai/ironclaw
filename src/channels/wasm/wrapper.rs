...
mod tests {
    ...
    #[test]
    fn test_http_request_scans_headers_before_host_credential_injection() {
        use super::{ChannelStoreData, ResolvedHostCredential};
        use std::collections::HashMap;
        ...
        let host_credentials = vec![ResolvedHostCredential {
            ...
            secret_value: "sk-proj-TESTFAKEKEY01234567890abcdef".to_string(),
        }];
        ...
        // If host credentials were scanned after injection, this would be a
        // "Potential secret leak blocked" error before the request path reached
        // method dispatch. Hitting the unsupported-method branch proves the
        // caller scanned the pre-injection headers.
        let error = result.expect_err("unsupported method should fail after leak scan");
        assert!(
            !error.contains("Potential secret leak blocked"),
            "host-injected credential should not trigger request leak scan: {error}"
        );
        assert!(
