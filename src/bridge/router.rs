...
pub async fn handle_expected(
    ...
    //
    // If the ensure step fails we still attempt to fire — a transient store
    // failure should not block feedback capture, and any pre-existing missions
    // for this user would still fire. But we remember that the ensure failed
    // so we can return a clearer "feedback captured but self-improvement
    // unavailable" message instead of the generic "no missions configured"
    // fallback, which would be misleading in that case.
    let mgr = state.effect_adapter.mission_manager().await;
    let mut ensure_failed = false;
    ...
    } else if ensure_failed {
        Ok(BridgeOutcome::Respond(
            "Feedback captured, but self-improvement is temporarily unavailable. \
