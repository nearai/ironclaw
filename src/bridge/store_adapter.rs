        ...
        for entry in entries {
            ...
            let project = match ws.read(&meta_path).await {
                ...
                Err(_) => {
                    ...
                    // and the project dashboard both see it. Scoped to the
                    // workspace owner so `list_projects(user_id)` returns
                    // it instead of filtering it out.
                    synth_bare_project(slug, ws.user_id())
                }
            };
...
/// wrote this slug, so it's what the model expects to see).
///
/// `user_id` is the owner of the workspace we're loading from, so the
/// synthesized project is correctly scoped to that user — `list_projects`
/// filters by `user_id`, and a bare project under `shared_owner_id` would
/// be invisible to its real owner and globally visible to everyone else.
fn synth_bare_project(slug: &str, user_id: &str) -> Project {
    let now = chrono::Utc::now();
    Project {
