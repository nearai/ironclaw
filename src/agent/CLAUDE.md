...
- The agent loop spawns concurrent message-handling tasks bounded by two layers: a per-user limit (`max_parallel_threads_per_user`, default 5) and a global limit (`max_parallel_threads`, default 20). The per-user check is **non-blocking** (`try_acquire_owned`): if the user is at capacity the message is immediately rejected with a rate-limit notice, keeping the main loop responsive for other users. The global semaphore is acquired **inside each spawned task** (not in the main loop) so the message stream is never starved when global capacity is exhausted. Per-user semaphores are managed by `TenantRateRegistry` (same pattern as LLM/job concurrency). The session mutex serializes state mutation, but it is not held for the full duration of a turn: long-running work (LLM calls, tool execution, I/O) can proceed concurrently across multiple threads for the same user, with contention only during lock-held sections. Same-thread messages queue via the `ThreadState::Processing` gate + TOCTOU re-check under lock. Spawned tasks are wrapped in `catch_unwind` — if a task panics, the thread state is reset from `Processing` to `Idle` via `fail_turn` so it doesn't become permanently wedged.
- **SSE event ordering**: With concurrent message handling, SSE events from different threads can interleave in the event stream. A `Thinking` event from thread B can arrive between thread A's response text and thread A's `Done` event. The web UI filters by `thread_id`, which mitigates this for the active thread view. Monitoring or logging tools that consume the raw SSE stream must **not** assume temporal ordering of events across threads. Response ordering across threads/users, including different threads for the same user, is **not** guaranteed.
