...
- The agent loop spawns concurrent message-handling tasks bounded by two semaphore layers: a per-user limit (`max_parallel_threads_per_user`, default 5) prevents a single user from exhausting all slots, and a global limit (`max_parallel_threads`, default 20) caps total concurrency. Per-user semaphores are managed by `TenantRateRegistry` (same pattern as LLM/job concurrency). The session mutex serializes state mutation, but it is not held for the full duration of a turn: long-running work (LLM calls, tool execution, I/O) can proceed concurrently across multiple threads for the same user, with contention only during lock-held sections. Same-thread messages queue via the `ThreadState::Processing` gate + TOCTOU re-check under lock. Response ordering across threads/users, including different threads for the same user, is **not** guaranteed.
