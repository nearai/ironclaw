...
- The agent loop spawns concurrent message-handling tasks bounded by two layers: a per-user limit (`max_parallel_threads_per_user`, default 5) and a global limit (`max_parallel_threads`, default 20). The per-user check is **non-blocking** (`try_acquire_owned`): if the user is at capacity the message is immediately rejected with a rate-limit notice, keeping the main loop responsive for other users. The global semaphore uses a blocking `acquire_owned` inside `tokio::select!` (with shutdown signals) since it represents actual system capacity. Per-user semaphores are managed by `TenantRateRegistry` (same pattern as LLM/job concurrency). The session mutex serializes state mutation, but it is not held for the full duration of a turn: long-running work (LLM calls, tool execution, I/O) can proceed concurrently across multiple threads for the same user, with contention only during lock-held sections. Same-thread messages queue via the `ThreadState::Processing` gate + TOCTOU re-check under lock. Response ordering across threads/users, including different threads for the same user, is **not** guaranteed.
