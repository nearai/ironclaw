...
pub enum HydrationError {
    ...
    #[error("User {user_id} does not have access to conversation {conversation_id}")]
    AccessDenied {
...
impl Agent {
    ...
    pub(super) async fn process_auth_token(
        ...
        match ext_mgr.auth(&pending.extension_name).await {
...
/// Rebuild chat messages from database records, including tool call history.
///
/// This function properly reconstructs the conversation context from database
/// records, handling user messages, assistant messages, and enriched tool_calls
/// rows that contain call_id, parameters, and results.
...
mod tests {
    ...
    // Helper for rebuild_chat_messages_from_db tests
