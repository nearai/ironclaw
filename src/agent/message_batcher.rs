...
/// with global defaults from the provided config.
pub struct MessageBatcher {
    ...
    /// Global batching config (from AgentConfig).
    global_config: BatchingConfig,
}

impl MessageBatcher {
    /// Create a new message batcher with the given global configuration.
    ///
    /// The global config provides defaults that per-channel config can override.
    /// If `global_config.enabled` is false, all batching is disabled.
    pub fn new(global_config: BatchingConfig) -> Self {
    ...
    ///
    /// If global batching is disabled, the message is immediately sent to subscribers.
    /// Otherwise, per-channel config is used to determine batching behavior.
    ...
    pub async fn push(&self, message: IncomingMessage) {
        // Global kill switch - if batching is disabled globally, pass through immediately
