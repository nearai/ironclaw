...
mod tests {
    ...
    fn run_with_completion_stack(test: impl FnOnce() + Send + 'static) {
        std::thread::Builder::new()
            .stack_size(16 * 1024 * 1024)
            .spawn(test)
            .expect("completion test thread should spawn")
            .join()
            .expect("completion test should not panic");
    }

    #[test]
    fn test_run_generates_output() {
        run_with_completion_stack(|| {
