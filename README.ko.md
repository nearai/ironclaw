...
- 소스 빌드에는 Rust 1.96+
- `webui-v2-beta`를 활성화한 소스 빌드에는 Node.js 22+와 Corepack/pnpm
- 선택한 LLM 공급자의 자격 증명
- PostgreSQL 15+는 호스팅/프로덕션 profile에서만 선택 사항이며 local-dev에는 필요하지 않음
...
표준 `ironclaw` 명령은 Reborn을 실행합니다. 초기화는
`~/.ironclaw/reborn` 아래에 격리된 설정을 만들며 v1 데이터베이스, 설정 또는 키체인을 수정하지 않습니다:
...
로컬 실행에는 PostgreSQL을 수동으로 만들 필요가 없습니다. 호스팅 환경에서 PostgreSQL을 선택할 수 있지만 연결 URL과 마스터 키는 기존 `IRONCLAW_REBORN_*` 환경 변수로 제공해야 합니다. 자세한 내용은 영어 README와 [Reborn onboarding](docs/reborn/onboarding.md)을 참조하세요.
...
CLI에서 공급자와 모델을 선택한 다음 안내된 자격 증명 환경 변수를 설정하세요:
...
공급자 검색과 선택은 [canonical CLI 모델 가이드](docs/reborn-binary.md#models)를 참조하세요.
...
- Runtime 상태는 기본적으로 로컬에 저장되며 호스팅/프로덕션 profile에서는 PostgreSQL을 선택할 수 있습니다
...
현재 릴리스는 Reborn 런타임을 기본으로 사용하며 표준 CLI 명령은 `ironclaw`입니다. `ENGINE_V2`를 설정할 필요가 없습니다.

```bash
# Reborn home, 설정 파일 및 WebUI token 초기화
ironclaw onboard

# 단일 메시지를 보내거나 대화형 REPL 시작
ironclaw run --message "안녕하세요"
ironclaw repl
