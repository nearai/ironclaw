...
class RebornWebUiV2LiveQaRunnerTests(unittest.TestCase):
    ...
    def _drive_submission_capture_state(
        ...
        *,
    ...
    ) -> tuple[run_live_qa.ProbeResult, dict[str, object]]:
        state = {
            "presses": 0,
            "dismissals": 0,
        ...
        assistant_replies = assistant_replies or ["Slack answer."]

        class FakeRequest:
            method = "POST"

            def __init__(self, content):
                self.post_data_json = {"content": content}
        ...
        class FakeResponse:
            ...
            def __init__(self, payload, content):
                ...
                self.request = FakeRequest(content)
        ...
        class FakeResponseInfo:
            ...
            def value(self):
                async def get_response():
                    ...
                    response = FakeResponse(self.payload, state["current_content"])
                    self_outer.assertTrue(self.predicate(response))
                    return response
        ...
        class FakePage:
            ...
            def expect_response(self, predicate, **_kwargs):
                ...
                return FakeResponseInfo(payload, predicate)
        ...
        async def fake_wait(_page, **_kwargs):
            ...
            reply_index = min(state["assistant_waits"], len(assistant_replies) - 1)
            state["assistant_waits"] += 1
            reply_text = assistant_replies[reply_index]
            return run_live_qa.AssistantReplyWaitResult(
                text_excerpt=reply_text,
        ...
        with (
            ...
            result = asyncio.run(
                run_live_qa._live_chat_case(
                    ...
                    routine_confirmation_follow_up=routine_confirmation_follow_up,
                )
            )
        return result, state
    ...
    def test_result_preserves_explicit_nonblocking_infrastructure_classification(self):
        result = run_live_qa._result(
    ...
    def test_follow_up_already_submitted_records_its_own_identity(self):
        submitted = {
            "outcome": "submitted",
            "thread_id": "thread-current",
            "accepted_message_ref": "msg:message-primary",
        ...
        replayed_follow_up = {
            "outcome": "already_submitted",
            "thread_id": "thread-current",
            "accepted_message_ref": "msg:message-follow-up",
            "run_id": "run-follow-up",
            "status": "running",
        }

        result, state = self._drive_submission_capture_state(
            response_payloads=[submitted, replayed_follow_up],
            assistant_replies=[
                "I can create that routine. Would you like me to go ahead?",
                "Routine created.",
            ],
            routine_confirmation_follow_up=True,
        )
        ...
        self.assertEqual(state["presses"], 2)
        self.assertEqual(
            result.details["submission_identity"]["run_id"],
            "run-primary",
        )
        self.assertEqual(
            [identity["run_id"] for identity in result.details["submission_identities"]],
            ["run-primary", "run-follow-up"],
        )
    ...
    def test_qa_6d_uses_unique_sheet_fixture_in_routine_prompt(self):
        captured_fixture: dict[str, object] = {}
        captured_routine: dict[str, object] = {}
        ...
        async def fake_create_google_spreadsheet_fixture(**kwargs):
            ...
            return {
                "spreadsheet_id": "crm-sheet-123",
                "spreadsheet_url": (
                    "https://docs.google.com/spreadsheets/d/crm-sheet-123/edit"
                ),
                "title": kwargs["title"],
                "sheet_name": "Sheet1",
                "values_written": True,
        ...
        async def fake_routine_creation_case(_ctx, **kwargs):
            ...
            return run_live_qa.ProbeResult(
                provider="test",
                mode="live:qa_6d_gmail_to_sheet_routine",
                success=True,
                latency_ms=1,
                details=kwargs.get("extra_details") or {},
            )
        ...
        with (
            ...
            result = asyncio.run(
                run_live_qa.case_qa_6d_gmail_to_sheet_routine(self._dummy_ctx())
            )
        ...
        self.assertEqual(
            captured_fixture["values"],
            [["Source", "Summary", "QA Marker"]],
        )
        self.assertRegex(str(captured_fixture["title"]), r"^REBORN_QA_6D_CRM_SHEET_\d+$")
        prompt = str(captured_routine["prompt"])
        self.assertIn("crm-sheet-123", prompt)
        self.assertIn(str(captured_fixture["title"]), prompt)
        self.assertIn("Do not search for a sheet named ABC", prompt)
        self.assertEqual(
            captured_routine["extra_details"]["crm_sheet_fixture"]["spreadsheet_id"],
            "crm-sheet-123",
        )

    def test_qa_7e_uses_direct_fixture_and_exact_append_instruction(self):
        captured_event: dict[str, object] = {}

        async def fake_create_google_spreadsheet_fixture(**kwargs):
            return {
                "spreadsheet_id": "bug-sheet-123",
                "spreadsheet_url": (
                    "https://docs.google.com/spreadsheets/d/bug-sheet-123/edit"
                ),
                "title": kwargs["title"],
                "sheet_name": "Sheet1",
                "values_written": True,
        ...
        async def fake_post_signed_slack_dm_event(_ctx, **kwargs):
            ...
            return {"event_id": kwargs["event_id"], "status_code": 200}
        ...
        with (
            ...
            patch.object(
                run_live_qa,
                "_live_chat_with_extensions_case",
                side_effect=AssertionError("QA 7E fixture setup must not use chat"),
            ),
            patch.object(
                run_live_qa,
                "_slack_preflight",
            ...
            patch.object(run_live_qa, "_slack_delivery_channel_id", return_value="D123"),
            patch.object(
                run_live_qa,
            ...
            result = asyncio.run(
                run_live_qa.case_qa_7e_slack_bug_sheet_delivery(self._dummy_ctx())
            )

        self.assertTrue(result.success)
        event_text = str(captured_event["text"])
        self.assertIn("google-sheets.append_values", event_text)
        self.assertIn('"spreadsheet_id":"bug-sheet-123"', event_text)
        self.assertIn('"range":"Sheet1!A:E"', event_text)
        self.assertIn("Do not inspect spreadsheet metadata first", event_text)
        self.assertEqual(
            result.details["sheet_fixture"]["spreadsheet_id"],
            "bug-sheet-123",
        )
    ...
    def test_live_google_side_effect_cases_install_required_extensions(self):
        ...
        self.assertEqual(
            captured["qa_4e_github_release_email_delivery"]["required_text"],
            [],
        )
        self.assertFalse(
            captured["qa_4e_github_release_email_delivery"]["enforce_marker"]
        )
    ...
    def test_slack_strategy_setup_prompt_avoids_connect_action_trigger(self):
        ...
        for case_name in ("qa_5d_slack_strategy_doc_answer",):
    ...
    def test_qa_10d_classifies_invalid_fixture_and_ground_truth_mismatch(self):
        ...
        self.assertEqual(invalid_fixture.details["failure_status"], "inconclusive")
        self.assertTrue(invalid_fixture.details["inconclusive"])
        self.assertFalse(invalid_fixture.details["blocking"])
    ...
    def test_qa_10g_scoped_contract_requires_new_history_call_and_global_preserves_prompt(
        ...
        def drive_scoped(statuses):
            ...
            indexed_search.assert_not_awaited()
        ...
        with (
            ...
            patch.object(
                ...
                new=AsyncMock(
                    return_value={"indexed": True, "attempts": 1, "latency_ms": 0}
                ),
                create=True,
            ),
            patch.object(
    ...
    def test_slack_search_index_polling_observes_hit_without_real_sleep(self):
        observations = [
            {"checked": True, "hits": []},
            {"checked": True, "hits": []},
            {"checked": True, "hits": [{"channel_id": "D0FIXTURE1"}]},
        ]
        with (
            patch.object(
                run_live_qa,
    ...
    def test_slack_search_index_timeout_is_bounded_and_does_not_sleep_past_deadline(
        self,
    ):
    ...
    def test_global_last_sent_skips_model_when_slack_index_is_stale(self):
        ...
        self.assertFalse(result.details["blocking"])
        self.assertEqual(
            result.details["slack_search_index"],
            {"indexed": False, "attempts": 3, "latency_ms": 20},
        )
        self.assertEqual(index_wait.await_args.kwargs["timeout"], 90.0)
        chat.assert_not_awaited()

    def test_global_last_sent_indexed_answer_mismatch_is_model_quality(self):
        async def fake_seed(_token, _channel_id, text, **_kwargs):
            self.assertTrue(text.startswith("LASTSENT_GLOBAL_"))
            return {"ok": True}

        wrong_reply = "an older Slack message"
        with (
            patch.object(
                run_live_qa,
                "_require_slack_personal_token",
            ...
            patch.object(
                ...
                "_wait_for_slack_search_index",
                new=AsyncMock(
                    return_value={"indexed": True, "attempts": 2, "latency_ms": 10}
                ),
                create=True,
            ),
            patch.object(
                ...
                new=AsyncMock(
                    return_value=(
                        run_live_qa.ProbeResult(
                            provider="test",
                            mode="live:qa_10g_slack_last_message_sent_global",
                            success=True,
                            latency_ms=1,
                            details={"text_excerpt": wrong_reply},
                        ),
                        wrong_reply,
                    )
                ),
        ...
        self.assertFalse(result.success)
        self.assertEqual(result.details["failure_class"], "model_quality")
        self.assertEqual(result.details["failure_category"], "answer_mismatch")
    ...
    def test_qa_10_fixture_preflights_are_nonblocking_inconclusive(self):
        ...
        for case_fn in (
            ...
            with self.subTest(case=case_fn.__name__):
                with (
                    patch.dict(os.environ, blank_env, clear=False),
                    patch.object(
                        run_live_qa,
                        "_slack_correctness_chat_reply",
                        side_effect=AssertionError(
                            "model call must not run for a fixture preflight"
                        ),
                    ),
                ):
                ...
                self.assertEqual(
                    result.details.get("failure_status"),
                    "inconclusive",
                    case_fn.__name__,
                )
                self.assertTrue(result.details.get("inconclusive"), case_fn.__name__)
                self.assertFalse(result.details.get("blocking"), case_fn.__name__)
    ...
    def test_slack_delivery_exhausted_evidence_reads_are_inconclusive(self):
        async def fake_creation(_ctx, **kwargs):
            return run_live_qa.ProbeResult(
                provider="test",
                mode=f"live:{kwargs['case_name']}",
        ...
        async def fake_delivery(*_args, **_kwargs):
            return {
                "slack_history": {"found": True},
                "trigger_run": {"run_id": "run-1"},
                "delivery_outcome": {"outcome": "delivered"},
            }

        async def unreadable_history(*_args, **_kwargs):
            return {"found": False, "error": "transient Slack read failure"}

        with (
            patch.object(run_live_qa, "_routine_creation_case", side_effect=fake_creation),
            patch.object(
                run_live_qa,
                "_wait_for_slack_delivery_marker",
            ...
            patch.object(
                run_live_qa,
                "_slack_history_contains_marker",
                side_effect=unreadable_history,
            ),
            patch.object(run_live_qa.asyncio, "sleep", new=AsyncMock()),
        ):
            result = asyncio.run(
                run_live_qa._slack_delivery_routine_case(
                    self._dummy_ctx(),
                    ...
                    routine_instruction="send the marker to Slack",
                    required_delivery_text=["status"],
                    exactly_once_grace_seconds=0.0,
                )
            )
    ...
    def test_slack_delivery_exhausted_workspace_sweep_is_inconclusive(self):
        async def fake_creation(_ctx, **kwargs):
            return run_live_qa.ProbeResult(
                provider="test",
                mode=f"live:{kwargs['case_name']}",
            ...
            patch.object(
                ...
                new=AsyncMock(
                    return_value={
                        "checked": False,
                        "permanent": False,
                        "error": "transient search failure",
                    }
                ),
            ),
            patch.object(run_live_qa.asyncio, "sleep", new=AsyncMock()),
    ...
    def test_run_cases_marks_missing_slack_delivery_target_as_precondition(self):
        async def fake_case(_ctx: run_live_qa.LiveQaContext) -> run_live_qa.ProbeResult:
            raise AssertionError("case should not run without a Slack delivery target")

        async def fail_start_reborn_server(*_args, **_kwargs):
            raise AssertionError("server should not start without a Slack delivery target")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cases = {
                "case_slack_delivery": run_live_qa.CaseSpec(
                    fake_case,
            ...
            prepared = run_live_qa.PreparedRebornHome(
                path=prepared_home,
                preflight={
                    "slack": {"delivery_target_present": False},
                    "slack_personal_auth": {},
                    "google_product_auth": {},
                    "telegram": {},
                    "github_auth": {},
            ...
            result = payload["results"][0]
            self.assertFalse(result["success"])
            self.assertEqual(result["details"]["case"], "case_slack_delivery")
            self.assertIn("outbound delivery target", result["details"]["error"])
            self.assertEqual(result["details"]["failure_class"], "precondition")
            self.assertEqual(result["details"]["failure_status"], "inconclusive")
            self.assertTrue(result["details"]["inconclusive"])
            self.assertFalse(result["details"]["blocking"])
