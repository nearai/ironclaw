#!/usr/bin/env bash
# Launch IronClaw with the WebChat v2 web UI for local testing.
#
# Handles the setup footguns from docs/ironclaw-binary.md for you:
#   - keeps the IronClaw home OUTSIDE the repo (serve uses the cwd as the
...
#
# Usage:
#   scripts/run-ironclaw-webui.sh                 # NEAR AI defaults
#   PROVIDER=openai scripts/run-ironclaw-webui.sh
#   PROVIDER=anthropic MODEL=claude-sonnet-4-20250514 scripts/run-ironclaw-webui.sh
#
...
#   IRONCLAW_SERVE_HOST   listen host (default: 127.0.0.1)
#   IRONCLAW_SERVE_PORT   listen port (default: 3000)
#   IRONCLAW_HOME             (default: $HOME/.ironclaw-demo)
#   IRONCLAW_WEBUI_USER_ID    (default: home's [identity].default_owner)
#   IRONCLAW_WEBUI_TOKEN      (default: generated and printed)
#
# The retired REBORN_HOST/REBORN_PORT spellings remain fallback aliases. A bare
# HOST would collide with zsh's auto-set machine hostname.
...
MODEL="${MODEL:-}"
IRONCLAW_SERVE_HOST="${IRONCLAW_SERVE_HOST:-${REBORN_HOST:-127.0.0.1}}"
IRONCLAW_SERVE_PORT="${IRONCLAW_SERVE_PORT:-${REBORN_PORT:-3000}}"
...
# would print an unusable URL here.
if [ "$IRONCLAW_SERVE_PORT" = "0" ]; then
  echo "error: IRONCLAW_SERVE_PORT=0 (kernel-assigned port) isn't usable for browser onboarding." >&2
  echo "       Set a fixed IRONCLAW_SERVE_PORT, or run the test-harness form directly:" >&2
...
export IRONCLAW_HOME="${IRONCLAW_HOME:-$HOME/.ironclaw-demo}"
...
# and symlinks, like serve does) so e.g. `../ironclaw-home` isn't mis-flagged.
...
if [ -n "$home_parent" ]; then
  ...
  case "$home_canonical/" in
    "$repo_canonical"/*)
      ...
      echo "       Point it somewhere else, e.g. \$HOME/.ironclaw-demo." >&2
      exit 1
      ;;
  esac
fi
...
==> Starting WebChat v2 on http://$IRONCLAW_SERVE_HOST:$IRONCLAW_SERVE_PORT/
    login token : $IRONCLAW_WEBUI_TOKEN
    login user  : $IRONCLAW_WEBUI_USER_ID
    IronClaw home: $IRONCLAW_HOME

EOF

exec "${CARGO[@]}" serve --host "$IRONCLAW_SERVE_HOST" --port "$IRONCLAW_SERVE_PORT"
