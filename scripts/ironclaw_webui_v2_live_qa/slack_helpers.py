"""Slack setup helpers for the IronClaw WebUI v2 live QA runner."""
...
SLACK_SIGNING_SECRET_ENV = "IRONCLAW_SLACK_SIGNING_SECRET"
SLACK_BOT_TOKEN_ENV = "IRONCLAW_SLACK_BOT_TOKEN"
SLACK_OAUTH_CLIENT_ID_ENV = "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID"
SLACK_OAUTH_CLIENT_SECRET_ENV = "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_SECRET"
...
def _has_persisted_slack_personal_dm_target(ironclaw_home: Path, user_id: str) -> bool:
    return _persisted_slack_personal_dm_payload(ironclaw_home, user_id) is not None


def _persisted_slack_personal_dm_payload(ironclaw_home: Path, user_id: str) -> dict[str, object] | None:
...
def _persisted_slack_personal_dm_channel_id(ironclaw_home: Path, user_id: str) -> str | None:
    payload = _persisted_slack_personal_dm_payload(ironclaw_home, user_id)
    if payload is None:
        return None
...
def _slack_setup_from_ironclaw_home(ironclaw_home: Path) -> dict[str, object] | None:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
    if not db_path.exists():
        return None
    try:
...
def _slack_setup_preflight(
    ...
    setup = _slack_setup_from_ironclaw_home(ironclaw_home) or {}
    ...
    return {
        "source": "ironclaw_home" if setup else "env",
        "configured": bool(
            installation_id
            and team_id
            and api_app_id
...
def _slack_setup_payload(
    ...
    *,
    ...
    preflight = _slack_setup_preflight(ironclaw_home, config_text, extra_env)
    setup = _slack_setup_from_ironclaw_home(ironclaw_home) or {}
    ...
    resolved_bot_user_id = str(bot_user_id or "").strip() or _slack_setup_field(
        ...
        config_text,
        "bot_user_id",
        "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_BOT_USER_ID",
    )
    resolved_shared_subject_user_id = (
        ...
        or _slack_setup_field(
            ...
            config_text,
            "shared_subject_user_id",
            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_SHARED_SUBJECT_USER_ID",
        )
    )
    required = {
...
def _has_slack_delivery_target(config_text: str, ironclaw_home: Path, user_id: str) -> bool:
    return _has_persisted_slack_personal_dm_target(ironclaw_home, user_id)


def _materialize_slack_env_from_ironclaw_home(
    ironclaw_home: Path,
    config_text: str,
...
def _seed_generated_slack_product_auth_if_configured(
    ...
    installation = _slack_env_field(
        [
            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INSTALLATION_ID",
            "IRONCLAW_SLACK_INSTALLATION_ID",
        ],
        extra_env,
    )
    api_app = _slack_env_field(
        [
            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_API_APP_ID",
            "IRONCLAW_SLACK_APP_ID",
            "IRONCLAW_SLACK_API_APP_ID",
        ],
        extra_env,
    )
    if not team_id or not slack_user_id or not installation or not api_app:
        ...
        if not installation:
            missing.append("IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INSTALLATION_ID")
        if not api_app:
            missing.append("IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_API_APP_ID")
    ...
    account_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/slack_personal/{user_id}",
        )
    )
    invocation_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/slack-personal-invocation/{user_id}",
        )
    )
    thread_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/slack-personal-thread/{user_id}",
        )
    )
