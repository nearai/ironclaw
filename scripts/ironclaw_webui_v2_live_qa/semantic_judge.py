"""Semantic completion judge for IronClaw WebUI v2 live QA text checks."""
...
async def _judge_assistant_reply_completion(
    *,
    ...
    if not _env_flag_enabled("IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE", default=True):
...
def _semantic_judge_passed(result: dict[str, object] | None) -> bool:
    ...
    try:
        threshold = float(
            os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE_MIN_CONFIDENCE", "0.75")
        )
    except ValueError:
        threshold = 0.75
...
def _judge_api_key_env() -> str:
    return os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE_API_KEY_ENV",
        os.environ.get(
...
def _judge_base_url() -> str:
    return os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE_BASE_URL",
        os.environ.get(
            "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_BASE_URL",
...
def _judge_model() -> str:
    return os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE_MODEL",
        os.environ.get(
