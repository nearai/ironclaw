...
def refresh_access_token() -> Tuple[Optional[str], str]:
    client_id = _secret("IRONCLAW_GOOGLE_CLIENT_ID")
    client_secret = _secret("IRONCLAW_GOOGLE_CLIENT_SECRET")
    refresh_token = _secret("AUTH_LIVE_GOOGLE_REFRESH_TOKEN")
    missing = [
        name
        for name, value in (
            ("IRONCLAW_GOOGLE_CLIENT_ID", client_id),
            ("IRONCLAW_GOOGLE_CLIENT_SECRET", client_secret),
