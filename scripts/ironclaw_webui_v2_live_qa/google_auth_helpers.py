"""Google product-auth helpers for IronClaw WebUI v2 live QA."""
...
from scripts.ironclaw_webui_v2_live_qa.env_helpers import (
    _env_present,
    _first_env_value,
    _non_empty_env,
    _section_env_name,
)
from scripts.ironclaw_webui_v2_live_qa.errors import LiveQaError
from scripts.ironclaw_webui_v2_live_qa.root_filesystem import (
...
DEFAULT_USER_ID = "ironclaw-webui-v2-live-qa-user"
...
def _google_required_env_for_block(
    ...
    *,
    ...
    required = ["IRONCLAW_GOOGLE_CLIENT_ID"]
    if preflight.get("missing_google_client_secret"):
        required.append("IRONCLAW_GOOGLE_CLIENT_SECRET")
    if requires_runtime_access or preflight.get("refresh_probe_failed"):
        ...
        if "IRONCLAW_GOOGLE_CLIENT_SECRET" not in required:
            required.append("IRONCLAW_GOOGLE_CLIENT_SECRET")
    return required
...
def _google_credential_action_for_block(preflight: dict[str, object]) -> str | None:
    if _google_refresh_probe_error(preflight) == "invalid_grant":
        return (
            ...
            "by IRONCLAW_GOOGLE_CLIENT_ID and "
            "IRONCLAW_GOOGLE_CLIENT_SECRET."
        )
    if (
        ...
        return (
            "Set IRONCLAW_GOOGLE_CLIENT_SECRET to the Google Cloud OAuth "
            "client secret matching IRONCLAW_GOOGLE_CLIENT_ID."
        )
    return None


def _stored_google_oauth_client_id_from_ironclaw_home(ironclaw_home: Path) -> tuple[str, str] | None:
...
def _materialize_google_oauth_env_for_ironclaw(
    ...
    if not client_id and ironclaw_home is not None:
        client_id = _stored_google_oauth_client_id_from_ironclaw_home(ironclaw_home)
    if client_id:
        materialized["IRONCLAW_GOOGLE_CLIENT_ID"] = client_id[1]
    ...
    if client_secret:
        materialized["IRONCLAW_GOOGLE_CLIENT_SECRET"] = client_secret[1]
    ...
    if redirect_uri:
        materialized["IRONCLAW_GOOGLE_OAUTH_REDIRECT_URI"] = redirect_uri[1]
    ...
    if hosted_domain:
        materialized["IRONCLAW_GOOGLE_HOSTED_DOMAIN_HINT"] = hosted_domain[1]
...
def _google_oauth_refresh_probe(
    ...
    if os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_SKIP_GOOGLE_REFRESH_PROBE"):
    ...
    master_key_path = ironclaw_home / "local-dev" / ".reborn-local-dev-secrets-master-key"
    if not master_key_path.exists():
        return {
            "checked": True,
            "ok": False,
            "error": "ironclaw_secret_master_key_missing",
...
def _google_runtime_access_token(
    ...
    if not db_path.exists() or not master_key_path.exists():
        ...
        raise LiveQaError("Google runtime token unavailable: IronClaw DB or secret key missing")
    ...
    if not client_id:
        stored_client_id = _stored_google_oauth_client_id_from_ironclaw_home(ironclaw_home)
        if stored_client_id:
    ...
    for _path, raw in account_rows:
        ...
        if access_secret and (expires_dt is None or expires_dt > now):
            return _decrypt_filesystem_secret(
                ...
                "source": "ironclaw_product_auth_access_secret",
                "refreshed": False,
        ...
        if response.status_code < 400 and access_token:
            return access_token, {
                "source": "ironclaw_product_auth_refresh_secret",
                "refreshed": True,
                "account_id": account_id,
...
def _seed_generated_google_product_auth_if_configured(ironclaw_home: Path, user_id: str) -> dict[str, object]:
    ...
    account_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/google/{user_id}",
        )
    )
    invocation_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/invocation/{user_id}",
        )
    )
    thread_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/thread/{user_id}",
        )
    )
    now = datetime.now(timezone.utc)
