"""Telegram and GitHub auth helpers for IronClaw WebUI v2 live QA."""
...
def _materialize_telegram_env_for_ironclaw(
    ...
    webhook_secret = _first_env_value(
        [
            ...
            "IRONCLAW_TELEGRAM_WEBHOOK_SECRET",
    ...
    chat_id = _first_env_value(
        [
            "IRONCLAW_WEBUI_V2_LIVE_QA_TELEGRAM_CHAT_ID",
    ...
    if chat_id:
        materialized["IRONCLAW_WEBUI_V2_LIVE_QA_TELEGRAM_CHAT_ID"] = chat_id[1]
    return materialized, {
...
def _telegram_preflight(
    ...
    *,
    ...
    return {
        ...
        "chat_id_present": _env_present("IRONCLAW_WEBUI_V2_LIVE_QA_TELEGRAM_CHAT_ID", extra_env),
        "copied_ironclaw_home_mentions_telegram": copied_home_mentions,
    ...
    *,
    ...
    if requires_github_auth and not ready:
        reason = (
            ...
            "account or PAT-seeded IronClaw home"
        )
    return {
...
def _seed_generated_github_product_auth_if_configured(ironclaw_home: Path, user_id: str) -> dict[str, object]:
    token_names = [
        "AUTH_LIVE_GITHUB_TOKEN",
        "IRONCLAW_GITHUB_TOKEN",
    ...
    account_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/github/{user_id}",
        )
    )
    invocation_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/github-invocation/{user_id}",
        )
    )
    thread_id = str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"ironclaw-webui-v2-live-qa/github-thread/{user_id}",
        )
    )
