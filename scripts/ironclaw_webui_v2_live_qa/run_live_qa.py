"""Live QA runner for IronClaw WebUI v2.
...
from scripts.ironclaw_webui_v2_live_qa.case_matrix import (  # noqa: E402
    CaseFn,
    CaseSpec,
    QA_SHEET_CASES,
    QA_SHEET_TAB,
...
from scripts.ironclaw_webui_v2_live_qa.errors import LiveQaError  # noqa: E402
from scripts.ironclaw_webui_v2_live_qa.external_auth_helpers import (  # noqa: E402
    _github_auth_preflight,
    _materialize_telegram_env_for_ironclaw,
    _seed_generated_github_product_auth_if_configured,
    _telegram_preflight,
)
from scripts.ironclaw_webui_v2_live_qa.env_helpers import (  # noqa: E402
    _env_present,
    _env_value,
    _first_env_value,
    _non_empty_env,
...
from scripts.ironclaw_webui_v2_live_qa.google_api_helpers import (  # noqa: E402
...
from scripts.ironclaw_webui_v2_live_qa.google_auth_helpers import (  # noqa: E402
    ...
    _materialize_google_oauth_env_for_ironclaw,
    _seed_generated_google_product_auth_if_configured,
)
from scripts.ironclaw_webui_v2_live_qa.root_filesystem import (  # noqa: E402
...
from scripts.ironclaw_webui_v2_live_qa.semantic_judge import (  # noqa: E402
    _compact_json,
    _judge_assistant_reply_completion,
    _semantic_judge_passed,
)
from scripts.ironclaw_webui_v2_live_qa.slack_helpers import (  # noqa: E402
    SLACK_BOT_TOKEN_ENV,
    ...
    _materialize_slack_env_from_ironclaw_home,
...
from scripts.ironclaw_webui_v2_live_qa.text_match import (  # noqa: E402
...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "ironclaw-webui-v2-live-qa"
DEFAULT_IRONCLAW_HOME = Path("/tmp/ironclaw-real-slack")
AUTH_TOKEN = "ironclaw-webui-v2-live-qa-token-0123456789abcdef"
...
# override via IRONCLAW_WEBUI_V2_LIVE_QA_CASE_ATTEMPTS.
try:
    LIVE_QA_CASE_ATTEMPTS = max(
        1, int(os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_CASE_ATTEMPTS", "2"))
    )
except ValueError:
...
class LiveQaContext:
    def __init__(
        ...
        *,
        ...
        self.ironclaw_home = ironclaw_home
        self.env = env


@dataclass
class PreparedIronClawHome:
    path: Path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run live Playwright QA checks against IronClaw WebUI v2."
    )
    parser.add_argument(
        "--case",
        action="append",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_HOME", DEFAULT_IRONCLAW_HOME)
        ),
        help=(
            "Source IronClaw home to copy for the run. Defaults to "
            "IRONCLAW_WEBUI_V2_LIVE_QA_HOME or /tmp/ironclaw-real-slack."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def _ironclaw_binary() -> Path:
    return _cargo_target_dir() / "debug" / "ironclaw"


def build_ironclaw_binary() -> Path:
    features = os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_FEATURES", "")
...
def _config_text(path: Path) -> str:
    try:
        ...
        raise LiveQaError(f"failed to read IronClaw config {path}: {exc}") from exc
...
def _write_minimal_ironclaw_config(path: Path, *, include_slack: bool) -> None:
    ...
    if not api_key:
        raise LiveQaError(
            f"IronClaw home is missing config.toml and {api_key_env} is unset; "
            "set IRONCLAW_WEBUI_V2_LIVE_QA_HOME to a complete IronClaw home or provide live LLM env."
        )
    provider_id = os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_PROVIDER_ID",
        "nearai",
    )
    model = os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_MODEL",
        os.environ.get("LIVE_OPENAI_COMPATIBLE_MODEL", "deepseek-ai/DeepSeek-V4-Flash"),
    )
    base_url = os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_LLM_BASE_URL")
...
def _auth_user_id() -> str:
    configured = os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_USER_ID", "").strip()
    if configured:
        return configured
    home = Path(os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_HOME", DEFAULT_IRONCLAW_HOME))
    discovered = _persisted_google_user_id(home)
    return discovered or DEFAULT_USER_ID


def _persisted_google_user_id(ironclaw_home: Path) -> str | None:
...
def prepare_ironclaw_home(
    args: argparse.Namespace,
    selected_cases: list[str],
    *,
    case_name: str | None = None,
) -> PreparedIronClawHome:
    ...
    source_home = args.ironclaw_home
    if not source_home.exists():
        source_home = create_generated_ironclaw_home(
            args.output_dir / "generated-ironclaw-home",
            include_slack=needs_slack,
        )

    prepared_home = args.output_dir / "ironclaw-home"
    if case_name:
    ...
    if not config_path.exists() and (prepared_home / "local-dev" / "reborn-local-dev.db").exists():
        _write_minimal_ironclaw_config(config_path, include_slack=needs_slack)
    ...
    google_env, google_env_preflight = _materialize_google_oauth_env_for_ironclaw(
        prepared_home,
    )
    telegram_env, telegram_env_preflight = _materialize_telegram_env_for_ironclaw()

    if _slack_enabled(config) and not _has_live_slack_env():
        secret_env, secret_preflight = _materialize_slack_env_from_ironclaw_home(
            prepared_home,
            config,
        )
    ...
    missing = [name for name in missing if not name.startswith("IRONCLAW_SLACK_")]
    if missing:
        raise LiveQaError(
            "IronClaw config references unset live env vars: " + ", ".join(missing)
        )
    ...
    if args.require_slack_live and needs_slack and not slack_enabled:
        raise LiveQaError(
            ...
            "in the prepared IronClaw config."
        )
    if slack_enabled and not _has_live_slack_env(process_env):
        if args.require_slack_live:
            raise LiveQaError(
                "IronClaw config enables Slack, but live Slack env vars are missing "
                "(expected IRONCLAW_SLACK_SIGNING_SECRET and "
                "IRONCLAW_SLACK_BOT_TOKEN unless overridden in config)."
            )
        if not needs_slack:
            _disable_slack_in_config(config_path)
            print(
                "[ironclaw-webui-v2-live-qa] Slack disabled in copied temp home because "
    ...
    if args.require_slack_live and needs_slack and slack_enabled and not slack_auth.get("ok"):
        ...
        print(
            "[ironclaw-webui-v2-live-qa] Slack env materialized from copied IronClaw home "
...
def create_generated_ironclaw_home(path: Path, *, include_slack: bool = False) -> Path:
    provider_id = os.environ.get(
        "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_PROVIDER_ID",
        "nearai",
    )
    ...
    _write_minimal_ironclaw_config(path / "config.toml", include_slack=include_slack)
    ...
    print(
        "[ironclaw-webui-v2-live-qa] Generated temp IronClaw home from live LLM env "
        f"(provider_id={provider_id}, model={model}, api_key_env={api_key_env}).",
        flush=True,
    )
    if google_seed.get("seeded"):
        print(
            "[ironclaw-webui-v2-live-qa] Seeded generated IronClaw home with "
    ...
    env.update(
        {
            ...
            "IRONCLAW_WEBUI_TOKEN": AUTH_TOKEN,
            "IRONCLAW_WEBUI_USER_ID": _auth_user_id(),
...
def case_llm_trace_env(output_dir: Path, case_name: str) -> dict[str, str]:
    ...
    return {
        ...
        "IRONCLAW_TRACE_MODEL_NAME": f"ironclaw-qa-{case_name}",
    }
...
async def start_ironclaw_server(
    binary: Path,
    ironclaw_home: Path,
    output_dir: Path,
    extra_env: dict[str, str] | None = None,
    ...
    if (
        _env_present("IRONCLAW_GOOGLE_CLIENT_ID", process_extra_env)
        and not _env_present("IRONCLAW_GOOGLE_OAUTH_REDIRECT_URI", process_extra_env)
        and not _env_present("GOOGLE_OAUTH_REDIRECT_URI", process_extra_env)
    ):
        process_extra_env["IRONCLAW_GOOGLE_OAUTH_REDIRECT_URI"] = (
            f"{base_url}/api/product-auth/oauth/google/callback"
        )
    ...
    if (
        ...
        process_extra_env["IRONCLAW_SLACK_PERSONAL_OAUTH_REDIRECT_URI"] = (
            f"{base_url}/api/product-auth/oauth/{_slack_auth_provider()}/callback"
        )
    ...
    proc = subprocess.Popen(
        ...
        stderr=err,
        text=True,
        env=server_env(ironclaw_home, output_dir / "os-home", process_extra_env),
        cwd=workspace_dir,
    )
    try:
...
async def _apply_slack_setup_api_after_start(
    *,
    base_url: str,
    prepared_home: PreparedIronClawHome,
) -> dict[str, object]:
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            try:
                ...
                print(
                    "[ironclaw-webui-v2-live-qa] failed to write browser diagnostics "
    ...
    *,
    ...
    for attempt in range(1, total + 1):
        ...
        if result.success or attempt >= total or not is_retriable(result):
            return result
        print(
            "[ironclaw-webui-v2-live-qa] retrying case after retriable failure "
...
async def _live_github_latest_release(owner: str, repo: str) -> dict[str, str]:
    ...
    headers = {
        ...
        "User-Agent": "ironclaw-webui-v2-live-qa",
    }
...
def _trigger_record_count(ironclaw_home: Path, routine_name: str | None = None) -> int:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
    if not db_path.exists():
        return 0
    with closing(sqlite3.connect(db_path)) as db:
...
async def _wait_for_trigger_record_after_count(
    ...
    *,
    ...
    last_count = _trigger_record_count(ironclaw_home, routine_name)
...
def _trigger_run_rows(ironclaw_home: Path, routine_name: str) -> list[dict[str, object]]:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
    if not db_path.exists():
        return []
    with closing(sqlite3.connect(db_path)) as db:
...
def _outbound_final_reply_targets(ironclaw_home: Path) -> dict[str, object]:
...
def _trigger_record_snapshot(ironclaw_home: Path, routine_name: str) -> dict[str, object]:
...
def _triggered_delivery_outcome(ironclaw_home: Path, run_id: str) -> dict[str, object] | None:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
    if not db_path.exists():
        return None
    with closing(sqlite3.connect(db_path)) as db:
...
def _delivered_gate_routes_for_run(ironclaw_home: Path, run_id: str) -> list[dict[str, object]]:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
...
def _slack_event_run_id_for_event(ironclaw_home: Path, event_id: str) -> str | None:
    db_path = ironclaw_home / "local-dev" / "reborn-local-dev.db"
...
async def _approve_delivered_gate_routes_for_run(
    ...
    *,
    ...
    approval_attempts: list[dict[str, object]] = []
    for route in _delivered_gate_routes_for_run(ctx.ironclaw_home, run_id):
...
async def _approve_slack_event_gates(
    ...
    *,
    ...
    run_id = _slack_event_run_id_for_event(ctx.ironclaw_home, event_id)
    if not run_id:
...
async def _wait_for_slack_event_run_id(
    ...
    *,
    ...
    raise AssertionError(
        "Slack event was not accepted into a IronClaw run before timeout. "
...
def _slack_delivery_channel_id(ctx: LiveQaContext) -> str | None:
    ...
    if isinstance(discovery, dict):
        ...
        if discovery.get("ok"):
            ...
            if channel_id:
                return channel_id
    persisted_channel_id = _persisted_slack_personal_dm_channel_id(ctx.ironclaw_home, _auth_user_id())
    if persisted_channel_id:
        return persisted_channel_id
    db_path = ctx.ironclaw_home / "local-dev" / "reborn-local-dev.db"
    if not db_path.exists():
        return None
    with closing(sqlite3.connect(db_path)) as db:
        row = db.execute(
...
async def _wait_for_slack_delivery_marker(
    ...
    *,
    ...
    approval_attempts: list[dict[str, object]] = []
    while time.monotonic() < deadline:
        rows = _trigger_run_rows(ctx.ironclaw_home, routine_name)
        if rows:
            last_rows = rows
            ...
            for row in rows:
                ...
                if not run_id:
                    continue
                outcome = _triggered_delivery_outcome(ctx.ironclaw_home, run_id)
                if outcome:
...
async def _extension_chat_connect_case(
    ...
    *,
    ...
    try:
        statuses = _capability_run_statuses(ctx.ironclaw_home, expected_capabilities)
...
async def case_qa_2a_gmail_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        marker="IRONCLAW_QA_2A_GMAIL_CONNECT_DONE",
...
async def case_qa_2b_calendar_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        marker="IRONCLAW_QA_2B_CALENDAR_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
...
async def case_qa_2c_drive_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        marker="IRONCLAW_QA_2C_DRIVE_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
...
async def case_qa_2e_calendar_prep_email_routine(ctx: LiveQaContext) -> ProbeResult:
    routine_name = "ironclaw-qa-2e-calendar-prep-email"
...
async def case_qa_2f_calendar_prep_email_delivery(ctx: LiveQaContext) -> ProbeResult:
    ...
    marker = f"IRONCLAW_QA_2F_CALENDAR_PREP_EMAIL_DELIVERED_{suffix}"
    try:
    ...
    email_subject = f"IronClaw QA 2F meeting prep {suffix}"
    email_body = (
        f"{marker}\n\n"
        "IronClaw WebUIv2 live QA 2F calendar-prep delivery check. "
    ...
    result = await _live_chat_with_extensions_case(
        ...
        extra_details={
            ...
            "target_source": (
                ...
                if _first_env_value(
                    [
                        "IRONCLAW_WEBUI_V2_LIVE_QA_EMAIL_TARGET",
...
async def case_qa_4a_gmail_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        display_name="Gmail",
        required_tools=["gmail.list_messages"],
        marker="IRONCLAW_QA_4A_GMAIL_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
            "with gmail.list_messages for at most one recent message."
        ),
...
async def case_qa_4b_github_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        marker="IRONCLAW_QA_4B_GITHUB_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
...
async def case_qa_6a_gmail_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        display_name="Gmail",
        required_tools=["gmail.list_messages"],
        marker="IRONCLAW_QA_6A_GMAIL_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
            "with gmail.list_messages for at most one recent message."
        ),
...
async def case_qa_5b_drive_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        display_name="Google Drive",
        required_tools=["google-drive.list_files"],
        marker="IRONCLAW_QA_5B_DRIVE_CONNECT_DONE",
        verification_instruction=(
            "After connecting, make exactly one safe read-only verification call "
            "with google-drive.list_files for at most one file."
        ),
...
async def case_qa_5c_strategy_doc_knowledge_base(ctx: LiveQaContext) -> ProbeResult:
    strategy_phrase = "IronClaw QA Strategy North Star: verify live WebUIv2 tool grounding."
    return await _live_chat_with_extensions_case(
        ctx,
...
async def case_qa_5d_slack_strategy_doc_answer(ctx: LiveQaContext) -> ProbeResult:
    ...
    doc_marker = f"IRONCLAW_QA_5D_STRATEGY_DOC_{suffix}"
    slack_marker = f"IRONCLAW_QA_5D_SLACK_STRATEGY_ANSWER_{suffix}"
    nonce = f"QA5D-NONCE-{uuid.uuid4()}"
    strategy_phrase = (
        "IronClaw QA 5D strategy north star: answer Slack questions from "
...
async def case_qa_6b_sheets_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        marker="IRONCLAW_QA_6B_SHEETS_CONNECT_DONE",
        verification_instruction=(
...
async def case_qa_6d_gmail_to_sheet_routine(ctx: LiveQaContext) -> ProbeResult:
    routine_name = "ironclaw-qa-6d-gmail-to-sheet"
    return await _routine_creation_case(
        ctx,
...
async def case_qa_6e_gmail_to_sheet_delivery(ctx: LiveQaContext) -> ProbeResult:
    started = time.monotonic()
    marker = f"IRONCLAW_QA_6E_GMAIL_TO_SHEET_DELIVERY_{int(time.time() * 1000)}"
    result = await _live_chat_with_extensions_case(
        ctx,
...
async def case_qa_7b_sheets_connect(ctx: LiveQaContext) -> ProbeResult:
    return await _extension_chat_connect_case(
        ...
        display_name="Google Sheets",
        required_tools=["google-sheets.read_values"],
        marker="IRONCLAW_QA_7B_SHEETS_CONNECT_DONE",
        verification_instruction=(
            "After connecting, do not create or modify any spreadsheet; just "
            "finish after the Google Sheets extension is active."
        ),
...
async def _routine_creation_case(
    ...
    *,
    ...
    before_count = _trigger_record_count(ctx.ironclaw_home, count_name)
    details = {
        "routine_name": routine_name,
    ...
    if result.success:
        ...
        after_count = _trigger_record_count(ctx.ironclaw_home, count_name)
        wait_ms = 0
...
async def case_qa_3c_endpoint_status_slack_routine(ctx: LiveQaContext) -> ProbeResult:
    routine_name = "ironclaw-qa-3c-endpoint-status-slack"
...
async def _slack_delivery_routine_case(
    ...
    *,
    ...
    if require_persisted_delivery_target:
        default_targets_before = _outbound_final_reply_targets(ctx.ironclaw_home)
    ...
    try:
        ...
        if require_persisted_delivery_target or expect_one_shot_schedule:
            record_snapshot = _trigger_record_snapshot(ctx.ironclaw_home, routine_name)
        ...
        if require_persisted_delivery_target and default_targets_before is not None:
            ...
            default_targets_after = _outbound_final_reply_targets(ctx.ironclaw_home)
...
async def case_qa_3d_endpoint_status_slack_delivery(ctx: LiveQaContext) -> ProbeResult:
    return await _slack_delivery_routine_case(
        ...
        routine_prefix="ironclaw-qa-3d-endpoint-status-slack-delivery",
        marker_prefix="IRONCLAW_QA_3D_ENDPOINT_STATUS",
        routine_instruction=(
...
async def case_qa_4d_github_release_slack_routine(ctx: LiveQaContext) -> ProbeResult:
    routine_name = "ironclaw-qa-4d-github-release-slack"
    return await _routine_creation_case(
        ctx,
...
async def case_qa_4e_github_release_email_delivery(ctx: LiveQaContext) -> ProbeResult:
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    marker = f"IRONCLAW_QA_4E_GITHUB_RELEASE_EMAIL_DELIVERED_{suffix}"
    try:
    ...
    email_subject = f"IronClaw QA 4E release {release['tag_name']} {suffix}"
    email_body = (
        f"{marker}\n\n"
...
async def case_qa_7e_slack_bug_sheet_delivery(ctx: LiveQaContext) -> ProbeResult:
    ...
    wall_started = time.time()
    suffix = str(int(wall_started * 1000))
    sheet_marker = f"IRONCLAW_QA_7E_BUG_TRACKER_SHEET_{suffix}"
    row_marker = f"IRONCLAW_QA_7E_BUG_ROW_{suffix}"
...
async def case_qa_7c_slack_bug_logger_routine(ctx: LiveQaContext) -> ProbeResult:
    started = time.monotonic()
    routine_name = "ironclaw-qa-7c-slack-bug-sheet"
    try:
        access_token, token_meta = _google_runtime_access_token(
            ctx.ironclaw_home,
            _auth_user_id(),
...
async def case_qa_8c_hn_keyword_slack_routine(ctx: LiveQaContext) -> ProbeResult:
    routine_name = "ironclaw-qa-8c-hn-keyword-slack"
    return await _routine_creation_case(
        ctx,
...
async def case_qa_8d_hn_keyword_slack_delivery(ctx: LiveQaContext) -> ProbeResult:
    return await _slack_delivery_routine_case(
        ...
        routine_prefix="ironclaw-qa-8d-hn-keyword-slack-delivery",
        marker_prefix="IRONCLAW_QA_8D_HN_KEYWORD",
        routine_instruction=(
...
async def _gated_qa_case(ctx: LiveQaContext, case_name: str) -> ProbeResult:
    ...
    return _result(
        ...
        started,
        {
            ...
            "message": (
                "This QA row is represented in the IronClaw WebUI v2 live lane, "
...
async def case_qa_9b_routine_dm_delivery_exactly_once(ctx: LiveQaContext) -> ProbeResult:
    ...
    return await _slack_delivery_routine_case(
        ...
        routine_prefix="ironclaw-qa-9b-dm-exactly-once",
        marker_prefix="IRONCLAW_QA_9B_DM_EXACTLY_ONCE",
        routine_instruction=(
...
async def case_qa_9c_slack_digest_names_not_ids(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    marker = f"IRONCLAW_QA_9C_DIGEST_{int(time.time() * 1000)}"
    result = await _live_chat_case(
        ctx,
        case_name=case_name,
        prompt=(
...
async def case_qa_9d_routine_per_trigger_delivery_target(ctx: LiveQaContext) -> ProbeResult:
    ...
    return await _slack_delivery_routine_case(
        ...
        routine_prefix="ironclaw-qa-9d-per-trigger-target",
        marker_prefix="IRONCLAW_QA_9D_PER_TRIGGER_TARGET",
        routine_instruction=(
...
async def case_qa_10a_slack_self_attribution(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10A_SELF_ATTRIBUTION_{suffix}"
...
async def case_qa_10b_slack_ooo_status(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10B_STATUS_{suffix}"
    details: dict[str, object] = {}
    try:
        personal_token = _require_slack_personal_token(ctx)
...
async def case_qa_10c_slack_thread_replies(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10C_THREADS_{suffix}"
...
async def case_qa_10d_slack_channel_membership(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10D_MEMBERSHIP_{suffix}"
    details: dict[str, object] = {}
    try:
        personal_token = _require_slack_personal_token(ctx)
...
async def case_qa_10e_slack_error_honesty(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10E_ERROR_HONESTY_{suffix}"
...
async def case_qa_10f_slack_mention_encoding(ctx: LiveQaContext) -> ProbeResult:
    ...
    wall_started = time.time()
    suffix = str(int(wall_started * 1000))
    answer_marker = f"IRONCLAW_QA_10F_MENTION_ANSWER_{suffix}"
...
async def case_qa_10g_slack_last_message_sent(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10G_LAST_SENT_{suffix}"
...
async def case_qa_10g_slack_last_message_sent_global(
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10G_GLOBAL_LAST_SENT_{suffix}"
...
async def case_qa_10h_slack_email_hallucination_guard(
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10H_EMAIL_GUARD_{suffix}"
    details: dict[str, object] = {}
    try:
        personal_token = _require_slack_personal_token(ctx)
        channel_id = await _require_slack_personal_bot_dm_channel(ctx)
...
async def case_qa_10i_slack_raw_entity_hygiene(ctx: LiveQaContext) -> ProbeResult:
    ...
    started = time.monotonic()
    suffix = str(int(time.time() * 1000))
    answer_marker = f"IRONCLAW_QA_10I_ENTITY_HYGIENE_{suffix}"
...
def write_case_manifest(output_dir: Path, selected_cases: list[str]) -> Path:
    qa_sheet_url = os.environ.get("IRONCLAW_WEBUI_V2_LIVE_QA_SHEET_URL", "").strip()
...
def export_case_trace(output_dir: Path, case_name: str, ironclaw_home: Path) -> dict[str, object]:
    ...
    if not db_path.exists():
        payload["error"] = f"IronClaw local-dev database not found at {db_path}"
    ...
    try:
        ...
        payload["error"] = f"failed to export IronClaw trace from {db_path}: {exc}"
        trace_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        return {"case": case_name, "path": str(trace_path), "entry_count": 0}
...
def write_preflight(output_dir: Path, prepared_home: PreparedIronClawHome) -> Path:
    payload = {
        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "ironclaw_home": str(prepared_home.path),
...
async def run_cases(args: argparse.Namespace) -> int:
    ...
    print(f"[ironclaw-webui-v2-live-qa] case_manifest={manifest_path}", flush=True)
    binary = _ironclaw_binary() if args.skip_build else build_ironclaw_binary()
    if not binary.exists():
        raise LiveQaError(
    ...
    for case_index, name in enumerate(selected_cases):
        ...
        prepared_home = prepare_ironclaw_home(args, [name], case_name=name)
        ...
        print(
            f"[ironclaw-webui-v2-live-qa] preflight={preflight_path} "
        ...
        if (
            ...
            print(
                f"[ironclaw-webui-v2-live-qa] case={name} success={result.success} "
        ...
        if case_spec.requires_slack and isinstance(slack_preflight, dict):
            ...
            if (
                ...
                if not slack_preflight.get("enabled_in_config"):
                    error = "live Slack is not enabled in the prepared IronClaw config"
        ...
        if (
            ...
            result = _result(
                ...
                started,
                {
                    ...
                    "required_env": [
                        "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_ROUTE_USER_ID",
                        "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INBOUND_USER_ID",
                    ],
                    "preflight": slack_preflight,
                },
            )
        ...
        proc, base_url = await start_ironclaw_server(
            binary,
            prepared_home.path,
            args.output_dir,
            server_extra_env,
        ...
        try:
            ctx = LiveQaContext(
                ...
                ironclaw_home=prepared_home.path,
            ...
            if case_spec.requires_slack and isinstance(slack_preflight, dict):
                ...
                shutil.copyfile(preflight_path, case_preflight_path)
            print(f"[ironclaw-webui-v2-live-qa] running case={name}", flush=True)
            ...
            if (
                ...
                try:
                    ...
                    print(
                        f"[ironclaw-webui-v2-live-qa] case={name} success=False "
            ...
            print(
                f"[ironclaw-webui-v2-live-qa] trace={trace_export['path']} "
    ...
    print(f"[ironclaw-webui-v2-live-qa] results={results_path}", flush=True)
    print(f"[ironclaw-webui-v2-live-qa] trace_index={trace_index_path}", flush=True)
    print(
        f"[ironclaw-webui-v2-live-qa] green_run_explanation={green_explanation_path}",
        flush=True,
    )
...
def main() -> int:
    ...
    args.ironclaw_home = args.ironclaw_home.resolve()
    ...
    try:
        ...
        print(f"[ironclaw-webui-v2-live-qa] {exc}", file=sys.stderr, flush=True)
        return 1
