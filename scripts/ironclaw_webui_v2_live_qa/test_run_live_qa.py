#!/usr/bin/env python3
"""Unit tests for the IronClaw WebUI v2 live QA runner helpers.

Run with::

    python3 scripts/ironclaw_webui_v2_live_qa/test_run_live_qa.py
"""
...
if __package__:
    ...
    import run_live_qa
    from scripts.ironclaw_webui_v2_live_qa import google_api_helpers
...
class IronClawWebUiV2LiveQaRunnerTests(unittest.TestCase):
    def _dummy_ctx(self) -> run_live_qa.LiveQaContext:
        return run_live_qa.LiveQaContext(
            base_url="http://127.0.0.1:9",
            output_dir=Path("/tmp"),
            ironclaw_home=Path("/tmp/ironclaw-home"),
            env={},
        )
    ...
    def test_slack_connect_case_uses_extensions_channels_surface(self):
        ...
        async def fake_webui_json(
            ...
            if method == "POST" and path == "/api/product-auth/accounts/list":
                            ...
                            {"kind": "tool"},
                            {
                                ...
                                "connection": {
                                    ...
                                    "instructions": (
                                        ...
                                        "configuration, then message Slack directly."
                                    ),
                                },
                            },
                        ],
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ctx = run_live_qa.LiveQaContext(
                ...
                ironclaw_home=output_dir / "ironclaw-home",
                env={},
            )
        ...
        self.assertIn("Slack", observed_expectations)
        self.assertFalse(any("/chat" in url for url, _wait in fake_page.gotos))
        self.assertEqual(
            result.details["slack_connect_surface"],
            "/extensions/channels",
        )
        self.assertEqual(result.details["slack_catalog_source"], "registry")
        self.assertEqual(result.details["slack_connection_strategy"], "oauth")
        self.assertEqual(result.details["slack_connect_display_name"], "Slack")
        self.assertEqual(
            fetched_paths,
            [
                ...
                "/api/product-auth/accounts/list",
    ...
    def test_webui_json_preserves_non_json_error_body_without_double_read(self):
        class FakePage:
            async def evaluate(self, source: str, _args: dict[str, object]):
                self.source = source
                return {"status": 404, "body": "route not found"}

        page = FakePage()
        with self.assertRaisesRegex(AssertionError, "HTTP 404: 'route not found'"):
            asyncio.run(
                run_live_qa._webui_json(
                    page,
                    "GET",
                    "/api/webchat/v2/missing",
                )
            )

        self.assertIn("await response.text()", page.source)
        self.assertNotIn("await response.json()", page.source)
    ...
    def test_slack_connect_case_fails_when_personal_oauth_not_ready(self):
        ...
        self.assertFalse(result.success)
        self.assertIn("generic setup projection", str(result.details["error"]))
        self.assertIn("slack_user_token", str(result.details["error"]))
    ...
    def test_slack_connect_instruction_validation_accepts_oauth_copy(self):
        ...
        self.assertFalse(
            run_live_qa._slack_connect_instructions_look_valid(
                "Message the IronClaw app in Slack to get a pairing code, "
                "then paste it here."
            )
        )
    ...
    def test_product_connect_cases_start_from_chat_then_verify_registry(self):
        ...
        def fake_capability_run_statuses(_ironclaw_home, capability_ids):
        ...
        with (
            ...
            result = asyncio.run(
                run_live_qa._extension_chat_connect_case(
                    ...
                    marker="IRONCLAW_QA_TEST_CONNECT_DONE",
    ...
    def test_routine_confirmation_follow_up_ignores_slack_pairing_gate(self):
        text = (
            "Connect Slack. Message the IronClaw app in Slack to get a "
                    ...
                    run_live_qa._wait_for_assistant_reply(
                        self._fake_assistant_reply_page(
                            ...
                            assistant_block_texts=[
                                "Earlier reply IRONCLAW_QA_DONE",
                                "Routine created without the marker.",
                            ],
                        ),
                        marker="IRONCLAW_QA_DONE",
    ...
    def test_wait_for_assistant_reply_reconfirms_marker_after_capture_race(self):
        ...
        # truncated snapshot.
        marker = "IRONCLAW_QA_8D_HN_KEYWORD_ROUTINE_CREATED_0123"
        truncated_reply = {
            # Ends mid-marker, exactly like the captured trace.
            "text": "Marker in final answer: `IRONCLAW_QA_8D_HN_KEYWORD_R",
            "final_reply_state": "true",
        }
        complete_reply = {
    ...
    def test_wait_for_assistant_reply_ignores_stale_terminal_failure(self):
        ...
        response_text = "Routine created. IRONCLAW_QA_DONE"
        ...
        with (
            ...
            chat, reply_text = asyncio.run(
                run_live_qa._slack_correctness_chat_reply(
                    ...
                    answer_marker="IRONCLAW_QA_10_TEST_EXACT",
                    extra_details={},
                )
            )
    ...
    def test_slack_correctness_capability_evidence_is_bound_to_submitted_turn(self):
        ...
        def create_store(ironclaw_home: Path) -> Path:
        ...
        def drive(
            *,
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ironclaw_home = Path(tmpdir) / "ironclaw-home"
                db_path = create_store(ironclaw_home)
                ...
                ctx = self._dummy_ctx()
                ctx.ironclaw_home = ironclaw_home
                with patch.object(
                    run_live_qa,
                    "_live_chat_case",
                    side_effect=fake_live_chat_case,
    ...
    def test_qa_10e_prompt_echo_without_current_turn_history_call_fails(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ctx = self._dummy_ctx()
            ctx.ironclaw_home = Path(tmpdir) / "ironclaw-home"
            with patch.object(
                run_live_qa,
                "_live_chat_case",
                side_effect=fake_live_chat_case,
    ...
    def test_wait_for_assistant_reply_does_not_judge_missing_marker(self):
        ...
        with (
            ...
            with self.assertRaisesRegex(AssertionError, "marker='IRONCLAW_QA_DONE'"):
                asyncio.run(
                    run_live_qa._wait_for_assistant_reply(
                        self._fake_assistant_reply_page(response_text),
                        marker="IRONCLAW_QA_DONE",
    ...
    def test_semantic_judge_passed_respects_confidence_threshold(self):
        with patch.dict(
            os.environ,
            {"IRONCLAW_WEBUI_V2_LIVE_QA_LLM_JUDGE_MIN_CONFIDENCE": "0.9"},
        ):
            self.assertTrue(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.dict(
                    os.environ,
                    {
                        "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_ROUTE_USER_ID_PATH": str(
                            user_id_path
                        )
                    },
                    clear=True,
    ...
    def test_slack_dm_route_discovery_rejects_missing_real_route_user(self):
        ...
        with (
            patch.dict(
                os.environ,
                {"IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INBOUND_USER_ID": "U0REBORNQA"},
                clear=True,
            ),
            patch.dict(sys.modules, {"httpx": fake_httpx}),
        ):
        ...
        self.assertIn("IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_ROUTE_USER_ID", result["required_env"])
    ...
    def test_start_ironclaw_server_sets_slack_oauth_redirect(self):
        captured: dict[str, object] = {}
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                proc, base_url = asyncio.run(
                    run_live_qa.start_ironclaw_server(
                        root / "ironclaw",
                        root / "ironclaw-home",
                        root / "out",
                        {
                            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID": "slack-client",
                            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_SECRET": "slack-secret",
                        },
                    )
                )
        ...
        self.assertEqual(
            env["IRONCLAW_SLACK_PERSONAL_OAUTH_REDIRECT_URI"],
            (
                "http://127.0.0.1:38555"
                "/api/product-auth/oauth/slack/callback"
            ),
        )

    def test_start_ironclaw_server_sets_slack_redirect_from_persisted_oauth_client_id(self):
        captured: dict[str, object] = {}
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ironclaw_home = root / "ironclaw-home"
            ironclaw_home.mkdir()
            ...
            env = {
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID": "",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID_PATH": "",
                "IRONCLAW_SLACK_PERSONAL_OAUTH_REDIRECT_URI": "",
                "IRONCLAW_SLACK_PERSONAL_OAUTH_REDIRECT_URI_PATH": "",
            }
            with (
    ...
    def test_qa_7c_prepares_bug_logging_sheet_before_sheet_prompt(self):
        ...
        self.assertEqual(captured_routine["routine_name"], "ironclaw-qa-7c-slack-bug-sheet")
        self.assertEqual(
    ...
    def test_qa_7d_accepts_signed_slack_event_into_ironclaw_run(self):
    ...
    def test_live_google_side_effect_cases_install_required_extensions(self):
        ...
        self.assertIn('"body":"IRONCLAW_QA_2F_CALENDAR_PREP_EMAIL_DELIVERED_', prompt_2f)
        ...
        self.assertIn('"body":"IRONCLAW_QA_4E_GITHUB_RELEASE_EMAIL_DELIVERED_', prompt_4e)
    ...
    def test_signed_slack_event_uses_extension_scoped_ingress_route(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ctx = run_live_qa.LiveQaContext(
                ...
                output_dir=root,
                ironclaw_home=root / "ironclaw-home",
                env={"IRONCLAW_SLACK_SIGNING_SECRET": "signing-secret"},
            )
            with (
                patch.dict(sys.modules, {"httpx": fake_httpx}),
                patch.object(
        ...
        self.assertEqual(
            posted_urls,
            ["http://127.0.0.1:3000/webhooks/extensions/slack/events"],
        )
    ...
    def test_slack_personal_dm_seed_satisfies_delivery_target(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                result["identity_binding"]["provider_user_id"],
                "slack:UQAUSER",
                "generic identity scope uses the installed extension id, while the "
                "outbound target retains the external Slack installation id",
            )
    ...
    def test_prepare_ironclaw_home_removes_copied_stale_dm_channel_route(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            ...
            args = argparse.Namespace(
                ...
                ironclaw_home=source_home,
            )
            env = {
                "NEARAI_API_KEY": "fake-live-llm-key",
                "LIVE_OPENAI_COMPATIBLE_API_KEY": "fake-live-llm-key",
                "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_API_KEY_ENV": "LIVE_OPENAI_COMPATIBLE_API_KEY",
            }

            with patch.dict(os.environ, env, clear=False):
                prepared = run_live_qa.prepare_ironclaw_home(
                    args,
                    ["qa_3a_slack_connect"],
                )
    ...
    def test_slack_event_run_id_reads_idempotency_record(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            home = Path(tmpdir) / "ironclaw-home"
            db_path = home / "local-dev" / "reborn-local-dev.db"
            db_path.parent.mkdir(parents=True)
            with closing(sqlite3.connect(db_path)) as db:
                db.execute(
    ...
    def test_generated_google_seed_creates_refreshable_product_auth_account(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                ...
                "IRONCLAW_GOOGLE_CLIENT_ID": "fake-client-id",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SKIP_GOOGLE_REFRESH_PROBE": "1",
            }
            with patch.dict(os.environ, env, clear=False):
    ...
    def test_google_runtime_token_refreshes_before_env_access_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                ...
                "AUTH_LIVE_GOOGLE_REFRESH_TOKEN": "fake-refresh-token",
                "IRONCLAW_GOOGLE_CLIENT_ID": "fake-client-id",
                "IRONCLAW_GOOGLE_CLIENT_SECRET": "fake-client-secret",
            }
            with patch.dict(os.environ, env, clear=False):
            ...
            self.assertEqual(meta["source"], "ironclaw_product_auth_refresh_secret")
    ...
    def test_generated_slack_seed_creates_live_user_product_auth_account(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                ...
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INSTALLATION_ID": "local-dev-installation",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_TEAM_ID": "T123",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_API_APP_ID": "A123",
            }
            with (
                patch.dict(os.environ, env, clear=False),
    ...
    def test_prepare_ironclaw_home_gates_missing_slack_without_raising(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            missing_source = root / "missing-source-home"
            args = argparse.Namespace(
                output_dir=root / "out",
                ironclaw_home=missing_source,
                require_slack_live=False,
            )
            env = {
                "LIVE_OPENAI_COMPATIBLE_API_KEY": "fake-live-llm-key",
            ...
            for name in (
                ...
                "IRONCLAW_SLACK_SIGNING_SECRET_PATH",
                "IRONCLAW_SLACK_BOT_TOKEN",
                "IRONCLAW_SLACK_BOT_TOKEN_PATH",
            ):
                env[name] = ""
    ...
    def test_slack_setup_payload_uses_persisted_oauth_client_id(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload, preflight = run_live_qa._slack_setup_payload(
                ...
                "[slack]\nenabled = true\n",
                {
                    "IRONCLAW_SLACK_BOT_TOKEN": "xoxb-bot",
                    "IRONCLAW_SLACK_SIGNING_SECRET": "signing-secret",
                    "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_SECRET": "oauth-secret",
                },
                bot_user_id="U-BOT",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload, preflight = run_live_qa._slack_setup_payload(
                ...
                "[slack]\nenabled = true\n",
                {
                    "IRONCLAW_SLACK_BOT_TOKEN": "xoxb-bot",
                    "IRONCLAW_SLACK_SIGNING_SECRET": "signing-secret",
                    "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID": "fresh-client-id",
                    "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_SECRET": "oauth-secret",
                },
                bot_user_id="U-BOT",
                shared_subject_user_id="user:web",
    ...
    def test_slack_setup_payload_requires_oauth_client_material(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cases = {
                ...
                "oauth_client_secret": {
                    **base_env,
                    "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_OAUTH_CLIENT_ID": "oauth-client-id",
                },
            }
    ...
    def test_slack_setup_api_failure_omits_response_body(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            prepared = run_live_qa.PreparedIronClawHome(
                path=ironclaw_home,
                env={"IRONCLAW_SLACK_BOT_TOKEN": "xoxb-bot-token"},
            )
            with (
                patch.dict(sys.modules, {"httpx": fake_httpx}),
                patch.object(
        ...
        self.assertIn("Extension setup API submit returned HTTP 400", error)
    ...
    def test_slack_setup_api_uses_generic_manifest_declared_setup_contract(self):
        ...
        setup_projection = {
            ...
            "phase": "installed",
            "secrets": [
                {"name": "slack_bot_token", "provided": False},
        ...
        class FakeAsyncClient:
            ...
            async def get(self, url, **_kwargs):
                ...
                if url.endswith("/api/webchat/v2/extensions"):
                    list_count = sum(
                        method == "GET" and request_url == url
                        for method, request_url, _payload in requests
                    )
                    return FakeResponse(
                        200,
                        {
                ...
                return FakeResponse(200, setup_projection)
            ...
            async def post(self, url, *, json=None, **_kwargs):
                ...
                return FakeResponse(200, configured)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            prepared = run_live_qa.PreparedIronClawHome(path=ironclaw_home)
            with (
                patch.dict(sys.modules, {"httpx": fake_httpx}),
                patch.object(
                    run_live_qa,
        ...
        setup_url = "http://127.0.0.1:38555/api/webchat/v2/extensions/slack/setup"
        self.assertEqual(
            requests,
            [
                (
    ...
    def test_slack_setup_api_requires_secret_presence_projection(self):
        ...
        error = str(raised.exception)
        self.assertIn("incomplete setup projection", error)
    ...
    def test_prepare_ironclaw_home_synthesizes_config_for_copied_db_home(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
    ...
    def test_generated_slack_home_uses_webui_setup_only(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                "LIVE_OPENAI_COMPATIBLE_API_KEY": "fake-live-llm-key",
                "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_API_KEY_ENV": "LIVE_OPENAI_COMPATIBLE_API_KEY",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_INSTALLATION_ID": "",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_TEAM_ID": "",
                "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_API_APP_ID": "",
                "IRONCLAW_SLACK_SIGNING_SECRET": "",
                "IRONCLAW_SLACK_BOT_TOKEN": "",
            }

            with patch.dict(os.environ, env, clear=True):
                run_live_qa.create_generated_ironclaw_home(home, include_slack=True)
    ...
    def test_qa_9c_digest_persists_leak_counts_not_raw_ids(self):
        ...
        scenarios = {
            "user_id": (
                "your DMs: U0BDC16TML3 said hi IRONCLAW_QA_9C_DIGEST",
                "U0BDC16TML3",
                "leaked_raw_user_id_count",
            ),
            "encoded_user_id": (
                "your DMs: <@UABCDEFGH> said hi IRONCLAW_QA_9C_DIGEST",
                "UABCDEFGH",
                "leaked_raw_user_id_count",
            ),
            "conversation_id": (
                "your DMs: D0BDC16TML3 has one message IRONCLAW_QA_9C_DIGEST",
                "D0BDC16TML3",
    ...
    def test_name_token_in_text_matches_word_boundary_tokens(self):
        ...
        self.assertFalse(
            run_live_qa._name_token_in_text("IRONCLAW_QA_10I_ANSWER_123", "QA X")
        )
    ...
    def test_qa_10_cases_fail_closed_without_slack_tokens(self):
        ...
        for name in (
            ...
            "IRONCLAW_WEBUI_V2_LIVE_QA_SLACK_USER_TOKEN",
        ):
            blank_env[name] = ""
    ...
    def test_live_canary_workflow_shards_cover_non_telegram_qa_suite(self):
        ...
        fixture_manifest_path = (
            ...
            / "tests/fixtures/llm_traces/ironclaw_qa/live_canary/case-manifest.json"
        )
        ...
        match = re.search(
            r"(?ms)^  ironclaw-webui-v2-live-qa:\n(?P<body>.*?)^  persona-rotating:",
            workflow,
        )
        self.assertIsNotNone(match, "IronClaw WebUI v2 live QA job missing")
        ...
        self.assertIsNotNone(
            ...
            "IronClaw WebUI v2 live QA all-case validation list missing",
        )
        all_shard_cases = [
            case_name.strip()
        ...
        self.assertIn(
            "Unknown IronClaw WebUI v2 live QA case",
            match.group("body"),
        )
        self.assertIn("target_pr is required when target_ref is supplied", workflow)
        self.assertIn(
            "Refusing to run ironclaw-webui-v2-live-qa with live secrets on a forked PR",
            workflow,
        )
        self.assertIn("target_ref does not match PR #${TARGET_PR} head SHA", workflow)
        self.assertIn("approve-ironclaw-webui-v2-pr-live-qa:", workflow)
        ...
        self.assertIn("- prepare-ironclaw-webui-v2-live-qa", match.group("body"))
        self.assertIn(
            "- preflight-ironclaw-webui-v2-google-oauth",
            match.group("body"),
        )
        self.assertIn("always() &&", match.group("body"))
        self.assertIn(
            "needs.prepare-ironclaw-webui-v2-live-qa.result == 'success'",
            match.group("body"),
        )
        self.assertIn("github.event_name == 'schedule'", match.group("body"))
        self.assertNotIn("github.event.schedule ==", match.group("body"))
        self.assertIn(
            "ref: ${{ needs.prepare-ironclaw-webui-v2-live-qa.outputs.checkout_ref }}",
            match.group("body"),
        )
        ...
        self.assertIn("IRONCLAW_WEBUI_V2_LIVE_QA_BUILD_SOURCE", match.group("body"))
        ...
        self.assertIn("IRONCLAW_WEBUI_V2_GOOGLE_CASES", match.group("body"))
        self.assertEqual(
        ...
        self.assertIn(
            "steps.mint_ironclaw_webui_v2_google_token.outputs.skip_shard != '1'",
            match.group("body"),
        )
        self.assertIn(
        ...
        google_preflight_match = re.search(
            r"(?ms)^  preflight-ironclaw-webui-v2-google-oauth:\n"
            r"(?P<body>.*?)^  prepare-ironclaw-webui-v2-live-qa:",
            workflow,
        )
        self.assertIsNotNone(
        ...
        self.assertIn(
            "IRONCLAW_GOOGLE_OAUTH_PREFLIGHT_STATUS: "
            "${{ needs.preflight-ironclaw-webui-v2-google-oauth.outputs.status }}",
            workflow,
        )

        prepare_match = re.search(
            r"(?ms)^  prepare-ironclaw-webui-v2-live-qa:\n(?P<body>.*?)^  ironclaw-webui-v2-live-qa:",
            workflow,
        )
        self.assertIsNotNone(prepare_match, "shared live QA binary preparation job missing")
        prepare_body = prepare_match.group("body")
        self.assertIn("needs: approve-ironclaw-webui-v2-pr-live-qa", prepare_body)
        self.assertIn("ironclaw-webui-v2-binary-${TARGET_REF}", prepare_body)
        self.assertIn("Build fallback IronClaw WebUI v2 binary once", prepare_body)
        self.assertIn("if ! (", prepare_body)
        self.assertIn("validate_ironclaw_binary_artifact.py", prepare_body)
        self.assertIn(
            '"${TARGET_REF}" \\\n                ""',
            prepare_body,
        )
        self.assertIn("--argjson features '[]'", prepare_body)
        self.assertNotIn("target/debug/ironclaw-reborn", prepare_body)
        self.assertIn(
        ...
        ironclaw_e2e = ironclaw_e2e_path.read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_case_manifest_distinguishes_targeted_from_placeholder_gates(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(
                os.environ,
                {"IRONCLAW_WEBUI_V2_LIVE_QA_SHEET_URL": sheet_url},
                clear=False,
            ):
    ...
    def test_gmail_delivery_target_prefers_explicit_env(self):
        target = asyncio.run(
            run_live_qa._gmail_delivery_target_email(
                ...
                extra_env={"IRONCLAW_WEBUI_V2_LIVE_QA_EMAIL_TARGET": "qa@example.test"},
            )
        )
    ...
    def test_extract_google_spreadsheet_id_from_url_or_label(self):
        ...
        self.assertIsNone(
            run_live_qa._extract_google_spreadsheet_id(
                "Spreadsheet created: IRONCLAW_QA_6E_GMAIL_TO_SHEET_DELIVERY_1782593757000"
            )
        )
    ...
    def test_extract_google_document_id_from_url_or_label(self):
        ...
        self.assertIsNone(
            run_live_qa._extract_google_document_id(
                "Document created: IRONCLAW_QA_5D_STRATEGY_DOC_1782597084534"
            )
        )
        self.assertIsNone(
            run_live_qa._extract_google_document_id(
                "Document: IRONCLAW_QA_5D_STRATEGY_DOC_1782599165051 (Google Docs)"
            )
        )
    ...
    def test_google_runtime_token_requires_client_secret_for_expired_copied_account(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                ...
                "IRONCLAW_GOOGLE_CLIENT_ID": "client-id",
            }
            with patch.dict(os.environ, env, clear=True):
                seed = run_live_qa._seed_generated_google_product_auth_if_configured(
                    home,
    ...
    def test_google_required_env_for_connect_block_keeps_client_id_only(self):
        ...
        self.assertEqual(required, ["IRONCLAW_GOOGLE_CLIENT_ID"])
    ...
    def test_google_credential_action_for_invalid_grant_requires_token_rotation(self):
        ...
        self.assertIn("IRONCLAW_GOOGLE_CLIENT_SECRET", action)
    ...
    def test_run_cases_exits_only_for_blocking_failures(self):
        def run_failed_case(*, tier: str, blocking: bool) -> tuple[int, dict]:
            ...
            async def fake_start_ironclaw_server(*_args, **_kwargs):
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                args = argparse.Namespace(
                    ...
                    case=[case_name],
                    output_dir=output_dir,
                    ironclaw_home=root / "source-home",
                    skip_build=True,
                    require_slack_live=False,
                )
                cases = {
                ...
                with (
                    patch.object(run_live_qa, "CASES", cases),
                    patch.object(run_live_qa, "QA_SHEET_CASES", {}),
                    patch.object(run_live_qa, "_ironclaw_binary", return_value=binary),
                    patch.object(
                        run_live_qa,
                        "prepare_ironclaw_home",
                        return_value=prepared,
                    ),
                    patch.object(
                        run_live_qa,
                        "start_ironclaw_server",
                        side_effect=fake_start_ironclaw_server,
                    ),
    ...
    def test_model_unavailable_short_circuits_remaining_cases_as_inconclusive(self):
        ...
        async def fake_start_ironclaw_server(
            _binary,
            ironclaw_home,
            _output_dir,
            _env,
        ):
            started_servers.append(ironclaw_home.name)
            return object(), "http://127.0.0.1:38555"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def fake_prepare_ironclaw_home(_args, _selected, *, case_name):
                prepared_cases.append(case_name)
                home = output_dir / "ironclaw-home" / case_name
                home.mkdir(parents=True)
                return run_live_qa.PreparedIronClawHome(
                    path=home,
                    preflight={
                        "slack": {},
                        "slack_personal_auth": {},
            ...
            with (
                ...
                patch.object(
                    run_live_qa,
                    "prepare_ironclaw_home",
                    side_effect=fake_prepare_ironclaw_home,
                ),
                patch.object(
                    run_live_qa,
                    "start_ironclaw_server",
    ...
    def test_run_cases_isolates_ironclaw_home_and_preflight_per_selected_case(self):
        async def fake_case(ctx: run_live_qa.LiveQaContext) -> run_live_qa.ProbeResult:
            return run_live_qa.ProbeResult(
                ...
                success=True,
                latency_ms=1,
                details={"ironclaw_home": str(ctx.ironclaw_home)},
            )
        ...
        async def fake_start_ironclaw_server(
            ...
            return object(), f"http://127.0.0.1/{ironclaw_home.name}"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            args = argparse.Namespace(
                ...
                case=["case_a", "case_b"],
                output_dir=output_dir,
                ironclaw_home=root / "missing-source-home",
                skip_build=True,
                require_slack_live=False,
            )
            cases = {
            ...
            self.assertEqual(status, 0)
            case_a_home = output_dir / "ironclaw-home" / "case_a"
            case_b_home = output_dir / "ironclaw-home" / "case_b"
            ...
            self.assertEqual(case_a_preflight["ironclaw_home"], str(case_a_home))
            self.assertEqual(case_b_preflight["ironclaw_home"], str(case_b_home))
    ...
    def test_run_cases_enables_per_case_llm_trace_recording(self):
        ...
        # the env actually reaches `start_ironclaw_server`.
        ...
        async def fake_start_ironclaw_server(
            ...
            captured_envs[ironclaw_home.name] = env
    ...
    def test_run_cases_fails_successful_model_case_when_trace_is_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env = {
                "LIVE_OPENAI_COMPATIBLE_API_KEY": "fake-live-llm-key",
                "IRONCLAW_WEBUI_V2_LIVE_QA_LLM_API_KEY_ENV": (
    ...
    def test_run_cases_blocks_slack_connect_without_personal_product_auth(self):
        ...
        async def fail_start_ironclaw_server(*_args, **_kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(
                    run_live_qa,
                    "start_ironclaw_server",
                    side_effect=fail_start_ironclaw_server,
                ),
                patch.object(run_live_qa, "stop_process"),
            ):
                status = asyncio.run(run_live_qa.run_cases(args))
    ...
    def test_run_cases_blocks_slack_workspace_mismatch(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(run_live_qa, "QA_SHEET_CASES", {}),
                patch.object(run_live_qa, "_ironclaw_binary", return_value=binary),
                patch.object(run_live_qa, "prepare_ironclaw_home", return_value=prepared),
                patch.object(
                    run_live_qa,
                    "start_ironclaw_server",
                    side_effect=fail_start_ironclaw_server,
