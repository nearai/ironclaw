"""Helpers for IronClaw local root-filesystem rows used by live QA fixtures."""
...
def _root_filesystem_json(db_path: Path, path: str) -> dict[str, object]:
    ...
    if not row:
        raise LiveQaError(f"expected IronClaw root filesystem entry is missing: {path}")
...
def _root_filesystem_secret_by_handle(db_path: Path, handle: str) -> dict[str, object]:
    ...
    if len(rows) != 1:
        raise LiveQaError(
            f"expected exactly one IronClaw secret record for handle {handle!r}, found {len(rows)}"
        )
...
def _decrypt_filesystem_secret(master_key: str, stored: dict[str, object]) -> str:
    try:
        ...
        raise LiveQaError(
            "Decrypting Slack secrets from the IronClaw home requires the e2e "
...
def _encrypt_filesystem_secret(
    *,
    ...
    try:
        ...
        raise LiveQaError(
            "Seeding Google OAuth secrets into a generated IronClaw home requires "
