...
def env_secret(name: str) -> str | None:
    ...
    path is set; otherwise it falls back to the raw environment variable for
    local development.
...
def reserve_loopback_port() -> int:
    ...
    Known TOCTOU: the kernel releases the port when this function returns, so a
    concurrent process could claim it before the caller binds. Prefer a child
    process's native ``--port 0`` support when available; otherwise retry with a
    newly reserved port after an ``EADDRINUSE`` failure.
    """
