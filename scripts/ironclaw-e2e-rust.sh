...
# Run the deterministic Rust-side IronClaw E2E gate.
# Usage:
#   scripts/ironclaw-e2e-rust.sh              # all groups
#   scripts/ironclaw-e2e-rust.sh architecture # boundary + host runtime spine
#   scripts/ironclaw-e2e-rust.sh runtimes     # dispatcher/runtime/process lanes
#   scripts/ironclaw-e2e-rust.sh substrates   # event/network/secret substrates
#
# Extra cargo test args can be passed through CARGO_TEST_ARGS, for example:
#   CARGO_TEST_ARGS='-- --nocapture' scripts/ironclaw-e2e-rust.sh architecture
...
run_architecture() {
  run_test ironclaw_architecture ironclaw_dependency_boundaries
  # Pins docs/ironclaw/contracts/telegram-v2.md: retired-taxonomy telegram
  # identifiers stay dead and no v1 pairing routes re-enter the IronClaw context.
  ...
  run_test ironclaw_host_runtime ironclaw_e2e_gate
  run_test ironclaw_host_runtime ironclaw_invoke_vertical_slice
...
case "${group}" in
  ...
  all)
    ...
    ;;
  *)
    echo "unknown IronClaw E2E group: ${group}" >&2
