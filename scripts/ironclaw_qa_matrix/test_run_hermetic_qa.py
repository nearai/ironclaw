#!/usr/bin/env python3
"""Unit tests for the IronClaw QA matrix hermetic runner.

Run with::

    python3 scripts/ironclaw_qa_matrix/test_run_hermetic_qa.py
"""
...
class IronClawQaMatrixHermeticRunnerTests(unittest.TestCase):
    ...
    def test_responses_external_tools_e2e_dry_run_writes_results_and_logs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(
                "test_ironclaw_responses_repeated_external_tools_round_trip",
                commands[0]["command"],
            )
            self.assertIn(
                "test_ironclaw_responses_rejects_wrong_external_tool_call_id",
                commands[0]["command"],
            )
