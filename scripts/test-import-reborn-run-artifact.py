...
class RunArtifactImporterTest(unittest.TestCase):
    def test_groups_parallel_calls_and_attaches_results_to_next_step(self) -> None:
        ...
        self.assertEqual(candidate["_promotion"]["scrub"]["status"], "verified")
        self.assertIsNone(candidate["_promotion"]["last_successful_replay"])
    ...
    def test_encodes_capability_id_at_the_replay_boundary(self) -> None:
        message = self.tool_message(2, "turn-a", "call-1", "builtin.read_file", "one result")
        artifact = self.artifact(
            [
                {"sequence": 1, "kind": "user", "content": "read it"},
                message,
                {"sequence": 3, "kind": "assistant", "content": "done"},
            ]
        )

        call = MODULE.trace_candidate(artifact, None)["turns"][0]["steps"][0][
            "response"
        ]["tool_calls"][0]

        self.assertEqual(call["name"], "builtin__read_file")
    ...
    def test_main_reports_malformed_nested_tool_calls_as_a_validation_error(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            MODULE.parse_args = lambda: types.SimpleNamespace(
                ...
                model_name=None,
                source_url="https://example.invalid/incidents/1",
    ...
    def test_load_rejects_unsafe_artifact_without_logging_raw_match(self) -> None:
        unsafe = "sk-proj-THIS_SECRET_MUST_NOT_APPEAR_1234567890"
        artifact = self.artifact(
            [
                {"sequence": 1, "kind": "user", "content": unsafe},
                {"sequence": 2, "kind": "assistant", "content": "done"},
            ]
        )
        with tempfile.TemporaryDirectory() as directory:
            path = pathlib.Path(directory) / "artifact.json"
            path.write_text(json.dumps(artifact), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "raw matches are intentionally omitted") as ctx:
