...
fixture_dir="${1:-tests/fixtures/llm_traces/ironclaw_qa}"

if [ ! -d "$fixture_dir" ]; then
  echo "IronClaw QA fixture directory not found: $fixture_dir" >&2
  exit 1
fi
...
if not files:
    print(f"no IronClaw QA fixture JSON files found under {fixture_dir}", file=sys.stderr)
    sys.exit(1)
...
if findings:
    print("IronClaw QA fixture scrub check failed:", file=sys.stderr)
    for path, label, line, snippet in findings:
        print(f"{path}:{line}: {label}: {snippet}", file=sys.stderr)
    sys.exit(1)

print(f"IronClaw QA fixture scrub check passed ({len(files)} files)")
PY
