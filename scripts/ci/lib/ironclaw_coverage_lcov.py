# Shared lcov aggregation for the IronClaw coverage CI scripts.
#
# Extracted (moved, not duplicated) from ironclaw-coverage-summary.sh so both
# that script and ironclaw-coverage-ratchet.sh share ONE lcov-parsing +
# exemption-filtering + by-crate-aggregation implementation. Behavior-
# preserving extraction: ironclaw-coverage-summary.sh's output is unchanged.
# Regression proof lives in scripts/ci/test-ironclaw-coverage.sh's M/A/B/C
# sections — they exercise this module transitively through
# ironclaw-coverage-summary.sh and ironclaw-coverage-comment.sh, not directly.
#
# Two entry points:
