#!/usr/bin/env bash
#
# Upsert a single sticky PR comment carrying the IronClaw integration-tier
...
# Enforcement lives entirely in ironclaw-coverage-ratchet.sh's own exit code,
...
#
# The comment body reuses ironclaw-coverage-summary.sh (the %, table and hole list
...
#   1. the coverage ratchet section (ironclaw-coverage-ratchet.sh's own report
#      output, verbatim) at the very top of the visible body — highest signal
#      first, and simpler than splicing into ironclaw-coverage-summary.sh's
#      output (which today is one opaque rendered string, no exposed seam to
#      insert into "before the per-crate table" specifically).
#   2. a breadth callout counting IronClaw crates with instrumented-but-uncovered
#      lines, also sourced from ironclaw-coverage-summary.sh (--zero-crates). The
#      callout is informational too — the "target: 0" is the roadmap goal, not
#      a check that can fail.
#
# Usage: ironclaw-coverage-comment.sh <lcov-path> <exemptions-toml-path> <floor-toml-path>
#
# Requires env: GH_TOKEN (for gh), GITHUB_REPOSITORY, PR_NUMBER.

set -euo pipefail

lcov_path="${1:?usage: ironclaw-coverage-comment.sh <lcov-path> <exemptions-toml-path> <floor-toml-path>}"
exemptions_path="${2:?usage: ironclaw-coverage-comment.sh <lcov-path> <exemptions-toml-path> <floor-toml-path>}"
floor_path="${3:?usage: ironclaw-coverage-comment.sh <lcov-path> <exemptions-toml-path> <floor-toml-path>}"
...
marker='<!-- ironclaw-coverage-sticky -->'
...
if [ "${#zero_crates[@]}" -gt 0 ]; then
  ...
  callout="⚠️ ${#zero_crates[@]} IronClaw crate(s) have 0 int-tier coverage (target: 0) — ${crate_list}"
fi
