#!/usr/bin/env bash
#
# Render a IronClaw-scoped coverage summary from a merged, crate-filtered lcov
# tracefile (see scripts/ci/ironclaw-coverage-merge-lcov.sh, which merges the 5
# per-lane `cargo llvm-cov report --lcov` outputs from the
# `ironclaw-integration-coverage` matrix in .github/workflows/ironclaw-tests.yml
# and filters to crates/ironclaw_* source files).
#
# The lcov input already covers every crate the int-tier suites link (all
# workspace `ironclaw_*` crates, not a IronClaw-only allowlist) — this script's
# job is purely per-crate aggregation, exemption handling, and rendering.
#
# Usage:
#   ironclaw-coverage-summary.sh <lcov-path> <exemptions-toml-path>
#     Writes a GitHub-flavored Markdown report to stdout. The caller redirects
#     it into "$GITHUB_STEP_SUMMARY" (or anywhere else).
#   ironclaw-coverage-summary.sh --zero-crates <lcov-path> <exemptions-toml-path>
#     Writes, one per line, the IronClaw crates that have instrumented lines but
...
#     ironclaw-coverage-comment.sh never has to recompute it.
#
...
lcov_path="${1:?usage: ironclaw-coverage-summary.sh [--zero-crates] <lcov-path> <exemptions-toml-path>}"
exemptions_path="${2:?usage: ironclaw-coverage-summary.sh [--zero-crates] <lcov-path> <exemptions-toml-path>}"
...
import ironclaw_coverage_lcov as cov
...
# ironclaw-coverage-ratchet.sh via scripts/ci/lib/ironclaw_coverage_lcov.py — this
...
lines = ["## IronClaw integration-tier coverage", ""]

if total > 0:
    lines.append(f"**Line coverage (IronClaw crates): {round2(pct)}%** — {hit} / {total} lines")
else:
    lines.append("**No IronClaw crate coverage data found** (0 instrumented lines matched the IronClaw crate filter).")
lines.append("")
