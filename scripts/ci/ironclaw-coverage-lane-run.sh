#!/usr/bin/env bash
#
# Run one instrumented IronClaw integration-tier coverage lane and produce that
# lane's lcov tracefile.
#
# The 34 tests/integration/ suites (see ironclaw-coverage-int-tier-tests.sh for
# the canonical enumeration) are split across 5 lanes in
# .github/workflows/ironclaw-tests.yml's `ironclaw-integration-coverage` matrix
# job: 4 modulo-partitions of the 27 flat `ironclaw_integration_*` suites, plus
# one dedicated lane for all 7 `ironclaw_group_*` suites.
#
...
# `ironclaw-group-tests` job via run-ironclaw-group-tests.sh, which stays as the
...
# invocation. This mirrors the retired ironclaw-coverage.yml workflow's
...
#
# Reuses ironclaw-coverage-int-tier-tests.sh as the single source of truth for
# suite discovery/naming (the tests/integration/ -> ironclaw_integration_*/
# ironclaw_group_* rewrite rules), so this script never re-derives that mapping.
#
# Modes (IRONCLAW_COV_LANE_MODE):
#   flat-partition  Modulo-partitions the 27 ironclaw_integration_* suites
#                   across IRONCLAW_COV_LANE_PARTITIONS lanes; IRONCLAW_COV_LANE_INDEX
#                   (0-based) selects this lane's slice — mirrors
#                   scripts/ci/run-ironclaw-root-partition.sh's partitioning.
#   group           Runs all ironclaw_group_* suites (7 total) — the one
#                   dedicated group coverage lane.
#
# Usage: IRONCLAW_COV_LANE_MODE=... [other env] ironclaw-coverage-lane-run.sh <output-lcov-path>

set -euo pipefail

output_lcov="${1:?usage: ironclaw-coverage-lane-run.sh <output-lcov-path>}"

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
mode="${IRONCLAW_COV_LANE_MODE:?IRONCLAW_COV_LANE_MODE must be set to 'flat-partition' or 'group'}"
test_timeout="${IRONCLAW_COV_LANE_TEST_TIMEOUT:-45m}"

# ironclaw-coverage-int-tier-tests.sh prints alternating "--test"/"<name>"
# lines; keep only the name lines (every 2nd line, portable awk — no GNU-only
# `sed -n 2~2p`).
mapfile -t all_names < <("${script_dir}/ironclaw-coverage-int-tier-tests.sh" | awk 'NR % 2 == 0')
...
case "${mode}" in
  flat-partition)
    partition_count="${IRONCLAW_COV_LANE_PARTITIONS:?IRONCLAW_COV_LANE_PARTITIONS must be set for flat-partition mode}"
    partition_index="${IRONCLAW_COV_LANE_INDEX:?IRONCLAW_COV_LANE_INDEX must be set for flat-partition mode}"

    if ! [[ "${partition_count}" =~ ^[0-9]+$ ]] || [ "${partition_count}" -lt 1 ]; then
      echo "IRONCLAW_COV_LANE_PARTITIONS must be a positive integer; got '${partition_count}'" >&2
      exit 1
    fi
    partition_count_int=$((10#${partition_count}))

    if ! [[ "${partition_index}" =~ ^[0-9]+$ ]]; then
      echo "IRONCLAW_COV_LANE_INDEX must be an integer in [0, ${partition_count_int}); got '${partition_index}'" >&2
      exit 1
    fi
    partition_index_int=$((10#${partition_index}))

    if [ "${partition_index_int}" -ge "${partition_count_int}" ]; then
      echo "IRONCLAW_COV_LANE_INDEX must be an integer in [0, ${partition_count}); got '${partition_index}'" >&2
      exit 1
    fi

    mapfile -t flat_names < <(printf '%s\n' "${all_names[@]}" | grep '^ironclaw_integration_' | LC_ALL=C sort)
    ...
    ;;
  group)
    mapfile -t selected_names < <(printf '%s\n' "${all_names[@]}" | grep '^ironclaw_group_' | LC_ALL=C sort)
    ;;
  *)
    echo "Unknown IRONCLAW_COV_LANE_MODE: ${mode} (expected 'flat-partition' or 'group')" >&2
    exit 1
    ;;
esac
...
if [ "${#selected_names[@]}" -eq 0 ]; then
  ...
  # (mirrors run-ironclaw-root-partition.sh). Write an empty tracefile so the
  # caller's `cargo llvm-cov report`-less contract (this script always
  # produces output_lcov) holds even in the empty case.
  echo "No IronClaw integration-tier suites assigned to this coverage lane (mode=${mode}); passing by design"
  : > "${output_lcov}"
  exit 0
fi
