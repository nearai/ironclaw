#!/usr/bin/env bash
#
# R-section cases for ironclaw-coverage-ratchet.sh (coverage-floor ratchet
# gate), split out of test-ironclaw-coverage.sh to keep that file under 1000
# lines as a fifth script's worth of cases joined the suite.
#
# NOT standalone: this file assumes it is `source`d from
# test-ironclaw-coverage.sh, reusing that script's shell state verbatim —
...
# assigned by the sourcing parent (test-ironclaw-coverage.sh); shellcheck can't
# see that when this file is linted on its own (only when the parent lints it
# with `-x`, which resolves cleanly).

# ---------------------------------------------------------------------------
# R. ironclaw-coverage-ratchet.sh (coverage-floor ratchet gate)
# ---------------------------------------------------------------------------
#
# ironclaw-coverage-ratchet.sh <lcov-path> <exemptions-toml> <floor-toml>. All
