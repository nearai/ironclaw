#!/usr/bin/env python3
"""Verify the required served IronClaw Responses API E2E inventory is complete."""
...
MANIFEST = ROOT / "tests/e2e/ironclaw_responses_e2e_tests.txt"
PRIMARY = ROOT / "tests/e2e/scenarios/test_ironclaw_responses_api.py"
LEGACY_PORT = ROOT / "tests/e2e/scenarios/test_ironclaw_webui_v2_legacy_responses_api.py"
ROUTE_TESTS = {
    "test_ironclaw_openai_compat_route_mounts_authenticated_aliases_served",
    "test_ironclaw_openai_compat_route_mounts_require_bearer_served",
}
...
def expected_node_ids() -> set[str]:
    primary = {
        ...
        if name.startswith("test_ironclaw_responses_") or name in ROUTE_TESTS
    }
...
def main() -> int:
    ...
    print(f"IronClaw Responses E2E manifest is complete ({len(expected)} tests).")
    return 0
