#!/usr/bin/env bash
#
# Regression tests for the IronClaw coverage CI helpers:
#   - ironclaw-coverage-merge-lcov.sh     (per-lane lcov merge + crate filter)
#   - ironclaw-coverage-summary.sh        (report + --zero-crates modes)
#   - ironclaw-coverage-comment.sh        (sticky PR comment upsert via `gh api`)
#   - ironclaw-coverage-int-tier-tests.sh (int-tier suite discovery)
#   - ironclaw-coverage-ratchet.sh        (coverage-floor ratchet gate)
#
...
#
# ironclaw-coverage-summary.sh and ironclaw-coverage-ratchet.sh share one lcov-
# parsing + exemption-filtering + by-crate-aggregation implementation
# (scripts/ci/lib/ironclaw_coverage_lcov.py) — the M/A/B/C sections below are
# this module's regression proof (exercised transitively through both
# consuming scripts), not a separate lib-level test file.
#
# The R section (ironclaw-coverage-ratchet.sh cases) lives in the sibling
# test-ironclaw-coverage-ratchet-cases.sh, `source`d near the end of this file —
...
#
# ironclaw-coverage-summary.sh and ironclaw-coverage-comment.sh consume a merged,
# crate-filtered lcov tracefile (scripts/ci/ironclaw-coverage-merge-lcov.sh) plus
...
# .github/workflows/ironclaw-tests.yml downloads 5 per-lane lcov artifacts,
...
#
# ironclaw-coverage-comment.sh shells out to `gh api`. It is exercised here
...
#
# ironclaw-coverage-int-tier-tests.sh derives its repo root from its own path
...
merge_sh="${script_dir}/ironclaw-coverage-merge-lcov.sh"
summary_sh="${script_dir}/ironclaw-coverage-summary.sh"
comment_sh="${script_dir}/ironclaw-coverage-comment.sh"
int_tier_sh="${script_dir}/ironclaw-coverage-int-tier-tests.sh"
ratchet_sh="${script_dir}/ironclaw-coverage-ratchet.sh"
...
# ---------------------------------------------------------------------------
# M. ironclaw-coverage-merge-lcov.sh
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# A. ironclaw-coverage-summary.sh (default report mode)
# ---------------------------------------------------------------------------
...
assert_contains "A1: aggregate matches hand-computed 86.67% (130/150)" "${CAP_OUT}" \
  '**Line coverage (IronClaw crates): 86.67%** — 130 / 150 lines'
...
assert_contains "A2: prints no-data message when the lcov file is empty" "${CAP_OUT}" \
  "No IronClaw crate coverage data found"
...
SF:/work/ironclaw/crates/ironclaw_zero/src/a.rs
LF:10
LH:0
end_of_record
SF:/work/ironclaw/crates/ironclaw_half/src/a.rs
LF:10
LH:5
end_of_record
EOF
...
assert_contains "A5: zero-covered crate row shows 0%" "${CAP_OUT}" "| \`ironclaw_zero\` | 0% | 0 / 10 |"
assert_line_before "A5: zero-covered crate sorted to top (lowest-covered first)" "${CAP_OUT}" \
  "\`ironclaw_zero\`" "\`ironclaw_half\`"

# A6: the crate filter now covers ALL crates/ironclaw_* (all workspace
# crates the int-tier suites link — a superset of the historical IronClaw-only
...
assert_contains "A6: any crates/ironclaw_* crate is included (not just the old IronClaw allowlist)" "${CAP_OUT}" \
...
assert_contains "A6: aggregate drops the non-crates file's 999 lines (5/10, not 5/1009)" "${CAP_OUT}" \
  '**Line coverage (IronClaw crates): 50%** — 5 / 10 lines'
...
# ---------------------------------------------------------------------------
# B. ironclaw-coverage-summary.sh --zero-crates
# ---------------------------------------------------------------------------

cat > "${fixtures_dir}/b1_mixed_zero.lcov" <<'EOF'
SF:/work/ironclaw/crates/ironclaw_zero_a/src/a.rs
LF:5
LH:0
end_of_record
SF:/work/ironclaw/crates/ironclaw_zero_b/src/a.rs
LF:3
LH:0
end_of_record
SF:/work/ironclaw/crates/ironclaw_partial/src/a.rs
LF:10
LH:4
end_of_record
SF:/work/ironclaw/crates/ironclaw_full/src/a.rs
LF:10
LH:10
end_of_record
EOF
...
assert_eq "B1: --zero-crates emits exactly the 2 zero-covered crate names" \
  "$(printf 'ironclaw_zero_a\nironclaw_zero_b')" "${CAP_OUT}"
...
# ---------------------------------------------------------------------------
# C. ironclaw-coverage-comment.sh (sticky PR comment upsert via a fake `gh`)
# ---------------------------------------------------------------------------
...
[
  ...
  { "id": 99, "body": "<!-- ironclaw-coverage-sticky -->\n\nstale summary body" }
]
JSON
...
if [ -f "${c1_log}" ]; then
  ...
  assert_contains "C1: POST body starts with the sticky marker" "${c1_body}" "<!-- ironclaw-coverage-sticky -->"
...
if [ -f "${c3_log}" ]; then
  ...
  assert_contains "C3: body contains the 0-coverage callout" "${c3_body}" \
    "⚠️ 1 IronClaw crate(s) have 0 int-tier coverage"
  assert_line_before "C3: callout is prepended before the coverage header" "${c3_body}" \
    "⚠️ 1 IronClaw crate(s) have 0 int-tier coverage" "## IronClaw integration-tier coverage"
else
...
# ironclaw-coverage-summary.sh has no exposed seam to splice "before the
# per-crate table" specifically — see ironclaw-coverage-comment.sh's own
# header comment).
...
if [ -f "${c10_log}" ]; then
  ...
  assert_line_before "C10: ratchet section precedes the 0%-crate callout" "${c10_body}" \
    "### Coverage ratchet" "⚠️ 1 IronClaw crate(s) have 0 int-tier coverage"
  assert_line_before "C10: ratchet section precedes the coverage header" "${c10_body}" \
    "### Coverage ratchet" "## IronClaw integration-tier coverage"
else
...
# malformed — missing [global] — so ironclaw-coverage-ratchet.sh itself exits
...
# ---------------------------------------------------------------------------
# D. ironclaw-coverage-int-tier-tests.sh (int-tier suite discovery)
# ---------------------------------------------------------------------------
#
...
setup_int_tier_case() {
  ...
  cp "${int_tier_sh}" "${case_dir}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
  chmod +x "${case_dir}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
  : > "${case_dir}/Cargo.toml"
  local candidate
  for candidate in "$@"; do
...
capture "${d1}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
...
assert_contains "D1: empty tests/integration/ prints the discovery error" "${CAP_ERR}" \
  "No IronClaw integration-tier test binaries discovered"

# D2: one tests/integration/foo.rs -> --test / ironclaw_integration_foo.
d2="${tmp_root}/d2"
setup_int_tier_case "${d2}" ironclaw_integration_foo
: > "${d2}/tests/integration/foo.rs"
capture "${d2}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
...
assert_eq "D2: single flat integration file emits its --test pair" \
  "$(printf -- '--test\nironclaw_integration_foo')" "${CAP_OUT}"

# D3: one tests/integration/group_bar/main.rs -> --test / ironclaw_group_bar.
d3="${tmp_root}/d3"
setup_int_tier_case "${d3}" ironclaw_group_bar
mkdir -p "${d3}/tests/integration/group_bar"
: > "${d3}/tests/integration/group_bar/main.rs"
capture "${d3}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
...
d4="${tmp_root}/d4"
setup_int_tier_case "${d4}" ironclaw_group_beta ironclaw_group_omega ironclaw_integration_alpha ironclaw_integration_zeta
...
capture "${d4}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
assert_exit_code "D4: multiple suites exits 0" 0 "${CAP_RC}"
assert_eq "D4: multiple suites sorted+deduped in expected order" \
  "$(printf -- '--test\nironclaw_group_beta\n--test\nironclaw_group_omega\n--test\nironclaw_integration_alpha\n--test\nironclaw_integration_zeta')" \
  "${CAP_OUT}"
...
d5="${tmp_root}/d5"
setup_int_tier_case "${d5}" ironclaw_integration_only
: > "${d5}/tests/integration/only.rs"
mkdir -p "${d5}/tests/integration/support"
: > "${d5}/tests/integration/support/mod.rs"
capture "${d5}/scripts/ci/ironclaw-coverage-int-tier-tests.sh"
...
assert_eq "D5: support/ dir is not discovered as a suite" \
  "$(printf -- '--test\nironclaw_integration_only')" "${CAP_OUT}"

# ---------------------------------------------------------------------------
# R. ironclaw-coverage-ratchet.sh (coverage-floor ratchet gate)
# ---------------------------------------------------------------------------
#
# Split into test-ironclaw-coverage-ratchet-cases.sh (sourced below, sharing
...
# shellcheck source=./test-ironclaw-coverage-ratchet-cases.sh
source "${script_dir}/test-ironclaw-coverage-ratchet-cases.sh"
