...
# IronClaw shared-persistence "group" suites are subdirectory `[[test]]` binaries
# (tests/integration/group_*/main.rs, `[[test]]` name ironclaw_group_<x>). Unlike
# the single-file ironclaw_integration_*.rs suites, each spins up one runtime and
...
test_timeout="${IRONCLAW_GROUP_TEST_TIMEOUT:-28m}"

# The directory basename is `group_<x>`; the `[[test]]` `name` field is
# `ironclaw_group_<x>` (see Cargo.toml) — the two differ by the `ironclaw_` prefix,
# so rewrite it explicitly rather than assuming dir basename == test name (e.g.
# tests/integration/group_memory -> ironclaw_group_memory). The `sh -c` predicate
...
if [ "${#test_names[@]}" -eq 0 ]; then
  echo "No IronClaw group tests discovered" >&2
  exit 1
fi
