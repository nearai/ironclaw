...
use ironclaw::{
    ...
    channels::{
        ...
        WebhookServerConfig, XmppChannel,
...
async fn async_main() -> anyhow::Result<()> {
    ...
    // Add XMPP channel if configured and not CLI-only mode.
    if !cli.cli_only
        && let Some(ref xmpp_config) = config.channels.xmpp
    {
        match XmppChannel::new(xmpp_config.clone()).await {
            Ok(xmpp_channel) => {
                channel_names.push("xmpp".to_string());
                channels.add(Box::new(xmpp_channel)).await;
                tracing::debug!(jid = %xmpp_config.jid, "XMPP channel enabled");
            }
            Err(e) => {
                return Err(e.into());
            }
