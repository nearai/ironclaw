...
pub struct PendingOAuthFlow {
    ...
    /// Secret name for persisting the client secret (MCP OAuth only).
    pub client_secret_secret_name: Option<String>,
    /// Expiry timestamp (Unix seconds) for the client secret, if applicable.
    pub client_secret_expires_at: Option<u64>,
    /// When set, activate the extension automatically after OAuth completes.
