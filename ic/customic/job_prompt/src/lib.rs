...
impl Guest for JobPromptTool {
    fn execute(req: Request) -> Response {
        ...
        host::log(LogLevel::Info, &format!("Processing job prompt request for: {}", input.task_description));
        ...
        if !has_auth {
            return Response {
                output: None,
                error: Some("Authentication required: Missing AUTH_TOKEN or API_KEY secret".to_string()),
            };
        }
...
fn generate_suggestions(input: &JobPromptInput) -> Vec<String> {
    ...
    if input.skill_level.is_none() {
        suggestions.push("Consider specifying a skill level (beginner, intermediate, advanced, expert)".to_string());
    }
