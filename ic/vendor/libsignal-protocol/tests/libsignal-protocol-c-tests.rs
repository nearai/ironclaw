...
fn mock_ctx() -> Context {
    ...
    Context::new(MockCrypto::new(Crypto::default()).random_func(fake_random_generator())).unwrap()
}

#[test]
fn test_curve25519_generate_public() {
    const ALICE_PRIVATE: &[u8] = &[
        0xc8, 0x06, 0x43, 0x9d, 0xc9, 0xd2, 0xc4, 0x76, 0xff, 0xed, 0x8f, 0x25, 0x80, 0xc0, 0x88,
        0x8d, 0x58, 0xab, 0x40, 0x6b, 0xf7, 0xae, 0x36, 0x98, 0x87, 0x90, 0x21, 0xb9, 0x6b, 0xb4,
        0xbf, 0x59,
    ];
    const ALICE_PUBLIC: &[u8] = &[
        0x05, 0x1b, 0xb7, 0x59, 0x66, 0xf2, 0xe9, 0x3a, 0x36, 0x91, 0xdf, 0xff, 0x94, 0x2b, 0xb2,
        0xa4, 0x66, 0xa1, 0xc0, 0x8b, 0x8d, 0x78, 0xca, 0x3f, 0x4d, 0x6d, 0xf8, 0xb8, 0xbf, 0xa2,
        0xe4, 0xee, 0x28,
    ];
    let ctx = mock_ctx();

    let alice_private_key = PrivateKey::decode_point(&ctx, ALICE_PRIVATE).unwrap();
    let expected_public_key = PublicKey::decode_point(&ctx, ALICE_PUBLIC).unwrap();
...
fn test_generate_pre_keys() {
    const PRE_KEY1: &[u8] = &[
        0x08, 0x01, 0x12, 0x21, 0x05, 0x8f, 0x40, 0xc5, 0xad, 0xb6, 0x8f, 0x25, 0x62, 0x4a, 0xe5,
        0xb2, 0x14, 0xea, 0x76, 0x7a, 0x6e, 0xc9, 0x4d, 0x82, 0x9d, 0x3d, 0x7b, 0x5e, 0x1a, 0xd1,
        0xba, 0x6f, 0x3e, 0x21, 0x38, 0x28, 0x5f, 0x1a, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
    ...
    const PRE_KEY2: &[u8] = &[
        0x08, 0x02, 0x12, 0x21, 0x05, 0x35, 0x80, 0x72, 0xd6, 0x36, 0x58, 0x80, 0xd1, 0xae, 0xea,
        0x32, 0x9a, 0xdf, 0x91, 0x21, 0x38, 0x38, 0x51, 0xed, 0x21, 0xa2, 0x8e, 0x3b, 0x75, 0xe9,
        0x65, 0xd0, 0xd2, 0xcd, 0x16, 0x62, 0x54, 0x1a, 0x20, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
    ...
    const PRE_KEY3: &[u8] = &[
        0x08, 0x03, 0x12, 0x21, 0x05, 0x79, 0xa6, 0x31, 0xee, 0xde, 0x1b, 0xf9, 0xc9, 0x8f, 0x12,
        0x03, 0x2c, 0xde, 0xad, 0xd0, 0xe7, 0xa0, 0x79, 0x39, 0x8f, 0xc7, 0x86, 0xb8, 0x8c, 0xc8,
        0x46, 0xec, 0x89, 0xaf, 0x85, 0xa5, 0x1a, 0x1a, 0x20, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45,
    ...
    const PRE_KEY4: &[u8] = &[
        0x08, 0x04, 0x12, 0x21, 0x05, 0x67, 0x5d, 0xd5, 0x74, 0xed, 0x77, 0x89, 0x31, 0x0b, 0x3d,
        0x2e, 0x76, 0x81, 0xf3, 0x79, 0x0b, 0x46, 0x6c, 0x77, 0x3b, 0x15, 0x21, 0xfe, 0xcf, 0x36,
        0x57, 0x79, 0x58, 0x37, 0x1e, 0xa5, 0x2f, 0x1a, 0x20, 0x60, 0x61, 0x62, 0x63, 0x64, 0x65,
...
fn test_generate_signed_pre_key() {
    ...
    const SIGNED_PRE_KEY: &[u8] = &[
        ,
    ];
    let ctx = mock_ctx();
...
fn test_generate_identity_key_pair() {
    const IDENTITY_KEY_PAIR: &[u8] = &[
        0x0a, 0x21, 0x05, 0x8f, 0x40, 0xc5, 0xad, 0xb6, 0x8f, 0x25, 0x62, 0x4a, 0xe5, 0xb2, 0x14,
        0xea, 0x76, 0x7a, 0x6e, 0xc9, 0x4d, 0x82, 0x9d, 0x3d, 0x7b, 0x5e, 0x1a, 0xd1, 0xba, 0x6f,
        0x3e, 0x21, 0x38, 0x28, 0x5f, 0x12, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
fn test_curve25519_signature() {
    ...
    const ALICE_IDENTITY_PUBLIC: &[u8] = &[
        0x05, 0xab, 0x7e, 0x71, 0x7d, 0x4a, 0x16, 0x3b, 0x7d, 0x9a, 0x1d, 0x80, 0x71, 0xdf, 0xe9,
        0xdc, 0xf8, 0xcd, 0xcd, 0x1c, 0xea, 0x33, 0x39, 0xb6, 0x35, 0x6b, 0xe8, 0x4d, 0x88, 0x7e,
        0x32, 0x2c, 0x64,
    ];
    ...
    let _alice_private_key = PrivateKey::decode_point(&ctx, ALICE_IDENTITY_PRIVATE).unwrap();
    let alice_public_key = PublicKey::decode_point(&ctx, ALICE_IDENTITY_PUBLIC).unwrap();
    let _alice_ephemeral = PublicKey::decode_point(&ctx, ALICE_EPHEMERAL_PUBLIC).unwrap();

    let got = alice_public_key.verify_signature(ALICE_EPHEMERAL_PUBLIC, ALICE_SIGNATURE);
    assert!(got.is_ok());
    ...
    for i in 0..ALICE_SIGNATURE.len() {
        ...
        let got = alice_public_key.verify_signature(ALICE_EPHEMERAL_PUBLIC, &modified);
        assert!(got.is_err());
    }
}
...
fn test_hkdf_vector_v2() {
    ...
    const OKM: &[u8] = &[
        0x6e, 0xc2, 0x55, 0x6d, 0x5d, 0x7b, 0x1d, 0x81, 0xde, 0xe4, 0x22, 0x2a, 0xd7, 0x48, 0x36,
...
fn test_basic_pre_key_v2() {
    ...
    .unwrap();
    let alice_session_builder = sig::session_builder(&ctx, &alice_store, &bob_address);
...
fn test_optional_one_time_pre_key() {
    ...
    let alice_session_cipher = sig::SessionCipher::new(&ctx, &alice_store, &bob_address).unwrap();

    // Create an outgoing message
    let msg = "L'homme est condamn� � �tre libre";
    let outgoing_message = alice_session_cipher.encrypt(msg.as_bytes()).unwrap();

    // Convert to an incoming message (this is technically a downcast from
    // CiphertextMessage to PreKeySignalMessage)
    let incoming_message = PreKeySignalMessage::try_from(outgoing_message).unwrap();
