...
impl ClaudeBridgeRuntime {
    ...
    pub async fn run(&self) -> Result<(), WorkerError> {
        ...
        let initial_session = match self
        ...
        let mut session_id = initial_session.session_id;

        // Follow-up loop: only consume prompts that are already queued when a
        // Claude session ends. If nothing is waiting, complete immediately so
        // autonomous subagents exit instead of idling in a poll loop.
        ...
        loop {
            match self.poll_for_prompt().await {
                ...
                Ok(None) => {
                    tracing::info!(
                        job_id = %self.config.job_id,
                        "No follow-up prompt queued after terminal Claude result; completing Claude job"
                    );
                    break;
                }
                Err(e) => {
                    tracing::warn!(
                        job_id = %self.config.job_id,
                        "Prompt polling error after Claude session completion: {}", e
                    );
                    break;
                }
            }
    ...
    async fn run_claude_session(
    ...
    ) -> Result<ClaudeSessionOutcome, WorkerError> {
        ...
        let mut terminal_status: Option<ClaudeSessionStatus> = None;
            ...
            match serde_json::from_str::<ClaudeStreamEvent>(&line) {
                Ok(event) => {
                    ...
                    if event.event_type == "result" {
                        terminal_status = Some(if event.is_error.unwrap_or(false) {
        ...
        match terminal_status.unwrap_or(ClaudeSessionStatus::Completed) {
            ClaudeSessionStatus::Completed => {
                if terminal_status.is_none() {
                    self.report_event(
                        "result",
                ...
                Ok(ClaudeSessionOutcome { session_id })
            }
            ClaudeSessionStatus::Error => Err(WorkerError::ExecutionFailed {
                reason: terminal_message
                    .unwrap_or_else(|| "Claude session reported an error".to_string()),
            }),
        }
    }
