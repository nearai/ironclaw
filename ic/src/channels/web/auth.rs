...
/// DB-backed token authenticator stub.
///
/// In this build, DB authentication is not available. All authentications
/// fall through to env-var token auth via `CombinedAuthState`.
#[derive(Clone)]
pub struct DbAuthenticator;

impl DbAuthenticator {
    pub fn new(_store: std::sync::Arc<dyn crate::db::Database>) -> Self {
        Self
    }
}
...
pub async fn auth_middleware(
    ...
    if let Some(auth_header) = headers.get("authorization")
        ...
        && let Some(identity) = auth.env_auth.authenticate(&value[7..])
    {
    ...
    if allows_query_token_auth(&request)
        ...
        && let Some(identity) = auth.env_auth.authenticate(&token)
    {
        request.extensions_mut().insert(identity.clone());
        return next.run(request).await;
    }
