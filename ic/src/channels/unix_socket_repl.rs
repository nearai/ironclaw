...
impl UnixSocketReplChannel {
    ...
    async fn write_message(
        ...
        let mut line =
            serde_json::to_string(msg).map_err(|e| ChannelError::InvalidMessage(e.to_string()))?;
        line.push('\n');
        write
    ...
    async fn handle_connection(
        ...
        match serde_json::from_str::<ReplMessage>(&buf) {
            ...
            Ok(other) => {
                warn!(
                    ?other,
                    "unix socket REPL: expected Connect, got other message"
                );
                return;
            }
            Err(e) => {
        ...
        loop {
            ...
            match reader.read_line(&mut buf).await {
                ...
                Ok(_) => match serde_json::from_str::<ReplMessage>(&buf) {
                    Ok(ReplMessage::Message { content, .. }) => {
                        let msg = IncomingMessage::new("unix_socket_repl", &owner_id, content)
                            .with_sender_id(session_id.clone())
                            .with_metadata(serde_json::json!({ "unix_session_id": &session_id }));
