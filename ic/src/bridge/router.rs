...
fn local_skill_to_v2(s: &crate::skills::LoadedSkill) -> ironclaw_skills::LoadedSkill {
    ...
    let activation = ActivationCriteria {
        ...
        exclude_keywords: s.manifest.activation.exclude_keywords.clone(),
        patterns: s.manifest.activation.patterns.clone(),
        tags: s.manifest.activation.tags.clone(),
        max_context_tokens: s.manifest.activation.max_context_tokens,
    };
    ...
    let trust = match s.trust {
        crate::skills::SkillTrust::Trusted => SkillTrust::Trusted,
        crate::skills::SkillTrust::Installed => SkillTrust::Installed,
    };

    let source = match &s.source {
        crate::skills::SkillSource::Workspace(p) => SkillSource::Workspace(p.clone()),
        crate::skills::SkillSource::User(p) => SkillSource::User(p.clone()),
        crate::skills::SkillSource::Bundled(p) => SkillSource::Bundled(p.clone()),
    };

    let compiled_patterns = activation
        .patterns
        .iter()
        .filter_map(|p| regex::Regex::new(p).ok())
        .collect();
    let lowercased_keywords = activation
        .keywords
        .iter()
        .map(|k| k.to_lowercase())
        .collect();
    let lowercased_exclude_keywords = activation
        .exclude_keywords
        .iter()
        .map(|k| k.to_lowercase())
        .collect();
...
pub async fn init_engine(agent: &Agent) -> Result<(), Error> {
    ...
    let auth_manager = if let Some(ss) = agent.tools().secrets_store().cloned() {
        // The local SkillRegistry (crate::skills::SkillRegistry) does not carry
        // credential specs (manifest.credentials), so passing None here is
        // semantically correct — there's nothing to look up.
