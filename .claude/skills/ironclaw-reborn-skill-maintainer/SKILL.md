...
Every rule below counters a rot pattern actually observed in this repo's guidance during the 2026-07 audit. Guidance is code: it has callers (agents), it breaks silently, and it needs the same review discipline.
...
1. **Recipes over maps.** Encode grep/verify *procedures* that re-derive facts, not hardcoded file:function maps. Observed: `reborn-feature`'s five grep recipes all survived two months of heavy refactoring; `trace.md`'s hardcoded map died in four places (deleted `src/llm/`, restructured web module). **When someone asks for a "key files list" in guidance, do not ship a bare list** — ship (a) the grep recipes that regenerate it, (b) at most a handful of stable anchor paths, and (c) beside *every* listed path, the one-line command that re-verifies it. A list without its regeneration recipe is `trace.md` waiting to happen.
...
2. **Verify every concrete reference at write time — and make it re-verifiable.** Branches die silently (`webui2-llm-config` was a skill's reference implementation; it's gone). Prefer in-tree worked examples over branch/PR refs. For each cited path/symbol, keep a one-line grep a maintainer can re-run.
3. **No universal claims without a count.** "Every crate has a CLAUDE.md" was false (50/77) the day it was written. Write "most; fall back to Cargo.toml + lib.rs" or generate the list.
...
5. **Never claim enforcement that doesn't exist.** `.claude/rules/architecture.md` cites a pre-commit "ARCH-SPRAWL Check #10" that was never implemented. Before writing "enforced by X", run X. If you add a check to `scripts/pre-commit-safety.sh` or `check-boundaries.sh`, add its self-test (`.claude/rules/review-discipline.md`: guardrail scripts are code) — and know there are two hook install paths (`.githooks/` vs `dev-setup.sh` symlink); a check is only real if both run it.
6. **After any extraction/move/rename, grep the guidance layer** for the old paths in the same PR: `.claude/`, `AGENTS.md`, `CLAUDE.md`, `crates/AGENTS.md`, `docs/reborn/contracts/`, skill bodies. This is how `src/llm/` and `src/channels/web/server.rs` citations outlived their targets.
...
10. **`crates/AGENTS.md` map hygiene**: when adding a crate, add its row; the map has drifted before (18 crates missing at audit time). Absence from the routing map = agents misrouted.
11. **Two evidence layers, two lifecycles.** Skill `references/` files are the **living** teaching copies (worked examples, exemplar tests) — update them as code evolves. Audit/postmortem snapshots are **immutable** — never edit one; cite one only as dated provenance ("the 2026-07 audit found…"), and never make a skill's operative content depend on a document that isn't versioned alongside it.
