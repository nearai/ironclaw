...
5. **Never claim enforcement that doesn't exist.** Before writing "enforced by X", run X. If you add a check to `scripts/pre-commit-safety.sh` or `check-boundaries.sh`, add its self-test (`.claude/rules/review-discipline.md`: guardrail scripts are code) — and know there are two hook install paths (`.githooks/` vs `dev-setup.sh` symlink); a check is only real if both run it.
