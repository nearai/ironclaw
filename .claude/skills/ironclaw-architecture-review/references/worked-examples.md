# Worked Examples — Good and Bad IronClaw Architecture Hygiene

Living curriculum for the checklist in `../SKILL.md`. Each example names its live in-tree exemplar and the command that re-verifies it still holds. Maintained under `ironclaw-skill-maintainer` rules.
...
```rust
/// Re-exported so `ironclaw_cli` never depends on `ironclaw_host_api` directly;
/// `ironclaw_cli_binary_crate_stays_separate_from_v1_root` enforces that edge.
