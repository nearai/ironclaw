---
name: ironclaw-extension-surfaces
description: Use when adding or changing a IronClaw integration — a new extension, a channel surface, model-callable tools, or a shared auth provider — or when deciding whether something is "a channel", "an extension", or "a tool". Maps the unified extension model (NEA-25) to the exact manifest sections, crates, seams, and tests.
---

# IronClaw Extension Surfaces
...
`crates/ironclaw_architecture/tests/ironclaw_retired_taxonomy.rs` — if your
...
and `[auth.*]` sections. The design is law: `docs/ironclaw/extension-runtime/`
...
3. Behavior lives in the extension's `ChannelAdapter` (`inbound` parse →
   ...
   `IronClawBuildInput::with_channel_extension_bindings` seam
...
4. Conversation/actor binding is **data, not per-channel code**: the
   ...
   `docs/ironclaw/contracts/conversation-binding.md`. The actor→user resolver is
   `ProviderIdentityActorResolver`
   (`crates/ironclaw_composition/src/provider_identity.rs`),
...
- Always finish with `cargo test -p ironclaw_architecture` — the specificity,
  ...
  `ironclaw_extension_specificity.rs`: put the name in the package/CLI, not
  generic crates.

## Sibling skills

`ironclaw-feature` (wiring a feature through the layers) ·
`ironclaw-architecture-review` (boundaries) ·
`ironclaw-testing` (tiers).
