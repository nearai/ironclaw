---
name: ironclaw-feature
description: Navigate building a user-facing feature in the IronClaw stack (a capability that crosses product_workflow → composition → webui_v2 → runtime/serve → frontend). Use when planning or implementing any new IronClaw settings page, endpoint, facade method, or runtime-backed capability — especially before writing code, to avoid rebuilding what already exists and to wire it in one pass instead of layer-by-layer.
---

# Building a feature in the IronClaw stack
...
example: `IronClawProviderAdmin` (catalog list/set-active over `config.toml`) and
...
grep -rn "async fn " crates/ironclaw_product_workflow/src/ironclaw_services.rs

# Existing composition-side admin/service handles (often already do the read/write)
grep -rln "IronClawProviderAdmin\|IronClaw.*Admin\|IronClaw.*Facade\|ProductCommandService" crates/ironclaw_composition/src
...
grep -rn "Swappable\|Reload\|UpdateSession\|FileExt\|SecretStore" crates/ironclaw_llm/src crates/ironclaw_config/src crates/ironclaw_secrets/src
```
...
browser (webui_v2_static JS)
  └ apiFetch → ironclaw_webui handler (descriptor + route)
      └ Arc<dyn IronClawServicesApi>  (ironclaw_product_workflow facade)
          └ port trait → composition impl (ironclaw_composition)
...
```
ironclaw_cli commands/serve.rs
  → build_runtime_input_with_options(boot) → IronClawRuntimeInput (+ with_* builders)  [runtime/mod.rs]
  → build_ironclaw_runtime(input)            → IronClawRuntime (defined in runtime.rs; factory.rs builds substrate)
...
handle?), and is it already on `IronClawRuntime` / `IronClawServices`, or must I
thread it through?** Thread inputs via `IronClawRuntimeInput::with_*` builders;
...
|---|---|---|
| Port | `ironclaw_product_workflow` | trait + DTOs + error type in `ironclaw_services/<feature>.rs`; re-export in `ironclaw_services.rs` + `lib.rs` |
| Facade | `ironclaw_product_workflow` | `Option<Arc<dyn Port>>` field + `with_*` builder + N `IronClawServicesApi` methods (give them **default "unavailable" bodies** so existing fakes/tests compile untouched) + an error mapper (port error → `IronClawServicesError`, whose ctors are `pub(super)`) |
| Impl | `ironclaw_composition` | the adapter (`mod <feature>.rs`, gated on the right Cargo feature if the dependency it needs is optional, e.g. `libsql`); register in `lib.rs` |
...
| Wiring | `ironclaw_composition` + `ironclaw_cli` | thread inputs through `IronClawRuntimeInput`/`IronClawRuntime`; attach in `build_webui_services`; pass from `serve.rs` |
...
| Tests | `tests/integration/` + crate tests | for whole-turn behavior, add a scripted-model harness case (see `tests/integration/CLAUDE.md` — mock only at the vendor-SDK seam); facade changes extend `crates/ironclaw_product_workflow/tests/ironclaw_services_contract.rs`, and handler changes extend `crates/ironclaw_webui/tests/webui_v2_handlers_contract.rs` |

## Boundary rules (the guardrails that will reject your PR)

- `ironclaw_composition` must **not** depend on the root `ironclaw` crate
  or `src/` — only extracted crates (`ironclaw_llm`, `ironclaw_secrets`,
  `ironclaw_auth`, …). v1 code under `src/channels/web/` is reference-only.
- webui_v2 handlers consume **only** `IronClawServicesApi`. No dispatcher,
...
cargo build -p ironclaw_composition --features libsql
...
- Port: `crates/ironclaw_product_workflow/src/ironclaw_services/llm_config.rs`
- Facade: `LlmConfigService` field + `with_llm_config_service` + delegating
  methods in `crates/ironclaw_product_workflow/src/ironclaw_services.rs`
- Impl: `crates/ironclaw_composition/src/llm_admin/llm_config_service.rs`
...
- Wiring: `build_llm_config_service` attach in
  `crates/ironclaw_composition/src/webui.rs`
