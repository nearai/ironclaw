...
Layer discipline here is enforced by machines, not vibes: `cargo test -p ironclaw_architecture` (38 boundary tests) and the per-crate contract tests are the real reviewers. Your job is the failures machines can't see. The 2026-07 architecture audit of this repo found exactly two decay modes the tests miss — **mass pooling inside a crate** and **speculative abstraction** — plus a set of named patterns to hold changes against.

## Checklist (run all six; each is checkable)

1. **New trait? Demand the second implementation.** A trait with one production impl is a ritual, not a boundary. Before accepting it, name the concrete second impl (a real backend, not a test fake) or the enforced boundary it serves (e.g. it keeps a forbidden dep out of a crate — verifiable in Cargo.toml). "So we can swap formatters later" / "so callers can hold `Arc<dyn …>`" is the exact rationalization to reject — later can add the trait when later arrives; extraction from a concrete type is mechanical. Justified counter-examples to copy: `RootFilesystem` (7 impls), `EmbeddingProvider` (5), `PolicySource` (4), and dependency-inversion ports whose impl *must* live up-layer (`SkillInferencePort`, `CapabilityDispatcher`). Unjustified precedent to not repeat: the `ironclaw_memory`/`ironclaw_memory_native` split (one provider; on audit watch).
2. **New code in `ironclaw_reborn_composition`? Prove it's assembly.** The crate's charter is service-graph wiring (~16k LoC of it); the audit found ~56k LoC of misplaced *behavior* (Slack host, product auth, domain services). If your change adds behavior — delivery logic, auth flows, a domain service, a serve surface — it belongs in an owning crate; the host-side-of-a-product role is a real crate (`ironclaw_reborn_webui_ingress` is the model; a Slack equivalent is roadmapped). Composition gets the `build_*`/`with_*` wiring only.
...
6. **File growing past 1,500 lines (3,000 = tracking issue)?** The rule is real (`.claude/rules/architecture.md` §5) but as of the 2026-07 audit its promised pre-commit check did **not** exist — assume you are the enforcement unless `grep -n 'ARCH-SPRAWL' scripts/pre-commit-safety.sh` now returns a hit. Same for `#[allow(clippy::too_many_arguments)]`: requires an `// arch-exempt: …, plan #NNNN` line above (2026-07 audit: only 12 of 36 allows carried it; don't add to the deficit).
...
| "Composition already has similar code, I'll put it next to that" | That similar code is audited debt with an extraction plan. Precedent-by-pollution isn't placement. |
...
`cargo test -p ironclaw_architecture` · `bash scripts/check-boundaries.sh` (v1-scoped; don't over-trust for crates/) · `cargo clippy -p <crate> --all-targets --all-features -- -D warnings` · if routes changed: `cargo test -p ironclaw_webui_v2 --test webui_v2_descriptors_contract`.
