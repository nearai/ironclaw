...
3. **New dependency edge? Check both the rules and their shape.** Run `cargo test -p ironclaw_architecture`. Know its blind spots: most rules are blocklists, so a *new* crate is unruled by default — if you add a crate, add its boundary rule in the same PR (`crates/ironclaw_architecture/tests/reborn_dependency_boundaries.rs`, `boundary_rules()`); never add a normal dep on the v1-only enclave (`ironclaw_engine`, `ironclaw_tui`, `ironclaw_gateway`, `ironclaw_oauth`).
