...
| Tests | `tests/support/reborn/` | for whole-turn behavior, add a scripted-model harness case (see `tests/support/reborn/CLAUDE.md` — mock only at the vendor-SDK seam); facade/handler changes extend `reborn_services_contract.rs` / `webui_v2_handlers_contract.rs` |
