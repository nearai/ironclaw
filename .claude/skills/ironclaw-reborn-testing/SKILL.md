...
3. **Whole-turn Reborn behavior (submit → runner → loop → reply), deterministic** → the in-process scripted-model harness (`tests/support/reborn/`, run as `cargo test --test reborn_<name>`, zero setup, offline). **Mock only at the vendor-SDK seam** (`TraceLlm`): the real `ironclaw_llm` decorator chain (retry/failover/circuit-breaker) must execute. Mocking at the gateway seam skips it — that's the `RebornBinaryE2EHarness` tier's job (fixture replay of a built binary), not yours by default.
...
- **Regression-per-fix is mechanically enforced** (commit-msg hook + `regression-test-check.yml`). Escape hatch `[skip-regression-check]` exists — using it on a real fix will be questioned in review.
...
- **The integration tier is NOT a PR gate** (as of the 2026-07 audit — re-verify: `grep -n integration .github/workflows/test.yml`): full `--features integration`/Postgres runs post-merge and nightly. A green PR does not mean the integration tier ran; run it locally when your change is DB/runtime-shaped: `cargo test --features integration`.
- **Never silently self-skip.** `if docker_missing { return }` inside a test is banned (observed 2026-07: the Docker sandbox security suite was hidden exactly this way — don't copy it). Skip loudly via feature gates or explicit env opt-outs.
