...
- **Never silently self-skip.** `if docker_missing { return }` inside a test is banned because it hides the suite from CI. Skip loudly via feature gates or explicit env opt-outs.
