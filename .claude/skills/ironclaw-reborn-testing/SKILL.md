...
- **Persistence = both backends.** PostgreSQL + libSQL parity where production-facing; the model is `ironclaw_hooks`' dual-backend shape (`src/postgres_backend/` + `src/libsql_backend/`, proved equivalent by `tests/parity_matrix.rs` and `tests/multi_host_adversarial.rs`). Feature-gate integration tests (`check-boundaries.sh` enforces the gating for root `tests/`).
