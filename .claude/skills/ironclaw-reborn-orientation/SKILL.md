...
- **Doc-name → code-name map** (docs lag the code): `TurnRunnerWorker` (in Architecture.md/design docs) = `TurnRunScheduler` + `RebornTurnRunExecutor` since #5085; "expired lease → RecoveryRequired" = actually terminal `Failed{lease_expired}` (RecoveryRequired is a legacy status).
- **Host-side product code** (serve/delivery/setup for a channel): the model is `ironclaw_reborn_webui_ingress` (WebChat's host side). Do **not** default it into `ironclaw_reborn_composition` — that crate is for assembly, and its existing Slack/auth mass is audited debt scheduled for extraction, not precedent.
- **New prompt templates**: a `.md` file inside the **owning Reborn crate**, loaded via `include_str!` (pattern: `crates/ironclaw_loop_support`, `crates/ironclaw_turns`, `crates/ironclaw_skill_learning` prompt files). `crates/ironclaw_engine/prompts/` is the **v1 engine's** directory — root CLAUDE.md still points there; don't follow it for new Reborn work.
...
3. Per-crate guidance: read the crate's `AGENTS.md`/`CLAUDE.md` first; some crates have none (11 at the 2026-07 audit) — fall back to `Cargo.toml` + `src/lib.rs` docs. The `crates/AGENTS.md` map has gaps (18 crates missing at audit time); absence from the map means nothing.
