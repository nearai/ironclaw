...
- **All new features: Reborn, in `crates/`** — never `src/` (v1 monolith, being strangled; nothing depends on it). Binary: **`ironclaw`**, built from `crates/ironclaw_reborn_cli` (root docs sometimes say "reborn_cli"). For any endpoint/facade/capability work, use the `reborn-feature` skill.
