...
> **v1-maintenance only.** This scaffolds into the legacy `src/` gateway. Per the repo's IronClaw-first rule, use it only to maintain existing v1 behavior — new user-visible events belong in the default IronClaw workspace (projection/SSE frame via `ironclaw_webui` + the event-stream substrate; start from the `ironclaw-feature` skill).
