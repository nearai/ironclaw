...
| Facade | `IronClawServicesApi` in `crates/ironclaw_product_workflow/src/ironclaw_services.rs` | `grep -n "async fn <name>" crates/ironclaw_product_workflow/src/ironclaw_services.rs` |
| Port impl | `crates/ironclaw_composition/src/<feature>*.rs` | `grep -rn "impl <PortTrait>" crates/ironclaw_composition/src` |
...
| Claim + execute | `TurnRunScheduler` → `IronClawTurnRunExecutor` (`crates/ironclaw_runner/src/`) | `grep -n "claim_next_run\|invoke_driver" crates/ironclaw_runner/src/turn_scheduler.rs crates/ironclaw_runner/src/turn_run_executor.rs` |
