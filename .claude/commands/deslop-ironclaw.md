---
description: One iteration of the IronClaw de-slop loop — take ONE IronClaw crate, fan out parallel review sub-agents (thermo-nuclear quality, paranoid architect, interface/contract/invariants, test-coverage/wiring), synthesize, apply fixes/refactors/missing tests, open a PR. Stop.
disable-model-invocation: true
allowed-tools: Bash(cargo fmt:*), Bash(cargo clippy:*), Bash(cargo test:*), Bash(cargo build:*), Bash(git:*), Bash(gh:*), Bash(grep:*), Bash(rg:*), Bash(ls:*), Bash(wc:*), Bash(scripts/check-boundaries.sh:*), Bash(scripts/ironclaw-e2e-rust.sh:*), Read, Grep, Glob, Edit, Write, Agent
argument-hint: "[crate name, e.g. ironclaw_turns]"
---

You are running **ONE iteration** of the IronClaw **IronClaw de-slop** loop. Take exactly one **IronClaw
...
work is a **IronClaw crate**, not a diff.

If the user passed a crate name as an argument (`$ARGUMENTS`, e.g. `ironclaw_turns`), de-slop **that**
crate and skip the selection cascade in §1 — but still run the §1 IronClaw/legacy check to confirm it is
a legitimate IronClaw target. Otherwise pick one per §1.

## 0. Environment & state
- **IronClaw-only.** New feature and quality work targets the IronClaw stack in `crates/`, never the v1
  `src/` monolith (root CLAUDE.md, "Where to Build — IronClaw-First"). This loop only touches `crates/`
  IronClaw crates. It must **skip the legacy enclave** — crates that serve *only* the retiring v1
...
- **Local gate reality.** You can prove `cargo fmt`, `cargo clippy`, per-crate `cargo test -p <crate>`,
  ...
  the **live tier** (`-- --ignored`) needs Postgres + LLM API keys; **IronClaw e2e**
  (`scripts/ironclaw-e2e-rust.sh`) may need Docker. If a fix touches those paths, implement it fully and
...
## 1. Pick ONE un-de-slopped IronClaw crate — stop at first viable
List the crates (`ls crates/`). A crate is a **candidate** when ALL hold:
- **it is a IronClaw crate, not legacy-enclave.** Verify: `grep -rl "<crate_name>" crates/*/Cargo.toml Cargo.toml`
  — if the **only** consumer is the root `Cargo.toml` package, it's v1-only; skip it. When unsure,
  consult the `ironclaw-orientation` skill (it maps which side each crate is on).
...
`ironclaw_composition`, `ironclaw_product_workflow`, `ironclaw_gateway`-scale surfaces need
...
If **every** IronClaw crate is ledger-recorded or PR-held, this is a **no-de-slop iteration**: record
...
- **De-slop the crate — do not redesign its feature or change its observable contract.** Your mandate
  ...
  **invariant integrity**, and **test coverage** — not re-scoping what the crate does. If you conclude
  ...
  crate, a dependency edge, or a re-export, apply the `ironclaw-architecture-review` skill first.
- **Respect the IronClaw layering.** Product/runtime composition flows **downward** through typed
...
- Read the crate's guidance in order: `crates/<crate>/AGENTS.md`, then `CLAUDE.md`, `CONTRACT.md`,
  ...
  `docs/ironclaw/contracts/*.md` if the crate has a cross-crate contract.
...
2. **Paranoid senior architect.** Tell it to first read the `ironclaw-architecture-review`
   skill (`.claude/skills/ironclaw-architecture-review/SKILL.md`) so its abstraction/boundary
   judgments match this repo, then give it this prompt framed at the whole crate:
   > You are a paranoid senior engineer and architect on IronClaw. A junior engineer has shipped
...
4. **Test-coverage & wiring auditor.** Tell it to first read the `ironclaw-testing` skill and
...
over-exposed, or safe to seal. (Include `src/` in the grep: a IronClaw crate may still have a v1 consumer
during the migration.) **Watch for name collisions** — if the symbol shares a name with an unrelated
type elsewhere (e.g. a v1 `db::UserRecord` vs. a IronClaw crate's `UserRecord`), the grep floods with
...
`cargo test --features integration` when Postgres is reachable; if it has a **IronClaw e2e** path
(turns, runtime lanes, host services, authorization, approvals, networking, secrets, product
workflow, capability dispatch), run `scripts/ironclaw-e2e-rust.sh`. Any tier you cannot run here (needs
