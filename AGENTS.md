...
Adding a channel means adding one capability surface of an extension — a `[channel]` section in the `reborn.extension_manifest.v3` manifest plus implementations of the channel traits (`ChannelIngress` / `ChannelDelivery` in `crates/contracts/ironclaw_extension_contracts/src/channel_adapter.rs`, and the one `ReplySink` in `crates/contracts/ironclaw_extension_contracts/src/reply.rs` that every declared `[channel.reply]` binds — `transport = "stream"` reconciles it progressively, `"message"` at terminal only; the run's answer is published by `ironclaw_assistant`'s reply-publication lane (`delivery_coordinator::publication`), never sent by a channel itself), wired through `RebornHostBindings::with_channel_extension_bindings` (`crates/app/ironclaw_composition/src/input.rs`) — never per-channel host code. Start from the `reborn-extension-surfaces` skill; the worked example is `crates/extensions/packages/slack/`; family rules in `crates/extensions/AGENTS.md`.
