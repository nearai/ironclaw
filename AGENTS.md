...
New persistence defaults to `RootFilesystem`/`ScopedFilesystem` and the mount catalog owned by `ironclaw_filesystem` (spec above); composition chooses concrete backends (PostgreSQL, libSQL, local filesystem) by profile. A domain that needs hand-written SQL must use the existing ADR-or-converge review path and name a dedicated ADR: triggers use [ADR 0003](docs/internal/adr/0003-triggers-keeps-hand-written-sql.md), and hooks use [ADR 0004](docs/internal/adr/0004-hooks-keeps-its-predicate-state-backends.md). Tenant telemetry is not a SQL exception: it uses a typed scoped-filesystem repository below `/tenant-shared/telemetry/v0`, with tenant-leading ordered projections and bounded half-open reads. Domain stores are thin typed wrappers and never branch on backend; keep dual-backend parity via shared conformance suites (`.claude/rules/database.md`). Read-modify-write uses the shared bounded CAS helper, never a process-local mutex held across backend I/O.
