...
<Tabs>
  <Tab title="Gateway token (single-user)">
    ...
    If `GATEWAY_AUTH_TOKEN` is not set, IronClaw prints a random 32-character token to the console on boot.
  ...
  <Tab title="API tokens (per-user)">
    ...
    # → {"id":"...","token":"<plaintext shown ONCE>","prefix":"..."}
...
|-------|----------|-------------|
| `input` | yes | A string, or an array of `message` / `function_call_output` items. See [Input shapes](#input-shapes). |
| `model` | no | Must be `"default"`. Per-request model override is not yet supported; configure providers via settings. |
| `instructions` | no | Per-turn system/developer instructions. Injected ahead of `input` as an `<instructions>` block. Not carried by `previous_response_id`. |
...
| `response.in_progress` | `response.in_progress` | Same shell after first activity. |
...
Per the OpenAI spec, `instructions` apply only to the current turn; they are not replayed when you pass `previous_response_id` on a later request. The agent's persistent identity files (`AGENTS.md`, `SOUL.md`, `USER.md`, `IDENTITY.md`) come from workspace memory and are not affected by this field.
...
| `404` | `invalid_request_error` | `previous_response_id` thread not owned by this user, or unknown response id. |
