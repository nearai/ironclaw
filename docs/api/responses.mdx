...
| `model` | yes | Any well-formed model name: non-empty, no surrounding whitespace, no control characters, at most 256 bytes. Carried through as a server-side routing hint and echoed back in the response; provider and model selection is configured via settings. `"default"` is the conventional placeholder. |
...
| `temperature` | no | Sampling temperature, `0.0`–`2.0` inclusive. Forwarded to the underlying provider request; values outside the range are rejected with `400`. |
| `tools` | no | Caller-supplied function tools. See [External tools](#external-tools). Rejected with `400` when the deployment has no external-tools support; an empty `tools: []` is treated as omitted. |
...
- `tool_choice` on a deployment without external-tools support (with external
  tools enabled it is accepted but currently ignored — there is no per-request
  tool-choice surface)
- Non-empty `tools` on a deployment without external-tools support
...
rejected_always = tool_choice
rejected_without_external_tools = tools
...
- **`tool_choice`**: not supported. Requests carrying it are rejected with `400`; `max_output_tokens` and other unlisted OpenAI fields are accepted and ignored.
- **`model`**: required, any well-formed name (≤ 256 bytes). It is a routing hint echoed back in the response; provider and model selection is server-side via settings.
