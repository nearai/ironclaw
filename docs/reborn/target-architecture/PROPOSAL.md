...
│   ├── ironclaw_extension_registry   ▣ [substrates] (moved layer, renamed) manifests (v3 wire/v2 internal),
...
│   ├── ironclaw_assistant            ▣ [products] (narrowed, renamed) ProductSurface implementation, workflow/
...
| `extensions/packages/*` | extension_contracts (+domains their charter names: auth for gsuite, traces/memory/skills/triggers for builtin tools, `memory` for the `[memory]` provider packages) | per charter | ✗ | ✗ (ports only) | **✗ product & product_contracts** for channel packages | ✗ | vendor SDKs allowed here only |
...
Plus the retained named rules: no crate outside `packages/mem0/` and the assembly layer names a memory provider (amended 2026-07-29 from "only composition names `memory_mem0`"); no substrate depends on the composition root; product-API crates never bind sockets; untrusted-ingress paths never construct trusted trigger submitters; concrete extension crates link only from the binary.
...
| 12 | `ironclaw_events` | **retain + rename + move** → `events/ironclaw_event_log` (amended 2026-07-29) | §6.3.1; kills the `events/events` stutter; delete dead jsonl helpers |
...
| 19 | `ironclaw_memory` | **retain + move** → `domains/` | §6.4.4; contract + conformance suite only — providers leave the family |
| 20 | `ironclaw_memory_native` | **retain + move** → `extensions/packages/memory-native/` (amended 2026-07-29) | §6.8.4; becomes the bundled `[memory]` provider package, installed by default; delete dead embedding port + 6 path-shims; drop unused prompt_envelope dep |
| 21 | `ironclaw_memory_mem0` | **retain + move** → `extensions/packages/mem0/` (amended 2026-07-29) | §6.8.4; becomes the alternative `[memory]` provider package; add guidance files |
...
10. **Unresolved decisions (explicitly not decided here):**
    ...
    - the `reborn_` rename batch (composition/config/cli-dir/openai_compat/event_store/identity/traces) is **recommended-but-severable**: every other change in this proposal works with old names. Separately, three renames are **decided, not severable** (2026-07-29 owner review): `ironclaw_events`→`ironclaw_event_log`, `ironclaw_extensions`→`ironclaw_extension_registry`, `ironclaw_product`→`ironclaw_assistant` — the family directories made each old name collapse into a `family/family` stutter.
