...
- **PR #6253** (Illia's interactive architecture-simplification explorer): models the 2026-07-17 design note that `main` already deleted as stale (#6670) and that this proposal supersedes; the explorer should be regenerated against this target or closed, coordinated with its author.
...
| `extensions/ironclaw_first_party_extensions` | allowed (+domains its charter names: auth, extractors, skills, memory, traces, triggers) | allowed | ✗ (ports only) | ✗ | **✗ product & product_contracts** | ✗ | vendor names sanctioned (scan-exempt); invoked only via capability dispatch |
...
| 14 | `ironclaw_event_projections` | **retain-narrow + move** → `events/` | §6.3.3; delete 3 dead subsystems → deps shrink to event_log+host_api |
...
**[#6696-shaped]** 1. `kernel/ironclaw_processes` — nothing structural: a new `ProcessKind` value + registered executor. 2. The owning package/domain — the executor implementation (receives mediated filesystem/network/resource services; e.g. the github package registers `github.repo_index`). 3. `events/`+`product` — progress projection over emitted typed events. Untouched: turns/runner (not a turn), composition (executor registration is input), approvals/resources (generic stages already applied at spawn). Pre-#6696, the same feature lands as a backgrounded capability via `CapabilityHost::spawn_json` with zero crate changes — the proof holds in both worlds. Default owner split: agent-behavior-shaped background work (reflection, self-improvement) is a loop-family concern — a driver/strategy scheduled as a process — while record-shaped jobs live with the owning domain.
...
3. **Process-journal work unmerged.** Four mapping rows are `[#6696]`-tagged (`processes` widening, `run_state` deletion, `approvals` widening, `runner` scheduler/await-edge shed). The target is valid with #6696 **not** landed (those crates simply retain today's charters in their target families); the contingency tags live in PROPOSAL/CHECKLIST/PLAN, while the family specs describe the steady-state target unconditionally by design; nothing else in the proposal depends on it. Do not pre-implement its schema.
...
- ☑ **Open-PR direction not presented as landed:** #6691/#6696 marked `IN FLIGHT` throughout PROPOSAL/CHECKLIST/PLAN (the family specs describe the steady-state target unconditionally by design — their disclaimer routes dispositions here); four mapping rows and two charters carry explicit `[#6696]` contingency; the proposal is valid if neither lands.
