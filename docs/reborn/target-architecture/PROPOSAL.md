...
**Validated against:** worktree `puzzle-carpet` = `origin/main` @ `dde662d5a0e8a9f9595c0a0cab4916e0ae05f1a5` (clean). The dossier's snapshot checkout (`~/work/repos/github.com/nearai/ironclaw`, `main` @ `8da42cef7`) is 5 commits behind this; the delta adds/removes no crates.
...
2. **The audit found vocabulary pooling, not missing layers.** `ironclaw_host_api` is 38% migrated product/channel/manifest vocabulary (its own module docs say so — `state.rs:4-6`, `channel_adapter.rs:12-14`); `ironclaw_turns` is "three crates wearing one name" (8-line `ids.rs` re-export shim, a 13.9k-line `run_profile/` contract tier, an 11k turn kernel); `ironclaw_product` defines ~17 single-impl ports whose real implementations live in composition/extension_host/operator. The fix is to give the pooled vocabulary contract-layer homes — not to invent new conceptual layers.
...
### 2.1 Workspace shape (`CURRENT`, measured this session)
...
**20 `LAYER_MATRIX_EXCEPTIONS` (`:3516-3648`) are the repo's own debt register**, each with a `removes_in` milestone:
...
Bottom→top: `host_api`(fan-in 56, fan-out 0) / `common` / `safety` / `observability` / `prompt_envelope` / `wasm_limiter` / `reborn_config` → `events`/`trust`/`network`/`memory`/`filesystem`/`llm`/… → domain stores (`secrets`, `threads`, `turns`, `extensions`, `reborn_event_store`, `memory_native`, `authorization`, `hooks`, …) → `auth`/`triggers`/`agent_loop`/`mcp`/`approvals`/`conversations`/`event_projections`/`first_party_extensions`/`processes` → `capabilities`/`outbound`/`event_streams` → `host_runtime` → `loop_host` → `runner` + `first_party_extension_ports` → `product` → `openai_compat`/`telegram_extension`/`operator`/`webui` → `extension_host` → `composition` → `ironclaw` (CLI).

Load-bearing facts this proposal is built on (each verified):
- **`extension_host` sits *above* product today** (normal dep on `ironclaw_product`, 106 references), because the ports it implements (delivery resolver/reply-context/admission/pairing/preference-codec) are *defined in product*, and its ingress calls product's sealed host-auth mint (`extension_ingress.rs:88,:94`).
- **`product` sits above `runner`/`loop_host`** for exactly one import each (failure-summary formatters at `projection/turn_events.rs:34`; one prompt constant at `:988`) — both pure data.
...
### 2.4 The four god crates (`CURRENT`, audited module-by-module)

- **`ironclaw_reborn_composition`** (77.0k lines, 51 internal deps, 69 `[dependencies]` entries): ~30% assembly / ~70% behavior. The `runtime/local_dev/**` tree (~11k lines) is the **production** capability/delivery/approval/skill path — `RuntimeSubstrate` has only `None|ProductionShaped` variants and `runtime.rs:3647` hardcodes `let local_runtime = Some(&services);`. Behavior inventory with owners named in §9: approval/authorization policy (~3.9k), trigger poller + trusted submit (~3.4k), WebUI automations panel service, admin-user service, trace capture (1.2k), system-prompt content (`root/default_system_prompt.rs`), HTTP route mounts (`llm_admin/openai_compat_serve.rs` 1.5k), project access gating, Google/NEAR-AI vendor modules. `build_reborn_services` — still cited by `crates/Architecture.md:297,902` — **does not exist**.
- **`ironclaw_extension_host`** (45.5k lines, 34 deps, no guidance file): a ~4k-line generic lifecycle/binding core (#6116) + channel ingress router/verifier (vendor-blind, manifest-recipe driven — the system's cleanest seam) + delivery/egress transports + ~24k of product-serve wiring landed from composition (#6616/#6669) + non-extension strays (`skill_learning.rs`, `bundled_skills.rs`+`build.rs`, Axum pairing routes). 30 public traits, modal production-impl count 1. Unpoliced `include_str!` edges into `../ironclaw_first_party_extensions/assets/…` (6 sites).
- **`ironclaw_host_runtime`** (46.5k lines, 31 internal deps incl. `libsql`, `deadpool-postgres`, `bollard`, `rcgen`): kernel pipeline construction (sole `CapabilityHost` construction site, `production.rs:979`) + closed lane executor + mediated egress/secret staging/obligations (`obligations.rs` 2,966 fuses three owners) + `first_party_tools/` (~7.3k, incl. the 1,867-line `trace_commons` product feature) + 2.9k of pure assembly + an unwired Docker/CA sandbox subsystem ("ships unwired until W6", `Cargo.toml:57-61`).
- **`ironclaw_runner`** (38.3k lines): ~7.2k genuinely-trusted worker control plane (scheduler/executor/exit-applier) + a 24-`with_*` loop-host factory + ~19k assembly that composition already reaches into via 17 module paths (`runtime.rs` `build_*` functions, 4.3k model gateway, 5.1k tool disclosure, 7.7k `subagent/` await-edge machinery that PR #6696 deletes).

### 2.5 Contract-vocabulary pooling (`CURRENT`)

- `ironclaw_host_api` (25.7k lines, 63 files, 46 modules; 45 wildcard-re-exported as a "contract prelude", `lib.rs:85-132`): ~9.8k lines (38%) are product/channel/manifest vocabulary (`product_adapter/` 6,178; `product_surface.rs` 608 — `ProductSurface` trait :352 with 2 production impls; `channel.rs` 732; `recipe.rs` 935; `package_lifecycle.rs` 650; `state.rs` 187; `operator_llm.rs` 140), several with behavior (rendering/parsing/`tracing::error!`/a `tokio::sync::mpsc` channel) that violates the crate's own charter.
- `ironclaw_turns` (33.7k lines): `ids.rs` is **8 lines** and `scope.rs` **1 line** of `pub use ironclaw_host_api::…` — the canonical turn IDs already live in `host_api/src/turn.rs`. Six consumers (`auth`, `event_streams`, `outbound`, `telegram_extension`, `triggers`, `event_projections`) use turns for vocabulary only and already depend on `host_api` directly. `run_profile/` (~13.9k lines, 11 `Loop*Port` traits, its own `CLAUDE.md`) is the real loop-contract tier with 6 consumers; the turn kernel proper (~11k) has 3.
- `ironclaw_product` (51.6k lines): `reborn_services` cluster 17.3k (34%); ~17 of 46 public traits have exactly one production impl, and those impls live in composition/extension_host/operator — ports exist to satisfy layering, and their *location* (product) is what forces `extension_host`/`operator`/`telegram` above product.
...
### 2.7 In-flight work (`IN FLIGHT` — direction, not law)

- **PR #6691** ("Refactor composition assembly into focused builders"): OPEN, non-draft, `REVIEW_REQUIRED`, head `c1ea0232b`. Composition net −9,421 lines; moves automation/communication/project-access/blocked-auth orchestration → product, indexed blocked-auth lookup → turns, bundled-skill migration → extension_host. Directionally identical to this proposal's composition contract; nothing here presumes its exact code.
- **PR #6696** ("Collapse lifecycle state into the row-native process journal"): OPEN **draft**, `CHANGES_REQUESTED`, head `94c6abdbf`, +31,475/−79,182. Its branch-only design doc (`docs/reborn/2026-07-26-process-kernel-next-collapses.md`) proposes: row-native journal; `ProcessKind::CapabilityInvocation` replacing the second lifecycle; **deleting `ironclaw_run_state`** (approval/gate persistence → `ironclaw_approvals`); child dependencies as process edges (deleting runner's 4.6k-line await-edge machinery); checkpoint payload unification; subagent goals as immutable process input; `ProcessSupervisor` generalizing the turn scheduler (runner registers a `ProcessKind::AgentTurn` executor). This proposal adopts the *ownership* direction (processes = lifecycle authority; run_state dissolves; runner narrows) and marks every dependent mapping entry as **contingent on #6696-or-equivalent landing**. Current `main` overlap is precisely characterized in §9 (run_state ↔ processes ↔ turns).
...
- **Against (fatal, in order):**
  ...
  4. **Review evidence argues against big-bang rewrites here**: the July train succeeded as owner-by-owner extractions; the one mega-change in flight (#6696) is draft + changes-requested precisely because collapsing lifecycle state is high-risk.
...
├── substrate/                        ▢ privileged mechanism substrates (kernel-mediated)
...
│   ├── ironclaw_events               ▣ [substrates]             typed redacted event/audit vocabulary + log traits
...
│   ├── ironclaw_memory               ▣ [substrates]             provider-neutral memory contract
│   ├── ironclaw_memory_native        ▣ [substrates]             native filesystem memory provider
│   ├── ironclaw_memory_mem0         ▣ [substrates]             mem0 backend (composition-selected, feature-gated)
...
│   ├── ironclaw_projects             ▣ [substrates]             project entity + membership ACL
...
│   ├── ironclaw_traces               ▣ [substrates] (renamed)   Trace Commons client: envelope/redaction/queue/credits
│   └── ironclaw_outbound             ▣ [substrates]             metadata-only outbound policy/state (sealed grants)
├── kernel/                           ▢ the authority perimeter (a family, deliberately not one crate)
...
│   │                                                            (*absorbs run_state approval/gate records — #6696-contingent)
...
│   │                                                            (*journal/supervisor shape is #6696-contingent DIRECTION)
...
│   └── ironclaw_run_state            ▣ [kernel] (transitional)  RETAINED until #6696-or-equivalent lands; then deleted
├── lanes/                            ▢ execution mechanisms for already-authorized work
│   ├── ironclaw_wasm                 ▣ [runtimes]               WASM component lane (+ wit/ moved beside it)
│   ├── ironclaw_wasm_limiter         ▣ [runtimes]               shared wasmtime ResourceLimiter
│   ├── ironclaw_mcp                  ▣ [runtimes]               MCP lane (host-mediated HTTP only)
│   └── ironclaw_sandbox              ▣ [runtimes] (NEW, by merge) sandbox/process lane: plan contract (ex process_sandbox)
...
│   ├── ironclaw_runner               ▣ [loops] (narrowed)       agent-turn executor, driver registry, loop-host factory
...
│   ├── ironclaw_extensions           ▣ [substrates] (moved layer) manifests (v3 wire/v2 internal), registry,
│   │                                                            installation/membership records
│   ├── ironclaw_extension_host       ▣ [loops] (narrowed, moved layer) generic lifecycle/binding host, ingress
│   │                                                            router+verifier, egress transports, recipes resolver
│   ├── ironclaw_extension_manager    ▣ [products] (NEW, by split) product-side extension management: catalog,
│   │                                                            lifecycle commands/capabilities, channel config,
│   │                                                            pairing workflows, credential views
│   └── packages/                     ▢ one directory per installable package (self-contained)
│       ├── first_party/              ▣ ironclaw_first_party_extensions [loops] — shared native executors
│       │   │                           (gsuite, web-access, coding, skills, + builtin tools absorbed from
│       │   │                           host_runtime/first_party_tools) and the package inventory
│       │   └── assets/<ext>/         ▢ manifest.toml, prompts/, schemas/, wasm/ per package
│       │       └── wasm-src/         ◇ excluded WASM tool source packages (github-tool, google-*-tool, slack-user-tool)
...
│                                       (absorbs ironclaw_telegram_v2_adapter)
├── product/                          ▢ first-party userland above the kernel
│   ├── ironclaw_product              ▣ [products] (narrowed)    ProductSurface implementation, workflow/admission,
│   │                                                            bindings, idempotency, delivery semantics, projections
...
│   ├── ironclaw_composition          ▣ [app] (renamed, narrowed) THE assembly root: deployment selection + wiring only
│   ├── ironclaw_cli                  ▣ [app] (dir renamed; package name stays `ironclaw`) binary: commands, serve,
│   │                                                            concrete-extension binding tables, registrars
│   ├── ironclaw_config               ▣ [app] (renamed, narrowed) boot config contracts (vendor sections removed)
│   └── ironclaw_architecture         ▣ [app]                    architecture tests (test-only crate)
...
(workspace root package)              ▣ ironclaw_reborn_integration_tests — unchanged home of tests/integration
```
...
- **Owns:** `ChannelAdapter` (5-method trait + `VerifiedInbound`/`InboundOutcome`/`NormalizedInboundMessage`/`OutboundEnvelope`/`OutboundPart`/`DeliveryReport`/`TargetQuery`/`TargetCandidate`/`ChannelError` and validators — today `host_api/src/product_adapter/channel_adapter.rs`); `ToolAdapter` + `RestrictedEgress`; `Extension`/`ExtensionEntrypoint`/`ExtensionBindings`/`check_binding` (today split between `host_api::extension` and `extension_host::entrypoint`); channel manifest-surface descriptors (`ChannelDescriptor`/`ChannelIngressDescriptor`/`ChannelEgressDescriptor`/`ChannelPresentation` — today `host_api::channel`); auth recipe schema (`OAuth2CodeRecipe`, `PkceMode`, … — today `host_api::recipe`); memory manifest surface (today `host_api::memory`); `InstallationState` + `LifecyclePublicState` + `AuthAccountState` (today `host_api::state`); channel-identity hooks (`ChannelConnectionScopeSource` etc.); `PreferenceTargetCodec` + `ReplyTargetBindingRef` re-home (today the product/turns types that force `telegram_extension`'s upward edges); **sealed verified-inbound evidence** — the `mark_request_signature_verified`/`mark_shared_secret_header_verified` mint family (today `product::auth` behind `host-auth-mint`), constructible only via the generic ingress verifier's crate-visible seam.
- **Must never contain:** the registry or installation stores (→ `ironclaw_extensions`); any lifecycle execution, binding orchestration, or ingress routing (→ `ironclaw_extension_host`); vendor names (scanner-enforced); WASM/MCP mechanics; product workflow.
...
- **Why a crate:** criterion 1+2 — it is the "extension runtime descriptors move to a neutral contract" target that four W7 exceptions (`mcp/scripts → extensions/resources` vocabulary need) and the channel packages' cleanliness rule both name; it lets `mcp`, `wasm`, `sandbox`, channel packages, `extension_host`, `product`, and the manager share one vocabulary with no registry/product edge. Without it, either lanes depend on the registry crate (today's exception) or packages depend on product (today's telegram shape).
...
- **Why a crate:** criterion 1+2 — six consumers (`agent_loop`, `loop_host`, `hooks`, `capabilities`, `extension_host`, `host_runtime`) need exactly this tier and today reach it through the kernel crate, producing seven W4.3 exceptions and the `hooks`-implements-kernel-vocabulary oddity; the repo already treats `run_profile/` as a distinct contract (own `CLAUDE.md`, ~13.9k lines). The name resolves the "turn_contracts JIT split" milestone: ID vocabulary goes to `host_api::turn` (where it already canonically lives), port/profile vocabulary comes here.
...
### 6.2 `crates/substrate/` — privileged mechanism substrates

**Family role:** durable/reusable *mechanisms* the kernel mediates: storage fabric, secret storage, network policy+transport, safety scanning, cross-cutting tracing. **Belongs:** backend-generic mechanism, containment, fail-closed local invariants. **Does not belong:** authority decisions (kernel family), domain record grammar (domains family), product/vendor behavior. **Layer range:** `substrates`. **Family AGENTS.md:** each crate states its mediation story — who may call it directly (kernel services, composition) and which callers must instead go through host mediation.

#### 6.2.1 `crates/substrate/ironclaw_filesystem` — retain
...
#### 6.2.2 `crates/substrate/ironclaw_secrets` — retain, narrow
...
#### 6.2.3 `crates/substrate/ironclaw_network` — retain, narrow
...
#### 6.2.4 `crates/substrate/ironclaw_safety` — retain
...
#### 6.2.5 `crates/substrate/ironclaw_observability` — retain as-is
...
- **6.4.11 `ironclaw_projects`** — retain (W2 standing decision honored). Project entity/membership ACL/`ProjectRepository`. Change: the authorization-gating adapter (`RebornProjectService`, today 665 lines in composition) moves here as the service half (product port stays in `product_contracts`) — the domain owns its access rules. Why a crate: healthy machine-enforced boundary, real production path.
...
- **6.4.14 `ironclaw_traces`** — retain, rename (from `ironclaw_reborn_traces`), restructure internally. Trace Commons client: envelope schema, deterministic redaction, submission queue/holds/telemetry, credits, device-key onboarding. Fixes: split the 17,467-line `contribution.rs` into chartered modules (schema/redaction/queue/credits/credentials); take a `ScopedFilesystem` instead of raw `dirs`/env access; drop the boundary-laundering re-export modules (`recording`, `paths`) — consumers import the owners; add guidance files. The `trace_commons` model-callable tool moves to the first-party package (§6.8.4). Why a crate: distinct external-service domain with a security-critical redaction obligation.
...
**Family role:** the mediation stages of the effect pipeline plus the two admission/lifecycle authorities. This family *is* the conceptual kernel — deliberately nine crates, because each stage is an independently consumed contract with its own fail-closed rules, and merging them would trade compiler-proven stage separation for module discipline. **Belongs:** decisions, leases, reservations, mediation, lifecycle authority, dispatch composition. **Does not belong:** product UX, loop strategy, vendor anything, lane mechanics, storage backends. **Layer range:** `kernel`. **Family AGENTS.md:** the pipeline diagram (§7), the "no stage skipping — first-party is a ceiling not a bypass" rule, and each crate's stage assignment.
...
- **6.5.3 `ironclaw_approvals`** — retain, widen (**#6696-contingent**). Exact-invocation approval resolution (`ApprovalResolver`: persist approve-record → issue lease, fail-closed ordering) + persistent-approval/auto-approve/permission-override policy stores. Widens by absorbing the approval-request + gate records from `run_state` when the journal collapse lands (`IN FLIGHT` slice 2 names exactly this move). Deletes: `ToolPermissionOverrideStorePort` (0 impls). Stage 3.5: human/policy resolution out-of-band. Why a crate: consent authority distinct from grant matching; 6 consumers.
...
- **6.5.7 `ironclaw_processes`** — retain, widen (**#6696-contingent DIRECTION**). Today: background-capability lifecycle records/stores/`ProcessHost`/`BackgroundProcessManager` (verified: production's only process kind is a backgrounded capability invocation). Target per the process-kernel direction: the **general durable lifecycle authority** — row-native journal, `ProcessSupervisor` (claim/lease/heartbeat/recovery/panic containment/shutdown), process kinds as registered executors (`ProcessKind::AgentTurn` from runner, `CapabilityInvocation` from host_runtime), child dependencies as process edges, checkpoint payload rows, immutable process input. Never: scheduling *policy*, model behavior, approval authority (journal records "waiting on approval X", approvals decides). Stage: durable lifecycle + recovery authority. Why a crate: the one-lifecycle invariant needs one owner; already consumed by capabilities/host_runtime/composition. Everything beyond today's shape is marked `DIRECTION` and gated on #6696-or-equivalent.
- **6.5.8 `ironclaw_turns`** — retain, narrow. The turn admission kernel: `TurnCoordinator` (accept/resume/cancel, one-active-run-per-thread, idempotency), `TurnStateRowStore`, `LoopExitApplier` + evidence validation, turn lifecycle events. Sheds: ID/scope vocabulary (already `host_api`'s), `run_profile/` (→ `loop_contracts`), `external_tool_catalog` (→ product, its self-described owner), the `product_adapter` compatibility re-export. Under #6696, its store/scheduler halves become projections/adapters over `processes` — an *internal* consolidation that does not move the crate. Consumers drop from 17 to ~4 (product, composition, runner, loop_host). Why a crate: admission + exit-validation authority ("LoopExit is a claim, not truth" lives here).
- **6.5.9 `ironclaw_host_runtime`** — retain, narrow (the kernel service graph). Keeps: `DefaultHostRuntime` (+ the `HostRuntime` port for upper tiers), CapabilityHost construction, the closed `RuntimeLaneExecutor` + lane adapters, mediated egress pipeline (policy+secret staging+sanitize), `BuiltinObligationHandler` + staged handoff stores, process executors, memory-service resolution (provider-agnostic), invocation services. Sheds (with owners): `first_party_tools/` → first-party package (§6.8.4); skill-management execution → skills/manager; extension binding/catalog defaults → `extension_host`; `sandbox_process/**` → `lanes/ironclaw_sandbox`; pure assembly (`builder.rs`/`production_wiring`) shrinks into composition-facing factories; `obligations.rs` splits internally into its three chartered owners (obligation handling ∣ staged handoffs ∣ process-obligation store). Never: vendor names, product features, DB drivers beyond what mediation itself needs. Why a crate: the privileged service graph — the thing `kernel-boundary.md` names as "the current concrete composition crate for kernel-facing services"; after narrowing it is exactly that and nothing else.
- **6.5.10 `ironclaw_run_state`** — retain **transitional**, then delete (**#6696-contingent**). Current overlap (verified): `RunRecord`/`RunStatus` duplicate process statuses under the same `InvocationId` with one real consumer (capabilities); `BlockedApproval`/`BlockedAuth` duplicated verbatim in `TurnStatus`; three parallel "what is blocked" handles. Until the collapse lands, it stays in the kernel family with a freeze charter ("no new consumers; approval/gate stores are the live part"); after it lands, approval/gate records live in `approvals` and invocation state is a process projection.
...
**Family role:** the four separated extension responsibilities, physically colocated: neutral surface vocabulary lives in `contracts/` (§6.1.2); this family holds **registry/records**, **generic hosting**, **product-side management**, and **concrete packages**. **Family AGENTS.md:** the unified model recap (extension = only installable product object; surfaces tool/channel/auth; runtime kind is loading, never taxonomy; vendor behavior only under `packages/`), the package-directory self-containment rule, and the package⇒crate rule from §5.

- **6.8.1 `ironclaw_extensions`** — retain, move layer (loops→substrates), re-charter honestly. Owns: manifest schemas (v3 wire / v2 internal / resolved + digest; reparse gate stays), the in-memory registry/catalog snapshots, and the durable installation/membership/credential-binding records with their CAS migrations (`installations.rs` — the charter stops claiming "side-effect free" for the record half; the registry half stays pure). Never: execution, secrets, trust decisions, vendor names outside `#[cfg(test)]`. Deps: `filesystem`, `host_api`, `extension_contracts`, `trust`-vocabulary via `host_api`. Layer substrates legalizes `capabilities → extensions` and `host_runtime → extensions` (two more W7 exceptions gone). Why a crate: the installation-lifecycle *record* authority (caller membership is "the only installation-lifecycle authority" per its contract) + manifest grammar, consumed by 7 crates.
...
- **6.8.3 `ironclaw_extension_manager`** — NEW by split (products layer). The product face of extensions: available-extension catalog + import, lifecycle commands/capabilities (`extension_lifecycle_capabilities`, lifecycle product service, `SharedCommandSurface` — the second `ProductSurface` impl), channel-config product service, pairing workflow orchestration, credential views, admin/operator/skill-auto-activate capability handlers. Owns the extension-management UX semantics; never owns lifecycle *authority* (calls `extension_host`) nor verification. Deps: `product_contracts`, `extension_contracts`, `extensions`, `extension_host`, `auth`, `host_runtime` port vocabulary as needed. Why a crate: it is a coherent product sub-owner with its own surface (composition already composes it as a separate `ProductSurface` implementation today — the crate makes the existing seam physical) and it is the piece whose absence is why `extension_host` currently sits above product. Fed by: the §6.8.2 shed list (the #6616/#6669 arrival inventory).
- **6.8.4 `crates/extensions/packages/`** — the colocated package family.
  - **`packages/first_party/` = `ironclaw_first_party_extensions`** — retain (name kept to limit churn), move path, widen. The sanctioned vendor-name home (scan-exempt), holding: the package inventory (`PACKAGES` table, ids, trust-effects), all `assets/<ext>/` trees (manifest/prompts/schemas/wasm + excluded `wasm-src/` packages), the native executors (gsuite, web-access, coding, skills), **and the builtin first-party tool handlers absorbed from `host_runtime/first_party_tools`** (http, shell, time, json, echo, schemas, outbound-delivery, memory tools, trigger management, skill management/url-install, trace_commons, spawn-subagent stub) — registered through the existing `FirstPartyHandlerRegistrar` pattern the binary already uses for gsuite/web-access. This resolves the remaining W7 host_runtime exceptions and moves ~7.3k product-feature lines out of the kernel. Never: loop-facing types (the cycle that forced `first_party_extension_ports` disappears once host_runtime no longer depends on this crate — after which that ports crate dissolves, §9).
...
- **6.10.4 `ironclaw_architecture`** — retain. Test-only enforcement; gains the §11 additions. Why a crate: test-only isolation by definition.

**`tools/`**: `ironclaw_stress` — retain as excluded-from-default-work diagnostic (workspace member, app layer; §12.8 notes the option of `default-members` exclusion); `ironclaw_silk_decoder` — retain excluded (decide wiring-or-removal per §12.10); root `fuzz/` — delete or re-point (unresolvable today); `ironclaw_safety/fuzz` — retain. **Workspace root** `ironclaw_reborn_integration_tests` — retain unchanged (the integration harness's crate-name dependencies are the churn cost tallied in §12.7).
...
```mermaid
flowchart TD
    EXT["Untrusted external systems\n(browsers, webhooks, OpenAI clients, vendors)"]
    LISTEN["webui listener + gateway middleware\n[product/ironclaw_webui]\nbody/rate/origin limits, host auth"]
    VERIFY["generic ingress router + verifier\n[extensions/ironclaw_extension_host]\nmanifest-recipe signature checks"]
    ADAPT["ChannelAdapter::inbound / protocol adapters\n[extensions/packages/*, product/ironclaw_openai_compat]\nparse only — no authority"]
    PRODUCT["ProductSurface impl + admission\n[product/ironclaw_product]\nbinding, idempotency, command grammar"]
    TURNS["turn admission kernel\n[kernel/ironclaw_turns]\none-active-run, durable accept"]
    PROC["process lifecycle authority\n[kernel/ironclaw_processes]\nclaim/lease/heartbeat/recovery"]
    LOOP["loop userland\n[loop/agent_loop via loop/runner+loop_host]\nstrategy only, port-scoped"]
    CAP["CapabilityHost membrane\n[kernel/ironclaw_capabilities]\ntrust→authz→approval→obligations"]
    STAGES["trust / authorization / approvals /\nresources / runtime_policy\n[kernel family]"]
    HR["mediated services + lane executor\n[kernel/ironclaw_host_runtime]\negress, secret staging, redaction"]
    LANES["lanes: wasm / mcp / sandbox / first-party\n[lanes family + packages]"]
    SUBS["substrates: filesystem / secrets / network\n[substrate family]"]
    EVID["events + event_store\n[events family] durable evidence"]
    PROJ["event_projections → event_streams\nderived, rebuildable, admission-checked"]
    OUT["outbound authority [domains/ironclaw_outbound]\nsealed grants, at-most-once attempts"]
    DELIV["DeliveryCoordinator [product] +\nChannelAdapter::deliver [packages]\nvia mediated egress"]
...
- **T7 — lane outcome → safe evidence** (`host_runtime` sanitize + obligations complete): redaction/output limits/leak checks, resource reconciliation, model-visible sanitized outcome; durable typed events + audit envelopes append (`events`→`event_store`). Raw payloads/host paths/secrets never persist in turn state (unchanged contract).
...
flowchart BT
    ...
    subgraph substrate
      FS[filesystem] ; SEC[secrets] ; NET[network] ; SAF[safety] ; OBS[observability]
    end
    subgraph events_f["events"]
      EV[events] ; ES[event_store] ; EP[event_projections] ; STR[event_streams]
    end
    subgraph domains
      THR[threads] ; CONV[conversations] ; TRG[triggers] ; MEMF[memory family]
    ...
    subgraph extensions_f["extensions"]
      EXR[extensions] ; EXH[extension_host] ; EXM[extension_manager] ; PKG[packages/*]
    end
    subgraph product_f["product"]
      PROD[product] ; OPR[operator] ; OAC[openai_compat] ; WUI[webui] ; HI[host_ingress]
    end
    subgraph app
...
| `substrate/` crates | — | siblings only where charted (filesystem→safety) | ✗ | ✗ | ✗ | ✗ | network owns reqwest; others no HTTP |
| `events/` crates | — | per §6.3 | ✗ | ✗ | ✗ | ✗ | DB/TLS only in event_store |
| `domains/` crates | — | allowed | ✗ kernel (incl. no `capabilities`/`host_runtime`) | ✗ | ✗ | ✗ | HTTP only in llm/traces/auth-engine/mem0 via their charters |
...
| `extensions/ironclaw_extensions` | — | allowed | ✗ | ✗ | ✗ | ✗ | — |
| `extensions/ironclaw_extension_host` | — | allowed | allowed | siblings/loop | **✗ product** (the restored invariant) | ✗ | ✗ axum |
| `extensions/packages/*` | extension_contracts (+domains their charter names: auth for gsuite, traces/memory/skills/triggers for builtin tools) | per charter | ✗ | ✗ (ports only) | **✗ product & product_contracts** for channel packages | ✗ | vendor SDKs allowed here only |
...
Plus the retained named rules: no crate but composition names `memory_mem0`; no substrate depends on the composition root; product-API crates never bind sockets; untrusted-ingress paths never construct trusted trigger submitters; concrete extension crates link only from the binary.
...
| `auth → turns` (follow-up) | the gate-prompt seam becomes a `host_api`/`extension_contracts` port; auth consumes vocabulary from `host_api::turn` |
...
| 7 | `ironclaw_filesystem` | **retain + move** → `substrate/` | §6.2.1; `HsmBackend` gated/deleted; `db.rs` removal stays scheduled |
| 8 | `ironclaw_secrets` | **retain-narrow + move** → `substrate/` | §6.2.2; placeholder subsystem deleted-until-built; direct-consumer tightening |
| 9 | `ironclaw_network` | **retain-narrow + move** → `substrate/` | §6.2.3; test transport behind `test-support` |
| 10 | `ironclaw_safety` | **retain + move** → `substrate/` | §6.2.4; redaction-family unification tracked §12.10 |
| 11 | `ironclaw_observability` | **retain + move** → `substrate/` | §6.2.5; evict `json_value_bytes`; add guidance |
| 12 | `ironclaw_events` | **retain + move** → `events/` | §6.3.1; delete dead jsonl helpers |
| 13 | `ironclaw_reborn_event_store` | **rename + move** → `events/ironclaw_event_store` | §6.3.2; fix stale feature docs; wrap the leaked `Pool` type |
| 14 | `ironclaw_event_projections` | **retain-narrow + move** → `events/` | §6.3.3; delete 3 dead subsystems → deps shrink to events+host_api |
| 15 | `ironclaw_event_streams` | **retain + move** → `events/` | §6.3.4 |
| 16 | `ironclaw_threads` | **retain + move** → `domains/` | §6.4.1 |
| 17 | `ironclaw_conversations` | **retain + move**, internal renames → `domains/` | §6.4.2; `SessionThreadService` collision fix is mandatory; DTO unification with host_api |
| 18 | `ironclaw_triggers` | **retain + move** → `domains/` | §6.4.3; persistence-idiom ADR-or-converge |
| 19 | `ironclaw_memory` | **retain + move** → `domains/` | §6.4.4; justified seam (2 production providers + conformance suite) |
| 20 | `ironclaw_memory_native` | **retain + move** → `domains/` | delete dead embedding port + 6 path-shims; drop unused prompt_envelope dep |
| 21 | `ironclaw_memory_mem0` | **retain + move** → `domains/` | add guidance files; stays composition-only + feature-gated |
| 22 | `ironclaw_skills` | **retain-narrow + move** → `domains/` (layer→substrates) | §6.4.7; delete ~4k dead lines; absorb activation-observer vocab |
| 23 | `ironclaw_auth` | **retain-narrow + move** → `domains/` | §6.4.8; delete `loopback_oauth`; gate `fakes.rs`; drop turns dep via port |
| 24 | `ironclaw_attachments` | **retain-widen + move** → `domains/` | §6.4.9; absorbs its product ports + composition impls (ends a 3-crate accidental seam) |
| 25 | `ironclaw_extractors` | **retain + move** → `domains/` | §6.4.10; typed error; guidance file |
| 26 | `ironclaw_projects` | **retain-widen + move** → `domains/` | §6.4.11; absorbs its composition service adapter (W2 decision honored: not folded) |
| 27 | `ironclaw_reborn_identity` | **rename + move** → `domains/ironclaw_identity` | §6.4.12; absorbs host_api user-identity store ports; resolve dual-binding-store ambiguity |
| 28 | `ironclaw_llm` | **retain-narrow + move** → `domains/` | §6.4.13; delete `reasoning.rs` (dead); fix providers.json reach; add boundary rule |
| 29 | `ironclaw_reborn_traces` | **rename + move + restructure** → `domains/ironclaw_traces` | §6.4.14; split 17.5k-line file; drop re-export laundering; ScopedFilesystem |
| 30 | `ironclaw_outbound` | **retain + move** → `domains/` | §6.4.15; delete 0-impl trait |
| 31 | `ironclaw_trust` | **retain + move** → `kernel/` | §6.5.1; decide inert sources §12.10 |
| 32 | `ironclaw_authorization` | **retain + move** → `kernel/` | §6.5.2 |
| 33 | `ironclaw_approvals` | **retain-widen + move** → `kernel/` | §6.5.3; **[#6696]** absorbs run_state approval/gate records; delete marker trait |
| 34 | `ironclaw_resources` | **retain + move** → `kernel/` | §6.5.4; absorbs budget constants from common |
| 35 | `ironclaw_runtime_policy` | **retain + move** → `kernel/` | §6.5.5 |
| 36 | `ironclaw_capabilities` | **retain + move** → `kernel/` | §6.5.6; internal host.rs split |
| 37 | `ironclaw_processes` | **retain-widen + move** → `kernel/` (layer runtimes→kernel) | §6.5.7; **[#6696]** journal/supervisor DIRECTION |
| 38 | `ironclaw_turns` | **retain-narrow (split out contracts)** → `kernel/` | §6.5.8; sheds run_profile→loop_contracts, vocab→host_api, external_tool_catalog→product |
| 39 | `ironclaw_host_runtime` | **retain-narrow (multi-way shed)** → `kernel/` | §6.5.9; sheds first_party_tools→package, sandbox→lane, assembly→composition-facing factories |
| 40 | `ironclaw_run_state` | **retain-transitional → delete-after-migration** (kernel/) | §6.5.10; **[#6696]**; freeze charter until then. *Shim/duplication class: lifecycle triplication* |
...
| 42 | `ironclaw_wasm` | **retain + move** → `lanes/` (+ `wit/` moves adjacent) | §6.6.1 |
...
| 49 | `ironclaw_runner` | **retain-narrow + move** → `loop/` (layer kernel→loops) | §6.7.3; **[#6696]** scheduler→processes + await-edge deletion; build_* → composition |
| 50 | `ironclaw_hooks` | **retain + move** → `loop/` (layer substrates→loops) | §6.7.4; persistence ADR-or-converge |
| 51 | `ironclaw_extensions` | **retain-recharter + move** → `extensions/` (layer loops→substrates) | §6.8.1; honest charter (records half is stateful) |
...
| 54 | `ironclaw_first_party_extensions` | **retain-widen + move** → `extensions/packages/first_party/` | §6.8.4; absorbs host_runtime first_party_tools; slack/telegram assets move to their packages |
...
| 59 | `ironclaw_product` | **retain-narrow (ports/DTOs out)** → `product/` | §6.9.1 |
...
| 64 | `ironclaw_reborn_composition` | **rename + retain-narrow (multi-way shed)** → `app/ironclaw_composition` | §6.10.1; the shed inventory is the section's list; aligned with #6691 (IN FLIGHT) |
...
| 67 | `ironclaw_architecture` | **retain + move** → `app/` | §6.10.4 + §11 additions |
...
| 70 | `ironclaw_reborn_integration_tests` (workspace root) | **retain** (excluded-tooling/testing) | unchanged home of `tests/integration`; rename churn cost in §12.7 |
...
**Untouched, and why:** `extension_host` (verification is recipe *data*; router/verifier are generic — proven by Slack's 15-line TOML seam), `extension_contracts`/`host_api` (Discord has no genuinely new generic requirement; adding one would be the flagged exception), `product` (binding/admission/delivery semantics are channel-agnostic), kernel/substrates (egress policy comes from the manifest allowlist), `composition` (receives bindings as input — enforced). The specificity scanner keeps "discord" out of everything but the package.

### 10.2 Final-answer delivery through Discord

Covered by 10.1 §2's `deliver` + `list_targets`: `outbound` (domains) already owns opt-in/attempt authority; `product`'s `DeliveryCoordinator` already owns semantics/retry; `extension_host` egress transport already carries mediated sends with host-injected `discord_bot_token`. **Zero non-package code changes** — that is the ownership table from BOUNDARIES.md §Channel, physicalized.
...
3. Nothing else: discovery/publication (`extensions`), binding (`extension_host`), authorization/approval/obligations/resources (kernel), egress+credential injection (`host_runtime`+`network`+`secrets`), evidence (events family) are all generic paths. Approval-if-required comes from the effect kinds + policy — no bypass because it's first-party (invariant 12).
...
2. `product/ironclaw_product` — the command descriptor constant + handler wiring in `RebornServices` (the frozen-surface ratchet forces the generic `invoke` path, not a new facade method) + the durable mutation call into `domains/ironclaw_threads` (the record authority: an `archived_at` state on `SessionThreadRecord`).
...
**[#6696-shaped]** 1. `kernel/ironclaw_processes` — nothing structural: a new `ProcessKind` value + registered executor. 2. The owning package/domain — the executor implementation (receives mediated filesystem/network/resource services; e.g. the github package registers `github.repo_index`). 3. `events/`+`product` — progress projection over emitted typed events. Untouched: turns/runner (not a turn), composition (executor registration is input), approvals/resources (generic stages already applied at spawn). Pre-#6696, the same feature lands as a backgrounded capability via `CapabilityHost::spawn_json` with zero crate changes — the proof holds in both worlds.

### 10.6 A new projection/read model ("extension health")

1. `events/ironclaw_event_projections` — the fold over installation/process/auth/delivery events (rebuildable, scope-checked). 2. `contracts/ironclaw_product_contracts` — the view DTO. 3. `product/ironclaw_product` — the `ProductView` constant + assembly from the projection + readiness sources. 4. `product/ironclaw_webui` — descriptor+handler+screen. Untouched: `events`/`event_store` (no new evidence kind needed), `event_streams` (transport-generic), all authorities (the screen's cache can't become authority — projections have no write ports, now also true in code after §2.6 deletions).
...
3. **Contracts purity:** the three new contracts crates + host_api/common/prompt_envelope get allowlists (internal deps per §6.1) plus a deny on external framework crates (`axum`, `reqwest`, `wasmtime`, `libsql`, `deadpool-postgres`, `tokio` beyond `sync`-free usage where feasible) — turning §6.1's family test into CI.
...
6. **Persistence idiom rule:** outside `substrate/ironclaw_filesystem` and `events/ironclaw_event_store`, no crate may depend on `libsql`/`deadpool-postgres`/`tokio-postgres` — with a shrink-only allowlist seeded `{triggers, hooks}` until their ADR-or-converge resolves (§12.6).
...
One `AGENTS.md` at each family root with the §6 family contract verbatim: what belongs, what does not, allowed layer range + dependency direction, the family's named ports, and the "before adding a crate here" gate. Crate-local guides keep the current convention (AGENTS/CLAUDE/CONTRACT), and the **six crates the audit found guidance-less** (`extension_host`, `operator`, `attachments`, `traces`, `mem0`, `prompt_envelope`, plus `triggers`' missing CLAUDE and `hooks`' missing AGENTS) get files as part of their move — the audit's clearest correlation was guidance-presence ↔ discipline. `crates/AGENTS.md` becomes a thin family index pointing at family roots (ending the drifting 200-line hand-maintained table).
...
2. **Persistence and migration compatibility.** Family moves and renames touch no storage paths. The risky classes are (i) anything #6696 touches (journal import of `/turns/rows/v1`, `/turns/state.json`, `/run-state/**` — that PR's own rollback contract applies and this proposal adds no new schema motion on top), (ii) `config.toml` vendor-section removal (§6.10.3) — constraint: a deprecation window where old sections parse into migration guidance (the existing `reject_legacy_slack_config` shape, relocated), (iii) trigger/hook SQL convergence if chosen (ADR path exists precisely so this is not forced).
3. **Process-journal work unmerged.** Four mapping rows are `[#6696]`-tagged (`processes` widening, `run_state` deletion, `approvals` widening, `runner` scheduler/await-edge shed). The target is valid with #6696 **not** landed (those crates simply retain today's charters in their target families); nothing else in the proposal depends on it. Do not pre-implement its schema.
...
6. **libSQL/PostgreSQL parity.** Unchanged for every fabric-routed domain (parity lives at `RootFilesystem` + conformance suites). The two hand-SQL crates (`triggers`, `hooks`) keep their own parity suites until the ADR-or-converge decision; §11.2.6's allowlist keeps the set from growing.
...
10. **Unresolved decisions (explicitly not decided here):**
    ...
    - the `reborn_` rename batch (composition/config/cli-dir/openai_compat/event_store/identity/traces) is **recommended-but-severable**: every other change in this proposal works with old names.

---

## 13. Final validation checklist

- ☑ **Every current workspace crate accounted for:** §9 rows 1–3, 7–52, and 54–68 cover all 64 `crates/` packages; 69–70 the tools/root packages; 71–74 excluded packages; 4–6, 53 the new crates. (66 workspace + 4 excluded classes; mechanically verified this session — every `cargo metadata` package name appears; no silent omissions, and `first_party_extension_ports`' implicit membership is called out.)
...
- ☑ **Conceptual layers ≠ one-to-one crates:** the kernel is 9 crates; the extension concept is 4 responsibilities across 2 families; families are explicitly non-boundaries (§5 legend, §11.2.1).
...
- ☑ **Open-PR direction not presented as landed:** #6691/#6696 marked `IN FLIGHT` throughout; four mapping rows and two charters carry explicit `[#6696]` contingency; the proposal is valid if neither lands.
...
*Measurements, file:line citations, and per-crate audit detail underlying this document were gathered from the live tree at `dde662d5a` during this session; re-verify against HEAD with the dossier's refresh commands before implementation planning.*
