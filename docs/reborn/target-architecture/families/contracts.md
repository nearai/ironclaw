...
- **Depends on:** `ironclaw_host_api` and `ironclaw_extension_contracts` for channel-facing DTO reuse. ✎ *Corrected 2026-08-01 (WS2.1): this entry and §6.1.3 both list `ironclaw_common`, but the crate has never held that edge and the enforced allowlist (`reborn_dependency_boundaries.rs`, `product_contracts_allowed`) is the two crates named here. WS1.6's `common` narrowing makes the omission deliberate rather than accidental.*
