...
**Layer(s):** substrates (`ironclaw_extension_registry`), loops (`ironclaw_extension_host`, the first-party support crate), products (`ironclaw_extension_manager`, every package crate) · **Crates:** 8 — `ironclaw_extension_registry`, `ironclaw_extension_host`, `ironclaw_extension_manager`, `ironclaw_first_party_extensions`, `ironclaw_slack_extension`, `ironclaw_telegram_extension`, `ironclaw_memory_native`, `ironclaw_memory_mem0` · **Security posture:** the host-to-extension trust membrane — a single generic verifier is the only code permitted to mint sealed confirmed-inbound evidence; concrete packages parse, render, and serve their declared surfaces but can never construct trust, and only the binary may link a concrete package crate.
...
- **Depends on:** `ironclaw_memory`, `ironclaw_filesystem`, `ironclaw_safety`, `ironclaw_host_api`, `ironclaw_prompt_envelope`, `extension_contracts`.
