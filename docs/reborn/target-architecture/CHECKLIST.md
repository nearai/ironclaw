...
- [ ] Colocate packages under `crates/extensions/packages/`: move `first_party_extensions` (with all `assets/**` incl. excluded `wasm-src` packages), `slack_extension`, `telegram_extension`; slack/telegram assets move beside their crates; update `scripts/build-wasm-extensions.sh`, `include_bytes!` paths, CI selectors.
