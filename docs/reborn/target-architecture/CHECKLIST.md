...
- [ ] Verify: `rg -n "ironclaw_product|ironclaw_assistant" crates/extensions/ironclaw_extension_host/Cargo.toml` → nothing; extension-specificity scanner allowlist shrinks accordingly.
...
- [ ] Verify: all `host_runtime→*`, `capabilities→extensions`, `mcp/scripts→*`, `processes→resources` W7 exceptions deleted; `rg "bollard|rcgen" crates/kernel/ironclaw_host_runtime/Cargo.toml` → nothing (path assumes the family move has landed).
...
- [ ] Merge `ironclaw_projects` into `ironclaw_identity` as a `projects` module (decided 2026-07-30): identity's pinned allowlist widens to the merged crate verbatim; composition's project-gating adapter moves in as the module's service half; the product port stays in `product_contracts`; "access resolution is never cached" becomes a module test.
...
- [ ] §5.1 naming rule enforced: dir==package-name assertion for non-package crates + non-crate-dir convention (a dir with `Cargo.toml` starts `ironclaw_`; one without never does) in the family⇄layer test; the rule text lands in `crates/AGENTS.md` and every family `AGENTS.md`.
