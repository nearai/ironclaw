...
- [ ] Complete the turn vocabulary in `host_api::turn`: move `TurnStatus` and the remaining turn DTOs that vocabulary-only consumers need; delete `turns/src/ids.rs` + `scope.rs` + `turns::product_adapter` re-export shims; repoint the six vocabulary-only consumers (`auth`, `event_streams`, `outbound`, `telegram_extension`, `triggers`, `event_projections`) to `host_api`.
...
- [ ] Crate guides added where the audit found none: `extension_host`, `extension_manager` (new), `operator`, `attachments`, `traces`, `memory_mem0`, `prompt_envelope`, `sandbox` (new), `extractors`, `observability`, plus `triggers` CLAUDE and `hooks` AGENTS; contracts crates get CLAUDE files with their admission tests.
...
- [ ] `cargo metadata` package set == PROPOSAL §5 tree (63 workspace members steady-state; script-verified).
