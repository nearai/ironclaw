...
- `ironclaw_webui_v2_static/src/router.rs::tests`:
  ...
  document CSP pins the three CDN script origins, keeps the nonce, and
  carries no `unsafe-eval` / no `'unsafe-inline'` in `script-src`), plus
...
- The HTML shell sets its **own** CSP first (row 3b), so the strict
  default never reaches it. That document CSP is **not** CDN-free: the
  SPA loads React / Tailwind / fonts from `esm.sh`, `cdn.jsdelivr.net`,
  `cdnjs.cloudflare.com`, and Google Fonts at runtime. It is comparable
  to v1 (which also allowed CDNs), hardened with a per-request nonce and
  `object-src 'none'`. The wallet-connect popup (row 3c) is looser still
  (`script-src 'self' 'unsafe-inline' https:`), scoped to one route.

Earlier drafts of this row claimed v2's CSP was "far stricter (no CDN
allowances)" because "the SPA bundles assets" — that is false for the
document and has been corrected: only the API surface is CDN-free.
