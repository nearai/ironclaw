...
| 2 | Constant-time token compare | SHA-256 hash + `subtle::ConstantTimeEq` over digests (`auth.rs:90-94,157-166`) | `EnvBearerAuthenticator` `ct_eq` on raw token (`lib.rs:191`); `SignedTokenSessionStore` verifies an HMAC-SHA256 signature before accepting session bearers (`signed_session_login.rs`) | **Keep** |
