...
2. **Missions / learning missions (Partial).** Engine v2's `MissionManager`
   ...
   through the normal turn pipeline), and `ironclaw_skill_learning` owns
...
4. **CodeAct / self-modify (Partial).** This is the most substantive delta.
   ...
   (`ironclaw_skill_learning`) carries the durable-knowledge half. Net: the
...
- **Unified learning-mission auto-firing** — Partial (note 2). Primitives exist
  (`ironclaw_triggers`, `ironclaw_skill_learning`); orchestration + automation
