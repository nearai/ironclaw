...
| WS-15 | Prompt context assembly: identity-file surface | `ironclaw_turns` + `ironclaw_loop_support` + `ironclaw_reborn` + `src/workspace` | [`prompt-context-assembly.md`](agent-loop-briefs/prompt-context-assembly.md) | Add `HostIdentityContextSource` trait (analogous to existing `HostSkillContextSource`) and the concrete `WorkspaceIdentityContextSource`. Wire it through `ThreadBackedLoopContextPort::load_loop_context()` so `LoopContextBundle.identity_messages` is populated from workspace identity files (`AGENTS.md`, `SOUL.md`, `USER.md`, `IDENTITY.md`, `TOOLS.md`, `BOOTSTRAP.md`) using primary-scope reads. Filename canon shared with `ironclaw_memory::safety::DEFAULT_PROMPT_PROTECTED_PATHS`; `HEARTBEAT.md` stays volatile and is routed after the stable identity prefix. |

**Minimum live-default E2E set:** WS-9 + WS-10 + WS-11 + WS-12 + WS-13 + WS-15 + WS-14 are the smallest combination that lets the normal no-profile submission path run through the framework end-to-end. WS-14's live-default smoke test is the merge gate — it hard-depends on all six siblings because it composes their real adapters with no mocks, including WS-10's `load_checkpoint_payload` in the cancellation/resume cases and WS-15's concrete workspace identity source. An opt-in `reborn-planned-default` smoke may run before WS-15, but it is not a default-path cutover.

**Sequencing:** WS-9, WS-10, WS-11, WS-12, WS-13, and WS-15 run in parallel after the skeleton (WS-0..WS-8) lands; none of those six depend on each other. WS-14 may land an opt-in profile smoke after WS-9/10/11/12/13, but its live-default cutover is **hard-gated** on WS-9/10/11/12/13/15: the merge criterion is a real-host smoke test that composes all adapters with no mocks (the cancellation smoke specifically asserts on WS-10's `load_checkpoint_payload`, blocked-gate resume consumes WS-11 `GateResolved`, and the default-path smoke asserts WS-15 identity messages). All seven follow-up briefs now live under [`agent-loop-briefs/`](agent-loop-briefs/) — pre-written rather than scoped-at-pickup — because each answers a concrete external question (profile-scoped tool surface, identity-file → prompt wiring, durable checkpoint shape, queue substrate seam, milestone surface additions, sync-snapshot cancellation, driver registration) whose answers are stable enough to commit before implementation begins.
