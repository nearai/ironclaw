...
| Memory indexer/search wiring | `ironclaw_memory` now owns the memory-document filesystem adapter seam plus PostgreSQL/libSQL `memory_documents` adapters, but production wiring for `memory_chunks`, document versions, embeddings, and search indexes is not complete. |
