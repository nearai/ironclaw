...
| Secrets | `[exists/partial]` `ironclaw_secrets` service boundary with scoped metadata, AES-256-GCM/HKDF encryption, encrypted-row repository contract, in-memory encrypted repository, feature-gated PostgreSQL/libSQL repository adapters over the existing secrets table, credential mapping metadata, and one-shot secret leases; app-level DB handle composition and runtime injection are not complete |
...
| Secret injection and durability | Scoped secret metadata, credential mapping metadata, AES-256-GCM/HKDF encryption, encrypted-row repository contract, in-memory encrypted repository, feature-gated PostgreSQL/libSQL encrypted repository adapters, and one-shot leases exist; app-level selection from DB handles, keychain master-key resolution, audit emission, rotation, secret lease consumption in obligation handlers, and production `InjectSecretOnce` secret material injection remain to be implemented. |
