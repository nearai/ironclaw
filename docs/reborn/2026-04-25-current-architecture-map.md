...
| Memory DB repository wiring | `ironclaw_memory` now owns the memory-document filesystem adapter seam, but production adapters for existing `memory_documents`, `memory_chunks`, document versions, embeddings, and search indexes are not wired yet. |
