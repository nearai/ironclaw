...
| Process sandbox lane | `[partial MVP]` `ironclaw_process_sandbox` owns dynamic sandbox process execution behind `ProcessSandboxBackend` plus a `ProcessExecutor` adapter, with typed `SandboxProcessPlan` validation, trusted-config Docker mount roots in the first `DockerProcessSandboxBackend`, split install/run phase command construction, credentialed-run broker requirement, egress-lockdown container contract, and broker header rewrite/redaction policy core; full product wiring, secure capture UI, production HTTPS MITM transport, warm pools, and Kubernetes `agent-sandbox` backend remain follow-up work |
