...
- `HostRuntimeServices` exists as a composition-only helper that builds `RuntimeDispatcher`, `CapabilityHost`, and `ProcessHost` handles from shared registry/filesystem/governor/authorizer/runtime/process services.
- Process lifecycle events exist through `EventingProcessStore`, and approval-resolution audit events exist through optional `ApprovalResolver` event sinks; both use shared `EventSink` implementations.
...
| Approvals/resume | `[exists/partial]` pending approval records, invocation fingerprints, approval resolver, fail-closed approval+lease persistence ordering/rollback, best-effort approve/deny audit events with JSONL persistence coverage, in-memory and filesystem-backed exact-invocation leases, `resume_json` replay checks |
...
| Runtime events | `[partial]` dispatcher/process/approval event vocabulary and in-memory/JSONL sinks for requested/selected/succeeded/failed, process lifecycle, and approve/deny audit metadata; event sink failures are ignored so observability outages do not alter runtime or control-plane outcomes |
...
| Durable leases | Filesystem-backed exact-invocation lease persistence now covers issue, claim, consume, revoke, reload, tenant/user/invocation isolation, and fail-closed approval+lease coordination; single-store ACID transactions, full audit retention policy, and reusable approval scopes are not complete. |
| User-facing scoped event API | Dispatcher/process/approval events and JSONL/in-memory sinks exist, but scoped SSE/WebSocket/reconnect APIs and projection reducers are not productized. |
