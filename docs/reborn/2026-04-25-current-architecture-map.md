...
| WASM lane | `[exists]` `WasmRuntimeAdapter` composition in `ironclaw_host_runtime` delegates to configured `WasmRuntime` and can enforce accepted `ApplyNetworkPolicy` obligations through `ironclaw_network::HardenedHttpEgressClient` or `WasmPolicyHttpClient` on host-mediated HTTP imports |
...
| Network execution boundary | Scoped network policy evaluation plus hardened runtime HTTP egress now cover DNS/private-address checks, redirect re-validation, pinned resolution, response-size bounds, and WASM host-HTTP `ApplyNetworkPolicy` enforcement; product proxying, credential injection, trace recording, non-WASM enforcement, and network egress resource reservation are not complete. |
...
| Concrete obligation handlers | Built-ins now cover metadata-only `AuditBefore` and WASM-enforced `ApplyNetworkPolicy` backed by hardened network egress; built-ins for `InjectSecretOnce`, `AuditAfter`, `RedactOutput`, `EnforceOutputLimit`, resource reservation, scoped mounts, and non-WASM network enforcement remain fail-closed until required runtime/input/output plumbing exists. |
