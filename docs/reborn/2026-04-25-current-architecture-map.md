...
| Memory plugin/indexer/search wiring | `ironclaw_memory` now owns the memory backend plugin contract and filesystem adapter plus PostgreSQL/libSQL adapters for `memory_documents`, `memory_chunks`/FTS, and `memory_document_versions`, but external MCP/WASM/Rust backend adapters, embedding-provider integration, metadata inheritance/schema validation, multi-scope search, and full search APIs are not complete. |
