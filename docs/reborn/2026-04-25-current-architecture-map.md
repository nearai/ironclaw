...
| Memory plugin/indexer/search wiring | `ironclaw_memory` now owns the memory backend plugin contract and filesystem adapter plus PostgreSQL/libSQL adapters for `memory_documents`, `memory_chunks`/FTS, and `memory_document_versions`, including metadata inheritance/schema validation and skip-indexing/versioning behavior; external MCP/WASM/Rust backend adapters, embedding-provider integration, multi-scope search, and richer hybrid/vector search APIs are not complete. |
