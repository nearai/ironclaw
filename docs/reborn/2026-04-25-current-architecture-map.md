...
| Approvals/resume | `[exists/partial]` pending approval records, invocation fingerprints, approval resolver, fail-closed approval+lease persistence ordering/rollback, best-effort approve/deny audit events, in-memory and filesystem-backed exact-invocation leases, `resume_json` replay checks |
...
| Runtime events | `[partial]` dispatcher/process/approval event vocabulary and sinks for requested/selected/succeeded/failed, process lifecycle, and approve/deny audit metadata; event sink failures are ignored so observability outages do not alter runtime or control-plane outcomes |
