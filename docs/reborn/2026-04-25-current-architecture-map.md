...
| Memory/workspace filesystem adapter | `[exists/partial]` `ironclaw_memory` owns `/memory/tenants/{tenant}/users/{user}/projects/{project}/...` path grammar, `MemoryDocumentFilesystem`, memory document scope/path types, repository/indexer seams, in-memory repository, PostgreSQL/libSQL adapters over the existing workspace table family, and a chunking indexer ported from current workspace chunk/hash behavior; embedding-provider integration, metadata inheritance, multi-scope search, and full search APIs are not complete |
...
| Memory indexer/search wiring | `ironclaw_memory` now owns the memory-document filesystem adapter seam plus PostgreSQL/libSQL adapters for `memory_documents`, `memory_chunks`/FTS, and `memory_document_versions`, but embedding-provider integration, metadata inheritance/schema validation, multi-scope search, and full search APIs are not complete. |
