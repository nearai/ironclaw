...
- `ProcessHost` exists as the current host-facing `status`, `kill`, `await_process`, and `subscribe` API over scoped process current state; when wired to `ProcessCancellationRegistry`, scoped kill also signals cooperative executor cancellation.
...
| Process persistence | `[exists]` process store/manager records, host-facing `ProcessHost` status/kill/await/subscribe APIs, cooperative cancellation tokens, background completion/failure transition protection, lifecycle events, and resource reservation ownership/cleanup |
...
| Process product APIs | Process records, scoped status/kill/await/subscribe APIs, cooperative cancellation tokens, lifecycle events, and resource cleanup ownership exist as service slices; output streams, forced abort handles, richer scoped read/projection APIs, durable subscription cursors, and event fanout are not complete. |
