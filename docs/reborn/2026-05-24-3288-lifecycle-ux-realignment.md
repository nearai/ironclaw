...
| Extension install/activate/remove | Local-dev composition wires extension search/install/activate/remove through `LifecycleProductFacade`; the Reborn CLI exposes that facade for operator testing, and local-dev plus production host-runtime composition expose model-visible `builtin.extension_search`, `builtin.extension_install`, `builtin.extension_activate`, and `builtin.extension_remove` capabilities over the same lifecycle port. | Extension lifecycle/config/package services behind facade. | Agent-loop lifecycle tools are wired for local-dev and production service profiles; production end-to-end agent runtime remains follow-up. |
