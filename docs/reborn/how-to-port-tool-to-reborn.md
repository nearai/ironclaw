...
| WASM API tool | Web search, Gmail, Google Drive/Sheets/Docs/Slides, GitHub, Slack user tools | Installed or bundled `RuntimeKind::Wasm` extension using `crates/ironclaw_wasm/wit/tool.wit` |
...
```text
<tool-extension>/
  -> wasm32-wasip2 component implementing crates/ironclaw_wasm/wit/tool.wit
...
| `wit_version` | build/compat note; component must match host `crates/ironclaw_wasm/wit/tool.wit` |
