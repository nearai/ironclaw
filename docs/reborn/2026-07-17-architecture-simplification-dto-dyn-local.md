...
| --- | --- | --- |
| **WebUI** | `ironclaw_webui` (routes/listen/auth/serve/static) + neutral Axum route-mount carriers in `ironclaw_host_ingress`; descriptor vocabulary remains in `ironclaw_host_api`; product-auth and runtime-built route mounts are supplied by host assembly over `ProductSurface` | one `WebUiProductAdapter` over `ProductSurface` (SPA stays an asset bundle) |
