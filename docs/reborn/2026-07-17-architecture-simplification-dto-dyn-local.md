...
| --- | --- | --- |
| **WebUI** | `ironclaw_webui` (routes/listen/auth/serve/static) + neutral route-mount vocabulary in `ironclaw_host_api`; product-auth and runtime-built route mounts are supplied by host assembly over `ProductSurface` | one `WebUiProductAdapter` over `ProductSurface` (SPA stays an asset bundle) |
