...
- `ironclaw_event_log` owns the redacted record vocabulary, cursor types, sink traits, and durable log traits.
- `ironclaw_event_store` owns standalone Reborn backend selection and storage adapters for those traits.
...
- parented dispatcher lifecycle events (`dispatch_requested`, `runtime_selected`,
  ...
  in `crates/ironclaw_composition/src/projection/tests/nested_dispatch_stream.rs`.
...
- PostgreSQL and libSQL adapters are available behind the crate's `postgres`
  ...
  `crates/ironclaw_event_store/migrations/`. The standalone `Libsql`
