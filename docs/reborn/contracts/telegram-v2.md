...
## Runtime enablement after v1 retirement

Telegram is owned exclusively by the Reborn channel host. Set
`[telegram].enabled = true` or `IRONCLAW_REBORN_TELEGRAM_ENABLED=true` to
mount it. The retired root runtime, its Telegram WASM channel, and the former
v1/v2 collision arbiter are no longer available.
...
mounted route and a tracer-only feature gate. Deltas now shipped:
...
- **Runtime enablement is Reborn-owned.** The canonical config field and
  `IRONCLAW_REBORN_TELEGRAM_ENABLED` override control the mounted host; no
  legacy collision mode remains.
