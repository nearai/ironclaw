...
Host HTTP egress must receive the host-runtime graph's `NoExposureGuard` at construction time rather than constructing its own detector. This keeps custom guard substitution on `HostRuntimeServices` effective for the HTTP exfiltration boundary and keeps direct `LeakDetector` use isolated inside `no_exposure_guard.rs`. When the guard blocks egress, the host records metadata-only observability (`ExposureBoundary`, capability id, and stable status) through debug diagnostics and any configured audit sink; payload bytes, URLs, headers, and secret previews must never appear in that record.

`ExposureBoundary::PublicApi` is currently wired for HTTP egress. `ExposureBoundary::DurableEvent` is wired for built-in `RedactOutput` publication. `InboundUserText`, `ModelVisibleToolOutput`, `SseEvent`, and `LogDiagnostic` remain explicit follow-up wiring points: future work must add call-site breadcrumbs or concrete guard calls where inbound text is normalized, model-visible tool output is prepared, SSE frames are projected, and diagnostics are emitted. Future customer-supplied redaction patterns must be scoped outside the `ExposureBoundary` enum by tenant/user/project/agent context so one customer's pattern set cannot affect another customer's boundary decisions.
