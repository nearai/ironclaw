...
`HostRuntime::visible_capabilities(...)` is a visibility producer only. `DefaultHostRuntime` applies the neutral `ironclaw_host_api::capability_surface::CapabilitySurfacePolicy` to the extension registry before authorization. Missing capability ids, grants, provider trust, trust ceilings, allowed runtimes, or allowed effects omit capabilities from the surface, as do authorizer denials or missing authorizer grants. An authorizer `RequireApproval` decision remains visible as `VisibleCapabilityAccess::RequiresApproval` only when `include_requires_approval` permits it. That visible status still follows the approval path; it does not grant access or bypass approval storage.

Loop composition resolves that policy once per run and passes the same final value to the host-visible request, the loop-side surface gate, and progressive disclosure. A host-backed capability factory must not replace the resolved value with an independent `allow_all` request: doing so makes metadata lookup and disclosure reason over a wider catalog than the execution surface. Targets absent from the resolved visible snapshot stay opaque and return model-visible capability failures; they do not escape provider-call registration as terminal invalid model output.

Surface versioning returns `sha256:<lower-hex>` over canonical JSON that includes the configured base version, `SurfaceKind`, every policy dimension (including the normalized capability-id scope), visibility-affecting context fields, grants, relevant provider trust ceilings, visible capability ids/providers/runtimes/effects/schemas, selected resource estimates, and visible access status. Unordered policy values and visible payloads are canonicalized before hashing. Direct invocation remains authoritative.
...
First-party `builtin.time` accepts ISO 8601 strings, JSON numbers, and numeric strings for the `input`/`timestamp` fields used by `parse`, `convert`, and `format`, and for both `input`/`timestamp` and `timestamp2` in `diff`. Numeric values represent Unix seconds and may include up to nine fractional digits, including Slack-style timestamps; integral values with an absolute magnitude of at least `100000000000` are interpreted as Unix milliseconds. Naive local timestamps still require an IANA timezone through `from_timezone` or `timezone`, and conversion/formatting preserve the existing timezone behavior.

First-party `builtin.json` keeps inline `data` behavior for parse, stringify, query, validate, and bounded collection analysis. Query and collection operations additionally accept exactly one scoped `file_path` below `/workspace` instead of `data`; the host reads it through the invocation's authorized mount view and rejects files larger than 8 MiB before materializing them. Invalid JSON text returns a structured, bounded parser location so the model can correct its input. Paths support an optional `$` root, root arrays, and repeated adjacent indices such as `$`, `$[1][0]`, `$.nodes[2].data[15][0]`, `[1][0]`, and `nodes[2].data[15][0]`; paths are bounded to 4096 bytes and 256 components and return structured, bounded validation diagnostics for invalid syntax or unresolved components. `length` counts selected arrays or objects, `last` selects the final item of a non-empty array, and `slice` returns an end-exclusive range capped at 4096 items. `aggregate` computes `sum`, `average`, `min`, or `max` over at most 100000 numeric array items; an optional non-negative `value_index` selects the numeric cell from each array row. Integer-only input aggregates exactly: `sum`, `min`, and `max` stay exact integers (results beyond the signed 64-bit range fail with a Resource error rather than rounding), and `average` rounds its exact integer sum once to the nearest double; input that mixes integers and decimals computes in floating point and fails with a Resource error when a sum or average leaves the finite range. Aggregates return the function, contributing item count, and finite numeric value. These operations do not add wildcard, filter, recursive-descent, expression, or function syntax to the path grammar. This contract is pinned by `first_party_tools::json::tests::collection_operations_cover_large_json_analysis_without_jsonpath_expressions`, `first_party_tools::json::tests::collection_operations_reject_unbounded_or_incompatible_inputs`, `first_party_tools::json::tests::aggregate_preserves_exact_integer_arithmetic`, and `json_runs_bounded_collection_operations`; run `cargo test -p ironclaw_host_runtime collection_operations` and `cargo test -p ironclaw_integration_tests --test reborn_integration_tool_call json_runs_bounded_collection_operations -- --exact`.

First-party handlers that need process effects receive execution through the selected `RuntimeProcessPort`. `ProcessBackendKind::UserSandbox` is wired through a typed `RebornRuntimeProcessBinding` that pairs the resolved policy with a `UserSandboxProcessPort`; composition rejects a sandbox profile without this complete binding and rejects the binding on every non-sandbox profile. The sandbox crate derives persistent workspace/checkpoint identity from a collision-resistant digest of `{tenant_id, user_id}`. Projects, threads, turns, and transient invocations for one user therefore share one workspace, while users in the same tenant and identical user ids in different tenants remain isolated. Container user identity and workspace sharing are explicit sandbox configuration, not composition-layer UID guesses or raw permission-bit assumptions.

Docker user sandbox command mounts are derived from the request `MountView`, never from caller-supplied host paths. The transport only resolves a `MountGrant.target` when trusted composition configured a matching virtual-root-to-host-root mount source. Mount aliases become container mount targets, with `/workspace` replacing the default scoped workspace bind when explicitly granted. Read-only binds require read + list + execute and no write/delete; writable binds require read + list + execute + write + delete because Docker directory binds cannot enforce delete separately from write. Unsupported permission shapes, unconfigured virtual roots, container system-path aliases, non-directory targets, and source escapes fail closed before container creation.
...
stdin and are never placed in provider CLI argv.
