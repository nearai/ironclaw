...
First-party `builtin.http` uses a 10 MiB bounded response/output ceiling for website-scale fetches, raises smaller model-supplied response-body limits to that ceiling, and clamps positive timeouts to the 30s runtime wall-clock ceiling. Larger responses still fail closed as output-too-large, and zero limits/timeouts remain invalid input. Filesystem saves are exposed through the separate `builtin.http.save` capability, which requires `WriteFilesystem`, requires `save_to`, and carries only a host-derived narrowed mount grant for the resolved destination. Network-only `builtin.http` must reject `save_to` before egress.
