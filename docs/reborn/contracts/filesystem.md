...
3. Path normalization rejects `..`, NUL/control characters, URLs, and raw host paths before backend resolution, except trusted local single-user `MountView`s may accept an already-authorized raw host prefix that is explicitly present as a mount alias. Raw host aliases are available only when the native path already satisfies the POSIX-shaped `MountAlias` contract; Windows local runtimes use `/workspace` and `/host` for native roots. Hosted and extension-declared mounts must continue to use stable scoped aliases such as `/workspace` or `/host`.
