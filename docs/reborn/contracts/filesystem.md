...
and `NOT` are not operators. Backends translate those shared semantics into
their native query language. A query with no searchable terms is a successful
empty result, not a backend or input failure.
