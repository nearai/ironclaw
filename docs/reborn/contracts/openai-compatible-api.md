...
**Status:** contract, identity, ProductSurface-backed Chat Completions,
...
These first slices are contract-first, with narrow ProductSurface-backed
...
requests through ProductSurface when host composition injects the surface
state. `POST /api/v1/responses`, `POST /v1/responses`, Responses retrieve,
and Responses cancel can use the same injected ProductSurface-backed Responses
...
| --- | --- | --- | --- |
| `/v1/chat/completions` | POST | `ProductSurface` | SSE-capable |
| `/api/v1/responses` | POST | `ProductSurface` | SSE-capable |
| `/v1/responses` | POST | `ProductSurface` | SSE-capable |
| `/api/v1/responses/{response_id}` | GET | `ProjectionOnly` | none |
| `/v1/responses/{response_id}` | GET | `ProjectionOnly` | none |
| `/api/v1/responses/{response_id}/cancel` | POST | `ProductSurface` | none |
| `/v1/responses/{response_id}/cancel` | POST | `ProductSurface` | none |
...
- Chat Completions projection reads must resolve through
  `ProductSurface::query(...)` and the returned canonical
...
- Ref mappings are two-stage: route code may reserve a pending public ref before
  ProductSurface side effects, then bind it to internal product-action,
...
- Timed-out create requests remain bounded by the shared Reborn turn-admission
  reservation held by ProductSurface / TurnCoordinator. OpenAI-compatible
...
- Routes `stream: true` through the projection-backed SSE translator when a
  projection streamer is injected; otherwise rejects it before ProductSurface
  side effects.
...
- Converts OpenAI-compatible messages into a `UserMessagePayload` and submits it
  through `ProductSurface`.
- Resolves the canonical projection read request through
  `ProductSurface::query(...)`, then waits through a
...
- Routes `stream: true` through the projection-backed SSE translator when a
  ...
  before ProductSurface side effects until a dedicated capability-view
  contract exists.
...
- Submits create requests through `ProductSurface` as user-message payloads and
...
- Cancels only when the opaque ref is authorized and bound to a Reborn run ref,
  then submits a typed ProductSurface control action. Nonexistent, unauthorized,
...
Chat and Responses create requests submit through `ProductSurface` and then
...
recorded accepted ProductSurface ack and must not resubmit after an accepted
