...
- After `TurnCoordinator` accepts a submitted run or requeues a resumed/retried run, it may emit a redacted queued-run wake hint containing only the canonical scope, `TurnRunId`, queued status, and event cursor. Under write-through, acceptance is already durable. Under the explicitly selected single-owner recovery-boundary profile, acceptance is process-local until the next recovery boundary and an unexpected restart may require the upstream trigger to re-drive it. Wake delivery is best-effort, is not a source of truth, must not fail the adapter call, and duplicate hints must be harmless.
