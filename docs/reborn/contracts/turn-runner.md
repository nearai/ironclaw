...
- Recovery converges an expired lease directly to a settled state rather than parking it in a distinct recovery status. Every branch clears current runner ownership and emits the matching redacted lifecycle event; the canonical-thread active lock is released exactly when the resulting status is terminal. The transition table:
...
- The legacy `RecoveryRequired` status still exists in the lifecycle vocabulary — it is terminal, releases the active lock, and explicit cancellation of it is terminal `Cancelled` — but expired-lease recovery no longer produces it.
