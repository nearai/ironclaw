...
The current slices define the core lease/recovery state machine, initial PostgreSQL/libSQL persistence adapters, pure `LoopExit` validation/mapping types, trusted `LoopExitApplier` policy derivation from host-owned evidence, and host-runtime production scheduler wiring. The `RetryTurnRequest`/`RetryTurnResponse` and coordinator/store trait contracts exist, but concrete failed-run retry persistence and runner resume execution remain follow-up work. Durable exit-id replay storage, transcript draft validation, side-effect boundary checkpoint cadence inside the loop, and safe explicit retry/fork UX remain follow-up slices.
