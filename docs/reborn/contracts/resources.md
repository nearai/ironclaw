...
The V1 crate started with an in-memory implementation for contract tests. Production now uses `FilesystemResourceGovernor`: process-local, per-account-sharded quota authority backed by the shared `RootFilesystem` append log. Quotas are process-global in the hosted runtime, so the in-process authority owns admission decisions; successful mutations update memory and enqueue their delta while holding the relevant per-account gate, then wait for durable group-commit acknowledgement after releasing that gate. This is not a distributed quota service: deployments must not run multiple independent filesystem governors over the same quota domain. Multi-replica deployments need one elected authority for each quota domain or a different distributed admission primitive. `ResourceGovernorStore` remains as the CAS snapshot compaction store at `/resources/snapshot.json`; restart recovery loads that snapshot and replays `/resources/deltas/log` from the recorded cursor.

The delta journal always uses the atomic `RootFilesystem::append_batch` contract, including for a one-delta flush. Supported SQLite/libSQL backends must classify all numeric `BUSY` and `LOCKED` results, including extended and remote variants, as `FilesystemError::BackendBusy`; supported PostgreSQL backends must classify serialization failures, deadlocks, and lock-unavailable results the same way. The journal retries only that typed transient outcome with bounded jittered backoff and stops starting new attempts after its retry window. Each individual SQL lock wait remains bounded by the backend's own timeout. It must not retry an untyped backend error or any operation that could have committed a partial side effect. In particular, a remote backend that cannot distinguish pre-commit contention from a lost post-commit response must not label that ambiguous transport outcome `BackendBusy`.

The authority mutex owns the journal-generation lifecycle boundary (`Vacant`, `Ready`, or `Recovering`). Invalidation poisons the current authority and publishes `Recovering` before releasing the mutex for journal-thread creation. Enqueue and reload fail closed while that state is visible. A successful restart transitions to `Vacant` for durable reload; a failed restart restores the poisoned `Ready` generation so subsequent work remains fail-closed. The active authority generation is checked while enqueueing, so an operation that raced with recovery cannot write stale in-memory state into a replacement journal worker. A mutation's own successful durable acknowledgement is final: a later request may poison the shared generation, but it cannot retroactively convert the already-committed mutation into an error.

Persistent implementations add a storage failure mode to every governor operation that touches durable state, including `set_limit`. Lock, read, write, serialization, deserialization, or snapshot schema validation failures return `ResourceError::Storage` and must fail closed: callers must not start costed or quota-limited work when storage state cannot be trusted. Default user/project budget seeding is part of that admission boundary; a failed seed read or write must reject model work before provider dispatch instead of continuing without the configured limit. If a journal write still fails after any permitted transient retries, the request returns an error, the failed authority generation is invalidated, and the next operation reloads the snapshot and journal before proceeding. An optimistic mutation from the failed generation must not survive that reload. Recovery never converts the failed request into success and never bypasses durable accounting.

Model-call accounting retains the required terminal action until it is durably confirmed. Cancellation and provider failure retain `Release`; provider success retains `Reconcile(actual)` with the known provider usage. A storage failure retries that same action against the recovered governor, removes the in-flight record only after success, and fails the run closed if durable finalization remains unavailable. Known spend must never be replaced with a release merely to clear a reservation. Durable snapshots are versioned with `schema_version`; unknown top-level fields, partial snapshots, malformed JSON, and unsupported future schema versions are rejected instead of being silently ignored.
...
- journal replacement is installed before the failed authority is exposed for reload, and restart failure leaves the poisoned generation fail-closed
- a mutation that received its own durable acknowledgement remains successful if later work poisons the authority generation
- an operation holding a stale authority generation cannot enqueue into the replacement journal worker
- failed default budget seeding rejects provider admission and is retried on the next call
- failed post-model reconciliation retries known actual usage rather than releasing it as zero spend
