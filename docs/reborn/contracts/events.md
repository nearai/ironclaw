...
Both surfaces carry typed scope metadata and must not contain raw host paths, raw secrets, raw request payloads, approval reasons, invocation fingerprints, lease IDs, lease contents, or runtime output. Runtime `error_kind` fields use the shared host-safe `ErrorKind` contract; unsafe detail-like values are collapsed to `Unclassified`.
