...
Authorization obligations are satisfied through a host-provided `CapabilityObligationHandler` seam. `CapabilityHost` calls `satisfy(...)` after `Decision::Allow { obligations }` and before dispatch, process start, or approval lease claim. For immediate dispatch/resume, it then calls `complete_dispatch(...)` after successful dispatch and before returning output to the caller. If no handler is configured, if the handler reports unsupported obligations, or if the handler fails, the invocation fails closed before the next downstream side effect or final result return. Handler failures use the stable `CapabilityObligationFailureKind` categories and must remain sanitized metadata, not raw input, host paths, secret material, runtime output, or detailed backend errors. The concrete `ironclaw_host_runtime::BuiltinObligationHandler` currently supports metadata-only `AuditBefore`/`AuditAfter`, `ApplyNetworkPolicy` preflight, direct-handle `InjectSecretOnce` lease/consume when a scoped `SecretStore` plus one-shot runtime injection store are configured, recursive `RedactOutput`, and post-dispatch `EnforceOutputLimit`. When composed through `HostRuntimeServices`, accepted network policies are handed to the WASM runtime adapter and can be enforced by `ironclaw_network::HardenedHttpEgressClient` for actual host-mediated WASM HTTP imports. Host runtime may also inject already-resolved HTTP credentials in that hardened WASM egress path after pre-injection leak scanning. Other runtime/input/output plumbing obligations still fail closed. This prevents authorizers from attaching requirements such as audit, output limits, network policy, secret injection, or resource reservations that callers silently ignore.
...
- runtime/input/output plumbing for non-WASM network I/O policy enforcement, spawned/background process output redaction/limits/audit-after, generic runtime environment injection, or credential-account resolution
