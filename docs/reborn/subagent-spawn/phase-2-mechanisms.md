...
| **P2.C** | `ironclaw_turn_runner` | `subagent` `PlannedDriver` + run-profile→driver binding |
| **P2.D** | `ironclaw_turn_runner` | `SubagentCompletionObserver` (`TurnEventSink`) |
...
### P1.C — `ironclaw_turn_runner` data (assumed present)

```rust
// ironclaw_turn_runner/src/subagent/flavor.rs  (P1.C)
...
// ironclaw_turn_runner/src/subagent/goal_store.rs  (P1.C — DB-BACKED in v1)
//
...
// ironclaw_turn_runner/src/subagent/gate_resolution.rs  (P1.C)
...
// ironclaw_turn_runner/src/subagent/tombstone_store.rs  (P1.C)
//
...
// ironclaw_turn_runner/src/subagent/spawn_result_payload.rs  (P1.C — schema)
//
...
// ironclaw_turn_runner/src/subagent/continuation_budget.rs  (P1.C)
//
...
> `ironclaw_turn_runner` at host-factory construction time (Phase 3 wiring). The
> stores themselves live in `ironclaw_turn_runner` (P1.C). This keeps the decorator
...
pub struct SubagentSpawnDeps {
    ...
    pub goal_store:       Arc<dyn SubagentGoalStorePort>,            // from ironclaw_turn_runner
    pub gate_store:       Arc<dyn SubagentGateResolutionStore>,  // from ironclaw_turn_runner
    pub flavor_resolver:  Arc<dyn SubagentFlavorResolver>,       // from ironclaw_turn_runner
...
impl SubagentPromptComposer {
    ...
    pub async fn inline_messages_for(
        ...
        //    direction_md is include_str!'d in ironclaw_turn_runner (P1.C); it is
...
## 3. P2.C — `ironclaw_turn_runner`: `subagent` `PlannedDriver` + run-profile→driver binding
...
```rust
// ironclaw_turn_runner/src/planned_driver.rs — the reusable adapter
...
// ironclaw_turn_runner/src/driver_registry.rs
impl DriverRegistry {
...
## 4. P2.D — `ironclaw_turn_runner`: `SubagentCompletionObserver` (`TurnEventSink`)
...
- `cargo clippy --all --benches --tests --examples --all-features` — zero
  warnings.
- `cargo test` green for `ironclaw_loop_host` and `ironclaw_turn_runner`.
- No new public API in `ironclaw_turns` or `ironclaw_agent_loop` beyond the
  ...
  `ironclaw_turn_runner` only (README §10 crate-boundary table).
