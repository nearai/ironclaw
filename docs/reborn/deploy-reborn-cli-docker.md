...
`Dockerfile.reborn` is the compatibility Dockerfile for the canonical
`ironclaw` binary. It enables the WebUI, Slack, Telegram, OpenAI-compatible,
libSQL, and Postgres production features. The image defaults to:
...
Railway/public deployments. Docker bridge runs must also bind the listener to
`0.0.0.0`; the published host port can remain restricted to `127.0.0.1`.
