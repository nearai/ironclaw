...
Initialize the canonical Reborn runtime, then start its interactive REPL:
...
Follow the [Quickstart Guide](/quickstart) to configure a provider and send your
first message. See the [canonical CLI reference](/reborn-binary) for profiles,
production storage, extensions, and WebUI serve mode.
