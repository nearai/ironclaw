...
Slack is configured entirely from the web interface — install the extension, complete
workspace OAuth, and set the app ids and channel routing from Admin Configuration. The
tokens live in the secret store.

Nothing goes in the configuration file. The `[slack]` setup keys are retired, and
`ironclaw serve` **refuses to start** if it finds `installation_id`, `team_id`,
`api_app_id`, `slack_user_id`, `user_id`, `shared_subject_user_id`, `channel_routes`,
`signing_secret_env`, or `bot_token_env`. `[slack].enabled` is accepted but inert, so an
older install keeps booting; there is no `IRONCLAW_REBORN_SLACK_ENABLED` env var either
— the gate it fed was removed in #6116.
