...
bot only responds to messages that mention it; in a DM it responds to everything.
...
method. `reactions:write` lets the bot mark the message it is working on
(👀 while a run is in progress, ✅ or ❌ when it finishes); without it the
reaction is skipped and the reply still arrives.
...
member, and each message runs as the person who mentioned the bot. In shared channels the
bot answers only when explicitly mentioned — a plain thread reply without a mention is
ignored, though approve/deny replies and `/ironclaw` commands still work; in DMs it answers
every message.
