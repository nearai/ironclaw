...
| `allowed_chat_ids`              | group chat IDs                 | `[]`      | Limit group processing to specific Telegram group chat IDs                      |
...
Use `allowed_chat_ids` to restrict group participation to specific Telegram groups while keeping direct-message policy separate. When the list is empty or omitted, all groups keep the previous behavior. When the list contains group IDs, messages from other groups are ignored. Direct messages are still controlled by `dm_policy`.

Allowed groups bypass `dm_policy` for group messages, so a bot can require pairing for DMs while still allowing anyone in an approved group to mention it. If `owner_id` is set, the owner restriction still takes precedence. Mention handling still follows `respond_to_all_group_messages` and `bot_username`.
...
Telegram group chat IDs are usually negative numbers. You can read them from update logs or a Telegram ID helper bot after sending a message in the group.
