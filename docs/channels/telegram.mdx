...
<Step title="Configure the Telegram channel in IronClaw">
    ...
    ```
    ironclaw-v1 onboard --channels-only
    ```
...
<Step title="Test your Telegram channel">
    Now that you have configured your Telegram channel, it is time to test it out. Start the `ironclaw-v1` agent if it is not already running:
    ...
    ```
    ironclaw-v1 pairing approve telegram <PAIRING_CODE>
    ```
</Step>
...
<AccordionGroup>
  ...
  <Accordion title="Owner binding timeout during setup">
    The wizard waits 120 seconds for the first message. If it times out, send `/start` to your bot in Telegram and re-run `ironclaw-v1 onboard --channels-only`.
  </Accordion>
</AccordionGroup>
