...
<Note>
Current first-party channels are shipped as packaged artifacts. For repository-maintained Reborn channels, use `crates/ironclaw_first_party_extensions/assets/<channel>/manifest.toml` plus checked-in schemas/prompts and CI-built WASM artifacts rather than adding a new legacy source tree.
</Note>
