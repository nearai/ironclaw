...
services:
  ironclaw:
    ...
    volumes:
      ...
      # Docker socket for sandbox job execution (Docker-in-Docker sibling containers)
...
DATABASE_URL=postgres://ironclaw:${POSTGRES_PASSWORD}@postgres:5432/ironclaw
...
docker run --rm \
  -v ironclaw_ironclaw_data:/source:ro \
  -v $(pwd)/backups:/dest \
  alpine tar czf /dest/ironclaw-data-$(date +%Y%m%d).tar.gz -C /source .

# Restore
docker run --rm \
  -v ironclaw_ironclaw_data:/dest \
...
The `docker-compose.yml` above already includes this mount:
