...
|---|---|---|
| **`DESIGN.md` constitution** | Highest-leverage gap; agentic UI without it invites hallucinated props/hex and skipped a11y. | New **repo-root `DESIGN.md`** (root files are exempt from the docs publication boundary; a `docs/` home must be `docs/internal/`, never `docs/design/`), seeded from `templates/DESIGN.template.md`; substitute React/Tailwind/`ironclaw` token prefix. |
