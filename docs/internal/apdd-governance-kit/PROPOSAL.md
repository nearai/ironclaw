...
|---|---|---|
| **`DESIGN.md` constitution** | Highest-leverage gap; agentic UI without it invites hallucinated props/hex and skipped a11y. | New **`docs/internal/design/DESIGN.md`** (under the fenced `internal/` tree, per IronClaw's internal-docs rule; repo-root is the alternative if treated as a `CLAUDE.md`/`AGENTS.md` peer — never `docs/design/`, which would publish), seeded from `templates/DESIGN.template.md`; substitute React/Tailwind/`ironclaw` token prefix. |
...
| **Critical User Journeys registry + `critical-flows` rule** | Turns IronClaw's e2e/Playwright coverage into a named regression baseline with a hot-path auto-load trigger. | `docs/internal/qa/CRITICAL_FLOWS.md` + `.claude/rules/critical-flows.md`. |

### ◑ ADAPT (take the idea; fit it to IronClaw's shape)

| Component | Adaptation |
|---|---|
| **Docs-first feature workflow (Layer ①)** | Adopt the three-doc set + registry, but **scope it to WebUI/product-surface features** and prefer the **lightweight single-file `FEATURE.md` tier** for most. Keep the existing `docs/internal/plans/*` dated plans for backend/infra epics. Site it under the fenced internal tree — `docs/internal/features/<slug>/` — rather than mandating it repo-wide; an unfenced `docs/<slug>/` page would be published by Mintlify and rejected by `scripts/ci/docs_publication_boundary.py`. |
...
| **Impact / dependency spec sections + a cross-functional-review *trigger*** | Adopt the spec's **§5 Architecture Impact** and the plan's **§4 Dependencies & Sequencing** as-is (they make blast radius + ordering explicit before code). For review routing, **do not** import the kit's generic §8 "team review" gate — IronClaw already has stronger raw material. Instead build the piece neither the kit nor IronClaw has today: an **explicit cross-functional-review trigger** — (a) a feature-spec field *"Cross-functional review required: engineering / product / design / none"*, and (b) a path-scoped rule that routes to the owning partner **role** by the area a diff touches — **engineering** for trust / persistence / capability / runtime boundaries, **product** for user-facing behavior or a product-surface contract, **design** for UI or the design system (`DESIGN.md` / tokens / a11y / taxonomy) — pulling the review *checks* from the existing [`review-discipline.md`](../../../.claude/rules/review-discipline.md) + the PR *Reborn Trust-Boundary Checklist* (they describe *what* to inspect, **not who**: neither names people or teams). This turns "circulate for review" into "this diff touches X → the owning-role reviewer must sign off." Mapping a role to an actual person/team is a separate concern (e.g. `CODEOWNERS`), out of scope for these prose rules. |
...
- A **`docs/internal/qa/CRITICAL_FLOWS.md`** catalogs IronClaw's real end-to-end journeys (e.g.
  onboarding/pairing, chat turn, extension auth, automation/job run) with hot-path
