...
- `crates/ironclaw_assistant/src/commands.rs` — registry: alias deletion; `CommandAudience`; `required_audience`.
- `crates/ironclaw_assistant/src/command_admission.rs` — `CommandActorRoleResolver` port + audience step in `DirectConversationCommandAdmission`.
- `crates/ironclaw_assistant/src/run_delivery/observer.rs` — `AccessDenied` feedback copy; user-audience help filter.
...
- `crates/ironclaw_composition/src/factory.rs` (~1260) + `src/runtime.rs` (~1664) — thread `AdminUserService` into deps.
- `crates/ironclaw_first_party_extensions/assets/{slack,telegram}/manifest.toml` — declare `["model", "status"]`.
- Tests: `crates/ironclaw_assistant/tests/product_commands_contract.rs`, `.../product_command_surface_contract.rs`, `.../run_delivery_contract.rs`, `crates/ironclaw_extension_host/src/channel_host/e2e_tests.rs`.
...
- Test: `crates/ironclaw_assistant/tests/product_commands_contract.rs`, `crates/ironclaw_assistant/tests/product_command_surface_contract.rs`
...
- [ ] **Step 1: Pin the new contract in tests (red).** In `product_commands_contract.rs`: `grep -n "progress\|aliases" crates/ironclaw_assistant/tests/product_commands_contract.rs`. At each hit:
...
- [ ] **Step 4: Run product suite; fix remaining alias references.** Run: `cargo test -p ironclaw_assistant --no-fail-fast`
...
- [ ] **Step 5: Sweep the rest of the workspace for alias references.** Run: `grep -rn "aliases" crates/ironclaw_assistant/ crates/ironclaw_extension_host/ --include="*.rs" | grep -v test` and `grep -rn "\"progress\"" crates/ironclaw_extension_host/src/channel_host/e2e_tests.rs`
...
**Files:**
- Modify: `crates/ironclaw_assistant/src/commands.rs`, `crates/ironclaw_assistant/src/lib.rs` (export)
...
Import `CommandAudience`, `required_audience`, `ProductModelCommand`, `LifecycleProductAction` from `ironclaw_assistant` (add to the existing `use`; check `lib.rs` re-exports and extend them if a name is missing).
...
- [ ] **Step 1: Write the failing tests.** In `run_delivery_contract.rs`, locate the existing command-feedback tests (`grep -n "post_command_feedback\|command_help\|PolicyDenied" crates/ironclaw_assistant/tests/run_delivery_contract.rs`) and mirror their harness shape for two new cases:
...
- [ ] **Step 2: Run to verify red.** Run: `cargo test -p ironclaw_assistant --test run_delivery_contract --no-fail-fast`
...
**Files:**
- Modify: `crates/ironclaw_assistant/src/command_admission.rs`, `crates/ironclaw_assistant/src/lib.rs`
- Create: `crates/ironclaw_extension_host/src/channel_command_roles.rs` (+ `lib.rs` module line)
- Modify: `crates/ironclaw_extension_host/src/channel_host.rs` (deps field ~331, admission construction ~761), `crates/ironclaw_extension_host/src/channel_host/e2e_tests.rs` (deps ~426), `crates/ironclaw_composition/src/factory.rs` (~1260), `crates/ironclaw_composition/src/runtime.rs` (~1664)
- Test: `crates/ironclaw_assistant/tests/product_command_surface_contract.rs`
...
```rust
// ironclaw_assistant::command_admission
#[async_trait]
...
pub admin_users: Arc<dyn ironclaw_assistant::AdminUserService>,
```
...
- [ ] **Step 2: Run to verify red.** Run: `cargo test -p ironclaw_assistant --test product_command_surface_contract --no-fail-fast`
...
- [ ] **Step 4: Run the product tier.** Run: `cargo test -p ironclaw_assistant --no-fail-fast`
...
Adjust the two `ProductSurfaceErrorCode` variants and the `ExternalActorRef` accessor (`.id()`) to the actual names found by: `grep -n "pub enum ProductSurfaceErrorCode" -A 20 crates/ironclaw_host_api/src/*.rs` and `grep -n "impl ExternalActorRef" -A 15 crates/ironclaw_assistant/src/*.rs crates/ironclaw_host_api/src/**/*.rs`. Confirm the provider string convention against the existing consumer `crates/ironclaw_extension_host/src/provider_identity.rs:144` (`.resolve_user_identity(&self.provider, provider_user_id)`) and source `provider` from the same per-extension value the assembly gives that policy.

- [ ] **Step 6: Thread deps + per-extension construction.** In `channel_host.rs`:
  - Add to `GenericChannelHostDeps` (~331): `pub admin_users: Arc<dyn ironclaw_assistant::AdminUserService>,` with a doc comment: `/// Admin-users directory backing channel-command role gating.`
...
// crates/ironclaw_assistant/src/reborn_services/admin_users.rs.
```

    Wire `admin_users: Arc::new(FakeAdminUsers::default())` into the deps literal for now (Task 5 makes it configurable).
  - `factory.rs` (~1260) and `runtime.rs` (~1664): thread the `AdminUserService` the composition already builds for the WebUI admin routes (`RebornAdminUserDirectory` in `crates/ironclaw_composition/src/admin_user_directory.rs`, constructed in `product_surface.rs`). Locate that construction (`grep -n "RebornAdminUserDirectory" crates/ironclaw_composition/src/`), share the `Arc` (store it on the same struct that carries `channel_identity_store`, or construct a second adapter from the same `RebornUserDirectory` handle), and set `admin_users:` in both deps literals.

- [ ] **Step 8: Run the crate suites.** Run: `cargo test -p ironclaw_assistant -p ironclaw_extension_host --no-fail-fast` and `cargo build -p ironclaw_composition`
...
- [ ] **Step 9: Run architecture tier (dependency edges changed).** Run: `cargo test -p ironclaw_architecture_tests --no-fail-fast`
Expected: PASS — `extension_host → ironclaw_assistant` already exists; no new edges.

- [ ] **Step 10: Commit.**

```bash
git add crates/ironclaw_assistant crates/ironclaw_extension_host crates/ironclaw_composition
...
- [ ] **Step 5: Full-suite sweep of every crate the PR touched.** Run: `cargo test -p ironclaw_assistant -p ironclaw_extension_host -p ironclaw_first_party_extensions -p ironclaw_composition --no-fail-fast`
...
Run: `cargo test -p ironclaw_architecture_tests --no-fail-fast`
