...
`.claude/rules/testing.md`; for cross-layer IronClaw tests, also read
...
The IronClaw integration harness scripts the vendor model response while keeping
the real product workflow, scheduler, agent loop, LLM decorator chain,
permissions, capabilities, and persistence in the path.

Use this for most new or changed production-wired IronClaw behavior.
...
3. Does the behavior cross Ironclaw components or perform a side effect? Add a
   hermetic IronClaw integration test.
...
For most production-wired IronClaw behavior, start with a hermetic IronClaw
...
2. hermetic IronClaw integration tests
...
| Whole IronClaw turn or cross-component behavior | `tests/integration/` | Use the scripted-model harness and assert at a meaningful seam. |
| Model tool choice or request shape | `tests/fixtures/llm_traces/ironclaw_qa/` and `tests/ironclaw_qa_recorded_behavior.rs` | Commit only scrubbed fixtures. |
| WebUI behavior | `tests/e2e/scenarios/test_<behavior>.py` | Use the IronClaw v2 fixtures for WebChat v2. |
...
| Real model or provider drift | `scripts/ironclaw_webui_v2_live_qa/` | Reuse the current live-QA lane when possible. |
...
flat test binary in the root `Cargo.toml` as `ironclaw_integration_<scenario>`.
...
```bash
cargo test --test ironclaw_integration_greeting
```
...
```bash
cargo test --test ironclaw_group_approvals
```
...
- fixture:
  [`tests/fixtures/llm_traces/ironclaw_qa/web_status_check.json`](../../tests/fixtures/llm_traces/ironclaw_qa/web_status_check.json)
- contract and replay:
  [`tests/ironclaw_qa_recorded_behavior.rs`](../../tests/ironclaw_qa_recorded_behavior.rs)
...
`tests/ironclaw_qa_recorded_behavior.rs`. Add a replay assertion when the trace
...
cargo test --test ironclaw_qa_recorded_behavior
```

### Browser E2E

Example:
[`tests/e2e/scenarios/test_ironclaw_webui_v2_smoke.py`](../../tests/e2e/scenarios/test_ironclaw_webui_v2_smoke.py)

This starts the standalone IronClaw server and proves that an authenticated user
reaches the chat shell while an anonymous user reaches the login screen.

```python
async def test_ironclaw_v2_serves_shell_and_gates_auth(
    ironclaw_v2_server, ironclaw_v2_browser
):
    authed_ctx = await ironclaw_v2_browser.new_context(
        viewport={"width": 1280, "height": 720}
    )
    authed_page = await authed_ctx.new_page()
    await authed_page.goto(
        f"{ironclaw_v2_server}/?token={IRONCLAW_V2_AUTH_TOKEN}"
    )
    await expect(authed_page.locator(SEL_V2["chat_composer"])).to_be_visible()
```

For IronClaw WebChat v2, use the `ironclaw_v2_*` fixtures and `SEL_V2` selectors.
If the scenario must be part of the IronClaw coverage gate, add its file or
pytest node ID to
[`tests/e2e/ironclaw_coverage_tests.txt`](../../tests/e2e/ironclaw_coverage_tests.txt).

```bash
cd tests/e2e
pytest scenarios/test_ironclaw_webui_v2_smoke.py
```
...
- implementation:
  [`scripts/ironclaw_webui_v2_live_qa/run_live_qa.py`](../../scripts/ironclaw_webui_v2_live_qa/run_live_qa.py)
...
Add or extend the case under `scripts/ironclaw_webui_v2_live_qa/`. Reuse the
current `ironclaw-webui-v2-live-qa` lane when possible. Change the workflow only
...
- Read the owning crate's `AGENTS.md`, `CLAUDE.md`, `CONTRACT.md`, or
  `README.md`.
- Read `tests/integration/CLAUDE.md` before changing the IronClaw integration
  harness.
