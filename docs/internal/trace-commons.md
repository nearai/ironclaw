...
at startup. The verifier rejects unsupported JWT algorithms and can run with
...
counts for configured, active, and inactive keys, not key material, key ids, or
...
require-`jti` flag, and whether EdDSA-only signed-token auth is required; it
never returns the secret or configured claim values.
...
fetching or syncing issuer-owned Ed25519 key records and making the issuer
keyset the only production upload-claim rotation path.
...
emergency `jti` denylists, and a deployment flag that rejects non-EdDSA bridge
credentials. The remaining issuer work is managed key distribution and a
production policy that accepts only the managed EdDSA issuer keyset.
