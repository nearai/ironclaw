...
| Central audit log | Partial | File-backed audit rows now include optional hash-chain fields plus a maintenance verifier while preserving legacy JSONL compatibility, and DB audit rows mirror those chain hashes for file-backed events. Audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations, retention/purge artifact invalidations, reviewer analytics/list/review-queue reads, dataset/benchmark/ranker exports, and per-trace replay export content reads, with `object_ref_id` mirrored when the body is read through a DB object ref; privileged delayed credit mutations now mirror typed safe metadata for event type, bounded delta, and hashed reason/reference fields. Production still needs DB-native tamper-evident append ordering, broader worker content-read coverage, and reconciliation. |
