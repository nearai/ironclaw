...
| Replay dataset export | Partial | Approved redacted slices can be exported by reviewer/admin tokens, DB metadata can drive replay export selection when envelope objects remain readable, and manifests now carry source-list hashes mirrored into audit `decision_inputs_hash`. Production needs object-primary reads, bulk export controls, per-trace audit, retention filters, and revocation invalidation. |
...
| Central audit log | Partial | File-backed audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations plus reviewer trace-list reads and dataset/benchmark/ranker exports; privileged delayed credit now requires reasoned mutation metadata. Production still needs tamper-evident audit, per-trace content-read coverage, and reconciliation. |
