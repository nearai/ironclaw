...
Set `TRACE_COMMONS_REQUIRE_DB_MIRROR_WRITES=true` during production cutover after DB dual-write parity checks pass. It requires `TRACE_COMMONS_DB_DUAL_WRITE=true` and makes submission, revocation, review decision, credit, replay export manifest, benchmark/ranker provenance, audit, and trace-content-read mirror failures return an internal error instead of silently continuing with file-only state. Submission and delayed-credit mirror failures also avoid publishing local file-side metadata/object or ledger rows for the failed operation; export artifact/provenance mirror failures still need the remaining object-store staging hardening before they have the same guarantee.
