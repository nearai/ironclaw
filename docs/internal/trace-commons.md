...
the token's role. Revocation/self-delete, revocation propagation, config-status,
tenant-policy admin, and tenant-access-grant admin routes stay available for
deprovisioning and recovery. Admin tokens can manage the current tenant's grant
rows through `GET|POST /v1/admin/tenant-access-grants` and
