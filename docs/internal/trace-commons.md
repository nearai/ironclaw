...
| Vector duplicate/novelty index | Partial | DB schema and storage contract now persist vector-entry metadata, nearest trace ids, cluster id, duplicate score, novelty score, and invalidation state. The embedding worker/vector payload index is still future work. |
