...
| Production relational DB and encrypted object storage | Partial | V25/V26 PostgreSQL/libSQL schema, shared `TraceCorpusStore`, both backend implementations, optional ingest DB mirror, and encrypted local artifact sidecar exist. DB-first reads, backfill, RLS/policy enforcement, and service-owned object storage remain. |
