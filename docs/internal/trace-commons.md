...
permissioning gate on trace submission: the authenticated tenant/principal must
have an active, unrevoked, unexpired `trace_tenant_access_grants` row with the
same role as the request token. Grant consent-scope and allowed-use allow-lists
are additional constraints; they intersect with any static-token or EdDSA claim
allow-lists and cannot upgrade the token's role.
