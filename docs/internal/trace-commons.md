...
| Production relational DB and encrypted object storage | Partial | V25-V33 PostgreSQL schema plus libSQL schema slices, shared `TraceCorpusStore`, both backend implementations, optional ingest DB mirror with contributor, tenant policy, reviewer metadata, replay selection reads, policy-gated DB object-ref-backed replay envelope reads, vector-entry metadata, compact replay export manifest metadata, replay export item snapshots, encrypted local artifact sidecar, service-owned local encrypted object-store mode, maintenance-triggered DB mirror backfill for submissions plus existing file-side credit/audit/replay-manifest rows, derived presence/status/hash diagnostics, split export-manifest kind diagnostics, separate object-ref presence/readability/hash-mismatch diagnostics, vector index gap diagnostics, reader-projection parity diagnostics, and initial PostgreSQL tenant RLS policies exist. Remote object storage, parity enforcement, `FORCE RLS`/service role policy hardening, and broader object-primary reads remain. |
| Central audit log | Partial | File-backed audit rows now include optional hash-chain fields plus a maintenance verifier while preserving legacy JSONL compatibility, DB audit rows mirror those chain hashes for file-backed events, and maintenance emits a DB mirror continuity report that checks hash-field format and previous-hash linkage for mirrored rows. Audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations, retention/purge artifact invalidations, tenant policy admin reads/writes, reviewer analytics/list/review-queue/audit-log reads, dataset/benchmark/ranker exports, and per-trace replay export content reads, with `object_ref_id` mirrored when the body is read through a DB object ref; privileged delayed credit and tenant policy mutations now mirror typed safe metadata with bounded values and hashes rather than raw request bodies. Production still needs DB-native payload hash recomputation/tamper-evident append ordering, broader worker content-read coverage, and reconciliation. |
