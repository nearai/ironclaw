...
can run benchmark conversion, retention workers can run retention-scoped
maintenance, and vector workers can run the dedicated vector-index worker route
or vector-index maintenance without access to review queues, audit logs, tenant
policy administration, or delayed credit mutation.
...
| Tenant token roles | Partial | Static tenant tokens support contributor/reviewer/admin plus scoped export, retention, vector, and benchmark worker behavior; vector workers now have a dedicated `/v1/workers/vector-index` route as well as vector-scoped maintenance compatibility; optional tenant submission policies can restrict allowed consent scopes and trace-card uses at ingest, DB-backed tenant policy records can drive submission policy reads behind `TRACE_COMMONS_DB_TENANT_POLICY_READS`, admin tokens can manage the current tenant's DB-backed contribution policy via `/v1/admin/tenant-policy`, policy admin reads/writes are audited with safe hash-only policy metadata, and production-like deployments can require every submitting tenant to have a policy entry. Production needs short-lived credentials, fuller central policy, RBAC/ABAC, and row-level tenant enforcement hardening. |
