...
| Central audit log | Partial | File-backed audit rows now include optional hash-chain fields while preserving legacy JSONL compatibility; audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations, retention/purge artifact invalidations, reviewer trace-list and review-queue reads, dataset/benchmark/ranker exports, and per-trace replay export content reads, with `object_ref_id` mirrored when the body is read through a DB object ref; privileged delayed credit mutations now mirror typed safe metadata for event type, bounded delta, and hashed reason/reference fields. Production still needs DB-backed tamper evidence, broader worker content-read coverage, and reconciliation. |
