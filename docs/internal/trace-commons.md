...
| Central audit log | Partial | File-backed audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations, reviewer trace-list reads, dataset/benchmark/ranker exports, and per-trace replay export content reads, with `object_ref_id` mirrored when the body is read through a DB object ref; privileged delayed credit now requires reasoned mutation metadata. Production still needs tamper-evident audit, broader worker content-read coverage, and reconciliation. |
