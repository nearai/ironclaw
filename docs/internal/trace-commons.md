...
| Replay dataset export | Partial | Approved redacted slices can be exported by reviewer/admin tokens, DB metadata can drive replay export selection when envelope objects remain readable, manifests carry source-list hashes mirrored into audit `decision_inputs_hash`, replay exports mirror compact DB manifest rows with invalidation timestamps, reviewer/admins can list replay manifest metadata, and each exported trace body read emits a tenant-scoped audit event. Production needs object-primary reads, bulk export controls, item-level manifest rows, and revocation propagation for already-published artifacts. |
...
| Production relational DB and encrypted object storage | Partial | V25/V26/V27/V28/V29 PostgreSQL/libSQL schema, shared `TraceCorpusStore`, both backend implementations, optional ingest DB mirror with contributor, reviewer metadata, replay selection reads, vector-entry metadata, compact replay export manifest metadata, encrypted local artifact sidecar, and maintenance-triggered DB mirror backfill exist. Object-primary replay/export reads, full reconciliation, RLS/policy enforcement, and service-owned object storage remain. |
| Central audit log | Partial | File-backed audit routes and optional DB audit reads cover core submit/review/credit/revoke mutations, reviewer trace-list reads, dataset/benchmark/ranker exports, and per-trace replay export content reads; privileged delayed credit now requires reasoned mutation metadata. Production still needs tamper-evident audit, broader worker content-read coverage, and reconciliation. |
| Retention enforcement | Partial | Submit records persist retention policy ids and expiry timestamps; maintenance marks expired submissions and derived records, mirrors DB expiration/artifact/export-manifest invalidation when configured, prunes cached exports that reference expired sources, and can explicitly purge expired file/encrypted local artifact copies by cutoff. Production still needs legal hold policy and service-owned object storage deletion workflows. |
| Revocation propagation to derived artifacts | Partial | Current revocation marks local/file status, mirrors DB status, writes tenant-scoped tombstones, invalidates DB-mirrored object refs, derived precheck rows, vector metadata entries, and replay export manifest rows, and blocks file-backed replay export. Production must invalidate benchmark, ranking, worker, and already-published export artifacts. |
...
| Benchmark conversion pipeline | Partial | Reviewer/admin conversion can produce tenant-scoped benchmark candidate artifacts with consent/status/risk filters, source-list hashes, and audit events. Production still needs controlled worker jobs, benchmark registry publication, evaluator results, and revocation invalidation for published artifacts. |
