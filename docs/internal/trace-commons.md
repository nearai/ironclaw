...
| Replay dataset export | Partial | Approved redacted slices can be exported by reviewer/admin tokens, DB metadata can drive replay export selection when envelope objects remain readable, manifests carry source-list hashes mirrored into audit `decision_inputs_hash`, replay exports mirror compact DB manifest rows with invalidation timestamps, and each exported trace body read emits a tenant-scoped audit event. Production needs object-primary reads, bulk export controls, item-level manifest rows, and revocation propagation for already-published artifacts. |
