...
`TRACE_COMMONS_SIGNED_TOKEN_REVOKED_JTIS` to a comma-separated emergency
denylist of JWT `jti` values. The config-status route reports only whether
signed-token auth and issuer/audience checks are enabled plus the revoked-`jti`
count and max TTL; it never returns the secret or configured claim values.
