...
| Production relational DB and encrypted object storage | Partial | V25/V26 PostgreSQL/libSQL schema, shared `TraceCorpusStore`, both backend implementations, optional ingest DB mirror, encrypted local artifact sidecar, and maintenance-triggered DB mirror backfill exist. DB-first reads, full reconciliation, RLS/policy enforcement, and service-owned object storage remain. |
