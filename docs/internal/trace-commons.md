...
| Production relational DB and encrypted object storage | Partial | V25/V26/V27/V28/V29/V30/V31 PostgreSQL schema plus libSQL schema slices, shared `TraceCorpusStore`, both backend implementations, optional ingest DB mirror with contributor, reviewer metadata, replay selection reads, vector-entry metadata, compact replay export manifest metadata, replay export item snapshots, encrypted local artifact sidecar, maintenance-triggered DB mirror backfill, and initial PostgreSQL tenant RLS policies exist. Object-primary replay/export reads, full reconciliation, transaction-local tenant context through all PG store paths, `FORCE RLS`/service role policy hardening, and service-owned object storage remain. |
