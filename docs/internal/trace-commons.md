...
| Vector duplicate/novelty index | Partial | DB schema, storage contract, dedicated vector worker route, maintenance-triggered metadata indexer, object-ref/readability gating before non-dry-run vector writes, per-source vector-index content-read audits with source object refs, and reconciliation gap diagnostics now persist/vector-check vector-entry metadata, nearest trace ids, cluster id, duplicate score, novelty score, and invalidation state for accepted canonical summaries. When encrypted artifact storage is configured, vector indexing also writes a redacted canonical-summary vector payload as an encrypted worker-intermediate object ref for later embedding-worker promotion. Real embedding generation, vector payload search, and model-backed duplicate/novelty workers are still future work. |
