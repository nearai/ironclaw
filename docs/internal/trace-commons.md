...
| Retention enforcement | Partial | Submit records persist retention policy ids and expiry timestamps; maintenance marks expired submissions and derived records, mirrors DB expiration/artifact invalidation when configured, prunes cached exports that reference expired sources, and can explicitly purge expired file/encrypted local artifact copies by cutoff. Production still needs legal hold policy, durable export invalidation, and service-owned object storage deletion workflows. |
