...
| Replay dataset export | Partial | Approved redacted slices can be exported by reviewer/admin tokens, DB metadata can drive replay export selection when envelope objects remain readable, manifests carry source-list hashes mirrored into audit `decision_inputs_hash`, and each exported trace body read emits a tenant-scoped audit event. Production needs object-primary reads, bulk export controls, durable export invalidation, and revocation propagation for already-published artifacts. |
...
| Retention enforcement | Partial | Submit records persist retention policy ids and expiry timestamps; maintenance marks expired submissions and derived records, mirrors DB expiration/artifact invalidation when configured, prunes cached exports that reference expired sources, and excludes expired traces from exports. Production still needs purge jobs, legal hold policy, durable export invalidation, and object deletion workflows. |
