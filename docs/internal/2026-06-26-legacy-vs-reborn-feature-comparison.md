...
| MCP and hosted extensions | Available | Partial | Legacy has MCP clients and registries. Reborn composes host-mediated MCP runtime slices, hosted MCP activation, product-auth credential staging, bundled Notion/NEAR AI slices, and exact literal-IPv4-loopback HTTP dispatch for installed-local packages; registry-installed plaintext and stdio remain blocked, with live-recorded parity still follow-up. |
