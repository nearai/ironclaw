...
| Web gateway | Available | Partial | Legacy has the broader gateway/control-plane surface. Reborn WebChat v2 is included in the default standalone binary through `serve`, but still beta. |
...
| WebChat/UI | Available | Partial | Legacy has Web gateway chat and dashboard views. Reborn WebChat v2 is supported through `serve` in the default standalone binary; it is an early beta operator surface. |
...
| WebUI SSO | Missing | Partial/Reborn-only | Reborn `serve` includes Google/GitHub browser SSO in the default standalone binary, with verified-email-domain admission and tenant-bound HMAC sessions. |
