...
- Propagate revocation and retention expiration to DB metadata before DB-first reads. Completed for submission status, tombstones, object-ref invalidation, derived-record invalidation, vector-entry invalidation, contributor credit/status reads, reviewer metadata reads, retention-expired submission/object/derived invalidation, and audit events for invalidation counts.
