...
- Replay dataset exports persist durable access-grant and export-job lifecycle rows; already-started jobs are terminalized as `failed` if DB metadata reads or required object-ref body reads fail before an export manifest can be published.
