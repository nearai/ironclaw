...
3. Add encrypted object storage.
   ...
   - Keep file-backed reads compatible by falling back from DB object ref to existing path layout; require active object refs with `TRACE_COMMONS_DB_REVIEWER_REQUIRE_OBJECT_REFS=true` for production review decisions and `TRACE_COMMONS_DB_REPLAY_EXPORT_REQUIRE_OBJECT_REFS=true` for production replay export reads.
