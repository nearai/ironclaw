...
The `shift` operation adds signed integer seconds, minutes, hours, days, or weeks to an explicit timestamp or the current time; days and weeks are fixed durations rather than calendar arithmetic. Signed components may cancel; range checks apply to each component, the final offset, and the resulting timestamp rather than intermediate subtotals. Invalid time inputs return bounded, typed input issues through capability dispatch. Optional `operation` and `utc_offset` fields retain their defaults when absent or normalized from null; present values of the wrong type return `TypeMismatch` instead of silently using a default. A supplied timezone continues to take precedence over `utc_offset`.
