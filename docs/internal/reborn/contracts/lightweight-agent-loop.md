...
default. The sequential policy spelling remains part of checkpoint and
observability compatibility, but the canonical executor does not select it for
new model-emitted batches. A host port may still require ordered batch entry for
an operational hazard in the concrete invocation set. Ordered entry sets
