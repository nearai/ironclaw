...
| Messages fixed at submit | Window-eviction compaction durably writes cumulative `CumulativeBarrier` summaries **mid-run**; prompt projection stops at the newest eligible barrier while original rows remain stored, so future turns inherit the checkpoint without replaying covered history. | compaction task and `model_recovery.rs::cumulative_compaction_barrier_replaces_earlier_summaries_and_raw_history` |
