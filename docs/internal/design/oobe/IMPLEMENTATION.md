...
3. **Connect CTA** ⚠ *(deferred — needs browser QA + a decision)* — unconnected card → the **shared** extension setup/OAuth flow (`setupExtension` + the shared `openAuthPopup` / `subscribeProductAuthOAuthCompletion`, or the agent's `AuthRequired` frame via `AuthOauthCard`); on success → suggested. Held because a faithful *pure* connect (distinct from Approve, per §2A) is real OAuth-popup orchestration that should be verified in a browser and needs a quick call on the entry: **pre-connect** via `setupExtension` vs. **connect-on-approve** via the backend `AuthRequired` gate. Investigated reusing `useExtensions()` directly: it is a **page-level hook** (no params; fetches the entire extensions catalog + gateway status) with no isolated single-extension connect primitive — wiring the card to it wholesale pulls in unrelated state, and hand-rolling a parallel OAuth popup/polling/error-mapping flow risks real, unverifiable bugs. Recommend extracting a narrow `connectExtension(extensionId)` helper from `useExtensions.ts` as a precursor (owned by whoever knows that flow best), or building slice 3 with dedicated browser QA. (Change 5.)
