...
3. **Connect CTA** ⚠ *(deferred — needs browser QA + a decision)* — unconnected card → the **shared** extension setup/OAuth flow (`setupExtension` + the shared `openAuthPopup` / `subscribeProductAuthOAuthCompletion`, or the agent's `AuthRequired` frame via `AuthOauthCard`); on success → suggested. Held because a faithful *pure* connect (distinct from Approve, per §2A) is real OAuth-popup orchestration that should be verified in a browser (currently unavailable) and needs a quick call on the entry: **pre-connect** via `setupExtension` vs. **connect-on-approve** via the backend `AuthRequired` gate. (Change 5.)
...
5. ⏳ **Single-active + error polish** *(half-landed)* — ✅ every other card locks (disabled + dimmed) while one runs, resolving the queue-vs-block UX as **block** (matches the mockup). ⏳ The `failed`-frame error/incomplete status still depends on slice 2b's `useChatEvents` wiring — not yet landed, so a run can't yet actually surface as failed. (Change 3 + 6.)
