...
> **Plan of record is the artifacts + this plan; the Foundational v1 implementation is landing behind an off-by-default flag.** It carries the interactive [mockup.html](mockup.html) and the [integration-review.html](integration-review.html) page (the visual review) plus the written package below. An earlier UI prototype of these concepts (PR #6994) was **rolled back** in review — it rendered mock "done for you" automations to real users and an autonomy selector that execution ignored (IronLoop/CodeRabbit blockers). So wherever this package says *"the prototype (#6994)"* or *"mock→`fetch` swap,"* read it as: the earlier prototype and the mockup **demonstrate** the target UI; implementation has since **restarted behind the `oobe_suggestions` flag** (off by default): slice 1 (`SuggestedTaskCard` + a gated surface) is landed and gate-green, so real users are unaffected until it is enabled — see [IMPLEMENTATION.md](IMPLEMENTATION.md).
