...
- Signed upload-claim auth supports EdDSA/Ed25519 public-key verification through default or `kid`-selected keys and JSON/file keysets with optional activation windows and safe active/inactive key-count diagnostics, while HS256 claims and static tokens remain internal bridge paths. Managed issuer distribution/governance is still remaining work.
