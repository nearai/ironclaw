...
| Extension manifests and activation | rc1 manifests with top-level `[[capabilities]]`; rc1 `installed`/`disabled`/`enabled` state | validated `ironclaw.capability_provider/v1` manifest section and normalized installation activation state | Translate capability declarations only inside the rc1 importer, run the complete 1.1 validator, preserve state, and never widen installed/disabled to enabled | Retained rc1 snapshot plus canonical compatibility row |
