...
| `qa-telegram:C7`<br>Telegram preserves valid Unicode and escaped controls through the conversation path | **partial** | `cargo test -p ironclaw_telegram_extension` payload::slice_tests::multibyte_slice_respects_utf16_offsets (+ command-argument control-char rejection via shared validation). A full unicode round-trip through the composed stack is not separately pinned; the journey drives ASCII. |
...
| `qa-telegram:F7`<br>Telegram ignores callback, membership, inline, and channel-only updates | **covered** | `cargo test -p ironclaw_telegram_extension` payload noop tests + `cargo test -p ironclaw_telegram_extension` telegram_dispatch::non_actionable_updates_are_acked_silently |
| `qa-telegram:F8`<br>A revoked Telegram bot token fails honestly until an administrator saves a valid replacement | **covered** | `cargo test -p ironclaw_telegram_extension` render_outbound_records_unauthorized_on_telegram_401 (honest failure while revoked) + `cargo test -p ironclaw_telegram_extension` telegram_setup::{rotation_bumps_revision_and_keeps_installation_identity, save_happy_path…} (recovery by re-save) |
...
| `qa-telegram:V1`<br>A routine result is delivered once to the paired Telegram target | **covered** | `cargo test -p ironclaw_composition telegram_target_is_enumerated_resolved_and_delivered_through_generic_wiring` drives the real bundled manifest and Telegram adapter through generic DM-target enumeration, caller-scoped resolution, the triggered-delivery hook, and Bot API egress without a fabricated resolver ref. |
...
| Row | Status | Evidence / reason |
|---|---|---|
| `coverage:installable-catalog:exact-package-set`<br>The built-in catalog exposes exactly the twelve supported installable packages | **covered** | Adapter/unit contract rows — `cargo test -p ironclaw_telegram_extension --lib` per-module tests. |
| `qa-telegram:P14:01:pairing-qr-payload`<br>The Telegram pairing QR encodes only the current bot deep link | **covered** | Adapter/unit contract rows — `cargo test -p ironclaw_telegram_extension --lib` per-module tests. |
| `qa-telegram:S2:01:constant-time-header-verifier`<br>The Telegram shared-secret verifier uses constant-time equality | **covered** | Adapter/unit contract rows — `cargo test -p ironclaw_telegram_extension --lib` per-module tests. |

## Static / Architecture (4 rows)

| Row | Status | Evidence / reason |
|---|---|---|
| `qa-telegram:I5:01:telegram-registry-shape`<br>The Registry exposes one installable Telegram channel package with zero tools | **covered** | `cargo test -p ironclaw_architecture_tests --test telegram_extension_gates` (retired-taxonomy zero + no v1 pairing-route literals) and the layer/boundary suites. |
| `qa-telegram:I5:02:telegram-forbidden-identifiers`<br>Legacy Telegram identifiers and the removed member Channels route stay absent | **covered** | `cargo test -p ironclaw_architecture_tests --test telegram_extension_gates` (retired-taxonomy zero + no v1 pairing-route literals) and the layer/boundary suites. |
| `qa-telegram:S7:03:webhook-url-secret-substitution`<br>Telegram webhook registration uses a generated secret handle without exposing it to product layers | **covered** | `cargo test -p ironclaw_architecture_tests --test telegram_extension_gates` (retired-taxonomy zero + no v1 pairing-route literals) and the layer/boundary suites. |
| `qa-telegram:V3:01:no-telegram-tool-registry`<br>Telegram contributes zero callable tools to every capability registry | **covered** | `cargo test -p ironclaw_architecture_tests --test telegram_extension_gates` (retired-taxonomy zero + no v1 pairing-route literals) and the layer/boundary suites. |
