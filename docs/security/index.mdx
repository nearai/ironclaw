...
flowchart TB
    subgraph Input["Input Layer"]
        Channels["Channels\nTUI, Web, Telegram, Webhook"]
    end

    subgraph Layer1["Layer 1: Safety"]
        Sanitizer["Sanitizer"]
        Validator["Validator"]
        Policy["Policy Engine"]
    end

    subgraph Layer2["Layer 2: WASM Sandbox"]
        Wasm["wasmtime\nMemory limits\nFuel metering"]
    end

    subgraph Layer3["Layer 3: Docker Sandbox"]
        Docker["Container isolation\nNetwork proxy\nCredential injection"]
    end

    subgraph Layer4["Layer 4: Secrets"]
        Crypto["AES-256-GCM\nOS keychain\nZero exposure"]
    end
