...
`google.*` carries the OAuth client id, redirect URI, and hosted-domain hint.

Slack and Telegram are **not** configured here. Install the extension and complete its
setup from the web interface; the `[slack]` setup keys are retired and make `ironclaw
