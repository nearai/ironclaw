...
<Warning>
This page documents the retained v1 runtime and its database-backed settings.
Use `ironclaw-v1` for the commands below. For the canonical `ironclaw` Reborn
configuration surface, see the [CLI reference](/reborn-binary#config).
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Some security-sensitive and advanced options remain environment-only. Manage database-backed values through the wizard or `ironclaw-v1 config` command.
  </Accordion>
</AccordionGroup>
...
```bash
# View current config
ironclaw-v1 config list

# Get specific value
ironclaw-v1 config get llm.backend

# Set value
ironclaw-v1 config set llm.backend nearai

# Delete value (reset to default)
ironclaw-v1 config delete llm.backend
```
