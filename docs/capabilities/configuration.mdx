...
`config.toml` selects _by id_ from the catalog `providers.json` defines. The compiled-in
...
Secrets are never stored in `config.toml`. Reference them by environment variable _name_:
...
<Warning>
  Pasting a secret value where a variable name is expected is rejected at parse
  time, not silently accepted. `api_key_env = "sk-ant-..."` will fail to load.
</Warning>
...
| Profile                                         | Use                                                                                                                                                                                                 |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hosted-single-tenant-volume-sandboxed`         | Default. One tenant with per-user sandboxed processes on a local Docker daemon. Needs a sandbox image built from `Dockerfile.sandbox-worker`; the agent loop runs as a Pi worker inside the sandbox |
| `hosted-single-tenant-volume-sandboxed-railway` | Preview. Same as above, with per-user sandbox lifecycle provided by Railway Sandboxes                                                                                                               |
| `local-dev`                                     | Single user on your own machine, with the loop in-process                                                                                                                                           |
...
<Warning>
  Profile choice is a security boundary, not a preset. A deployment that serves
  more than one user must not run a profile with single-user host semantics. See
  [Security](/security).
</Warning>
