...
The canonical namespaces are directly below that home:
...
├── logs/        # operational logs; retained per operator logging policy
├── cache/       # disposable operational cache
└── tmp/         # disposable temporary files
```

`logs/`, `cache/`, and `tmp/` are operational namespaces, not authoritative
application state.
...
| Variable | Purpose |
| --- | --- |
| `IRONCLAW_REBORN_HOME` | Installation boundary for configuration and the direct `state/`, `system/`, `workspaces/`, `runtime/`, `logs/`, `cache/`, and `tmp/` namespaces |
