...
<Steps>
  <Step title="Gate">
    Skills declare prerequisites in `requires.bins`, `requires.env`, and `requires.config`. These fields are parsed but are not enforced during selection. All installed skills proceed to scoring regardless of their declared requirements.
  </Step>
