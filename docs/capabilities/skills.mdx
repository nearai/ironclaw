...
<Steps>
  ...
  <Step title="Attenuate">
    Trust-based content visibility is applied. Installed skills (from IronHub) expose only their description — the full prompt body is not injected and the agent cannot self-activate them. Trusted skills have full content loaded. See Trust Levels below.
  </Step>
</Steps>
...
| Trust Level   | Source                                                      | Content visibility                          | Model self-activation |
|---------------|-------------------------------------------------------------|---------------------------------------------|-----------------------|
| **Trusted**   | User-placed in `~/.ironclaw/skills/` or workspace `skills/` | Full prompt body available to the model     | Yes                   |
| **Installed** | Downloaded from IronHub registry via `skill_install`        | Description only — full body not injected   | No                    |
...
Skills in trusted directories are loaded as-is. Skills in `installed_skills/` expose only their description — the full prompt body is not injected and the agent cannot self-activate them.
