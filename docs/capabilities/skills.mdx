...
<Steps>
  ...
  <Step title="Attenuate">
    Trust-based tool ceiling is applied. Installed skills (from IronHub) lose access to dangerous tools regardless of what the skill requests. Trusted skills retain full tool access. See Trust Levels below.
  </Step>
</Steps>
...
| **Installed** | Downloaded from IronHub registry via `skill_install`        | Read-only tools only — no shell, no file write, no HTTP |
