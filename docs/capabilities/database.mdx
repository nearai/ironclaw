...
<Warning>
This page documents retained v1 database settings. Use `ironclaw-v1` for the
commands below. Canonical Reborn local-dev storage is isolated under
`~/.ironclaw/reborn`; hosted and production storage is documented in the
[CLI reference](/reborn-binary#run).
</Warning>
...
<Warning>
If you start IronClaw without configuring a database, you'll see: `DATABASE_URL is required`. Run `ironclaw-v1 onboard` or set the environment variables manually.
</Warning>
...
<Warning>
`DATABASE_URL` is **required** when using PostgreSQL. If not set, IronClaw will fail to start with the error: `DATABASE_URL is required — Run 'ironclaw-v1 onboard' or set DATABASE_URL environment variable`.
</Warning>
