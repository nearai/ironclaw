...
Under the default `hosted-single-tenant-volume-sandboxed` profile (and the
explicit `local-dev` and `hosted-single-tenant-volume` profiles), IronClaw
keeps state in embedded database files inside your IronClaw home:
...
<Warning>
  `hosted-single-tenant-volume` does **not** accept `[storage]`, despite the
  name. It is a volume-backed variant of local embedded storage, not a
  PostgreSQL profile. Pairing it with a `[storage]` block leaves the instance
  unable to start.
</Warning>
...
<Warning>
  The URL is environment-only. `config.toml` may name the variable that holds
  it, but must never contain the URL itself — it carries the database password.
</Warning>
...
<Warning>
  Losing the master key makes every stored credential unrecoverable. Back it up
  somewhere other than the machine running IronClaw. See [Security](/security).
</Warning>
...
| Profile  | `hosted-single-tenant-volume-sandboxed` (default), `local-dev`, `hosted-single-tenant-volume` | `hosted-single-tenant`, `production` |
