...
- Workspace-write sandboxed one-shot commands on Kubernetes still fail closed because uploaded workspaces do not sync writes back to the host.
- Sandboxed one-shot commands on Kubernetes still fail closed by default when they depend on allowlist-only networking.

`ironclaw doctor` also reports whether the remaining cluster-side Stage 3 prerequisites are configured:

- `IRONCLAW_K8S_NATIVE_NETWORK_CONTROLS=true`
- `IRONCLAW_K8S_PROJECTED_RUNTIME_CONFIG=true`

When both are set, doctor reports that read-only one-shot sandbox commands can use uploaded workspaces, runtime config can use projected files, and the cluster-side prerequisites are ready. Workspace-write one-shot commands still need Docker until workspace write-back exists, so Kubernetes remains on the Stage 2 contract.
