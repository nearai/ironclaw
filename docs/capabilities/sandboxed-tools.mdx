...
## Container Runtime Stages

IronClaw currently treats container backends as staged capabilities rather than as one flat "sandbox parity" promise.

- Docker is the reference full sandbox runtime.
- Kubernetes is currently a Stage 2 project-backed runtime.
- Stage 2 Kubernetes jobs receive project content and per-job config through orchestrator-served bootstrap artifacts.
- Read-only sandboxed one-shot command execution on Kubernetes now uses uploaded workspace archives instead of host mounts.
- When `IRONCLAW_K8S_NATIVE_NETWORK_CONTROLS=true` is declared ready, sandboxed one-shot commands that depend on allowlist-only networking can run on Kubernetes.
- When `IRONCLAW_K8S_PROJECTED_RUNTIME_CONFIG=true` is declared ready, runtime config files can use Kubernetes-projected file delivery instead of local mounts.
- Kubernetes API credentials are host-side platform credentials, not per-job secrets. IronClaw resolves them in this order: in-cluster ServiceAccount, encrypted platform kubeconfig secret (`sandbox_kubernetes_kubeconfig`), then local/default kubeconfig as a compatibility fallback.
- Workspace-write sandboxed one-shot commands on Kubernetes return pending workspace changes for explicit review and apply.
- Sandboxed one-shot commands on Kubernetes still fail closed by default when they depend on allowlist-only networking unless native network controls are ready.

`ironclaw doctor` also reports whether the remaining cluster-side signals are configured:

- `IRONCLAW_K8S_NATIVE_NETWORK_CONTROLS=true`
- `IRONCLAW_K8S_PROJECTED_RUNTIME_CONFIG=true`

When native network controls are ready, doctor reports that allowlist-only sandboxed one-shot commands can run on Kubernetes. When projected runtime config is ready, doctor reports that runtime config can use projected files. Workspace-write one-shot commands still return pending changes for explicit review and apply, so Kubernetes remains on the Stage 2 contract.
