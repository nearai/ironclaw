...
`ironclaw` is the canonical standalone executable for IronClaw. The legacy v1
implementation remains in the workspace during migration, but it is not part of
the IronClaw release package.
...
`ironclaw` is the shipping IronClaw CLI. Some commands remain early
...
- production extension/tool execution: `extension` and `skills` need a
  ...
  local-dev IronClaw services`. In a default source build (no `postgres`
...
`serve` starts the WebChat v2 HTTP listener so you can drive IronClaw from a
browser. This is the fastest way to exercise the full loop (ingress → turn
runner → LLM provider → timeline) end to end.

**Shortcut:** `scripts/run-ironclaw-webui.sh` wraps the steps below — it keeps the
IronClaw home outside the repo, configures the route, generates the WebUI token,
and launches `serve`. Export your provider key first, then run it:

```bash
export NEARAI_API_KEY=...                 # or OPENAI_API_KEY / ANTHROPIC_API_KEY
scripts/run-ironclaw-webui.sh               # NEAR AI default
PROVIDER=openai scripts/run-ironclaw-webui.sh
```

It prints the login token and the `http://127.0.0.1:3000/` URL. Override
`PROVIDER`, `MODEL`, `IRONCLAW_SERVE_HOST`, `IRONCLAW_SERVE_PORT`, or
`IRONCLAW_HOME` via the environment. The legacy `REBORN_HOST` and
`REBORN_PORT` names remain fallbacks. The manual steps below are equivalent.

### Quick start

```bash
# 1. For serve/run/repl the IronClaw home must live OUTSIDE your current working
...
export IRONCLAW_HOME="$HOME/.ironclaw-demo"
...
| `workspace root must not overlap default skill root /skills` | IronClaw home is **inside** the current working directory | Point `IRONCLAW_HOME` at a path outside your repo/cwd. |
...
rejected if that root overlaps any default storage root IronClaw manages —
`/skills` (`<ironclaw-home>/local-dev/skills`), `/tenant-shared/skills`,
`/system/skills`, or `/system/extensions`. If the home is nested inside the cwd
(e.g. `IRONCLAW_HOME="$PWD/.ironclaw-home"`), those roots fall under the
workspace root and boot is rejected. Keep the home outside the directory you
launch from — the default `~/.ironclaw` already satisfies this.

(Resolved per-user skills live under
`<ironclaw-home>/local-dev/tenants/default/users/<owner>/skills`; the flat
...
The IronClaw channel registry is not wired yet. The command stays visible in
`--help`/completions for a stable future name, but invoking `channels list`
returns an error and a non-zero exit instead of resolving IronClaw home, reading v1 channel
...
Searches and manages local-dev IronClaw extensions through the same lifecycle facade exposed to product surfaces. Available extension packages are read from `/system/extensions`, which maps to `<ironclaw-home>/local-dev/system/extensions` for the local-dev profile.
...
The commands are scoped to IronClaw boot/config resolution and do not create or read v1 state directories.
...
Generates shell completion scripts without resolving IronClaw home, reading v1 state, or creating directories.
...
Shows the resolved IronClaw state root, its source, selected profile, and explicit v1-state status without creating directories.
...
`config path`, `doctor`, and other read-only surfaces do not create IronClaw
state or seed config files.

### `doctor`

Validates and reports IronClaw boot configuration without creating state directories or starting runtime services.
...
Same treatment as `channels list` above: the IronClaw hook registry is not
...
Same treatment: the IronClaw log source is not wired yet, so `logs` stays
...
First-run bootstrap for the standalone IronClaw home. It resolves
`IRONCLAW_HOME` (or the default `~/.ironclaw`), creates the home
...
yet). See `docs/ironclaw/onboarding.md` for the full slice description and the
completion-marker schema.

### `models list` / `models status` / `models set-provider`

Shows IronClaw model purpose slots and route status, and configures the default
LLM route.
...
(`ironclaw_llm`) is a mandatory dependency of the IronClaw CLI, so there is no
build of the binary without `IronClawProviderAdmin` linked in.

### `profile list`

Lists the supported IronClaw boot profiles without resolving IronClaw home, reading v1 state, or creating directories.
...
Starts the standalone IronClaw runtime and reads messages from stdin. The no-profile path targets the planned AgentLoop runtime (`reborn-planned-default`). Without model provider environment variables, the runtime still starts but messages fail cleanly because no LLM gateway is wired.
...
Starts an interactive IronClaw session backed by the composed runtime, reading
...
Reports configured IronClaw skills from `<ironclaw-home>/<profile-subdir>/skills`
and `<ironclaw-home>/<profile-subdir>/system/skills` through the IronClaw
...
IronClaw must not use the current v1 IronClaw state root by default.
...
[`docs/ironclaw/default-contracts.md`](ironclaw/default-contracts.md) for
compatibility and rollback details.

## Profiles

Use `IRONCLAW_PROFILE` to select the boot profile.
`IRONCLAW_REBORN_PROFILE` remains a lower-precedence compatibility alias.
...
Run these before changing IronClaw CLI behavior:
...
cargo test -p ironclaw_architecture ironclaw
...
IRONCLAW_HOME="$(mktemp -d)/ironclaw-home" \
  cargo run -q -p ironclaw --bin ironclaw -- config path
...
3. resolve and pass `IronClawCliContext` from dispatch only when the command needs boot config;
4. keep pure commands independent from IronClaw home resolution;
...
Do not port the current `src/cli/*` command tree wholesale. Port commands one at a time, starting with IronClaw-owned or read-only surfaces.

## Release packaging

Pushing a matching `ironclaw-v*` tag starts the cargo-dist release workflow for
the IronClaw `ironclaw` package. cargo-dist builds the binary for two GNU Linux,
...
This tag path is IronClaw-only. It does not compile or publish the legacy v1
...
only the finalization step for the IronClaw GitHub Release.

`.github/workflows/ironclaw-release-compile.yml` remains available as an
