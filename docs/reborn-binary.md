...
The WebChat v2 web UI **is** supported through `serve` in the default Reborn
CLI binary. It is still an early beta operator surface, and runtime config/env
must explicitly provide WebUI auth before the listener starts. See
...
always a mistake. Slack host-beta ingress code is present in the default binary,
but routes mount only when `[slack].enabled = true` or
`IRONCLAW_REBORN_SLACK_ENABLED=true` is configured.
