...
`ironclaw` is the canonical executable boundary for the Reborn runtime. The v1
runtime and its separate executable have been retired.

This binary is available as the workspace package `ironclaw_reborn_cli` and builds the executable named `ironclaw`.

## Current status

`ironclaw` is the default IronClaw runtime and operator surface.
