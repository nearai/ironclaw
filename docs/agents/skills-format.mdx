...
| `metadata.openclaw.requires.bins` | string[] | No | Binaries that must exist on `PATH`. Skill is gated out if any are missing. |
| `metadata.openclaw.requires.env` | string[] | No | Environment variables that must be set. Skill is gated out if any are missing. |
