...
2. **Present.** One shared panel contract for both surfaces (in-chat blocked card and Extensions card — RC-10 parity): pairing code, deep link `https://t.me/<bot_username>?start=<CODE>` (8-char code is valid `start` payload), expiry countdown, live connection status (poll or SSE — pinned in planning to whatever the existing channel-connection panel does). In chat, the run parks on the existing **`BlockedAuth` gate** with a requirement carrying `provider = telegram`, `requester_extension = telegram`; the display preview renders the pairing card via the `channel_connection_required` output kind with render chrome stripped from model-visible output. No new blocking machinery.
