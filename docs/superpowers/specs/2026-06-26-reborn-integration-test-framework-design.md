...
**But the raw construction API is the verbosity we are escaping.** A current hand-built trace costs dozens of lines for "a tool call then a text reply" (see the existing tool-call traces in `e2e_tool_param_coercion.rs`): nested `TraceStep { request_hint: None, response: TraceResponse::ToolCalls { tool_calls: vec![TraceToolCall { id, name, arguments }], input_tokens, output_tokens }, expected_tool_results: Vec::new() }` per step. That is exactly what new tests must not look like. The `RebornScriptedReply` façade exists to collapse each of those steps to one line, auto-filling `id`, token counts, `request_hint: None`, and `expected_tool_results: []`. The façade is therefore **mandatory**, not polish — it is the mechanism that delivers the readability contract in §4.2.
...
| Secrets / OAuth | `SecretStore` / `RuntimeHttpEgress` (token exchange) | `StaticSecretStore` (fixture handles); refresh worker not spawned | `.with_secret(handle, value)` to seed; for a full OAuth connect-flow (create flow → callback → persist account) use `build_oauth_product_auth_for_test()` + `ScriptedOAuthTokenEgress` from `ironclaw_composition::test_support` — **Built, slice 7** |
...
| Trace Commons (telemetry) | `ContributionHttpSink` (`ironclaw_trace_commons`) | **already captured** — the agent path (`HostEgressContributionSink`) routes through `RuntimeHttpEgress`, so `RecordingRuntimeHttpEgress` records it; no new sink needed | assert by filtering `recorded_egress.requests()` by the contribution URL |
...
This is not an extra mechanism — each subsystem's durable impl already persists through the *same* `RootFilesystem` that §3.2 controls (via `ScopedFilesystem<F>` or `RootFilesystem::write_file`; e.g. auth records under `/secrets/product-auth/`, installs as rows under `/system/extensions/.installations/`, skills under `/user-skills/`). The harness's `StorageMode` selects the backing filesystem: under `LibSql` every subsystem rides the one `LibSqlRootFilesystem` (real CAS, JSON serialization, and migrations for all of them at once); under `InMemory` they use the same filesystem stores over an in-memory backend. Either way, persisting them for real is **free** — no per-store fake to write.
...
2. **Raw `LlmTrace::new` / `TraceTurn` / `TraceStep` / `TraceResponse` / `TraceToolCall` construction is forbidden in new Reborn integration tests.** That nested-struct form (dozens of lines for a short scripted exchange in existing v1-stack tests such as `e2e_tool_param_coercion.rs`) is exactly what `RebornScriptedReply` replaces. Existing v1 tests keep theirs; this work does not migrate them.
...
- **Process-port injection seam — no production change (slice 5 correction).** The original Step 5b plan called for a `#[cfg(any(test, feature = "test-support"))]` accessor in `ironclaw_composition::factory` mirroring the Step 4 mount-helper. This was based on a wrong assumption: the integration harness's hot path is `harness.rs`'s `local_dev_host_runtime_with_http_egress`, which builds `HostRuntimeServices` directly — not `build_local_runtime`. `HostRuntimeServices::with_runtime_process_port_dyn` is already a public builder method and is sufficient. No production file was modified in slice 5.
