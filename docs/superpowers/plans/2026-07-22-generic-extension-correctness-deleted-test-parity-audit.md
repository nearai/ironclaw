...
| `crates/ironclaw_webui/src/product_auth/oauth.rs` | 27 | `ironclaw_auth` engine + product-auth API and `ironclaw_webui` route adapters | re-expressed; callback/blocked-gate whole paths remain mandatory |
...
| `crates/ironclaw_auth/src/product_auth/oauth/oauth_gate.rs` | 16 | `ironclaw_auth` concrete engine/client contracts | re-expressed by owner crate |
