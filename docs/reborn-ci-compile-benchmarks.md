...
The current benchmark evidence does **not** justify continuing the OVH/sccache
and shared-builder line as the main CI-speed strategy.

What is worth keeping from this PR:
...
- More OVH/sccache tuning. Removing OVH regressed, but retaining it has not
  produced a stable 30-50% wall-clock win. The Redis cache appears to be a
  partial mitigation for cache misses/latency, not a decisive solution.
- Hakari as generated by `cargo-hakari manage-deps`; it violated explicit
  dependency-boundary tests and did not show compelling timing.
...
The remaining plausible speed work is not another cache wiring tweak or
same-run Cargo batching trick. It is dependency-graph and feature-fanout
reduction inside the slow Rust jobs. A separate H16 dependency-image benchmark
would first need repo/package permission changes and an intentionally hardened
image lifecycle.
...
## H19: Batch Root-Test Cargo Invocations

Advice under test:

- Stop compiling/linking the same root-package graph once per root test binary
  inside each partition.

Change under test:

- `scripts/ci/run-reborn-root-partition.sh` now builds one `cargo test` command
  per partition:
  - before: loop over selected test names and run
    `cargo test --test <name> -- --nocapture` once per test file;
  - after: run `cargo test --test <a> --test <b> ... -- --nocapture` once.
- The script also avoids `mapfile` so the local smoke path works on older
  macOS Bash.

Why this was plausible:

- Logs from the latest retained-shape validation showed later root test files
  still spent about `14s` per `cargo test --test ...` invocation compiling or
  relinking `ironclaw_webui_v2`, `ironclaw_reborn_composition`, and the root
  `ironclaw` crate, while the test binaries themselves often ran in seconds or
  less.
- The same logs showed high sccache hit counts but large aggregate cache-read
  time, so avoiding repeated invocations inside one already-warm job was a
  narrower lever than adding more cache infrastructure.

Local validation:

- `bash -n scripts/ci/run-reborn-root-partition.sh` passed.
- Empty-partition smoke passed:
  `REBORN_ROOT_TEST_PARTITIONS=999 REBORN_ROOT_TEST_PARTITION=998
  scripts/ci/run-reborn-root-partition.sh`.
- `cargo test --test reborn_minimal_dispatch_parity --test
  reborn_response_order_parity --no-run` accepted repeated `--test` selectors
  and began compiling before the local timeout.

Benchmark result:

- Latest PR-attached retained-shape baseline:
  [`28734793077`](https://github.com/nearai/ironclaw/actions/runs/28734793077)
  at commit `d05de5d39`, success, wall clock `11m10s`.
- First H19 PR-attached run:
  [`28736236743`](https://github.com/nearai/ironclaw/actions/runs/28736236743)
  at commit `bd82c12bd`, success, wall clock `12m00s`.
- H19 dispatch confirmation:
  [`28736632268`](https://github.com/nearai/ironclaw/actions/runs/28736632268)
  at commit `bd82c12bd`, success, wall clock `8m32s`.

| Metric | Latest PR baseline | H19 PR run | H19 dispatch confirmation |
| --- | ---: | ---: | ---: |
| Workflow wall clock | `670s` | `720s` | `512s` |
| Root `0` | `569s` | `457s` | `442s` |
| Root `1` | `643s` | `525s` | `459s` |
| Root `2` | `630s` | `540s` | `464s` |
| Root `3` | `570s` | `549s` | `499s` |
| Root max | `643s` | `549s` | `499s` |
| Root average | `603s` | `518s` | `466s` |
| `adapters-misc` | `568s` | `673s` | `134s` |

Decision:

- Retain H19. It reduced the root-test long pole in both valid samples without
  reducing test coverage or adding jobs.
- Do not overstate it as a whole-workflow fix. The PR-attached run's overall
  wall clock regressed because `adapters-misc` spiked from `568s` to `673s`;
  the dispatch confirmation then showed the same bucket at `134s`, so bucket
  cache variance remains larger than the root-script improvement in some runs.

## H20: Batch Crate-Bucket Cargo Invocations

Advice under test:

- Apply the same "one Cargo invocation per job" idea to Reborn crate buckets:
  run one `cargo test -p <a> -p <b> ...` per bucket instead of looping over
  packages.

Change under test:

- Added `scripts/ci/run-reborn-crate-bucket.sh`.
- Converted each package's existing feature flags into package-scoped Cargo
  features such as `ironclaw_product_workflow/libsql`.
- Replaced the YAML package loop with a call to the new script.
- Added the new script to `scripts/ci/classify-test-scope.sh` so future edits
  would trigger Reborn tests.

Local validation:

- `bash -n` passed for the new script and related CI scripts.
- `classify-test-scope.sh` classified the new script as `has_reborn_tests=true`.
- A local Cargo probe accepted package-scoped feature syntax and began
  compiling:
  `cargo test -p ironclaw_product_adapters -p ironclaw_product_workflow
  --features
  ironclaw_product_adapters/test-support,ironclaw_product_adapters/host-auth-mint,ironclaw_product_workflow/test-support,ironclaw_product_workflow/libsql
  --all-targets --no-run`.

Benchmark result:

- H20 PR run:
  [`28736740319`](https://github.com/nearai/ironclaw/actions/runs/28736740319)
  at commit `44bd83e85`, cancelled after the negative signal was clear.
- Workflow elapsed before cancellation: `12m08s`.
- `adapters-misc` was still running when cancelled, after `677s`.

| Bucket | Latest PR baseline | H20 run | Delta |
| --- | ---: | ---: | ---: |
| `adapters-misc` | `568s` | cancelled at `677s` | `+109s` and incomplete |
| `host-runtime` | `445s` | `525s` | `+80s` |
| `composition-core` | `433s` | `479s` | `+46s` |
| `webui-ingress` | `391s` | `455s` | `+64s` |
| `product-workflow` | `383s` | `394s` | `+11s` |
| `llm-mcp` | `310s` | `357s` | `+47s` |
| `agent-runtime` | `289s` | `246s` | `-43s` |
| `memory-skills` | `173s` | `249s` | `+76s` |

Decision:

- Reject and revert H20.
- The likely reason is Cargo feature unification. Batching packages into one
  invocation saves per-package Cargo process overhead, but it also forces the
