...
<Warning>
**Important:** IronClaw's orchestrator binds to `0.0.0.0:50051` on Linux for container communication. This port should **not** be exposed externally.

```bash
# Block orchestrator port from external access
sudo ufw deny 50051
```
</Warning>
...
# Block orchestrator port (internal only)
sudo ufw deny 50051
...
server {
  ...
  ssl_certificate /etc/letsencrypt/live/docs.ironclaw.ai/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/docs.ironclaw.ai/privkey.pem;
