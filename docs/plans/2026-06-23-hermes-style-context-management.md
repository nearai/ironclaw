...
- Changes contained to: agent-loop prompt assembly (`crates/ironclaw_agent_loop/src/executor/prompt.rs`, `model.rs`), the model gateway (`crates/ironclaw_turn_runner/src/model_gateway.rs`), a new tool catalog/index, a new prompt-view compressor, and a new artifact store.
