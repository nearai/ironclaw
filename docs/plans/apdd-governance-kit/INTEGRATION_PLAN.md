...
| 1.5 | Consolidate the **embedded-AI-agent invariants** into a single referenced "What NOT to do" list (keys/secrets at rest, LLM-data-never-deleted, capability-dispatch scoping, credential/extension identity) — cross-linking existing CLAUDE.md/safety rules, not duplicating them. | `CLAUDE.md` / `.claude/rules/` |
...
| Step | Action | Target |
|---|---|---|
| 2.1 | `npx storybook init` in the frontend; add to `package.json` scripts + devDeps (it's already resolvable in `node_modules`). | `crates/ironclaw_webui/frontend` |
...
| 2.6 | Add a **Storybook/a11y CI job** to the frontend test lane (axe = warn → error when clean). Reuse the existing Vitest browser lane; no new expensive runner. | `.github/workflows/reborn-*` (frontend lane) |

**Exit:** `npm run storybook` works; primitives have stories that run under
Vitest; a11y checks report; the agent can introspect real props via MCP.
...
Phase 0 ──► Phase 1 (design constitution)  ← highest value, ship first
                │
                ├─► Phase 2 (Storybook)     ← depends on §1 tokens/taxonomy
                │
                └─► Phase 3 (docs-first)    ← independent of 2; can parallelize
                                │
                                └─► Phase 4 (CUJs + visual regression)
```
