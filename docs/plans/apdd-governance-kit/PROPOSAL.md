...
| **Impact / dependency spec sections + a cross-functional-review *trigger*** | Adopt the spec's **§5 Architecture Impact** and the plan's **§4 Dependencies & Sequencing** as-is (they make blast radius + ordering explicit before code). For review routing, **do not** import the kit's generic §8 "team review" gate — IronClaw already has stronger raw material. Instead build the piece neither the kit nor IronClaw has today: an **explicit cross-functional-review trigger** — (a) a feature-spec field *"Cross-functional review required: security / data / design / none"*, and (b) a path-scoped rule that, when a diff crosses a **trust / persistence / capability** boundary, names the required partner **role** (security / data / design) and pulls the review *checks* from the existing [`review-discipline.md`](../../../.claude/rules/review-discipline.md) + the PR *Reborn Trust-Boundary Checklist* — note those describe *what* to inspect at a boundary, **not who**: neither names people or teams. This turns "circulate for review" into "this diff touches X → a Y-role reviewer must sign off." Mapping a role to an actual person/team is a separate concern (e.g. `CODEOWNERS`), out of scope for these prose rules. |
