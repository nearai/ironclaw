...
| `crates/ironclaw_agent_loop/src/strategies/compaction.rs` | New trait `CompactionPolicy { fn should_force_compact(&self, state: &LoopExecutionState) -> Option<CompactionReason>; }`. v1 impl `ByteCapPolicy { caps: HashMap<CapabilityId, u64> }`. Defaults: `SPAWN_SUBAGENT_CAPABILITY_ID → 48_000`, `builtin.http → 32_000`, `builtin.web_fetch → 32_000`, default fallback 32_000. Sig accepts `&LoopExecutionState` only (no `BudgetState` type exists; v2 `BudgetFractionPolicy` extends sig if #4311 needs more). |
...
2. Land `CompactionPolicy` trait + `ByteCapPolicy` impl with unit tests (pure function).
...
| 4 | `pending_capability_bytes: HashMap<CapabilityId, u64>` | LOCKED IN | agent_loop already depends on `ironclaw_host_api`; `CapabilityId` already used in `state.rs:169`. Counter map = allowed per CLAUDE.md. Future per-cap policies (`BudgetFractionPolicy`) consume it. |
...
- **`ironclaw_agent_loop` deps unchanged.** No imports from `ironclaw_reborn`, host_runtime, dispatcher, network, secrets, DB backends. `CompactionPolicy` trait + `ByteCapPolicy` impl stay crate-private. New file `executor/post_capability.rs` follows "one decision axis per file" rule.
...
- **Logging discipline for background paths** (Reviewer 4 G3). `PostCapabilityStage`, `SubagentRestartReconciler::replay`, `completion_observer.rs` background branch must use `debug!` not `info!` per `/Users/henry/Code/ironclaw/CLAUDE.md`.
