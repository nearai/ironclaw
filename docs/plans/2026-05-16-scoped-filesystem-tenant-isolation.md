...
| `ironclaw_memory` | `LibSqlMemoryDocumentRepository`, `PostgresMemoryDocumentRepository` | **Deleted** — no production callers (composition already used `FilesystemMemoryDocumentRepository`). `db_memory_repository_contract.rs` and the four `e2e_*` / `memory_filesystem_vertical_integration.rs` tests that exercised the legacy repos are deleted; substrate semantics (versioning, chunk replace, metadata cascade, hybrid search fusion) keep coverage via `FilesystemMemoryDocumentRepository` over `InMemoryBackend` in `memory_backend_contract.rs` + `memory_filesystem_contract.rs` + the inline `repo/filesystem.rs` `#[cfg(test)]` suite, and backend-specific durability moves to `ironclaw_filesystem`'s own backend contract tests. The `libsql` / `postgres` features and the `deadpool-postgres` / `libsql` / `pgvector` / `tokio-postgres` deps drop off `ironclaw_memory/Cargo.toml`. |
