...
| `ironclaw_threads` | **Done** — `FilesystemSessionThreadService` takes `Arc<ScopedFilesystem<F>>`; per-thread/message/summary/idempotency records under `/threads` alias; cross-tenant isolation regression test | follow-up (`reborn/fs-rework-threads`) |
