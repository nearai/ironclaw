...
| `qa-telegram:B1:04:first-paired-inbound`<br>The first paired Telegram DM enters the canonical runner and receives one reply | **covered** | `cargo test --test ironclaw_integration_telegram_journey` whole-journey seam 4 (paired DM → canonical runner → one rendered reply) |
...
| `qa-telegram:C1`<br>A paired Telegram DM routes to its member-owned thread | **covered** | `cargo test --test ironclaw_integration_telegram_journey` whole-journey seam 4 + telegram_two_users_stay_isolated_across_pairing_reply_and_unpair (thread ownership per bound user) |
...
| `qa-telegram:D1:01:telegram-thread-isolation`<br>Telegram messages create threads owned by their bound users | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_two_users_stay_isolated_across_pairing_reply_and_unpair |
| `qa-telegram:D1:02:telegram-reply-isolation`<br>Telegram replies return only to the originating bound user | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_two_users_stay_isolated_across_pairing_reply_and_unpair (per-chat replies + no-cross-bleed matrix) |
| `qa-telegram:D2`<br>An unknown Telegram user receives only the static pairing hint | **covered** | `cargo test -p ironclaw_telegram_extension` telegram_dispatch::{unpaired_hints_are_throttled_per_chat} + `cargo test --test ironclaw_integration_telegram_journey` multi-user scenario R8 leg (post-unpair DM → hint, no turn) |
| `qa-telegram:D4`<br>Unpairing User A leaves User B’s Telegram binding unchanged | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_two_users_stay_isolated_across_pairing_reply_and_unpair (A unpairs; B's binding, thread, and delivery untouched) |
| `qa-telegram:F1`<br>A retried Telegram update produces exactly one turn and one reply | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_duplicate_updates_and_send_failures_stay_honest (redelivered update_id → exactly one turn/reply) + `cargo test -p ironclaw_telegram_v2_adapter` idempotency contract |
...
| `qa-telegram:F3:01:telegram-send-403`<br>A Telegram 403 marks the delivery failed and does not retry storm | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_duplicate_updates_and_send_failures_stay_honest (403 → exactly one Bot API attempt, no retry storm) + `cargo test -p ironclaw_telegram_v2_adapter` unauthorized mapping |
| `qa-telegram:F3:02:telegram-send-recovery`<br>The next Telegram send succeeds after a prior blocked-recipient failure clears | **covered** | `cargo test --test ironclaw_integration_telegram_journey` telegram_duplicate_updates_and_send_failures_stay_honest (next turn's reply delivers after the failure clears) |
...
| `qa-telegram:P1:01:pairing-code-consume`<br>A valid Telegram deep-link code binds only the existing IronClaw member | **covered** | `cargo test -p ironclaw_telegram_extension` telegram_pairing::consume_happy_path_binds_targets_and_dispatches + `cargo test --test ironclaw_integration_telegram_journey` whole-journey seam 3 |
...
| `qa-telegram:P9`<br>One Telegram binding resumes all matching blocked threads and no unrelated gate | **covered** | composition suite (`--lib`) BlockedAuthResumeFanout suites (provider-keyed, resumes every matching run, unrelated gates untouched) + `cargo test --test ironclaw_integration_telegram_journey` resume assert |
| `qa-telegram:R1`<br>DRAFT — qa-telegram:R1 | **draft** | DRAFT parent — carried by R1:01…R1:06 |
| `qa-telegram:R1:01:telegram-configure`<br>The clean lifecycle walk establishes Telegram through Admin > Channels | **covered** | `cargo test --test ironclaw_integration_telegram_journey` whole-journey seam 1 |
| `qa-telegram:R1:02:telegram-pair`<br>A freshly installed Telegram channel accepts one member pairing | **covered** | `cargo test --test ironclaw_integration_telegram_journey` whole-journey seam 3 |
...
| `qa-telegram:R2`<br>Removing Telegram unpairs only the requesting member | **covered** | composition suite (`--lib`) extension_removal_cleanup::telegram_removal_cleanup_disconnects_the_removing_user + `cargo test --test ironclaw_integration_telegram_journey` multi-user scenario (self-service disconnect leg) |
...
| `qa-telegram:R8`<br>A Telegram DM after member removal returns a connection hint without starting a turn | **covered** | `cargo test --test ironclaw_integration_telegram_journey` multi-user scenario (post-unpair DM → static hint, no turn) |
...
| `qa-telegram:V1`<br>A routine result is delivered once to the paired Telegram target | **partial** | Parts pinned: hook registration + same-config idempotency (composition suite (`--lib`) telegram_host_beta::build_telegram_host_runtime_mounts_wires_outbound_provider_and_trigger_hook), the generic triggered-run driver outcome suites (composition suite (`--lib`) channel_delivery driver tests), the telegram protocol observer case, and DM-target listing (`cargo test --test ironclaw_integration_telegram_journey`). A single composed trigger-fire→DM walk is not driven; candidate for the triggered_delivery_outcome bin with a telegram leg. |
...
| Row | Status | Evidence / reason |
|---|---|---|
| `qa-install:A-4a:01:telegram-page-install-entry`<br>Telegram page install exposes member pairing without tenant bot secrets | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-remove-reconfigure:RC-10:03:browser-channel-configure`<br>Only an administrator can edit Telegram deployment configuration in Admin > Channels | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-remove-reconfigure:RC-2:01:telegram-admin-token-save`<br>The Admin > Channels form masks Telegram secrets and omits blank existing values from save | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-remove-reconfigure:RM-H8:01:telegram-user-removal`<br>A member confirms Telegram removal without seeing or changing admin bot configuration | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:B1:03:telegram-setup-browser-state`<br>Telegram admin save updates browser status and serves a pre-paired DM without restart | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:B7:02:telegram-setup-ui-visibility`<br>Only operators see Telegram channel configuration in the Admin UI | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I1`<br>The Extensions page installs Telegram and completes member pairing | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I2:03:telegram-pairing-card-render`<br>A blocked Telegram run renders pairing chrome outside model-visible content | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I3`<br>Chat and Extensions pairing panels share one durable Telegram contract | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I4:01:unconfigured-page-install`<br>Telegram page activation fails closed before tenant bot setup | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I5:03:telegram-catalog-fields`<br>Telegram Registry fields expose member install and connect state without host configuration | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I5:04:telegram-catalog-network-calls`<br>Telegram Registry install and connect use only current lifecycle endpoints | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:I7`<br>Telegram pairing completion updates open page and chat projections live | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P10:02:pairing-gate-thread-switch`<br>Switching WebChat threads preserves the Telegram pairing gate in its owner thread | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P10:03:pairing-gate-page-reload`<br>Reloading WebChat rehydrates the same durable Telegram pairing gate | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P11`<br>Dismissing and reopening a Telegram pairing card does not mutate its gate | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P14:02:pairing-qr-renewal`<br>Renewal replaces the Telegram QR and makes a stale QR fail closed | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P14:03:cross-device-live-status`<br>Phone-completed Telegram pairing updates the desktop panel in real time | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P1:04:pairing-live-status-browser`<br>Telegram pairing updates the open member panel and blocked chat without a refresh | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P2:02:pairing-code-copy-affordance`<br>The Telegram pairing panel exposes a keyboard-copyable code and searchable bot username | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P3:03:pairing-countdown-render`<br>The Telegram pairing countdown reaches expiry without showing a usable stale code | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P3:04:cross-surface-code-renewal`<br>Renewing a Telegram pairing code replaces it on every open member surface | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:P8:02:page-pairing-chat-update`<br>Pairing from the Extensions page updates the same member's blocked chat | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-telegram:S7:04:browser-secret-redaction`<br>Telegram member and admin pages never render stored secret values | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-webui:EX4`<br>Activate renders the correct outcome for ready, auth-gated, and channel packages | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-webui:LV7`<br>A cross-tab channel connection wakes and resumes the exact waiting thread once | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
| `qa-webui:SP3`<br>Connecting Telegram on the Extensions page clears a same-tab chat gate | **not-automated** | Browser-visible flow — belongs to tests/e2e Playwright (ironclaw_v2_* fixtures); not automatable in the hermetic Rust tier. |
