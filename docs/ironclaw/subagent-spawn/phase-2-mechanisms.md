...
impl SubagentPromptComposer {
    /// Called by the IronClaw host factory when assembling the child run's
...
> own file `subagent/driver.rs`, matching the ironclaw-crate rule "Add a new file
...
```
   ...
   ┌───────────┼────────────┐
 P1.A        P1.B          P1.C
 (turns)   (agent_loop)  (IronClaw data)
   │           │            │
   ├───────────┼────────────┤
   ▼           ▼            ▼
