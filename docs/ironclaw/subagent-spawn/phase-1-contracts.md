...
| New `LoopFamilyId` value | `"subagent"` (wire string) | P1.B | IronClaw driver binding (P2.C) |
| New lineage fields on `TurnRunRecord` | `parent_run_id: Option<TurnRunId>`, `subagent_depth: u32`, `spawn_tree_root_run_id: Option<TurnRunId>` | P1.A | IronClaw submit path (P2.A), observer (P2.D), reservation table (P1.C/P2) |
...
- **Neutral read-model sink** — define a IronClaw-neutral projection target in
...
- **Projection consumer** — lives in `crates/ironclaw_event_projections/`
  (the IronClaw read-model boundary). Consumes `TurnLifecycleEvent`. Per event:
...
|---|---|---|
| `LoopGateKind` | **add it** | observability/wire enum; mission/cron/trigger families will add more gate kinds; external (IronClaw) test code constructs it but never exhaustively matches it |
...
both PostgreSQL and libSQL through the IronClaw turn-state persistence layer. There
