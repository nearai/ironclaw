...
`crates/ironclaw_composition` for concrete product-live assembly,
...
inside the IronClaw composition layer, declares the `spawn_subagent` capability
...
> `crates/ironclaw_composition` that constructs the durable DB-backed
...
| `crates/ironclaw_runner/src/production_readiness.rs` | Add `IronClawLoopProductionComponent::SubagentGoalStore`, `::SubagentCompletionObserver`, `::SubagentResultTombstoneStore`, `::SubagentRestartReconciler`, `::SubagentAutonomousContinuationBudget`; extend `IronClawLoopComponentGraphReadiness` with the five fields; add `subagent_driver_requirements()`. |
| `crates/ironclaw_composition/src/lib.rs` | `pub mod` the new `subagent_runtime` and `subagent_restart_reconciler` modules (see §1.2). |
| `crates/ironclaw_composition/CLAUDE.md` | Add entry points for concrete subagent assembly and restart reconciliation. This matches the crate guardrail that top-level production/app startup composition lives here. |

### 1.2 Files created

| File | Purpose |
|---|---|
| `crates/ironclaw_composition/src/subagent_runtime.rs` | The **concrete subagent assembly seam**. Owns `SubagentRuntimeParts` and `build_subagent_runtime`, which builds the durable DB-backed goal store, the durable tombstone store, the autonomous-continuation budget, accepts the root-provided pending-gate projection sink, constructs the spawn-capable capability port, wires the `SubagentCompletionObserver`, spawns the `RestartReconciler` periodic task, and returns a fully wired `IronClawRuntimeLoopComposition` with a composite `TurnEventSink`. Keeps `ironclaw_runner/src/runtime.rs` free of concrete-store construction and keeps root `src/` pending-gate types out of IronClaw crates. |
| `crates/ironclaw_composition/src/subagent_restart_reconciler.rs` | The `RestartReconciler` itself — `run_once()` (startup sweep) and `spawn_periodic(interval)` returning a `JoinHandle<()>`. Pure replay logic; idempotent via the same `external_event_id` the live observer uses. Wired by `subagent_runtime.rs`. |
| `crates/ironclaw_composition/tests/subagent_spawn_e2e.rs` | All end-to-end integration tests (§5). |
| `crates/ironclaw_composition/tests/subagent_runtime_wiring.rs` | Composition-level tests: family/driver registration, profile resolution, composite event-sink registration, reconciler periodic-task spawn, budget injection, tombstone-store injection, DB-backed goal store, production readiness for the subagent family. |

> The `subagent_runtime.rs` split exists because `crates/ironclaw_composition`
...
///
/// IronClaw composition root for loop families. v1 binds two Builtin families:
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    /// Optional event sink for IronClaw lifecycle projections/observers.
...
the observer live in `crates/ironclaw_composition`; `runtime.rs` only
...
`crates/ironclaw_composition`. Do not add root-store-specific fields to
...
```rust
// crates/ironclaw_composition/src/subagent_runtime.rs
...
```rust
// crates/ironclaw_composition/src/subagent_runtime.rs  (new file)
...
use ironclaw_runner::{
    runtime::{
        DefaultPlannedRuntimeParts, IronClawRuntimeLoopComposition, build_default_planned_runtime,
...
pub struct SubagentRuntimeParts {
    ...
    /// read model. IronClaw composition accepts this neutral sink; it must not
...
/// Build a fully wired IronClaw runtime composition with subagent spawn enabled.
///
...
) -> Result<IronClawRuntimeLoopComposition<T, S, G>, DefaultPlannedRuntimeBuildError>
where
    T: TurnStateStore + TurnRunTransitionPort + Send + Sync + 'static,
    S: SessionThreadService + ?Sized + Send + Sync + 'static,
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
...
struct SubagentTestHarness {
    composition: IronClawRuntimeLoopComposition<MemTurnStore, MemThreadService, ScriptedGateway>,
...
| `subagent_runtime_passes_production_readiness` | `validate_ironclaw_loop_production_readiness` with the subagent profile selected, `IronClawLoopComponentGraphReadiness::production_verified()` extended with `production_verified` subagent components → `ProductionReady`, `issues.is_empty()`. |
...
pub struct IronClawLoopComponentGraphReadiness {
    ...
    pub subagent_result_tombstone_store: IronClawComponentReadiness,
    pub subagent_autonomous_continuation_budget: IronClawComponentReadiness,
    pub subagent_completion_observer: IronClawComponentReadiness,
    pub subagent_restart_reconciler: IronClawComponentReadiness,
}
```

- `production_verified()` sets all five new fields to
  `IronClawComponentReadiness::production_verified(Required)`.
...
2. The subagent **run profile** must appear in the configured-profiles list fed
   to `validate_ironclaw_loop_production_readiness`. Its
...
cargo test -p ironclaw_architecture --test ironclaw_dependency_boundaries
scripts/ironclaw-e2e-rust.sh architecture
...
- **Architecture guardrails** — `cargo test -p ironclaw_architecture --test
  ironclaw_dependency_boundaries` and `scripts/ironclaw-e2e-rust.sh architecture`
...
3. **[P2]** Confirm `DefaultTurnCoordinator::with_event_sink` exists (or get it
   added to P1.A — it is a coordination contract, not IronClaw composition).
...
9. `production_readiness.rs`: add the two `IronClawLoopProductionComponent`
   variants, the two `IronClawLoopComponentGraphReadiness` fields, the
...
11. `crates/ironclaw_composition/src/subagent_runtime.rs` (new):
    `SubagentRuntimeParts`, `SubagentSpawnCaps`, `build_subagent_runtime`,
    `build_product_live_subagent_runtime` (§3.7).
12. `crates/ironclaw_composition/src/lib.rs`: `pub mod
