# IronClaw Architecture Simplification: Fewer DTOs, Less `dyn`, No Local-Specific Structs
...
This note proposes a **fundamental** simplification of the IronClaw host/runtime
...
seams as if they were trust boundaries.* IronClaw has a few real trust boundaries —
...
- Open issues already target this surface: **#6168** ("Shrink the
  `ironclaw_composition` god-crate 24% → ~10%") and **#6144** item 1
...
IronClaw has a few real **trust boundaries** — places where untrusted code or data meets
...
IronClaw's kernel boundary must obey the same rule: **adding a feature, or a
...
`ironclaw_composition`** — a whole shadow local-dev *runtime*
...
- `LocalHostProcessPort` → `HostProcessPort`: it names the trust boundary (a process
  ...
  (`docs/ironclaw/2026-05-11-trust-boundary-stack-note.md`); `Local` obscures it,
  `Host` states it.
...
fix is not a config flag but IronClaw's own rule (*everything goes through
...
(`IronClawServicesApi`, `ironclaw_product_workflow/src/ironclaw_services.rs`) has
...
```rust
/// Target end-state: the only surface a product terminal/channel uses. Turn
/// lifecycle + two conduits; frozen. The terminal normalizes input into this
/// facade and renders projection/output from it; it does not implement product
/// semantics behind it.
///
/// TARGET ACTOR BINDING: ingress constructs one `ProductSurface` per
/// authenticated session, sealed to that actor. The current proto-surface still
/// accepts a caller on `invoke`/`query`; the target removes caller ingress from
/// these conduits or backs it with conformance tests. In that end-state the
/// facade itself is the capability. Conversation/run/gate IDs are designators
/// only, NEVER bearer authority; streaming, cancellation, and gate resolution
/// authorize against the bound actor, not against knowledge of an ID.
trait ProductSurface {
    ...
    //    Same resolve → authorize → dispatch pipeline (§3); target state binds
    //    origin and actor from the facade, not a per-call caller parameter. ──
...
validation, JSON serialization into `IronClawViewPage`, capability-id parsing,
...
1. **Freeze the facade now.** Check in the current `IronClawServicesApi` method
...
where must the result return?" IronClaw needs the same invariant in IronClaw terms:
...
identity binding — and consumes only `ProductSurface`.** `ironclaw_composition`
...
| --- | --- | --- |
| **WebUI** | `ironclaw_webui_v2` (routes, ~7.2K) + `ironclaw_webui_ingress` (listen/auth/serve, ~6.2K) + `ironclaw_webui_v2_static` (SPA) + `composition/src/webui/` (~4.6K) + `web_access.rs` / `host_ingress.rs` | one `WebUiProductAdapter` over `ProductSurface` (SPA stays an asset bundle) |
...
| **OpenAI-compat API** | `ironclaw_openai_compat` (~7.5K) + ~441 refs in composition | one OpenAI-compat terminal over product workflow / `ProductSurface` |
| **CLI** | canonical `ironclaw` package over the `IronClawRuntime` facade | CLI adapter over `ProductSurface` (`IronClawRuntime` is today's proto-`ProductSurface`) |
...
`ironclaw_composition`.** A product is added by writing an adapter, registering it in
...
| --- | --- | --- | --- | --- |
| 1 | `ProductSurface` (§5.2) | every user-initiated action: 6 turn-lifecycle methods + `invoke` (all mutations) + `query` (all reads) | target: **actor-bound at mint** — one facade per authenticated session; IDs are designators, never bearer authority. Current proto-surface still accepts caller input on `invoke`/`query` until the final sealing slice lands. | product boundary (trusted↔trusted) |
...
When every §5.8/§4.4 move lands, `ironclaw_composition` is a small crate
...
- The runtime-policy resolver is sound (it guarantees `HostedMultiTenant` →
  ...
  (`crates/ironclaw_composition/src/local_runtime_profile.rs`).
...
| Product facade | frozen 87-method `IronClawServicesApi` trait | turn lifecycle + `invoke`/`query` (8 methods); mutations are descriptors with an origin→gate matrix (§5.2.1) |
...
`ironclaw_composition` to wire shared substrates from a config value. Mostly
...
| Products in composition (§5.8) | no `slack` / `webui` / `telegram` / `openai` / HTTP-route / transport identifier in `ironclaw_composition` | `ironclaw_architecture` | freeze the current product footprint; fail on new product/transport code landing in composition | composition is assembler-only; products are adapters + a `DeploymentConfig` list |
| Product facade (§5.2) | `IronClawServicesApi` method-set allowlist; every capability descriptor declares an origin→gate matrix (§5.2.1) | `ironclaw_architecture` | freeze the current 87-method set, **generated from the trait block itself** (set membership, not a file-wide grep); fail on any new method; fail on any descriptor without a gate matrix; the `Ungated`-for-`LoopRun` allowlist is pinned to its reviewed seed (§14) and may only change by reviewed diff; the meta-capability set is pinned with its `AskAlways` floor (§5.2.6) | facade = turn lifecycle + `invoke`/`query`; all mutations are matrix-declared descriptors |
...
(`.claude/rules/review-discipline.md`, `ironclaw-skill-maintainer`):
...
channel extension; no generic source file changes), and `ironclaw_retired_taxonomy.rs` pinning
...
witness pattern; `docs/ironclaw/2026-05-11-trust-boundary-stack-note.md`); no caller can
...
- **Synthetic product tools** (`project_create`, `skill_activate`, `result_read`,
  ...
  origin→gate-matrix shape then absorbs the `IronClawServicesApi` mutations one by one.
...
| --- | --- | --- |
| Result DTO collapse | **Done** — `CapabilityOutcome` and result mirrors are deleted; `host_api::Resolution` is canonical. | `crates/ironclaw_host_api/src/resolution.rs` (`Resolution`, `Outcome`, `Blocked`, `Suspension`); `crates/ironclaw_architecture/tests/ironclaw_capability_dto_collapse_ratchet.rs` bans retired mirror names. |
| Request-side DTO collapse | **Done** — retired request mirror DTO names are banned; `LoopRequest`, tuple parts, and private `RuntimeLaneRequest` carry the remaining distinct states. | `crates/ironclaw_architecture/tests/ironclaw_capability_dto_collapse_ratchet.rs::RETIRED_COLLAPSE_DTOS`; `crates/ironclaw_turns/src/run_profile/host.rs` (`LoopRequest`); `crates/ironclaw_dispatcher/src/lib.rs` (`RuntimeLaneRequest`). |
| Request/witness vocabulary | **Done** — `Invocation`, `Actor`, `InvocationOrigin`, `Authorized`, and dispatch-through-witness are live. | `crates/ironclaw_host_api/src/invocation.rs`; `crates/ironclaw_host_api/src/authorized.rs`; `crates/ironclaw_host_api/src/dispatch.rs`; `crates/ironclaw_architecture/tests/ironclaw_authorized_seal_ratchet.rs`. |
...
| Origin→gate matrix | **Live, tightening remains** — matrices are descriptor data and ratcheted; `LoopRun` ungated set is a reviewed 17-id seed to shrink. | `crates/ironclaw_host_api/src/capability.rs` (`OriginGatePolicy`, `OriginGateMatrix`, `UNGATED_LOOP_RUN_CAPABILITIES`); `crates/ironclaw_architecture/tests/ironclaw_origin_gate_matrix_ratchet.rs`. |
...
| Generic extension/channel runtime | **Landed** — `ChannelAdapter` + `ironclaw_extension_host`; Slack/Telegram are extension adapters, not bespoke product trees. | `crates/ironclaw_product_adapters/src/channel_adapter.rs::ChannelAdapter`; `crates/ironclaw_extension_host/src/lib.rs::ExtensionHost`; `crates/ironclaw_slack_extension/src/channel.rs::SlackChannelAdapter`; `crates/ironclaw_telegram_extension/src/channel.rs::TelegramChannelAdapter`; `crates/ironclaw_architecture/tests/ironclaw_extension_specificity.rs`. |
| Product facade collapse | **Started, mostly remaining** — `invoke`/`query` exist and the facade is frozen at 87 methods; migrations to descriptors/views remain. | `crates/ironclaw_product_workflow/src/ironclaw_services.rs` (`IronClawServicesApi::invoke`, `IronClawServicesApi::query`, `ProductSurface`, `ProductView`, `ProductCapabilityDescriptor`); `crates/ironclaw_architecture/tests/ironclaw_facade_method_freeze_ratchet.rs`. |
| Deployment mode as data | **Partial** — renames/ratchets are live, but profile enums and profile-edge branching still exist. | `crates/ironclaw_architecture/tests/ironclaw_localdev_typename_ratchet.rs`; remaining mode vocabulary in `crates/ironclaw_host_api/src/runtime_policy.rs`. |
...
- **Origin→gate matrix is live (#6396 / #6432).** `OriginGatePolicy` /
  ...
  descriptors carry matrices, and `ironclaw_origin_gate_matrix_ratchet.rs` pins the
...
- **Slice A — delete hand-written `InMemory*Store`s (§4.3) is complete for the
  ...
  `ironclaw_inmemory_store_ratchet.rs` file was deleted in #6430 because its
...
- **Slice B — `Local*` → config (§4.4), ratchets and many renames done; config
  ...
  further, but `IronClawCompositionProfile::{LocalDev,LocalYolo,...}` and some
...
- **Unified generic extension runtime landed (#6116).** The old
  ...
  `ironclaw_extension_host` plus `ChannelAdapter` in
  `ironclaw_product_adapters`; Slack and Telegram now live as
...
- **§10 ratchets live, updated for current shape.** The active architecture
  ...
  `IronClawServicesApi` frozen method set is currently 87 methods, including the
  canonical `invoke` and `query` conduits; a new product operation must be a
...
- **§5.2 `ProductSurface`** facade collapse — the `IronClawServicesApi` freeze has
  ...
  `IronClawServicesApi` methods were removed from the ratchet allowlist. This is
...
- `crates/Architecture.md` — IronClaw kernel-boundary / substrate architecture thesis.
- `docs/ironclaw/2026-05-11-trust-boundary-stack-note.md` — trust-boundary baseline invariants.
- `docs/ironclaw/2026-04-25-storage-catalog-and-placement.md` — storage placement.
...
- `crates/ironclaw_product_workflow/src/ironclaw_services.rs` — `IronClawServicesApi`: the frozen 87-method proto-`ProductSurface` trait that §5.2's `invoke`/`query` conduits replace (methods → matrix-declared capability descriptors + view descriptors).
...
- `crates/ironclaw_composition/src/local_dev_capability_policy.rs` — `LocalDevConstraintSource` and the ~66-identifier `LocalDev*` shadow runtime to collapse to config (§4.4).
- `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs` — boundary tests that must stay green; home for the "no `Local*` type names" and "freeze `host_api`" checks.
...
- Issue #6170 (shell cross-tenant escape — the §6 case study); `crates/ironclaw_composition/src/local_runtime_profile.rs` (the `HostedSingleTenant → LocalSingleUser` mapping), `crates/ironclaw_host_runtime/src/planner.rs` (process/filesystem fail-closed rules).
