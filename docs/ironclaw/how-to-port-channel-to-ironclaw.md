# How to port a v1 channel to IronClaw

This guide helps maintainers decide how an IronClaw v1 channel should move onto the IronClaw product-surface path.

The important distinction is that v1 used one `Channel` abstraction for several different things. IronClaw should keep those categories separate.

## Quick decision tree

Ask first: **is this channel an IronClaw-owned user surface, or an external protocol integration?**

| v1 source | Examples | IronClaw target |
| --- | --- | --- |
| Host-owned native surface | TUI, Web Gateway, local REPL, built-in local control endpoints | Native IronClaw surface that enters the product workflow directly |
...
## Why IronClaw keeps two boundary types

IronClaw should converge on **one product workflow entrypoint**, not one adapter type for every surface.
...
(TUI/Web/REPL)                     (Telegram/Slack/Signal/etc.)
      │                                      │
      ▼                                      ▼
Native IronClaw surface              WASM ProductAdapter component
      │                                      │
      └──────────── ProductWorkflow ─────────┘
                         │
                         ▼
                  IronClaw turn/runtime path
```
...
host request / UI event
  ...
  → native IronClaw surface handler
...
- Session/user/thread binding stays in native IronClaw host code.
...
- [ ] Identify how the surface maps to a IronClaw product workflow submission.
...
external protocol payload
  → IronClaw host verifies protocol auth
...
| v1 channel concept | IronClaw ProductAdapter concept |
| --- | --- |
...
| v1 channel activation | IronClaw adapter installation/runtime config |
...
- IronClaw WASM ProductAdapter host runtime that loads components and exposes egress/secrets/delivery/logging capabilities beyond the current native runner.
- Component-level ProductAdapter contract test harness.
- Production IronClaw route/deployment documentation for selecting v1 or IronClaw for a given external webhook.
