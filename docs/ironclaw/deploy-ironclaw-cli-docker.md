# IronClaw CLI Docker Deployment
...
```bash
docker build -f Dockerfile -t ironclaw:local .
```
...
IronClaw listener itself is bound to loopback, such as a non-Docker local run or a
host-network run.
...
keep `IRONCLAW_SERVE_HOST=0.0.0.0`. The IronClaw WebUI service serves
...
will use `$RAILWAY_VOLUME_MOUNT_PATH/ironclaw` by default when Railway
exposes a volume mount. Existing volumes that only contain an
`ironclaw-reborn` directory are adopted in place for compatibility. Without a
volume, Railway deployments using
...
`/data/ironclaw`; an existing `/data/ironclaw-reborn` volume is adopted when
the canonical directory does not exist. Without a Railway volume, that path is ephemeral. The
hosted single-tenant volume profile stores runtime/control-plane state under
that IronClaw home on the mounted volume and does not require
`IRONCLAW_POSTGRES_URL`. The container workdir is `/workspace` so the
workspace root stays separate from IronClaw's state and skill roots.
...
For public WebUI Google login, use the IronClaw WebUI SSO variables and an HTTPS
