# IronClaw Contract — Trigger System
...
It does **not** own a parallel agent loop, product adapter lifecycles, or outbound delivery targets. A trigger fire is routed into the normal IronClaw turn pipeline and then persists through the same turn, run, and recovery machinery as any other inbound submission.
...
- The fire must enter the normal IronClaw inbound/turn pipeline.
...
`trigger_pause`, and `trigger_resume` as first-party IronClaw capabilities.
...
selection. Local-dev IronClaw exposes model-visible outbound target
...
V1 acceptance does not require external delivery. A valid V1 trigger fire is one that submits a cron-backed synthetic inbound turn and persists through the normal IronClaw turn path.
...
- `active_hold` appears on both `IronClawAutomationProductFacade::list_automations`
...
- Reason vocabulary (`IronClawAutomationHoldReason` / `ActiveHoldReason`):
