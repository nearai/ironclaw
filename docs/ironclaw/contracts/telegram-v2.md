# Telegram Channel (IronClaw host)
...
> `docs/ironclaw/extension-runtime/overview.md`. This document is retained as the
...
adapter in `crates/ironclaw_composition/src/telegram/telegram_host_beta.rs`.
...
| Connect strategy | `IronClawChannelConnectStrategy::WebGeneratedCode` |
...
the env name unchanged (config compat). Their meaning is now: **the IronClaw
...
adapter proven against recorded payloads and fake IronClaw services, with no
...
- **Host wiring exists.** Admin setup, manifest-projected public webhook
  ...
  `crates/ironclaw_composition/src/telegram/telegram_host_beta.rs`;
...
Live proof rides the IronClaw WebUI v2 live-QA lane
(`scripts/ironclaw_webui_v2_live_qa/`), whose Telegram cases drive admin
