# IronClaw OpenAI-Compatible API Contract
...
The OpenAI-compatible API is a IronClaw product/API ingress surface for clients
...
composition-supplied projection-stream port; IronClaw keepalive/control frames and
...
- Client-supplied OpenAI tools are model-only compatibility data in this
  migration. They are not IronClaw capabilities and must not execute through the
  capability host.
...
- Durable ref mappings are persisted behind `OpenAiCompatRefStore`; the
  ...
  IronClaw local-dev host composition places the production route's tenant-owned
...
- Non-streaming Chat Completions wait timeout detaches from the wait, not from
  ...
  unavailable error.
- Timed-out create requests remain bounded by the shared IronClaw turn-admission
...
- SSE translation consumes a composition-supplied projection stream port and
  emits OpenAI-compatible events from `ProductProjectionItem` state. IronClaw
...
`openai_compat_router_with_state(...)` inside the IronClaw protected route stack
...
- Resolves the canonical projection read request through
  ...
  composition-supplied projection reader. The local-dev IronClaw composition
...
- Cancels only when the opaque ref is authorized and bound to a IronClaw run ref,
...
The SSE translator must suppress IronClaw keepalive/control frames, raw
...
both streaming and non-streaming creates until IronClaw has a capability-view
...
code must not execute them as IronClaw capabilities.
