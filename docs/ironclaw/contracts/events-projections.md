# IronClaw events and projections contract

**Date:** 2026-04-25
**Status:** Draft contract
**Depends on:** `docs/ironclaw/contracts/run-state.md`, `docs/ironclaw/contracts/host-api.md`, `docs/ironclaw/contracts/communication-delivery-resolution.md`
...
IronClaw runtime events and audit records have two owning crates:

- `ironclaw_events` owns the redacted record vocabulary, cursor types, sink traits, and durable log traits.
- `ironclaw_event_store` owns standalone IronClaw backend selection and storage adapters for those traits.
...
- PostgreSQL and libSQL adapters are available behind the crate's `postgres` and `libsql` features. Their schema files live in `crates/ironclaw_event_store/migrations/`, and the factory runs those migrations before returning the service graph.
