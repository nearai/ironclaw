# IronClaw Contract — Memory and Workspace Services
...
`ironclaw_memory` owns durable memory/workspace document semantics for IronClaw.
...
The canonical IronClaw memory path should include agent scope:
...
IronClaw memory should expose focused services over the shared backend, not one monolithic production `Workspace` clone.
