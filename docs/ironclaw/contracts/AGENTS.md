# Agent Map — `docs/ironclaw/contracts/`

## Purpose

This directory is the **source of truth for IronClaw boundary contracts** consumed by `ironclaw_host_api`, `ironclaw_capabilities`, and (after the Manifest v2 cutover) `ironclaw_extensions`.
...
- `openai-compatible-api.md` — IronClaw-native OpenAI-compatible Chat Completions / Responses ingress contract.
...
- Full manifests, full JSON schemas, and full profile contracts are **cold**: stored under `docs/ironclaw/contracts/` and reachable from the registry, never serialized into model context.
...
- Provider-specific (third-party) profile claims — those ship inside the extension's own manifest, not under `docs/ironclaw/contracts/`.
