# IronClaw WASM runtime contract

The IronClaw WASM runtime executes sandboxed extension components through the canonical component-model ABI declared in `wit/tool.wit`.
...
The abandoned JSON pointer/length ABI (`alloc`, `invoke_json`, `output_ptr`, `output_len`, and runtime-specific HTTP imports such as `http_request_utf8`) is not part of IronClaw.
...
- Keep V1 `src/tools/wasm/*` and `src/channels/wasm/*` as compatibility references only; IronClaw is a separate binary path.
