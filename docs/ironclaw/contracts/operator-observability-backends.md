# IronClaw Operator Observability Backend Contracts
...
`GET /api/webchat/v2/operator/status` should report a bounded, redacted snapshot of the current IronClaw host.
...
compatibility wrapper over that same query path while `IronClawServicesApi`
shrinks toward §5.2.

Required payload shape, nested under the `operator_status` field of the root
`IronClawOperatorCommandPlaneResponse`:

- `state`: one of the stable `IronClawOperatorStatusState` values.
...
- IronClaw composition readiness state.
...
`GET /api/webchat/v2/operator/diagnostics` is the canonical IronClaw doctor
...
a compatibility wrapper over that same query path while `IronClawServicesApi`
shrinks toward §5.2.
