# IronClaw Contract - First-Party Skills Extension
...
IronClaw skills are portable `SKILL.md` bundles supplied by a first-party
in-process extension. The extension owns skill storage, catalog, lifecycle, and
source enumeration. IronClaw core owns the authority boundary: trust and
...
IronClaw local-dev selection follows a catalog/list-first flow: the model may
...
The bundle format remains portable `SKILL.md`. IronClaw-specific declarations
...
- frontmatter cannot declare tenant, user, agent, project, trust promotion, or
  capability grants.
- unknown IronClaw metadata must be ignored or rejected according to the provider
...
promotion state. IronClaw core still computes the effective runtime decision.
...
skill declares desired capability/profile/egress/secret needs
  ...
  -> IronClaw policy evaluates trust, visibility, grants, leases, and obligations
...
SkillBundleSource
  ...
  -> IronClaw trust and visibility gate
...
If the extension later moves out of process, the IronClaw core contract should not
...
- Do not couple IronClaw skills to engine `MemoryDoc` or `src/workspace`.
