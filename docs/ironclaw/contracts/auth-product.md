...
This slice is contract-first. `ironclaw_auth` defines IronClaw-native vocabulary,
traits, validation helpers, and fake services. `ironclaw_composition`
owns the production filesystem-backed adapter and factory wiring. #3811 adds a
IronClaw composition seam, #3812 adds callback completion handling, #3881 mounts
the first IronClaw-native OAuth start/callback HTTP routes through
`ironclaw_composition`, #3882 adds the composition-facing manual-token
...
components with IronClaw components: V1 route handlers, pending maps, extension
...
respective IronClaw substrate contracts.
...
`IronClawProductAuthServices` is the single composition bundle for the product
...
`IronClawProductAuthServices` for flow preflight/callback handling. The handler
...
authorization URL from configured IronClaw product-auth client metadata and keeps
...
migration, but IronClaw paths should enter through `ProductWorkflow` or the
WebUI-facing `IronClawServicesApi` facade. They must not call V1 pending maps,
...
remains a host-composition concern around `IronClawProductAuthServices`.
...
IronClaw product auth records are scoped by `AuthProductScope` plus opaque ids:
...
accelerators. Production product authority must be durable IronClaw state, not
V1 pending maps.

## Durable Production Slice (#4175)

Production product-auth records use the `ironclaw_auth` contract types and are
stored by the `ironclaw_composition` filesystem adapter under the normal
IronClaw scoped filesystem substrate, rooted at the caller's
...
`ScopedFilesystem` and `SecretStore` used by the rest of IronClaw; callers no
...
older v1/v2 tool paths only until a IronClaw settings-config resolver explicitly
...
- Credential refresh must go through `CredentialAccountService::refresh_account`
  or `IronClawProductAuthServices::refresh_credential_account`. Refresh requests
...
- GSuite first-party capabilities continue to choose a Google account
  ...
  secret is account-specific. IronClaw composition stages that selected access
...
`IronClawProductAuthServices::request_manual_token_setup` to mint the typed
challenge and `IronClawProductAuthServices::submit_manual_token` with a
non-serializable `IronClawManualTokenSubmitRequest` after reading the dedicated
...
Production implementations must route provider HTTP through IronClaw
...
local-dev/testing. Production profiles must receive durable IronClaw-native auth
...
The IronClaw composition mounts host-owned HTTP routes that enter
`IronClawProductAuthServices` (see
`crates/ironclaw_composition/src/product_auth/serve/mod.rs`). All mutation
...
| `GET`  | `/api/product-auth/oauth/google/callback` | Public static Google OAuth callback; resolves flow/scope from auth-owned encoded state, validates the durable state hash/PKCE claim, and completes through `IronClawProductAuthServices`. |
...
- These routes are an explicit exemption from the "everything goes through
  ...
  and enter `IronClawProductAuthServices`.

---

## V1 Behavior Inventory

| Product behavior | V1 evidence path | IronClaw owner | First-slice status |
| --- | --- | --- | --- |
| Extension/provider OAuth start | `src/extensions/manager.rs`, `src/auth/mod.rs` | `AuthFlowManager`, `CredentialSetupService`, `AuthProviderClient` | IronClaw-native route mounted in composition; production provider wiring deferred |
| Hosted OAuth callback | `src/channels/web/features/oauth/mod.rs` | `IronClawProductAuthServices::handle_oauth_callback`, `ProductAuthTurnGateResumeDispatcher` for turn-gate resume continuations | IronClaw-native route mounted in composition; production provider wiring deferred |
| Local OAuth callback | `src/extensions/manager.rs`, `src/auth/oauth.rs` | `AuthFlowManager`, `AuthProviderClient` | Inventory only |
| Manual token entry from chat | `src/agent/agent_loop.rs`, `src/agent/thread_ops.rs` | `AuthInteractionService` secure submit via `IronClawProductAuthServices::{request_manual_token_setup,submit_manual_token}` | IronClaw facade ready; route migration deferred |
