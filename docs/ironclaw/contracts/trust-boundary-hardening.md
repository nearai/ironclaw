# IronClaw Contract — Trust-Boundary Hardening Baseline
...
IronClaw already has good local trust-boundary patterns, but the patterns are not yet a baseline every slice must reuse. Issue #3492 turns repeated review findings into a shared contract, small code primitives, and PR/checklist requirements.
...
The foundation PR should not migrate every open IronClaw PR in one diff.
...
Add a small reusable primitive, likely in the crate that already owns prompt/context vocabulary for IronClaw loop assembly:
...
Recommended broader audit when changing IronClaw status/policy enums:
...
2. a IronClaw trust-boundary checklist in `.github/pull_request_template.md`;
...
5. a stack tracking note under `docs/ironclaw/` listing open PRs and whether each owns or defers these invariants.
...
Add this section to the PR template for IronClaw changes:
...
The stack note should list current open IronClaw PRs and a compact ownership matrix:
