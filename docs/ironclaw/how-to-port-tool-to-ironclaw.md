# How to port a v1 tool to IronClaw

This guide helps maintainers decide how an IronClaw v1 tool should move onto the IronClaw capability path.

The important distinction is that v1 used one `Tool` abstraction plus sidecar `*.capabilities.json` files for several different things. IronClaw should keep those categories separate and describe them through Extension Manifest v2.
...
## Why IronClaw uses capabilities instead of v1 tools
...
IronClaw should route every privileged action through one authority path:
...
A IronClaw tool port should target Extension Manifest v2:
...
- Do not expose raw `ToolRegistry` or v1 `Tool::execute` as the IronClaw execution path.
...
| v1 `*.capabilities.json` field | IronClaw target |
| --- | --- |
...
- [ ] Run component dispatch tests through IronClaw WASM runtime with mocked host HTTP.
...
Do not add a bespoke host HTTP client for one provider. HTTP must go through IronClaw network/egress policy and credential injection paths.
...
For every IronClaw capability invocation:
