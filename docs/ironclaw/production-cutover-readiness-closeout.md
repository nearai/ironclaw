# IronClaw Production Cutover Readiness Closeout
...
> configs (Railway, the GCP systemd unit, Docker CI) were repointed at IronClaw
...
> back to; rollback is now profile/deployment-based only (disable the IronClaw
> profile or roll back the deploy). The `legacy_main_does_not_compose_ironclaw_runtime`
> test this doc cites was removed along with `src/main.rs`.

This note is the final closeout map for the IronClaw production wiring and
cutover-readiness epic. It does not make IronClaw default-on and it does not
...
traffic cannot reach a partially wired IronClaw graph.

## Source Of Truth

Production cutover is controlled by `IronClawCompositionProfile` plus typed
`IronClawBuildInput` / `IronClawRuntimeInput`:

| Profile | Runtime behavior |
| --- | --- |
| `disabled` | Default-off. `build_ironclaw_services` returns no runtime facades and a blocking disabled readiness diagnostic. `build_ironclaw_runtime` rejects live traffic. |
...
The live runtime boundary is `build_ironclaw_runtime`. CLI/WebUI entrypoints reach
...
| Production cutover gate before serving traffic | #4619 | #4682 | Runtime cutover-gate tests and `build_ironclaw_runtime` caller-path coverage |
| PostgreSQL production storage config | #4551 | #4631 | Production Postgres factory tests in `facade_factory.rs` |
| Production `build_ironclaw_runtime` launch | #4615 | #4645 | Production runtime launch path and readiness checks |
| Backend-parity readiness coverage | #4620 | #4713 | `libsql_substrate_readiness_diagnostics_cover_required_backend_gaps`, `postgres_substrate_readiness_diagnostics_cover_required_backend_gaps`, `build_ironclaw_runtime_allows_validated_production_readiness` |
| Operator status/config surfaces consuming readiness | #4595, #4593 | #4737, #4736 | `docs/ironclaw/contracts/operator-observability-backends.md`, `docs/ironclaw/contracts/operator-effective-config.md` |

## Acceptance Criteria Mapping

| #3026 criterion | Closeout evidence |
| --- | --- |
| Explicit typed production profile/config path | `IronClawCompositionProfile`, `IronClawBuildInput`, `IronClawRuntimeInput`, profile parsing tests, and `run_honors_boot_profile_from_config_file`. |
| Disabled mode exposes no partial IronClaw production services | `IronClawServices::disabled`, `disabled_returns_empty_services`, `disabled_readiness_is_redaction_safe`, and `runtime_rejects_disabled_profile_before_local_substrate_lookup`. |
...
| AppBuilder/default startup stays clear | IronClaw production composition remains in `ironclaw_composition`. (Historical: this was also guarded by `legacy_main_does_not_compose_ironclaw_runtime` against legacy `src/main.rs`, removed along with `src/` under Tier B.) |
| IronClaw binary remains thin bootstrap | `ironclaw` delegates to command modules and IronClaw-owned factories; `ironclaw_binary_main_is_thin_bootstrap` guards this mechanically. |
| WebUI/Product Workflow consume facade APIs | `IronClawWebuiBundle`, `build_webui_services`, product-live adapter tests, and crate guardrails in `crates/ironclaw_composition/CLAUDE.md`. |
| Required graph components included or diagnosed | Host-runtime `ProductionWiringReport`, IronClaw readiness diagnostic component mapping, and #4620 backend-parity readiness tests. |
| PostgreSQL/libSQL parity evidence | #4620 tests plus #4551/#4615 production storage/runtime launch work. |
| No hidden legacy/IronClaw dual writers | IronClaw startup is separate from legacy `main.rs`; migration/compatibility writes remain under #3029 and are not silently performed by production readiness. |
| Rollback/default-off behavior before final default-on | Default profile is `disabled`; operators can stop the IronClaw binary or switch profile back to `disabled` before any future irreversible migration/default-on cutover. |

## Rollback And Default-Off Contract

Before IronClaw becomes default-on, rollback is profile/deployment based:

1. Switch IronClaw profile to `disabled`, or stop the standalone IronClaw binary.
...
Any future bridge that writes both legacy and IronClaw state must define its own
...
| Issue | Disposition |
| --- | --- |
| #3029 migration/compatibility bridges | Still owns actual v1/IronClaw migration and bridge behavior. #3026 only proves production readiness stays default-off and does not create hidden dual writers. |
...
cargo test -p ironclaw_architecture ironclaw_binary_main_is_thin_bootstrap
```
