...
This slice adds a Docker-backed compatibility lane for dynamic process execution through IronClaw process lifecycle. It is intentionally separate from the manifest-derived `ironclaw_scripts` lane.
...
- Wire product approval and secure secret capture into the IronClaw process capability path.
