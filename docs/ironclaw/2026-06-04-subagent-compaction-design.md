...
5. `crates/ironclaw_product_adapters/src/outbound.rs:638` — extend `ProductProjectionItem::RunStatus`; `crates/ironclaw_composition/src/projection.rs:1025` (`run_status_projection_state`) will need to pass them through (does not today).
...
- `crates/ironclaw_composition/src/projection.rs:1025` — `run_status_projection_state` plumbing
...
Closing criteria for #4474: steps 2–6 merged; background subagent feature toggle defaults to off until E2E green; documented contract under `docs/ironclaw/`. Step 1 can merge independently ahead of all of them.
