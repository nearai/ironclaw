...
**Branch scope:** `reborn-integration` — all touched crates are IronClaw-owned
...
IronClaw compaction is userland strategy per the kernel boundary contract (`docs/ironclaw/contracts/kernel-boundary.md` §3 lists "profile presentation and summarization strategy" as userland). This spec adds no kernel surface and requires no contract-freeze amendment.
...
- Legacy `src/agent/`, `src/bridge/`, or `engine_v2` paths. This work targets IronClaw crates only.
...
`SystemInferencePort` is the host-owned inference boundary used by the compaction task. The IronClaw implementation dispatches directly through the host-managed model gateway with already-sanitized system/input text, no assistant prompt-bundle authority, and no capability surface. The same port and pattern serve future system inference tasks (goal refresh, error classification, memory consolidation). Tasks with real branching, validation, or invariant logic live in `loop_support` task modules.

## 4. Ownership and file map

All listed paths are IronClaw-owned crates. Cross-crate dependency rules and architecture boundary tests are exercised by `cargo test -p ironclaw_architecture` (run after any dependency, public API, facade, scope, or boundary change per `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs` and `ironclaw_composition_boundaries.rs`).
...
Implementation lands in four phases, each independently mergeable. Each phase touches IronClaw crates only and runs the architecture boundary test suite (`cargo test -p ironclaw_architecture`).
...
Per `.claude/rules/architecture.md`, `.claude/rules/testing.md`, `crates/ironclaw_agent_loop/CLAUDE.md`, and `docs/ironclaw/contracts/_contract-freeze-index.md` §9 review rubric:
...
**Architecture suite:**
- `cargo test -p ironclaw_architecture` after public API or dependency-boundary changes (mandatory per `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs`).
...
- IronClaw contracts honored without amendment: `turns-agent-loop.md`, `agent-loop-protocol.md`, `lightweight-agent-loop.md`, `kernel-boundary.md`, `events-projections.md`, `turn-runner.md`, `loop-exit.md`.
- IronClaw architecture boundary tests: `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs`, `ironclaw_composition_boundaries.rs`.
