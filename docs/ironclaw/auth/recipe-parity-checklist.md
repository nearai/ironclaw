...
3. **The specificity scanner is present on this rollup** (unlike main):
   ...
   `ironclaw_retired_taxonomy.rs` exist here, so §0.5.1 is enforceable — but the folded
...
(The `ironclaw_webui_ingress` GitHub *sign-in* is a separate login system, out of scope here.)
...
1. **Recipe-only (§0.5.1).** On this rollup the specificity scanner
   (`ironclaw_extension_specificity.rs`) DOES exist. The folded engine changes (A6 scope clamp, A1
