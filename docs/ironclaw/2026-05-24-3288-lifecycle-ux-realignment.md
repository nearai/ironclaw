# #3288 IronClaw lifecycle UX realignment
...
remain with their existing IronClaw services. Lifecycle projections use:
...
| Surface | Current implementation | IronClaw lifecycle owner | Status |
| --- | --- | --- | --- |
...
- The standalone `ironclaw` CLI still reports several catalog surfaces as
  not wired; do not read CLI command availability as lifecycle migration
  completion.
- WebUI setup now returns a IronClaw lifecycle projection, but it intentionally
