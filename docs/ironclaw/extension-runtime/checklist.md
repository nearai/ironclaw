...
- [x] MAN-9 Retired-taxonomy gate still passes (no `slack_bot`/
  ...
  `ironclaw_code_never_references_retired_taxonomy`
  (`crates/ironclaw_architecture/tests/ironclaw_retired_taxonomy.rs`) scans
...
- [x] REC-1 Compile once → persisted resolved record + manifest digest; all
  ...
  (`crates/ironclaw_architecture/tests/ironclaw_manifest_reparse_gate.rs`)
...
- [x] LIFE-6 The installation state machine is one shared enum — an honest
  ...
  extensions wire as `IronClawExtensionInfo.installation_state`.
...
- [ ] LIFE-12 Removing one extension preserves grants of a shared vendor
  ...
  *policy* is NOT enforced yet: the production hook
  ...
  (`crates/ironclaw_composition/src/extension_host/generic_host.rs`) is a
...
- [x] TOOL-3 Authorization, approvals, obligations, resource reservation,
  ...
  lease rejected before dispatch), `ironclaw_invoke_vertical_slice.rs`
...
- [x] TOOL-8 `slack.send_message` remains an explicit side-effect tool; final
  ...
  leg (`test_ironclaw_slack_channel_e2e.py`) both observe the coordinated
...
- [x] AUTH-1 One engine implements `oauth2_code` and `api_key`; there is no
  ...
  `crates/ironclaw_composition/src/product_auth/credentials/product_auth_providers.rs`);
  ...
  gate `ironclaw_generic_code_names_no_concrete_extension` passes against a
...
- [x] AUTH-3 State/CSRF, PKCE, TTL, and callback replay are enforced; exactly
  ...
  (`crates/ironclaw_composition/src/product_auth/durable/tests.rs`).
...
- [x] AUTH-9 The auth account state machine is one shared enum
  ...
  `IronClawAuthAccount.state` is exactly this enum (`ironclaw_services/types.rs`),
  ...
  (`ironclaw_services_contract.rs`), driven through `list_extensions`; the
  projection `vendor_auth_accounts` (`ironclaw_services/extensions.rs`) maps a
...
- [x] AUTH-13 Callback route keeps the existing
  ...
  `crates/ironclaw_composition/src/product_auth/serve/mod.rs`; the
...
- [x] AUTH-14 Slack end-to-end: blocked tool → gate → scripted callback →
  ...
  `crates/ironclaw_composition/src/product_auth/serve/mod.rs`), and the
  ...
  the P6 §10 e2e (`tests/e2e/scenarios/test_ironclaw_slack_channel_e2e.py`). The
...
- [x] OUT-9 Delivery attempt persistence passes on both DBs. — the P5 delivery
  ...
  separately-cited `ironclaw_integration_outbound_store_durability` binary is
...
- [x] OUT-12 Trace contributions use generic extension/surface origin ids;
  concrete variants are deleted. — `ironclaw_traces` carries
...
- [x] DEL-5 The `ProductAdapter` contract is fully retired (owner decision:
  ...
  ported, not deleted:** the live test double `IronClawTestProductAdapter` → the
  trait-free `IronClawTestIngress` (builds `ParsedProductInbound` directly on the
...
- [x] DEL-6 Composition constructs no concrete extension and mounts no
  ...
  adapters through `IronClawBuildInput::with_channel_extension_bindings`).
  Gates green: `ironclaw_generic_code_names_no_concrete_extension` +
  `concrete_extension_crates_link_only_from_the_binary_and_tests`
  (`crates/ironclaw_architecture/tests/ironclaw_extension_specificity.rs`,
...
- [x] DEL-7 Only the canonical `ironclaw` CLI package and tests depend on concrete extension
  ...
  adapter + extras through `IronClawBuildInput::with_channel_extension_bindings`.
- [ ] DEL-8 The concrete-name scanner allowlist is empty. — In progress: the
  shrink-only `ALLOWLIST` (`ironclaw_extension_specificity.rs`) is **86**
  ...
  - **Lane 1 done** — i18n `github` (11) carved as localized UI copy (the SSO
    ...
    reworded and cleared. The `ironclaw-extension-surfaces` skill how-to was
    rewritten to the v3 manifest shape (REL-4, not an allowlist entry).
  - **Lane 2 done** — the `ironclaw_traces::contribution` redaction
...
- [x] UI-1 The wire carries surface keys, the installation state enum, the
  ...
  — the wire carries surfaces (`IronClawExtensionSurface`), the §6.1
  `InstallationState` enum (`IronClawExtensionInfo.installation_state`, replacing
  the `activation_status` string), the §6.3 auth-account enum via the
  per-vendor accounts list (`IronClawAuthAccount.state`, replacing
  `connected: Option<bool>`), and setup field/secret descriptors
  (`ironclaw_services/types.rs`). One golden fixture pins the full shape:
  `list_extensions_golden_wire_multi_surface_extension_freezes_accounts_list`
  (`ironclaw_services_contract.rs`) freezes an arbitrary channel on a
...
- [ ] UI-2 The channels tab renders every channel surface with the same
  ...
  *browser*" is not achievable — the acme fixture is a Rust integration-test
  ...
  flow by `test_ironclaw_slack_channel_e2e.py` (UI-6, httpx). The remaining gap
...
- [x] UI-6 The existing Python e2e harness covers: configure → connect →
  ...
  added. — `tests/e2e/scenarios/test_ironclaw_slack_channel_e2e.py` over
...
- [x] MIG-1 OAuth grant/account storage is reused (vendor id strings
  ...
  `ironclaw_services/extensions.rs`), frozen by the golden fixture
  `list_extensions_golden_wire_multi_surface_extension_freezes_accounts_list`
...
- [x] MIG-4 Old installation lifecycle records backfill into the standard
  ...
  (`crates/ironclaw_composition/src/extension_host/generic_host.rs`):
...
- [x] MIG-6 OAuth callback URLs are unchanged (no vendor reconfiguration
  ...
  (`test_ironclaw_slack_channel_e2e.py`).
...
- [ ] TEST-4 The acme fixture drives the full generic path end-to-end in the
  ...
  `IronClawBuildInput` seam. P4 landed the inbound leg —
  `signed_acme_post_flows_through_the_production_mount_into_a_turn`.
...
- [x] TEST-6 The specificity scanner derives forbidden names from the package
  ...
  (`crates/ironclaw_architecture/tests/ironclaw_extension_specificity.rs`); the
...
- [x] TEST-7 Allowlist shrinkage is enforced: stale entries fail, new
  ...
  `ironclaw_generic_code_names_no_concrete_extension`; same discipline on the
...
- [ ] REL-4 `docs/ironclaw/contracts/*`, the `ironclaw-extension-surfaces` skill,
  `FEATURE_PARITY.md`, and `CHANGELOG.md` describe the shipped system. —
  Partial: the **`ironclaw-extension-surfaces` skill is done** — rewritten to the
  v3 manifest shape (`[[tools]]`/`[channel]`/`[auth.<vendor>]`/`[mcp]`,
  `VendorId`, live `assets/slack/manifest.toml` example), every cited path
  verified against HEAD. `docs/ironclaw/contracts/*`, `FEATURE_PARITY.md`, and
