...
(`crates/ironclaw_product_workflow/src/ironclaw_services/{types,extensions}.rs`);
...
| OAuth branches on Slack; providers multiplexed by string | `crates/ironclaw_composition/src/product_auth/serve/oauth.rs`, `.../credentials/product_auth_providers.rs`, `.../oauth/google_oauth.rs`, `src/slack/slack_personal_oauth.rs` |
...
| Lifecycle emits Slack-specific connection copy; workflow has Slack cleanup literals | `crates/ironclaw_composition/src/extension_host/extension_lifecycle.rs`, `crates/ironclaw_product_workflow/src/ironclaw_services/extensions.rs` |
...
| Concrete channel variants in trace contributions | `crates/ironclaw_traces/src/contribution.rs` |
...
| `ironclaw_traces` | Generic extension/surface origin ids replace concrete variants |
| `ironclaw_architecture` | New specificity scanner + dependency gates (section 12) |
| `ironclaw_migration` | One-time migrations (section 11) |
...
- `CommunicationPresentationPolicy` derived from `[channel.presentation]`
  ...
  variants in `crates/ironclaw_traces/src/contribution.rs` with
...
`product_workflow/ironclaw_services/extensions.rs` (standard removal pipeline
...
- Wire additions (backend `ironclaw_services/types.rs`): full surface key per
...
- Browser coverage reuses the existing Python harness
  (`tests/e2e/ironclaw_webui_harness.py` + `fake_slack_api.py`): one scenario —
...
Small, versioned, idempotent, in `ironclaw_migration`, tested on both
...
- **Architecture gates** (`crates/ironclaw_architecture/tests/`):
  - keep `ironclaw_retired_taxonomy.rs`;
  - add `ironclaw_extension_specificity.rs`: scans generic `crates/**/src`,
...
| **P7** | Allowlist → zero; `check-generic-without-concrete.sh` in CI; docs (`docs/ironclaw/contracts/*`, `ironclaw-extension-surfaces` skill, `FEATURE_PARITY.md`, `CHANGELOG.md`); checklist fully evidenced | P6 |
...
(accepted 2026-07-13,
`docs/ironclaw/extension-runtime/adr/0001-multiple-accounts-per-vendor.md`).
