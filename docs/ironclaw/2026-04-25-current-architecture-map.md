...
docs/ironclaw/contracts/settings-config.md
docs/ironclaw/contracts/turns-agent-loop.md
docs/ironclaw/contracts/turn-persistence.md
docs/ironclaw/contracts/conversation-binding.md
...
`docs/ironclaw/contracts/triggers.md`; `ironclaw_triggers` now owns trigger
...
capabilities. IronClaw composition now wires a configurable background trigger
...
trusted inbound construction, and IronClaw architecture dependency tests.
...
IronClaw has one host core with many adapters and runtime ports. It should not grow one host per vendor or per transport.
...
The current IronClaw stack includes these contract and implementation slices. These rows do not claim full product completion unless they explicitly use `[fully implemented]`:
...
| Architecture guardrails | `[partially implemented]` `ironclaw_architecture` test crate walks `cargo metadata` and enforces central IronClaw dependency-boundary rules; per-crate guardrail files document local invariants |
...
| Subagent spawn design | `[proposed design]` `docs/ironclaw/subagent-spawn/README.md` and its phase docs specify child-loop spawning for the IronClaw agent loop. No runtime implementation is claimed by this row; mergeable implementation still needs the contracts, DB-backed stores, projection wiring, and caller-level tests described there. |
...
| Reference/userland loop runtime | The default parent agent loop should be a shipped reference loop running on the kernel surface and emitting `Reply | CapabilityCalls`; custom loop families are expected later. It is not yet a IronClaw runtime/service. |
| Subagent spawn runtime | A proposed design exists in `docs/ironclaw/subagent-spawn/README.md`, but `spawn_subagent`, child loop families, dependent-run gates, background wake delivery, pending-gate projection wiring, and production DB-backed subagent stores are not implemented yet. |
...
- `docs/ironclaw/contracts/agent-loop-protocol.md`
- `docs/ironclaw/contracts/lightweight-agent-loop.md`
- `docs/ironclaw/contracts/runtime-workflows.md`
- `docs/ironclaw/contracts/live-vertical-slice.md`
