# IronClaw Replay Harness

IronClaw should reuse IronClaw's existing replay harness and extend it for IronClaw compatibility evidence.

This document describes the target shape. It does not mean every IronClaw-specific fixture exists today.
...
- `.github/workflows/ironclaw-tests.yml` (the `IronClaw QA recorded fixtures` job
  + `scripts/ci/check-ironclaw-qa-fixtures.sh`) — the IronClaw recorded-fixture
  gate that replaced the v1 `replay-gate.yml`;
- `tests/snapshots/` (the `golden_payload__*.snap` insta snapshots checked by
  `ironclaw_integration_golden_payload`).
...
- Let agents validate IronClaw changes without live LLM/provider calls.
...
## Target IronClaw fixture families
...
A future IronClaw-specific coverage script can be added once IronClaw durable event
