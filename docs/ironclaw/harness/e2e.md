# IronClaw E2E Harness

This document is the branch-local map for the dedicated IronClaw E2E gate. IronClaw is not missing tests: `main` already contains extensive crate-level contract and integration coverage. The gap this branch closes is a single, named E2E workflow that runs the IronClaw architecture spine together and keeps a small product-surface smoke check beside it.

## What already exists

| Area | Existing coverage |
| --- | --- |
| Architecture boundaries | `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs` |
...
| Dedicated IronClaw E2E spine | `crates/ironclaw_host_runtime/tests/ironclaw_e2e_gate.rs` |
...
## Dedicated IronClaw E2E goal

The dedicated gate should answer one question:

```text
Can the IronClaw architecture path still execute happy, blocked, denied, failed,
...
The IronClaw E2E happy path is covered by a dedicated `ironclaw_e2e_gate.rs` spine test plus the broader host-runtime and runtime-lane tests:
...
The dedicated gate includes new IronClaw E2E gate tests plus existing tests for these non-happy paths:
...
This smoke test does not replace the full browser E2E workflow. It proves the branch remains product-bootable while the Rust IronClaw gate proves architecture behavior.

## Local commands

Run the deterministic Rust IronClaw gate:
...
scripts/ironclaw-e2e-rust.sh runtimes
scripts/ironclaw-e2e-rust.sh substrates
```

The script expands to the dedicated `ironclaw_e2e_gate.rs` tests plus the current IronClaw boundary, host-runtime, capability-host, dispatcher, WASM, Script, MCP, process, event, filesystem, network, secret, resource, run-state, approval, and authorization contract tests. Use the script as the source of truth for local/CI parity rather than copying individual `cargo test` commands.
...
pytest scenarios/test_ironclaw_gateway_smoke.py -v --timeout=120
```

## CI ownership

`ironclaw-e2e.yml` is intentionally separate from `e2e.yml`:

- IronClaw changes can run a focused architecture gate without destabilizing the main browser E2E matrix.
