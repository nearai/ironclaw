# Set Up Slack for the IronClaw Binary
...
stored in the IronClaw secret store. Do not put OAuth client secrets or LLM keys
in `config.toml`.
...
Slack is disabled unless the mounted or seeded IronClaw config enables it.

## Public Endpoint

Slack Events API must reach the IronClaw listener over a public HTTPS URL:

```text
https://<public-host>/webhooks/extensions/slack/events
```

Slack personal OAuth must also redirect back to the IronClaw product-auth
callback:

```text
https://<public-host>/api/product-auth/oauth/slack_personal/callback
```
...
```bash
export IRONCLAW_HOME="$PWD/.ironclaw-home"
...
| Bot user | Optional IronClaw user id for Slack host-mediated egress. Defaults to the WebUI operator. |
| Shared subject | Optional IronClaw user scope available for shared-channel routing. |
| Bot token | Slack bot token. Stored in the IronClaw secret store; never returned by the API. |
| Signing secret | Slack signing secret. Stored in the IronClaw secret store; never returned by the API. |
...
- Add user token scopes:
  - `users:read` for binding the authenticated Slack user to the IronClaw user.
...
- After the user installs and configures the Slack extension, the OAuth callback
  binds that Slack user to the authenticated IronClaw user.
- A DM to the app routes through the OAuth-connected IronClaw user.
...
Confirm the IronClaw config sets [slack].enabled = true, or that the deployment env sets IRONCLAW_SLACK_ENABLED=true, then restart `ironclaw`.

### Slack route never receives events

Confirm the Slack Request URL is exactly https://<public-host>/webhooks/extensions/slack/events, the public URL reaches the IronClaw listener, and Socket Mode is disabled for this host path.
...
Confirm the Slack redirect URL is exactly https://<public-host>/api/product-auth/oauth/slack_personal/callback, the user scope includes users:read, the app was reinstalled after changing OAuth settings, and the WebUI Slack setup client id/client secret match the Slack app.

### Channel mention does not reach IronClaw
