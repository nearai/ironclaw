...
- **Activity** — ordered model and tool lifecycle events. Previous and next
  buttons navigate runs observed for this thread during the browser session.
- **Stats** — available token, latency, model-call, and tool-call totals.
...
Remove `debug=true` to return to the ordinary chat layout. The inspector is
diagnostic only: opening, closing, or navigating it never changes a run.
