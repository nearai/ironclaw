...
Use `debug=false` to disable the inspector for the current browser tab and
return to the ordinary chat layout. The inspector is diagnostic only: opening,
closing, or navigating it never changes a run.
