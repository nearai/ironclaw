...
///
/// A search, not counted `.parent()` hops: the family move
/// (`crates/loop/ironclaw_loop_host`, PROPOSAL §5) makes any fixed depth false,
/// and the wrong answer (`crates/`) exists — so the failure would be silent
/// rather than loud (CHECKLIST WS10).
fn workspace_root() -> PathBuf {
    ...
    panic!("crate lives under workspace crates directory");
}
