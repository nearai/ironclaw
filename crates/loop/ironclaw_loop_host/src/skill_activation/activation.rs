...
/// that fails the whole skill-context build, not a truncation. At `512 * (250 + 64)` this was
/// 160,768, two and a half times that cap, so a large enough catalog took the runtime down instead
/// of listing fewer skills. Past what fits, the answer is `skill_search` (#4428).
...
const _: () = assert!(
    ...
    "the rendered listing must fit the single snippet it ships as, or the skill-context build \
