...
2. When a result has `schema_complete=true`, use its returned `parameters` schema and invoke it directly. When `schema_complete=false`, or when you need to inspect an ambiguous result, call `tool_describe` with its `name` first.
