...
Both seams live in `ironclaw_turn_runner`. From a standalone
...
an `ironclaw_turn_runner` integration test
...
either (a) depending on `ironclaw_turn_runner` as a dev-dep (heavy + the
...
3. Accept the friction and document that NumericSum hooks must be
   integration-tested via `ironclaw_turn_runner`. Cheapest, but it's a
