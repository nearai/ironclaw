...
- This crate must not depend on `ironclaw_turn_runner`, host runtime crates, product
...
- The framework never sees `AgentLoopDriver`; `PlannedDriver` in
  `ironclaw_turn_runner` adapts runner-facing driver calls to this crate's executor.
