...
- Read-model DTOs: `ThreadTimeline`/`TimelineEntry`/`TimelineEntryKind`, `RunStatusProjection`/`RunProjectionStatus`, and `MemoryAuditProjectionMetadata` (audit entries carry the shared `ironclaw_host_api::audit::AuditStage`); plus the `DurableMemoryAuditSink`.
