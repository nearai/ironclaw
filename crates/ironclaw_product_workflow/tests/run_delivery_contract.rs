...
#[async_trait]
impl ironclaw_product_workflow::ConversationBindingService for EffectiveMembershipBindingService {
    async fn resolve_binding(
        &self,
    ...
    > {
        if !self.authorized() {
            return Err(ironclaw_product_workflow::ProductWorkflowError::BindingAccessDenied);
        }
        Ok(binding())
    }
    ...
    ) -> Result<ResolvedStoredProductReplyTarget, ironclaw_product_workflow::ProductWorkflowError>
    {
        if !self.authorized() {
            return Err(ironclaw_product_workflow::ProductWorkflowError::BindingAccessDenied);
        }
    ...
    T: TurnCoordinator + 'static,
{
    ...
    let services = RunDeliveryServices {
        binding_service: Arc::clone(&binding)
            as Arc<dyn ironclaw_product_workflow::ConversationBindingService>,
        thread_service: Arc::clone(&threads) as Arc<dyn SessionThreadService>,
        turn_coordinator: Arc::clone(&turns) as Arc<dyn TurnCoordinator>,
        outbound_store: Arc::clone(&store) as Arc<dyn OutboundStateStore>,
