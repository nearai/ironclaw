...
//!
//! Hoisted from `ironclaw_webui_v2` (2026-07) so non-route consumers — the
//! Reborn TUI client, and any future thin client — can depend on the wire
//! contract without depending on the WebUI v2 route/handler crate. This
//! module is still the *rendering* contract, not a durable event log: it
//! deliberately does not expose adapter routing metadata such as
//! installation ids, reply binding refs, external conversation refs, or
//! delivery attempt ids. `ironclaw_webui_v2::schema` re-exports these two
//! types so existing SSE handler code is unchanged.
