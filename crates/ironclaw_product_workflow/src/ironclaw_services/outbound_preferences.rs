...
pub struct IronClawOutboundPreferencesFacade {
    preferences: Arc<dyn CommunicationPreferenceRepository>,
    targets: Arc<dyn OutboundDeliveryTargetProvider>,
}

impl std::fmt::Debug for IronClawOutboundPreferencesFacade {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("IronClawOutboundPreferencesFacade")
...
impl IronClawOutboundPreferencesFacade {
    pub fn new(
        preferences: Arc<dyn CommunicationPreferenceRepository>,
        targets: Arc<dyn OutboundDeliveryTargetProvider>,
    ) -> Self {
        ...
        let (final_reply_target, final_reply_target_status) =
            match record.and_then(|record| record.final_reply_target.as_ref()) {
                Some(target) => match self.summary_for_reply_target(caller, target).await? {
                    ...
                    None => (None, IronClawOutboundDeliveryTargetStatus::Unavailable),
                },
    ...
    fn response_for_resolved_final_reply_target(
        ...
        let final_reply_target = resolved_final_reply_target
            .map(|entry| ironclaw_summary_from_outbound(&entry.summary))
            .transpose()?;
    ...
    ) -> Result<Option<IronClawOutboundDeliveryTargetSummary>, IronClawServicesError> {
        self.targets
    ...
    ) -> Result<OutboundDeliveryTargetEntry, IronClawServicesError> {
        let target_id = outbound_target_id_from_ironclaw(target_id)?;
        self.targets
...
#[async_trait]
impl OutboundPreferencesProductFacade for IronClawOutboundPreferencesFacade {
    ...
    async fn list_outbound_delivery_targets(
        ...
        let targets = self
            ...
            .map(|entry| {
                Ok(IronClawOutboundDeliveryTargetOption {
                    target: ironclaw_summary_from_outbound(&entry.summary)?,
                    capabilities: ironclaw_capabilities_from_outbound(&entry.capabilities),
                })
            })
            .collect::<Result<Vec<_>, IronClawServicesError>>()?;
...
fn outbound_target_id_from_ironclaw(
    target_id: &IronClawOutboundDeliveryTargetId,
) -> Result<OutboundDeliveryTargetId, IronClawServicesError> {
    OutboundDeliveryTargetId::new(target_id.as_str()).map_err(|_| IronClawServicesError {
...
fn ironclaw_summary_from_outbound(
    summary: &OutboundDeliveryTargetSummary,
) -> Result<IronClawOutboundDeliveryTargetSummary, IronClawServicesError> {
    let target_id = IronClawOutboundDeliveryTargetId::new(summary.target_id.as_str())
...
fn ironclaw_capabilities_from_outbound(
    capabilities: &ironclaw_outbound::DeliveryTargetCapabilities,
) -> IronClawOutboundDeliveryTargetCapabilities {
    IronClawOutboundDeliveryTargetCapabilities {
...
fn outbound_target_projection_error() -> IronClawServicesError {
...
fn outbound_target_not_found() -> IronClawServicesError {
    IronClawServicesError {
...
fn map_outbound_repository_error(error: OutboundError) -> IronClawServicesError {
    match error {
        OutboundError::InvalidRequest { .. }
        | OutboundError::PreferenceTargetMissing { .. }
        | OutboundError::SubscriptionScopeMismatch
        | OutboundError::DeliveryNotFound => IronClawServicesError {
            code: IronClawServicesErrorCode::InvalidRequest,
            kind: IronClawServicesErrorKind::Validation,
            status_code: 400,
            retryable: false,
        ...
        OutboundError::AccessDenied => IronClawServicesError {
        ...
        OutboundError::CasConflict => IronClawServicesError {
        ...
        OutboundError::Backend | OutboundError::Serialization => IronClawServicesError {
...
mod tests {
    ...
    async fn get_preferences_projects_stored_final_target_for_authenticated_user() {
        ...
        .await;
        let facade = IronClawOutboundPreferencesFacade::new(store, provider);
    ...
    async fn get_preferences_returns_none_when_stored_target_not_in_provider() {
        ...
        assert_eq!(
            response.final_reply_target_status,
            IronClawOutboundDeliveryTargetStatus::Unavailable
        );
    }

    #[tokio::test]
    async fn get_preferences_maps_backend_read_error_to_unavailable() {
        let facade = IronClawOutboundPreferencesFacade::new(
    ...
    async fn set_preferences_validates_target_id_before_writing_reply_target() {
        ...
        let facade = IronClawOutboundPreferencesFacade::new(store.clone(), provider);
    ...
    async fn set_preferences_maps_write_cas_conflict_to_conflict() {
        ...
        assert_eq!(error.code, IronClawServicesErrorCode::Conflict);
        assert_eq!(error.kind, IronClawServicesErrorKind::Conflict);
    ...
    async fn target_provider_errors_are_propagated_by_get_set_and_list() {
        ...
        .await;
        let facade = IronClawOutboundPreferencesFacade::new(store, Arc::new(FailingTargetProvider));
    ...
    async fn target_registry_aggregates_channel_neutral_providers_for_default_selection() {
        ...
        let facade = IronClawOutboundPreferencesFacade::new(store.clone(), registry);
    ...
    async fn target_registry_propagates_resolver_failure_for_get_and_set() {
        ...
        .await;
        let facade =
            IronClawOutboundPreferencesFacade::new(store, Arc::new(ResolveFailingTargetProvider));
    ...
    fn repository_error_mapping_distinguishes_authority_conflict_and_backend_errors() {
        for invalid_request_error in [
            ...
            assert_eq!(mapped.kind, IronClawServicesErrorKind::Validation);
            assert_eq!(mapped.status_code, 400);
            assert!(!mapped.retryable);
        }

        let access_denied = map_outbound_repository_error(OutboundError::AccessDenied);
        assert_eq!(access_denied.code, IronClawServicesErrorCode::Forbidden);
        assert_eq!(
            access_denied.kind,
        ...
        assert_eq!(cas_conflict.code, IronClawServicesErrorCode::Conflict);
        assert_eq!(cas_conflict.kind, IronClawServicesErrorKind::Conflict);
    ...
    fn assert_unavailable_backend_error(error: IronClawServicesError) {
    ...
    fn target_id(value: &str) -> IronClawOutboundDeliveryTargetId {
        IronClawOutboundDeliveryTargetId::new(value).expect("valid target id")
    }
