...
use super::types::{IronClawExtensionOnboardingPayload, IronClawExtensionOnboardingState};

#[derive(Debug, Clone, PartialEq, Eq)]
pub(super) struct ExtensionOnboarding {
    pub(super) state: Option<IronClawExtensionOnboardingState>,
    pub(super) onboarding: Option<IronClawExtensionOnboardingPayload>,
...
fn credential_onboarding(summary: &LifecycleExtensionSummary) -> ExtensionOnboarding {
    ...
    let state = if has_oauth {
        IronClawExtensionOnboardingState::AuthRequired
    } else {
        IronClawExtensionOnboardingState::SetupRequired
    };
    ...
    ExtensionOnboarding {
        state: Some(state),
        onboarding: Some(IronClawExtensionOnboardingPayload {
...
fn no_credential_onboarding(
    ...
    let state = match phase {
        InstallationState::Installed | InstallationState::Configured => {
            Some(IronClawExtensionOnboardingState::Installed)
        }
        InstallationState::Failed => Some(IronClawExtensionOnboardingState::Failed),
        _ => None,
    };
    ...
    ExtensionOnboarding {
        ...
        onboarding: instructions
            .as_ref()
            .map(|instructions| IronClawExtensionOnboardingPayload {
...
mod tests {
    ...
    fn github_manual_token_projects_setup_required_message() {
        ...
        assert_eq!(
            onboarding.state,
            Some(IronClawExtensionOnboardingState::SetupRequired)
        );
    ...
    fn google_oauth_projects_auth_required_message() {
        ...
        assert_eq!(
            onboarding.state,
            Some(IronClawExtensionOnboardingState::AuthRequired)
        );
    ...
    fn credential_ready_failed_extension_preserves_failed_state() {
        ...
        assert_eq!(
            onboarding.state,
            Some(IronClawExtensionOnboardingState::Failed)
        );
        assert_eq!(onboarding.awaiting_token, None);
        assert_eq!(
            onboarding.instructions.as_deref(),
