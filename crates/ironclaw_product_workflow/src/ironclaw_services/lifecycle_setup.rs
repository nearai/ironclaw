...
use crate::{
    ChannelConfigFacade, IronClawChannelConfigField, IronClawExtensionCredentialSetup,
    IronClawExtensionSetupField, IronClawExtensionSetupSecret, IronClawServicesError,
    IronClawServicesErrorCode, IronClawSetupExtensionResponse, IronClawViewDescriptor,
    ...
    LifecycleProductSurfaceContext, ProductWorkflowError, WebUiAuthenticatedCaller,
...
pub const EXTENSION_SETUP_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    id: "extension_setup",
    paginated: false,
};
...
pub(super) async fn setup_extension(
    ...
    if action == SetupAction::Submit {
        ...
        if channel_config.is_none() && !submit.fields.is_empty() {
            return Err(IronClawServicesError::service_unavailable(true));
        }
        ...
        if !channel_values.is_empty() {
            let port =
                channel_config.ok_or_else(|| IronClawServicesError::service_unavailable(true))?;
...
fn route_channel_config_values(
    ...
    channel_fields: &[IronClawChannelConfigField],
    requirements: &[LifecycleExtensionCredentialRequirement],
) -> Result<Vec<(String, String)>, IronClawServicesError> {
...
async fn setup_extension_response(
    ...
    let package_ref = lifecycle
        .package_ref
        .clone()
        .ok_or_else(IronClawServicesError::internal_invariant)?;
    ...
    for field in channel_fields.iter().filter(|field| field.secret) {
        ...
        secrets.push(IronClawExtensionSetupSecret {
            ...
            setup: IronClawExtensionCredentialSetup::ManualToken,
            credential_ref: None,
        });
    }
    ...
    let fields = channel_fields
        ...
        .map(|field| IronClawExtensionSetupField {
            name: field.name.clone(),
            prompt: field.label.clone(),
            optional: false,
            placeholder: None,
    ...
    Ok(IronClawSetupExtensionResponse {
        package_ref,
...
fn setup_action(
    request: &WebUiSetupExtensionRequest,
) -> Result<SetupAction, IronClawServicesError> {
...
pub(super) fn validation_error(
    ...
    IronClawServicesError::from(WebUiInboundValidationError::new(field, code))
}

pub(super) fn map_lifecycle_error(error: ProductWorkflowError) -> IronClawServicesError {
    match error {
        ProductWorkflowError::InvalidBindingRequest { .. }
        | ProductWorkflowError::UnsupportedActionKind { .. } => IronClawServicesError::from_status(
        ...
        | ProductWorkflowError::UnknownInstallation => IronClawServicesError::internal_invariant(),
    }
}
...
mod tests {
    ...
    fn provider_instance_not_configured_maps_to_sanitized_400() {
        ...
        assert_eq!(mapped.code, IronClawServicesErrorCode::InvalidRequest);
