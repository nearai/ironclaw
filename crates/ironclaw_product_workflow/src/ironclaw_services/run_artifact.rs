...
use ironclaw_traces::contribution::DeterministicTraceRedactor;
use ironclaw_traces::redaction::redact_sensitive_json;
use serde::{Deserialize, Serialize};

use super::{
    IronClawGetRunStateRequest, IronClawGetRunStateResponse, IronClawLogEntry,
    IronClawLogQueryRequest, IronClawServices, IronClawServicesError, IronClawServicesErrorCode,
    IronClawViewDescriptor, IronClawViewProvider, OPERATOR_LOGS_MAX_LIMIT,
    ProductCapabilityInvoker, WebUiAuthenticatedCaller, bounded_log_query, map_thread_error,
    parse_run_id_field, parse_thread_id_field,
};

pub const RUN_ARTIFACT_SCHEMA: &str = "ironclaw.run_artifact.v1";
pub const RUN_ARTIFACT_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    id: "run_artifact",
    paginated: false,
};
...
pub struct IronClawRunArtifactRequest {
    pub thread_id: String,
    pub run_id: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct IronClawRunArtifact {
    pub schema: String,
    pub generated_at: DateTime<Utc>,
    pub thread_id: String,
    pub run: IronClawGetRunStateResponse,
    ...
    V: IronClawViewProvider + Clone + 'static,
{
    pub(super) async fn build_run_artifact(
        ...
        request: IronClawRunArtifactRequest,
    ) -> Result<IronClawRunArtifact, IronClawServicesError> {
    ...
    async fn artifact_logs(
        ...
        let query = bounded_log_query(IronClawLogQueryRequest {
