...
use super::{
    AUTO_APPROVE_CONFIG_KEY, IronClawOperatorConfigGetResponse, IronClawOperatorConfigListResponse,
    ...
    ProductCapabilityInvoker, TOOL_CONFIG_PREFIX, WebUiAuthenticatedCaller,
...
pub const OPERATOR_CONFIG_LIST_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    id: "operator_config_list",
    paginated: false,
};

pub const OPERATOR_CONFIG_KEY_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    id: "operator_config_key",
    paginated: false,
};

pub const OPERATOR_CONFIG_VALIDATE_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    ...
    V: IronClawViewProvider + Clone + 'static,
{
    pub(super) async fn build_operator_config_list_view(
    ...
    ) -> Result<IronClawOperatorConfigListResponse, IronClawServicesError> {
        ...
        Ok(IronClawOperatorConfigListResponse {
            entries,
            precedence: vec![
                "locked".to_string(),
                "override".to_string(),
    ...
    pub(super) async fn build_operator_config_key_view(
        ...
        let OperatorConfigKeyViewParams { key } =
            serde_json::from_value(params).map_err(IronClawServicesError::internal_from)?;
        let Some(config) = &self.operator_approval_config else {
            let _ = (caller, key);
