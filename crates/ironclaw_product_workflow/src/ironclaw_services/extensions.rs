...
use crate::{
    ChannelAuthAccountState, ChannelConnectionFacade, IronClawAccountBindingSource,
    IronClawAuthAccount, IronClawExtensionInfo, IronClawExtensionListResponse,
    IronClawExtensionOnboardingState, IronClawExtensionRegistryEntry,
    IronClawExtensionRegistryResponse, IronClawExtensionSurface, IronClawServicesError,
    IronClawVendorAuthAccounts, LifecycleExtensionSummary, LifecycleInstalledExtensionSummary,
    LifecycleProductAction, LifecycleProductContext, LifecycleProductFacade,
    LifecycleProductPayload, LifecycleProductResponse, LifecycleProductSurfaceContext, ProductView,
    WebUiAuthenticatedCaller, WebUiInboundValidationCode,
};
...
pub const EXTENSIONS_VIEW: ProductView<serde_json::Value, IronClawExtensionListResponse> =
    ProductView::unpaginated("extensions");

pub const EXTENSION_REGISTRY_VIEW: ProductView<
    serde_json::Value,
    IronClawExtensionRegistryResponse,
...
pub(super) async fn list_extensions(
...
) -> Result<IronClawExtensionListResponse, IronClawServicesError> {
    ...
    // records, projected onto `IronClawExtensionInfo::activation_error`.
    let activation_errors = facade
        .installed_activation_errors(context)
        .await
        .map_err(map_lifecycle_error)?;
    Ok(IronClawExtensionListResponse {
...
pub(super) async fn list_extension_registry(
    ...
    caller: WebUiAuthenticatedCaller,
) -> Result<IronClawExtensionRegistryResponse, IronClawServicesError> {
    ...
    Ok(IronClawExtensionRegistryResponse {
...
) -> Result<LifecycleProductResponse, IronClawServicesError> {
    facade
...
) -> Result<Vec<IronClawExtensionInfo>, IronClawServicesError> {
    let resolved = stream::iter(installed)
        .map(|installed| {
            ...
            async move {
                ...
                .await?;
                Ok::<_, IronClawServicesError>((installed, readiness))
            }
        })
...
) -> IronClawExtensionRegistryEntry {
...
) -> Vec<IronClawExtensionSurface> {
    summary
        .surface_kinds
        .iter()
        .filter_map(|kind| match kind {
            CapabilitySurfaceKind::Tool => Some(IronClawExtensionSurface::Tool),
            CapabilitySurfaceKind::Auth => Some(IronClawExtensionSurface::Auth),
            CapabilitySurfaceKind::Channel => Some(IronClawExtensionSurface::Channel {
                inbound: summary
                    .channel_directions
                ...
                binding_source: resolved_account_id
                    .as_ref()
                    .map(|_| IronClawAccountBindingSource::Default),
...
fn vendor_auth_accounts(
...
) -> Vec<IronClawVendorAuthAccounts> {
    ...
    vec![IronClawVendorAuthAccounts {
        ...
        accounts: vec![IronClawAuthAccount {
            account_id: vendor,
...
mod tests {
    ...
    use super::*;
    use crate::ironclaw_services::StaticChannelConnectionFacade;
    use crate::{
        ChannelConnectionFacade, ExtensionCredentialStatusRequest,
        ExtensionCredentialSubmitRequest, IronClawExtensionOnboardingState, IronClawServicesError,
        IronClawServicesErrorCode, IronClawServicesErrorKind,
    ...
    fn product_adapter_surface_projects_channel_kind() {
        ...
        assert!(
            entry
                .surfaces
                .iter()
                .any(|surface| matches!(surface, IronClawExtensionSurface::Channel { .. })),
            "{:?}",
            entry.surfaces
        );
    }
