...
//! (`ironclaw_composition`), which owns the provider catalog overlay,
...
use super::error::{IronClawServicesError, IronClawServicesErrorCode, IronClawServicesErrorKind};
use super::{
    IronClawServices, IronClawViewDescriptor, IronClawViewProvider, ProductCapabilityInvoker,
};
use crate::{WebUiAuthenticatedCaller, WebUiInboundValidationCode, WebUiInboundValidationError};

pub const LLM_CONFIG_VIEW: IronClawViewDescriptor = IronClawViewDescriptor {
    id: "llm_config",
    paginated: false,
};
...
/// price against and [`IronClawGetRunStateResponse::cost`] would be `None` even
...
///
/// [`IronClawGetRunStateResponse::cost`]: crate::IronClawGetRunStateResponse::cost
...
/// `IronClawServicesError` taxonomy; no backend strings, paths, or secrets cross
...
pub(super) fn map_llm_config_error(error: LlmConfigServiceError) -> IronClawServicesError {
    match error {
        LlmConfigServiceError::InvalidRequest { .. } => IronClawServicesError::from_status(
        ...
        LlmConfigServiceError::Unavailable => IronClawServicesError::service_unavailable(true),
        LlmConfigServiceError::Internal => IronClawServicesError::internal_invariant(),
    }
}

/// Error returned when an LLM-config method is invoked but no service is wired.
pub(super) fn llm_config_unavailable() -> IronClawServicesError {
...
fn llm_config_input_error(field: &'static str) -> IronClawServicesError {
