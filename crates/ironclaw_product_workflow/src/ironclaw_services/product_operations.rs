        ...
        V: IronClawViewProvider + Clone + 'static,
    {
        match self {
            ...
            Self::AutomationPause => {
                let request: IronClawAutomationRequest = product_command_input(input.into_json()?)?;
                services
            ...
            Self::AdminUserUpdate => {
                let request: IronClawAdminUpdateUserProductRequest =
                    product_command_input(input.into_json()?)?;
                services
                    .update_admin_user(
                        caller,
                        request.user_id,
                        IronClawAdminUpdateUserRequest {
            ...
            Self::AdminUserSetStatus => {
                let request: IronClawAdminSetStatusProductRequest =
                    product_command_input(input.into_json()?)?;
                services
            ...
            Self::AdminUserSetRole => {
                let request: IronClawAdminSetRoleProductRequest =
                    product_command_input(input.into_json()?)?;
                services
                    .set_admin_user_role(
                        caller,
                        request.user_id,
                        IronClawAdminSetRoleRequest { role: request.role },
                    )
                    .await?;
                Ok(())
            }
            Self::AdminUserDelete => {
                let request: IronClawAdminUserRequest = product_command_input(input.into_json()?)?;
                services.delete_admin_user(caller, request.user_id).await?;
                Ok(())
            }
            Self::AdminUserPutSecret => {
                let request: IronClawAdminPutSecretProductRequest =
                    product_command_input(input.into_json()?)?;
