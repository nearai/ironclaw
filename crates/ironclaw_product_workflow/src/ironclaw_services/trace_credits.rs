...
//! [`ironclaw_traces::contribution::trace_scope_key`]).
...
pub const TRACE_CREDITS_VIEW: ProductView<serde_json::Value, IronClawTraceCreditsResponse> =
    ProductView::unpaginated("trace_credits");

pub const TRACE_ACCOUNT_TRACES_VIEW: ProductView<serde_json::Value, IronClawAccountTracesResponse> =
...
pub struct IronClawTraceCreditsResponse {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub holds: Vec<IronClawTraceHold>,
...
pub struct IronClawAccountTrace {
    pub submission_id: String,
    pub status: String,
    pub pending_credit: f32,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// (same semantics as [`IronClawTraceCreditsResponse::enrolled`]).
...
/// any traces.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IronClawAccountTracesResponse {
    pub enrolled: bool,
    pub traces: Vec<IronClawAccountTrace>,
}

/// Result of authorizing a held trace for submission.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct IronClawTraceHoldAuthorizeResponse {
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct IronClawAccountLoginLinkResponse {
...
pub(super) async fn account_login_link_for_user(
    tenant_id: &TenantId,
    user_id: &UserId,
) -> Result<IronClawAccountLoginLinkResponse, AccountLoginLinkMintError> {
    match mint_account_login_link(tenant_id, user_id).await {
        Ok(link) => Ok(IronClawAccountLoginLinkResponse {
            minted: true,
            enrolled: true,
            url: Some(link.url),
        }),
        Err(AccountLoginLinkError::NotEnrolled) => Ok(IronClawAccountLoginLinkResponse {
            minted: false,
            enrolled: false,
            url: None,
        }),
...
pub(super) async fn account_traces_for_user(
    ...
    let traces = items
        ...
        .collect();
    Ok(IronClawAccountTracesResponse {
        enrolled: true,
        traces,
    })
}
    ...
    V: IronClawViewProvider + Clone + 'static,
{
    ...
    ) -> Result<IronClawTraceCreditsResponse, IronClawServicesError> {
        let actor = caller.actor();
        let scope = ironclaw_traces::contribution::trace_scope_key(
            caller.tenant_id.as_str(),
            actor.user_id.as_str(),
        );
        local_trace_credits_for_user(&scope).map_err(IronClawServicesError::internal_from)
    }

    pub(super) async fn build_trace_account_traces_view(
        &self,
        caller: WebUiAuthenticatedCaller,
    ) -> Result<IronClawAccountTracesResponse, IronClawServicesError> {
        let actor = caller.actor();
...
/// [`ironclaw_traces::contribution::trace_scope_key`]); the submission id
...
pub(super) fn local_trace_credits_for_user(
    scope: &str,
) -> Result<IronClawTraceCreditsResponse, String> {
    ...
    let holds: Vec<IronClawTraceHold> = view
        .manual_review_holds
        .into_iter()
        .map(|hold| IronClawTraceHold {
            submission_id: hold.submission_id.to_string(),
            reason: hold.reason,
        })
        .collect();
    Ok(IronClawTraceCreditsResponse {
        enrolled,
