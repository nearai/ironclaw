...
use crate::{
    ...
    LifecyclePackageRef, WebUiAuthenticatedCaller,
};
...
) -> Result<ExtensionCredentialReadiness, IronClawServicesError> {
...
) -> Result<RequirementCredentialReadiness, IronClawServicesError> {
...
fn credential_status_request(
    ...
    extension_id: &ExtensionId,
    requirement: &LifecycleExtensionCredentialRequirement,
) -> Result<ExtensionCredentialStatusRequest, IronClawServicesError> {
...
pub(super) fn provider_for_requirement(
    requirement: &LifecycleExtensionCredentialRequirement,
) -> Result<AuthProviderId, IronClawServicesError> {
    AuthProviderId::new(requirement.provider.as_str())
        .map_err(|_| IronClawServicesError::internal_invariant())
}

fn provider_scopes_for_requirement(
    requirement: &LifecycleExtensionCredentialRequirement,
) -> Result<Vec<ProviderScope>, IronClawServicesError> {
...
fn is_retryable_status_failure(error: &IronClawServicesError) -> bool {
    error.retryable
        && (error.code == IronClawServicesErrorCode::Unavailable
            || error.kind == IronClawServicesErrorKind::ServiceUnavailable)
}
...
fn warn_retryable_status_failure(
    ...
    tracing::warn!(
        target: "ironclaw::extension_credentials",
