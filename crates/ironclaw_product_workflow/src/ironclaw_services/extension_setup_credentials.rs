...
use crate::{
    IronClawExtensionCredentialSetup, IronClawExtensionSetupSecret, IronClawServicesError,
    LifecycleExtensionCredentialRequirement, LifecycleExtensionCredentialSetup,
    LifecycleProductPayload, LifecycleProductResponse, WebUiInboundValidationCode,
...
pub(super) async fn project(
...
) -> Result<Vec<IronClawExtensionSetupSecret>, IronClawServicesError> {
    ...
    for requirement in requirements {
        ...
        secrets.push(IronClawExtensionSetupSecret {
...
) -> Result<SetupSubmitPayload, IronClawServicesError> {
    let payload = request
        .payload
...
pub(super) async fn submit_manual_tokens(
    ...
    let service =
        extension_credentials.ok_or_else(|| IronClawServicesError::service_unavailable(true))?;
...
fn setup_projection(
    ...
    extension_id: &ExtensionId,
    requirement: &LifecycleExtensionCredentialRequirement,
) -> IronClawExtensionCredentialSetup {
    match &requirement.setup {
        LifecycleExtensionCredentialSetup::ManualToken => {
            IronClawExtensionCredentialSetup::ManualToken
        }
        LifecycleExtensionCredentialSetup::OAuth { scopes } => {
            IronClawExtensionCredentialSetup::OAuth {
                account_label: credential_label(extension_id, requirement),
                scopes: scopes.clone(),
                invocation_id: scope.resource.invocation_id.to_string(),
            }
        }
        LifecycleExtensionCredentialSetup::Pairing => IronClawExtensionCredentialSetup::Pairing,
    }
}
...
mod tests {
    ...
    use crate::{
        ExtensionCredentialStatusRequest, IronClawServicesErrorCode, IronClawServicesErrorKind,
    };
    ...
    async fn project_preserves_non_status_credential_errors() {
        ...
        assert_eq!(error.code, IronClawServicesErrorCode::InvalidRequest);
        assert_eq!(error.kind, IronClawServicesErrorKind::Validation);
    ...
    async fn project_preserves_non_retryable_unavailable_credential_errors() {
        ...
        assert_eq!(error.code, IronClawServicesErrorCode::Unavailable);
        assert_eq!(error.kind, IronClawServicesErrorKind::ServiceUnavailable);
    ...
    impl ExtensionCredentialSetupService for FailingCredentialSetupService {
        ...
        ) -> Result<Option<CredentialAccountProjection>, IronClawServicesError> {
        ...
        ) -> Result<CredentialAccountId, IronClawServicesError> {
