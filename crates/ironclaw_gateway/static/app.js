...
function createMessageElement(role, content, options = {}) {
  ...
  let userAttachments = [];
  if (role === 'user') {
    const parsed = options.attachments
      ? {
          text: content,
          attachments: options.attachments,
          copyText: options.copyText || content,
        }
    ...
    options.copyText = options.copyText || parsed.copyText;
  } else if (role === 'system') {
  ...
  if (role === 'user') {
    renderMessageAttachments(div, userAttachments);
  }

  if (role === 'assistant' || role === 'user') {
    div.classList.add('has-copy');
    div.setAttribute('data-copy-text', options.copyText || content);
...
IronClaw.api = {
   ...
   */
  share: function(opts) {
    if (!opts || !opts.imageDataUrl) return;
    ...
    if (!overlay) {
      ...
      overlay.innerHTML =
        ...
        '    <span class="share-title">Share your gains</span>' +
        ...
        '    </button>' +
        '    <button class="share-btn share-copy" title="Copy image">' +
        ...
        '    </button>' +
        '    <button class="share-btn share-download" title="Download image">' +
