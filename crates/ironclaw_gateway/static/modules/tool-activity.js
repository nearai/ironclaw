...
function createToolLines(tools) {
  ...
  container.className = 'tool-lines';
  ...
  (tools || []).forEach(function(tool) {
    ...
    dot.className = 'tool-line__dot';
    if (tool.status === 'ok' || tool.status === 'completed') dot.className += ' is-ok';
    else if (tool.status === 'running') dot.className += ' is-run';
    else if (tool.status === 'warn') dot.className += ' is-warn';
    else if (tool.status === 'error' || tool.status === 'failed') dot.className += ' is-err';
    line.appendChild(dot);

    var name = document.createElement('span');
    name.className = 'tool-line__name';
    name.textContent = humanizeToolName(tool.name);
    line.appendChild(name);

    var args = document.createElement('span');
    args.className = 'tool-line__args';
    if (tool.args) {
    ...
    meta.className = 'tool-line__meta';
    if (tool.duration_ms) meta.textContent = (tool.duration_ms / 1000).toFixed(1) + 's';
...
function syntaxHighlightArgs(argsStr) {
  if (!argsStr) return '';
  var safe = escapeHtml(String(argsStr));
  // Highlight keys (word:)
  safe = safe.replace(/(\w+):/g, '<span class="k">$1:</span>');
  // Highlight quoted strings
  safe = safe.replace(/"([^"]*?)"/g, '"<span class="s">$1</span>"');
  // Highlight numbers
  safe = safe.replace(/\b(\d+(?:\.\d+)?)\b/g, '<span class="n">$1</span>');
  return safe;
}
