...
// Replaces the raw `mission_create` tool-output JSON with a styled inline card.
...
function renderMissionCreatedCard(data) {
  ...
  badge.className = 'mission-card-badge mission-status-' + statusKey;
  ...
  function appendDetail(labelText, valueText) {
    const row = document.createElement('div');
    row.className = 'mission-card-detail';
  ...
  if (data.mission_id) {
    appendDetail('ID', data.mission_id.substring(0, 8));
  }
  if (data.cadence) {
    appendDetail(I18n.t('missions.cadence'), data.cadence);
  }
  if (data.project_id) {
    appendDetail(I18n.t('missions.project'), data.project_id.substring(0, 8));
  }
