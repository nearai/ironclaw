...
// `i18n.ts` imports React/html from `./html.ts` (a browser-only module),
// so it cannot be imported directly under node. Follow the crate's
// established vm-context harness (see pages/chat/lib/chat-input.test.mts):
...
function loadI18n() {
  let source = readFileSync(new URL("./i18n.ts", import.meta.url), "utf8");
  source = source
    .split("\n")
    .filter((line) => !line.startsWith("import "))
    .join("\n");
  // The real locale loaders call dynamic `import("../i18n/<lang>.ts")`,
