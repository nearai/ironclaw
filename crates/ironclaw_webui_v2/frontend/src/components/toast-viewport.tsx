...
import hotToast, { resolveValue, Toaster, type Toast } from "react-hot-toast";
...
const ICON = { blank: "bolt", success: "check", error: "close" };
  ...
  return (
    <Toaster position="bottom-right" gutter={8} containerStyle={{ zIndex: 10000 }}>
      {(item: Toast) => (
        ...
        >
          <Icon name={ICON[item.type] || "bolt"} className="h-4 w-4 shrink-0" />
