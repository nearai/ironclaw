...
function FeaturedProviderRow({ entry, provider, configured, isBusy, login, t, onUse, onSetUp }) {
  ...
  if (entry.auth === "nearai") {
    actions = (<NearAiSetupMenu provider={provider} isBusy={isBusy} login={login} t={t} onSetUp={onSetUp} />);
    ...
    actions = (<Button type="button" variant="primary" size="sm" disabled={isBusy} onClick={() => onUse(provider)}>
      {t("llm.use")}
    </Button>);
  } else {
