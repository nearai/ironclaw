...
import { groupProvidersByStatus } from "../lib/llm-providers.ts";
...
function renderProviderManagement({ providers, activeProviderId = "nearai", searchQuery = "" }) {
  ...
  vm.runInNewContext(
    sourceForTest("./provider-management.ts", ["ProviderManagement"]),
    context
  );
...
function evalIsLocalDevOrigin({ hostname }) {
  ...
  vm.runInNewContext(
    sourceForTest("../hooks/useProviderLogin.ts", ["isLocalDevOrigin"]),
    context
  );
...
function createProviderCardHarness() {
  ...
  vm.runInNewContext(
    sourceForTest("./provider-card.ts", ["ProviderCard"]),
    context
  );
...
function createNearAiSetupMenuHarness() {
  ...
  vm.runInNewContext(
    sourceForTest("../../onboarding/onboarding-page.ts", ["NearAiSetupMenu"]),
    context
  );
...
function runProviderLogin({ hostname, activeProviderId = null, popupClosed = false }) {
  ...
  vm.runInNewContext(
    sourceForTest("../hooks/useProviderLogin.ts", ["useProviderLogin"]),
    context
  );
