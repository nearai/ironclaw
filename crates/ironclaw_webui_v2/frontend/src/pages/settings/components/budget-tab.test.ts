...
// Extract the pure formatters from budget-tab.tsx: strip the imports (the vm
// context has no module loader) and expose formatUsd/formatPercent. The
// crate's vm harness transpiles the file's JSX, but only the formatters run at
// module top level, so no design-system stubs are required.
function budgetTabHelpers() {
  const source = readFileSync(new URL("./budget-tab.tsx", import.meta.url), "utf8");
  const lines = [];
