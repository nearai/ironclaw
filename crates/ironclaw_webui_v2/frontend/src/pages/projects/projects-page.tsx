...
export function ProjectsPage() {
  ...
  const headerActions = (
    <>
      {projectId && (<Button variant="ghost" onClick={() => navigate("/projects")}>{t("projects.allProjects")}</Button>)}
    </>
  );
  ...
  if (projectId) {
    if (workspaceState.isLoading) {
      ...
      content = (
        ...
        >
          <Button variant="secondary" onClick={() => navigate("/projects")}>{t("projects.returnToProjects")}</Button>
        </EmptyPanel>
      );
    } else {
      content = (
