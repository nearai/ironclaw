    ...
    return (
      <div className="flex">
        <div className="flex flex-wrap gap-2">
          {imgs.map((img, i) =>
            img.data_url
              ? (<img key={i} src={img.data_url} className="max-h-64 rounded-lg border border-iron-700 object-cover" alt="Generated result" />)
              : (
        ...
        >
          ...
          {images && images.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {images.map((src, i) => (<img key={i} src={src} className="max-h-48 rounded-lg border border-iron-700 object-cover" alt="Message attachment" />))}
            </div>
          )}
