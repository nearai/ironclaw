...
export function RunDots({ runs = [] }) {
  ...
  return (
    ...
    >
        ...
        return (
          <span
            ...
            className={cn(
              ...
              // In-progress runs are live/healthy green and pulse so work in
              // flight is obvious at a glance.
              run.status === "running" &&
                "bg-[var(--v2-positive-text)] ring-[color-mix(in_srgb,var(--v2-positive-text)_60%,transparent)] animate-pulse",
              run.status === "unknown" &&
      ...
      {overflow > 0 && (
        <span
          className="ml-1 inline-flex h-5 items-center rounded-full bg-[var(--v2-surface-soft)] px-1.5 font-mono text-[10px] tabular-nums text-[var(--v2-text-faint)]"
          title={t("automations.runs.showingOf", {
            shown: visibleRuns.length,
            total: list.length,
          })}
        >
...
export function RunHistorySummary({ runs = [], className = "" }) {
  ...
  if (!view.total) {
    return (
      <span className={cn("text-[11px] text-[var(--v2-text-faint)]", className)}>
        {t("automations.table.noRuns")}
      </span>
    );
  }
  ...
  return (
    <div
      className={cn(
        "w-full rounded-[var(--v2-radius-md)] border border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] p-4",
        className
      )}
    >
      <div className="flex items-baseline justify-between gap-3">
        <div className="flex items-baseline gap-1.5">
          <span className="text-lg font-semibold tabular-nums leading-none text-[var(--v2-text-strong)]">
            {view.total}
          </span>
...
// running shares the healthy green of success (live activity) per review.
const RUN_STATUS_TEXT = {
  success: "text-[var(--v2-positive-text)]",
  signal: "text-[var(--v2-positive-text)]",
...
const RUN_STATUS_DOT = {
  ...
  signal: "bg-[var(--v2-positive-text)] animate-pulse",
