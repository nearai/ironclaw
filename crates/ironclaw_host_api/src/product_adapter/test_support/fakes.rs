...
use crate::product_adapter::{
    ...
    OutboundDeliverySink, ProductAdapterError, ProductInboundAck, ProductInboundEnvelope,
    ProductOutboundEnvelope, ProductRejection, ProjectionCursor, ProjectionStream,
    ProjectionSubscriptionRequest, ProtocolHttpEgress, ProtocolHttpEgressError,
};
...
pub fn assert_no_raw_attachment_bytes(envelopes: &[ProductInboundEnvelope]) {
    for envelope in envelopes {
        if let crate::product_adapter::inbound::ProductInboundPayload::UserMessage(payload) =
            envelope.payload()
        {
...
pub fn fake_rejection(
    kind: crate::product_adapter::inbound::ProductRejectionKind,
    reason: &str,
) -> ProductRejection {
