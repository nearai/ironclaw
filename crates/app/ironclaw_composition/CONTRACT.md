...
| `RebornRuntime::voice_input_enabled()` | Whether a transcription provider resolved from the deployment's LLM config (`support::transcription::build_transcription_provider` — NEAR AI backend with an API key, model from `IRONCLAW_TRANSCRIPTION_MODEL`). The serve path reads it into `WebuiServeConfig::with_voice_input_enabled` so the composer's microphone button and the transcribe route agree; the route itself stays mounted and fails closed when it is false |
