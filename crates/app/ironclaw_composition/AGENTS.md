...
- LLM catalog wiring is **not** owned here — `llm_catalog` belongs to `ironclaw_operator` (`crates/product/ironclaw_operator/src/llm_admin/llm_catalog.rs`); this crate's `src/llm_admin/` holds only `nearai_login_serve`, `nearai_mcp`, and `openai_compat_serve`.
