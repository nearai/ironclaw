...
fn reborn_contracts_crates_carry_a_checked_size_ceiling() {
    ...
    const SIZE_CEILINGS: &[(&str, usize)] = &[
        ...
        // failure message, never counted by eye.
        // Union re-measured 20_678 -> 20_745 on 2026-08-24 after merging
        // #7812 into the profile-stable storage branch. The two changes touch
        // disjoint neutral declarations (`ids.rs`/`path.rs` versus
        // `prepared_context.rs`/`capability_surface.rs`); this is the merged
        // tree's exact report, not a sum of the two historical pins.
        ("ironclaw_host_api", 20_745),
