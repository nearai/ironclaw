    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidProfile { name, value } => write!(
                formatter,
                "{name} must be one of local-dev, local-dev-yolo, hosted-single-tenant, hosted-single-tenant-volume, hosted-single-tenant-volume-sandboxed, hosted-single-tenant-volume-sandboxed-railway, production, migration-dry-run; got {value:?}"
            ),
        }
    }
}
