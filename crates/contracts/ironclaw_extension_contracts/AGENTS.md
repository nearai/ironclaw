...
minus one for the feature-gated `test_support` module — 21 at the time of
writing, the figure `README.md` states (no architecture test currently pins
this count against the table below — treat the table as a map, not an
enforced inventory, until one exists):
...
| `channel_adapter` | The two channel capability traits — `ChannelIngress` and `ChannelDelivery` — plus `ChannelSurfaces` (whose `reply` slot holds the `reply::ReplySink` a declared `[channel.reply]` binds) and their DTO family (`VerifiedInbound`, `InboundOutcome`, `NormalizedInboundMessage`/`InboundAttachment`, `OutboundEnvelope`/`Part`/`Target`, `DeliveryReport`, `DirectTargetProvisionRequest`, `ChannelError`, `ProductTriggerReason`). `ChannelAttachmentRef` is package-internal parse→fetch state, never host ingress vocabulary. The traits pair with vendor `[channel.ingress]` and `[channel.delivery]`; authenticated-session ingress stays host-owned with no adapter half. Direct-target provisioning is an optional typed operation on `ChannelDelivery`, not a target-search grammar. |
...
| `memory` | The `[memory]` manifest surface: `MemoryDescriptor`, `MemoryLifecycleHook`, and the `[[memory.scheduled_ops]]` declaration family (`MemoryScheduledOp`, `MemoryScheduledTrigger`, `MemoryScheduledOpKind`, `MemoryScheduledPass`, `MIN_SCHEDULED_OP_INTERVAL_TURNS`, `MAX_SCHEDULED_PASS_MODEL_CALLS`). Same split as `channel`: shape and section-local invariants here (closed trigger vocabulary, exactly-one op kind, cost floor, model-call ceiling, one op per trigger); the two cross-section rules — a pass's tools must be a subset of the manifest's own `[[tools]]`, and only a first-party/system manifest may declare a pass op — are `ironclaw_extension_registry::v3::validate_memory_scheduled_ops`, because only the manifest layer sees the whole file. Asset resolution for `pass.prompt` stays host-side and fail-closed, like `guidance_doc`. |
...
| `reply` | The channel-neutral reply seam a declared `[channel.reply]` binds, `stream` or `message` alike (the deployment's authenticated-session channel gets the host's projection sink; every vendor package binds its own): `ReplySink::reconcile`; `ReplyDocument` (desired state, evolved only through its bounded semantic mutators) and `ReplyRevision`; the bounded display/answer/reasoning newtypes (`ReplyDisplayText`, `ReplyDisplayPreview`, `ReplyAnswerText`, `ReplyReasoningText`, `ReplyThreadAnchor` — `bounded_text!` over the `REPLY_*_MAX_BYTES` constants); `ReplyReconcileRequest`/`ReplySinkReport` (the sink's checkpoint + provider evidence, bounded by `REPLY_SINK_CHECKPOINT_MAX_BYTES`, `REPLY_PROVIDER_REF_MAX_BYTES`, `REPLY_MAX_PROVIDER_REFS`); `ReplyReconcilePoint` and the cadence rule `ReplyTransport::reconciles_at` (`ReplyTransport` itself is a `channel` descriptor). Arrived 2026-08-31 with progressive reply publication (`docs/internal/design/2026-08-31-progressive-reply-publication.md`); re-verify with `rg -n "^pub (struct|enum|trait|const) " crates/contracts/ironclaw_extension_contracts/src/reply.rs`. |
