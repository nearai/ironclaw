...
| `channel_adapter` | The two channel capability traits — `ChannelIngress` and `ChannelDelivery` — plus `ChannelSurfaces` (whose `reply` slot holds the `reply::ReplySink` a declared `[channel.reply]` binds) and their DTO family (`VerifiedInbound`, `InboundOutcome`, `NormalizedInboundMessage`/`InboundAttachment`, `OutboundEnvelope`/`Part`/`Target`, `DeliveryReport`, `DirectTargetProvisionRequest`, `ChannelError`, `ProductTriggerReason`). `ChannelAttachmentRef` is package-internal parse→fetch state, never host ingress vocabulary. The traits pair with vendor `[channel.ingress]` and `[channel.delivery]`; `[channel.reply]` (stream or message) binds a `reply::ReplySink` — the deployment's authenticated-session channel gets the host's projection sink, every vendor package binds its own — and authenticated-session ingress stays host-owned with no adapter half. The `reply` module is the channel-neutral reply vocabulary: `ReplyDocument` (desired state, evolved only through its bounded semantic mutators), `ReplyRevision`, the bounded display/answer/reasoning newtypes, `ReplyReconcileRequest`/`ReplySinkReport` (checkpoint + provider evidence), and the cadence rule `ReplyTransport::reconciles_at`. Direct-target provisioning is an optional typed operation on `ChannelDelivery`, not a target-search grammar. |
