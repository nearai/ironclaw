...
pub struct ThreadMessage {
    ...
    pub content: String,
    /// Transient multimodal parts for the current in-memory LLM request.
    ///
    /// Image attachments can be large data URLs. The durable message content
    /// already carries attachment metadata and project paths, so these parts
    /// must not be serialized into persisted thread history.
    #[serde(skip)]
