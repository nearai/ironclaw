    ...
    L: SigningLedger,
{
        ...
        S: CustodialSignerLike,
    {
        ...
        match self.ledger.create(&LedgerKey::new(binding.context.tenant.clone(), gate_ref.clone())).await {
            Ok(()) => {}
            Err(ironclaw_attestation::LedgerError::AlreadyExists) => {
                ...
                let state = self.ledger.state(&LedgerKey::new(binding.context.tenant.clone(), gate_ref.clone())).await?;
