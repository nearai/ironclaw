-- Frozen snapshot of the v1 legacy libSQL schema's consolidated `SCHEMA`
-- constant (formerly `src/db/libsql_migrations.rs`, deleted under Tier B —
-- see docs/plans/2026-07-02-reborn-internal-module-refactor.md §8).
--
-- Test-fixture-only: `migration_roundtrip.rs` executes this batch to build a
-- v1 database shaped exactly like a real (pre-Tier-B) install, then seeds
-- rows into it via raw SQL, so the migration's frozen read path
-- (`src/legacy_snapshot/`) has something realistic to read. Do not "clean up"
-- or diverge this from what a real v1 database looked like — fidelity to the
-- original schema is the entire point.
