...
pub async fn verify_migration(
    ...
    match manifest.status {
        ...
        MigrationStatus::Applied | MigrationStatus::Verifying => {
            ...
            verifying.transition(MigrationStatus::Verified)
        }
...
async fn validate_manifest_source(
    ...
    if source_descriptor(&options.source) != manifest.source
        || target_descriptor(&options.target).locator_fingerprint
            != manifest.target.locator_fingerprint
    {
        return Err(MigrationError::InvalidInput(
            "migration source or target does not match the sealed plan".to_string(),
        ));
    }
    let source = source::V1Source::open(&options.source).await?;
...
async fn collect_inventory(
    ...
    let (source_home, source_path) = match &options.source {
        SourceDb::LibSql { path } => (path.parent(), Some(path.as_path())),
        SourceDb::Postgres { .. } => (None, None),
    };
