...
async fn production_profile_rejects_in_memory_before_returning_service_graph() {
    let result = build_ironclaw_event_stores(
        IronClawProfile::Production,
        IronClawEventStoreConfig::InMemory,
    )
    .await;
    ...
    assert!(matches!(
        error,
        IronClawEventStoreError::ProductionInMemoryDisabled
    ));
...
async fn local_and_test_profiles_allow_explicit_in_memory_stores() {
    for profile in [IronClawProfile::LocalDev, IronClawProfile::Test] {
        let stores = build_ironclaw_event_stores(profile, IronClawEventStoreConfig::InMemory)
            .await
...
async fn production_jsonl_requires_explicit_single_node_acceptance_without_leaking_root() {
    ...
    assert!(matches!(
        error,
        IronClawEventStoreError::ProductionJsonlRequiresAcceptance
    ));
...
async fn production_postgres_rejects_remote_sslmode_disable_before_connecting() {
    ...
    assert!(matches!(
        error,
        IronClawEventStoreError::RemotePostgresClearTextDisabled
    ));
    let displayed = error.to_string();
...
async fn postgres_connection_failure_does_not_fall_back_or_leak_secret_config() {
    ...
    assert!(
        !matches!(
            error,
            IronClawEventStoreError::BackendUnavailable {
                backend: "postgres"
            }
        ),
