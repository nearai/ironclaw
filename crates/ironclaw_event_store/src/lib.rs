//! IronClaw-owned durable event and audit store backends.
//!
//! This crate is the production-composition side of the IronClaw event
...
//! `Postgres` variants of [`IronClawEventStoreConfig`] open a backend-specific
...
pub struct PostgresPoolTlsOptions {
    pub ssl_mode_override: Option<IronClawPostgresSslMode>,
    pub allow_remote_cleartext: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IronClawPostgresSslMode {
    Disable,
    Prefer,
    Require,
}

impl std::str::FromStr for IronClawPostgresSslMode {
    type Err = String;
...
pub fn open_postgres_pool(
    url: SecretString,
) -> Result<deadpool_postgres::Pool, IronClawEventStoreError> {
...
/// Backend configuration for IronClaw durable event/audit stores.
///
...
#[derive(Debug)]
pub enum IronClawEventStoreConfig {
...
/// IronClaw composition profile controlling which fallbacks are legal.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Durable event and audit log handles consumed by IronClaw composition.
#[derive(Clone)]
pub struct IronClawEventStores {
...
#[derive(Debug, Error)]
pub enum IronClawEventStoreError {
    #[error("production IronClaw event store cannot use in-memory storage")]
    ...
    #[error("production IronClaw event store cannot use cleartext http:// libSQL URL")]
    ProductionLibsqlClearTextDisabled,
    #[error(
        "production IronClaw libSQL event store requires an explicit local path or remote URL scheme"
    )]
    ProductionLibsqlAmbiguousTarget,
    #[error(
        "remote IronClaw Postgres event store requires sslmode=require unless remote cleartext is explicitly allowed (sslmode=disable rejected)"
    )]
    RemotePostgresClearTextDisabled,
    #[error("IronClaw Postgres pool max_size must be greater than 0")]
    InvalidPostgresPoolMaxSize,
    #[error("{backend} IronClaw event store backend is not enabled in this build")]
    BackendUnavailable { backend: &'static str },
    #[error("{backend} IronClaw event store failed during {operation}")]
    BackendOperation {
        backend: &'static str,
        operation: &'static str,
    },
    #[error("IronClaw event store I/O failed during {operation}")]
    Io {
        operation: &'static str,
        #[source]
        source: std::io::Error,
...
/// Build durable event and audit logs for a standalone IronClaw composition path.
pub async fn build_ironclaw_event_stores(
    profile: IronClawProfile,
    config: IronClawEventStoreConfig,
) -> Result<IronClawEventStores, IronClawEventStoreError> {
    match config {
        IronClawEventStoreConfig::InMemory => {
            if profile == IronClawProfile::Production {
                return Err(IronClawEventStoreError::ProductionInMemoryDisabled);
            }
            Ok(IronClawEventStores {
        ...
        IronClawEventStoreConfig::Jsonl {
            ...
            if profile == IronClawProfile::Production && !accept_single_node_durable {
                return Err(IronClawEventStoreError::ProductionJsonlRequiresAcceptance);
            }
            create_secure_dir_all(&root)
                .await
                .map_err(|source| IronClawEventStoreError::io("initialize jsonl root", source))?;
        ...
        IronClawEventStoreConfig::Postgres { url, tls_options } => {
            postgres_backed::build(url, tls_options).await
        }
        IronClawEventStoreConfig::PostgresPool { pool } => {
...
/// Build a [`IronClawEventStores`] from any [`RootFilesystem`] by routing the
...
pub fn build_ironclaw_event_stores_from_root_filesystem<F>(
    root: Arc<F>,
) -> Result<IronClawEventStores, IronClawEventStoreError>
where
...
fn build_events_scoped_filesystem<F>(
    root: Arc<F>,
) -> Result<Arc<ScopedFilesystem<F>>, IronClawEventStoreError>
where
    F: RootFilesystem + Send + Sync + 'static,
{
    let alias =
        MountAlias::new("/events").map_err(|_| IronClawEventStoreError::BackendOperation {
            backend: "filesystem",
            operation: "construct events mount alias",
        })?;
    let target =
        VirtualPath::new("/events").map_err(|_| IronClawEventStoreError::BackendOperation {
            backend: "filesystem",
    ...
    .map_err(|_| IronClawEventStoreError::BackendOperation {
        backend: "filesystem",
...
fn validate_production_libsql_target(path_or_url: &str) -> Result<(), IronClawEventStoreError> {
    match classify_libsql_target(path_or_url) {
        LibsqlTargetClass::RemoteCleartext => {
            Err(IronClawEventStoreError::ProductionLibsqlClearTextDisabled)
        }
        LibsqlTargetClass::InMemory => Err(IronClawEventStoreError::ProductionInMemoryDisabled),
        LibsqlTargetClass::Bare => Err(IronClawEventStoreError::ProductionLibsqlAmbiguousTarget),
...
mod libsql_backed {
    ...
    use super::{
        IronClawEventStoreError, IronClawEventStores, wrap_root_filesystem_as_event_stores,
    };
    ...
    pub(super) async fn build(
        ...
        filesystem.run_migrations().await.map_err(|source| {
            IronClawEventStoreError::backend("libsql", "run migrations", source)
        })?;
    ...
    async fn build_database(
        path_or_url: &str,
        auth_token: Option<SecretString>,
    ) -> Result<Arc<libsql::Database>, IronClawEventStoreError> {
        let db = if is_remote_libsql(path_or_url) {
                ...
                && !parent.as_os_str().is_empty()
            {
                tokio::fs::create_dir_all(parent).await.map_err(|source| {
                    IronClawEventStoreError::io("initialize libsql parent", source)
                })?;
            }
            libsql::Builder::new_local(path_or_url).build().await
        };
        db.map(Arc::new)
            .map_err(|source| IronClawEventStoreError::backend("libsql", "connect", source))
    }
...
mod postgres_backed {
    ...
    use super::{
        IronClawEventStoreError, IronClawEventStores, IronClawPostgresSslMode,
        PostgresPoolTlsOptions, wrap_root_filesystem_as_event_stores,
    };
    ...
    pub(super) async fn build_from_pool(
        ...
        filesystem.run_migrations().await.map_err(|source| {
            IronClawEventStoreError::backend("postgres", "run migrations", source)
        })?;
        wrap_root_filesystem_as_event_stores(filesystem)
    }
    ...
    pub(super) fn build_pool(
        ...
        max_size: usize,
        tls_options: PostgresPoolTlsOptions,
    ) -> Result<Pool, IronClawEventStoreError> {
        if max_size == 0 {
            return Err(IronClawEventStoreError::InvalidPostgresPoolMaxSize);
        }
        let raw_url = url.expose_secret();
        let mut pg_config: Config = raw_url.parse().map_err(|source| {
            IronClawEventStoreError::backend("postgres", "parse connection string", source)
        })?;
        ...
        let manager = if remote_cleartext {
            if !tls_options.allow_remote_cleartext {
                return Err(IronClawEventStoreError::RemotePostgresClearTextDisabled);
            }
            tracing::warn!(
                target = "ironclaw::event_store::postgres",
                "remote IronClaw Postgres cleartext connection explicitly allowed; use only on a trusted private network"
            );
        ...
        Pool::builder(manager)
            ...
            .build()
            .map_err(|source| IronClawEventStoreError::backend("postgres", "build pool", source))
    }
    ...
    fn enforce_remote_ssl_mode(config: &mut Config) -> Result<(), IronClawEventStoreError> {
        match config.get_ssl_mode() {
            SslMode::Disable => Err(IronClawEventStoreError::RemotePostgresClearTextDisabled),
            SslMode::Prefer => {
    ...
    impl From<IronClawPostgresSslMode> for SslMode {
        fn from(value: IronClawPostgresSslMode) -> Self {
            match value {
                IronClawPostgresSslMode::Disable => SslMode::Disable,
                IronClawPostgresSslMode::Prefer => SslMode::Prefer,
                IronClawPostgresSslMode::Require => SslMode::Require,
            }
        }
    }
    ...
    /// store is empty.
    fn make_rustls_connector() -> Result<MakeRustlsConnect, IronClawEventStoreError> {
        ...
        .map_err(|source| IronClawEventStoreError::backend("postgres", "configure rustls", source))?
    ...
    mod tests {
        ...
        use crate::{IronClawEventStoreError, IronClawPostgresSslMode, PostgresPoolTlsOptions};
        use secrecy::SecretString;
        ...
        fn sslmode_aliases_parse_to_internal_modes() {
            for value in ["allow", "prefer"] {
                assert_eq!(
                    value.parse::<IronClawPostgresSslMode>().expect("parse"),
                    IronClawPostgresSslMode::Prefer,
            ...
            for value in ["require", "verify-ca", "verify-full"] {
                assert_eq!(
                    value.parse::<IronClawPostgresSslMode>().expect("parse"),
                    IronClawPostgresSslMode::Require,
...
impl JsonlDurableEventLog {
    ...
    // [`build_ironclaw_event_stores`] so the single-node-durable acceptance
...
mod tests {
    ...
    async fn production_rejects_in_memory_libsql_target() {
        ...
        assert!(matches!(
            result,
            Err(IronClawEventStoreError::ProductionInMemoryDisabled)
        ));
    }
    ...
    async fn production_accepts_mixed_case_secure_libsql_scheme() {
        ...
        for url in ["HTTPS://example.invalid", "LibSQL://example.invalid"] {
            ...
            match result {
                Err(IronClawEventStoreError::ProductionInMemoryDisabled)
                | Err(IronClawEventStoreError::ProductionLibsqlClearTextDisabled)
                | Err(IronClawEventStoreError::ProductionLibsqlAmbiguousTarget) => {
