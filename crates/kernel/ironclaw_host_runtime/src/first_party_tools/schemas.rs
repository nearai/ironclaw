...
mod tests {
    ...
    fn trigger_create_prompt_description_scopes_web_app_no_delivery_to_unnamed_destinations() {
        ...
        assert!(
            ...
            ) && !description.contains("unavailable to scheduled automations"),
            "the prompt schema must allow future scheduled loop-runs to use the owning user's linked integrations: {description}"
        );
    }
