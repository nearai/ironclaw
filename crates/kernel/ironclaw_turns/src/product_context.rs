//! Product turn-origin resolver for ingress boundaries.

use crate::{ProductTurnContext, RunOriginAdapter, TurnOriginKind, TurnOwner, TurnSurfaceType};

pub const WEBUI_SOURCE_CHANNEL: &str = "webui";
pub const CLI_SOURCE_CHANNEL: &str = "cli";

/// Ingress classification. Callers collapse their trust-policy and
/// trigger-adapter signal into one value so the resolver cannot receive a
/// contradictory pair.
...
/// Resolve a WebUI submission. Always `WebUi`, no adapter/surface, source
/// channel `webui`.
...
mod tests {
    ...
    fn trusted_non_trigger_adapter_yields_inbound() {
        let adapter = RunOriginAdapter::new("telegram").unwrap();
        let ctx = resolve_inbound(
