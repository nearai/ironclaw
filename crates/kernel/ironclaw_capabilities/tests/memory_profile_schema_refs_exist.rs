...
//! `docs/internal/reborn/contracts/memory-profiles.md` resolve to real files on disk.
//!
...
fn memory_profile_schema_refs_exist_on_disk() {
    ...
    let contracts_dir = manifest_dir
        .ancestors()
        .map(|ancestor| ancestor.join("docs/internal/reborn/contracts"))
        .find(|candidate| candidate.is_dir())
        .expect("could not locate docs/internal/reborn/contracts from crate manifest dir");
