...
fn sheet_parts(package: &OoxmlPackage) -> Result<Vec<(String, String)>, DocumentError> {
    ...
    if package.has_part(WORKBOOK_RELS_PART) {
        let rels = package.part_str(WORKBOOK_RELS_PART)?;
        ...
        for (name, rid) in &names {
            let target = targets
                ...
                .ok_or_else(|| DocumentError::MalformedPart {
                    part: WORKBOOK_RELS_PART.to_string(),
                    detail: format!("sheet {name:?} references missing relationship {rid:?}"),
                })?;
            let part = format!(
                "xl/{}",
            ...
            Event::End(tag) => match local_name(tag.name().as_ref()) {
                b"c" if open => {
                    ...
                    let resolved = if is_shared {
                        let index = value.trim().parse::<usize>().map_err(|error| {
                            DocumentError::MalformedPart {
                                part: "worksheet".to_string(),
                                detail: format!(
                                    "cell {reference} has invalid shared-string index {value:?}: {error}"
                                ),
                            }
                        })?;
                        Some(shared.get(index).cloned().ok_or_else(|| {
                            DocumentError::MalformedPart {
                                part: "worksheet".to_string(),
                                detail: format!(
                                    "cell {reference} references missing shared-string index {index}"
                                ),
                            }
                        })?)
                    ...
                    b"c" if in_target_row && !wrote => {
                        ...
                        let Some(index) = column_index(&column) else {
                            return Err(DocumentError::MalformedPart {
                                part: "worksheet".to_string(),
                                detail: format!(
                                    "cell reference {reference:?} has an invalid column"
                                ),
                            });
                        };
...
fn column_index(column: &str) -> Option<u32> {
    column
        .bytes()
        ...
        match event {
            ...
            Event::Start(ref tag) | Event::Empty(ref tag) => {
                if local_name(tag.name().as_ref()) == b"Relationship"
                    ...
                    && out.insert(id.clone(), target).is_some()
                {
                    return Err(DocumentError::MalformedPart {
                        part: WORKBOOK_RELS_PART.to_string(),
...
fn parse_cell_reference(reference: &str) -> Result<(String, u32, u32), DocumentError> {
    let column = column_of(reference).ok_or_else(|| DocumentError::UnknownAddress {
        address: reference.to_string(),
    })?;
    let row = row_of(reference).ok_or_else(|| DocumentError::UnknownAddress {
        address: reference.to_string(),
    })?;
    let expected = format!("{column}{row}");
    let column_index = column_index(&column).filter(|index| *index <= 16_384);
    if row == 0 || row > 1_048_576 || expected != reference.to_ascii_uppercase() {
        return Err(DocumentError::UnknownAddress {
            address: reference.to_string(),
        });
    }
    let column_index = column_index.ok_or_else(|| DocumentError::UnknownAddress {
        address: reference.to_string(),
    })?;
    Ok((column, column_index, row))
}

fn reject_grouped_formula_target(xml: &str, target: &str) -> Result<(), DocumentError> {
    let mut reader = quick_xml::Reader::from_str(xml);
    ...
    loop {
        ...
        match event {
            Event::Eof => return Ok(()),
            Event::Start(ref tag) if local_name(tag.name().as_ref()) == b"c" => {
                in_target = attribute(tag, b"r").as_deref() == Some(target);
            }
            Event::End(ref tag) if local_name(tag.name().as_ref()) == b"c" => {
                in_target = false;
            }
            Event::Start(ref tag) | Event::Empty(ref tag)
                if in_target && local_name(tag.name().as_ref()) == b"f" =>
            {
                if let Some(kind) = attribute(tag, b"t")
                    && matches!(kind.as_str(), "shared" | "array" | "dataTable")
                {
                    return Err(DocumentError::InapplicableEdit {
                        address: target.to_string(),
                        detail: format!("cannot replace a {kind} formula independently"),
                    });
                }
            }
            _ => {}
...
mod review_regressions {
    ...
    #[test]
    fn phonetic_shared_string_runs_are_not_display_text() {
        let shared = r#"<sst><si><t>Amount</t><rPh><t>アマウント</t></rPh></si></sst>"#;
        let package = OoxmlPackage::read(&crate::test_fixtures::package(&[(
            SHARED_STRINGS_PART,
            shared.as_bytes().to_vec(),
        )]))
        .unwrap();
        assert_eq!(read_shared_strings(&package).unwrap(), vec!["Amount"]);
    }

    #[test]
    fn malformed_shared_string_indexes_fail_loudly() {
        let error = parse_cells(
            r#"<worksheet><c r="A1" t="s"><v>nope</v></c></worksheet>"#,
            &["ok".to_string()],
        )
        .unwrap_err();
        assert!(matches!(error, DocumentError::MalformedPart { .. }));
    }

    #[test]
    fn cell_references_are_bounded_to_excel_limits() {
        for reference in ["C0", "C1048577", "XFE1", "AAAAAAAAAAAAAAAAAAAA1"] {
            let error = edit_xlsx(
                &expenses_xlsx(),
                &[XlsxEdit::SetCellFormula {
                    sheet: "Expenses".to_string(),
    ...
    #[test]
    fn duplicate_workbook_relationship_ids_are_rejected() {
        let mut package = OoxmlPackage::read(&two_sheet_reversed_xlsx()).unwrap();
        let duplicate = r#"<Relationships><Relationship Id="rId1" Target="worksheets/sheet1.xml"/><Relationship Id="rId1" Target="worksheets/sheet2.xml"/></Relationships>"#;
        package
            .replace_part(WORKBOOK_RELS_PART, duplicate.as_bytes().to_vec())
            .unwrap();
        let error = read_xlsx(&package.write().unwrap()).unwrap_err();
        assert!(matches!(error, DocumentError::MalformedPart { .. }));
    }

    #[test]
    fn grouped_formula_targets_are_rejected() {
        for kind in ["shared", "array", "dataTable"] {
            let xml = format!(
                r#"<worksheet><sheetData><row r="1"><c r="A1"><f t="{kind}" si="0">1</f></c></row></sheetData></worksheet>"#
            );
            let error = set_cell_formula(&xml, "A1", "2").unwrap_err();
            assert!(
                matches!(error, DocumentError::InapplicableEdit { .. }),
                "{kind}: {error}"
            );
        }
    }
    ...
    #[test]
    fn multiple_formula_edits_in_one_call_all_survive() {
        let edited = edit_xlsx(
            &expenses_xlsx(),
            &[
                XlsxEdit::SetCellFormula {
        ...
        let xml = OoxmlPackage::read(&edited)
            ...
            .part_str("xl/worksheets/sheet1.xml")
            .unwrap();
        assert!(xml.contains("SUM(C2:C4)"));
        assert!(xml.contains("C5*2"));
    }
}
