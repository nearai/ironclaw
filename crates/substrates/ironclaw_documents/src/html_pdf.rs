...
//! |---|---|
//! | Headings | `<h1>` `<h2>` `<h3>` |
//! | Blocks | `<p>`, `<ul>`/`<ol>` with `<li>`, `<hr>` (vertical spacing), `<blockquote>` |
...
pub fn html_to_pdf(html: &str, options: &PdfOptions) -> Result<Vec<u8>, DocumentError> {
    ...
    if let Some(warning) = warnings.first() {
        return Err(DocumentError::Write(format!(
            "PDF serialization warning on page {} op {}: {}",
            warning.page, warning.op_id, warning.msg
        )));
    }
    Ok(bytes)
}
    ...
    while index < bytes.len() {
        if bytes[index] == b'<' {
            if html[index..].starts_with("<!--") {
                ...
                let Some(end) = html[index + 4..].find("-->") else {
                    break;
                };
                index += 4 + end + 3;
                continue;
...
mod tests {
    ...
    fn content_streams(pdf: &[u8]) -> Vec<u8> {
            ...
            .position(|window| window == b"stream")
        {
            ...
            if at >= 3 && &pdf[at - 3..at] == b"end" {
                offset = at + b"stream".len();
                continue;
            }
            let body = at + b"stream".len();
            let Some(end) = pdf[body..]
                .windows(b"endstream".len())
                .position(|window| window == b"endstream")
            else {
                break;
            };
            out.extend_from_slice(&pdf[body..body + end]);
            offset = body + end + b"endstream".len();
        }
        out
    }
    ...
    fn rendered_text(html: &str) -> String {
        let pdf = html_to_pdf(html, &PdfOptions::default()).unwrap();
        let mut warnings = Vec::new();
        ...
        assert!(
            warnings.is_empty(),
            "generated PDF must parse cleanly: {warnings:?}"
        );
        document
            .extract_text()
            .into_iter()
    ...
    #[test]
    fn comments_with_greater_than_are_consumed_through_the_terminator() {
        let text = rendered_text("<p>before <!-- 1 > 0 --> after</p>");
        assert!(text.contains("before after"), "{text:?}");
        assert!(!text.contains("0 -->"), "{text:?}");
    }

    #[test]
    fn parser_regressions_survive_the_public_render_path() {
        assert!(rendered_text("<p>hello <!-- note --> world</p>").contains("hello world"));
        assert!(rendered_text("<p>a & b &amp; c</p>").contains("a & b & c"));
    }
}
