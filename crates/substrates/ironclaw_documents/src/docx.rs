...
mod tests {
    ...
    #[test]
    fn an_edit_list_with_no_edits_has_deterministic_serialized_bytes() {
        let original = redlined_docx();
...
mod review_regressions {
    ...
    #[test]
    fn rejecting_a_deletion_preserves_del_text_attributes() {
        let body = r#"<w:p><w:del w:id="1"><w:r><w:delText xml:space="preserve"> restored </w:delText></w:r></w:del></w:p>"#;
        let edited = edit_docx(
            &docx_with_body(body),
            &[DocxEdit::ResolveAllRevisions {
                disposition: RevisionDisposition::Reject,
        ...
        let xml = OoxmlPackage::read(&edited)
            ...
            .unwrap();
        assert!(
            xml.contains(r#"<w:t xml:space="preserve"> restored </w:t>"#),
            "{xml}"
        );
    }
}
