...
- Accepted inbound messages must snapshot message-scoped reply targets; stable conversation bindings ignore and strip per-message IDs, but egress routing for accepted messages must preserve them.
...
- Accepted message idempotency and turn-submission idempotency are separate: adapter retries must reuse the accepted message ref and canonical actor and retry submission until the message is marked submitted. Rotate submit idempotency keys after busy/transient failures so turn-store replay cannot strand the message; keep the original submit key for permanent turn rejections.
