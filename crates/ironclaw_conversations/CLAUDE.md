...
- Accepted message idempotency and turn-submission idempotency are separate: adapter retries must reuse the accepted message ref and canonical actor and retry submission until the message is marked submitted. Reserve installation-wide external event IDs during resolution and reject route drift before creating a second thread. Rotate submit idempotency keys after busy/transient failures so turn-store replay cannot strand the message; keep the original submit key for permanent turn rejections.
