...
use ironclaw_loop_support::{
    HostIdentityContextSource, HostManagedModelGateway, HostManagedModelResponseObserver,
    HostSkillContextSource, ThreadBackedLoopModelPort,
};
    ...
    G: HostManagedModelGateway + ?Sized,
{
    ...
    pub(super) model_response_observer: Option<Arc<dyn HostManagedModelResponseObserver>>,
}
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync,
{
    async fn stream_model(
        ...
        if let Some(observer) = self.model_response_observer.as_ref() {
            model_port = model_port.with_model_response_observer(Arc::clone(observer));
        }
        model_port
