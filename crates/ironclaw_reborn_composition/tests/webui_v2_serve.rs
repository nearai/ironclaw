...
mod openai_compat_mount_tests {
    ...
    async fn openai_chat_timeout_keeps_shared_turn_admission_until_terminal_release() {
        ...
        let turn_state = Arc::new(
            in_memory_turn_state_store().with_admission_limit_provider(Arc::new(limits)),
        );
        ...
        assert_eq!(turn_state.active_admission_reservations().await.unwrap().len(), 1);
        ...
        assert!(turn_state.active_admission_reservations().await.unwrap().is_empty());
