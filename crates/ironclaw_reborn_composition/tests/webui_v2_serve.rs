...
async fn product_auth_oauth_routes_create_flow_and_complete_callback() {
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        ...
                        "opaque_state": "route-state-secret",
                        "pkce_verifier": "route-pkce-secret",
    ...
    assert!(!start_body.contains("route-state-secret"));
    assert!(!start_body.contains("route-pkce-secret"));
    ...
    let invocation_id = callback_scope["invocation_id"]
        .as_str()
    ...
    let callback_uri = format!(
        "/api/reborn/product-auth/oauth/callback/{flow_id}\
         ...
         &provider=github\
         &account_label=work%20github\
         &code=route-auth-code\
         &scopes=repo"
    )
    .replace(' ', "");
    ...
    assert_eq!(callback_json["flow_id"], flow_id);
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        ...
                        "opaque_state": "provider-denied-state",
                        "pkce_verifier": "provider-denied-pkce",
                        "expires_at": expires_at
                    })
                    .to_string(),
                ))
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .uri(
                    format!(
                        "/api/reborn/product-auth/oauth/callback/{}\
                     ?user_id={USER}\
                     &agent_id={AGENT}\
                     &project_id={PROJECT}\
                     &invocation_id={}\
                     &state=unknown-flow-state\
                     &error=access_denied",
                        uuid::Uuid::new_v4(),
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "provider": "github",
                        "authorization_url": "https://provider.example/oauth",
                        "opaque_state": "exchange-failed-state",
                        "pkce_verifier": "exchange-failed-pkce",
                        "expires_at": expires_at
                    })
                    .to_string(),
                ))
            ...
            Request::builder()
                ...
                .uri(
                    format!(
                     ...
                     &project_id={PROJECT}\
                     &invocation_id={invocation_id}\
                     &state=exchange-failed-state\
                     &provider=github\
                     &account_label=work%20github\
                     &code=exchange-failed-code\
                     &scopes=repo"
                    )
                    .replace(' ', ""),
                )
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "provider": "github",
                        "authorization_url": "https://provider.example/oauth",
                        "opaque_state": "wrong-scope-state",
                        "pkce_verifier": "wrong-scope-pkce",
                        "expires_at": expires_at
                    })
                    .to_string(),
                ))
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .uri(
                    format!(
                        "/api/reborn/product-auth/oauth/callback/not-a-flow-id\
                     ?user_id={USER}\
                     &agent_id={AGENT}\
                     &project_id={PROJECT}\
                     &invocation_id={}\
                     &state=malformed-flow-state\
                     &provider=github\
                     &account_label=work%20github\
                     &code=malformed-flow-code",
                        ironclaw_host_api::InvocationId::new(),
                    )
                    .replace(' ', ""),
                )
