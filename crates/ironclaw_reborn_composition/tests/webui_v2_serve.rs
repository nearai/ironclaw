...
async fn ws_upgrade_uses_canonical_host_over_client_host_when_configured() {
    ...
    use ironclaw_reborn_webui_ingress::WebuiServeConfig;
    ...
    let app = ironclaw_reborn_webui_ingress::webui_v2_app(bundle, config).expect("app");
    let (addr, handle) = spawn_serve(app).await;
...
async fn every_webui_v2_descriptor_is_mounted_on_composed_app() {
    ...
    for descriptor in ironclaw_reborn_webui_ingress::webui_v2::webui_v2_routes() {
