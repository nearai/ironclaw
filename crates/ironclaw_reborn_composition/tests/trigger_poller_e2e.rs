...
/// Verify that the poller respects timezone-converted next_run_at values.
///
/// A cron of "0 9 * * *" in America/New_York means 09:00 ET = 14:00 UTC in
/// winter (UTC-5). We use 2026-01-02 (a Friday in January, no DST ambiguity).
///
/// Because the trigger poller calls `Utc::now()` on every tick and the
/// TriggerPollerWorker's clock is not injectable through `RebornRuntime`'s
/// public interface, we create the trigger record directly via the repository
/// with a manually computed `next_run_at` derived from `TriggerSchedule::next_slot_after`.
/// The two halves of this test exercise:
///
/// (a) NOT-YET-DUE: `next_run_at` is set to 1 hour in the future; the poller
///     must NOT fire within 500ms.
/// (b) ALREADY-DUE: `next_run_at` is back-dated so the poller sees it as overdue;
///     the poller MUST fire and deliver the trigger prompt to the LLM gateway.
///
/// The timezone-correctness assertion is: `TriggerSchedule::next_slot_after` at
/// 2026-01-02 13:59:00 UTC returns 2026-01-02 14:00:00 UTC (9:00 AM Eastern),
/// not the UTC-interpreted 09:00 UTC slot.
#[tokio::test]
async fn trigger_poller_fires_named_timezone_trigger_at_converted_utc_slot() {
    use chrono::TimeZone;

    // ── Timezone-correctness assertion ───────────────────────────────
    // "0 9 * * *" in America/New_York on a winter date must resolve to 14:00 UTC.
    // This locks the schedule<→UTC conversion before we exercise the poller.
    let schedule_ny = TriggerSchedule::cron_with_timezone("0 9 * * *", "America/New_York")
        .expect("valid NY schedule");

    // Reference instant: 2026-01-02 13:59:00 UTC (one minute before 9 AM ET).
    let before_9am_et = chrono::Utc.with_ymd_and_hms(2026, 1, 2, 13, 59, 0).unwrap();
    let next_after_before = schedule_ny
        .next_slot_after(before_9am_et)
        .expect("slot computation succeeds")
        .expect("there is a next slot after 13:59 UTC");

    // The next slot must be 2026-01-02 14:00:00 UTC (09:00 ET), not 09:00 UTC.
    let expected_14_utc = chrono::Utc.with_ymd_and_hms(2026, 1, 2, 14, 0, 0).unwrap();
    assert_eq!(
        next_after_before, expected_14_utc,
        "next_slot_after(13:59 UTC) for '0 9 * * *' America/New_York should be 14:00 UTC (9 AM ET), \
         got {next_after_before}"
    );

    // ── Part (a): poller does NOT fire a not-yet-due trigger ─────────
    {
        let root = tempfile::tempdir().expect("tempdir");
        let recording_gateway = Arc::new(RecordingGateway {
            requests: Arc::new(TokioMutex::new(Vec::new())),
        ...
        // next_run_at is 1 hour from now — not yet due.
        let record = TriggerRecord {
            ...
            agent_id: Some(agent_id),
            project_id: None,
            name: "ny-tz-not-yet-due".to_string(),
            source: TriggerSourceKind::Schedule,
            schedule: TriggerSchedule::cron_with_timezone("0 9 * * *", "America/New_York")
                .expect("valid NY schedule"),
            completion_policy: TriggerCompletionPolicy::Recurring,
            prompt: TRIGGER_PROMPT.to_string(),
            state: TriggerState::Scheduled,
        ...
        // Give the poller 500ms (~25 ticks at 20ms) to attempt a fire.
        tokio::time::sleep(Duration::from_millis(500)).await;

        let repo_after = repo.clone();
        runtime.shutdown().await.expect("runtime shutdown");

        let captured = recording_gateway.captured_message_contents().await;
        let current = repo_after
            .get_trigger(tenant_id.clone(), record.trigger_id)
            .await
            .expect("get_trigger")
        ...
        assert!(
            current.last_fired_slot.is_none(),
            "(a) poller must not fire a not-yet-due NY trigger — last_fired_slot: {:?}",
            current.last_fired_slot
        );
        assert!(
            current.last_run_at.is_none(),
            "(a) poller must not set last_run_at for a not-yet-due NY trigger"
        );
        assert!(
            captured.is_empty(),
            "(a) LLM gateway must receive no requests for a not-yet-due NY trigger — \
             captured: {captured:?}"
        );
    }

    // ── Part (b): poller DOES fire an already-due trigger ────────────
    {
        let root = tempfile::tempdir().expect("tempdir");
        let recording_gateway = Arc::new(RecordingGateway {
            requests: Arc::new(TokioMutex::new(Vec::new())),
        ...
        // Back-date next_run_at so the trigger is already due for the poller.
        // This represents the instant at or after 2026-01-02 14:00 UTC (9 AM ET)
        ...
        let record = TriggerRecord {
            ...
            agent_id: Some(agent_id),
            project_id: None,
            name: "ny-tz-already-due".to_string(),
            source: TriggerSourceKind::Schedule,
            schedule: TriggerSchedule::cron_with_timezone("0 9 * * *", "America/New_York")
                .expect("valid NY schedule"),
            completion_policy: TriggerCompletionPolicy::Recurring,
        ...
        while Instant::now() < deadline {
            ...
            if mutated {
                record_was_mutated = true;
                let contents = recording_gateway.captured_message_contents().await;
                if contents.iter().any(|c| c.contains(TRIGGER_PROMPT)) {
                    prompt_seen = true;
                }
            }
        ...
        assert!(
            record_was_mutated,
            "(b) poller must fire the already-due NY trigger — record: {settled:?}"
        );
        assert!(
            prompt_seen,
            "(b) LLM gateway must receive trigger prompt for already-due NY trigger — \
             captured: {captured:?}"
        );
        assert!(
            settled.last_fired_slot.is_some(),
            "(b) last_fired_slot must be set after fire — record: {settled:?}"
        );
        assert!(
            settled.last_run_at.is_some(),
            "(b) last_run_at must be set after fire — record: {settled:?}"
        );
        // The fire slot is the back-dated next_run_at, so the worker's
        // post-fire recompute must land on the schedule's timezone-aware
        // next slot after it. Exact equality catches a regression where
        // the recompute silently falls back to bare-UTC evaluation while
        // the stored timezone string survives untouched.
        let expected_next_run_at = settled
            .schedule
            .next_slot_after(original_next_run_at)
            .expect("next slot computation succeeds")
            .expect("cron schedule has future slots");
        assert_eq!(
            settled.next_run_at, expected_next_run_at,
            "(b) post-fire next_run_at must be the timezone-aware next slot \
             after the fired slot — fired: {original_next_run_at:?}, settled: {:?}",
            settled.next_run_at
        );
    }
}
