...
/// Live-repro harness for the demo-stack defect: removing an installed
/// channel extension (telegram) reported success while the durable
/// installation row survived and the projection stayed active. Drives the
/// REAL lifecycle port remove with an authenticated actor — the same shape
/// the webui remove capability dispatches — and asserts the durable row and
/// caller projection actually flip.
#[tokio::test]
