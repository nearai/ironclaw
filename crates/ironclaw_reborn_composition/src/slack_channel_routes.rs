...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct SlackChannelRouteSubjectOwner {
    subject_user_id_prefix: String,
}

impl SlackChannelRouteSubjectOwner {
    pub(crate) fn managed_channel_subjects() -> Self {
        Self {
            subject_user_id_prefix: MANAGED_CHANNEL_SUBJECT_PREFIX.to_string(),
        }
    }

    pub(crate) fn owns(&self, subject_user_id: &str) -> bool {
        subject_user_id.starts_with(&self.subject_user_id_prefix)
    }
}
...
impl SlackChannelSubjectAssigner {
    pub(crate) fn new(
        ...
        Self {
            ...
            installation_id,
            team_id,
            owner: SlackChannelRouteSubjectOwner::managed_channel_subjects(),
        }
    }

    pub(crate) fn owner(&self) -> &SlackChannelRouteSubjectOwner {
        &self.owner
    }
    ...
    pub(crate) fn owns_route(&self, route: &SlackChannelRoute) -> bool {
        self.owner.owns(&route.subject_user_id)
    }
...
impl SlackChannelRouteStore for InMemorySlackChannelRouteStore {
    ...
    async fn replace_managed_routes(
        ...
        let requested = assignments
            ...
            .collect::<HashSet<_>>();
        routes.retain(|key, current_subject| {
            &key.tenant_id != tenant_id
                || &key.installation_id != installation_id
                || key.team_id != team_id
                || !owner.owns(current_subject.as_str())
...
mod tests {
    ...
    #[tokio::test]
    async fn in_memory_replace_managed_routes_preserves_manual_subject_routes() {
        ...
        assert_eq!(
            store
                ...
                .as_ref()
                .map(UserId::as_str),
            Some(manual_ops_subject.as_str())
        );
    }
    ...
    impl SlackChannelRouteStore for CountingRouteStore {
        ...
        async fn replace_managed_routes(
            ...
            _installation_id: &AdapterInstallationId,
            _team_id: &str,
            _owner: SlackChannelRouteSubjectOwner,
