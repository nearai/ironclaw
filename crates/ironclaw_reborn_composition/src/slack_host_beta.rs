...
mod tests {
    ...
    async fn slack_allowed_channels_are_reachable_through_webui_v2_app() {
        ...
        let mounts = build_slack_host_beta_mounts(&runtime, config_without_channel_routes())
            .expect("mounts");
        let bundle =
            build_webui_services_with_slack_host_beta_mounts(&runtime, None, Some(&mounts), true)
                .expect("webui bundle");
    ...
    async fn slack_channel_routes_are_not_mounted_for_non_operator_authenticator() {
        ...
        let mounts = build_slack_host_beta_mounts(&runtime, config_without_channel_routes())
            .expect("mounts");
        let bundle =
            build_webui_services_with_slack_host_beta_mounts(&runtime, None, Some(&mounts), false)
                .expect("webui bundle");
