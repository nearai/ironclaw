...
async fn filesystem_manual_token_rotation_removes_previous_secret() {
    ...
    // from SecretStorePort so it does not accumulate orphaned material.
    use ironclaw_auth::{
    ...
    // The old handle must have been purged from SecretStorePort after the rotation.
    assert!(
        concrete_secret_store
            .metadata(&scope.resource, &old_handle)
            .await
            .unwrap()
            .is_none(),
        "v1 secret must be purged from SecretStorePort after rotation"
    );
    ...
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_some(),
        "v2 secret must be present in SecretStorePort"
    );
}
...
async fn filesystem_cleanup_for_lifecycle_deactivates_owner_and_revokes_on_uninstall() {
    ...
    // Uninstall: account revoked, secrets purged from SecretStorePort.
    ...
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_none(),
        "Uninstall must delete access secret from SecretStorePort"
    );
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_none(),
        "Uninstall must delete refresh secret from SecretStorePort"
    );
}
...
async fn filesystem_oauth_reauth_purges_previous_provider_secrets() {
    ...
    // and refresh secret handles must be deleted from SecretStorePort so repeated
    ...
    // Pre-populate SecretStorePort to simulate provider client having stored these
    ...
    // Old handles must have been purged from SecretStorePort.
    assert!(
        concrete_secret_store
            .metadata(&scope.resource, &access_v1)
            .await
            .unwrap()
            .is_none(),
        "v1 access handle must be purged from SecretStorePort after re-auth"
    );
    assert!(
        concrete_secret_store
            .metadata(&scope.resource, &refresh_v1)
            ...
            .unwrap()
            .is_none(),
        "v1 refresh handle must be purged from SecretStorePort after re-auth"
    );
    ...
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_some(),
        "v2 access handle must be present in SecretStorePort after re-auth"
    );
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_some(),
        "v2 refresh handle must be present in SecretStorePort after re-auth"
    );
}
...
async fn filesystem_manual_token_submit_cleans_up_secret_when_account_write_fails() {
    ...
    // fails the newly-stored secret must be deleted from SecretStorePort so it does
    ...
    assert!(
        concrete_secret_store
            ...
            .unwrap()
            .is_none(),
        "orphaned secret must be purged from SecretStorePort after failed account write"
    );
}
...
async fn filesystem_oauth_cas_conflict_branch_purges_previous_secrets() {
    ...
    // SecretStorePort so repeated re-auths do not accumulate dead handles.
    ...
    // Old secrets must be purged from SecretStorePort.
    assert!(
        concrete_secret_store
