...
/// `has_account` — the credential pre-flight's cheap existence probe (used by
/// `ironclaw_host_runtime::production::credential_preflight_check`) — must
/// report `true` for a configured, connected account, without leasing or
/// refreshing anything (the fixture's `NoopRuntimeCredentialAccountRefresher`
/// would surface as a wrong result if refresh were ever invoked; here we
/// simply assert the presence probe agrees with `resolve_access_secret`).
#[tokio::test]
async fn has_account_reports_true_for_configured_account() {
    let accounts = Arc::new(InMemoryAuthProductServices::new());
    let scope =
        ResourceScope::local_default(UserId::new("alice").unwrap(), InvocationId::new()).unwrap();
    let auth_scope = AuthProductScope::new(scope.clone(), AuthSurface::Api);
    ...
    let present = resolver
        .has_account(RuntimeCredentialAccountRequest {
            scope: &scope,
            provider: &RuntimeCredentialAccountProviderId::new("github").unwrap(),
            setup: &RuntimeCredentialAccountSetup::ManualToken,
            provider_scopes: &[],
    ...
    assert!(present, "a configured account must report present");
}

/// `has_account` must report `false` (not an error) for an account that was
/// never connected — the pre-flight's ordinary "not configured" case, which
/// `credential_preflight_check` folds into `AuthRequired`.
#[tokio::test]
async fn has_account_reports_false_for_missing_account() {
    let accounts = Arc::new(InMemoryAuthProductServices::new());
    let resolver = resolver_with_accounts(accounts);
    let scope =
        ResourceScope::local_default(UserId::new("alice").unwrap(), InvocationId::new()).unwrap();
    ...
    assert!(!present, "no configured account must report not-present");
}

/// `has_account` must report `false` — not `true` — when account selection
/// is ambiguous (`AuthProductError::AccountSelectionRequired`, e.g. mixed
/// reusable + shared-admin accounts, see
/// `duplicate_selection::resolver_does_not_auto_select_mixed_reusable_and_shared_admin_accounts`).
/// An ambiguous selection also resolves to `AuthRequired` at dispatch time
/// via `resolve_access_secret`, so the pre-flight must not skip ahead and
/// report the account as usably connected — that would let the flow reach
/// the approval gate and sandbox spin-up for a capability that is about to
/// fail the same way anyway.
#[tokio::test]
async fn has_account_reports_false_for_ambiguous_account_selection() {
    let accounts = Arc::new(InMemoryAuthProductServices::new());
    let scope =
        ResourceScope::local_default(UserId::new("alice").unwrap(), InvocationId::new()).unwrap();
    let auth_scope = AuthProductScope::new(scope.clone(), AuthSurface::Api);
    ...
    assert!(
        !present,
        "an ambiguous account selection must not report present — dispatch would AuthRequired anyway"
    );
}
