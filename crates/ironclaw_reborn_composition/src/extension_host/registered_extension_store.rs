...
use ironclaw_host_api::{
    CapabilityId, ExtensionId, InvocationId, ResourceScope, TenantId, UserId, VirtualPath,
    sha256_digest_token,
};
...
/// AC2: the set of extension ids that are BOTH enabled AND visible to the
/// caller's resource scope. This default-deny model-visible-capability filter
/// ensures disabled or cross-tenant registered extensions never reach the
/// model's toolbox.
pub(crate) async fn owner_visible_enabled_extension_ids_for_scope(
    ...
    let enabled_ids: BTreeSet<ExtensionId> = installation_store
        ...
        .collect();
    let visible_ids: BTreeSet<ExtensionId> = installation_store
        .list_manifests()
        .await?
        .into_iter()
        .filter(|record| {
            record
                .manifest()
                .source
                .visible_to_resource_scope(Some(scope))
        })
        .map(|record| record.extension_id().clone())
        .collect();
    Ok(enabled_ids.intersection(&visible_ids).cloned().collect())
}
...
pub(crate) async fn operator_config_excluded_extension_ids(
    ...
    for record in installation_store.list_manifests().await? {
        if !record
            .manifest()
            .source
            .visible_to_resource_scope(Some(caller))
        {
...
impl RegisteredExtensionStore {
    ...
    /// Legacy test-only `/system/extensions/registered/<owner>` directory
    /// retained for compatibility fixtures. Production uses
    /// `/system/extensions/registered/<tenant>/<owner>`.
    #[cfg(test)]
    ...
    /// `/system/extensions/registered/<tenant>/<owner>/<extension_id>` cannot erase the
    ...
    fn discovered_capability_ids_path_for_tenant(
        tenant_id: &TenantId,
        owner: &UserId,
        extension_id: &ExtensionId,
    ) -> Result<VirtualPath, ProductWorkflowError> {
        ...
        F: RootFilesystem + ?Sized,
    {
        ...
        let stored: StoredDiscoveredCapabilityIds =
            serde_json::from_slice(&versioned.entry.body)
                .map_err(|error| invalid_stored_inventory_json(&error))?;
    ...
    pub(crate) async fn load_discovered_capability_ids_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        extension_id: &ExtensionId,
    ) -> Result<Vec<CapabilityId>, ProductWorkflowError>
    where
        F: RootFilesystem + ?Sized,
    {
        let path = Self::discovered_capability_ids_path_for_tenant(
            &scope.tenant_id,
            &scope.user_id,
            extension_id,
        )?;
        ...
        let stored: StoredDiscoveredCapabilityIds =
            serde_json::from_slice(&versioned.entry.body)
                .map_err(|error| invalid_stored_inventory_json(&error))?;
        if stored.schema_version != DISCOVERED_CAPABILITY_IDS_SCHEMA_VERSION
            || stored.owner != scope.user_id
            || &stored.extension_id != extension_id
        {
            return Err(invalid_stored_inventory(
                "stored schema, owner, or extension does not match the inventory path".to_string(),
            ));
        }
        let canonical = canonical_discovered_capability_ids(extension_id, &stored.capability_ids)
            .map_err(invalid_stored_inventory)?;
    ...
    pub(crate) async fn replace_discovered_capability_ids_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        extension_id: &ExtensionId,
        capability_ids: &[CapabilityId],
    ...
    pub(crate) async fn delete_discovered_capability_ids_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        extension_id: &ExtensionId,
    ) -> Result<(), ProductWorkflowError>
    ...
    pub(crate) fn mint_hosted_mcp_extension_id(
        ...
        Self::mint_hosted_mcp_extension_id_for_tenant(
    ...
    fn mint_hosted_mcp_extension_id_for_tenant(
        tenant_id: &TenantId,
        owner: &UserId,
        url: &str,
    ) -> Result<ExtensionId, ProductWorkflowError> {
        ...
        let mut input = Vec::with_capacity(
            tenant_id.as_str().len() + owner.as_str().len() + 2 + normalized_url.len(),
        );
        input.extend_from_slice(tenant_id.as_str().as_bytes());
        input.push(0x1f);
    ...
    pub(crate) fn synthesize_hosted_mcp_manifest_toml(
        ...
        Self::synthesize_hosted_mcp_manifest_toml_for_tenant(
            &TenantId::from_trusted(ironclaw_host_api::LOCAL_DEFAULT_TENANT_ID.to_string()),
            owner,
            input,
        )
    ...
    pub(crate) fn synthesize_hosted_mcp_manifest_toml_for_scope(
        scope: &ResourceScope,
        input: &RegisterHostedMcpDescriptorInput,
    ) -> Result<RegisteredHostedMcpDescriptorDraft, ProductWorkflowError> {
        Self::synthesize_hosted_mcp_manifest_toml_for_tenant(
    ...
    fn synthesize_hosted_mcp_manifest_toml_for_tenant(
        tenant_id: &TenantId,
        owner: &UserId,
        input: &RegisterHostedMcpDescriptorInput,
    ) -> Result<RegisteredHostedMcpDescriptorDraft, ProductWorkflowError> {
        ...
        let extension_id =
            Self::mint_hosted_mcp_extension_id_for_tenant(tenant_id, owner, &input.url)?;
        ...
        let manifest_record =
            parse_registered_manifest_for_tenant(tenant_id, owner, manifest_toml.clone(), None)?;
        #[cfg(not(test))]
    ...
    pub(crate) async fn put_hosted_mcp_descriptor_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        input: RegisterHostedMcpDescriptorInput,
    ) -> Result<RegisteredHostedMcpDescriptorPut, ProductWorkflowError>
    where
        F: RootFilesystem + ?Sized,
    {
        let draft = Self::synthesize_hosted_mcp_manifest_toml_for_scope(scope, &input)?;
        let prior_same_name = Self::find_same_owner_same_name_descriptor_for_scope(
            fs,
            scope,
            draft.extension_id(),
            &input.name,
        ...
        .await?;
        let existing_count =
            Self::descriptor_count_for_tenant_owner(fs, &scope.tenant_id, &scope.user_id).await?;
        let existing_same_id = Self::descriptor_exists_for_tenant(
            fs,
            &scope.tenant_id,
            &scope.user_id,
            draft.extension_id(),
        ...
        let manifest_path =
            Self::manifest_path_for_tenant(&scope.tenant_id, &scope.user_id, draft.extension_id())?;
        fs.write_file(&manifest_path, draft.manifest_toml().as_bytes())
            .await
            .map_err(|error| ProductWorkflowError::Transient {
                reason: format!("failed to write registered extension manifest: {error}"),
    ...
    pub(crate) async fn delete_descriptor_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        extension_id: &ExtensionId,
    ) -> Result<(), ProductWorkflowError>
    ...
    {
        let descriptor_root =
            Self::descriptor_root_for_tenant(&scope.tenant_id, &scope.user_id, extension_id)?;
        match fs.delete(&descriptor_root).await {
            Ok(())
            | Err(FilesystemError::NotFound { .. })
            | Err(FilesystemError::MountNotFound { .. }) => Ok(()),
    ...
    pub(crate) async fn find_same_owner_same_name_descriptor_for_scope<F>(
        fs: &F,
        scope: &ResourceScope,
        replacement_id: &ExtensionId,
        name: &str,
    ...
    {
        let normalized_name = name.trim();
        for package in Self::list_for_scope(fs, scope).await? {
            let manifest = &package.package.manifest;
            if &manifest.id != replacement_id && manifest.name == normalized_name {
                let ExtensionRuntime::Mcp { url: Some(url), .. } = &manifest.runtime else {
                    continue;
                ...
                return Ok(Some(RegisteredHostedMcpDescriptor {
                    ...
                    manifest_toml: package.manifest_toml.clone(),
                    #[cfg(test)]
                    manifest_record: parse_registered_manifest_for_tenant(
                        &scope.tenant_id,
                        &scope.user_id,
    ...
    /// Test-only compatibility reader for legacy owner-only fixtures. Production
    /// callers must use [`Self::list_for_scope`] so tenant identity is explicit.
    #[cfg(test)]
    ...
    /// the calling owner and tenant via [`list_for_scope`].
        ...
        F: RootFilesystem + ?Sized,
    {
        ...
        for tenant_entry in entries {
            ...
            for entry in owner_entries {
                ...
                let mut scope = ResourceScope::local_default(owner, InvocationId::new())
                    .map_err(map_binding_error)?;
                scope.tenant_id = tenant_id.clone();
                packages.extend(Self::list_for_scope(fs, &scope).await?);
            }
        }
        Ok(packages)
    }
    ...
    async fn descriptor_count_for_tenant_owner<F>(
        fs: &F,
        tenant_id: &TenantId,
        owner: &UserId,
    ) -> Result<usize, ProductWorkflowError>
    where
        F: RootFilesystem + ?Sized,
    {
        let root = Self::owner_root_for_tenant(tenant_id, owner)?;
        let entries = match fs.list_dir(&root).await {
            Ok(entries) => entries,
            Err(FilesystemError::NotFound { .. }) | Err(FilesystemError::MountNotFound { .. }) => {
                return Ok(0);
    ...
    async fn descriptor_exists_for_tenant<F>(
        fs: &F,
        tenant_id: &TenantId,
        owner: &UserId,
        extension_id: &ExtensionId,
    ...
    where
        F: RootFilesystem + ?Sized,
    {
        let root = Self::descriptor_root_for_tenant(tenant_id, owner, extension_id)?;
        match fs.stat(&root).await {
            Ok(stat) => Ok(stat.file_type == FileType::Directory),
            Err(FilesystemError::NotFound { .. }) | Err(FilesystemError::MountNotFound { .. }) => {
                Ok(false)
...
fn invalid_stored_inventory_json(error: &serde_json::Error) -> ProductWorkflowError {
    ...
    invalid_stored_inventory(format!(
        "inventory JSON is invalid ({category}) at line {}, column {}",
        error.line(),
        error.column()
    ))
}
...
fn parse_registered_manifest(
    ...
    parse_registered_manifest_for_tenant(
        &TenantId::from_trusted(ironclaw_host_api::LOCAL_DEFAULT_TENANT_ID.to_string()),
        owner,
        manifest_toml,
        manifest_hash,
...
fn parse_registered_manifest_for_tenant(
    tenant_id: &TenantId,
    owner: &UserId,
    manifest_toml: String,
    manifest_hash: Option<ironclaw_extensions::ManifestHash>,
