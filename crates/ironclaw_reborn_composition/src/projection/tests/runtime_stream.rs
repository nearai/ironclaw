...
// arch-exempt: large_file, product projection stream contract coverage remains co-located, plan #1
#[tokio::test]
async fn product_event_stream_does_not_render_nested_dispatch_failure_as_failed_run() {
    ...
    assert!(!contains_run_status(&events, child_invocation_id, "failed"));
        ...
        matches!(
            ...
            ProductOutboundPayload::CapabilityActivity(activity)
                if activity.invocation_id == child_invocation_id
                    ...
                    && activity.turn_run_id == Some(TurnRunId::from_uuid(parent_invocation_id.as_uuid()))
