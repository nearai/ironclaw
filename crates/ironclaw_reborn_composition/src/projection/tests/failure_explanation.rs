...
fn failure_summary_covers_every_loop_failure_kind_category() {
    ...
    for (category, expected_summary) in expected {
        let summary = crate::reborn_failure_summary_for_category(Some(category));
...
fn failure_summary_covers_reborn_failure_category_constants() {
    ...
    for (category, expected_summary) in expected {
        assert_eq!(
            crate::reborn_failure_summary_for_category(Some(category)),
            expected_summary,
            "category {category}"
        );
    }
...
fn failure_summary_uses_safe_generic_fallback_for_unknown_categories() {
    let summary = crate::reborn_failure_summary_for_category(Some("new_snake_case_code"));
