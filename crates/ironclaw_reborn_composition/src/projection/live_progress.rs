                ...
                self.publish_capability_activity(
                    ...
                    TerminalCapabilityActivity {
                        ...
                        error_summary: safe_summary
                            .as_ref()
                            .and_then(|summary| safe_error_summary(summary.as_str())),
                    },
                );
            }
...
fn safe_error_summary(summary: &str) -> Option<String> {
    let summary = sanitize_model_visible_text(summary).trim().to_string();
    if summary.is_empty() {
        None
    } else {
        Some(summary)
