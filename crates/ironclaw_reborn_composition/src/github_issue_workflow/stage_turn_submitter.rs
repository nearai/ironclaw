//! [`StageTurnSubmitter`] implementation that dispatches a workflow stage turn
//! through the thread/turn coordination layer.
//!
//! [`IronClawStageTurnSubmitter`] ensures the deterministic stage thread, replays
//! or accepts the stage prompt as an inbound message, submits the turn under the
//! stage's capability profile, and stamps the trusted stage-binding metadata onto
//! the thread that [`super::stage_result_sink`] later reads back. The shared
//! stage-thread `kind` discriminator and the adapter id live in the parent module
//! and are referenced here via `super::`.
...
use ironclaw_github_issue_workflow::{
    GithubIssueStage, GithubIssueWorkflowError, GithubIssueWorkspaceSessionId, StageTurnSubmitter,
    SubmitStageTurnOutcome, SubmitStageTurnRequest, WorkflowActorScope,
};
use ironclaw_host_api::{AgentId, MountView, ThreadId, UserId};
...
use super::capability_profiles::stage_capability_profile_id;
use super::workspace_manager::workflow_workspace_mount_view;
use super::{GITHUB_ISSUE_WORKFLOW_STAGE_THREAD_KIND, WORKFLOW_ADAPTER_ID};
