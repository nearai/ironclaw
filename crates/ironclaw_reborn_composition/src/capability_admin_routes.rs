...
//!
//! Admin-gated (operator-config admin on this slice; the role-based
//! `WebUiAuthenticatedCaller::is_admin` / `UserRole::Admin` lands with #5270 —
//! see `ensure_admin` TODO) routes that write **tenant-wide** extension
//! installations to #4544's
...
/// enumeration) and the caller must be an operator-config admin.
///
// TODO(#5261): the role-based `WebUiAuthenticatedCaller::is_admin()`
// (`UserRole::Owner`/`Admin`) lands with the user-role work (#5270), which is
// NOT in this availability slice's base (engine #5344 + #4544). Until #5270
// merges, the only admin signal present on this branch is the legacy
// operator-config bit, which is exactly the non-role half of integration's
// `is_admin()` (`self.role.is_admin() || self.operator_webui_config`). Restore
// `caller.is_admin()` once `role` is available here.
fn ensure_admin(
...
mod tests {
    ...
    // TODO(#5261): once #5270's `role` field lands on this base, restore the
    // role-based caller (`UserRole::Admin`/`Member`) here. Until then this slice
    // gates on the operator-config admin bit (see `ensure_admin`), so the test
    // caller carries that signal directly.
    fn request(method: &str, uri: &str, tenant: &str, user: &str, is_admin: bool) -> Request<Body> {
        let mut builder = Request::builder()
            ...
            .extension(WebUiAuthenticatedCaller {
                ...
                agent_id: None,
                project_id: None,
                operator_webui_config: is_admin,
            });
