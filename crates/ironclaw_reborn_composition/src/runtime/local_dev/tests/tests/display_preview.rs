...
async fn capability_io_writes_display_preview_to_durable_history() {
    ...
    assert_eq!(envelope.status, CapabilityDisplayPreviewStatus::Completed);
    assert_eq!(
...
#[tokio::test]
async fn capability_io_writes_failed_display_preview_to_durable_history() {
    let run_context = run_context("durable-failed-display-preview").await;
    let thread_scope = ThreadScope {
        tenant_id: run_context.scope.tenant_id.clone(),
        agent_id: run_context.scope.agent_id.clone().expect("agent id"),
        project_id: run_context.scope.project_id.clone(),
    ...
    capability_io
        .write_capability_result(CapabilityResultWrite {
            ...
            invocation_id,
            capability_id: &capability_id,
            status: ironclaw_loop_support::CapabilityResultStatus::Failed,
    ...
    assert_eq!(envelope.output_summary.as_deref(), Some("json output"));
    assert!(
        envelope
            .output_preview
            .as_deref()
            .is_some_and(|preview| preview.contains("credential missing"))
    );
}
