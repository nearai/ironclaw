...
mod tests {
    ...
    /// Records `on_capability_result` callbacks so the result-hook seam can be
    /// asserted end-to-end through `write_capability_result` — the one place
    /// reborn forwards a completed capability's output to the trajectory
    /// observer. Guards against a future refactor silently dropping the hook.
    #[derive(Debug, Default)]
    struct RecordingResultObserver {
        results: std::sync::Mutex<Vec<(String, String, serde_json::Value)>>,
    }

    impl crate::RebornTrajectoryObserver for RecordingResultObserver {
    ...
    async fn capability_io_forwards_result_to_trajectory_observer() {
        let run_context = run_context("result-observer").await;
        let thread_scope = ThreadScope {
            tenant_id: run_context.scope.tenant_id.clone(),
            agent_id: run_context.scope.agent_id.clone().expect("agent id"),
            project_id: run_context.scope.project_id.clone(),
        ...
        assert_eq!(
            results.len(),
            1,
            "observer should see exactly one capability result"
        );
        let (call_id, observed_capability, observed_output) = &results[0];
        assert_eq!(
            call_id,
            input_ref.as_str(),
            "result call_id should correlate with the staged input ref"
        );
        assert_eq!(
            observed_capability,
        ...
        assert_eq!(
            ...
            "observer should receive the staged result output"
        );
    }
