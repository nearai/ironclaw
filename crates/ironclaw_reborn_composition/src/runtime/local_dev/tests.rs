...
mod tests {
    ...
    use ironclaw_threads::{
        ...
        MessageKind, ThreadHistoryRequest, ToolResultReferenceEnvelope, ToolResultSafeSummary,
    };
    use ironclaw_turns::{
    ...
    #[tokio::test]
    async fn capability_io_writes_typed_display_preview_to_durable_history() {
        let run_context = run_context("typed-durable-preview").await;
        let thread_scope = ThreadScope {
            tenant_id: run_context.scope.tenant_id.clone(),
            agent_id: run_context.scope.agent_id.clone().expect("agent id"),
            project_id: run_context.scope.project_id.clone(),
