...
impl From<ironclaw_extension_host::reborn::RebornExtensionHostBuildError> for RebornBuildError {
    fn from(error: ironclaw_extension_host::reborn::RebornExtensionHostBuildError) -> Self {
        match error {
            ironclaw_extension_host::reborn::RebornExtensionHostBuildError::InvalidConfig {
                reason,
            } => Self::InvalidConfig { reason },
            ironclaw_extension_host::reborn::RebornExtensionHostBuildError::Filesystem(error) => {
                Self::Filesystem(error)
            }
            ironclaw_extension_host::reborn::RebornExtensionHostBuildError::Mount(error) => {
                Self::Mount(error)
            }
        }
    }
