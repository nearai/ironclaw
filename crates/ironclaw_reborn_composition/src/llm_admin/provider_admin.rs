...
mod tests {
    ...
    /// One captured request: method, path, and the raw `Authorization`
    /// header value (if any).
    ...
    /// `endpoint_against_canned_response` test helper, plus request capture
    /// so this test can assert probe_candidate actually reached the
    /// CONFIGURED base URL with the ENTERED key — the seam that regressed
    /// once already (probe used an empty base URL and always reported
    /// "could not reach").
    ...
    #[cfg(feature = "root-llm-provider")]
    fn write_stub_provider(home: &ironclaw_reborn_config::RebornHome, base_url: &str) {
        std::fs::create_dir_all(home.path()).expect("create reborn home dir");
        crate::ProviderRepo::new(home.providers_file_path())
    ...
    /// build its request against the provider's CONFIGURED base URL (from
    /// the registry/overlay) carrying the ENTERED key (the inline candidate
    /// argument), not a blank/default URL. A regression back to an empty
    /// base URL would make the stub never see a connection and this test
    /// would time out / fail on `ok`.
    #[cfg(feature = "root-llm-provider")]
    #[tokio::test]
    ...
    /// A 401 from the configured endpoint must surface as `ok: false`
    /// (never a bare `Err`) — matching `probe_candidate_provider`'s
    /// no-separate-error-channel contract that `probe_candidate` forwards.
    #[cfg(feature = "root-llm-provider")]
    #[tokio::test]
    async fn probe_candidate_reports_401_as_not_ok() {
        let (base_url, _request_rx) = spawn_models_stub("HTTP/1.1 401 Unauthorized", "").await;
