...
use ironclaw_auth::{
    ...
    CredentialRecoveryProjection, CredentialRecoveryReason, CredentialSelectionInput,
    ManualTokenSetupRequest, NewAuthFlow, NewCredentialAccount, OAuthCallbackClaimRequest,
    OAuthProviderExchange, Timestamp,
};

use super::{credential_status_for_completed_flow, is_terminal_status, scope_matches};

pub(crate) struct PreparedCallbackFlow {
    pub(crate) scope: ironclaw_auth::AuthProductScope,
    pub(crate) update_binding: Option<CredentialAccountUpdateBinding>,
    pub(crate) expected_pkce_verifier_hash: Option<ironclaw_auth::PkceVerifierHash>,
}
...
fn validate_scoped_update_binding(
    ...
    if &account.provider != provider {
        return Err(invalid_request(provider_mismatch));
    }
...
pub(crate) fn validate_credential_status_transition(
    current: ironclaw_auth::CredentialAccountStatus,
    next: ironclaw_auth::CredentialAccountStatus,
) -> Result<(), AuthProductError> {
...
fn invalid_request(reason: impl Into<String>) -> AuthProductError {
...
fn credential_status_transition_allowed(
    ...
    use ironclaw_auth::CredentialAccountStatus::{
...
fn recovery_kind_and_reason_for_status(
    ...
    match status {
        ...
        ironclaw_auth::CredentialAccountStatus::PendingSetup => (
            CredentialRecoveryKind::SetupRequired,
            CredentialRecoveryReason::PendingSetup,
        ),
        ironclaw_auth::CredentialAccountStatus::Missing => (
            CredentialRecoveryKind::SetupRequired,
            CredentialRecoveryReason::AccountMissing,
        ),
        ironclaw_auth::CredentialAccountStatus::Inactive => (
            CredentialRecoveryKind::SetupRequired,
        ...
        ironclaw_auth::CredentialAccountStatus::Revoked => (
            CredentialRecoveryKind::ReauthorizeRequired,
