...
use ironclaw_auth::{
    AuthChallenge, AuthContinuationRef, AuthFlowKind, AuthFlowManager, AuthFlowRecordSource,
    AuthFlowStatus, AuthInteractionService, AuthProductError, AuthProductScope, AuthProviderId,
...
async fn filesystem_flow_record_source_projects_session_scoped_manual_flows() {
    ...
    let service = FilesystemAuthProductServices::new_with_flow_projection_scope(
        filesystem,
        secret_store,
    ...
    let snapshot = AuthFlowRecordSource::flow_records_snapshot(&service)
        .await
        .unwrap();
