...
use ironclaw_product_adapters::{AdapterInstallationId, TargetQuery};
...
/// Thin wrappers over the adapter crate's binding-ref grammar
/// (`ironclaw_slack_v2_adapter::preference_targets`) mapping construction
/// failures onto this module's service-error vocabulary. Deleted with the
/// lane (callers move to the adapter crate directly).
pub(crate) fn slack_shared_channel_reply_target_binding_ref(
    ...
    ironclaw_slack_v2_adapter::slack_shared_channel_reply_target_binding_ref(
        installation_id,
        agent_id,
        project_id,
        team_id.as_str(),
...
fn slack_personal_dm_reply_target_binding_ref(
    ...
    ironclaw_slack_v2_adapter::slack_personal_dm_reply_target_binding_ref(
        installation_id,
        agent_id,
        project_id,
        team_id.as_str(),
...
// `ExternalConversationRef::conversation_fingerprint`. Retained locally for
// the provider's config-validating reply-target parser above; the canonical
// grammar lives in the adapter crate.
...
mod tests {
    ...
    // The pure binding-ref decode/predicate tests moved with the grammar to
    // `ironclaw_slack_v2_adapter::preference_targets`.
}
