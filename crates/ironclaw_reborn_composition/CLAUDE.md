...
| Operator status/readiness | v1 doctor/status fragments | `GET /api/webchat/v2/operator/status` | Mapped through `RebornServicesApi`; currently projects composed Reborn readiness plus explicit unsupported channel/extension probes |
| Operator logs | `GET /api/logs/events`, `GET\|PUT /api/logs/level` | `POST /api/webchat/v2/operator/logs/query` | Route and facade mapped; real log backend wiring remains behind `OperatorLogsService` |
| Service lifecycle | `ironclaw service {install,start,stop,status}` | `POST /api/webchat/v2/operator/service` | Route and facade mapped; unsupported targets return typed lifecycle status until a platform backend is injected |
