...
| `ironclaw_reborn_webui_ingress` | `ironclaw_reborn_webui_ingress/AGENTS.md`, `Cargo.toml` | Host-owned WebChat v2 gateway: router composition, typed route mounts, auth and descriptor-driven policy middleware, security headers, listener binding, and serve loop. | Product/API route semantics, transcript storage, v1 channel code, product adapter transport shims. |
...
- Reborn binary/composition: `ironclaw_reborn_config` for boot config; `ironclaw_reborn_composition` for production wiring; `ironclaw_reborn_cli` for commands; `ironclaw_runner` for standalone adapters/driver registry; `ironclaw_reborn_webui_ingress` for the host-owned WebChat v2 gateway and listener lifecycle.
