...
| `ironclaw_ironhub` | `Cargo.toml`, `src/lib.rs` | IronHub catalog client: signed-catalog fetch/verification/caching, registry install orchestration through `ironclaw_extension_host` seams, the `builtin.ironhub_*` tool surface, and the HMAC deep-link install service behind `ironclaw_product_contracts::ironhub::IronhubLinkService`. | Generic registry seams (`registry_extension_package` stays in `ironclaw_extension_host`); execution runtimes, secret storage, serve/assembly layers. |
