...
| `ironclaw_channel_host` | `ironclaw_channel_host/AGENTS.md` | Vendor-neutral channel-host ports/helpers: identity lookup, `ChannelDeliveryProtocol` seam, outbound-target provider port, ingress projection + webhook rate/error helpers, host-state JSON records, auth-continuation + paired-status ports. | Concrete channel strings/adapters, the delivery observer/driver (composition-owned), mount assembly or server lifecycle. |
| `ironclaw_telegram_extension` | `ironclaw_telegram_extension/AGENTS.md` | The Telegram channel host domain (single `telegram` extension): bot setup + Bot API client, filesystem host state, WebGeneratedCode pairing, DM-only dispatch pre-router, manifest-projected webhook serve fragment + dynamic installation resolver, actor identity, admin/pairing routes, connectable/connection facades, outbound DM targets, `TelegramDeliveryProtocol`. | `RebornRuntime` wiring/mount assembly (composition's `telegram_host_beta`), the delivery observer/driver, other channels' code. |
