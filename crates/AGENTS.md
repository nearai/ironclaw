...
| `ironclaw_release_migration` | `Cargo.toml`, `src/lib.rs`, `docs/internal/rc1-to-1.1-startup-migration.md` | App-level coordinator for the bounded `1.0.0-rc.1` to `1.1.0-rc.1` startup migration: database-wide lease/barrier, ordered domain transforms, cross-domain read-back, and redacted completion evidence. | Owning domain transforms, general versioned schema evolution, or long-lived composition/runtime policy. |
