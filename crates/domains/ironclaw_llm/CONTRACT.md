...
**HTTP request timeout:** Non-streaming NEAR AI requests have a 60-second total timeout (`DEFAULT_REQUEST_TIMEOUT_SECS` in `config.rs`). Streaming requests start one absolute progress deadline before sending the request; that same deadline covers response headers, non-success response body reads, and the wait for the first semantic model progress. After progress, it is re-armed only by nonempty text, reasoning, tool-call id/name/argument fragments, or a terminal finish reason. Empty SSE/keepalive events and usage-only frames do not reset it. There is no total wall-clock timeout; active long answers must not be cancelled merely because they exceed 60 seconds. The 10 s connect timeout and 30 s TCP keepalive from the shared hardened client also apply (see "Shared client timeout hygiene" below). Rate limit `Retry-After` headers are parsed (both delay-seconds and HTTP-date formats) and forwarded as `LlmError::RateLimited { retry_after }` for the `RetryProvider` to honor.
...
- `model_metadata().context_length` is now consumed at runtime: `ironclaw_loop_host`'s `LlmProviderModelGateway::advertised_context_window_tokens` reads it — only when the returned `ModelMetadata::id` matches the model the request will actually be served by — to derive the per-run prompt context budget. A provider that populates `context_length` therefore changes prompt sizing and compaction thresholds for runs served by that model; a provider that leaves it `None` keeps the compiled-in default. A provider that may serve a call from more than one model (routing, fan-out) advertises the smallest window among them, and a table lookup returns `None` for a model it does not know — a guessed window is worse than none.
