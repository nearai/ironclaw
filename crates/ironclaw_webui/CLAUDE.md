...
last-admin protection). Private-user and managed-agent creation return only the
user record, never a bearer credential. Managed-resource operations use the
separate canonical target-policy gate. `webui.v2.settings.tools` is a normal authenticated-caller
