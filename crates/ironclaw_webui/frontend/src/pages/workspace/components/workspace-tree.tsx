...
import { Skeleton } from "@ironclaw/ui";
...
const TreeNode = React.memo(function WorkspaceTreeNode({
  ...
  if (entry.is_dir) {
    ...
    return (
      <div
        ...
        className="outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_oklab,var(--v2-accent-text)_70%,transparent)] focus-visible:ring-inset"
      >
        <div
          className={[
        ...
        {isExpanded && (
          <div role="group" className="space-y-1">
            {childQuery.isLoading
              ? (<div role="status" className="px-4 py-2 text-xs text-[var(--v2-text-muted)]">{t("workspace.loading")}</div>)
              : childQuery.isError
  ...
  return (
    <div
      ...
      className={[
        "flex min-h-8 w-full cursor-pointer items-center gap-2 rounded-md px-2 text-left text-sm outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_oklab,var(--v2-accent-text)_70%,transparent)] focus-visible:ring-inset",
        selectedPath === entry.path
          ? "bg-[var(--v2-accent-soft)] text-[var(--v2-accent-text)] shadow-[inset_2px_0_0_currentColor]"
