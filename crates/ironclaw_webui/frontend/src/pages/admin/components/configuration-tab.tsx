...
import { Button, Input, Panel } from "@ironclaw/design-system";
...
export function AdminConfigurationTab() {
  ...
  return (
    <section className="space-y-5" data-testid="admin-configuration-page">
      <header>
        <p className="font-mono text-[11px] uppercase tracking-[var(--v2-tracking-caps)] text-[var(--v2-accent-text)]">Admin</p>
        <h1 className="mt-2 text-xl font-medium text-[var(--v2-text-strong)]">Extension configuration</h1>
        <p className="mt-2 max-w-3xl text-sm text-[var(--v2-text-muted)]">
    ...
    <Panel className="p-5 sm:p-6" data-testid="admin-configuration-group">
      <form onSubmit={submit}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-medium text-[var(--v2-text-strong)]">{group.display_name}</h2>
              <span className={group.complete ? "text-xs text-[var(--v2-positive-text)]" : "text-xs text-[var(--v2-warning-text)]"}>
                {group.complete ? "Configured" : "Configuration required"}
              </span>
            </div>
            {group.description && <p className="mt-1 text-sm text-[var(--v2-text-muted)]">{group.description}</p>}
          ...
          {group.fields.map((field) => {
            ...
            return (
              <div key={field.handle}>
                <label htmlFor={`${group.group_id}-${field.handle}`} className="mb-1 block text-xs text-[var(--v2-text-muted)]">
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          {saved && <span className="text-sm text-[var(--v2-positive-text)]" role="status">Configuration saved.</span>}
