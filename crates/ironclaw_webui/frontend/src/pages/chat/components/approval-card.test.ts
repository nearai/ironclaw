...
function findPrimaryOnClick(node) {
  ...
  if (
    ...
    node.values?.includes("primary")
  ) {
    const index = node.strings.findIndex((value) => value.includes("onClick="));
    if (index !== -1 && typeof node.values[index] === "function") return node.values[index];
  }
  for (const value of node.values || []) {
