...
export function RunDots({ runs = [] }) {
  ...
  if (!visibleRuns.length) {
    return (<span className="text-xs text-[var(--v2-text-faint)]">{t("automations.table.noRuns")}</span>);
  }
  ...
  return (
    ...
    >
      {visibleRuns.map((run) => (
        <span
          ...
          className={cn(
            ...
            run.status === "running" &&
              "border-[color-mix(in_srgb,var(--v2-info-text)_60%,var(--v2-panel-border))] bg-[var(--v2-info-text)]",
            run.status === "unknown" &&
      ...
      {overflow > 0 &&
      (<span
        className="ml-0.5 font-mono text-[11px] text-[var(--v2-text-faint)]"
...
export function RunHistorySummary({ runs = [], className = "" }) {
  ...
  if (!view.total) {
    return (<span className={cn("text-[11px] text-[var(--v2-text-faint)]", className)}>
  ...
  return (
    <div className="grid gap-3 border-b border-[var(--v2-panel-border)] py-3 last:border-0 sm:grid-cols-[6.5rem_minmax(0,1fr)_auto] sm:items-center">
      ...
      <div className="min-w-0">
        <div className="text-sm font-medium text-[var(--v2-text-strong)]">{run.fired_label}</div>
