...
export function CommandPalette({ open, onClose, threadsState, onNewChat, onToggleTheme }) {
  ...
  return (
    <div className="fixed inset-0 z-[var(--v2-z-modal)] flex items-start justify-center p-4 pt-[12vh]" role="dialog" aria-modal="true" aria-label={t("command.dialogLabel")}>
      <button type="button" aria-label={t("common.close")} onClick={onClose} className="absolute inset-0 bg-[var(--v2-scrim)]"></button>
          ...
          {filtered.map((command, index) => {
            ...
            return (
              <React.Fragment key={command.id}>
                {showGroup &&
                (<li key={`g-${command.group}`} className="px-2 pb-1 pt-2 text-[10px] font-medium uppercase tracking-[var(--v2-tracking-caps)] text-[var(--v2-text-faint)]">{command.group}</li>)}
                <li>
                  <button
                    type="button"
