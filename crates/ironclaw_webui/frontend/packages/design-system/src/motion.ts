...
/**
 * Reduced-motion hook — framer-free (a `matchMedia` subscription via
 * `useSyncExternalStore`) so that importing motion constants never pulls
 * the framer-motion runtime into a route's initial bundle. Under
 * prefers-reduced-motion, movement is dropped but opacity fades are kept
 * (gentler, not zero — see the Motion policy in app.css and Emil
 * Kowalski's reduced-motion guidance).
 */
