...
///
/// Fallible on purpose. The previous signature swallowed a serialization
/// failure into an empty `Vec<u8>`, which is a *valid-looking* proof that then
/// fails somewhere downstream with a misleading error instead of at the point
/// of failure. Serialization failure is close to unreachable here (the payload
/// is owned `String`/`Vec<u8>` fields), which is exactly why it should surface
/// rather than be silently encoded as "no proof".
pub fn encode_injected_proof(
    payload: &InjectedProofPayload,
    ...
    serde_json::to_vec(payload).map_err(|e| SigningProviderError::Provider {
        reason: format!("failed to encode injected proof payload: {e}"),
    })
}
