...
//!    [`ironclaw_turns::run_profile::sanitize_model_visible_text`], which misses
//!    all of those shapes. The registry pass *redacts in place* (it never blocks
//!    the whole string) so the descriptive cause survives with only secret
//!    values masked.
...
//! so this scrubbing lives here (loop support already depends on both) and at
//! the `ironclaw_reborn` composition layer — never in the contract crate.
