        ...
        Ok(ListPendingApprovalsResponse {
            approvals: self
                ...
                .map(|(gate_ref, run_id)| PendingApprovalInteractionView {
                    ...
                    action: ironclaw_product::ApprovalInteractionActionView::Other,
                })
                .collect(),
        })
    }
...
async fn actor_user_resolver_rechecks_revocation_before_turn_submission() {
    ...
    let err = workflow
        .submit_inbound(sample_envelope("resolver-revoked-mid-resolution"))
        .await
...
async fn concrete_product_surface_bot_mention_uses_shared_route() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![ironclaw_product::ProductConversationRouteKind::Shared]
    );
}
...
async fn concrete_product_surface_reuses_prepared_binding_for_content_only_policy_rewrite() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![ironclaw_product::ProductConversationRouteKind::Direct]
    );
}
...
async fn concrete_product_surface_recomputes_route_after_policy_rewrites_trigger() {
    ...
    assert_eq!(
        binding.route_kinds(),
        vec![
            ironclaw_product::ProductConversationRouteKind::Direct,
            ironclaw_product::ProductConversationRouteKind::Shared,
        ]
    );
}
