...
use ironclaw_product::{
    ...
    ChannelAuthAccountState, ChannelConnectionRequirement, ChannelConnectionService,
    CodexLoginStart, CommandResultView, EXTENSION_IMPORT_CAPABILITY_ID,
    ...
    StaticOperatorStatusService, THREAD_ARTIFACT_MAX_MESSAGES, THREAD_ARTIFACT_VIEW,
...
fn operator_config_entry_value<'a>(
    response: &'a ironclaw_product::RebornOperatorConfigListResponse,
    key: &str,
) -> &'a serde_json::Value {
    &response
        .entries
...
async fn get_operator_setup_returns_snapshot_from_llm_config() {
    ...
    assert!(response.steps.iter().any(|step| {
        step.name == "profile"
            && step.status == ironclaw_product::RebornOperatorSetupStepStatus::Complete
    }));
    assert!(response.steps.iter().any(|step| {
...
async fn run_operator_setup_ignores_redacted_webui_access_token_sentinel() {
    ...
    assert_eq!(
        webui_step.status,
        ironclaw_product::RebornOperatorSetupStepStatus::Complete
    );
    assert_eq!(
        llm_config.set_active_calls.lock().expect("lock").as_slice(),
        [SetupSetActiveCall {
