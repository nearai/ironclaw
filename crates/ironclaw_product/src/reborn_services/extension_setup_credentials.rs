...
> {
    ...
    for requirement in requirements {
        ...
        secrets.push(RebornExtensionSetupSecret {
            ...
            // An account row is not readiness -- only a `Configured` one is.
            provided: account
                .as_ref()
...
async fn submit_manual_token_requirement(
    ...
    let existing =
        credential_status_for_requirement_strict(service, scope.clone(), extension_id, requirement)
            .await?;
    // Only a usable account lets an omitted secret pass: an expired or revoked
    // row must still demand a new token.
...
mod tests {
    ...
    async fn project_preserves_retryable_unavailable_credential_status_as_unknown() {
        ...
        // A retryable outage is not a missing credential: it must not be
        // projected as `MissingRequired` and force a spurious setup prompt.
