...
use super::{
    ChannelInboundSurfaceOutcome, ChannelInboundSurfaceRequest, ProductSurface,
    ProductSurfaceError, ProjectFsFile, RebornAttachmentBytes, WebUiAuthenticatedCaller,
};
...
impl<Params, Output> ProductOperation<Params, Output> {
    ...
    pub fn request(&self, input: Params) -> Result<ProductOperationRequest, ProductSurfaceError>
    where
        Params: Serialize,
    {
        Ok(ProductOperationRequest {
            operation_id: self.id.as_str().to_string(),
            input: serde_json::to_value(input).map_err(ProductSurfaceError::internal_from)?,
            typed_input: None,
        })
    }
}
...
impl ProductOperationResponse {
    pub fn json<T: Serialize>(value: T) -> Result<Self, ProductSurfaceError> {
        Ok(Self::Json(
    ...
    pub fn into_json<T: DeserializeOwned>(self) -> Result<T, ProductSurfaceError> {
        match self {
            Self::Json(value) => {
                serde_json::from_value(value).map_err(ProductSurfaceError::internal_from)
            }
            Self::ProjectFile(_) | Self::Attachment(_) | Self::ChannelInbound(_) => Err(
                ProductSurfaceError::internal_from("operation returned non-JSON"),
            ),
        }
    }

    pub fn into_project_file(self) -> Result<ProjectFsFile, ProductSurfaceError> {
        match self {
            Self::ProjectFile(file) => Ok(file),
            Self::Json(_) | Self::Attachment(_) | Self::ChannelInbound(_) => Err(
                ProductSurfaceError::internal_from("operation returned non-file result"),
            ),
        }
    }

    pub fn into_attachment(self) -> Result<RebornAttachmentBytes, ProductSurfaceError> {
        match self {
            Self::Attachment(bytes) => Ok(bytes),
            Self::Json(_) | Self::ProjectFile(_) | Self::ChannelInbound(_) => Err(
                ProductSurfaceError::internal_from("operation returned non-attachment bytes"),
            ),
        }
    }

    pub fn into_channel_inbound(self) -> Result<ChannelInboundSurfaceOutcome, ProductSurfaceError> {
        match self {
            Self::ChannelInbound(outcome) => Ok(*outcome),
            Self::Json(_) | Self::ProjectFile(_) | Self::Attachment(_) => Err(
                ProductSurfaceError::internal_from("operation returned non-channel result"),
            ),
        }
    }
}
