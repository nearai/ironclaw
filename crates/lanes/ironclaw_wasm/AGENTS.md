...
- Own and use the canonical WIT/component-model ABI in this crate's own `wit/` directory (`wit/tool.wit` and `wit/channel.wit`, both present today; `src/bindings.rs` reaches `wit/tool.wit` relative to the crate root, the wit-bindgen default). The directory moved here from the repo root under CHECKLIST WS4 / PROPOSAL §6.6.1: this crate is the ABI's owner, so the files live inside it and travel with it through the WS7 family move.
  ...
  - `wit/tool.wit` (`near:agent@0.4.0`) and `wit/channel.wit` (`near:agent@0.3.1`) are the *same* WIT package name at two versions, so the directory cannot be handed to bindgen as a directory — always name the single file.
  ...
  - `near:agent@0.4.0` moved `tool.wit` from the `invoke-json` blob ABI to a typed `wit-result`-shaped response (`WitToolOutcome::Success`/`Failure`); the runtime also accepts components compiled against the frozen legacy world (`near:agent@0.3.0`, `wit/legacy/tool_v0_3_0.wit`) through a binding-level fallback in `WitToolRuntime::prepare`, removed in PR 4.
