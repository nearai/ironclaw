...
- Own scoped secret metadata, credential mapping metadata, AES-256-GCM/HKDF encryption, encrypted-row repository boundaries, storage, and one-shot lease mechanics only.
...
- Concrete persistence adapters may implement `EncryptedSecretRepository`, but this crate must not depend on workflow/runtime/event/authorization/process crates or concrete Reborn filesystem/runtime crates.
