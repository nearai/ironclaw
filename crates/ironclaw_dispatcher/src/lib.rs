...
use ironclaw_host_api::{
    CapabilityDescriptor, CapabilityDispatchError, CapabilityDispatchFailureKind,
    CapabilityDispatchRequest, CapabilityDispatchResult, CapabilityDispatcher, CapabilityId,
    ExtensionId, ResourceEstimate, ResourceReceipt, ResourceScope, ResourceUsage, RuntimeKind,
};
    ...
    pub async fn dispatch_json(
        ...
        let Some(adapter) = self.runtime_adapters.get(&runtime) else {
            let kind = match runtime {
                RuntimeKind::Wasm | RuntimeKind::Script | RuntimeKind::Mcp => {
