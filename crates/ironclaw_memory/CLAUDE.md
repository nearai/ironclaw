...
- Metadata/search behavior should stay reuse-first: preserve current `DocumentMetadata`, `.config` inheritance, `skip_indexing`, `skip_versioning`, schema validation, FTS query escaping, and version hash semantics unless deliberately changed with tests/docs.
