...
pub fn http_mcp_endpoint(package: &ExtensionPackage) -> Option<McpEgressEndpoint> {
...
pub fn installed_local_mcp_loopback_target(
...
mod tests {
    ...
    #[test]
    fn installed_local_stdio_mcp_remains_denied() {
        ...
        let mut package = registry
            ...
            .expect("local provider")
            .clone();
        package.manifest.runtime = ExtensionRuntime::Mcp {
        ...
        assert!(http_mcp_endpoint(&package).is_none());
    }
