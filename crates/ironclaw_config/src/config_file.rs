//! Boot-time TOML config for the standalone IronClaw binary.
//!
...
//! `ironclaw_composition` (IronClawDriverChoice, IronClawHarnessId).
...
/// `ironclaw_composition::IronClawRuntimeApiVersion::V1`. A future
/// major bump fails parse closed; minor bumps are accepted.
pub const IRONCLAW_CONFIG_API_VERSION: &str = "ironclaw.runtime/v1";
...
pub struct IronClawConfigFile {
    ...
    /// Durable storage selection for production IronClaw boot.
    ///
    ...
    /// Per-slot LLM selection. Keyed by IronClaw model slot name. Today
...
pub struct BootSection {
    ...
    /// against `IronClawCompositionProfile`. Examples: `"local-dev"`,
...
pub struct DriversSection {
    ...
    /// `IronClawDriverChoice`: `"text_only"`, `"planned"`.
...
/// Durable storage backend names accepted by the IronClaw production boot config.
...
pub struct StorageSection {
    ...
    /// Environment variable name containing the IronClaw secret master key.
...
impl DefaultLlmSlotUpdateSession {
    pub fn default_llm_slot(
        &self,
    ) -> Result<Option<LlmSlotSelection>, IronClawConfigFileUpdateError> {
    ...
    ) -> Result<(), IronClawConfigFileUpdateError> {
...
impl GoogleOauthConfigUpdateSession {
    pub fn google_section(&self) -> Result<Option<GoogleSection>, IronClawConfigFileUpdateError> {
        let Some(table) = self
            .doc
            .get("google")
            .and_then(toml_edit::Item::as_table_like)
...
pub fn begin_google_oauth_config_update(
    path: &Path,
) -> Result<GoogleOauthConfigUpdateSession, IronClawConfigFileUpdateError> {
...
#[derive(Debug, Error)]
pub enum IronClawConfigFileError {
...
#[derive(Debug, Error)]
pub enum IronClawConfigFileUpdateError {
    ...
    Validate {
        path: PathBuf,
        source: Box<IronClawConfigFileError>,
    },
...
impl IronClawConfigFile {
    ...
    pub fn load(path: &Path) -> Result<Option<Self>, IronClawConfigFileError> {
        match fs::read_to_string(path) {
            ...
            Err(source) => Err(IronClawConfigFileError::Io {
    ...
    /// `--config-string`, tests).
    pub fn parse_text(text: &str, attributed_path: &Path) -> Result<Self, IronClawConfigFileError> {
        let parsed: Self =
            toml::from_str(text).map_err(|source| IronClawConfigFileError::Toml {
    ...
    fn validate(&self, attributed_path: &Path) -> Result<(), IronClawConfigFileError> {
        ...
        let check =
            |label: Cow<'static, str>, value: &str| -> Result<(), IronClawConfigFileError> {
                reject_inline_secret(label, value).map_err(|source| {
                    IronClawConfigFileError::InlineSecret {
                        path: path_str(),
                        source,
                    }
                })
        ...
        let check_non_empty_trimmed =
            |label: Cow<'static, str>, value: &str| -> Result<(), IronClawConfigFileError> {
                ...
                if value.trim().is_empty() {
                    return Err(IronClawConfigFileError::InvalidField {
                        path: path_str(),
        ...
        if let Some(budget) = &self.budget {
            ...
            for (label, value) in [
                ...
                if let Some(v) = value
                    ...
                    && v < 0.0
                {
                    return Err(IronClawConfigFileError::InvalidApiVersion {
                        path: path_str(),
...
pub fn begin_default_llm_slot_update(
    path: &Path,
) -> Result<DefaultLlmSlotUpdateSession, IronClawConfigFileUpdateError> {
    let lock_file = acquire_update_lock(path)?;
    let doc = load_edit_document(path)?;
...
fn acquire_update_lock(path: &Path) -> Result<fs::File, IronClawConfigFileUpdateError> {
    let lock_path = config_update_lock_path(path);
    if let Some(parent) = lock_path.parent() {
        fs::create_dir_all(parent).map_err(|source| IronClawConfigFileUpdateError::Lock {
    ...
    let file = fs::OpenOptions::new()
        ...
        .truncate(false)
        .open(&lock_path)
        .map_err(|source| IronClawConfigFileUpdateError::Lock {
            path: lock_path.clone(),
            source,
        })?;
    file.lock()
...
fn load_edit_document(
    path: &Path,
) -> Result<toml_edit::DocumentMut, IronClawConfigFileUpdateError> {
    match fs::read_to_string(path) {
        Ok(text) => text.parse::<toml_edit::DocumentMut>().map_err(|source| {
            IronClawConfigFileUpdateError::Parse {
                path: path.to_path_buf(),
                source,
            }
        }),
        ...
        Err(source) => Err(IronClawConfigFileUpdateError::Read {
            path: path.to_path_buf(),
            source,
        }),
    }
...
fn write_edit_document(
    ...
    IronClawConfigFile::parse_text(&text, path).map_err(|source| {
        IronClawConfigFileUpdateError::Validate {
            path: path.to_path_buf(),
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|source| IronClawConfigFileUpdateError::Write {
            path: parent.to_path_buf(),
            source,
        })?;
    }
    let mut tmp = tempfile::NamedTempFile::new_in(path.parent().unwrap_or_else(|| Path::new(".")))
        .map_err(|source| IronClawConfigFileUpdateError::Write {
            path: path.to_path_buf(),
            source,
        })?;
    ...
    tmp.persist(path)
        .map_err(|error| IronClawConfigFileUpdateError::Write {
            path: path.to_path_buf(),
            source: error.error,
        })?;
    Ok(())
...
fn validate_api_version(found: &str, path: &Path) -> Result<(), IronClawConfigFileError> {
    ...
    let major: u32 = major_str
        .parse()
        .map_err(
            |error: std::num::ParseIntError| IronClawConfigFileError::InvalidApiVersion {
                path: path.display().to_string(),
                found: found.to_string(),
    ...
    if let Some(minor_str) = parts.next() {
        let _minor: u32 = minor_str
            ...
            .map_err(|error: std::num::ParseIntError| {
                IronClawConfigFileError::InvalidApiVersion {
                    path: path.display().to_string(),
                    found: found.to_string(),
    ...
    if major != 1 {
        return Err(IronClawConfigFileError::IncompatibleApiVersion {
            path: path.display().to_string(),
            found: found.to_string(),
            expected: IRONCLAW_CONFIG_API_VERSION,
        });
    }
    Ok(())
}
...
fn validate_env_var_reference(
    ...
    value: &str,
    path: &Path,
) -> Result<(), IronClawConfigFileError> {
    ...
    Err(IronClawConfigFileError::InvalidField {
        path: path.display().to_string(),
...
mod tests {
    ...
    fn missing_file_is_ok_none() {
        ...
        let result = IronClawConfigFile::load(&path).expect("missing file must not error");
        assert!(result.is_none());
    }

    #[test]
    fn empty_file_parses_to_all_none() {
        let cfg = IronClawConfigFile::parse_text("", &attributed()).expect("empty TOML is valid");
...
"#;
        let cfg = IronClawConfigFile::parse_text(toml, &attributed()).expect("must parse");
...
"#,
    ...
    fn default_llm_update_rejects_malformed_existing_toml() {
        ...
        assert!(matches!(err, IronClawConfigFileUpdateError::Parse { .. }));
    }
...
"#,
    ...
    fn google_oauth_update_writes_new_section() {
        ...
        let cfg = IronClawConfigFile::load(&path)
            .expect("valid config")
            .expect("config present");
...
"#;
        let err = IronClawConfigFile::parse_text(toml, &attributed())
            .expect_err("unknown key must fail parse");
        assert!(matches!(err, IronClawConfigFileError::Toml { .. }));
    }
...
"#;
        ...
        assert!(matches!(err, IronClawConfigFileError::InlineSecret { .. }));
...
pool_max_size = 24
"#;
        let cfg = IronClawConfigFile::parse_text(toml, &attributed())
...
"#;
        ...
        assert!(matches!(err, IronClawConfigFileError::InvalidField { .. }));
        assert!(
...
"#;
        let err =
            IronClawConfigFile::parse_text(toml, &attributed()).expect_err("major bump must fail");
        assert!(matches!(
            err,
            IronClawConfigFileError::IncompatibleApiVersion { .. }
        ));
    }
...
"#;
    ...
    fn trigger_poller_absent_section_yields_none() {
        let cfg = IronClawConfigFile::parse_text("", &attributed()).expect("empty TOML must parse");
