...
pub const LEGACY_IRONCLAW_HOME_ENV: &str = "IRONCLAW_REBORN_HOME";

const V1_BASE_DIR_ENV: &str = "IRONCLAW_BASE_DIR";

/// Source used to resolve [`IronClawHome`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IronClawHomeSource {
    Env,
    LegacyEnv,
    Default,
    LegacyDefault,
...
impl IronClawHomeSource {
    pub fn label(self) -> &'static str {
        match self {
            Self::Env => IRONCLAW_HOME_ENV,
            Self::LegacyEnv => LEGACY_IRONCLAW_HOME_ENV,
...
/// Resolved, validated state root for the standalone IronClaw binary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IronClawHome {
    path: PathBuf,
    source: IronClawHomeSource,
}
...
impl IronClawHome {
    pub fn resolve_from_env() -> Result<Self, IronClawConfigError> {
        Self::resolve_from_env_parts_with_v1_base(
            ...
            env::var_os(LEGACY_IRONCLAW_HOME_ENV),
            env::var_os("HOME"),
    ...
    fn resolve_from_env_parts_with_v1_base(
        ...
        if let Some(raw_home) = ironclaw_home {
            ...
            return Ok(Self {
                path,
                source: IronClawHomeSource::Env,
            });
        }

        if let Some(raw_home) = legacy_ironclaw_home {
            validate_non_empty(&raw_home, LEGACY_IRONCLAW_HOME_ENV)?;
            let path = PathBuf::from(raw_home);
            validate_absolute(&path, LEGACY_IRONCLAW_HOME_ENV)?;
            validate_no_parent_components(&path, LEGACY_IRONCLAW_HOME_ENV)?;
            validate_not_root(&path, LEGACY_IRONCLAW_HOME_ENV)?;
            ...
            return Ok(Self {
                path,
                source: IronClawHomeSource::LegacyEnv,
            });
        }
        ...
        Err(first_error.unwrap_or(IronClawConfigError::MissingHome))
    }
    ...
    pub fn source(&self) -> IronClawHomeSource {
        self.source
    }
    ...
    /// `IronClawConfigFile::load` returns `Ok(None)` when it doesn't
...
fn validate_non_empty(value: &OsString, name: &'static str) -> Result<(), IronClawConfigError> {
    if value.as_os_str().is_empty() {
        return Err(IronClawConfigError::EmptyPath { name });
    }
    Ok(())
}
...
fn default_home_from_candidate(
    raw_home: OsString,
    label: &'static str,
) -> Result<(PathBuf, IronClawHomeSource), IronClawConfigError> {
    ...
    if legacy.exists() {
        Ok((legacy, IronClawHomeSource::LegacyDefault))
    } else {
        Ok((default, IronClawHomeSource::Default))
    }
}

fn validate_absolute(path: &Path, name: &'static str) -> Result<(), IronClawConfigError> {
    if !path.is_absolute() {
        return Err(IronClawConfigError::RelativePath {
            name,
...
fn validate_no_parent_components(
    path: &Path,
    name: &'static str,
) -> Result<(), IronClawConfigError> {
    if path
        .components()
        .any(|component| matches!(component, Component::ParentDir))
    {
        return Err(IronClawConfigError::ParentPath {
            name,
            path: path.to_path_buf(),
        });
    }
...
fn validate_not_v1_state_root(
    ...
    .flatten()
    {
        if paths_overlap(path, candidate) {
            return Err(IronClawConfigError::V1StateRoot {
                name: LEGACY_IRONCLAW_HOME_ENV,
                path: path.to_path_buf(),
            });
        }
    }
...
fn validate_not_root(path: &Path, name: &'static str) -> Result<(), IronClawConfigError> {
    if path.parent().is_none() {
        return Err(IronClawConfigError::RootPath {
            name,
            path: path.to_path_buf(),
        });
    }
...
/// Error returned when standalone IronClaw boot configuration is invalid.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum IronClawConfigError {
...
impl fmt::Display for IronClawConfigError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingHome => write!(
                formatter,
                "HOME or USERPROFILE must be set when {LEGACY_IRONCLAW_HOME_ENV} is unset"
            ),
...
impl error::Error for IronClawConfigError {}
