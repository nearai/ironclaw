//! First-run `config.toml` seeding for IronClaw runtime startup.
//!
...
use crate::{
    IRONCLAW_CONFIG_API_VERSION, IronClawConfigFile, IronClawConfigFileError, IronClawProfile,
};

#[derive(Debug, Error)]
pub enum IronClawConfigSeedError {
...
pub enum IronClawConfigSeedOutcome {
    Seeded,
    AlreadyPresent,
}
...
pub fn seed_default_config_file_if_missing(
    path: &Path,
) -> Result<IronClawConfigSeedOutcome, IronClawConfigSeedError> {
    let text = first_run_config_toml();
    IronClawConfigFile::parse_text(&text, path).map_err(|source| {
        IronClawConfigSeedError::Validate {
    ...
    if let Some(parent) = parent {
        fs::create_dir_all(parent).map_err(|source| IronClawConfigSeedError::CreateParent {
    ...
    let mut tmp = tempfile::NamedTempFile::new_in(parent).map_err(|source| {
        IronClawConfigSeedError::CreateTemp {
            path: path.to_path_buf(),
            source,
        }
    })?;
    tmp.write_all(text.as_bytes())
        .map_err(|source| IronClawConfigSeedError::WriteTemp {
    ...
    tmp.persist_noclobber(path).map_or_else(
        |error| {
            if error.error.kind() == std::io::ErrorKind::AlreadyExists {
                Ok(IronClawConfigSeedOutcome::AlreadyPresent)
            } else {
                Err(IronClawConfigSeedError::Persist {
                    path: path.to_path_buf(),
                    source: error.error,
                })
            }
        },
        |_| Ok(IronClawConfigSeedOutcome::Seeded),
    )
}

fn first_run_config_toml() -> String {
    let profile = IronClawProfile::default();
    format!(
...
mod tests {
    ...
    fn seed_default_config_file_writes_sparse_first_run_config() {
        ...
        let path = temp.path().join("ironclaw").join("config.toml");

        let outcome = seed_default_config_file_if_missing(&path).expect("seed should succeed");

        assert_eq!(outcome, IronClawConfigSeedOutcome::Seeded);
        ...
        let parsed = IronClawConfigFile::load(&path)
    ...
    fn seed_default_config_file_does_not_clobber_existing_config() {
        ...
        assert_eq!(outcome, IronClawConfigSeedOutcome::AlreadyPresent);
    ...
    fn seed_default_config_file_handles_concurrent_first_run_race() {
        ...
        assert_eq!(
            outcomes
                .iter()
                .filter(|outcome| **outcome == IronClawConfigSeedOutcome::Seeded)
                .count(),
            1,
        ...
        assert_eq!(
            outcomes
                .iter()
                .filter(|outcome| **outcome == IronClawConfigSeedOutcome::AlreadyPresent)
                .count(),
            outcomes.len() - 1,
    ...
    fn seed_default_config_file_reports_parent_creation_failure() {
        ...
        assert!(
            matches!(error, IronClawConfigSeedError::CreateParent { .. }),
