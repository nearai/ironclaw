...
//! invariant for blueprints — `docs/ironclaw/contracts/secrets.md` requires
...
//! `ironclaw run`. That way the muscle-memory carries straight
...
#[error(
    "config field `{label}` looks like inline secret material ({pattern}); \
     ...
     never paste the value directly (see docs/ironclaw/contracts/secrets.md, epic #3036)"
)]
