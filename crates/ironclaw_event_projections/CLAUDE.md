...
Product-facing read models over IronClaw durable event/audit logs.
...
- `PendingGateProjection`, a IronClaw turn-event read model over typed blocked/resume/terminal `TurnLifecycleEvent` facts.
...
Legacy root `src/gate/PendingGateStore` writers still exist for the pre-IronClaw engine path. Treat them as legacy engine owners until a composition adapter maps `PendingGateProjectionSink` rows into that store; do not add new direct IronClaw pending-gate writers outside the projection consumer.
