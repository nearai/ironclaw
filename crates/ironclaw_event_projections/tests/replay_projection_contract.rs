...
// arch-exempt: large_file, central replay projection contract coverage remains co-located, plan #1
#[tokio::test]
async fn replay_projection_keeps_nested_dispatch_failure_out_of_run_status() {
    let log = Arc::new(InMemoryDurableEventLog::new());
    let service = ReplayEventProjectionService::new(Arc::clone(&log));
    let thread_id = ThreadId::new("thread-nested-dispatch-failure").unwrap();
    let parent_invocation_id = InvocationId::new();
    let child_invocation_id = InvocationId::new();
    let parent_scope = scope_for_thread_with_invocation(thread_id.clone(), parent_invocation_id);
    let child_scope = scope_for_thread_with_invocation(thread_id, child_invocation_id);
