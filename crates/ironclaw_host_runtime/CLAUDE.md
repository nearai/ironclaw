...
- Built-in obligation handling here is limited to metadata-only `AuditBefore` and `ApplyNetworkPolicy` preflight; keep secret injection, audit-after, redaction, output limits, resource reservation, and scoped mounts fail-closed until their owning runtime/input/output plumbing exists.
