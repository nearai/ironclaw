...
#[tokio::test]
async fn apply_network_policy_obligation_fails_closed_for_wasm_without_http_client() {
    let (filesystem, package) =
        wasm_package_with_module(http_module_bytes("https://api.example.test/v1/echo", 0, ""));
    let registry = Arc::new(registry_with_package(package));
    let filesystem = Arc::new(filesystem);
    ...
    assert!(matches!(
        err,
        ironclaw_capabilities::CapabilityInvocationError::Dispatch(error)
            if error.kind == CapabilityDispatchFailureKind::Wasm
    ));
}
...
async fn apply_network_policy_obligation_fails_closed_for_script_runtime_without_network_plumbing()
{
    ...
    assert!(matches!(
        err,
        ironclaw_capabilities::CapabilityInvocationError::Dispatch(error)
            if error.kind == CapabilityDispatchFailureKind::Script
    ));
