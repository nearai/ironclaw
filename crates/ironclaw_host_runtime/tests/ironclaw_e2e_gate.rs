...
#[tokio::test]
async fn ironclaw_e2e_gate_invokes_script_through_host_runtime_with_status_events_and_resources() {
    ...
    let input = json!({
        ...
        "host_path_sentinel": "/private/tmp/ironclaw-e2e-gate"
    });
    let outcome = runtime
        .invoke_capability((
            context,
    ...
    for forbidden in [
        ...
        "/private/tmp/ironclaw-e2e-gate",
    ] {
        assert!(
            !serialized.contains(forbidden),
            "durable IronClaw E2E event replay leaked {forbidden}: {serialized}"
        );
    }
}

#[tokio::test]
async fn ironclaw_e2e_gate_blocks_for_approval_resumes_once_and_rejects_replay() {
    let fixture = approval_resume_fixture();
    let runtime = fixture.services.host_runtime_for_local_testing();
    let context = execution_context_without_grants();
    let scope = context.resource_scope.clone();
    let invocation_id = context.invocation_id;
    let input = json!({"message": "approval resume through IronClaw E2E gate"});
    ...
    let replay = runtime
        .resume_capability((
            ...
            script_capability_id(),
            ResourceEstimate::default(),
            json!({"message": "approval resume through IronClaw E2E gate"}),
        ))
        .await
        .unwrap();
...
#[tokio::test]
async fn ironclaw_e2e_gate_fails_unsupported_obligations_before_runtime_events_or_success() {
    let run_state = Arc::new(ironclaw_run_state::in_memory_backed_run_state_store());
...
#[tokio::test]
async fn ironclaw_e2e_gate_redacts_runtime_output_before_public_result() {
    let run_state = Arc::new(ironclaw_run_state::in_memory_backed_run_state_store());
    let events = InMemoryEventSink::new();
    let services = HostRuntimeServices::new(
        Arc::new(registry_with_manifest(SCRIPT_MANIFEST)),
...
#[tokio::test]
async fn ironclaw_e2e_gate_sanitizes_runtime_backend_failure_before_public_surfaces() {
    let run_state = Arc::new(ironclaw_run_state::in_memory_backed_run_state_store());
    let events = InMemoryEventSink::new();
    let services = HostRuntimeServices::new(
        Arc::new(registry_with_manifest(SCRIPT_MANIFEST)),
...
#[tokio::test]
async fn ironclaw_e2e_gate_blocks_oversized_runtime_output_before_publication() {
    let run_state = Arc::new(ironclaw_run_state::in_memory_backed_run_state_store());
    let events = InMemoryEventSink::new();
    let services = HostRuntimeServices::new(
        Arc::new(registry_with_manifest(SCRIPT_MANIFEST)),
...
#[tokio::test]
async fn ironclaw_e2e_gate_host_http_consumes_staged_policy_and_secret_once() {
    ...
    secret_store
        .put(
            scope.clone(),
            handle.clone(),
            SecretMaterial::from("sk-ironclaw-e2e-staged-secret"),
            None,
        )
        .await
        .unwrap();
    ...
    let response = service
        ...
        .await
        .expect("host HTTP egress should use staged IronClaw policy and secret material");
    assert_eq!(response.status, 200);
    {
        ...
        assert_eq!(
            ...
            Some(&(
                ...
                "Bearer sk-ironclaw-e2e-staged-secret".to_string()
            ))
        );
    }
    let replay = service
