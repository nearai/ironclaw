...
use ironclaw_turns::{SanitizedFailure, TurnRunId};
...
pub(super) fn result_to_outcome(
    ...
    match result {
        Ok(Ok(())) => {
            latency::operation_ok("execute_claimed_run", thread_id, run_id, started_at);
            ExecutorTaskOutcome::Completed
        }
        Ok(Err(error)) => {
            latency::operation_error("execute_claimed_run", thread_id, run_id, started_at, &error);
