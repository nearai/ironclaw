...
"#;
    ...
    // means in Rust (`FailureKind::fate`). Before this test the two never met:
    // the e2e matrix asserted a *string* in a preview payload, and the Rust
    // tests enumerated `FailureKind::ALL` synthetically without ever seeing a
    // profile. So no test said what `http_429` or `expired_credential` actually
    // does to a run.
    //
    ...
    //     -> WasmGuestErrorKind                  wasm_guest_error_kind (this file)
    //     -> DispatchError                       wasm_guest_dispatch_error (this file)
    ...
    /// One profile's expected journey through the classification chain.
    struct FaultProfileFate {
        /// Name as declared in `tests/e2e/provider_fault_proxy.py`.
        profile: &'static str,
        /// HTTP status the proxy responds with.
        status: u16,
        /// Kind tag the GitHub guest emits for that status.
        guest_kind: &'static str,
    ...
    const STATUS_PROFILE_FATES: &[FaultProfileFate] = &[
        ...
        // `ModelVisible`, an expired token stops prompting the user to
        // reconnect and starts being reported to the model as a tool failure.
        FaultProfileFate {
            profile: "http_401",
            status: 401,
            guest_kind: "auth_required",
        ...
        // 403 and 429 are deliberately NOT retried on the capability path. Note
        // this disagrees with the LLM path, where a rate limit IS retried with
        // backoff (`ironclaw_llm::retry::is_retryable`). The asymmetry is
        // intentional -- a provider 429 usually needs a changed plan, not a
        // faster loop -- but it is asserted here so it stays a decision.
        FaultProfileFate {
            profile: "http_403",
            status: 403,
            guest_kind: "client",
        ...
        // 5xx reaching the model rather than being retried is the surprising
        // row. The guest collapses every non-special status to
        // `operation_failed`, so a 500 is indistinguishable from a 404 by the
        // time the runtime sees it, and the loop's `Retry` fate is never
        // reached. Retrying a provider 5xx would need the guest to emit a
        // distinct kind first.
        FaultProfileFate {
            profile: "http_500",
            status: 500,
    ...
    /// Profiles that break the transport instead of returning a status.
    ///
    /// Their classification runs through `sanitize_host_error`, which matches
    /// on the *host's* error text, so the resulting kind cannot be derived from
    /// the profile declaration -- it takes a running host to know. They are
    /// listed rather than omitted so the inventory gate below stays honest
    /// about what is and is not pinned here; their coverage is the e2e
    ...
    fn guest_error_payload(status: u16, kind: &str) -> String {
        serde_json::json!({
    ...
    fn status_fault_profiles_classify_to_their_declared_fate() {
        let capability = CapabilityId::new("github.list_issues").unwrap();

        for case in STATUS_PROFILE_FATES {
            let payload = guest_error_payload(case.status, case.guest_kind);
            ...
            assert_eq!(
                ...
                "fault profile `{}` (HTTP {}) classified as {failure_kind:?}",
                case.profile, case.status
            );
            assert_eq!(
                failure_kind.fate(),
                case.expected_fate,
                "fault profile `{}` (HTTP {}) has fate {:?}",
                case.profile,
                case.status,
                failure_kind.fate()
            );
    ...
    fn a_401_parks_for_reauth_and_is_never_reported_as_a_tool_failure() {
        let capability = CapabilityId::new("github.list_issues").unwrap();
        let payload = guest_error_payload(401, "auth_required");
    ...
    #[test]
    fn every_declared_fault_profile_is_classified_or_named_as_transport_shaped() {
        // The denominator comes from the e2e catalogue, so adding a profile
        // there without deciding what it means here fails this test.
        let catalogue = include_str!("../../../../tests/e2e/provider_fault_proxy.py");
        let declared: Vec<&str> = catalogue
            .lines()
            .skip_while(|line| !line.contains("PROVIDER_FAULT_PROFILES"))
            .filter_map(|line| {
                let trimmed = line.trim();
                // Profile keys are the only `"name": ProviderFaultProfile(`
                // lines in the mapping.
                trimmed
                    .strip_suffix(": ProviderFaultProfile(")
                    .map(str::trim)
        ...
        for profile in declared {
            let classified = STATUS_PROFILE_FATES
                .iter()
                .any(|case| case.profile == profile)
                || TRANSPORT_SHAPED_PROFILES.contains(&profile);
            assert!(
                classified,
                "fault profile `{profile}` is declared in \
                 tests/e2e/provider_fault_proxy.py but no test says what it \
                 means to a run. Add it to STATUS_PROFILE_FATES with its \
                 expected FailureKind and fate, or to TRANSPORT_SHAPED_PROFILES \
                 if its classification depends on live host error text."
            );
        }
    }
}
