...
/// How the memory binding resolves, before constructing a provider.
...
pub struct MemoryServiceResolver {
    ...
    /// Registered third-party providers, keyed by extension id.
    /// Empty in the default/native configuration, so behavior is unchanged
    /// unless composition injects a provider (e.g. mem0).
    third_party_providers: BTreeMap<String, Arc<dyn MemoryService>>,
}
...
impl std::fmt::Debug for MemoryServiceResolver {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            ...
            .field("third_party_providers", &self.third_party_providers.len())
            .finish()
    }
}
...
impl MemoryServiceResolver {
    ...
    pub fn native() -> Self {
        Self {
            policy: None,
            third_party_providers: BTreeMap::new(),
        }
    }
    ...
    /// Register a concrete third-party provider for `extension_id`.
    ///
    /// When the resolved memory binding is
    /// `ThirdParty { extension_id }` and an instance is registered here under the
    /// same id, [`resolve_provider`](Self::resolve_provider) returns
    ...
    pub fn with_third_party_provider(
        mut self,
    ...
    /// Resolve the bound provider over a per-invocation filesystem.
    ///
    /// Returns `None` (fail-closed) when the memory binding is disabled,
    ...
    pub fn resolve_provider(
        ...
        match self.provider_resolution() {
            ProviderResolution::Native => Some(Arc::new(NativeMemoryService::from_filesystem(
                filesystem,
            ...
            // — a third-party memory binding does NOT get write-time
            ...
            ProviderResolution::ThirdParty(extension_id) => self
                .third_party_providers
                .get(&extension_id)
                .map(Arc::clone),
            ProviderResolution::Unavailable => None,
        }
    }
    ...
    fn provider_resolution(&self) -> ProviderResolution {
        let Some(policy) = &self.policy else {
            return ProviderResolution::Native;
        };
        match policy.binding() {
            MemoryProviderBinding::Native => ProviderResolution::Native,
            MemoryProviderBinding::ThirdParty { extension_id } => {
                ProviderResolution::ThirdParty(extension_id.as_str().to_string())
            }
            MemoryProviderBinding::Disabled => ProviderResolution::Unavailable,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn native_resolver_builds_a_provider() {
        let resolver = MemoryServiceResolver::native();
        assert!(resolver.resolve_provider(filesystem(), None).is_some());
        // Default == native.
        assert!(
    ...
    fn explicit_native_policy_builds_a_service() {
        let resolver = MemoryServiceResolver::from_policy(policy_with_provider("ironclaw.memory"));
        assert!(resolver.resolve_provider(filesystem(), None).is_some());
    }

    #[test]
    fn disabled_binding_resolves_to_none() {
        let resolver = MemoryServiceResolver::from_policy(policy_with_provider(
    ...
    fn third_party_binding_without_registered_provider_resolves_to_none() {
        ...
        let resolver = MemoryServiceResolver::from_policy(policy_with_provider("acme.honcho"));
        assert!(resolver.resolve_provider(filesystem(), None).is_none());
    }
    ...
    async fn third_party_binding_resolves_to_registered_provider() {
        ...
        let resolver = MemoryServiceResolver::from_policy(policy_with_provider("acme.honcho"))
            .with_third_party_provider("acme.honcho", Arc::new(MarkerProvider));
    ...
    fn registered_provider_for_a_different_id_still_fails_closed() {
        ...
        let resolver = MemoryServiceResolver::from_policy(policy_with_provider("acme.honcho"))
            .with_third_party_provider("other.provider", Arc::new(MarkerProvider));
        assert!(resolver.resolve_provider(filesystem(), None).is_none());
    }
}
