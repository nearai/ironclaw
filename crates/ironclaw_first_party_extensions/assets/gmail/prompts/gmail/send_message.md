...
Pass `message.to`, `message.subject`, and `message.body` exactly as the user requested. Use `message.cc`, `message.bcc`, or `message.from` only when the user explicitly provides those fields. Use `message.raw` only when the user or system provides a complete pre-encoded Gmail raw payload. Do not infer recipients or send content unless the user has clearly authorized the message.
