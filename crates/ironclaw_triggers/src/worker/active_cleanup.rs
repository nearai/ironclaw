...
impl TriggerPollerWorker {
    pub(super) async fn clear_terminal_active_fires(
        ...
        for item in lookup_items {
            ...
            // triggers this clears as failed so the schedule can advance to its
            // next slot. For CompleteAfterFirstFire triggers the gate is still
            // answerable and must NOT be cleared here — leaving it pending lets
            // the one-shot remain active until the gate is resolved, after which
            // the run reaches Terminal and the existing Terminal path clears it
            // and marks it Completed. Clearing a fire-once blocked run would
            // hide it as Completed even though it never fired successfully; and
            // setting it back to Scheduled would refire-loop since the one-shot
            // slot is already in the past. Terminal runs clear with whatever
            // status they reached. Exhaustive (no wildcard) so a new variant
            ...
            let clear: Option<(TriggerPollerFireOutcome, TriggerRunHistoryStatus)> = match state {
                ...
                TriggerActiveRunState::Blocked => match record.completion_policy {
                    crate::TriggerCompletionPolicy::Recurring => Some((
                        TriggerPollerFireOutcome::ClearedBlockedActive { run_id },
                        TriggerRunHistoryStatus::Error,
                    )),
                    crate::TriggerCompletionPolicy::CompleteAfterFirstFire => None,
                },
