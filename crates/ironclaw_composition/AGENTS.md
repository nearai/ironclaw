...
- Facade-shaped production composition root for IronClaw.
- Top-level factories that expose `HostRuntime`, `TurnCoordinator`, readiness, runtime/profile inputs, and LLM catalog wiring: `IronClawServices`/`build_ironclaw_services` (`factory`), `IronClawBuildInput`/`IronClawBuildError`, and the feature-gated LLM catalog resolvers (`llm_admin::llm_catalog`).
- The `IronClawRuntime` conversation-level facade (`IronClawRuntime`/`build_ironclaw_runtime`, `AssistantReply`, `ConversationId`, `IronClawRuntimeError`) and its runtime inputs (`IronClawRuntimeInput`/`IronClawRuntimeIdentity`, `TurnRunnerSettings`/`PollSettings`, heartbeat/poll-interval defaults).
...
- Live v1/product traffic routing; callers must opt into explicit IronClaw adapters.
...
- Run `scripts/ironclaw-e2e-rust.sh` for production wiring changes.
