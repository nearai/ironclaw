...
- Own only top-level IronClaw composition for production/app startup.
- Expose facade-shaped handles only: `HostRuntime`, `TurnCoordinator`, product-auth `IronClawProductAuthServices`, WebUI `ProductSurface`, readiness.
- Keep lower substrate handles private to factories and owning crates.
- Substrate handles MAY be exposed via `#[cfg(any(test, feature = "test-support"))]` pub accessors on `IronClawRuntime` when downstream integration tests need to drive production-shape state the facade doesn't yet surface (e.g. seeding `TriggerRecord` rows, `pair_external_actor` calls). These seams ship zero bytes in production binaries. New test-support accessors must carry a doc-comment naming the production call site they mirror and an explicit note that the handle is for tests only.
- Outbound state stores are composition-owned via `IronClawRuntimeSubstrate`; do not construct `FilesystemOutboundStateStore` in consumer modules (lint-enforced via `clippy::disallowed-methods`).
...
- Do not route live v1/product traffic here; callers must opt in through explicit IronClaw adapters.
...
- Product auth refresh and lifecycle cleanup callers should use
  `IronClawProductAuthServices::refresh_credential_account` and
...
- OAuth callback routes should only parse/validate HTTP input and call
  `IronClawProductAuthServices` for flow preflight/callback handling; the
...
- Manual-token setup routes should call
  `IronClawProductAuthServices::request_manual_token_setup` for the typed
  challenge and `IronClawProductAuthServices::submit_manual_token` with a
  one-shot `IronClawManualTokenSubmitRequest` for the dedicated secret-submit
...
- `IronClawProductAuthServices::flow_record_source` is an optional WebUI/local-dev
...
The IronClaw-side host composition for the WebChat v2 HTTP gateway lives
in this crate. Implements Path A of
`docs/ironclaw/how-to-port-channel-to-ironclaw.md` (native host-owned
...
| Symbol | Role |
|---|---|
| `IronClawWebuiBundle` (in [`src/webui/facade.rs`](src/webui/facade.rs)) | `{ api: Arc<dyn ProductSurface>, product_auth: Option<Arc<IronClawProductAuthServices>>, readiness }` — the v2 product surface, optional product-auth route service, plus readiness snapshot |
| `build_webui_services(runtime, event_stream)` | Compose a `IronClawWebuiBundle` from an already-built `IronClawRuntime`; reuses the runtime's thread service / turn coordinator, product-auth services, and runtime-owned `EventStreamManager` projection stream unless a caller supplies a custom stream |
| `IronClawProjectionServices` (in `src/projection.rs`) | Runtime-owned projection/event-stream composition; owns the single local-dev `EventStreamManager` and creates product-specific `ProjectionStream` adapters over it |
...
| `ProtectedRouteMount` | Host-supplied protected API route fragment merged inside the WebUI bearer-auth layer with descriptor-driven body/rate limits. IronClaw OpenAI-compatible routes use this seam; do not use it for v1 gateway routers. |
...
IronClaw-native product-auth surface:
...
- `GET /api/product-auth/oauth/callback/{flow_id}` is a hosted
  ...
  `IronClawProductAuthServices` for flow preflight/callback handling. The route never
...
- `POST /api/product-auth/manual-token/submit` is inside the
  ...
  `IronClawProductAuthServices`, and returns the resulting
...
IronClaw user through the host-owned identity binding store. Slack personal setup is OAuth-only; the old browser
...
IronClaw-native auth router. v1 gateway code remains untouched —
...
Mapping of every v1 gateway entrypoint to its IronClaw native-surface
...
| Operator status/readiness | v1 doctor/readiness surfaces | `GET /api/webchat/v2/operator/status` | Mapped to IronClaw readiness projection through the product facade; left unmounted with other operator routes for multi-user authenticators |
...
| Operator service lifecycle | `src/cli/service.rs` command path | `POST /api/webchat/v2/operator/service` | Mapped to a IronClaw composition lifecycle backend; launchd/systemd user services are supported, other OS targets report unsupported |
...
- **Bearer / OIDC / cookie auth** — none of these are shared with v1's
  `auth_middleware`. The IronClaw binary owns its own
...
- **CORS** — `CorsLayer` allow-origin = `config.allowed_origins`. The
  IronClaw `serve` subcommand should set this to the bound listener's
...
Per Path A in `docs/ironclaw/how-to-port-channel-to-ironclaw.md`:
...
The `serve` subcommand builds a full local-dev `IronClawRuntime`, asks
...
// `ironclaw_product_api_crates_do_not_bind_http_ingress` forbids
// product/API crates from owning server lifecycle).
let runtime = build_ironclaw_runtime(input).await?;
