...
const EMBEDDED_REBORN_SKILL_SUMMARIES_JSON: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/embedded_ironclaw_skill_summaries.json"
));
const EMBEDDED_REBORN_SKILL_BUNDLES_JSON: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/embedded_ironclaw_skill_bundles.json"
));
...
struct EmbeddedIronClawSkillSummary {
    name: String,
    version: String,
    description: String,
    keywords: Vec<String>,
...
#[derive(Debug, Deserialize)]
struct EmbeddedIronClawSkillBundle {
    name: String,
    files: Vec<EmbeddedIronClawSkillFile>,
}

#[derive(Debug, Deserialize)]
struct EmbeddedIronClawSkillFile {
    path: String,
    bytes: Vec<u8>,
}
...
pub(crate) async fn ensure_bundled_ironclaw_skills_installed(
    local_dev_storage_root: &Path,
) -> Result<(), IronClawBuildError> {
    let bundled_skills = embedded_ironclaw_skill_bundles()?;
...
pub(crate) fn bundled_ironclaw_skill_summaries() -> Result<Vec<SkillSummary>, IronClawBuildError> {
    Ok(embedded_ironclaw_skill_summaries()?
        .into_iter()
...
fn embedded_ironclaw_skill_summaries()
-> Result<Vec<EmbeddedIronClawSkillSummary>, IronClawBuildError> {
...
fn embedded_ironclaw_skill_bundles() -> Result<Vec<EmbeddedIronClawSkillBundle>, IronClawBuildError>
{
...
fn reject_existing_symlink(path: &Path, label: &str) -> Result<(), IronClawBuildError> {
...
fn system_skills_root_path() -> Result<VirtualPath, IronClawBuildError> {
...
impl BundledSkillInstallLock {
    ...
    async fn release(self, filesystem: &dyn RootFilesystem) -> Result<(), IronClawBuildError> {
...
async fn install_bundled_skill(
    filesystem: &dyn RootFilesystem,
    system_skills_root: &VirtualPath,
    skill: EmbeddedIronClawSkillBundle,
) -> Result<(), IronClawBuildError> {
...
async fn read_managed_marker(
    filesystem: &dyn RootFilesystem,
    skill_dir: &VirtualPath,
) -> Result<Option<BundledSkillMarker>, IronClawBuildError> {
...
async fn path_exists(
    filesystem: &dyn RootFilesystem,
    path: &VirtualPath,
) -> Result<bool, IronClawBuildError> {
...
fn child_path(parent: &VirtualPath, child: &str) -> Result<VirtualPath, IronClawBuildError> {
...
fn validated_bundle_file_path(path: &str) -> Result<PathBuf, IronClawBuildError> {
...
fn bundled_skill_hash(skill: &EmbeddedIronClawSkillBundle) -> String {
...
fn invalid_config(reason: impl std::fmt::Display) -> IronClawBuildError {
...
mod tests {
    ...
    /// Zero-legacy gate for embedded skill guidance: the IronClaw binary embeds
    ...
    /// IronClaw automation conversation its keywords match. The automation
    /// advisor must teach the IronClaw capability surface instead.
    #[test]
    fn embedded_skills_teach_ironclaw_trigger_tools_not_retired_v1_routines() {
        let bundles = embedded_ironclaw_skill_bundles().expect("embedded bundles parse");
        ...
        assert!(
            ...
            "routine-advisor must teach the IronClaw trigger_create capability"
        );
        assert!(
    ...
    #[tokio::test]
    async fn bundled_ironclaw_skills_include_current_repo_bundles_and_assets() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local_dev_root = dir.path().join("local-dev");

        ensure_bundled_ironclaw_skills_installed(&local_dev_root)
            .await
    ...
    #[tokio::test]
    async fn bundled_ironclaw_skills_do_not_overwrite_unmanaged_system_skills() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local_dev_root = dir.path().join("local-dev");
    ...
    #[tokio::test]
    async fn bundled_ironclaw_skills_skip_unchanged_managed_dirs() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local_dev_root = dir.path().join("local-dev");
    ...
    #[tokio::test]
    async fn bundled_ironclaw_skills_replace_changed_managed_dirs() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local_dev_root = dir.path().join("local-dev");
        let skill_dir = local_dev_root.join("system/skills/code-review");
    ...
    #[tokio::test]
    async fn bundled_ironclaw_skills_remove_stale_managed_dirs() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local_dev_root = dir.path().join("local-dev");
