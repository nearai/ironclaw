...
pub(crate) struct IronClawChannelConfigFacade {
    service: Arc<ChannelConfigService>,
}

impl IronClawChannelConfigFacade {
...
#[async_trait]
impl ironclaw_product_workflow::ChannelConfigFacade for IronClawChannelConfigFacade {
    async fn field_status(
        &self,
        extension_id: &ExtensionId,
    ) -> Result<Vec<ironclaw_product_workflow::IronClawChannelConfigField>, IronClawServicesError>
    {
        match self.service.status(extension_id).await {
            Ok(statuses) => Ok(statuses
                .into_iter()
                .map(
                    |status| ironclaw_product_workflow::IronClawChannelConfigField {
                        name: status.handle,
                        label: status.label,
...
fn map_channel_config_error(error: ChannelConfigError) -> IronClawServicesError {
    use ironclaw_product_workflow::{IronClawServicesErrorCode, IronClawServicesErrorKind};
    match error {
        ChannelConfigError::NotInstalled { .. } => IronClawServicesError {
        ...
        ChannelConfigError::UnknownField { .. } => IronClawServicesError {
        ...
        ChannelConfigError::Storage { .. } => IronClawServicesError {
        ...
        ChannelConfigError::Reactivation { .. } => IronClawServicesError {
