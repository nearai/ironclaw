...
use crate::{IronClawUserIdentityLookup, IronClawUserIdentityLookupError};
...
async fn slack_dm_delivers_final_reply_after_immediate_ack() {
    let harness = build_harness(TurnMode::Complete {
        assistant_text: "hello from IronClaw".into(),
    })
    .await;
    ...
    assert_eq!(messages[0]["text"], "hello from IronClaw");
}

#[tokio::test]
async fn slack_dm_for_personally_bound_user_routes_through_ironclaw_identity() {
    let harness = build_harness(TurnMode::Complete {
...
#[async_trait]
impl IronClawUserIdentityLookup for RecordingUserIdentityLookup {
