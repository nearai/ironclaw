...
use crate::extension_host::bundled_skills::bundled_ironclaw_skill_summaries;
use crate::extension_host::lifecycle::{
    IronClawLocalSkillManagementError, build_existing_local_dev_skill_management_port,
};

pub async fn list_ironclaw_local_skills(
    owner_id: impl Into<String>,
    local_dev_storage_root: impl Into<std::path::PathBuf>,
) -> Result<Vec<ironclaw_skills::SkillSummary>, IronClawSkillListError> {
    let mut skills =
    ...
    let bundled_skills = bundled_ironclaw_skill_summaries()?;
...
pub enum IronClawSkillListError {
    #[error(transparent)]
...
) -> IronClawSkillListError {
    match error {
        IronClawLocalSkillManagementError::InvalidContext { reason } => {
            IronClawSkillListError::InvalidRequest { reason }
        }
        IronClawLocalSkillManagementError::Skill(error) => map_skill_management_error(error),
    }
}

fn map_skill_management_error(error: SkillManagementError) -> IronClawSkillListError {
    match error.kind() {
        SkillManagementErrorKind::InvalidInput
        | SkillManagementErrorKind::NotFound
        | SkillManagementErrorKind::Conflict
        | SkillManagementErrorKind::InvalidSkill => IronClawSkillListError::InvalidRequest {
            reason: error
                .reason()
                .unwrap_or("skill management request rejected")
                .to_string(),
        },
        SkillManagementErrorKind::FilesystemDenied => IronClawSkillListError::AccessDenied,
        SkillManagementErrorKind::Resource => IronClawSkillListError::Unavailable {
...
mod tests {
    ...
    #[tokio::test]
    async fn local_skill_list_lists_all_skills_from_ironclaw_storage() {
        ...
        let result = list_ironclaw_local_skills("list-owner", &storage_root)
            .await
            .expect("list skills");
    ...
    async fn local_skill_list_rejects_non_directory_storage_root() {
        ...
        let error = match list_ironclaw_local_skills("list-owner", &storage_root).await {
        ...
        assert!(
            matches!(
                error,
                IronClawSkillListError::Build(IronClawBuildError::InvalidConfig { .. })
            ),
    ...
    async fn local_skill_list_prefers_embedded_bundled_summary_over_storage_system_skill() {
        ...
        let bundled_code_review = bundled_ironclaw_skill_summaries()
