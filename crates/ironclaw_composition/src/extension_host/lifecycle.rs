...
use crate::extension_host::extension_lifecycle::IronClawLocalExtensionManagementPort;
...
#[derive(Clone)]
pub(crate) struct IronClawLocalSkillManagementPort {
    owner_user_id: UserId,
    filesystem: Arc<dyn RootFilesystem>,
    skill_management_mount_resolver: Arc<SkillManagementMountResolver>,
}

impl IronClawLocalSkillManagementPort {
    #[cfg(test)]
    pub(crate) fn new(
        owner_user_id: UserId,
        filesystem: Arc<dyn RootFilesystem>,
    ...
    pub(crate) fn owner_scope(&self) -> Result<ResourceScope, IronClawLocalSkillManagementError> {
    ...
    fn skill_context_for_scope(
        &self,
        scope: ResourceScope,
    ) -> Result<SkillManagementContext, IronClawLocalSkillManagementError> {
        let mounts =
    ...
    pub(crate) async fn list_for_scope(
        &self,
        scope: ResourceScope,
    ) -> Result<Vec<ironclaw_skills::SkillSummary>, IronClawLocalSkillManagementError> {
    ...
    pub(crate) async fn search_for_scope(
        ...
        query: &str,
        limit: usize,
    ) -> Result<ironclaw_skills::SkillSearchResult, IronClawLocalSkillManagementError> {
        let context = self.skill_context_for_scope(scope)?;
    ...
    pub(crate) async fn read_content_for_scope(
        ...
        scope: ResourceScope,
        name: &str,
    ) -> Result<ironclaw_skills::SkillContentResult, IronClawLocalSkillManagementError> {
        let context = self.skill_context_for_scope(scope)?;
    ...
    pub(crate) async fn update_for_scope(
        ...
        name: &str,
        content: &str,
    ) -> Result<ironclaw_skills::SkillUpdateResult, IronClawLocalSkillManagementError> {
        let context = self.skill_context_for_scope(scope)?;
    ...
    pub(crate) async fn install_for_scope(
        ...
        name: Option<&str>,
        content: &str,
    ) -> Result<ironclaw_skills::SkillInstallResult, IronClawLocalSkillManagementError> {
        let context = self.skill_context_for_scope(scope)?;
        Ok(install_skill(
            &context,
            SkillInstallRequest {
    ...
    pub(crate) async fn remove_for_scope(
        ...
        scope: ResourceScope,
        name: &str,
    ) -> Result<ironclaw_skills::SkillRemoveResult, IronClawLocalSkillManagementError> {
        let context = self.skill_context_for_scope(scope)?;
...
pub(crate) enum IronClawLocalSkillManagementError {
...
impl From<SkillManagementError> for IronClawLocalSkillManagementError {
...
pub(crate) fn build_local_skill_management_port<F>(
    owner_user_id: UserId,
    filesystem: Arc<F>,
) -> Result<Arc<IronClawLocalSkillManagementPort>, crate::IronClawBuildError>
where
    F: RootFilesystem + 'static,
{
    ...
    Ok(Arc::new(
        IronClawLocalSkillManagementPort::new_with_mount_resolver(
            owner_user_id,
            filesystem,
            mount_resolver,
        ),
...
pub(crate) fn build_existing_local_dev_skill_management_port(
...
) -> Result<Option<Arc<IronClawLocalSkillManagementPort>>, crate::IronClawBuildError> {
    let owner_id = owner_id.into();
    let local_dev_storage_root = local_dev_storage_root.into();
    if !local_dev_storage_root.try_exists().map_err(|error| {
        crate::IronClawBuildError::InvalidConfig {
    ...
    if !local_dev_storage_root.is_dir() {
        return Err(crate::IronClawBuildError::InvalidConfig {
    ...
    let owner_user_id =
        UserId::new(owner_id).map_err(|error| crate::IronClawBuildError::InvalidConfig {
            reason: error.to_string(),
        })?;
    build_local_skill_management_port(owner_user_id, Arc::new(filesystem)).map(Some)
}

fn invalid_skill_context(error: impl std::fmt::Display) -> IronClawLocalSkillManagementError {
    IronClawLocalSkillManagementError::InvalidContext {
        reason: error.to_string(),
    }
}

#[derive(Clone)]
pub(crate) struct IronClawLocalLifecycleFacade {
    skill_management: Arc<IronClawLocalSkillManagementPort>,
    extension_management: Option<Arc<IronClawLocalExtensionManagementPort>>,
...
impl IronClawLocalLifecycleFacade {
    pub(crate) fn new(skill_management: Arc<IronClawLocalSkillManagementPort>) -> Self {
        Self {
            skill_management,
    ...
    pub(crate) fn with_extension_management(
        mut self,
        extension_management: Arc<IronClawLocalExtensionManagementPort>,
    ) -> Self {
...
#[async_trait]
impl LifecycleProductFacade for IronClawLocalLifecycleFacade {
    async fn execute(
        &self,
        context: LifecycleProductContext,
        action: LifecycleProductAction,
...
fn map_local_skill_management_error(
    error: IronClawLocalSkillManagementError,
) -> ProductWorkflowError {
    match error {
        IronClawLocalSkillManagementError::InvalidContext { reason } => {
            ProductWorkflowError::InvalidBindingRequest { reason }
        }
        IronClawLocalSkillManagementError::Skill(error) => map_skill_error(error),
    }
}
...
mod tests {
    ...
    fn lifecycle_fixture() -> (
        ...
        filesystem
            ...
            .expect("mount storage root");
        let skill_management = Arc::new(IronClawLocalSkillManagementPort::new(
        ...
        let facade = IronClawLocalLifecycleFacade::new(skill_management);
