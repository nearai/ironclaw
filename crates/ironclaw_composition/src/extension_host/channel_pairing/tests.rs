...
#[async_trait]
impl IronClawUserIdentityBindingStore for InMemoryIdentity {
...
#[async_trait]
impl crate::provider_identity::IronClawUserIdentityLookup for InMemoryIdentity {
    async fn resolve_user_identity(
        &self,
        provider: &str,
        provider_user_id: &str,
    ) -> Result<Option<UserId>, crate::provider_identity::IronClawUserIdentityLookupError> {
        Ok(self
            .bindings
            .lock()
    ...
    async fn user_has_provider_binding(
        ...
        provider: &str,
        user_id: &UserId,
    ) -> Result<bool, crate::provider_identity::IronClawUserIdentityLookupError> {
        Ok(self
            .bindings
            .lock()
            .expect("bindings lock")
...
#[async_trait]
impl IronClawUserIdentityBindingDeleteStore for InMemoryIdentity {
...
fn fixture_with(
    ...
    let service = ChannelPairingService::new(ChannelPairingServiceParts {
        ...
        identity_bind: Arc::clone(&identity) as Arc<dyn IronClawUserIdentityBindingStore>,
        identity_lookup: Arc::clone(&identity)
            as Arc<dyn crate::provider_identity::IronClawUserIdentityLookup>,
        identity_delete: Arc::clone(&identity) as Arc<dyn IronClawUserIdentityBindingDeleteStore>,
        continuation: Arc::clone(&dispatcher) as Arc<dyn IronClawAuthContinuationDispatcher>,
...
async fn consume_refuses_codes_for_foreign_installations_and_bound_senders() {
    ...
    fixture
        .identity
        .bind_user_identity(IronClawUserIdentityBinding {
            provider: IronClawIdentityProviderId::new(EXT).expect("provider"),
            provider_user_id: IronClawIdentityProviderUserId::new(format!("{INSTALL}:u-7"))
