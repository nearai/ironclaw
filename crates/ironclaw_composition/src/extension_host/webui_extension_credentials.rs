...
use ironclaw_product_workflow::{
    ...
    ExtensionCredentialSubmitRequest, IronClawServicesError, IronClawServicesErrorCode,
    IronClawServicesErrorKind, LifecycleExtensionCredentialSetup,
};

use crate::{
    IronClawManualTokenSetupRequest, IronClawManualTokenSubmitRequest, IronClawProductAuthServices,
...
impl ProductAuthExtensionCredentialSetup {
    pub(crate) fn new(product_auth: Arc<IronClawProductAuthServices>) -> Self {
        Self { product_auth }
    }
}
...
impl ExtensionCredentialSetupService for ProductAuthExtensionCredentialSetup {
    ...
    ) -> Result<Option<ironclaw_auth::CredentialAccountProjection>, IronClawServicesError> {
        let selector = self
            .product_auth
    ...
    async fn submit_manual_token(
    ...
    ) -> Result<ironclaw_auth::CredentialAccountId, IronClawServicesError> {
        let label =
        ...
        let mut setup = IronClawManualTokenSetupRequest::new(
            request.scope.clone(),
            request.provider,
            label,
...
fn map_auth_error(error: crate::IronClawAuthProductError) -> IronClawServicesError {
    match error.code {
        ...
        | AuthErrorCode::FlowAlreadyTerminal => services_error(
            IronClawServicesErrorCode::Internal,
            IronClawServicesErrorKind::BlockedAuthentication,
            500,
            error.retryable,
        ),
...
fn invalid_auth_setup_request() -> IronClawServicesError {
    services_error(
...
mod tests {
    ...
    use crate::{IronClawAuthContinuationDispatcher, IronClawProductAuthServices};
