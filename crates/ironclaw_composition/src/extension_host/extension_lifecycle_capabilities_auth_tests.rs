...
use crate::{
    ...
    IronClawServices, build_ironclaw_services,
};
...
/// runtime-credential-selection path `build_ironclaw_services` wires up — not
...
///
/// Drives ONLY the composition's public surface: `build_ironclaw_services` (the
...
/// `IronClawProductAuthServices`'s existing `pub(crate)`
