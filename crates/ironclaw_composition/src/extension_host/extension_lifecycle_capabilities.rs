...
impl FirstPartyCapabilityHandler for ExtensionLifecycleToolHandler {
    async fn dispatch(
        ...
        let output = serde_json::to_value(without_model_visible_connection_chrome(response))
            .map_err(|error| {
                tracing::debug!(
                    target: "ironclaw::extension_lifecycle",
                    ?error,
...
mod tests {
    ...
    use super::*;
    use crate::{IronClawBuildInput, IronClawServices, OAuthClientConfig, build_ironclaw_services};
    use ironclaw_host_api::InstallationState;
    use ironclaw_product_workflow::{ChannelConnectionRequirement, IronClawChannelConnectStrategy};
    ...
    /// `IronClawBuildInput::local_dev(..)` fixture now fails closed
    ...
    async fn local_dev_extension_activate_hosted_mcp_stages_discovery_and_publishes_tools() {
        ...
        let services = build_ironclaw_services(
            IronClawBuildInput::local_dev("extension-tools-hosted-mcp-owner", storage_root.clone())
