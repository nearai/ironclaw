...
use crate::provider_identity::{
    IronClawUserIdentityBinding, IronClawUserIdentityBindingDeleteStore,
    IronClawUserIdentityBindingError, IronClawUserIdentityBindingStore, IronClawUserIdentityLookup,
    IronClawUserIdentityLookupError,
};
...
impl FilesystemChannelIdentityStore {
    ...
    /// composed (`build_ironclaw_services`' channel egress scope). Tests only.
    ...
    async fn write_user_binding_index_marker(&self, binding: &IronClawUserIdentityBinding) {
    ...
    async fn reconcile_identity_version_mismatch(
        ...
        let Some((existing, _)) = self
            ...
            return Err(IronClawUserIdentityBindingError::Backend(
        ...
        Err(IronClawUserIdentityBindingError::ProviderIdentityAlreadyBound)
    }
}

#[async_trait::async_trait]
impl IronClawUserIdentityLookup for FilesystemChannelIdentityStore {
    async fn resolve_user_identity(
        ...
        let user_id = UserId::new(record.user_id)
            .map_err(|error| IronClawUserIdentityLookupError::InvalidUserId(error.to_string()))?;
        Ok(Some(user_id))
    }
...
#[async_trait::async_trait]
impl IronClawUserIdentityBindingStore for FilesystemChannelIdentityStore {
...
#[async_trait::async_trait]
impl IronClawUserIdentityBindingDeleteStore for FilesystemChannelIdentityStore {
...
impl StoredChannelUserIdentity {
    fn from_binding(binding: &IronClawUserIdentityBinding, created_at: DateTime<Utc>) -> Self {
        Self {
...
fn map_lookup_fs_error(error: FilesystemError) -> IronClawUserIdentityLookupError {
    IronClawUserIdentityLookupError::Backend(error.to_string())
}

fn map_binding_fs_error(error: FilesystemError) -> IronClawUserIdentityBindingError {
    IronClawUserIdentityBindingError::Backend(error.to_string())
}
...
mod tests {
    ...
    use crate::provider_identity::{IronClawIdentityProviderId, IronClawIdentityProviderUserId};
    ...
    fn binding(provider_user_id: &str, user: &str) -> IronClawUserIdentityBinding {
        IronClawUserIdentityBinding {
            provider: IronClawIdentityProviderId::new("vendorx").expect("provider"),
            provider_user_id: IronClawIdentityProviderUserId::new(provider_user_id)
