...
use ironclaw_conversations::IronClawFilesystemConversationServices;
...
use ironclaw_product_workflow::{
    ...
    DeliveryCoordinator, IdempotencyLedger, IronClawFilesystemIdempotencyLedger,
...
pub(crate) struct ChannelWorkflowState {
    ...
    pub(crate) conversations: Arc<IronClawFilesystemConversationServices>,
}
    ...
    F: RootFilesystem + 'static,
{
    async fn build(
        ...
        let ledger = IronClawFilesystemIdempotencyLedger::new(Arc::clone(&scoped), ledger_scope)
            .with_settled_entry_limit(settled_limit)
            .with_settled_prune_interval(prune_interval);
        let conversations = IronClawFilesystemConversationServices::new(scoped)
            .await
...
pub(crate) struct GenericChannelHostDeps {
    ...
    pub(crate) identity_lookup:
        Option<Arc<dyn crate::provider_identity::IronClawUserIdentityLookup>>,
