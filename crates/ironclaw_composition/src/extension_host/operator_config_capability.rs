...
use ironclaw_product_workflow::{
    IronClawOperatorToolCatalog, IronClawOperatorToolInfo,
...
pub(crate) fn insert_handler(
    ...
    tool_catalog: Arc<dyn IronClawOperatorToolCatalog>,
) -> Result<(), HostApiError> {
...
async fn find_operator_tool(
    catalog: &dyn IronClawOperatorToolCatalog,
    capability_id: &CapabilityId,
    caller: &UserId,
    started: Instant,
) -> Result<IronClawOperatorToolInfo, FirstPartyCapabilityError> {
    catalog
...
fn tool_permission_locked(tool: &IronClawOperatorToolInfo) -> bool {
    tool.default_permission == PermissionMode::Deny || hard_floor_tool(tool)
}

fn hard_floor_tool(tool: &IronClawOperatorToolInfo) -> bool {
...
mod tests {
    ...
    async fn tool_permission_handler_writes_persistent_policy_and_override() {
        ...
        let tool_catalog: Arc<dyn IronClawOperatorToolCatalog> =
            Arc::new(StaticToolCatalog(vec![IronClawOperatorToolInfo {
    ...
    struct StaticToolCatalog(Vec<IronClawOperatorToolInfo>);

    #[async_trait]
    impl IronClawOperatorToolCatalog for StaticToolCatalog {
        async fn list_operator_tools(&self, _caller: &UserId) -> Vec<IronClawOperatorToolInfo> {
            self.0.clone()
        }
    }
}
