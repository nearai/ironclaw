...
use ironclaw_product_workflow::{
    ...
    ExtensionAccountSetupError, ExtensionAccountSetupRegistry, IronClawChannelConnectStrategy,
    IronClawServicesError, LifecycleBlockerRef, LifecycleExtensionSummary,
    ...
    LifecycleSearchExtensionSummary, ProductWorkflowError, WebUiAuthenticatedCaller,
};
use tokio::sync::{Mutex, RwLock, Semaphore};

use crate::IronClawProductAuthServices;
...
/// [`IronClawProductAuthServices::cleanup_credentials_for_lifecycle`]. This is the
...
pub(crate) trait ExtensionCredentialCleanup: Send + Sync {
    ...
    ) -> Result<SecretCleanupReport, IronClawServicesError>;
}

#[async_trait]
impl ExtensionCredentialCleanup for IronClawProductAuthServices {
    async fn cleanup_for_lifecycle(
        &self,
        request: SecretCleanupRequest,
    ) -> Result<SecretCleanupReport, IronClawServicesError> {
        IronClawProductAuthServices::cleanup_credentials_for_lifecycle(self, request)
            .await
            .map_err(|error| {
                IronClawServicesError::internal_from(format!(
...
pub(crate) struct IronClawLocalExtensionManagementPort {
    ...
    /// `IronClawLocalRuntimeServices::channel_disconnect_slot`. Removing
    ...
    /// composition in `build_ironclaw_runtime`, or the channel-connection test
...
pub(crate) async fn restore_extension_lifecycle_state(
    ...
    for installation in canonicalize_persisted_installation_rows(installation_store).await? {
        ...
        // v1->IronClaw migration tool ahead of catalog package materialization
...
impl IronClawLocalExtensionManagementPort {
    pub(crate) fn new(
        filesystem: Arc<dyn RootFilesystem>,
        catalog: AvailableExtensionCatalog,
        installation_store: Arc<dyn ExtensionInstallationStore>,
    ...
    /// `IronClawLocalRuntimeServices` so a fill by runtime composition (or the
    ...
    /// in when constructing `IronClawLocalExtensionManagementPort`. For tests
    ...
    pub(crate) async fn remove(
        ...
        let mut response = {
            ...
            if removes_connectable_channel && let Some(actor_user_id) = authenticated_actor_user_id
            {
                ...
                // `build_ironclaw_runtime`, the channel-connection test bundle).
...
impl crate::extension_host::channel_config::ChannelConfigReactivation
    for IronClawLocalExtensionManagementPort
{
...
fn activation_success_message(
    ...
    if package_declares_inbound_product_adapter(package) {
        ...
        let connect_guidance = match connection.strategy {
            IronClawChannelConnectStrategy::OAuth => format!(
            ...
            IronClawChannelConnectStrategy::InboundProofCode
            | IronClawChannelConnectStrategy::WebGeneratedCode
            | IronClawChannelConnectStrategy::QrCode
            | IronClawChannelConnectStrategy::AdminManagedChannels => format!(
...
pub(crate) fn channel_connect_strategy(
    package: &ExtensionPackage,
) -> IronClawChannelConnectStrategy {
    ...
    if uses_oauth {
        IronClawChannelConnectStrategy::OAuth
    } else {
        IronClawChannelConnectStrategy::InboundProofCode
    }
}
...
pub(crate) fn channel_connection_requirement(
    channel_id: &str,
    display_name: &str,
    strategy: IronClawChannelConnectStrategy,
    account_setup: Option<&ExtensionAccountSetupDescriptor>,
    ...
    let (instructions, input_placeholder, submit_label, error_message) = match strategy {
        ...
        | IronClawChannelConnectStrategy::WebGeneratedCode
        | IronClawChannelConnectStrategy::QrCode
        | IronClawChannelConnectStrategy::AdminManagedChannels => (
...
mod tests {
    ...
    fn channel_connect_strategy_is_manifest_driven_not_name_based() {
        ...
        assert_eq!(requirement.strategy, IronClawChannelConnectStrategy::OAuth);
    ...
    impl ExtensionRemovalCleanupAdapter for RecordingExtensionRemovalCleanupAdapter {
        ...
        async fn cleanup(
            ...
            if let Some(detail) = self.failure_detail {
                return Err(IronClawServicesError::internal_from(detail));
            }
            Ok(())
        }
    }
...
"#;
    ...
    impl ChannelConnectionFacade for RecordingChannelConnectionFacade {
        ...
        ) -> Result<std::collections::HashMap<String, bool>, IronClawServicesError> {
    ...
    fn connectable_channel_removal_fixture(
        ...
        tempfile::TempDir,
        std::path::PathBuf,
        crate::extension_host::lifecycle::IronClawLocalLifecycleFacade,
    ...
    async fn declared_cleanup_survives_fresh_service_restart_without_catalog_package() {
        ...
        let restarted_port = IronClawLocalExtensionManagementPort::new(
...
"#
    ...
    async fn restore_skips_installation_absent_from_catalog_and_restores_valid_installation() {
        ...
        // package for — e.g. a placeholder row written by the standalone
        // v1->IronClaw migration tool — must not abort restore for every other
        // installation.
        let (_dir, _storage_root, port, _active_registry, installation_store, _trust_policy) =
            extension_management_port_fixture_with_catalog_service_and_trust(
                AvailableExtensionCatalog::from_packages(vec![fixture_extension_package()]),
    ...
    async fn extension_activate_rejects_lifecycle_package_without_installation() {
        ...
        let port = IronClawLocalExtensionManagementPort::new(
            Arc::new(filesystem),
            AvailableExtensionCatalog::from_packages(Vec::new()),
    ...
    impl ExtensionCredentialCleanup for FailThenQuarantineExtensionCredentialCleanup {
        async fn cleanup_for_lifecycle(
            ...
            match self.calls.fetch_add(1, Ordering::SeqCst) {
                0 => Err(IronClawServicesError::internal_from(
    ...
    async fn ui_facade_extension_remove_revokes_exclusive_credential_at_convergence_point() {
        ...
        // `IronClawLocalExtensionManagementPort::remove`, so credential revocation
    ...
    async fn already_absent_catalog_repair_persists_tombstone_before_cleanup() {
        ...
        let package_ref =
            LifecyclePackageRef::new(LifecyclePackageKind::Extension, "github").expect("valid ref");
        let removal_scope = hosted_mcp_scope("lifecycle-owner");
        let repair_port = IronClawLocalExtensionManagementPort::new(
        ...
        let no_catalog_port = IronClawLocalExtensionManagementPort::new(
            Arc::clone(&base_port.filesystem),
            AvailableExtensionCatalog::from_packages(Vec::new()),
            installation_store.clone(),
            Arc::clone(&base_port.lifecycle_service),
    ...
    async fn manifest_only_retry_removes_orphaned_active_runtime_and_files() {
        ...
        let retry_port = IronClawLocalExtensionManagementPort::new(
    ...
    fn extension_management_port_fixture_with_catalog_and_service(
        ...
        tempfile::TempDir,
        std::path::PathBuf,
        Arc<IronClawLocalExtensionManagementPort>,
        Arc<SharedExtensionRegistry>,
        Arc<FilesystemExtensionInstallationStore>,
    ) {
    ...
    fn extension_management_port_fixture_with_removal_cleanup(
        ...
        let extension_management = Arc::new(
            IronClawLocalExtensionManagementPort::new(
                root_filesystem,
                catalog,
                installation_store.clone(),
    ...
    fn extension_management_port_fixture_with_credential_cleanup(
    ...
    ) -> (tempfile::TempDir, Arc<IronClawLocalExtensionManagementPort>) {
        let dir = tempfile::tempdir().expect("tempdir");
        let storage_root = dir.path().join("local-dev");
        std::fs::create_dir_all(storage_root.join("system/extensions")).expect("storage root");
        let mut filesystem = DiskFilesystem::new();
        ...
        let active_registry = Arc::new(SharedExtensionRegistry::new(ExtensionRegistry::new()));
        let installation_store = Arc::new(filesystem_installation_store());
        let extension_management = Arc::new(IronClawLocalExtensionManagementPort::new(
            root_filesystem,
            catalog,
            installation_store,
            Arc::new(Mutex::new(ExtensionLifecycleService::new(
    ...
    /// `IronClawLocalExtensionManagementPort::new` alone (proving the opt-in
    ...
    // `IronClawLocalExtensionManagementPort::new` with no
    ...
    fn extension_lifecycle_fixture_with_all_cleanup(
        ...
        let skill_management = Arc::new(
            crate::extension_host::lifecycle::IronClawLocalSkillManagementPort::new(
        ...
        let active_registry = Arc::new(SharedExtensionRegistry::new(ExtensionRegistry::new()));
        let installation_store = Arc::new(filesystem_installation_store());
        let mut extension_management_port = IronClawLocalExtensionManagementPort::new(
            root_filesystem,
            catalog,
            installation_store.clone(),
            Arc::new(Mutex::new(lifecycle_service)),
        ...
        let facade =
            crate::extension_host::lifecycle::IronClawLocalLifecycleFacade::new(skill_management)
    ...
    fn extension_port_with_delete_installation_failing_store(
        ...
        tempfile::TempDir,
        IronClawLocalExtensionManagementPort,
        Arc<SharedExtensionRegistry>,
    ...
    /// wired into every test `IronClawLocalExtensionManagementPort`.
...
"#
    ...
    /// mirrors the placeholder rows the standalone v1->IronClaw migration tool
...
version = "0.1.0"
description = "Placeholder row from the v1->IronClaw migration tool"
trust = "third_party"
