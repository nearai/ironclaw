...
//! an installation-scoped [`IronClawUserIdentityBinding`] for the
...
use crate::provider_identity::{
    IronClawIdentityProviderId, IronClawIdentityProviderUserId, IronClawUserIdentityBinding,
    IronClawUserIdentityBindingDeleteStore, IronClawUserIdentityBindingError,
    IronClawUserIdentityBindingStore, installation_scoped_provider_user_id,
};
...
pub struct ChannelIdentityBindingConfig {
    ...
    pub(crate) binding_store: Arc<dyn IronClawUserIdentityBindingStore>,
    ...
    /// token material.
    pub(crate) rollback_store: Arc<dyn IronClawUserIdentityBindingDeleteStore>,
...
impl ChannelIdentityBindingConfig {
    ...
    pub fn for_test(
        ...
        binding_store: Arc<dyn IronClawUserIdentityBindingStore>,
        rollback_store: Arc<dyn IronClawUserIdentityBindingDeleteStore>,
    ) -> Self {
        Self {
            tenant_id,
...
pub(crate) async fn bind_channel_identities_for_callback(
    ...
    let mut bound: Vec<IronClawIdentityProviderUserId> = Vec::new();
...
async fn bind_one_target(
    ...
    user_id: &UserId,
) -> Result<IronClawIdentityProviderUserId, AuthProductError> {
    let scope = target
        .scope_source
    ...
    let binding = IronClawUserIdentityBinding {
        provider: IronClawIdentityProviderId::new(provider)
            .map_err(|_| AuthProductError::MalformedCallback)?,
        provider_user_id: IronClawIdentityProviderUserId::new(
            installation_scoped_provider_user_id(&scope.installation_id, identity.subject.as_str()),
        )
        .map_err(|_| AuthProductError::MalformedCallback)?,
    ...
    config
        ...
        .map_err(|error| match error {
            IronClawUserIdentityBindingError::ProviderIdentityAlreadyBound => {
                AuthProductError::ProviderIdentityAlreadyConnected
            }
            IronClawUserIdentityBindingError::InvalidIdentityField { .. } => {
                AuthProductError::MalformedCallback
            }
            IronClawUserIdentityBindingError::Backend(_) => AuthProductError::BackendUnavailable,
        })?;
...
async fn roll_back_bindings(
    ...
    provider: &str,
    user_id: &UserId,
    bound: &[IronClawIdentityProviderUserId],
) {
    for provider_user_id in bound {
...
"#;
    ...
    async fn matching_identity_binds_installation_scoped_and_rollback_undoes_it() {
        ...
        assert_eq!(
            ...
            vec![IronClawUserIdentityBinding {
                provider: IronClawIdentityProviderId::new("acmechat").expect("provider"),
                provider_user_id: IronClawIdentityProviderUserId::new(format!(
    ...
    impl RecordingIdentityStore {
        ...
        fn bindings(&self) -> Vec<IronClawUserIdentityBinding> {
    ...
    impl IronClawUserIdentityBindingStore for RecordingIdentityStore {
        async fn bind_user_identity(
            ...
            {
                return Err(IronClawUserIdentityBindingError::ProviderIdentityAlreadyBound);
            }
