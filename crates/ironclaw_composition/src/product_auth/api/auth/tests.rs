...
#[test]
fn ironclaw_product_auth_services_new_accepts_separate_impls() {
    ...
    let services = IronClawProductAuthServices::new(
        flow_manager.clone(),
...
fn with_host_managed_nearai_credential_scope_rejects_thread_scoped_value() {
    ...
    let services =
        IronClawProductAuthServices::from_shared(shared, Arc::new(NoopAuthContinuationDispatcher));
    ...
    assert!(matches!(error, IronClawBuildError::InvalidConfig { .. }));
}
...
#[test]
fn ironclaw_product_auth_services_from_shared_clones_arc_per_trait() {
    let shared = Arc::new(SharedAuthTestDouble);
...
#[test]
fn ironclaw_product_auth_ports_from_shared_with_provider_uses_separate_provider_client() {
    let shared = Arc::new(SharedAuthTestDouble);
    let provider_client: Arc<dyn AuthProviderClient> = Arc::new(SharedAuthTestDouble);
    let shared_ptr = arc_data_ptr(&shared);
    let provider_ptr = arc_data_ptr(&provider_client);

    let ports = IronClawProductAuthServicePorts::from_shared_with_provider(shared, provider_client);
...
#[async_trait::async_trait]
impl IronClawManualTokenFlowService for SharedAuthTestDouble {
...
/// Build a minimal `IronClawProductAuthServices` for `cancel_blocked_auth_flow`
