//! IronClaw-native product-auth route composition.
//!
...
//! credential mutation, and continuation dispatch to [`IronClawProductAuthServices`].
...
use ironclaw_product_workflow::{
    EXTENSIONS_VIEW, IronClawExtensionListResponse, IronClawServicesError, IronClawViewQuery,
    LifecyclePackageKind, ProductSurface, WebUiAuthenticatedCaller,
};
...
use crate::product_auth::api::auth::IronClawOAuthStartFlowRequest;
use crate::{
    IronClawManualTokenSetupRequest, IronClawManualTokenSubmitRequest,
    IronClawManualTokenSubmitResponse, IronClawOAuthCallbackError, IronClawOAuthCallbackOutcome,
    IronClawOAuthCallbackRequest, IronClawOAuthCallbackResponse, IronClawProductAuthServices,
};
...
pub struct ProductAuthRouteState {
    product_auth: Arc<IronClawProductAuthServices>,
...
struct IronClawServicesInstalledExtensionLookup {
    api: Arc<dyn ProductSurface>,
}

#[async_trait::async_trait]
impl InstalledExtensionLookup for IronClawServicesInstalledExtensionLookup {
    async fn is_installed(
        &self,
        caller: &WebUiAuthenticatedCaller,
        extension_id: &ExtensionId,
        ...
        let page = self
            ...
            .await?;
        let inventory: IronClawExtensionListResponse =
...
impl ProductAuthRouteState {
    ...
    pub fn with_webui_api(mut self, webui_api: Arc<dyn ProductSurface>) -> Self {
        self.installed_extension_lookup =
            Some(Arc::new(IronClawServicesInstalledExtensionLookup {
                api: webui_api,
            }));
        self
    }
...
impl std::fmt::Debug for ProductAuthRouteState {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        builder
            .field("product_auth", &"Arc<IronClawProductAuthServices>")
            .field(
...
// mutations enter `IronClawProductAuthServices` directly; they are not in-turn
// tool calls and must not surface raw secrets through the model-visible
// tool-dispatch path. Contract: `docs/ironclaw/contracts/auth-product.md`.
...
pub(super) struct ProductAuthRouteFailure {
    status: StatusCode,
    body: IronClawOAuthCallbackError,
}
...
impl From<IronClawOAuthCallbackError> for ProductAuthRouteFailure {
    fn from(error: IronClawOAuthCallbackError) -> Self {
        route_failure_from_callback_error(error)
    }
}

pub(super) fn route_failure_from_callback_error(
    error: IronClawOAuthCallbackError,
...
pub(super) async fn scoped_update_binding_for_requester(
    ...
    match account {
        ...
        Err(AuthProductError::AccountSelectionRequired) => {
            tracing::warn!(
                target: "ironclaw_composition::product_auth::oauth",
...
pub(super) fn compose_authorization_url(
    ...
    let invocation_id = scope.resource.invocation_id.to_string();
    {
        ...
        query.append_pair("ironclaw_flow_id", &flow_id);
        query.append_pair("ironclaw_user_id", scope.resource.user_id.as_str());
        query.append_pair("ironclaw_invocation_id", &invocation_id);
        if let Some(agent_id) = &scope.resource.agent_id {
            query.append_pair("ironclaw_agent_id", agent_id.as_str());
        }
        if let Some(project_id) = &scope.resource.project_id {
            query.append_pair("ironclaw_project_id", project_id.as_str());
        }
        if let Some(thread_id) = &scope.resource.thread_id {
            query.append_pair("ironclaw_thread_id", thread_id.as_str());
        }
        if let Some(session_id) = &scope.session_id {
            query.append_pair("ironclaw_session_id", session_id.as_str());
        }
    }
...
///
/// Every protected product-auth route enters `IronClawProductAuthServices` the
...
mod tests {
    use super::*;
    use crate::IronClawAuthContinuationDispatcher;
    ...
    #[async_trait]
    impl IronClawAuthContinuationDispatcher for NoopDispatcher {
    ...
    #[async_trait]
    impl IronClawAuthContinuationDispatcher for RecordingDispatcher {
        async fn dispatch_auth_continuation(
            &self,
    ...
    fn test_state() -> ProductAuthRouteState {
        ProductAuthRouteState::new(
            Arc::new(IronClawProductAuthServices::local_dev_in_memory(Arc::new(
                NoopDispatcher,
            ))),
            TenantId::new("tenant-alpha").expect("tenant"),
            None,
    ...
    async fn extension_oauth_start_runs_dcr_for_recipes_without_client_credentials() {
        ...
        let product_auth =
            IronClawProductAuthServices::local_dev_in_memory(Arc::new(NoopDispatcher))
    ...
    async fn vendor_oauth_callback_completes_a_started_flow() {
        ...
        // `IronClawProductAuthServices::new` wires no account record source: the
        ...
        let product_auth = Arc::new(
            IronClawProductAuthServices::new(
                flow_manager,
                interaction_service,
    ...
    async fn extension_oauth_start_aborts_the_started_flow_when_uninstall_races() {
        ...
        let shared = Arc::new(ironclaw_auth::InMemoryAuthProductServices::new());
        let product_auth =
            IronClawProductAuthServices::from_shared(shared.clone(), Arc::new(NoopDispatcher))
                .with_auth_engine(engine);
    ...
    async fn oauth_callback_retrieves_pkce_from_gate_store_when_route_cache_misses() {
        ...
        let IronClawOAuthCallbackOutcome::Authorized { provider_request } = outcome else {
    ...
    async fn vendor_oauth_callback_resumes_blocked_turn_gate() {
        ...
        let product_auth = Arc::new(
            IronClawProductAuthServices::from_shared(shared.clone(), dispatcher.clone())
