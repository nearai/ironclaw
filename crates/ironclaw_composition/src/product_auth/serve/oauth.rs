...
pub(super) async fn oauth_start_handler(
    ...
    let flow = run_with_backend_timeout(
        state
            .product_auth
            .start_setup_oauth_flow(IronClawOAuthStartFlowRequest {
                flow_id: None,
                scope: scope.clone(),
...
async fn vendor_oauth_callback_attempt(
    ...
    *known_flow_id = Some(flow_id);
    ...
    let callback_request = IronClawOAuthCallbackRequest {
        scope: callback_scope.clone(),
        flow_id,
        opaque_state_hash: state_hash.clone(),
...
fn oauth_callback_route_result_response(
    headers: &HeaderMap,
    response: Result<IronClawOAuthCallbackResponse, ProductAuthRouteFailure>,
) -> Result<Response, ProductAuthRouteFailure> {
...
fn oauth_callback_response(
    headers: &HeaderMap,
    response: IronClawOAuthCallbackResponse,
) -> Response {
...
fn oauth_callback_completion_html(response: &IronClawOAuthCallbackResponse) -> Response {
...
pub(super) async fn callback_outcome_from_query(
...
) -> Result<IronClawOAuthCallbackOutcome, ProductAuthRouteFailure> {
    if query
        .error
        .as_deref()
        .is_some_and(|value| !value.is_empty())
    {
        return Ok(IronClawOAuthCallbackOutcome::ProviderDenied);
    }
    ...
    Ok(IronClawOAuthCallbackOutcome::Authorized {
        provider_request: OAuthProviderCallbackRequest {
            provider,
            account_label,
            authorization_code: OAuthAuthorizationCode::new(code.clone_secret())
