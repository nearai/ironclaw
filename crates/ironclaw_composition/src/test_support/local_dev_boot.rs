//! IronClaw integration-test framework local-dev boot accessors.
//!
...
//! boot sequence so the integration-test harness (`tests/support/ironclaw_parity_qa/`)
...
///
/// Called by the IronClaw integration-test framework's `StorageMode::LibSql`
/// builder arm (`tests/support/ironclaw_parity_qa/builder.rs:build_storage_composite`) so
...
) -> Result<(), crate::IronClawBuildError> {
...
/// downstream integration tests (the IronClaw integration-test framework in
/// `tests/support/ironclaw_parity_qa/`) construct one real `LibSqlRootFilesystem` over a
...
) -> Result<std::sync::Arc<ironclaw_secrets::FilesystemSecretStore<F>>, crate::IronClawBuildError>
where
    F: ironclaw_filesystem::RootFilesystem + 'static,
{
...
///
/// Wired by the IronClaw integration-test harness's one-runtime group assembly
/// (`tests/support/ironclaw_parity_qa/group.rs`'s `into_group`, which builds the group's
