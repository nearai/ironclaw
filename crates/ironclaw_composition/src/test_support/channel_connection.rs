...
//! §6.4) over a composed `IronClawServices`' own generic stores — the durable
...
//! production wiring in `IronClawRuntime::generic_channel_connection_facade`
//! (`runtime.rs`). The built facade is late-bound into
//! `IronClawLocalRuntimeServices::channel_disconnect_slot`, the same
//! slot `build_ironclaw_runtime` fills in production, so
...
//! `IronClawRuntime::channel_identity_binding_config`. The persisted binding is
...
use crate::provider_identity::IronClawUserIdentityLookup;
...
pub struct ChannelConnectionTestBundle {
    ...
    identity_lookup: Arc<dyn IronClawUserIdentityLookup>,
...
/// `IronClawRuntime::generic_channel_connection_facade` (facade construction)
/// plus the `build_ironclaw_runtime` slot fill, and assembles the identity
/// binding config like `IronClawRuntime::channel_identity_binding_config`.
///
...
pub fn build_channel_connection_for_test(
    services: &crate::IronClawServices,
    ...
    // Same construction as `IronClawRuntime::generic_channel_connection_facade`:
    ...
    let facade: Arc<dyn ChannelConnectionFacade> = Arc::new(GenericChannelConnectionFacade::new(
        ...
        Arc::clone(&identity_store) as Arc<dyn IronClawUserIdentityLookup>,
    ...
    // Same assembly as `IronClawRuntime::channel_identity_binding_config`: the
...
impl ChannelConnectionTestBundle {
    ...
    /// (`ironclaw_product_workflow/src/ironclaw_services/extensions.rs`).
    ...
    /// on boot (`build_ironclaw_services` →
    ...
    pub async fn active_identity_bindings_after_reopen(
        ...
        let lookup: Arc<dyn IronClawUserIdentityLookup> = store;
