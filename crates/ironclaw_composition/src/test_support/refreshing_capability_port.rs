...
//!
//! Lets the IronClaw integration-test harness assemble its capability port
...
pub struct RefreshingCapabilityPortTestParts {
    ...
    /// crate-private (`pub(crate)`) `IronClawLocalExtensionManagementPort` so it
    ...
    pub trajectory_observer: Option<std::sync::Arc<dyn crate::IronClawTrajectoryObserver>>,
...
/// `IronClawLocalExtensionManagementPort`. Hides the type from the
/// integration-test crate, which cannot name it (it is only `pub(crate)`
/// inside `ironclaw_composition`); the private type is recovered
...
pub struct ExtensionManagementTestHandle {
    extension_management: std::sync::Arc<
        crate::extension_host::extension_lifecycle::IronClawLocalExtensionManagementPort,
    >,
}
...
impl ExtensionManagementTestHandle {
    ...
    /// `pub`) so the crate-private `IronClawLocalExtensionManagementPort` type
