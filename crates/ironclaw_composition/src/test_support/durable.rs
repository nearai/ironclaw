...
/// path never drifts from `build_ironclaw_services`. Tests only.
...
pub async fn open_local_dev_approval_request_store_for_test(
    storage_root: &std::path::Path,
) -> Result<std::sync::Arc<dyn ironclaw_run_state::ApprovalRequestStore>, crate::IronClawBuildError>
{
...
pub async fn open_local_dev_trigger_repository_for_test(
    storage_root: &std::path::Path,
) -> Result<std::sync::Arc<dyn ironclaw_triggers::TriggerRepository>, crate::IronClawBuildError> {
