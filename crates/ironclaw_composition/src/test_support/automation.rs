//! Test-support constructor for [`crate::IronClawAutomationProductFacade`]
...
/// Build the production `IronClawAutomationProductFacade` over
/// `trigger_repository` plus the harness's own turn-state store, for
/// `IronClawServices::with_automation_product_facade`
/// (`ironclaw_product_workflow::IronClawServices`) test wiring. The turn-state
    ...
    F: RootFilesystem + Send + Sync + 'static,
{
    ...
    Arc::new(
        crate::automation::facade::IronClawAutomationProductFacade::new(
            trigger_repository,
            active_run_lookup,
        ),
    )
...
/// the `IronClawAutomationProductFacade` wrapper. For test harnesses that need
