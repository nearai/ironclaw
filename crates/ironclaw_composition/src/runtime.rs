//! Assembled IronClaw runtime: substrate + drivers + worker, started as one.
//!
...
//! `build_ironclaw_services`. It is the **only** place in the workspace where
...
//!
//! - [`build_ironclaw_runtime`] — construct + start the runtime
//! - [`IronClawRuntime`] — task-level handle (`new_conversation`,
...
//! property that satisfies the "narrow IronClaw public surface" requirement
//! pinned by `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs`.

// arch-exempt: large_file, needs IronClaw runtime helper extraction, plan #4471
...
#[cfg(any(test, feature = "test-support"))]
use ironclaw_product_workflow::IronClawOutboundDeliveryTargetId;
...
use crate::outbound::{
    IronClawOutboundPreferencesFacade, MutableOutboundDeliveryTargetRegistry,
    OUTBOUND_DELIVERY_TARGET_SET_CAPABILITY_ID, OutboundDeliveryTargetProvider,
    outbound_delivery_synthetic_provider,
};
use crate::projection::{IronClawProjectionServices, build_ironclaw_projection_services};
...
use crate::runtime_input::{
    IronClawRuntimeIdentity, IronClawRuntimeInput, PollSettings, TriggerFireAccessChecker,
...
use crate::{
    IronClawBuildError, IronClawProductAuthServices, IronClawReadiness, IronClawServices,
...
struct RuntimeStoreParts<'a> {
    local_runtime: Option<&'a crate::factory::IronClawRuntimeSubstrate>,
...
fn local_runtime_parts(
    local_runtime: &crate::factory::IronClawRuntimeSubstrate,
) -> RuntimeStoreParts<'_> {
...
fn production_runtime_parts<F>(
    graph: &Arc<crate::factory::IronClawProductionRuntimeStoreGraph<F>>,
...
fn enforce_runtime_cutover_gate(
    ...
    } = traffic
    {
        if readiness.state != required_readiness {
            return Err(IronClawRuntimeError::InvalidArgument {
                reason: format!(
...
pub use skills::{
    IronClawSkillActivation, IronClawSkillActivationMode, IronClawSkillAsset, IronClawSkillBundle,
    IronClawSkillExecutionPlan, IronClawSkillExecutionResult, IronClawSkillSourceKind,
};
...
/// Stable identifier for a IronClaw CLI conversation. Wraps a `ThreadId`.
...
/// Accepted-turn handle returned by `IronClawRuntime::submit_user_turn`. Holds
...
/// [`IronClawRuntime::send_user_message_until_gate`], which mirrors the
/// production [`IronClawRuntime::send_user_message`] submit path but returns when
...
#[cfg(any(test, feature = "test-support"))]
#[derive(Debug, Clone)]
pub enum IronClawTurnDriveOutcome {
...
/// Errors returned by `IronClawRuntime` methods.
#[derive(Debug, Error)]
pub enum IronClawRuntimeError {
    #[error("IronClaw runtime build failed: {0}")]
    Build(#[from] IronClawBuildError),
...
impl From<TurnError> for IronClawRuntimeError {
    fn from(value: TurnError) -> Self {
        Self::TurnCoordinator(value.to_string())
    }
}

impl From<DefaultPlannedRuntimeBuildError> for IronClawRuntimeError {
...
/// Started, running IronClaw agent runtime.
///
/// `IronClawRuntime` is the single user-facing handle returned by
/// [`build_ironclaw_runtime`]. Downstream code never reaches into the substrate
/// or worker machinery: it talks to the runtime through task-level methods.
pub struct IronClawRuntime {
    ...
    reply_target_binding_ref: ReplyTargetBindingRef,
    projection_services: IronClawProjectionServices,
    ...
    boot: Option<ironclaw_config::IronClawBootConfig>,
    /// Hot-swap handle for the live LLM provider, when one was wired at boot.
    llm_reload: Option<IronClawLlmReloadParts>,
}
...
impl RegistryPersistentApprovalGranteeResolver {
    fn new(registry: Arc<ExtensionRegistry>) -> Result<Self, IronClawRuntimeError> {
        let outbound_delivery_target_set_provider = outbound_delivery_synthetic_provider()
            .map_err(|error| IronClawRuntimeError::InvalidArgument {
...
/// `build_ironclaw_runtime` and `test_support::local_dev_approval_interaction_service_for_test`
...
) -> Result<Arc<dyn ApprovalInteractionService>, IronClawRuntimeError> {
...
/// `IronClawIntegrationGroup`'s own `build_default_planned_runtime`, whose runs
...
// struct exposed via a single `IronClawRuntime::trigger_poller_test_handles()`
...
/// services type per substrate (local `IronClawFilesystemConversationServices` /
/// `InMemoryConversationServices`, or the production graph's
/// `IronClawFilesystemConversationServices`) and hands it in by value, so this
...
) -> Result<TriggerPollerServices, IronClawRuntimeError>
where
...
fn trigger_poller_authorization_required_error() -> IronClawRuntimeError {
    IronClawRuntimeError::InvalidArgument {
...
/// substrate. Mirrors [`IronClawRuntime::ironclaw_user_directory`] but as a free
/// function usable during `build_ironclaw_runtime` (before a `IronClawRuntime`
...
fn poller_user_directory(
    services: &IronClawServices,
    local_runtime: Option<&crate::factory::IronClawRuntimeSubstrate>,
    tenant_id: &TenantId,
    actor_user_id: &UserId,
    agent_id: &AgentId,
    project_id: Option<&ProjectId>,
) -> Option<Arc<dyn ironclaw_identity::IronClawUserDirectory>> {
    if let Some(local) = local_runtime {
        let store = ironclaw_identity::FilesystemIronClawIdentityStore::new(
        ...
        return Some(Arc::new(store) as Arc<dyn ironclaw_identity::IronClawUserDirectory>);
    }
    if let Some(production) = services.production_runtime.as_ref() {
        let directory: Arc<dyn ironclaw_identity::IronClawUserDirectory> = match production {
            crate::factory::IronClawProductionRuntimeServices::LibSql(graph) => {
                Arc::new(ironclaw_identity::FilesystemIronClawIdentityStore::new(
            ...
            crate::factory::IronClawProductionRuntimeServices::Postgres(graph) => {
                Arc::new(ironclaw_identity::FilesystemIronClawIdentityStore::new(
                    Arc::clone(&graph.scoped_filesystem),
                    tenant_id.clone(),
                    actor_user_id.clone(),
...
/// context source exactly as production does in [`build_ironclaw_runtime`], and
...
impl IronClawRuntime {
    /// Snapshot of the substrate facades produced by `build_ironclaw_services`.
    /// Exposed for diagnostics / readiness reporting; **not** for traffic.
    pub fn services(&self) -> &IronClawServices {
        &self.services
    }
    ...
    pub(crate) fn webui_boot_config(&self) -> Option<&ironclaw_config::IronClawBootConfig> {
        self.boot.as_ref()
    }
    ...
    pub(crate) fn webui_llm_reload_trigger(&self) -> Option<Arc<dyn crate::LlmReloadTrigger>> {
        ...
        Some(Arc::new(
            crate::llm_admin::llm_reload::IronClawLlmReloadAdapter::new(
                boot.clone(),
    ...
    /// call site `IronClawProductionRuntimeServices::trigger_repository`, which
    ...
    /// Open the canonical IronClaw identity resolver on the runtime's existing
    ...
    ) -> ironclaw_identity::FilesystemIronClawIdentityStore<F>
    where
        F: RootFilesystem + 'static,
    {
        ironclaw_identity::FilesystemIronClawIdentityStore::new(
            scoped_filesystem,
    ...
    pub async fn open_ironclaw_identity_resolver(
        &self,
        _tenant_id: &TenantId,
    ) -> Option<
        Result<
            Arc<dyn ironclaw_identity::IronClawIdentityResolver>,
            ironclaw_identity::IronClawIdentityError,
        >,
    > {
        ...
        if let Some(local) = self.services.local_runtime.as_ref() {
            ...
            return Some(Ok(
                Arc::new(store) as Arc<dyn ironclaw_identity::IronClawIdentityResolver>
            ));
        }
        ...
        if let Some(production) = self.services.production_runtime.as_ref() {
            let store: Arc<dyn ironclaw_identity::IronClawIdentityResolver> = match production {
                crate::factory::IronClawProductionRuntimeServices::LibSql(graph) => {
    ...
    /// substrate. Same store [`open_ironclaw_identity_resolver`] uses
    /// (`FilesystemIronClawIdentityStore` implements both traits), so admin CRUD
    ...
    pub(crate) fn ironclaw_user_directory(
        &self,
    ) -> Option<Arc<dyn ironclaw_identity::IronClawUserDirectory>> {
        if let Some(local) = self.services.local_runtime.as_ref() {
            return Some(
                Arc::new(self.identity_store_over(Arc::clone(&local.identity_filesystem)))
                    as Arc<dyn ironclaw_identity::IronClawUserDirectory>,
            );
        }
        // Production-shaped substrate (#5013): same identity mount and per-call
        // tenant partitioning as the local path — see `open_ironclaw_identity_resolver`.
        if let Some(production) = self.services.production_runtime.as_ref() {
            let directory: Arc<dyn ironclaw_identity::IronClawUserDirectory> = match production {
                crate::factory::IronClawProductionRuntimeServices::LibSql(graph) => {
                    Arc::new(self.identity_store_over(Arc::clone(&graph.scoped_filesystem)))
    ...
    /// [`Self::ironclaw_user_directory`] (`pub(crate)`), which integration tests
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn ironclaw_user_directory_for_tests(
        &self,
    ) -> Option<Arc<dyn ironclaw_identity::IronClawUserDirectory>> {
        self.ironclaw_user_directory()
    }
    ...
    pub(crate) fn ironclaw_admin_secret_provisioner(
        &self,
    ...
    pub(crate) fn ironclaw_project_service(
        &self,
    ...
    /// [`IronClawRuntimeInput::with_admin_api_token_minter`], if any.
    pub(crate) fn ironclaw_admin_token_minter(
        &self,
    ) -> Option<Arc<dyn crate::AdminApiTokenMinter>> {
    ...
    /// ([`IronClawRuntime::webui_turn_coordinator`]) — so downstream integration
        ...
        Some(
            crate::extension_host::channel_identity::ChannelIdentityBindingConfig {
                ...
                binding_store: Arc::clone(&identity_store)
                    as Arc<dyn crate::provider_identity::IronClawUserIdentityBindingStore>,
                rollback_store: identity_store
                    as Arc<dyn crate::provider_identity::IronClawUserIdentityBindingDeleteStore>,
                post_bind_factory,
                overrides: Vec::new(),
            },
        )
        ...
        Some(Arc::new(
            crate::extension_host::channel_connection::GenericChannelConnectionFacade::new(
                ...
                Arc::clone(&identity_store)
                    as Arc<dyn crate::provider_identity::IronClawUserIdentityLookup>,
                identity_store
                    as Arc<dyn crate::provider_identity::IronClawUserIdentityBindingDeleteStore>,
                credential_cleanup,
    ...
    ) -> Result<OutboundDeliveryTargetRegistrationOutcome, IronClawRuntimeError> {
    ...
    /// to `IronClawServices::read_write_workspace_filesystem` — the single owner
    ...
    /// `build_ironclaw_runtime` and shut down via [`Self::shutdown`] —
    ...
    /// observer through
    /// [`crate::IronClawRuntimeInput::with_budget_event_observer`]
    ...
    pub fn apply_resolved_budget_gate(
    ...
    ) -> Result<ironclaw_resources::BudgetApprovalGate, IronClawRuntimeError> {
        ...
        let gate = local_runtime
            ...
            .ok_or_else(|| IronClawRuntimeError::InvalidArgument {
    ...
    pub async fn new_conversation(&self) -> Result<ConversationId, IronClawRuntimeError> {
        let thread_id =
            ThreadId::new(format!("ironclaw-conv-{}", Uuid::new_v4())).map_err(|reason| {
                IronClawRuntimeError::InvalidArgument {
        ...
        self.thread_service
            ...
            .await
            .map_err(|error| IronClawRuntimeError::ThreadService(error.to_string()))?;
    ...
    pub async fn send_user_message(
        ...
        conversation: &ConversationId,
        text: &str,
    ) -> Result<AssistantReply, IronClawRuntimeError> {
    ...
    async fn send_user_message_internal(
                ...
                .clear_accepted_message(&submitted.scope, &submitted.accepted_message_ref)
        {
            if reply.is_ok() {
                ...
                return Err(IronClawRuntimeError::TurnSubmission(
    ...
    async fn submit_user_turn(
        ...
        cancellation: &CancellationToken,
        capture_skill_execution_plan: bool,
    ) -> Result<SubmittedTurn, IronClawRuntimeError> {
        ...
        if self.turn_scheduler.is_stopped() {
            let error = IronClawRuntimeError::WorkerStopped;
            trace_runtime_latency_error(
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                return Err(IronClawRuntimeError::ThreadService(error.to_string()));
            }
        };

        let accepted_message_ref = AcceptedMessageRef::new(format!("msg:{}", accepted.message_id))
            .map_err(|reason| IronClawRuntimeError::InvalidArgument { reason })?;
        ...
        if capture_skill_execution_plan {
            let adapter = self
                ...
                .as_ref()
                .ok_or(IronClawRuntimeError::SkillExecutionUnavailable)?;
            ...
            if let Err(error) = adapter.record_user_message_for_execution(
                ...
                return Err(IronClawRuntimeError::TurnSubmission(error.to_string()));
            }
            trace_runtime_latency_ok(
                "record_skill_execution_message",
                &conversation.0,
            ...
            Err(error) => {
                ...
                if let Some(skill_activation_source) = &self.skill_activation_source {
                    skill_activation_source
                        ...
                        .map_err(|clear_error| {
                            IronClawRuntimeError::TurnSubmission(clear_error.to_string())
                        })?;
                }
        ...
        if cancellation.is_cancelled() {
            if let Some(skill_activation_source) = &self.skill_activation_source {
                skill_activation_source
                    .clear_accepted_message(&scope, &accepted_message_ref)
                    .map_err(|error| IronClawRuntimeError::TurnSubmission(error.to_string()))?;
            }
            self.cancel_run(
                &scope,
                run_id,
            ...
            .await?;
            return Err(IronClawRuntimeError::OperationCancelled);
        }
    ...
    /// Submit a skill-aware message through the normal IronClaw loop and return
    /// the structured activation plan produced during prompt construction.
    pub async fn execute_skill_message(
        &self,
        conversation: &ConversationId,
        text: &str,
    ) -> Result<IronClawSkillExecutionResult, IronClawRuntimeError> {
        let adapter = self
            .skill_execution_adapter
            .as_ref()
            .ok_or(IronClawRuntimeError::SkillExecutionUnavailable)?;
        ...
        Ok(IronClawSkillExecutionResult { plan, reply })
    }
    ...
    pub async fn read_skill_execution_asset(
        &self,
        conversation: &ConversationId,
        plan: &IronClawSkillExecutionPlan,
        activation: &IronClawSkillActivation,
        path: impl AsRef<str>,
    ) -> Result<IronClawSkillAsset, IronClawRuntimeError> {
        if plan.run_context().thread_id != conversation.0 {
            return Err(IronClawRuntimeError::SkillExecution(
    ...
    pub async fn shutdown(self) -> Result<(), IronClawRuntimeError> {
    ...
    fn turn_scope_for(&self, thread_id: &ThreadId) -> TurnScope {
        // IronClawRuntime is bound to a single actor user, so its turns are
    ...
    fn skill_execution_plan_for_run(
        ...
        scope: &TurnScope,
        run_id: TurnRunId,
    ) -> Result<IronClawSkillExecutionPlan, IronClawRuntimeError> {
        adapter
            .take_execution_plan_for_run(scope, run_id)
            .map_err(|error| IronClawRuntimeError::SkillExecution(error.to_string()))?
            .map(IronClawSkillExecutionPlan::from_first_party)
            .ok_or_else(|| {
                IronClawRuntimeError::SkillExecution(
                    "skill activation plan unavailable".to_string(),
                )
            })
    }
    ...
    async fn wait_for_terminal(
        ...
        run_id: TurnRunId,
        cancellation: &CancellationToken,
    ) -> Result<TurnRunState, IronClawRuntimeError> {
        let start = std::time::Instant::now();
        loop {
            if self.turn_scheduler.is_stopped() {
                return Err(IronClawRuntimeError::WorkerStopped);
            }
            let state = self
                .turn_coordinator
            ...
            if start.elapsed() > self.poll_settings.max_total {
                ...
                return Err(IronClawRuntimeError::RunTimeout {
    ...
    ///
    /// The QA-trace recorder (`tests/support/ironclaw_parity_qa/qa_trace.rs`) uses this so
    ...
    pub async fn send_user_message_until_gate(
        ...
        conversation: &ConversationId,
        text: &str,
    ) -> Result<IronClawTurnDriveOutcome, IronClawRuntimeError> {
        ...
        let outcome = async {
            ...
            if state.status.is_terminal() {
                Ok(IronClawTurnDriveOutcome::Terminal(AssistantReply {
                    conversation: conversation.clone(),
                    run_id: submitted.run_id,
                    status: state.status,
                ...
                let gate_ref = state.gate_ref.clone().ok_or_else(|| {
                    IronClawRuntimeError::TurnSubmission(format!(
                        "run parked on {:?} without a gate ref",
                        state.status
                    ))
                })?;
                Ok(IronClawTurnDriveOutcome::BlockedOnGate {
                    run_id: submitted.run_id,
                    status: state.status,
                    gate_ref,
    ...
    ) -> Result<CancelRunResponse, IronClawRuntimeError> {
        let response = self
            .turn_coordinator
    ...
    async fn append_webui_loop_cancelled(
        ...
        self.webui_event_log
            ...
            .await
            .map(|_| ())
            .map_err(|error| IronClawRuntimeError::TurnCoordinator(error.to_string()))
    }
    ...
    async fn read_latest_assistant_text(
        ...
        thread_id: &ThreadId,
        run_id: TurnRunId,
    ) -> Result<Option<String>, IronClawRuntimeError> {
        let history = self
            .thread_service
...
/// Build and start a IronClaw agent runtime.
///
/// On return, the turn-runner worker is already running in the background and
/// the returned `IronClawRuntime` is ready to accept `send_user_message` calls.
///
/// **Currently supported profiles:** `IronClawCompositionProfile::LocalDev`,
/// `IronClawCompositionProfile::LocalDevYolo`,
/// `IronClawCompositionProfile::HostedSingleTenant`, and
/// `IronClawCompositionProfile::Production` are wired end-to-end here. Production
/// starts only after readiness diagnostics validate that live traffic can be
/// exposed without a partial cutover.
pub async fn build_ironclaw_runtime(
    input: IronClawRuntimeInput,
) -> Result<IronClawRuntime, IronClawRuntimeError> {
    let IronClawRuntimeInput {
    ...
    let mut services_input = services_input.ok_or(IronClawRuntimeError::InvalidArgument {
        reason: "IronClawRuntimeInput.services is required".to_string(),
    })?;
    ...
    // Capture the resolved policy before `build_ironclaw_services` consumes the
    ...
    let runtime_policy =
        services_input
            .runtime_policy()
            .cloned()
            .ok_or(IronClawRuntimeError::InvalidArgument {
                reason: "IronClawRuntimeInput.services must include a resolved runtime policy"
                    .to_string(),
            })?;
    ...
    let actor_user_id =
        UserId::new(owner_id.clone()).map_err(|reason| IronClawRuntimeError::InvalidArgument {
    ...
    let mut services = build_ironclaw_services(services_input).await?;
    ...
    let runtime_parts =
        match deployment.substrate() {
            RuntimeSubstrate::Local => {
                let local_runtime = services.local_runtime.as_ref().ok_or(
                    IronClawRuntimeError::InvalidArgument {
                        reason: "local-dev IronClawServices did not provide runtime substrate"
                            .to_string(),
                    },
                )?;
            ...
            RuntimeSubstrate::ProductionShaped => {
                let production_runtime = services.production_runtime.as_ref().ok_or(
                    IronClawRuntimeError::InvalidArgument {
                        reason: "production IronClawServices did not provide runtime substrate"
                            .to_string(),
                    },
                )?;
    ...
    let mut projection_services = build_ironclaw_projection_services(
        Arc::clone(&event_log),
    ...
    let outbound_preferences_facade: Option<Arc<dyn OutboundPreferencesProductFacade>> =
        match (local_runtime, &outbound_delivery_target_registry) {
            (Some(local_runtime), Some(registry)) => {
                let registry = Arc::clone(registry);
                let provider: Arc<dyn OutboundDeliveryTargetProvider> = registry;
                Some(Arc::new(IronClawOutboundPreferencesFacade::new(
    ...
    let (
        ...
        .ok_or(IronClawRuntimeError::HostRuntimeUnavailable)?;
        (
        ...
        // `IronClawRuntimeInput::with_trajectory_observer`).
    ...
    // the typed `HooksActivationConfig` carried in `IronClawRuntimeInput` (master
    ...
    let hook_dispatcher_builder_factory = if let Some(local_runtime) = local_runtime {
        ...
        return Err(IronClawRuntimeError::MalformedConfig {
    ...
    // queue dir.
    let runtime_owner_trace_scope = ironclaw_traces::contribution::trace_scope_key(
    ...
    > = match (local_runtime, outbound_preferences_facade.clone()) {
        (Some(local_runtime), Some(outbound_preferences_facade)) => {
            let mut lifecycle_facade =
                crate::extension_host::lifecycle::IronClawLocalLifecycleFacade::new(Arc::clone(
    ...
    subagent_await_edge_settler
        ...
        .map_err(|error| IronClawRuntimeError::MalformedConfig {
    ...
    if trigger_poller.enabled {
        ...
        for grant in trigger_fire_access.grants() {
            match grant {
                ...
                TriggerFireAccessGrant::TenantMembership { agent, project } => {
                    ...
                    // `ironclaw_user_directory` opens, built here from whichever
        ...
        let conversation_services = match local_runtime {
            ...
            None => match services.production_runtime.as_ref() {
                Some(crate::factory::IronClawProductionRuntimeServices::LibSql(graph)) => {
                    graph.trigger_conversation_services.clone()
                }
                Some(crate::factory::IronClawProductionRuntimeServices::Postgres(graph)) => {
                    graph.trigger_conversation_services.clone()
                }
                None => {
                    return Err(IronClawRuntimeError::InvalidArgument {
    ...
    // `IronClawRuntimeInput::with_budget_event_observer`.
    ...
    if let (Some(boot_config), Some(reload_parts)) = (boot.as_ref(), llm_reload.as_ref()) {
        let boot_reload_adapter = crate::llm_admin::llm_reload::IronClawLlmReloadAdapter::new(
            boot_config.clone(),
    ...
    // (`IronClawLocalExtensionManagementPort::remove`) disconnects the caller's
...
fn build_webui_auth_interaction_service(
    product_auth: Option<&IronClawProductAuthServices>,
...
fn optional_nonzero_u32_env(
    key: &'static str,
) -> Result<Option<std::num::NonZeroU32>, IronClawRuntimeError> {
    match std::env::var(key) {
        ...
        Err(error) => Err(IronClawRuntimeError::InvalidArgument {
...
/// `regex_skill_activation_enabled` flag from [`IronClawRuntimeInput`] is
...
/// the full [`build_ironclaw_runtime`] path, where an accidental
...
/// Parse the IronClaw skill-injection mode from the
...
fn skill_injection_mode_env() -> Result<SkillInjectionMode, IronClawRuntimeError> {
...
fn skill_injection_mode_from(value: &str) -> Result<SkillInjectionMode, IronClawRuntimeError> {
    match value.trim().to_ascii_lowercase().as_str() {
        "" | "listing" => Ok(SkillInjectionMode::Listing),
        "full" => Ok(SkillInjectionMode::Full),
        other => Err(IronClawRuntimeError::InvalidArgument {
            reason: format!(
...
fn local_dev_filesystem_skill_context_source(
    ...
    tenant_id: &TenantId,
    regex_skill_activation_enabled: bool,
) -> Result<ComposedSkillContextSource, IronClawRuntimeError> {
    ...
    .map_err(|reason| IronClawRuntimeError::InvalidArgument {
...
/// [`IronClawLlmReloadAdapter::reload`], invoked once after boot construction.
async fn overlay_stored_llm_key_for_nearai_mcp_bootstrap(
    llm: Option<ResolvedIronClawLlm>,
    services: &IronClawServices,
) -> Result<Option<ResolvedIronClawLlm>, IronClawRuntimeError> {
    ...
    if let Some(stored) = keys
        ...
        .await
        .map_err(|error| IronClawRuntimeError::LlmProvider(error.to_string()))?
    {
...
fn validate_runtime_identity(
    identity: IronClawRuntimeIdentity,
) -> Result<ValidatedRuntimeIdentity, IronClawRuntimeError> {
    let tenant_id = TenantId::new(identity.tenant_id).map_err(|reason| {
        IronClawRuntimeError::InvalidArgument {
            reason: format!("tenant id: {reason}"),
        }
    })?;
    let agent_id = AgentId::new(identity.agent_id).map_err(|reason| {
        IronClawRuntimeError::InvalidArgument {
...
/// (`IronClawLlmReloadAdapter::reload`). No cost table is derived here: there's
...
/// `ResolvedIronClawLlm::with_provider_factory` seam would be silently dropped on
/// the cold-boot path.
async fn build_production_model_gateway(
    provider_factory: Option<crate::runtime_input::IronClawProviderFactory>,
) -> Result<
    (
        Arc<dyn ironclaw_loop_host::HostManagedModelGateway>,
        Option<ironclaw_loop_host::StaticModelCostTable>,
        Option<IronClawLlmReloadParts>,
    ),
    IronClawRuntimeError,
> {
...
pub(crate) struct IronClawLlmReloadParts {
...
async fn build_placeholder_llm_gateway(
    provider_factory: Option<crate::runtime_input::IronClawProviderFactory>,
) -> Result<LlmGatewayBundle, IronClawRuntimeError> {
    let session =
...
fn wrap_swappable_gateway(
    ...
    let model_profile_id = ModelProfileId::new("interactive_model").map_err(|reason| {
        IronClawRuntimeError::LlmProvider(format!("invalid interactive model profile id: {reason}"))
    })?;
