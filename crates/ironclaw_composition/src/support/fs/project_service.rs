...
use ironclaw_product_workflow::{
    ...
    IronClawListProjectsRequest, IronClawListProjectsResponse, IronClawProjectInfo,
    IronClawProjectMemberInfo, IronClawProjectMemberStatus, IronClawProjectResponse,
    IronClawProjectRole, IronClawProjectState, IronClawRemoveMemberRequest,
    IronClawUpdateMemberRoleRequest, IronClawUpdateProjectRequest, ProjectCaller, ProjectService,
    ProjectServiceError,
};
...
pub(crate) struct IronClawProjectService {
    repository: Arc<dyn ProjectRepository>,
}

impl IronClawProjectService {
...
#[async_trait]
impl ProjectService for IronClawProjectService {
    async fn list_projects(
        ...
        Ok(IronClawListProjectsResponse { projects })
    }
    ...
    async fn list_members(
        ...
        let members = self
            ...
            .map(member_info)
            .collect();
        Ok(IronClawListMembersResponse { members })
    }
...
fn project_info(record: ProjectRecord, role: ProjectRole) -> IronClawProjectInfo {
    IronClawProjectInfo {
...
fn member_info(record: ProjectMemberRecord) -> IronClawProjectMemberInfo {
...
fn project_role_to_product(role: ProjectRole) -> IronClawProjectRole {
    match role {
        ProjectRole::Owner => IronClawProjectRole::Owner,
        ProjectRole::Editor => IronClawProjectRole::Editor,
        ProjectRole::Viewer => IronClawProjectRole::Viewer,
    }
}

fn project_role_from_product(role: IronClawProjectRole) -> ProjectRole {
    match role {
        IronClawProjectRole::Owner => ProjectRole::Owner,
        IronClawProjectRole::Editor => ProjectRole::Editor,
        IronClawProjectRole::Viewer => ProjectRole::Viewer,
    }
}

fn project_state_to_product(state: ProjectState) -> IronClawProjectState {
    match state {
        ProjectState::Active => IronClawProjectState::Active,
        ProjectState::Archived => IronClawProjectState::Archived,
    }
}

fn project_state_from_product(state: IronClawProjectState) -> ProjectState {
    match state {
        IronClawProjectState::Active => ProjectState::Active,
        IronClawProjectState::Archived => ProjectState::Archived,
    }
}

fn member_status_to_product(status: ProjectMemberStatus) -> IronClawProjectMemberStatus {
    match status {
        ProjectMemberStatus::Active => IronClawProjectMemberStatus::Active,
        ProjectMemberStatus::Revoked => IronClawProjectMemberStatus::Revoked,
    }
}
...
mod tests {
    ...
    fn svc(repo: FakeRepo) -> IronClawProjectService {
        IronClawProjectService::new(Arc::new(repo))
    }
    ...
    async fn list_drops_project_revoked_after_listing() {
        ...
        let response = service
            .list_projects(caller(), IronClawListProjectsRequest { limit: None })
            .await
            .unwrap();
    ...
    async fn update_member_role_rejects_revoked_member() {
        ...
        let err = service
            .update_member_role(
                caller(),
                IronClawUpdateMemberRoleRequest {
                    project_id: record.project_id.into_string(),
                    user_id: "bob".to_string(),
                    role: IronClawProjectRole::Viewer,
                },
            )
            .await
            .unwrap_err();
    ...
    async fn create_reports_owner_role() {
        ...
        let response = service
            ...
            .await
            .unwrap();
        assert_eq!(response.project.role, IronClawProjectRole::Owner);
    }
}
