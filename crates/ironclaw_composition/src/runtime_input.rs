//! Input DTO for the assembled IronClaw runtime (`build_ironclaw_runtime`).
//!
//! `IronClawRuntimeInput` extends `IronClawBuildInput` (which is substrate-only)
...
//!   wire the first-party filesystem skill source from scoped IronClaw skill
//!   roots.
//!
...
/// Caller-owned identity for an assembled IronClaw runtime.
///
...
pub struct IronClawRuntimeIdentity {
...
impl IronClawRuntimeIdentity {
    pub fn ironclaw_cli() -> Self {
        Self {
...
impl Default for IronClawRuntimeIdentity {
    fn default() -> Self {
        Self::ironclaw_cli()
    }
}
...
/// at the serve/run edge, and `build_ironclaw_runtime` builds the matching
...
#[derive(Clone)]
pub struct ResolvedIronClawLlm {
    provider_id: String,
    model: String,
    ...
    pub(crate) provider_factory: Option<IronClawProviderFactory>,
}

/// Decorator over the config-built LLM provider. See
/// [`ResolvedIronClawLlm::with_provider_factory`].
pub type IronClawProviderFactory = Arc<
...
impl std::fmt::Debug for ResolvedIronClawLlm {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ResolvedIronClawLlm")
...
impl ResolvedIronClawLlm {
    ...
    pub fn with_provider_factory(mut self, factory: IronClawProviderFactory) -> Self {
    ...
    /// files at all (the reason the committed ironclaw_qa fixtures were only ever
    ...
    pub fn with_env_trace_recording(self) -> Self {
        ...
        let factory: IronClawProviderFactory =
            Arc::new(
...
/// Completion polling policy for `IronClawRuntime::send_user_message`.
#[derive(Debug, Clone)]
...
/// Full input for `build_ironclaw_runtime` — substrate config plus the extras
/// needed to assemble a runnable IronClaw agent.
#[derive(Default)]
pub struct IronClawRuntimeInput {
    pub services: Option<IronClawBuildInput>,
    pub llm: Option<ResolvedIronClawLlm>,
    ...
    pub boot: Option<IronClawBootConfig>,
    ...
    /// The deployment's fire-time access policy. `build_ironclaw_runtime` builds
    ...
    pub identity: IronClawRuntimeIdentity,
    ...
    /// supplying. When unset, `build_ironclaw_runtime` falls back to
    ...
    pub trajectory_observer: Option<Arc<dyn crate::IronClawTrajectoryObserver>>,
...
impl IronClawRuntimeInput {
    ...
    pub fn from_services(services: IronClawBuildInput) -> Self {
        Self {
            ...
            identity: IronClawRuntimeIdentity::default(),
    ...
    /// passing. Without this, `build_ironclaw_runtime` falls back to
    ...
    /// have no capability/result hook to forward to. `build_ironclaw_runtime`
    ...
    pub fn with_trajectory_observer(
        mut self,
        observer: Arc<dyn crate::IronClawTrajectoryObserver>,
    ) -> Self {
    ...
    pub fn with_resolved_llm(mut self, llm: ResolvedIronClawLlm) -> Self {
        self.llm = Some(llm);
        self
    }
    ...
    pub fn with_boot_config(mut self, boot: IronClawBootConfig) -> Self {
        self.boot = Some(boot);
        self
    }
    ...
    pub fn with_identity(mut self, identity: IronClawRuntimeIdentity) -> Self {
    ...
    /// Test-only hook: drive `build_ironclaw_runtime` with a stub
