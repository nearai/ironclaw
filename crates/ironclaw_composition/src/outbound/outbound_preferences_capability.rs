...
impl FirstPartyCapabilityHandler for SetOutboundPreferencesHandler {
    async fn dispatch(
        ...
        let input: IronClawSetOutboundPreferencesRequest = serde_json::from_value(request.input)
...
fn map_services_error(
    error: ironclaw_product_workflow::IronClawServicesError,
) -> RuntimeDispatchErrorKind {
    match (error.code, error.kind) {
        (IronClawServicesErrorCode::InvalidRequest, _)
        | (IronClawServicesErrorCode::NotFound, _) => RuntimeDispatchErrorKind::InputEncode,
        (IronClawServicesErrorCode::Forbidden, _) => RuntimeDispatchErrorKind::PolicyDenied,
        (IronClawServicesErrorCode::Unavailable, IronClawServicesErrorKind::ServiceUnavailable) => {
            RuntimeDispatchErrorKind::Backend
        }
        (IronClawServicesErrorCode::Conflict, _) => RuntimeDispatchErrorKind::OperationFailed,
