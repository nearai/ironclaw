...
async fn local_dev_oauth_turn_gate_callback_resumes_default_turn_coordinator() {
    ...
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev("local-dev-auth-owner", dir.path().join("local-dev"))
    ...
    let response = product_auth
        .handle_oauth_callback(crate::IronClawOAuthCallbackRequest {
            scope: auth_scope.clone(),
            flow_id: flow.id,
            opaque_state_hash: state_hash(),
            outcome: crate::IronClawOAuthCallbackOutcome::Authorized {
...
async fn local_dev_google_oauth_backend_builds_with_host_provider_config() {
    let dir = tempfile::tempdir().expect("tempdir");
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev("local-dev-google-oauth-owner", dir.path().join("local-dev"))
...
async fn production_libsql_oauth_callback_fans_out_to_all_owner_provider_blocked_runs() {
    ...
    let graph = match production_runtime {
        ...
        IronClawProductionRuntimeServices::Postgres(_) => panic!("expected libsql runtime"),
    };
    let actor = TurnActor::new(UserId::new("alice").unwrap());
...
async fn local_dev_notion_oauth_backend_builds_with_host_provider_config() {
    let dir = tempfile::tempdir().expect("tempdir");
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev("local-dev-notion-oauth-owner", dir.path().join("local-dev"))
            .with_vendor_oauth_client(
                "google",
                OAuthClientConfig {
                    client_id: OAuthClientId::new("google-client-123").expect("client id"),
...
#[tokio::test]
async fn oauth_callback_exchanges_vendor_recipe_through_ironclaw_product_auth_boundary() {
...
async fn oauth_callback_with_stale_gate_converges_without_resuming() {
    let dir = tempfile::tempdir().expect("tempdir");
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev("local-dev-auth-stale-owner", dir.path().join("local-dev"))
            .with_product_auth_ports(in_memory_product_auth_ports()),
    )
    .await
    .expect("local-dev services build");
...
#[cfg(test)]
fn in_memory_product_auth_ports() -> IronClawProductAuthServicePorts {
    IronClawProductAuthServicePorts::from_shared(Arc::new(InMemoryAuthProductServices::new()))
}
...
fn authorized_request(
    scope: AuthProductScope,
    flow_id: AuthFlowId,
) -> crate::IronClawOAuthCallbackRequest {
