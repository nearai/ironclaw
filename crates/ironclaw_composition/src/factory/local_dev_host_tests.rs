...
async fn local_yolo_policy_mounts_confirmed_host_home_as_host() {
    ...
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev_with_profile(
...
async fn local_yolo_policy_requires_confirmed_host_home_root() {
    let dir = tempfile::tempdir().expect("tempdir");
    let error = build_ironclaw_services(
        IronClawBuildInput::local_dev_with_profile(
            IronClawCompositionProfile::LocalDevYolo,
            "local-dev-yolo-host-owner",
...
async fn confirmed_host_home_root_is_rejected_without_matching_policy() {
    ...
    let error = build_ironclaw_services(
        IronClawBuildInput::local_dev("local-dev-host-owner", dir.path().join("local-dev"))
