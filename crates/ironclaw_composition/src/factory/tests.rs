...
use crate::{
    IronClawReadinessDiagnostic, IronClawReadinessState, runtime::SKILL_ACTIVATE_CAPABILITY_ID,
};
...
fn libsql_build_resource_governor_guard_requires_singleton_authority() {
    ...
    assert!(matches!(
        ...
        Err(IronClawBuildError::InvalidConfig { reason })
...
#[test]
fn build_ironclaw_services_uses_filesystem_resource_governor() {
    ...
    let services = runtime
        .block_on(build_ironclaw_services(IronClawBuildInput::local_dev(
...
fn extension_installation_state_path_stays_legacy_for_local_dev() {
    let path =
        local_dev_extension_installation_state_path(IronClawCompositionProfile::LocalDev, None)
            .expect("state path");
...
fn extension_installation_state_path_uses_durable_tenant_root_for_hosted() {
    let identity = IronClawLocalRuntimeIdentity {
...
async fn local_runtime_with_failing_trigger_conversations() -> Arc<IronClawRuntimeSubstrate> {
    ...
    Arc::new(IronClawRuntimeSubstrate {
...
async fn hosted_single_tenant_rejects_local_dev_storage_input() {
    let dir = tempfile::tempdir().expect("tempdir");
    let input = IronClawBuildInput::local_dev(
    ...
    let error = match build_ironclaw_services(input).await {
        Ok(_) => {
            panic!("hosted single-tenant must use hosted single-tenant Postgres storage")
        }
        Err(error) => error,
    };
    let IronClawBuildError::InvalidConfig { reason } = error else {
...
async fn local_dev_memory_documents_persist_across_rebuilds() {
    ...
    let services =
        build_ironclaw_services(IronClawBuildInput::local_dev(owner, local_dev_root.clone()))
            .await
...
async fn local_dev_default_product_auth_preserves_manual_token_across_rebuilds() {
    ...
    let challenge = product_auth
        .request_manual_token_setup(crate::IronClawManualTokenSetupRequest::new(
            scope.clone(),
    ...
    let submitted = product_auth
        .submit_manual_token(crate::IronClawManualTokenSubmitRequest::new(
            scope.clone(),
...
async fn resolve_local_dev_secret_master_key_rejects_malformed_file_with_path_context() {
    ...
    match error {
        IronClawBuildError::InvalidConfig { reason } => {
            assert!(
...
async fn resolve_local_dev_secret_master_key_rejects_set_but_empty_env_without_persisting() {
    ...
    for empty in ["", "   ", "\n\t "] {
        ...
        match error {
            IronClawBuildError::InvalidConfig { reason } => assert!(
                reason.contains(ironclaw_secrets::keychain::SECRETS_MASTER_KEY_ENV),
                "error must name the env var, got: {reason}"
            ),
            other => panic!("expected InvalidConfig, got {other:?}"),
...
fn nearai_bootstrap_input_with_base(
    ...
    IronClawBuildInput::local_dev(owner, root).with_nearai_mcp_bootstrap_config(
...
fn nearai_bootstrap_input(owner: &str, root: PathBuf, api_key: &str) -> IronClawBuildInput {
...
async fn production_libsql_turn_state_uses_configured_runtime_identity() {
    ...
    let graph = match production_runtime {
        IronClawProductionRuntimeServices::LibSql(graph) => graph,
        IronClawProductionRuntimeServices::Postgres(_) => {
...
async fn production_libsql_builder_rejects_invalid_owner_id_at_composition_boundary() {
    ...
    let result = build_ironclaw_services(
        IronClawBuildInput::libsql(
            IronClawCompositionProfile::Production,
            "",
            db,
    ...
    assert!(
        matches!(result, Err(IronClawBuildError::InvalidConfig { ref reason }) if reason.contains("must not be empty")),
...
async fn local_dev_nearai_mcp_auto_bootstraps_from_injected_config() {
    ...
    let services = build_ironclaw_services(nearai_bootstrap_input_with_base(
        owner,
        dir.path().join("local-dev"),
...
async fn local_dev_nearai_mcp_rebootstrap_reuses_existing_account() {
    ...
    let first = build_ironclaw_services(nearai_bootstrap_input(owner, root, "nearai-first-key"))
        .await
        .expect("first local-dev services build");
...
async fn local_dev_nearai_mcp_bootstrap_reinstalls_discovered_reused_credential() {
    ...
    let services = build_ironclaw_services(nearai_bootstrap_input(
        owner,
        dir.path().join("local-dev"),
        "nearai-test-key",
    ))
...
async fn local_dev_services_persist_thread_records_across_rebuilds() {
    ...
    let services =
        build_ironclaw_services(IronClawBuildInput::local_dev("persist-owner", root.clone()))
            .await
            .expect("first local-dev services build");
    services
        .local_runtime
...
fn local_dev_workspace_root_overlapping_skill_root_is_rejected() {
    ...
    for skill_root in [
        ...
        for workspace_root in [
            ...
            assert!(
                matches!(error, IronClawBuildError::InvalidConfig { .. }),
...
fn disabled_services_do_not_include_repl_runtime_substrate() {
    let services = IronClawServices::disabled();
    ...
    assert_eq!(services.readiness.state, IronClawReadinessState::Disabled);
}

#[test]
fn production_readiness_reflects_product_auth_presence() {
    let without_auth = readiness_for(IronClawCompositionProfile::Production, true, true, false);
    assert_eq!(
        without_auth.state,
    ...
    let with_auth = readiness_for(IronClawCompositionProfile::Production, true, true, true);
    assert_eq!(with_auth.state, IronClawReadinessState::ProductionValidated);
...
fn readiness_for_profile_diagnostics_cover_cutover_states() {
    ...
    let yolo = readiness_for(IronClawCompositionProfile::LocalDevYolo, true, true, true);
    assert_eq!(yolo.state, IronClawReadinessState::DevOnly);
    assert_eq!(
        yolo.diagnostics,
...
async fn enable_global_auto_approve_for_context(
    local_runtime: &IronClawRuntimeSubstrate,
    context: &ExecutionContext,
) {
    local_runtime
...
/// `IronClawRuntimeSubstrate` and compares their data halves via
