//! Provider-catalog resolution for the assembled IronClaw runtime.
//!
...
//!    that `build_ironclaw_runtime` wires through the shared LLM provider
//!    chain.
//!
...
use ironclaw_config::{
    IronClawBootConfig, IronClawConfigFile, LlmSlotSelection, reject_inline_secret,
};
use ironclaw_llm::{
...
#[derive(Debug, Error)]
pub enum IronClawLlmCatalogError {
    ...
    /// Explicit IronClaw provider overlay could not be loaded.
...
/// Resolve the default IronClaw runtime LLM from boot config, TOML selection,
/// and environment fallback.
///
/// This is the composition-owned provider/auth seam for standalone IronClaw
/// ingress. Callers pass boot inputs; provider-specific auth details stay in
/// `ironclaw_llm` and never leak into CLI commands.
pub fn resolve_ironclaw_runtime_llm(
    boot: &IronClawBootConfig,
    config_file: Option<&IronClawConfigFile>,
) -> Result<Option<ResolvedIronClawLlm>, IronClawLlmCatalogError> {
...
fn resolve_llm_from_env(
    ...
    ironclaw_llm::resolve_llm_config_from_env(Some(boot.home().providers_file_path().as_path()))
        .map(|maybe_config| maybe_config.map(ResolvedIronClawLlm::from_llm_config))
        .map_err(|source| IronClawLlmCatalogError::EnvResolution { source })
}
...
/// 3. Validate IronClaw-specific secret/env-var policy on catalog and selection.
...
) -> Result<ironclaw_llm::LlmConfig, IronClawLlmCatalogError> {
    let registry = ProviderRegistry::try_load_from_path(user_providers_path)
        .map_err(|source| IronClawLlmCatalogError::CatalogLoad { source })?;
...
fn resolve_allow_missing_key_against_registry(
    ...
    match resolve_against_registry(selection, registry) {
        Ok(config) => Ok(config),
        Err(IronClawLlmCatalogError::ApiKeyEnvUnset { .. }) => {
            let provider_id = selection
                .provider_id
                .as_deref()
                .ok_or(IronClawLlmCatalogError::MissingProviderId)?;
...
/// IronClaw-specific catalog checks used by runtime composition.
#[derive(Debug, thiserror::Error)]
pub enum ProviderCatalogValidationError {
    #[error("invalid provider catalog JSON: {0}")]
    Parse(#[from] serde_json::Error),
    #[error(transparent)]
    Invalid(#[from] IronClawLlmCatalogError),
}

pub fn validate_ironclaw_provider_catalog_contents(
    contents: &str,
...
pub fn resolve_against_registry(
    ...
    let provider =
        registry
            .find(provider_id)
            .ok_or_else(|| IronClawLlmCatalogError::UnknownProvider {
...
fn map_selection_resolution_error(
    ...
    match source {
        ProviderResolutionError::MissingApiKey {
            ...
            {
                Some(env) => IronClawLlmCatalogError::ApiKeyEnvUnset {
                    provider: provider.id.clone(),
                    env,
                },
                None => IronClawLlmCatalogError::ApiKeyEnvUnconfigured {
                    provider: provider.id.clone(),
                },
            }
        }
        ProviderResolutionError::MissingBaseUrl {
            provider: error_provider,
        } if error_provider == provider.id => IronClawLlmCatalogError::BaseUrlUnconfigured {
            provider: provider.id.clone(),
        },
        source => IronClawLlmCatalogError::EnvResolution {
            source: source.into_llm_error(),
        },
    }
}

fn validate_catalog(registry: &ProviderRegistry) -> Result<(), IronClawLlmCatalogError> {
...
fn validate_catalog_text(
    ...
    if reject_inline_secret("provider catalog field", value).is_err() {
        return Err(IronClawLlmCatalogError::CatalogFieldInvalid {
...
mod tests {
    ...
    fn provider_no_base_url_required(id: &str) -> ProviderDefinition {
        ProviderDefinition {
            ...
            base_url_env: Some("IRONCLAW_TEST_OPTIONAL_BASE_URL_UNSET_DO_NOT_SET_9c13".to_string()),
            base_url_required: false,
            api_key_env: None,
            api_key_required: false,
            model_env: "IRONCLAW_TEST_MODEL_UNSET_DO_NOT_SET_9c13".to_string(),
    ...
    fn provider_github_copilot_no_key_required(id: &str) -> ProviderDefinition {
        ProviderDefinition {
            ...
            api_key_env: None,
            api_key_required: false,
            model_env: "IRONCLAW_TEST_GITHUB_COPILOT_MODEL_UNSET_DO_NOT_SET_9c13".to_string(),
    ...
    fn provider_with_protocol(id: &str, protocol: ProviderProtocol) -> ProviderDefinition {
        ProviderDefinition {
            ...
            api_key_env: None,
            api_key_required: false,
            model_env: "IRONCLAW_TEST_DEDICATED_MODEL_UNSET_DO_NOT_SET_9c13".to_string(),
    ...
    fn unknown_provider_lists_known_ids() {
        ...
        match err {
            IronClawLlmCatalogError::UnknownProvider { requested, known } => {
    ...
    fn missing_required_api_key_env_fails_closed() {
        ...
        // free.
        let env_name = "IRONCLAW_TEST_UNSET_API_KEY_DO_NOT_SET_8a3f1c2e";
        debug_assert!(
        ...
        assert!(matches!(
            err,
            IronClawLlmCatalogError::ApiKeyEnvUnset { .. }
        ));
    }
    ...
    fn allow_missing_key_resolves_a_required_key_provider_without_the_env_var_set() {
        let env_name = "IRONCLAW_TEST_UNSET_API_KEY_ALLOW_MISSING_DO_NOT_SET_7d2b";
        debug_assert!(
            std::env::var(env_name).is_err(),
            "test depends on `{env_name}` being unset"
        );
    ...
    fn allow_missing_key_does_not_mask_a_missing_api_key_env_configuration() {
        ...
        assert!(
            matches!(err, IronClawLlmCatalogError::ApiKeyEnvUnconfigured { .. }),
    ...
    fn registry_with_provider_treated_as_keyless_only_affects_the_matched_provider() {
        let registry = ProviderRegistry::new(vec![
            provider_with_required_key("alpha", "IRONCLAW_TEST_ALPHA_KEY_DO_NOT_SET_7d2b"),
            provider_with_required_key("beta", "IRONCLAW_TEST_BETA_KEY_DO_NOT_SET_7d2b"),
        ]);
    ...
    fn malformed_api_key_env_fails_without_echoing_value() {
        ...
        assert!(matches!(
            err,
            IronClawLlmCatalogError::CatalogFieldInvalid {
                field: "api_key_env",
                ..
            }
        ));
    ...
    fn missing_required_base_url_fails_closed() {
        ...
        assert!(matches!(
            err,
            IronClawLlmCatalogError::BaseUrlUnconfigured { .. }
        ));
    }
    ...
    fn explicit_malformed_provider_overlay_fails_closed() {
        ...
        assert!(matches!(err, IronClawLlmCatalogError::CatalogLoad { .. }));
    }
}
