...
//! IronClaw host composition for OpenAI-compatible API routes.
//!
//! The route crate owns DTOs and HTTP handlers, but the IronClaw host owns the
...
use ironclaw_openai_compat::FilesystemOpenAiCompatRefStore;
...
use ironclaw_product_workflow::{
    IronClawStreamEventsRequest, IronClawTimelineRequest, LlmConfigService, LlmConfigServiceError,
    LlmConfigSnapshot, ProductSurface, TIMELINE_VIEW, WebUiAuthenticatedCaller,
};
...
) -> Result<ProtectedRouteMount, IronClawBuildError> {
...
fn openai_compat_ref_root(tenant_id: &TenantId) -> Result<VirtualPath, IronClawBuildError> {
