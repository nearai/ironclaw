...
//!
//! Ties together the read/set-active surface ([`IronClawProviderAdmin`]), the
...
//! IronClaw already resolves an LLM at boot:
...
use ironclaw_config::{IronClawBootConfig, LlmSlotSelection};
...
use crate::{IronClawProviderAdmin, LlmKeyStore, ProviderRepo};
...
pub struct IronClawLlmConfigService {
...
impl IronClawLlmConfigService {
    pub fn new(boot: IronClawBootConfig, keys: LlmKeyStore) -> Self {
    ...
    fn admin(&self) -> IronClawProviderAdmin {
        IronClawProviderAdmin::new(self.boot.clone())
    }
    ...
    async fn admin_list_async(
        &self,
    ) -> Result<crate::IronClawProviderList, crate::IronClawProviderAdminError> {
        let admin = self.admin();
        tokio::task::spawn_blocking(move || admin.list(None, true))
            .await
            .map_err(|error| crate::IronClawProviderAdminError::InvalidRequest {
    ...
    async fn set_provider_async(
        ...
        id: String,
        model: Option<String>,
    ) -> Result<(), crate::IronClawProviderAdminError> {
        let admin = self.admin();
...
#[async_trait]
impl LlmConfigService for IronClawLlmConfigService {
    async fn snapshot(
        &self,
    ...
    async fn start_codex_login(
        ...
        tokio::spawn(async move {
            ...
            if let Err(error) = IronClawProviderAdmin::new(boot).set_provider("openai_codex", None)
            {
...
pub(crate) async fn apply_nearai_login(
    ...
    IronClawProviderAdmin::new(boot.clone())
...
fn metadata_env_key_set(metadata: &crate::IronClawProviderMetadata) -> bool {
...
fn provider_snapshot_base_url(
    provider_id: &str,
    metadata: Option<&crate::IronClawProviderMetadata>,
) -> Option<String> {
...
/// [`crate::IronClawProviderAdmin::probe_candidate`] (onboard's pre-write
/// key/model verification step).
///
/// - Reuses [`IronClawLlmConfigService::probe_provider`]'s `custom_definition`
...
fn map_admin_error(error: crate::IronClawProviderAdminError) -> LlmConfigServiceError {
    use crate::IronClawProviderAdminError as E;
    match error {
...
mod tests {
    ...
    fn boot_for_home(ironclaw_home: &std::path::Path) -> IronClawBootConfig {
        let home = IronClawHome::resolve_from_env_parts(
            Some(ironclaw_home.as_os_str().to_os_string()),
            None,
            None,
        )
    ...
    async fn upsert_provider_persists_overlay_stores_key_and_preserves_existing_key() {
        ...
        let boot = boot_for_home(&ironclaw_home);
        let keys = key_store();
        let service = IronClawLlmConfigService::new(boot.clone(), keys.clone());
    ...
    async fn snapshot_batches_stored_key_metadata_lookup() {
        ...
        let service = IronClawLlmConfigService::new(boot, LlmKeyStore::new(store));
    ...
    async fn probe_override_requires_inline_key_before_using_stored_key() {
        ...
        let boot = boot_for_home(&ironclaw_home);
        let keys = key_store();
        let service = IronClawLlmConfigService::new(boot, keys);
    ...
    async fn probe_keyless_local_provider_is_not_blocked_on_missing_api_key() {
        ...
        let ironclaw_home = temp.path().join("ironclaw-home");
        let boot = boot_for_home(&ironclaw_home);
        let service = IronClawLlmConfigService::new(boot, key_store());
    ...
    /// end-to-end through the real `resolve_ironclaw_runtime_llm`. The env-vs-
    ...
    #[tokio::test]
    async fn ironclaw_runtime_llm_honors_explicit_config_selection() {
        let temp = tempfile::tempdir().expect("tempdir");
        let ironclaw_home = temp.path().join("ironclaw-home");
        let boot = boot_for_home(&ironclaw_home);

        crate::llm_admin::provider_admin::IronClawProviderAdmin::new(boot.clone())
            .set_provider("nearai", Some("deepseek-ai/DeepSeek-V4-Flash"))
            .expect("persist active selection");
        let config_file =
            ironclaw_config::IronClawConfigFile::load(&boot.home().config_file_path())
                .expect("load config file");

        let resolved = crate::llm_admin::llm_catalog::resolve_ironclaw_runtime_llm(
            &boot,
            config_file.as_ref(),
        )
    ...
    async fn upsert_active_failure_rolls_back_overlay_and_new_key() {
        let temp = tempfile::tempdir().expect("tempdir");
        let ironclaw_home = temp.path().join("ironclaw-home");
        std::fs::create_dir_all(ironclaw_home.join("config.toml")).expect("mkdir config path");
        let boot = boot_for_home(&ironclaw_home);
        let keys = key_store();
        let service = IronClawLlmConfigService::new(boot.clone(), keys.clone());
