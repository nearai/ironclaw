...
use ironclaw_product_workflow::{
    ExtensionCredentialSetupService, ExtensionCredentialSubmitRequest, IronClawServicesError,
    IronClawServicesErrorCode, IronClawServicesErrorKind, LifecyclePackageKind,
...
use crate::extension_host::extension_lifecycle::{
    ExtensionActivationMode, IronClawLocalExtensionManagementPort,
};
use crate::extension_host::webui_extension_credentials::ProductAuthExtensionCredentialSetup;
use crate::{IronClawBuildError, IronClawProductAuthServices};
...
pub(crate) async fn bootstrap_nearai_mcp(
...
) -> Result<NearAiMcpBootstrapOutcome, IronClawBuildError> {
    ...
    let provider =
        AuthProviderId::new("nearai").map_err(|error| IronClawBuildError::InvalidConfig {
            reason: format!("NEAR AI MCP provider id is invalid: {error}"),
        })?;
    let requester_extension =
        ExtensionId::new("nearai").map_err(|error| IronClawBuildError::InvalidConfig {
...
fn is_nearai_mcp_disabled_or_removed(error: &IronClawServicesError) -> bool {
    error.code == IronClawServicesErrorCode::Forbidden
        && error.kind == IronClawServicesErrorKind::ParticipantDenied
}

fn is_nearai_mcp_product_auth_temporarily_unavailable(error: &IronClawServicesError) -> bool {
