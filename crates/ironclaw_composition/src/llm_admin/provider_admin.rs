//! IronClaw provider-admin facade.
//!
...
//! IronClaw `$IRONCLAW_REBORN_HOME/config.toml` and reads the shared provider
//! catalog through `ironclaw_llm`.

use std::{fmt, path::PathBuf};

use ironclaw_config::{
    DefaultLlmSlotUpdate, IronClawBootConfig, IronClawConfigFile, LlmSlotFieldUpdate,
    LlmSlotSelection, begin_default_llm_slot_update, update_default_llm_slot,
};
use serde::Serialize;
use thiserror::Error;

#[derive(Debug, Clone)]
pub struct IronClawProviderAdmin {
    boot: IronClawBootConfig,
}

impl IronClawProviderAdmin {
    pub fn new(boot: IronClawBootConfig) -> Self {
        Self { boot }
    }
    ...
    pub fn list(
        ...
        provider: Option<&str>,
        verbose: bool,
    ) -> Result<IronClawProviderList, IronClawProviderAdminError> {
        let home = self.boot.home();
        let registry = self.load_registry()?;
        let config = IronClawConfigFile::load(&home.config_file_path()).map_err(|source| {
            IronClawProviderAdminError::LoadConfig {
        ...
        let providers = if let Some(provider) = provider {
            let def = registry.find(provider).ok_or_else(|| {
                IronClawProviderAdminError::UnknownProvider {
        ...
        Ok(IronClawProviderList {
            providers,
            config_file: home.config_file_path(),
            providers_file: home.providers_file_path(),
            v1_state: IronClawV1State::NotUsed,
        })
    }

    pub fn status(&self) -> Result<IronClawProviderStatus, IronClawProviderAdminError> {
        let home = self.boot.home();
        let registry = self.load_registry()?;
        let config = IronClawConfigFile::load(&home.config_file_path()).map_err(|source| {
            IronClawProviderAdminError::LoadConfig {
        ...
        Ok(IronClawProviderStatus {
            routes: if active.is_some() {
                IronClawModelRoutesState::Configured
            } else {
                IronClawModelRoutesState::NotConfigured
            },
            default: active.map(|selection| IronClawProviderSelection {
    ...
    pub fn set_model(
        &self,
        model: &str,
    ) -> Result<IronClawProviderWriteOutcome, IronClawProviderAdminError> {
        let model = model.trim();
        if model.is_empty() {
            return Err(IronClawProviderAdminError::InvalidRequest {
        ...
        let session = begin_default_llm_slot_update(&config_path).map_err(|source| {
            IronClawProviderAdminError::UpdateConfig {
        ...
        let provider_id = session
            .default_llm_slot()
            .map_err(|source| IronClawProviderAdminError::UpdateConfig {
                path: config_path.clone(),
                source: Box::new(source),
            })?
            .as_ref()
            .and_then(|selection| selection.provider_id.as_deref())
            .ok_or_else(|| IronClawProviderAdminError::InvalidRequest {
        ...
        Ok(IronClawProviderWriteOutcome {
    ...
    pub fn resolve_provider_id(
        &self,
        provider: &str,
    ) -> Result<String, IronClawProviderAdminError> {
        ...
        let def =
            registry
                .find(provider)
                .ok_or_else(|| IronClawProviderAdminError::UnknownProvider {
                    provider: provider.to_string(),
                    providers_file: home.providers_file_path(),
                    known: known_provider_ids(&registry),
                })?;
    ...
    pub fn effective_api_key_required(
        &self,
        provider_id: &str,
    ) -> Result<Option<bool>, IronClawProviderAdminError> {
        let registry = self.load_registry()?;
    ...
    ///   `ironclaw config set` / `models set-provider`, just not on
    ///   the numbered menu.
    ...
    ///   `ironclaw_dependency_boundaries`).
    /// - `api_key_required` is a MENU-LEVEL value — see
    ///   [`effective_api_key_required`]'s doc for the `nearai` override.
    pub fn menu_entries(&self) -> Result<Vec<ProviderMenuEntry>, IronClawProviderAdminError> {
        let registry = self.load_registry()?;
        Ok(registry
            .selectable()
            .into_iter()
    ...
    /// variables — the same env resolution `resolve_ironclaw_runtime_llm`'s
    ...
    /// directly, per `ironclaw_dependency_boundaries`) can offer onboard's
    ...
    pub fn detect_env_llm(&self) -> Result<Option<DetectedEnvLlm>, IronClawProviderAdminError> {
        ...
        ironclaw_llm::resolve_provider_config_from_env(Some(providers_path.as_path()))
            ...
            .map_err(|source| IronClawProviderAdminError::EnvDetection {
                source: Box::new(source),
            })
    }
    ...
    pub fn resolve_env_api_key(
        &self,
        provider_id: &str,
    ) -> Result<Option<secrecy::SecretString>, IronClawProviderAdminError> {
        let providers_path = self.boot.home().providers_file_path();
        let resolved =
            ironclaw_llm::resolve_provider_config_from_env(Some(providers_path.as_path()))
                .map_err(|source| IronClawProviderAdminError::EnvDetection {
    ...
    pub async fn probe_candidate(
        ...
        model: Option<&str>,
    ) -> Result<ProviderProbeOutcome, IronClawProviderAdminError> {
        let home = self.boot.home();
        let registry = self.load_registry()?;
    ...
    fn load_registry(&self) -> Result<ironclaw_llm::ProviderRegistry, IronClawProviderAdminError> {
        let providers_path = self.boot.home().providers_file_path();
        ironclaw_llm::ProviderRegistry::try_load_from_path(Some(providers_path.as_path())).map_err(
            |error| IronClawProviderAdminError::LoadRegistry {
                path: providers_path,
...
pub struct IronClawProviderList {
    pub providers: Vec<IronClawProviderInfo>,
    #[serde(skip_serializing)]
    pub config_file: PathBuf,
    #[serde(skip_serializing)]
    pub providers_file: PathBuf,
    pub v1_state: IronClawV1State,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct IronClawProviderInfo {
    pub id: String,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub metadata: Option<IronClawProviderMetadata>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct IronClawProviderMetadata {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct IronClawProviderStatus {
    pub routes: IronClawModelRoutesState,
    pub default: Option<IronClawProviderSelection>,
    #[serde(skip_serializing)]
    pub config_file: PathBuf,
    #[serde(skip_serializing)]
    pub providers_file: PathBuf,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct IronClawProviderSelection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct IronClawProviderWriteOutcome {
...
/// [`IronClawProviderAdmin::detect_env_llm`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// see [`IronClawProviderAdmin::probe_candidate`].
...
/// `ironclaw_cli::commands::config::init::PROVIDERS_STUB` seeds into
/// a fresh `providers.json` — see [`IronClawProviderAdmin::menu_entries`]'s
...
/// [`IronClawProviderAdmin::menu_entries`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// IronClaw onboarding purposes — the value [`IronClawProviderAdmin::menu_entries`]
/// puts in [`ProviderMenuEntry::api_key_required`], and what
/// [`IronClawProviderAdmin::effective_api_key_required`] returns for a single
...
/// not wired in IronClaw (no `SessionRenewer` attaches at `serve` boot), so
...
pub enum IronClawV1State {
    #[serde(rename = "not-used")]
    NotUsed,
}

impl IronClawV1State {
...
impl fmt::Display for IronClawV1State {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
pub enum IronClawModelRoutesState {
...
impl fmt::Display for IronClawModelRoutesState {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(self.as_str())
    }
}
...
#[derive(Debug, Error)]
pub enum IronClawProviderAdminError {
    ...
    LoadConfig {
        path: PathBuf,
        source: Box<ironclaw_config::IronClawConfigFileError>,
    },
    ...
    UpdateConfig {
        path: PathBuf,
        source: Box<ironclaw_config::IronClawConfigFileUpdateError>,
    },
    /// [`IronClawProviderAdmin::detect_env_llm`]'s "partial env" outcome: some
...
/// precedence in [`crate::llm_admin::llm_catalog::resolve_ironclaw_runtime_llm`]:
...
fn active_llm_selection(
    config: Option<&IronClawConfigFile>,
    registry: &ironclaw_llm::ProviderRegistry,
    providers_path: Option<&std::path::Path>,
) -> Option<ActiveLlmSelection> {
    if let Some(selection) = config.and_then(IronClawConfigFile::default_llm_slot) {
...
fn provider_info(
    ...
    IronClawProviderInfo {
        ...
        active_model,
        metadata: verbose.then(|| IronClawProviderMetadata {
            aliases: def.aliases.clone(),
...
mod tests {
    ...
    fn test_admin() -> IronClawProviderAdmin {
        let temp = tempfile::tempdir().expect("tempdir");
        let home = ironclaw_config::IronClawHome::resolve_from_env_parts(
            Some(temp.path().join("ironclaw-home").as_os_str().to_os_string()),
            None,
            None,
        )
        ...
        std::mem::forget(temp);
        IronClawProviderAdmin::new(IronClawBootConfig::new(
            home,
            ironclaw_config::IronClawProfile::LocalDev,
        ))
    }
    ...
    fn menu_entries_lists_nearai_first_and_requires_an_api_key() {
        ...
        // Menu-level override: no session-token auth wired in IronClaw, so
        // nearai requires a key here despite the raw catalog entry.
        assert!(
            first.api_key_required,
            "nearai must require an API key on the IronClaw onboard menu (no session-token auth \
             wired): {first:?}"
        );
    }
    ...
    fn menu_entries_excludes_the_example_overlay_provider() {
        ...
        .expect("valid IronClaw home");
        std::fs::create_dir_all(home.path()).expect("create IronClaw home dir");
        ...
        let admin = IronClawProviderAdmin::new(IronClawBootConfig::new(
            home,
            ironclaw_config::IronClawProfile::LocalDev,
        ));
        let entries = admin.menu_entries().expect("menu entries load");
    ...
    // `ironclaw_product_api_crates_do_not_bind_http_ingress` greps every
