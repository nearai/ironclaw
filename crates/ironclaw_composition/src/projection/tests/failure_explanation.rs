...
fn failure_summary_covers_every_loop_failure_kind_category() {
    ...
    for (category, expected_summary) in expected {
        let summary =
            ironclaw_runner::failure_summary::ironclaw_failure_summary_for_category(Some(category));
...
#[test]
fn failure_summary_covers_ironclaw_failure_category_constants() {
    let expected = [
        (
    ...
    let source_values = ironclaw_failure_category_constant_values_from_source();
    let expected_values = expected
        .iter()
        .map(|(category, _)| *category)
        .collect::<std::collections::BTreeSet<_>>();
...
fn failure_summary_uses_safe_generic_fallback_for_unknown_categories() {
    let summary = ironclaw_runner::failure_summary::ironclaw_failure_summary_for_category(Some(
        "new_snake_case_code",
    ));
...
fn ironclaw_failure_category_constant_values_from_source()
