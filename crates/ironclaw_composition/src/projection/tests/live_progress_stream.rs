...
struct LiveProjectionFixture {
    ...
    thread_id: ThreadId,
    scope: TurnScope,
    services: IronClawProjectionServices,
...
async fn webui_event_stream_rebases_live_cursor_from_prior_process_epoch() {
    ...
    let services_before_restart = build_ironclaw_projection_services(
        Arc::clone(&event_log),
    ...
    let services_after_restart = build_ironclaw_projection_services(
        event_log,
...
// same `IronClawProjectionServices` over a run's lifetime (e.g. the milestone
...
// `build_ironclaw_projection_services` and `live_projection_publisher` — a
