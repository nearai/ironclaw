//! Provider-identity → IronClaw user resolution for channel surfaces.
//!
...
pub struct IronClawIdentityProviderId(String);

impl IronClawIdentityProviderId {
    pub fn new(value: impl Into<String>) -> Result<Self, IronClawUserIdentityBindingError> {
...
impl std::fmt::Display for IronClawIdentityProviderId {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IronClawIdentityProviderUserId(String);

impl IronClawIdentityProviderUserId {
    pub fn new(value: impl Into<String>) -> Result<Self, IronClawUserIdentityBindingError> {
        let value = value.into();
...
impl std::fmt::Display for IronClawIdentityProviderUserId {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(self.as_str())
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IronClawUserIdentityBinding {
    pub provider: IronClawIdentityProviderId,
    pub provider_user_id: IronClawIdentityProviderUserId,
    pub user_id: UserId,
}

#[derive(Debug, Clone, PartialEq, Eq, Error)]
pub enum IronClawUserIdentityBindingError {
    #[error("IronClaw user identity binding backend unavailable: {0}")]
    Backend(String),
    #[error("provider identity is already bound to a different IronClaw user")]
    ProviderIdentityAlreadyBound,
    #[error("invalid IronClaw user identity {field}: {reason}")]
...
pub trait IronClawUserIdentityBindingStore: Send + Sync {
...
#[async_trait::async_trait]
pub trait IronClawUserIdentityBindingDeleteStore: Send + Sync {
...
fn validate_identity_value(
    ...
    if value.is_empty() {
        return Err(IronClawUserIdentityBindingError::InvalidIdentityField {
            field,
...
#[derive(Debug, Error)]
pub enum IronClawUserIdentityLookupError {
    #[error("IronClaw user identity backend unavailable: {0}")]
    Backend(String),
...
#[async_trait::async_trait]
pub trait IronClawUserIdentityLookup: Send + Sync {
    async fn resolve_user_identity(
        &self,
        provider: &str,
        provider_user_id: &str,
    ) -> Result<Option<UserId>, IronClawUserIdentityLookupError>;
    ...
    async fn user_has_provider_binding(
        ...
        provider: &str,
        user_id: &UserId,
    ) -> Result<bool, IronClawUserIdentityLookupError>;
    ...
    async fn user_has_provider_binding_with_provider_user_id_prefix(
        ...
        Err(IronClawUserIdentityLookupError::Backend(
...
pub struct ProviderIdentityActorResolver {
    ...
    lookup: Arc<dyn IronClawUserIdentityLookup>,
...
mod tests {
    ...
    fn resolver(lookup: Arc<dyn IronClawUserIdentityLookup>) -> ProviderIdentityActorResolver {
    ...
    async fn any_actor_kind_resolver_matches_every_kind_for_its_adapter() {
        ...
        let resolver = ProviderIdentityActorResolver::for_any_actor_kind(
            "acmechat",
            "acmechat",
            lookup.clone() as Arc<dyn IronClawUserIdentityLookup>,
        );
    ...
    #[async_trait::async_trait]
    impl IronClawUserIdentityLookup for RecordingLookup {
        async fn resolve_user_identity(
            &self,
            provider: &str,
            provider_user_id: &str,
    ...
    #[async_trait::async_trait]
    impl IronClawUserIdentityLookup for RevocableLookup {
        async fn resolve_user_identity(
            &self,
            _provider: &str,
            provider_user_id: &str,
    ...
    #[async_trait::async_trait]
    impl IronClawUserIdentityLookup for FailingLookup {
        async fn resolve_user_identity(
            &self,
            _provider: &str,
            _provider_user_id: &str,
        ) -> Result<Option<UserId>, IronClawUserIdentityLookupError> {
            Err(IronClawUserIdentityLookupError::Backend("db down".into()))
        }
