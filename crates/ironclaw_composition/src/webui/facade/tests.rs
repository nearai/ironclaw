...
use ironclaw_product_workflow::{
    IronClawOperatorToolCatalog, IronClawOperatorToolInfo, OPERATOR_SERVICE_LIFECYCLE_OPERATION,
};
...
async fn operator_tool_catalog_hides_foreign_private_tools() {
    ...
    let port = Arc::new(IronClawLocalExtensionManagementPort::new(
    ...
    let ids_for = |tools: Vec<IronClawOperatorToolInfo>| {
        tools
            .into_iter()
...
async fn build_webui_services_wires_lifecycle_owner_identity() {
    ...
    let input = crate::IronClawRuntimeInput::from_services(
        crate::IronClawBuildInput::local_dev("runtime-owner", dir.path().join("local-dev"))
            .with_runtime_policy(
                crate::local_dev_runtime_policy().expect("local-dev policy resolves"),
            ),
    )
    .with_identity(crate::IronClawRuntimeIdentity {
        tenant_id: "tenant-alpha".to_string(),
        agent_id: "agent-alpha".to_string(),
        source_binding_id: "webui-test-source".to_string(),
        reply_target_binding_id: "webui-test-reply".to_string(),
    });
    let runtime = crate::build_ironclaw_runtime(input)
        .await
    ...
    let error = OPERATOR_SERVICE_LIFECYCLE_OPERATION
        .execute_on(
            bundle.api.as_ref(),
            caller("bob"),
            ironclaw_product_workflow::IronClawOperatorServiceLifecycleRequest {
                action: ironclaw_product_workflow::IronClawOperatorServiceLifecycleAction::Status,
            },
        )
        .await
...
async fn readiness_operator_status_service_generates_timestamp_per_call() {
    let service = ReadinessOperatorStatusService::new(IronClawReadiness::disabled());
...
async fn readiness_operator_status_includes_stable_readiness_diagnostics() {
    ...
    assert_eq!(response.overall, IronClawOperatorStatusState::Blocked);
    let readiness_check = response
        .checks
        .iter()
        .find(|check| check.id == "readiness_composition_profile")
        .expect("readiness diagnostic check");
    assert_eq!(readiness_check.status, IronClawOperatorStatusState::Blocked);
    assert_eq!(
        readiness_check.severity,
        IronClawOperatorStatusSeverity::Critical
    );
    assert!(
...
async fn readiness_operator_status_keeps_info_diagnostics_ready() {
    let service = ReadinessOperatorStatusService::new(IronClawReadiness {
        profile: crate::IronClawCompositionProfile::Production,
        state: crate::IronClawReadinessState::ProductionValidated,
        ...
        diagnostics: vec![IronClawReadinessDiagnostic {
            profile: crate::IronClawCompositionProfile::Production,
            component: crate::IronClawReadinessDiagnosticComponent::RuntimeHttpEgress,
            reason: crate::IronClawReadinessDiagnosticReason::Unverified,
    ...
    assert_eq!(response.overall, IronClawOperatorStatusState::Ready);
    let readiness_check = response
        .checks
        .iter()
        .find(|check| check.id == "readiness_runtime_http_egress")
        .expect("readiness info diagnostic check");
    assert_eq!(readiness_check.status, IronClawOperatorStatusState::Ready);
    assert_eq!(
        readiness_check.severity,
