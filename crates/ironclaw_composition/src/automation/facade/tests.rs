...
use ironclaw_product_workflow::{
    ...
    AutomationListRequest, AutomationProductFacade, IronClawAutomationHoldReason,
    IronClawAutomationRecentRunStatus, IronClawAutomationRunStatus, IronClawAutomationSource,
    IronClawAutomationState, IronClawListThreadsResponse, IronClawServices,
    IronClawServicesErrorCode, IronClawServicesErrorKind, ListPendingApprovalsRequest,
    ListPendingApprovalsResponse, PendingApprovalInteractionView, ProductAgentBoundCaller,
    ProductSurface, ProductWorkflowError, ResolveApprovalInteractionRequest,
    ResolveApprovalInteractionResponse, THREADS_VIEW, WebUiAuthenticatedCaller,
    WebUiListThreadsRequest, approval_gate_ref, automation_trigger_thread_metadata_json,
};
...
use super::IronClawAutomationProductFacade;
...
fn facade_over(repo: Arc<dyn TriggerRepository>) -> IronClawAutomationProductFacade {
    IronClawAutomationProductFacade::new(repo, missing_lookup())
}
...
async fn automation_facade_forwards_caller_scope_to_repository() {
    ...
    assert_eq!(result[0].state, IronClawAutomationState::Scheduled);
...
async fn automation_facade_maps_active_trigger_states() {
    ...
    let active_states = [
        ...
        (TriggerState::Paused, IronClawAutomationState::Paused, true),
    ];
...
async fn automation_facade_maps_run_history_and_skips_batch_when_run_limit_zero() {
    ...
    assert_eq!(mapped.status, IronClawAutomationRecentRunStatus::Ok);
...
async fn notification_thread_list_discovers_pending_approval_from_real_run_history() {
    ...
    let response: IronClawListThreadsResponse = THREADS_VIEW
...
async fn automation_facade_maps_trigger_run_status_and_last_status() {
    ...
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].last_status, Some(IronClawAutomationRunStatus::Ok));

    // Verify Running status mapping via run record helper
    let run = make_run_record(id, TriggerRunHistoryStatus::Running);
    let mapped = super::map_recent_run(&run).expect("map_recent_run");
    assert_eq!(mapped.status, IronClawAutomationRecentRunStatus::Running);
}
...
fn map_trigger_error_preserves_blocked_materialization_semantics() {
    ...
    assert_eq!(error.kind, IronClawServicesErrorKind::ParticipantDenied);
...
async fn automation_facade_include_completed_returns_completed_automations() {
    ...
    assert_eq!(
        completed.state,
        IronClawAutomationState::Completed,
...
async fn automation_facade_active_hold_nonterminal_reports_in_progress() {
    ...
    let facade = IronClawAutomationProductFacade::new(repo, lookup);
    let result = facade
        .list_automations(c, automation_list_request(10, 0))
        .await
        .expect("list automations");
    ...
    assert_eq!(hold.reason, IronClawAutomationHoldReason::InProgress);
    assert!(
        hold.since.is_some(),
...
async fn automation_facade_active_hold_blocked_auth_reports_auth() {
    ...
    assert_eq!(hold.reason, IronClawAutomationHoldReason::Auth);
}
...
async fn automation_facade_claimed_but_unaccepted_record_reports_other_without_lookup() {
    ...
    assert_eq!(hold.reason, IronClawAutomationHoldReason::Other);
}
