...
use ironclaw_product_workflow::{AutomationName, AutomationProductFacade, IronClawAutomationState};
...
async fn pause_and_resume_update_scoped_trigger_state() {
    ...
    assert_eq!(
        ...
        IronClawAutomationState::Paused
    );
    assert!(
    ...
    assert_eq!(
        ...
        IronClawAutomationState::Scheduled
    );
    assert_eq!(
        repo.list_due_triggers(now(), 10)
            .await
