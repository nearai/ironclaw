...
//!   `IronClawStorageInput`. This value carries only the policy request.

use ironclaw_event_store::IronClawProfile;
...
use crate::readiness::{IronClawReadinessDiagnostic, IronClawReadinessState};

/// Which runtime substrate a deployment assembles.
///
/// Replaces the `requires_production_shape` / `uses_local_runtime_substrate`
/// profile predicates as the value `build_ironclaw_services` and
/// `build_ironclaw_runtime` dispatch on: a deployment selects a substrate, it
...
/// `IronClawStorageInput` variants: the question "does this deployment take a
...
pub enum StorageShape {
    ...
    None,
    /// A local filesystem root (`IronClawStorageInput::LocalDev`).
    LocalDevRoot,
...
pub enum TrafficPolicy {
    /// IronClaw is switched off; starting a runtime is an error.
    Disabled,
    ...
    Serve {
        required_readiness: IronClawReadinessState,
...
/// policy on `IronClawBuildInput` instead.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// `match`ing a [`IronClawCompositionProfile`] — which substrate to assemble,
...
/// - Storage roots, workspace paths, and connection pools are runtime
///   *handles*, not deployment policy — they continue to ride
///   `IronClawStorageInput`. This value carries only the policy request and the
///   shape selections.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DeploymentConfig {
    ...
    /// `ironclaw_deployment_mode_branching_ratchet` architecture test holds the
    /// line.
    profile: IronClawCompositionProfile,
    ...
    event_store_profile: IronClawProfile,
...
impl DeploymentConfig {
    /// IronClaw switched off: no substrate, no traffic, disabled readiness.
    pub fn disabled() -> Self {
        Self {
            ...
            event_store_profile: IronClawProfile::LocalDev,
    ...
    pub fn local_dev() -> Self {
        Self {
            profile: IronClawCompositionProfile::LocalDev,
            ...
            traffic: TrafficPolicy::Serve {
                required_readiness: IronClawReadinessState::DevOnly,
                veto_on_production_blocking_diagnostic: false,
            },
            readiness: ReadinessContract {
                state: IronClawReadinessState::DevOnly,
                diagnostics: vec![IronClawReadinessDiagnostic::local_dev()],
            },
            event_store_profile: IronClawProfile::LocalDev,
            hosted_extension_installation_state: false,
    ...
    pub fn local_dev_yolo(confirm_host_access: bool) -> Self {
        Self {
            profile: IronClawCompositionProfile::LocalDevYolo,
            policy_request: Some(RuntimePolicyRequest {
                deployment: DeploymentMode::LocalSingleUser,
            ...
            readiness: ReadinessContract {
                state: IronClawReadinessState::DevOnly,
                diagnostics: vec![IronClawReadinessDiagnostic::local_dev_yolo()],
            },
            ..Self::local_dev()
        }
    }
    ...
    pub fn hosted_single_tenant() -> Self {
        Self {
            ...
            substrate: RuntimeSubstrate::Local,
            traffic: TrafficPolicy::Serve {
                required_readiness: IronClawReadinessState::HostedSingleTenantValidated,
                veto_on_production_blocking_diagnostic: false,
            },
            readiness: ReadinessContract {
                state: IronClawReadinessState::HostedSingleTenantValidated,
                diagnostics: vec![IronClawReadinessDiagnostic::hosted_single_tenant()],
            },
            event_store_profile: IronClawProfile::LocalDev,
    ...
    pub fn hosted_single_tenant_volume() -> Self {
        Self {
            ...
            readiness: ReadinessContract {
                state: IronClawReadinessState::HostedSingleTenantVolumePreviewValidated,
                diagnostics: vec![IronClawReadinessDiagnostic::hosted_single_tenant_volume()],
            },
            hosted_extension_installation_state: true,
            storage_shape: StorageShape::LocalDevRoot,
    ...
    pub fn production() -> Self {
        Self {
            ...
            traffic: TrafficPolicy::Serve {
                required_readiness: IronClawReadinessState::ProductionValidated,
                veto_on_production_blocking_diagnostic: true,
            },
            readiness: ReadinessContract {
                state: IronClawReadinessState::ProductionValidated,
                diagnostics: Vec::new(),
            },
            event_store_profile: IronClawProfile::Production,
            hosted_extension_installation_state: false,
    ...
    pub fn migration_dry_run() -> Self {
        Self {
            profile: IronClawCompositionProfile::MigrationDryRun,
            traffic: TrafficPolicy::ValidateOnly,
            readiness: ReadinessContract {
                state: IronClawReadinessState::MigrationDryRunValidated,
                diagnostics: Vec::new(),
            },
            ..Self::production()
        }
    ...
    pub fn for_profile(profile: IronClawCompositionProfile, confirm_host_access: bool) -> Self {
        match profile {
            IronClawCompositionProfile::Disabled => Self::disabled(),
            IronClawCompositionProfile::LocalDev => Self::local_dev(),
            IronClawCompositionProfile::LocalDevYolo => Self::local_dev_yolo(confirm_host_access),
            IronClawCompositionProfile::HostedSingleTenant => Self::hosted_single_tenant(),
            IronClawCompositionProfile::HostedSingleTenantVolume => {
                Self::hosted_single_tenant_volume()
            }
            IronClawCompositionProfile::Production => Self::production(),
            IronClawCompositionProfile::MigrationDryRun => Self::migration_dry_run(),
        }
    }
    ...
    pub(crate) fn event_store_profile(&self) -> IronClawProfile {
    ...
    /// on `IronClawBuildInput` instead. Distinguishing "no request" from "a
...
#[derive(Debug, Error)]
pub enum IronClawRuntimeProfileError {
    #[error("profile={profile} is not a local IronClaw runtime profile")]
    UnsupportedProfile { profile: IronClawCompositionProfile },
    ...
    MissingPolicyRequest { profile: IronClawCompositionProfile },
}

#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct IronClawRuntimeProfileOptions {
...
pub(crate) fn deployment_config_for_profile(
    profile: IronClawCompositionProfile,
    options: IronClawRuntimeProfileOptions,
) -> Result<DeploymentConfig, IronClawRuntimeProfileError> {
    ...
    if !config.uses_local_dev_storage_input() {
        return Err(IronClawRuntimeProfileError::UnsupportedProfile { profile });
    }
    Ok(config)
}
...
pub fn local_runtime_build_input(
    ...
    root: PathBuf,
) -> Result<IronClawBuildInput, IronClawRuntimeProfileError> {
    local_runtime_build_input_with_options(
        profile,
        owner_id,
        root,
        IronClawRuntimeProfileOptions::default(),
    )
}
...
pub fn local_runtime_build_input_with_options(
    ...
    if profile == IronClawCompositionProfile::HostedSingleTenantVolume {
    ...
    let policy = deployment
        .resolve()?
        .ok_or(IronClawRuntimeProfileError::MissingPolicyRequest { profile })?;
    Ok(
        IronClawBuildInput::local_dev_from_deployment(deployment, owner_id, root)
...
pub(crate) fn hosted_single_tenant_volume_build_input(
    ...
    let policy = hosted_single_tenant_volume_runtime_policy()
        .map_err(IronClawRuntimeProfileError::Policy)?;
    Ok(IronClawBuildInput::local_dev_with_profile(
        IronClawCompositionProfile::HostedSingleTenantVolume,
        owner_id,
        root,
    )
...
pub fn local_dev_yolo_runtime_policy(
    ...
    .map_err(|error| match error {
        IronClawRuntimeProfileError::Policy(error) => error,
        IronClawRuntimeProfileError::UnsupportedProfile { .. } => {
            unreachable!("local-dev-yolo is a local runtime profile")
        }
        IronClawRuntimeProfileError::MissingPolicyRequest { .. } => {
...
fn local_runtime_policy(
    profile: IronClawCompositionProfile,
    options: IronClawRuntimeProfileOptions,
) -> Result<EffectiveRuntimePolicy, IronClawRuntimeProfileError> {
...
mod tests {
    ...
    fn readiness_contract_travels_on_the_config() {
        ...
        assert_eq!(disabled.readiness().state, IronClawReadinessState::Disabled);
...
mod local_runtime_profile_tests {
    ...
    fn yolo_disclosure_reaches_both_the_carried_deployment_and_the_resolved_policy() {
        ...
        // The hazard being pinned: `IronClawBuildInput::new` cannot know the
        ...
        let dir = std::env::temp_dir().join("ironclaw-yolo-disclosure-test");
    ...
    fn unconfirmed_yolo_fails_closed_before_an_input_is_built() {
        let dir = std::env::temp_dir().join("ironclaw-yolo-unconfirmed-test");
        ...
        assert!(matches!(
            error,
            IronClawRuntimeProfileError::Policy(ResolveError::YoloRequiresDisclosure { .. })
        ));
    }
