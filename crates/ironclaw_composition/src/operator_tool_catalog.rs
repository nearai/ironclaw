...
use ironclaw_product_workflow::{IronClawOperatorToolCatalog, IronClawOperatorToolInfo};
...
pub(crate) struct ActiveRegistryOperatorToolCatalog {
    ...
    synthetic_tools: Arc<[IronClawOperatorToolInfo]>,
    ...
    owner_source: Option<Arc<IronClawLocalExtensionManagementPort>>,
}

impl ActiveRegistryOperatorToolCatalog {
    pub(crate) fn new(
        registry: Arc<SharedExtensionRegistry>,
        synthetic_tools: Vec<IronClawOperatorToolInfo>,
        owner_source: Option<Arc<IronClawLocalExtensionManagementPort>>,
    ) -> Self {
        Self {
            registry,
...
#[async_trait]
impl IronClawOperatorToolCatalog for ActiveRegistryOperatorToolCatalog {
    async fn list_operator_tools(&self, caller: &UserId) -> Vec<IronClawOperatorToolInfo> {
        ...
        let mut tools = snapshot
            ...
            .map(|descriptor| IronClawOperatorToolInfo {
