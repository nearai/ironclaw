...
//! concrete `ComposedTurnStateStore` type: `IronClawIntegrationGroup`'s
...
//! `IronClawServices.local_runtime.turn_state`, which is this crate's own
//! `build_ironclaw_services` composition. Production wiring is unaffected:
//! `build_ironclaw_runtime` still passes `Arc::clone(&local_runtime.turn_state)`
...
// `IronClawIntegrationGroup`'s `FilesystemTurnStateRowStore<HarnessTurnBackend>`)
