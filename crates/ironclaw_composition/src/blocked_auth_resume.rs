...
use crate::product_auth::api::auth::IronClawAuthContinuationDispatcher;
...
pub(crate) struct BlockedAuthResumeFanout {
    inner: Arc<dyn IronClawAuthContinuationDispatcher>,
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for BlockedAuthResumeFanout {
...
mod tests {
    ...
    #[async_trait]
    impl IronClawAuthContinuationDispatcher for RecordingInnerDispatcher {
        async fn dispatch_auth_continuation(
            &self,
            event: AuthContinuationEvent,
        ) -> Result<(), AuthProductError> {
