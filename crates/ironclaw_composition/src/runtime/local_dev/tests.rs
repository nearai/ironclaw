...
mod tests {
    ...
    use ironclaw_product_workflow::{
        IronClawOutboundDeliveryTargetId, LifecyclePackageKind, LifecyclePackageRef,
    ...
    use crate::outbound::{
        IronClawOutboundPreferencesFacade, OutboundDeliveryTargetEntry,
    ...
    async fn extension_remove_tool_discloses_generic_unpair_disconnect_semantics() {
        ...
        let services = crate::build_ironclaw_services(
            crate::IronClawBuildInput::local_dev_with_profile(
    ...
    async fn local_dev_result_read_continues_exactly_where_first_look_preview_truncated() {
        let dir = tempfile::tempdir().expect("tempdir");
        let services = crate::build_ironclaw_services(crate::IronClawBuildInput::local_dev(
    ...
    async fn local_dev_project_create_tool_persists_project_visible_to_owner() {
        ...
        let listed = local_runtime
            ...
            .list_projects(
                ...
                ironclaw_product_workflow::IronClawListProjectsRequest { limit: None },
            )
            .await
    ...
    async fn local_dev_outbound_delivery_targets_list_and_target_set_use_provider() {
        ...
        let local_runtime = services
            ...
            .expect("local runtime substrate");
        let slack_target_id =
            IronClawOutboundDeliveryTargetId::new("slack:test-dm").expect("target id");
        ...
        let outbound_preferences_facade: Arc<dyn OutboundPreferencesProductFacade> =
            Arc::new(IronClawOutboundPreferencesFacade::new(
        ...
        let missing_target_id =
            IronClawOutboundDeliveryTargetId::new("slack:missing-approved-dm").expect("target id");
    ...
    async fn local_dev_yolo_outbound_delivery_target_set_bypasses_approval_gate() {
        let dir = tempfile::tempdir().expect("tempdir");
        let services = crate::build_ironclaw_services(
            crate::IronClawBuildInput::local_dev(
        ...
        let local_runtime = services
            ...
            .expect("local runtime substrate");
        let slack_target_id =
            IronClawOutboundDeliveryTargetId::new("slack:yolo-dm").expect("target id");
        let slack_target_summary = OutboundDeliveryTargetSummary::new(
            OutboundDeliveryTargetId::new(slack_target_id.as_str()).expect("target id"),
            "slack",
            "Slack DM",
        ...
        let missing_target_id =
            IronClawOutboundDeliveryTargetId::new("slack:missing-dm").expect("target id");
        let missing_set_candidate = port
            .register_provider_tool_call(RegisterProviderToolCallRequest::new(
                provider_tool_call_with_name(
                    "builtin__outbound_delivery_target_set",
    ...
    async fn local_dev_capability_port_omits_host_disclosure_without_confirmed_host_mount() {
        ...
        let raw_workspace = workspace_root
            ...
            .into_owned();
        let services = crate::build_ironclaw_services(
            crate::IronClawBuildInput::local_dev("local-dev-no-host-owner", storage_root)
                .with_local_dev_workspace_root(workspace_root.clone()),
        )
        .await
        .expect("local-dev services build"); // safety: test-only assertion in #[cfg(test)] module.
