...
use ironclaw_product_workflow::{
    IronClawOutboundDeliveryTargetId, IronClawServicesError, IronClawServicesErrorCode,
...
/// killing the turn.
///
/// Safe summaries stay fixed and host-authored: `IronClawServicesError` carries a
...
fn outbound_delivery_outcome(
    ...
    match error.code {
        IronClawServicesErrorCode::InvalidRequest | IronClawServicesErrorCode::NotFound => {
...
mod tests {
    ...
    use ironclaw_product_workflow::IronClawServicesErrorKind;
    use ironclaw_turns::run_profile::LoopSafeSummary;

    fn service_error(code: IronClawServicesErrorCode) -> IronClawServicesError {
    ...
    fn invalid_request_is_a_recoverable_tool_failure_not_terminal() {
        let outcome =
            outbound_delivery_outcome(service_error(IronClawServicesErrorCode::InvalidRequest))
    ...
    fn not_found_is_a_recoverable_tool_failure_not_terminal() {
        let outcome = outbound_delivery_outcome(service_error(IronClawServicesErrorCode::NotFound))
    ...
    fn unauthenticated_is_a_recoverable_denial_not_terminal() {
        let outcome =
            outbound_delivery_outcome(service_error(IronClawServicesErrorCode::Unauthenticated))
    ...
    fn forbidden_is_a_recoverable_denial_not_terminal() {
        let outcome =
            outbound_delivery_outcome(service_error(IronClawServicesErrorCode::Forbidden))
    ...
    fn conflict_is_a_recoverable_tool_failure_not_terminal() {
        let outcome = outbound_delivery_outcome(service_error(IronClawServicesErrorCode::Conflict))
    ...
    fn rate_limited_is_a_recoverable_tool_failure_not_terminal() {
        let outcome =
            outbound_delivery_outcome(service_error(IronClawServicesErrorCode::RateLimited))
    ...
    fn unavailable_is_a_recoverable_tool_failure_not_terminal() {
        let outcome =
            outbound_delivery_outcome(service_error(IronClawServicesErrorCode::Unavailable))
    ...
    fn internal_service_error_stays_terminal() {
        let error = outbound_delivery_outcome(service_error(IronClawServicesErrorCode::Internal))
    ...
    fn set_delivery_target_summary_is_fixed_and_validator_safe() {
        ...
        let target = IronClawOutboundDeliveryTargetId::new("slack/<channel>")
