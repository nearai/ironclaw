...
/// `IronClawServices::disabled()` is the non-local-dev shape (no
...
fn local_dev_test_support_interaction_service_accessors_return_none_without_local_dev_runtime() {
    ...
    let services = super::IronClawServices::disabled();
...
async fn runtime_channel_identity_bind_uses_deployment_channel_before_user_activation() {
    ...
    let build_input = IronClawBuildInput::local_dev(
    ...
    let input =
        IronClawRuntimeInput::from_services(build_input).with_identity(IronClawRuntimeIdentity {
...
/// [`IronClawRuntimeInput`] must reach
...
fn local_dev_selector_config_uses_large_skill_context_budget() {
    ...
    assert_eq!(
        ...
        "local-dev IronClaw skill activation should match the legacy 6000-token skill budget"
    );
}
...
fn readiness_for_runtime_gate(
    ...
    diagnostics: Vec<crate::IronClawReadinessDiagnostic>,
) -> IronClawReadiness {
    IronClawReadiness {
        profile,
        state,
        facades: crate::IronClawFacadeReadiness {
            host_runtime: true,
            turn_coordinator: true,
            product_auth: true,
        },
        workers: crate::IronClawWorkerReadiness {
            turn_runner: true,
...
fn cutover_gate(
    profile: IronClawCompositionProfile,
    readiness: &crate::IronClawReadiness,
...
fn runtime_cutover_gate_allows_validated_production_readiness() {
    ...
    cutover_gate(IronClawCompositionProfile::Production, &readiness)
...
fn runtime_cutover_gate_rejects_blocking_production_diagnostic() {
    let readiness = readiness_for_runtime_gate(
        ...
        IronClawReadinessState::ProductionValidated,
        vec![
            crate::IronClawReadinessDiagnostic::production_blocker(
                IronClawCompositionProfile::Production,
                crate::IronClawReadinessDiagnosticComponent::RuntimePolicy,
                crate::IronClawReadinessDiagnosticReason::LocalOnly,
            )
            .expect("production profile should create a blocker"),
        ],
    );

    let error = cutover_gate(IronClawCompositionProfile::Production, &readiness)
        .expect_err("blocking production diagnostic prevents runtime start");
    let IronClawRuntimeError::InvalidArgument { reason } = error else {
...
fn runtime_cutover_gate_rejects_migration_dry_run_runtime_start() {
    let readiness = readiness_for_runtime_gate(
        ...
        IronClawReadinessState::MigrationDryRunValidated,
        Vec::new(),
    );

    let error = cutover_gate(IronClawCompositionProfile::MigrationDryRun, &readiness)
...
fn runtime_cutover_gate_allows_local_dev_readiness() {
    let readiness = readiness_for_runtime_gate(
        ...
        IronClawReadinessState::DevOnly,
        vec![crate::IronClawReadinessDiagnostic::local_dev()],
    );

    cutover_gate(IronClawCompositionProfile::LocalDev, &readiness)
...
fn runtime_cutover_gate_allows_hosted_single_tenant_readiness() {
    ...
    cutover_gate(IronClawCompositionProfile::HostedSingleTenant, &readiness)
...
fn runtime_cutover_gate_rejects_local_dev_readiness_for_hosted_single_tenant() {
    ...
    let error = cutover_gate(IronClawCompositionProfile::HostedSingleTenant, &readiness)
...
fn production_scheduler_wake_guard_rejects_production_with_absent_wiring() {
    ...
    let IronClawRuntimeError::InvalidArgument { reason } = err else {
...
fn production_scheduler_wake_guard_passes_local_dev_with_absent_wiring() {
    ...
    super::check_production_scheduler_wake_wiring(IronClawCompositionProfile::LocalDev, &None)
...
use ironclaw_product_workflow::{
    CREATE_THREAD_OPERATION, IronClawExtensionCredentialSetup, IronClawOutboundPreferencesResponse,
    IronClawServicesErrorCode, IronClawServicesErrorKind, IronClawSetupExtensionResponse,
    IronClawSkillListResponse, IronClawStreamEventsRequest, IronClawSubmitTurnResponse,
    LifecyclePackageKind, LifecyclePackageRef, LifecycleProductPayload, LifecycleReadinessBlocker,
    ProductCapabilityInput, RESOLVE_GATE_OPERATION, SUBMIT_TURN_OPERATION,
...
use crate::IronClawRuntimeProcessBinding;
...
use crate::runtime_input::{
    IronClawRuntimeIdentity, IronClawRuntimeInput, PollSettings, TriggerFireAccessCheck,
...
use crate::{
    IronClawCompositionProfile, IronClawReadiness, IronClawReadinessState, IronClawRuntimeError,
};
use ironclaw_config::{IronClawBootConfig, IronClawHome, IronClawProfile};

use super::{
    IronClawSkillSourceKind, TRUSTED_LAPTOP_ACCESS_AUDIT_KIND, TRUSTED_LAPTOP_ACCESS_AUDIT_STATUS,
    TRUSTED_LAPTOP_ACCESS_AUDIT_TARGET, build_ironclaw_runtime,
};

const RUNTIME_POLL_TIMEOUT: Duration = Duration::from_secs(10);
const RUNTIME_SEND_TIMEOUT: Duration = Duration::from_secs(15);

async fn stop_turn_runner_worker_for_manual_state_test(runtime: &super::IronClawRuntime) {
...
async fn root_llm_gateway_bootstraps_nearai_session_token_from_env() {
    let _token_guard =
        RuntimeEnvGuard::set("NEARAI_SESSION_TOKEN", "sess_ironclaw_env_token").await;
    ...
    assert_eq!(auth_header, "Bearer sess_ironclaw_env_token");
}

#[tokio::test]
async fn runtime_nearai_mcp_bootstraps_from_nearai_session_token() {
    let _token_guard =
        RuntimeEnvGuard::set("NEARAI_SESSION_TOKEN", "sess_ironclaw_mcp_token").await;
    let root = tempfile::tempdir().expect("tempdir");
    let session_dir = tempfile::tempdir().expect("session tempdir");
    ...
    let llm = crate::runtime_input::ResolvedIronClawLlm::from_llm_config(config);

    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-nearai-session-mcp-owner", local_dev_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
...
async fn runtime_nearai_mcp_bootstraps_from_stored_nearai_api_key() {
    ...
    let services = crate::build_ironclaw_services(
        IronClawBuildInput::local_dev("runtime-nearai-stored-mcp-owner", local_dev_root.clone())
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .await
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-nearai-stored-mcp-owner", local_dev_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_resolved_llm(llm)
    .with_identity(IronClawRuntimeIdentity {
...
async fn runtime_nearai_mcp_prebuild_api_key_is_not_replaced_by_stored_key() {
    ...
    let services = crate::build_ironclaw_services(
        IronClawBuildInput::local_dev(owner, local_dev_root.clone())
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .await
    .expect("services build for stored key seed");
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(owner, local_dev_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_resolved_llm(llm)
    .with_identity(IronClawRuntimeIdentity {
...
/// via `ResolvedIronClawLlm::with_provider_factory` (how the bench wraps an
...
async fn provider_factory_survives_live_reload() {
    ...
    let factory: crate::runtime_input::IronClawProviderFactory = Arc::new(move |inner| {
...
/// wired *only* via `ResolvedIronClawLlm::with_env_trace_recording`. Nothing
...
/// committed ironclaw_qa fixtures were recorded through the in-process harness,
...
async fn env_trace_recording_attaches_recorder_factory_only_when_enabled() {
    ...
    // Disabled: no factory attached; the resolved LLM is returned unchanged.
    {
        ...
        let disabled = crate::runtime_input::ResolvedIronClawLlm::from_llm_config(config.clone())
    ...
    // Enabled: the serve/run resolution path attaches the recording factory.
    {
        ...
        let enabled = crate::runtime_input::ResolvedIronClawLlm::from_llm_config(config)
            .with_env_trace_recording();
        assert!(
...
/// `ResolvedIronClawLlm` carrying a `provider_factory` must have that factory
/// invoked during `build_ironclaw_runtime`, i.e. the caller's instrumentation
/// wrapper is threaded into the cold-boot gateway.
///
/// PR #6174 collapsed the boot path to `build_placeholder_llm_gateway()`, which
/// hardcoded `None` for the factory, so `ResolvedIronClawLlm::with_provider_factory`
...
/// the real caller (`build_ironclaw_runtime`) instead.
#[tokio::test]
async fn provider_factory_runs_during_production_boot() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("provider-factory-boot-owner", local_dev_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_resolved_llm(llm)
    .with_identity(IronClawRuntimeIdentity {
    ...
    let _runtime = build_ironclaw_runtime(input).await.expect("runtime builds");
...
/// `IronClawLlmReloadAdapter::reload()` invoked once inside
/// `build_ironclaw_runtime` — by supplying a real `boot` config (so the
/// reload adapter can re-resolve `[llm.default]` from disk) instead of
/// pre-baking the stored key into a directly-supplied `ResolvedIronClawLlm`
...
async fn local_dev_runtime_startup_uses_stored_nearai_api_key_after_restart() {
    ...
    // by `IronClawLlmReloadAdapter::reload`) sets exactly that field from the
    ...
    let services = crate::build_ironclaw_services(
        IronClawBuildInput::local_dev("runtime-nearai-stored-key-owner", local_dev_root.clone())
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .await
    .expect("services build for stored key seed");
    ...
    // calls it.
    std::fs::write(
        IronClawHome::resolve_from_env_parts(
    ...
    let boot = IronClawBootConfig::new(
        IronClawHome::resolve_from_env_parts(
            Some(config_home_dir.as_os_str().to_os_string()),
            None,
            None,
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-nearai-stored-key-owner", local_dev_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
...
async fn production_runtime_rejects_enabled_hooks_without_local_runtime() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::libsql(
            crate::IronClawCompositionProfile::Production,
    ...
    let err = match build_ironclaw_runtime(input).await {
        Ok(runtime) => {
    ...
    assert!(
        matches!(
            err,
            super::IronClawRuntimeError::MalformedConfig { ref reason }
...
#[tokio::test]
async fn build_ironclaw_runtime_allows_validated_production_readiness() {
    let dir = tempfile::tempdir().expect("tempdir");
    let db = Arc::new(
        libsql::Builder::new_local(dir.path().join("reborn.db"))
            .build()
...
#[tokio::test]
async fn build_ironclaw_runtime_rejects_trajectory_observer_for_production() {
    let dir = tempfile::tempdir().expect("tempdir");
    let db = Arc::new(
        libsql::Builder::new_local(dir.path().join("reborn.db"))
            .build()
    ...
    assert!(
        matches!(err, super::IronClawRuntimeError::InvalidArgument { ref reason }
...
async fn local_dev_yolo_records_trusted_laptop_access_audit_event() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev_with_profile(
            crate::IronClawCompositionProfile::LocalDevYolo,
...
async fn local_dev_runtime_rejects_trigger_poller_without_creator_authorization() {
    ...
    assert!(
        matches!(err, super::IronClawRuntimeError::InvalidArgument { reason } if reason.contains("fire-time creator access checker"))
    );
}
...
async fn local_dev_runtime_rejects_invalid_trigger_poller_worker_config() {
    ...
    assert!(
        matches!(err, super::IronClawRuntimeError::InvalidArgument { reason } if reason.contains("poll_interval must be non-zero"))
    );
}
...
async fn send_user_message_returns_completed_assistant_text_with_recording_gateway() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-success-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    let local_runtime = runtime
        ...
        .local_runtime
        .as_ref()
        .expect("runtime should use local-dev IronClawServices substrate");
    assert!(
        Arc::ptr_eq(&runtime.thread_service, &local_runtime.thread_service),
        "REPL runtime should use the thread service owned by IronClawServices"
    );
    assert!(
        Arc::ptr_eq(
            ...
            runtime
                ...
                .turn_coordinator
                .as_ref()
                .expect("IronClawServices turn coordinator")
        ),
        "REPL runtime should drive turns through IronClawServices"
    );
    let conversation = runtime.new_conversation().await.expect("conversation");
    let reply = tokio::time::timeout(
        RUNTIME_SEND_TIMEOUT,
...
async fn send_user_message_preserves_model_unavailable_after_retry_budget() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-model-outage-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_auto_queues_trace_for_enrolled_scope() {
    use ironclaw_traces::contribution as trace_contribution;
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(&owner, root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_persists_personal_owner_for_webui() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(actor_owner_id, root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_renders_cli_origin_in_model_request() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-origin-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_until_gate_returns_blocked_on_auth_gate() {
    ...
    let (run_id, gate_ref) = match outcome {
        super::IronClawTurnDriveOutcome::BlockedOnGate {
            run_id,
            status,
            gate_ref,
            ..
        ...
        super::IronClawTurnDriveOutcome::Terminal(reply) => {
...
async fn cancel_run_propagates_to_subagent_children() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-cancel-child-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_uses_caller_supplied_skill_context_source() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-skill-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_exposes_host_runtime_capabilities_to_model_calls() {
    ...
    let gateway_for_runtime: Arc<dyn HostManagedModelGateway> = gateway.clone();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-tools-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
/// observer fires through a real `build_ironclaw_runtime` turn — driving the
...
impl crate::IronClawTrajectoryObserver for RecordingTrajectoryObserver {
...
/// `IronClawRuntimeInput::with_raw_trajectory_observer`. This drives the
...
async fn local_dev_runtime_forwards_tool_call_trajectory_to_raw_observer() {
    ...
    let observer = Arc::new(RecordingTrajectoryObserver::default());
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-trajectory-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_safe_preview_observer_receives_bounded_payload() {
    ...
    let observer = Arc::new(RecordingTrajectoryObserver::default());
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-preview-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_prefers_configured_skill_context_source_over_filesystem_default() {
    ...
    let skill_context_source: Arc<dyn HostSkillContextSource> = skill_source;
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-skill-override-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_wires_filesystem_skills_by_default_to_model_calls() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-filesystem-skill-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn execute_skill_message_returns_plan_and_reads_active_bundle_assets() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-skill-exec-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert_eq!(
        ...
        Some(IronClawSkillSourceKind::User)
    );
...
async fn local_dev_runtime_fails_closed_for_ambiguous_explicit_skill_before_model_call() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-ambiguous-skill-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_suppresses_explicit_setup_skill_when_workspace_marker_exists() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-setup-marker-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_activates_setup_skill_when_workspace_marker_is_absent() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-setup-marker-absent-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_rejects_workspace_overlapping_default_skill_roots() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-overlap-owner", storage_root)
...
async fn local_dev_runtime_skips_invalid_filesystem_skill_before_model_call() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-bad-skill-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_runtime_maps_workspace_to_configured_root() {
    ...
    let gateway_for_runtime: Arc<dyn HostManagedModelGateway> = gateway.clone();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-workspace-owner", root.path().join("local-dev"))
...
async fn local_dev_runtime_webui_bundle_reuses_thread_and_turn_facades() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    let IronClawSubmitTurnResponse::Submitted { run_id, .. } = submitted else {
    ...
    assert_eq!(bundle.readiness.state, IronClawReadinessState::DevOnly);

    runtime.shutdown().await.expect("runtime shutdown");
}

/// Caller-level regression for the production attachment-landing path:
/// drives `IronClawRuntime::webui_workspace_filesystem()` — the exact method
...
/// shared `IronClawServices::read_write_workspace_filesystem` recipe via the
/// `local_dev_attachment_test_support_for_test` seam, but never call through
/// this `IronClawRuntime` wrapper itself; this closes that gap so a future
...
async fn query_webui_extension_setup(
    ...
    let page = api
        .query(
            caller,
            ironclaw_product_workflow::IronClawViewQuery {
...
async fn local_dev_webui_bundle_uses_local_lifecycle_facade_for_setup_extension() {
    ...
    let IronClawExtensionCredentialSetup::OAuth { scopes, .. } = &google_secret.setup else {
    ...
    assert!(
        !setup.blockers.iter().any(|blocker| matches!(
            ...
            LifecycleReadinessBlocker::Runtime { ref_id: Some(ref_id) }
                if ref_id.as_str() == "ironclaw_lifecycle_facade_unwired"
        )),
...
async fn local_dev_webui_bundle_exposes_outbound_preferences_facade() {
    ...
    let cleared_preferences: IronClawOutboundPreferencesResponse =
    ...
    let targets: ironclaw_product_workflow::IronClawOutboundDeliveryTargetListResponse =
...
async fn local_dev_webui_bundle_invokes_skill_install_with_scoped_mounts() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-skill-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    let skills: IronClawSkillListResponse =
...
async fn webui_route_rejects_list_automations_without_agent_binding() {
    ...
    let mut runtime = build_ironclaw_runtime(input).await.expect("runtime builds");
...
async fn build_webui_services_without_local_runtime_returns_503_on_list_automations() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-no-host-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_webui_bundle_routes_approval_gates_into_interaction_service() {
    ...
    assert_eq!(err.code, IronClawServicesErrorCode::NotFound);
    assert_eq!(err.kind, IronClawServicesErrorKind::NotFound);
...
async fn local_dev_webui_bundle_routes_auth_gates_into_interaction_service() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-auth-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert_eq!(err.code, IronClawServicesErrorCode::NotFound);
    assert_eq!(err.kind, IronClawServicesErrorKind::BlockedAuthentication);
    assert_eq!(err.status_code, 404);
    runtime.shutdown().await.expect("runtime shutdown");
}
...
async fn local_dev_webui_spawn_approval_emits_redacted_audit_and_grants_process() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-audit-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn local_dev_webui_bundle_records_selectable_filesystem_skill_context() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-webui-skill-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
        tenant_id: "runtime-webui-skill-tenant".to_string(),
    ...
    let submitted = SUBMIT_TURN_OPERATION
        ...
        .await
        .expect("submit turn");
    let IronClawSubmitTurnResponse::Submitted {
        thread_id,
...
async fn multi_tool_call_response_survives_surface_change_mid_register() {
    ...
    struct LifecycleFacadeHandle {
        facade: crate::extension_host::lifecycle::IronClawLocalLifecycleFacade,
    }
    ...
    let extension_management = local_runtime
        ...
        .expect("extension management")
        .clone();
    let facade = crate::extension_host::lifecycle::IronClawLocalLifecycleFacade::new(
...
/// it requires `build_ironclaw_runtime` + full turn-runner control that only the runtime test
...
async fn rejected_busy_message_not_auto_resubmitted_after_run_cancellation() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-rejected-busy-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    let IronClawSubmitTurnResponse::RejectedBusy {
        notice: notice_b,
    ...
    assert!(
        matches!(response_c, IronClawSubmitTurnResponse::Submitted { .. }),
...
async fn scheduler_liveness_not_stopped_under_contention() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("scheduler-liveness-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert!(
        matches!(
            result_after_stop,
            Err(super::IronClawRuntimeError::WorkerStopped)
        ),
...
/// `Err(IronClawRuntimeError::WorkerStopped)` without submitting the turn.
#[tokio::test]
async fn scheduler_stopped_rejects_send_user_message() {
    ...
    assert!(
        matches!(result, Err(IronClawRuntimeError::WorkerStopped)),
