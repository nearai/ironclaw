...
use crate::runtime_input::{IronClawRuntimeIdentity, IronClawRuntimeInput, TurnRunnerSettings};
use crate::{IronClawProductAuthServicePorts, IronClawRuntimeProcessBinding};

use super::{IronClawRuntime, build_ironclaw_runtime};
...
async fn local_dev_runtime_auth_interactions_are_unavailable_without_flow_record_source() {
    ...
    let ports = IronClawProductAuthServicePorts::from_shared(auth);
    let root = tempfile::tempdir().expect("tempdir");
    let runtime = build_runtime(
...
async fn build_runtime(
...
) -> Result<IronClawRuntime, super::IronClawRuntimeError> {
    let mut services = IronClawBuildInput::local_dev(owner, storage_root)
    ...
    let runtime = build_ironclaw_runtime(
        IronClawRuntimeInput::from_services(services)
