...
use crate::input::IronClawBuildInput;
use crate::runtime_input::{IronClawRuntimeIdentity, IronClawRuntimeInput, PollSettings};

use super::{IronClawRuntimeError, build_ironclaw_runtime};
...
async fn local_dev_runtime_rejects_non_file_default_system_prompt() {
    ...
    let error = match build_ironclaw_runtime(input).await {
        Ok(runtime) => {
            runtime.shutdown().await.expect("runtime shutdown");
    ...
    match error {
        IronClawRuntimeError::Build(build_error) => {
...
fn runtime_input(
    ...
    requests: Arc<StdMutex<Vec<HostManagedModelRequest>>>,
) -> IronClawRuntimeInput {
    let gateway = Arc::new(RecordingGateway { requests });
    IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-system-prompt-owner", storage_root)
