// Test-support accessors mirroring `build_ironclaw_runtime`'s approval/auth
// interaction-service wiring, for harnesses that build their own planned
// runtime and bypass `build_ironclaw_runtime` (W5-WEBUI-API-2).
//
// `turn_coordinator` is caller-supplied, never `self.turn_coordinator`: a
// `IronClawServices` from `build_ironclaw_services` alone carries the coordinator
...
impl IronClawServices {
    /// Real `DefaultApprovalInteractionService` wired like `build_ironclaw_runtime`, via the
    ...
    ) -> Result<Option<Arc<dyn ApprovalInteractionService>>, IronClawRuntimeError> {
    ...
    /// helper `build_ironclaw_runtime` uses. `None` only without a local-dev runtime; falls
    ...
    /// `IronClawServices`' own `local_runtime.turn_state` (e.g.
    /// `IronClawIntegrationGroup`, whose runs execute against its own
