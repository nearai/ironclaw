...
//! the IronClaw identity user-directory + admin secret provisioner + a token
//! minter.
//!
...
pub(crate) struct IronClawAdminUserDirectory {
    directory: Arc<dyn IronClawUserDirectory>,
    secrets: Arc<dyn AdminSecretProvisioner>,
    token_minter: Arc<dyn AdminApiTokenMinter>,
}

impl IronClawAdminUserDirectory {
    pub(crate) fn new(
        directory: Arc<dyn IronClawUserDirectory>,
        secrets: Arc<dyn AdminSecretProvisioner>,
        token_minter: Arc<dyn AdminApiTokenMinter>,
    ...
    async fn tenant_scoped_user(
        ...
        tenant: &TenantId,
        user_id: &UserId,
    ) -> Result<Option<IronClawUser>, AdminUserError> {
        let user = self
            .directory
            .get_user(user_id)
            .await
...
#[async_trait]
impl AdminUserService for IronClawAdminUserDirectory {
    async fn list_users(
        &self,
        tenant: &TenantId,
...
fn to_admin_record(user: IronClawUser) -> AdminUserRecord {
    AdminUserRecord {
...
fn role_to_identity(role: AdminUserRole) -> IronClawUserRole {
    match role {
        AdminUserRole::Owner => IronClawUserRole::Owner,
        AdminUserRole::Admin => IronClawUserRole::Admin,
        AdminUserRole::Member => IronClawUserRole::Member,
    }
}

fn role_from_identity(role: IronClawUserRole) -> AdminUserRole {
    match role {
        IronClawUserRole::Owner => AdminUserRole::Owner,
        IronClawUserRole::Admin => AdminUserRole::Admin,
        IronClawUserRole::Member => AdminUserRole::Member,
    }
}

fn status_to_identity(status: AdminUserStatus) -> IronClawUserStatus {
    match status {
        AdminUserStatus::Active => IronClawUserStatus::Active,
        AdminUserStatus::Suspended => IronClawUserStatus::Suspended,
    }
}

fn status_from_identity(status: IronClawUserStatus) -> AdminUserStatus {
    match status {
        IronClawUserStatus::Active => AdminUserStatus::Active,
        IronClawUserStatus::Suspended => AdminUserStatus::Suspended,
    }
}

/// Map identity errors into the coarse port error, scrubbing storage paths
/// (`IronClawIdentityError::Backend` carries a `ScopedPath`).
fn map_identity_error(error: IronClawIdentityError) -> AdminUserError {
    match error {
        IronClawIdentityError::UserNotFound(_) => AdminUserError::NotFound,
        IronClawIdentityError::Backend(_) => AdminUserError::Unavailable,
        // A persisted-id inconsistency or a channel-actor misuse is not
        // retryable and not the client's fault.
        IronClawIdentityError::InvalidUserId(_)
        | IronClawIdentityError::ChannelActorNotMintable => AdminUserError::Internal,
        ...
        IronClawIdentityError::UserSuspended(_) => AdminUserError::Internal,
    }
}
