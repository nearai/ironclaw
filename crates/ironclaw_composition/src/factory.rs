// arch-exempt: large_file, needs IronClaw composition helper extraction, plan #4469
use std::{
...
use crate::IronClawProductAuthServicePorts;
...
use crate::extension_host::lifecycle::{
    IronClawLocalSkillManagementPort, build_local_skill_management_port,
};
...
use crate::extension_host::{
    ...
    extension_lifecycle::{
        ActiveExtensionPublisher, ExtensionCredentialCleanup, IronClawLocalExtensionManagementPort,
...
use crate::input::{
    IronClawLocalRuntimeIdentity, IronClawRuntimeProcessBinding, IronClawStorageInput,
};
...
use crate::runtime_input::IronClawRuntimeIdentity;
use crate::support::fs::IronClawProjectService;
use crate::web_access::register_bundled_web_access_first_party_handlers;
use crate::{
    IronClawAuthContinuationDispatcher, IronClawBuildError, IronClawBuildInput,
    IronClawCompositionProfile, IronClawFacadeReadiness, IronClawProductAuthServices,
    IronClawReadiness, IronClawWorkerReadiness,
};
...
use ironclaw_conversations::{InboundTurnError, IronClawFilesystemConversationServices};
...
/// and the canonical IronClaw identity store, so each rides the same
/// `reborn-local-dev.db` rather than opening a second handle to the file
/// (see `IronClawRuntime::open_ironclaw_identity_resolver`).
...
/// IronClaw home / local-dev root directory. `pub` (re-exported from `lib.rs`)
...
fn apply_runtime_process_binding<F, G, S, R>(
    ...
    binding: IronClawRuntimeProcessBinding,
...
{
    match binding {
        IronClawRuntimeProcessBinding::None => services,
        IronClawRuntimeProcessBinding::TenantSandbox { process_port } => {
...
fn apply_post_edit_check_from_env<F, G, S, R>(
    services: HostRuntimeServices<F, G, S, R>,
) -> Result<HostRuntimeServices<F, G, S, R>, IronClawBuildError>
where
    F: ironclaw_filesystem::RootFilesystem + 'static,
    G: ironclaw_resources::ResourceGovernor + 'static,
    S: ironclaw_processes::ProcessStore + 'static,
    R: ironclaw_processes::ProcessResultStore + 'static,
{
    match PostEditCheckConfig::from_env() {
        ...
        Ok(None) => Ok(services),
        Err(error) => Err(IronClawBuildError::InvalidConfig {
            reason: error.to_string(),
        }),
    }
}
...
fn require_product_auth_runtime_ports<F, G, S, R>(
    services: &HostRuntimeServices<F, G, S, R>,
) -> Result<ProductAuthProviderRuntimePorts, IronClawBuildError>
where
    F: ironclaw_filesystem::RootFilesystem + 'static,
    G: ironclaw_resources::ResourceGovernor + 'static,
    S: ironclaw_processes::ProcessStore + 'static,
...
pub struct IronClawServices {
    ...
    pub(crate) skill_management: Option<Arc<IronClawLocalSkillManagementPort>>,
    pub(crate) local_runtime: Option<Arc<IronClawRuntimeSubstrate>>,
    // arch-exempt: optional_arc, local-dev vs production split pending IronClawServices split, plan #4471
    pub(crate) production_runtime: Option<IronClawProductionRuntimeServices>,
    ...
    /// `build_default_planned_runtime` runs; consumed by `build_ironclaw_runtime`
    ...
    /// are consumed by `build_ironclaw_runtime` when the channel host assembly
    /// starts.
...
pub(crate) enum CredentialRefreshWorkerReady {
    ...
    Ready {
        ...
        refresh_port: Arc<IronClawProductAuthServices>,
    },
...
/// Production wiring for [`IronClawServices::start_channel_host_assembly`]:
...
/// [`IronClawServices::start_channel_host_assembly_for_test`]: the test group
...
impl IronClawServices {
    ...
    pub fn channel_config_facade(
        ...
        Some(Arc::new(
            crate::extension_host::channel_config::IronClawChannelConfigFacade::new(service),
        ))
    }
    ...
    pub(crate) fn start_channel_host_assembly(
        ...
        let identity_lookup = local_runtime
            ...
            .clone()
            .map(|store| store as Arc<dyn crate::provider_identity::IronClawUserIdentityLookup>);
        Some(
    ...
    ///
    /// Single owner of this recipe — both `IronClawRuntime::webui_workspace_filesystem`
    ...
    #[cfg(feature = "test-support")]
    pub fn local_dev_approval_test_parts(&self) -> Option<IronClawApprovalTestParts> {
        let local_runtime = self.local_runtime.as_ref()?;
        ...
        Some(IronClawApprovalTestParts {
            approval_requests,
            capability_leases,
            gate_record_store,
    ...
    /// `IronClawLocalExtensionManagementPort`. For tests only — zero bytes
    ...
    /// `IronClawServices` can drive its own real `StagedCapabilityIo` through
    ...
    /// wires into `IronClawRuntimeSubstrate::outbound_preferences` via
    ...
    /// `IronClawRuntime::webui_workspace_filesystem` uses in production, via the
...
async fn active_extension_authority_for_test(
    extension_management: &IronClawLocalExtensionManagementPort,
    grantee: &ExtensionId,
) -> Result<ActiveExtensionAuthorityForTest, ironclaw_product_workflow::ProductWorkflowError> {
...
/// Bundle returned by [`IronClawServices::local_dev_attachment_test_support_for_test`]
...
#[cfg(feature = "test-support")]
#[derive(Clone)]
pub struct IronClawApprovalTestParts {
...
pub(crate) struct IronClawRuntimeSubstrate {
    ...
    pub(crate) trigger_conversation_services:
        tokio::sync::OnceCell<IronClawFilesystemConversationServices>,
    pub(crate) checkpoint_state_store: Arc<dyn CheckpointStateStore>,
    pub(crate) loop_checkpoint_store: Arc<dyn LoopCheckpointStore>,
    pub(crate) thread_service: Arc<dyn SessionThreadService>,
    /// Scoped filesystem backing the canonical IronClaw identity store, so it
    /// rides the host `RootFilesystem` abstraction like every other durable
    /// IronClaw store rather than a raw DB handle. Only the WebUI v2 SSO surface
    ...
    pub(crate) skill_management: Arc<IronClawLocalSkillManagementPort>,
    ...
    pub(crate) extension_management: Option<Arc<IronClawLocalExtensionManagementPort>>,
    ...
    /// (`build_ironclaw_runtime`, via
    /// `IronClawRuntime::generic_channel_connection_facade`) or by the
    ...
    /// `IronClawLocalExtensionManagementPort::channel_disconnect_slot`).
...
pub(crate) enum IronClawProductionRuntimeServices {
    LibSql(Arc<IronClawProductionRuntimeStoreGraph<LibSqlRootFilesystem>>),
    Postgres(Arc<IronClawProductionRuntimeStoreGraph<PostgresRootFilesystem>>),
}

pub(crate) struct IronClawProductionRuntimeStoreGraph<F>
where
    F: RootFilesystem + 'static,
{
    ...
    pub(crate) trigger_conversation_services:
        ironclaw_conversations::IronClawFilesystemConversationServices,
}

impl IronClawProductionRuntimeServices {
...
impl IronClawRuntimeSubstrate {
    pub(crate) async fn durable_trigger_conversation_services(
        &self,
    ) -> Result<IronClawFilesystemConversationServices, InboundTurnError> {
        let filesystem = Arc::clone(&self.subagent_goal_filesystem);
        self.trigger_conversation_services
            .get_or_try_init(|| async move {
                IronClawFilesystemConversationServices::new(filesystem).await
            })
            .await
            .cloned()
    }
...
struct IronClawStoreGraph {
    ...
    local_runtime: Arc<IronClawRuntimeSubstrate>,
    resource_governor: Arc<ComposedResourceGovernor>,
    process_services: ComposedProcessServices,
    trigger_repository: Arc<dyn TriggerRepository>,
}

struct IronClawStoreGraphInput {
    filesystem: Arc<CompositeRootFilesystem>,
    owner_user_id: UserId,
    local_runtime_identity: Option<IronClawLocalRuntimeIdentity>,
    ...
    /// Raw libSQL substrate handle, carried so the canonical IronClaw identity
    /// store rides the same `reborn-local-dev.db` instead of opening a second
    /// handle (see `IronClawRuntime::open_ironclaw_identity_resolver`).
    identity_substrate_db: Option<Arc<libsql::Database>>,
}

impl std::fmt::Debug for IronClawServices {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut debug = formatter.debug_struct("IronClawServices");
        debug
...
// arch-exempt: optional_arc, IronClawServices fields are Optional because disabled()/local-dev paths don't wire all production services; proper factories always set them, plan #4469
...
pub async fn build_ironclaw_services(
    input: IronClawBuildInput,
) -> Result<IronClawServices, IronClawBuildError> {
    tracing::debug!(
    ...
    match substrate {
        crate::deployment::RuntimeSubstrate::None => Ok(IronClawServices::disabled()),
...
fn auth_continuation_dispatcher(
    ...
    >,
) -> Arc<dyn IronClawAuthContinuationDispatcher> {
    let single_run: Arc<dyn IronClawAuthContinuationDispatcher> = Arc::new(
...
struct ProductAuthServicesCompositionInput {
    ports: IronClawProductAuthServicePorts,
    turn_coordinator: Arc<dyn ironclaw_turns::TurnCoordinator>,
...
) -> Result<Arc<IronClawProductAuthServices>, IronClawBuildError> {
...
/// backend through `IronClawStorageInput::HostedSingleTenantPostgres`; local-dev
...
async fn build_local_runtime(
    ...
    let provider_instance_readiness =
        provider_instance_readiness_map([ProviderInstanceReadinessInput {
            ...
            remediation: ironclaw_config::google_setup_steps_text(),
        }]);
    ...
    std::fs::create_dir_all(&root).map_err(|_| IronClawBuildError::InvalidConfig {
    ...
    std::fs::create_dir_all(&workspace_root).map_err(|_| IronClawBuildError::InvalidConfig {
    ...
    let owner_user_id =
        UserId::new(owner_id).map_err(|error| IronClawBuildError::InvalidConfig {
            reason: error.to_string(),
        })?;
    ...
    crate::extension_host::bundled_skills::ensure_bundled_ironclaw_skills_installed(&root).await?;
    ...
    let mut store_graph = build_local_runtime_store_graph(IronClawStoreGraphInput {
        ...
        None => {
            {
                ...
                let services = IronClawProductAuthServicePorts::from_shared_with_provider(
    ...
    let mut available_extensions =
        filesystem_catalog.map_err(|error| IronClawBuildError::InvalidConfig {
    ...
    let operator_tool_catalog: Arc<dyn ironclaw_product_workflow::IronClawOperatorToolCatalog> =
    ...
    let outbound_preferences_facade: Arc<dyn OutboundPreferencesProductFacade> =
        Arc::new(crate::outbound::IronClawOutboundPreferencesFacade::new(
        ...
        let channel_pairing_registry_built = {
            ...
            for descriptor in &account_setup_descriptors {
                ...
                if descriptor.connection_requirement.strategy
                    != ironclaw_product_workflow::IronClawChannelConnectStrategy::WebGeneratedCode
                {
                    continue;
                }
                ...
                let roots =
                    ...
                    .map_err(|reason| IronClawBuildError::InvalidConfig { reason })?;
...
fn local_dev_extension_lifecycle_surface_context(
    owner_user_id: UserId,
    local_runtime_identity: Option<&IronClawLocalRuntimeIdentity>,
) -> Result<LifecycleProductSurfaceContext, IronClawBuildError> {
    let default_identity = IronClawRuntimeIdentity::ironclaw_cli();
...
fn local_dev_nearai_mcp_owner_scope(
    owner_user_id: UserId,
    local_runtime_identity: Option<&IronClawLocalRuntimeIdentity>,
) -> Result<ResourceScope, IronClawBuildError> {
    let context =
...
fn default_runtime_owner_scope(
    ...
    let identity = IronClawRuntimeIdentity::ironclaw_cli();
...
fn configured_runtime_owner_scope(
    owner_user_id: UserId,
    local_runtime_identity: &IronClawLocalRuntimeIdentity,
) -> ResourceScope {
...
fn local_dev_extension_installation_state_path(
    ...
    local_runtime_identity: Option<&IronClawLocalRuntimeIdentity>,
) -> Result<VirtualPath, IronClawBuildError> {
...
async fn build_local_runtime_store_graph(
    input: IronClawStoreGraphInput,
) -> Result<IronClawStoreGraph, IronClawBuildError> {
    let IronClawStoreGraphInput {
        filesystem,
        owner_user_id,
        local_runtime_identity,
        runtime_policy,
    ...
    let turn_state_filesystem =
        owner_turn_state_filesystem(Arc::clone(&filesystem), &turn_state_scope)
            .map_err(IronClawBuildError::Mount)?;
    ...
    let skill_mounts =
        skill_management_mount_view().map_err(|error| IronClawBuildError::InvalidConfig {
            reason: error.to_string(),
        })?;
    let capability_policy = Arc::new(builtin_capability_policy().map_err(|error| {
        IronClawBuildError::InvalidConfig {
    ...
    let local_runtime = Arc::new(IronClawRuntimeSubstrate {
        ...
        project_service: Arc::new(IronClawProjectService::new(Arc::clone(&project_repository))),
...
async fn validate_trigger_delivery_target_against_registry(
    ...
    let target_id =
        ironclaw_outbound::OutboundDeliveryTargetId::new(target.as_str()).map_err(|error| {
            tracing::debug!(
                target = "ironclaw::trigger_create",
                %error,
...
struct LocalRuntimeTriggerCreatorPairingHook {
    runtime: Arc<IronClawRuntimeSubstrate>,
}
...
{
    filesystem: Arc<ScopedFilesystem<F>>,
    conversations: tokio::sync::OnceCell<IronClawFilesystemConversationServices>,
}
    ...
    F: RootFilesystem + 'static,
{
    async fn after_trigger_persisted(&self, record: &TriggerRecord) -> Result<(), TriggerError> {
        ...
        let conversations = self
            .conversations
            .get_or_try_init(|| async move {
                IronClawFilesystemConversationServices::new(filesystem)
                    .await
                    .map_err(|error| {
                        trigger_pairing_error(TriggerPairingFailureSource::ConversationInit, error)
                    })
...
) -> Result<RootFilesystemBundle, IronClawBuildError> {
...
/// (`ironclaw_cli`) must use this instead of hardcoding the filename.
...
async fn open_local_dev_libsql_database(
    root: &Path,
) -> Result<Arc<libsql::Database>, IronClawBuildError> {
...
) -> Result<DurableBackend, IronClawBuildError> {
    {
...
fn local_dev_project_filesystem(
    ...
    workspace_root: &Path,
    host_home_root: Option<&HostHomeRoot>,
) -> Result<DiskFilesystem, IronClawBuildError> {
...
/// (`build_ironclaw_services` → `FilesystemChannelIdentityStore::new` over the
...
pub(crate) async fn open_local_dev_root_filesystem_for_test(
    storage_root: &Path,
) -> Result<Arc<dyn RootFilesystem>, IronClawBuildError> {
...
/// (mirrors the production install-store load in [`build_ironclaw_services`],
...
pub(crate) async fn open_local_dev_extension_installation_store_for_test(
    storage_root: &Path,
) -> Result<Arc<dyn ExtensionInstallationStore>, IronClawBuildError> {
    let workspace_root = storage_root.join("workspace");
    let bundle = build_local_runtime_root_filesystem(
        storage_root,
        &workspace_root,
...
pub(crate) async fn open_local_dev_approval_request_store_for_test(
    storage_root: &Path,
) -> Result<Arc<dyn ironclaw_run_state::ApprovalRequestStore>, IronClawBuildError> {
    let mut composite = CompositeRootFilesystem::new();
...
pub(crate) async fn open_local_dev_outbound_preferences_store_for_test(
    storage_root: &Path,
) -> Result<Arc<dyn CommunicationPreferenceRepository>, IronClawBuildError> {
    let mut composite = CompositeRootFilesystem::new();
    mount_default_local_dev_database_roots(storage_root, &mut composite).await?;
...
///
/// - Pre-composition entry point `ironclaw onboard` needs: it must
...
pub async fn open_local_dev_secret_store(
    root: &Path,
) -> Result<Arc<dyn SecretStore>, IronClawBuildError> {
    let db = open_local_dev_libsql_database(root).await?;
...
/// enforces, mapping a rejection to a `IronClawBuildError` that names *where the
...
async fn resolve_local_dev_secret_master_key(
    root: &Path,
) -> Result<ironclaw_secrets::SecretMaterial, IronClawBuildError> {
...
fn write_local_dev_secret_master_key(path: &Path, key: &str) -> Result<(), IronClawBuildError> {
    #[cfg(unix)]
    {
    ...
    #[cfg(not(any(unix, windows)))]
    {
        let _ = path;
        let _ = key;
        Err(IronClawBuildError::InvalidConfig {
            reason:
...
///   `ironclaw_dependency_boundaries.rs::ironclaw_cli_binary_crate_stays_separate_from_v1_root`.
...
) -> Result<MountDescriptor, IronClawBuildError> {
    Ok(MountDescriptor {
...
fn local_dev_event_log(
    filesystem: Arc<CompositeRootFilesystem>,
) -> Result<Arc<dyn DurableEventLog>, IronClawBuildError> {
    ...
    Ok(Arc::new(
        ironclaw_event_store::FilesystemDurableEventLog::new(scoped),
    ))
}

fn local_dev_audit_log(
    filesystem: Arc<CompositeRootFilesystem>,
) -> Result<Arc<dyn DurableAuditLog>, IronClawBuildError> {
    let scoped = Arc::new(ScopedFilesystem::with_fixed_view(
        filesystem,
        MountView::new(vec![MountGrant::new(
            MountAlias::new("/events")?,
    ...
    Ok(Arc::new(
        ironclaw_event_store::FilesystemDurableAuditLog::new(scoped),
    ))
}

fn canonicalize_local_dev_path(path: &Path, label: &str) -> Result<PathBuf, IronClawBuildError> {
    std::fs::canonicalize(path).map_err(|_| IronClawBuildError::InvalidConfig {
...
fn build_workspace_filesystems(
    ...
    workspace_root: &Path,
    host_home_root: Option<&HostHomeRoot>,
) -> Result<WorkspaceFilesystems, IronClawBuildError> {
...
fn canonicalize_local_dev_existing_dir(
    path: &Path,
    label: &str,
) -> Result<PathBuf, IronClawBuildError> {
    let path = canonicalize_local_dev_path(path, label)?;
    let metadata = std::fs::metadata(&path).map_err(|_| IronClawBuildError::InvalidConfig {
...
fn canonicalize_local_dev_host_home_root(path: &Path) -> Result<PathBuf, IronClawBuildError> {
...
fn backfill_local_dev_legacy_user_skills_for_tenant(
    ...
    std::fs::create_dir_all(&scoped_root).map_err(|error| IronClawBuildError::InvalidConfig {
        reason: format!("local-dev scoped skill root could not be initialized: {error}"),
    })?;

    for entry in
        std::fs::read_dir(legacy_root).map_err(|error| IronClawBuildError::InvalidConfig {
            reason: format!(
    ...
    {
        let entry = entry.map_err(|error| IronClawBuildError::InvalidConfig {
            reason: format!(
                "local-dev legacy skills root '{}' could not be inspected: {error}",
                legacy_root.display()
            ),
    ...
    std::fs::write(&marker, b"").map_err(|error| IronClawBuildError::InvalidConfig {
        reason: format!(
...
fn copy_local_dev_legacy_skill_entry(
    ...
    while let Some((source, destination, depth)) = pending.pop_front() {
        ...
        if metadata.is_dir() {
            ...
            for entry in
                std::fs::read_dir(&source).map_err(|error| IronClawBuildError::InvalidConfig {
                    reason: format!(
        ...
        if let Some(parent) = destination.parent() {
            std::fs::create_dir_all(parent).map_err(|error| IronClawBuildError::InvalidConfig {
                reason: format!(
                    "local-dev scoped skill directory '{}' could not be initialized: {error}",
                    parent.display()
                ),
...
pub(crate) fn builtin_extension_registry() -> Result<ExtensionRegistry, IronClawBuildError> {
    ...
    registry
        .insert(builtin_first_party_package().map_err(|error| {
            IronClawBuildError::InvalidConfig {
...
) -> Result<ExtensionRegistry, IronClawBuildError> {
    let mut registry = ExtensionRegistry::new();
    registry
        .insert(
...
) -> Result<FirstPartyCapabilityRegistry, IronClawBuildError> {
...
fn local_dev_builtin_extension_registry() -> Result<ExtensionRegistry, IronClawBuildError> {
    let mut registry = builtin_extension_registry()?;
    let builtin_id =
        ExtensionId::new("builtin").map_err(|error| IronClawBuildError::InvalidConfig {
...
pub fn builtin_first_party_trust_policy() -> Result<HostTrustPolicy, IronClawBuildError> {
    let policy =
        builtin_capability_policy().map_err(|error| IronClawBuildError::InvalidConfig {
            reason: format!("local-dev capability policy is invalid: {error}"),
        })?;
    ...
    for bundle in ironclaw_first_party_extensions::packages::bundled_packages() {
        ...
        entries.push(AdminEntry::for_local_manifest(
            PackageId::new(bundle.id).map_err(|error| IronClawBuildError::InvalidConfig {
...
async fn build_production_shaped(
    ...
    let IronClawBuildInput {
        ...
        runtime_policy,
        // The notifier field on `IronClawBuildInput` is kept for backward
    ...
    match storage {
        IronClawStorageInput::Disabled | IronClawStorageInput::LocalDev { .. } => {
            Err(IronClawBuildError::InvalidConfig {
                reason: format!(
        ...
        IronClawStorageInput::Libsql {
            ...
            // 2. The wiring is threaded through `IronClawServices` →
            ...
            let context = IronClawProductionBuildContext {
                profile,
                wiring_config,
                production_wiring,
                product_auth_ports,
...
async fn resolve_secret_master_key(
    ...
    resolve_explicit_or_keychain_master_key(explicit)
        .await?
        .ok_or(IronClawBuildError::MissingSecretMasterKey)
}

struct IronClawProductionWiring {
    trust_policy: Arc<HostTrustPolicy>,
    runtime_policy: EffectiveRuntimePolicy,
    turn_run_wake_notifier: Arc<dyn ironclaw_turns::TurnRunWakeNotifier>,
    runtime_process_binding: IronClawRuntimeProcessBinding,
}

struct IronClawProductionBuildContext {
    profile: IronClawCompositionProfile,
    wiring_config: ironclaw_host_runtime::ProductionWiringConfig,
    production_wiring: IronClawProductionWiring,
    product_auth_ports: Option<IronClawProductAuthServicePorts>,
    ...
    local_runtime_identity: Option<IronClawLocalRuntimeIdentity>,
    turn_state_store_limits: ironclaw_turns::TurnStateStoreLimits,
    /// The pre-minted scheduler wake wiring to carry to `IronClawServices` so
    /// `build_ironclaw_runtime` can hand it to `build_default_planned_runtime` via
...
fn production_wiring(
    ...
    runtime_process_binding: IronClawRuntimeProcessBinding,
) -> Result<IronClawProductionWiring, IronClawBuildError> {
    let trust_policy = trust_policy.ok_or(IronClawBuildError::MissingProductionTrustPolicy)?;
    if !trust_policy.has_sources() {
        return Err(IronClawBuildError::EmptyProductionTrustPolicy);
    }
    let runtime_policy = runtime_policy.ok_or(IronClawBuildError::MissingRuntimePolicy)?;
...
fn planned_run_profile_resolver() -> Result<Arc<InMemoryRunProfileResolver>, IronClawBuildError> {
    Ok(Arc::new(
        ironclaw_runner::planned_driver_factory::default_planned_run_profile_resolver().map_err(
            |error| IronClawBuildError::PlannedRunProfileResolver {
                reason: error.to_string(),
            },
        )?,
    ))
...
fn substrate_only_default_owner_id() -> Result<UserId, crate::IronClawCompositionError> {
    let identity = IronClawRuntimeIdentity::ironclaw_cli();
    ...
    UserId::new(identity.tenant_id).map_err(crate::IronClawCompositionError::Mount)
}
...
) -> Result<crate::LibSqlProductionHostRuntimeServices, crate::IronClawCompositionError>
where
...
) -> Result<(), crate::IronClawCompositionError> {
    if process_local_singleton {
        return Ok(());
    }
    Err(crate::IronClawCompositionError::InvalidConfig {
...
) -> Result<crate::PostgresProductionHostRuntimeServices, crate::IronClawCompositionError>
where
    TPolicy: ironclaw_trust::TrustPolicy + 'static,
    TWake: ironclaw_turns::TurnRunWakeNotifier + 'static,
{
    ...
    let event_store = ironclaw_event_store::build_ironclaw_event_stores_from_root_filesystem(
        Arc::clone(&filesystem),
    )?;
    build_filesystem_production_host_runtime_services(
        FilesystemProductionHostRuntimeServicesInput {
    ...
    F: RootFilesystem + 'static,
{
    ...
    runtime_policy: crate::IronClawProductionRuntimePolicy,
    turn_run_wake_notifier: Arc<TWake>,
    surface_version: CapabilitySurfaceVersion,
}

enum FilesystemProductionEventStoresInput {
    Config(ironclaw_event_store::IronClawEventStoreConfig),
    Prebuilt(ironclaw_event_store::IronClawEventStores),
}
...
fn ensure_postgres_event_store_config(
    ...
    match config {
        ironclaw_event_store::IronClawEventStoreConfig::Postgres { .. } => Ok(()),
        ironclaw_event_store::IronClawEventStoreConfig::PostgresPool { .. } => Ok(()),
        _ => Err(crate::IronClawCompositionError::InvalidConfig {
...
async fn warm_resource_governor_for_composition<F>(
    resource_governor: FilesystemResourceGovernor<F>,
) -> Result<FilesystemResourceGovernor<F>, crate::IronClawCompositionError>
where
    F: RootFilesystem + 'static,
{
    warm_resource_governor_with_error(resource_governor, |error| {
        crate::IronClawCompositionError::InvalidConfig {
...
) -> Result<FilesystemProductionHostRuntimeServices<F>, crate::IronClawCompositionError>
where
    F: RootFilesystem + 'static,
    TPolicy: ironclaw_trust::TrustPolicy + 'static,
    TWake: ironclaw_turns::TurnRunWakeNotifier + 'static,
{
    ...
    let owner_scope = default_runtime_owner_scope(owner_user_id)
        .map_err(crate::IronClawCompositionError::Mount)?;
        ...
        FilesystemProductionEventStoresInput::Config(config) => {
            services
                .with_ironclaw_event_store_config(
                    ironclaw_event_store::IronClawProfile::Production,
                    config,
                )
                .await?
        }
        FilesystemProductionEventStoresInput::Prebuilt(stores) => {
            services.with_production_ironclaw_event_stores(stores)
        }
    };
    ...
    let services = match PostEditCheckConfig::from_env() {
        ...
        Ok(None) => services,
        Err(error) => {
            return Err(crate::IronClawCompositionError::InvalidConfig {
                reason: error.to_string(),
            });
        }
    };
    ...
    let services = services
        ...
        .map_err(crate::IronClawCompositionError::from)?;
    ...
    F: RootFilesystem + 'static,
{
    ...
    ) -> Result<Self, crate::IronClawCompositionError> {
        Ok(Self::new(
            scoped_filesystem,
...
) -> Result<FilesystemSecretCredentialStores<F>, crate::IronClawCompositionError>
where
    F: RootFilesystem + 'static,
{
    let master_key = resolve_explicit_or_keychain_master_key(master_key)
        .await?
        .ok_or(crate::IronClawCompositionError::MissingSecretMasterKey)?;
    ...
    F: RootFilesystem + 'static,
{
    ...
    event_store: ironclaw_event_store::IronClawEventStoreConfig,
}
...
async fn warm_resource_governor_for_build<F>(
    resource_governor: FilesystemResourceGovernor<F>,
) -> Result<FilesystemResourceGovernor<F>, IronClawBuildError>
where
    F: RootFilesystem + 'static,
{
    warm_resource_governor_with_error(resource_governor, |error| {
...
async fn build_backend_production<F>(
    context: IronClawProductionBuildContext,
    stores: ProductionStoreBundle<F>,
    trigger_repository: Arc<dyn TriggerRepository>,
    production_runtime_services: impl FnOnce(
        Arc<IronClawProductionRuntimeStoreGraph<F>>,
    ) -> IronClawProductionRuntimeServices,
...
where
    F: RootFilesystem + 'static,
{
    let IronClawProductionBuildContext {
        profile,
        wiring_config,
        production_wiring,
        product_auth_ports,
    ...
    let turn_state_scope = match local_runtime_identity.as_ref() {
        ...
        None => {
            default_runtime_owner_scope(owner_user_id.clone()).map_err(IronClawBuildError::Mount)?
        }
    };
    let turn_state_filesystem =
    ...
    let skill_management = Arc::new(IronClawLocalSkillManagementPort::new_with_mount_resolver(
        owner_user_id.clone(),
    ...
    let event_stores = ironclaw_event_store::build_ironclaw_event_stores(
    ...
    let project_service: Arc<dyn ProjectService> =
        Arc::new(IronClawProjectService::new(project_repository));
    // Trigger conversation services over the production scoped filesystem —
    ...
    let trigger_conversation_services =
        IronClawFilesystemConversationServices::new(Arc::clone(&stores.scoped_filesystem))
            .await
            .map_err(|error| IronClawBuildError::InvalidConfig {
                reason: format!("trigger conversation services unavailable: {error}"),
            })?;
    let production_runtime_graph = Arc::new(IronClawProductionRuntimeStoreGraph {
    ...
    // `IronClawProductionRuntimeServices::turn_run_snapshot_source` (#5886).
    let trigger_active_run_lookup: Arc<dyn TriggerActiveRunLookup> = Arc::new(
    ...
    .with_production_ironclaw_event_stores(event_stores)
    ...
    let product_auth_ports = match product_auth_ports {
        ...
        None => {
            ...
            IronClawProductAuthServicePorts::from_shared_with_provider(
                durable,
                provider_composition
                    .client
                    .clone()
...
async fn build_libsql_production(
    ...
    trigger_repository.run_migrations().await.map_err(|error| {
        IronClawBuildError::InvalidConfig {
    ...
    let stores = ProductionStoreBundle::new(
        ...
        resource_governor,
        secret_master_key,
        ironclaw_event_store::IronClawEventStoreConfig::Libsql {
            path_or_url,
            auth_token,
        },
    )
    ...
    build_backend_production(
        ...
        stores,
        trigger_repository,
        IronClawProductionRuntimeServices::LibSql,
...
async fn build_postgres_production(
    ...
    _tls_options: ironclaw_event_store::PostgresPoolTlsOptions,
    secret_master_key: ironclaw_secrets::SecretMaterial,
    process_local_resource_governor_singleton: bool,
) -> Result<IronClawServices, IronClawBuildError> {
    use ironclaw_filesystem::PostgresRootFilesystem;
    ...
    let stores = ProductionStoreBundle::new(
        ...
        resource_governor,
        secret_master_key,
        ironclaw_event_store::IronClawEventStoreConfig::PostgresPool { pool },
    )
    .await?;
    ...
    build_backend_production(
        ...
        stores,
        trigger_repository,
        IronClawProductionRuntimeServices::Postgres,
