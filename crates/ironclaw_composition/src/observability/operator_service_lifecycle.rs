//! Local OS service lifecycle backend for the IronClaw operator facade.
//!
...
use ironclaw_product_workflow::{
    IronClawServiceLifecycleAction, IronClawServiceLifecycleRequest,
    IronClawServiceLifecycleResponse, IronClawServiceLifecycleState, IronClawServicesError,
    IronClawServicesErrorCode, IronClawServicesErrorKind, OperatorServiceLifecycleService,
...
#[derive(Clone)]
pub(crate) struct IronClawLocalServiceLifecycle {
...
impl std::fmt::Debug for IronClawLocalServiceLifecycle {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("IronClawLocalServiceLifecycle")
...
impl IronClawLocalServiceLifecycle {
    ...
    fn unsupported_response(
        action: IronClawServiceLifecycleAction,
    ) -> IronClawServiceLifecycleResponse {
        IronClawServiceLifecycleResponse {
            action,
            state: IronClawServiceLifecycleState::Unsupported,
    ...
    fn missing_home_response(
        ...
        IronClawServiceLifecycleResponse {
            action,
            state: IronClawServiceLifecycleState::Failed,
    ...
    fn executable_path_for_action(
        &self,
        action: IronClawServiceLifecycleAction,
    ) -> Result<&PathBuf, IronClawServiceLifecycleResponse> {
        self.executable
            .as_ref()
    ...
    fn webui_boot_env_for_action(
        &self,
        action: IronClawServiceLifecycleAction,
    ) -> Result<&WebuiBootEnv, IronClawServiceLifecycleResponse> {
        self.webui_boot_env
            .as_ref()
            .map_err(|message| Self::failed_response(action, message))
    }
    ...
    fn install(&self) -> IronClawServiceLifecycleResponse {
        let action = IronClawServiceLifecycleAction::Install;
        ...
        IronClawServiceLifecycleResponse {
            action,
            state: IronClawServiceLifecycleState::Installed,
            message: "local IronClaw service unit is installed".to_string(),
            remediation: None,
        }
    }

    fn start(&self) -> IronClawServiceLifecycleResponse {
        let action = IronClawServiceLifecycleAction::Start;
        match self.platform {
            ServicePlatform::Linux => {
                ...
                self.run_checked(
                    ...
                    IronClawServiceLifecycleState::Running,
    ...
    fn stop(&self) -> IronClawServiceLifecycleResponse {
        let action = IronClawServiceLifecycleAction::Stop;
        match self.platform {
            ServicePlatform::Linux => self.run_checked(
                action,
                "systemctl",
                &["--user", "stop", SYSTEMD_UNIT],
                IronClawServiceLifecycleState::Stopped,
            ...
            ServicePlatform::Macos => {
                ...
                IronClawServiceLifecycleResponse {
                    action,
                    state: IronClawServiceLifecycleState::Stopped,
    ...
    fn status(&self) -> IronClawServiceLifecycleResponse {
        let action = IronClawServiceLifecycleAction::Status;
        match self.platform {
            ServicePlatform::Linux => {
                let output = self
                    .runner
                ...
                match output {
                    ...
                    Ok(output) if output.stdout_text().trim() == "failed" => Self::status_response(
                        IronClawServiceLifecycleState::Failed,
                        "local IronClaw service is failed",
                    ),
                    Ok(_) => Self::status_response(
                        IronClawServiceLifecycleState::Unknown,
    ...
    fn status_response(
        state: IronClawServiceLifecycleState,
        message: &str,
    ) -> IronClawServiceLifecycleResponse {
        IronClawServiceLifecycleResponse {
            action: IronClawServiceLifecycleAction::Status,
            state,
            message: message.to_string(),
            remediation: None,
        }
    ...
    fn run_checked(
        ...
        program: &str,
        args: &[&str],
        success_state: IronClawServiceLifecycleState,
        success_message: &str,
    ) -> IronClawServiceLifecycleResponse {
        match self.require_command_success(action, program, args) {
            Ok(()) => IronClawServiceLifecycleResponse {
                action,
                state: success_state,
    ...
    fn require_command_success(
        ...
        program: &str,
        args: &[&str],
    ) -> Result<(), IronClawServiceLifecycleResponse> {
    ...
    fn systemd_unit(
        &self,
        action: IronClawServiceLifecycleAction,
    ) -> Result<String, IronClawServiceLifecycleResponse> {
...
impl Default for IronClawLocalServiceLifecycle {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl OperatorServiceLifecycleService for IronClawLocalServiceLifecycle {
    async fn control_service(
        &self,
        caller: WebUiAuthenticatedCaller,
        request: IronClawServiceLifecycleRequest,
    ) -> Result<IronClawServiceLifecycleResponse, IronClawServicesError> {
        ...
        tokio::task::spawn_blocking(move || {
            let _permit = permit;
            match action {
                IronClawServiceLifecycleAction::Install => service.install(),
                IronClawServiceLifecycleAction::Start => service.start(),
                IronClawServiceLifecycleAction::Stop => service.stop(),
                IronClawServiceLifecycleAction::Status => service.status(),
            }
        })
        .await
        .map_err(|error| {
            tracing::debug!(%error, "service lifecycle task failed");
            IronClawServicesError::internal_from("service lifecycle task failed")
        })
    }
}
...
fn webui_env_names_for_boot_config(
    ...
    let config = ironclaw_config::IronClawConfigFile::load(&config_path).map_err(|error| {
...
mod tests {
    ...
    ) -> IronClawLocalServiceLifecycle {
        IronClawLocalServiceLifecycle::for_test(
            ServicePlatform::Macos,
    ...
    async fn linux_install_writes_unit_and_runs_allowlisted_systemctl_commands() {
        ...
        let response = service
            .control_service(
                test_caller(),
                IronClawServiceLifecycleRequest {
                    action: IronClawServiceLifecycleAction::Install,
                },
            )
            .await
            .expect("install response");

        assert_eq!(response.state, IronClawServiceLifecycleState::Installed);
    ...
    #[test]
    fn webui_env_names_follow_ironclaw_config_file() {
        let temp = TempDir::new().expect("tempdir");
        std::fs::write(
...
"#,
        ...
        .expect("config file");
        let boot = ironclaw_config::IronClawBootConfig::resolve_from_env_parts(
    ...
    #[test]
    fn webui_env_names_sanitize_ironclaw_config_load_errors() {
        let temp = TempDir::new().expect("tempdir");
    ...
    async fn linux_install_escapes_systemd_special_characters_in_executable_path() {
        let temp = TempDir::new().expect("tempdir");
        let runner = Arc::new(RecordingRunner::new("inactive"));
        let service = IronClawLocalServiceLifecycle::for_test(
            ServicePlatform::Linux,
            Some(temp.path().to_path_buf()),
    ...
    async fn linux_install_rejects_systemd_environment_line_breaks_before_writing_unit() {
        ...
        assert_eq!(response.state, IronClawServiceLifecycleState::Failed);
        assert_eq!(
            response.message,
    ...
    async fn linux_status_maps_service_manager_output_without_raw_command_text() {
        ...
        assert_eq!(response.state, IronClawServiceLifecycleState::Running);
    ...
    async fn linux_start_failure_returns_failed_state() {
        ...
        let response = service
            .control_service(
                test_caller(),
                IronClawServiceLifecycleRequest {
                    action: IronClawServiceLifecycleAction::Start,
                },
            )
            .await
    ...
    async fn install_without_executable_path_fails_before_writing_unit() {
        let temp = TempDir::new().expect("tempdir");
        let service = IronClawLocalServiceLifecycle::for_test_with_executable_error(
            ServicePlatform::Linux,
            Some(temp.path().to_path_buf()),
    ...
    async fn macos_stop_failure_returns_failed_state() {
        ...
        let response = service
            .control_service(
                test_caller(),
                IronClawServiceLifecycleRequest {
                    action: IronClawServiceLifecycleAction::Stop,
                },
            )
            .await
    ...
    async fn macos_status_requires_numeric_pid_for_running_state() {
        ...
        assert_eq!(response.state, IronClawServiceLifecycleState::Stopped);
    }
    ...
    async fn control_service_rejects_non_operator_callers_before_commands() {
        ...
        assert_eq!(error.code, IronClawServicesErrorCode::Forbidden);
        assert!(runner.calls().is_empty());
    }
    ...
    async fn unsupported_platform_reports_unsupported() {
        ...
        assert_eq!(response.state, IronClawServiceLifecycleState::Unsupported);
        assert!(response.remediation.is_some());
    }
