...
//!
//! The IronClaw runtime is intentionally sealed — the high-level
//! [`crate::IronClawRuntime`] hands back only the final assistant reply, and the
...
//! [`IronClawTrajectoryObserver`] via
//! [`IronClawRuntimeInput::with_trajectory_observer`](crate::IronClawRuntimeInput::with_trajectory_observer)
//! to receive those events live.
//!
//! # Facade ownership
//!
//! [`IronClawTrajectoryObserver`] is a composition-owned trait. The capability
...
//! install ([`with_trajectory_observer`](crate::IronClawRuntimeInput::with_trajectory_observer))
...
//! [`with_raw_trajectory_observer`](crate::IronClawRuntimeInput::with_raw_trajectory_observer).
//!
//! # Threading contract
//!
...
/// IronClaw run. See the [module docs](self) for the data-exposure and threading
/// contract. `call_id` is the capability input ref and correlates the two
/// callbacks.
pub trait IronClawTrajectoryObserver: std::fmt::Debug + Send + Sync {
...
/// Wraps a [`IronClawTrajectoryObserver`] so it receives [`safe_preview_value`]
...
pub(crate) struct SafePreviewTrajectoryObserver {
    inner: Arc<dyn IronClawTrajectoryObserver>,
}

impl SafePreviewTrajectoryObserver {
    pub(crate) fn wrap(
        inner: Arc<dyn IronClawTrajectoryObserver>,
    ) -> Arc<dyn IronClawTrajectoryObserver> {
        Arc::new(Self { inner })
    }
}

impl IronClawTrajectoryObserver for SafePreviewTrajectoryObserver {
    fn on_capability_input(
        &self,
        call_id: &str,
        capability_id: &str,
...
/// Adapts a composition-owned [`IronClawTrajectoryObserver`] to the substrate
...
pub(crate) fn as_capability_observer(
    observer: Arc<dyn IronClawTrajectoryObserver>,
