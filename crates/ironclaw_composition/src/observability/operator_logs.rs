...
use ironclaw_product_workflow::{
    ...
    IronClawServicesError, OperatorLogsService, WebUiAuthenticatedCaller,
...
impl From<&IronClawLogQueryRequest> for OperatorLogQueryMode {
    fn from(request: &IronClawLogQueryRequest) -> Self {
        if request.follow {
            Self::Follow
...
impl OperatorLogBuffer {
    ...
    pub fn record(&self, level: IronClawLogLevel, target: &str, message: String) {
    ...
    fn normalize_query_request(
        &self,
        mut request: IronClawLogQueryRequest,
    ...
    fn query(&self, request: IronClawLogQueryRequest) -> IronClawLogQueryResponse {
        let limit = request
            .limit
        ...
        let Ok(state) = self.state.lock() else {
            return IronClawLogQueryResponse {
        ...
        match mode {
            OperatorLogQueryMode::Follow => {
                ...
                for entry in state.entries.iter() {
                    ...
                    let api_entry = IronClawLogEntry::from(entry.clone());
                ...
                for entry in state.entries.iter().rev() {
                    ...
                    if selected.len() >= limit {
                        next_cursor = selected
                            .last()
                            .map(|entry: &IronClawLogEntry| format!("before:{}", entry.id));
                        break;
                    }
                    let api_entry = IronClawLogEntry::from(entry.clone());
                    let entry_bytes = response_entry_bytes(&api_entry);
...
fn response_entry_bytes(entry: &IronClawLogEntry) -> usize {
...
impl StoredLogEntry {
    fn matches_query(&self, request: &IronClawLogQueryRequest) -> bool {
...
impl From<StoredLogEntry> for IronClawLogEntry {
...
pub fn capture_tracing_log(
    ...
    operator_log_buffer().record_with_fields(
        ironclaw_level_from_tracing(level),
        target,
        message,
        &fields,
    );
...
fn ironclaw_level_from_tracing(level: &tracing::Level) -> IronClawLogLevel {
    match *level {
        tracing::Level::TRACE => IronClawLogLevel::Trace,
        tracing::Level::DEBUG => IronClawLogLevel::Debug,
        tracing::Level::INFO => IronClawLogLevel::Info,
        tracing::Level::WARN => IronClawLogLevel::Warn,
        tracing::Level::ERROR => IronClawLogLevel::Error,
    }
}
...
mod tests {
    ...
    fn query_returns_newest_first_and_paginates() {
        ...
        for index in 0..5 {
            buffer.record(
                IronClawLogLevel::Info,
                "ironclaw::test",
                format!("message {index}"),
            );
        }

        let first = buffer.query(IronClawLogQueryRequest::default().set_limit(2));
        assert_eq!(
            first
                .entries
                .iter()
        ...
        let second = buffer.query(
            IronClawLogQueryRequest::default()
                .set_limit(2)
                .set_cursor(cursor),
        );
        assert_eq!(
    ...
    fn query_evicts_oldest_entries_when_capacity_is_reached() {
        ...
        let response = buffer.query(IronClawLogQueryRequest::default().set_limit(10));
    ...
    fn query_filters_by_level_and_target() {
        ...
        buffer.record(IronClawLogLevel::Warn, "ironclaw::beta", "beta".to_string());
        buffer.record(IronClawLogLevel::Warn, "other::beta", "other".to_string());

        let response = buffer.query(
            IronClawLogQueryRequest::default()
                .set_limit(10)
                .set_level(IronClawLogLevel::Warn)
                .set_target("ironclaw"),
        );
    ...
    fn record_redacts_secret_shaped_messages() {
        ...
        let response = buffer.query(IronClawLogQueryRequest::default().set_limit(1));
    ...
    fn filtered_follow_cursor_stays_at_last_returned_match() {
        let buffer = OperatorLogBuffer::new(10);
        buffer.record(IronClawLogLevel::Info, "ironclaw::test", "base".to_string());
        buffer.record(
            IronClawLogLevel::Warn,
            "ironclaw::test",
            "match".to_string(),
    ...
    fn filtered_follow_cursor_allows_later_filter_changes() {
        ...
        let info_response = buffer.query(
            IronClawLogQueryRequest::default()
                ...
                .set_level(IronClawLogLevel::Info)
                .set_follow(true),
        );
    ...
    fn query_enforces_response_byte_budget() {
        ...
        let response = buffer.query(IronClawLogQueryRequest::default().set_limit(100));
    ...
    fn response_entries_bytes_after_push_counts_json_array_overhead() {
        let first = IronClawLogEntry::from(StoredLogEntry {
            id: 1,
            timestamp: Utc::now(),
            level: IronClawLogLevel::Info,
            target: "ironclaw::test".to_string(),
        ...
        let second = IronClawLogEntry::from(StoredLogEntry {
            id: 2,
            timestamp: Utc::now(),
            level: IronClawLogLevel::Warn,
            target: "ironclaw::test".to_string(),
