//! Autonomous Trace Commons turn-end capture for the IronClaw runtime.
//!
...
//! delivered when the same scope runs under the v1 binary; a IronClaw-native
...
use ironclaw_traces::ConversationMessage;
use ironclaw_traces::client::{
    TraceClientAutonomousCaptureOutcome, TraceClientAutonomousCaptureRequest, TraceClientHost,
    TraceClientScope,
};
use ironclaw_traces::contribution::{self as trace, resolve_effective_capture_policy};
...
pub(crate) async fn capture_turn_trace(
    ...
    let outcome = TraceClientHost
        .prepare_autonomous_envelope_from_messages(TraceClientAutonomousCaptureRequest {
            ...
            // IronClaw thread transcripts carry no structured outcome payload;
...
/// Adapt IronClaw thread transcript records into the neutral conversation
...
mod tests {
    ...
    fn unique_scope(label: &str) -> String {
        format!("ironclaw-trace-capture-{label}-{}", Uuid::new_v4())
    }
    ...
    fn tool_result_reference_with_provider_call_becomes_tool_calls_message() {
        // The IronClaw capture adapter must reconstruct tool-call turns from the
