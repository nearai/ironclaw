...
/// `build_ironclaw_runtime` wires) the runtime's FS is constructed once per
/// identity in `build_ironclaw_services`, so it is per-identity by construction;
/// production wiring (a follow-up, since `build_ironclaw_runtime` only supports
...
) -> Result<ironclaw_host_api::VirtualPath, IronClawBuildError> {
...
/// [`RootFilesystem`] already built in `build_ironclaw_services`.
...
) -> Result<HookProjectionRegistry, IronClawBuildError>
where
