...
use crate::error::IronClawBuildError;
...
fn ironclaw_replay_error(
    reason: String,
) -> ironclaw_runner::loop_driver_host::IronClawLoopDriverHostError {
    ironclaw_runner::loop_driver_host::IronClawLoopDriverHostError::InvalidRequest { reason }
}
...
) -> Result<HookDispatcherBuilder, IronClawBuildError> {
...
pub(super) fn project_extension_hook_sets(
...
) -> Result<Vec<ProjectedExtensionHooks>, IronClawBuildError> {
    ...
    for projection in projections {
        ...
        let entries = match project_hook_entries(&extension_id_str, &projection.hooks) {
            ...
            Err(reason) => {
                if trusted {
                    return Err(IronClawBuildError::InvalidConfig { reason });
                }
                emit_hook_quarantined(tenant_id, &extension_id_str, &reason, hook_count);
                continue;
            }
...
) -> Result<Option<HookDispatcherBuilderFactory>, IronClawBuildError> {
...
/// ([`crate::runtime::build_ironclaw_runtime`]) holds the real `tenant_id`, so it
...
where
    F: Fn(HookDispatcherBuilder) -> Result<HookDispatcherBuilder, IronClawBuildError>
        + Send
        + Sync
        + 'static,
{
