...
//! observer through
//! [`IronClawRuntimeInput::with_budget_event_observer`](crate::IronClawRuntimeInput).
...
impl BudgetEventObserver for TracingBudgetEventObserver {
    fn observe(&self, event: BudgetEvent) {
        tracing::debug!(?event, "IronClaw budget event observed");
    }
}
...
/// [`CancellationToken`]. The handle is held by `IronClawRuntime` and
/// awaited inside `IronClawRuntime::shutdown` so the task always exits
