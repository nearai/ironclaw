...
//! IronClaw composition root.
//!
//! Two entry points:
//!
//! - [`build_ironclaw_services`] — substrate/product facades (host runtime,
//!   turn coordinator, product auth). Useful when an outer harness wires the loop
//!   drivers / turn-runner itself (e.g. v1 `AppBuilder`).
//! - [`build_ironclaw_runtime`] — full runtime assembly: substrate + loop
//!   driver registry + LLM model gateway + turn-runner worker, spawned
//!   as one unit. This is the single entry
//!   point used by the standalone `ironclaw` binary and any
//!   future IronClaw ingress.
//!
//! Downstream callers should not name internal IronClaw types directly:
//! [`IronClawRuntime`] exposes only task-level methods, so callers never
...
pub use automation::facade::IronClawAutomationProductFacade;
pub use automation::trigger_poller::PostSubmitDeliveryHook;
pub use error::IronClawBuildError;
...
pub use extension_host::extension_lifecycle_command::{
    IronClawExtensionLifecycleCommand, IronClawExtensionLifecycleCommandError,
...
pub use extension_host::skill_listing::{IronClawSkillListError, list_ironclaw_local_skills};
#[cfg(feature = "test-support")]
pub use factory::AttachmentTestSupport;
#[cfg(feature = "test-support")]
pub use factory::ChannelHostAssemblyTestWiring;
#[cfg(feature = "test-support")]
pub use factory::IronClawApprovalTestParts;
...
pub use factory::{IronClawServices, build_ironclaw_services, builtin_first_party_trust_policy};
...
pub use input::{
    ChannelExtensionBinding, IronClawBuildInput, IronClawRuntimeProcessBinding, OAuthClientConfig,
};
/// OAuth redirect-URI newtype re-exported so the `ironclaw_cli` binary
...
/// `ironclaw_cli_binary_crate_stays_separate_from_v1_root` boundary test (in
...
// Re-exported for `ironclaw_cli` (`runtime/mod.rs` turn-failure display):
...
pub use ironclaw_runner::failure_summary::ironclaw_failure_summary_for_category;
...
pub use ironclaw_skills::{
    ManagedSkillSource as IronClawSkillSource, SkillSummary as IronClawSkillSummary,
    skill_summary_json as ironclaw_skill_summary_json,
};
pub use ironclaw_triggers::TriggerId;
pub use ironclaw_turns::TurnStatus;
pub use llm_admin::llm_catalog::{
    IronClawLlmCatalogError, ProviderCatalogValidationError, resolve_against_registry,
    resolve_ironclaw_runtime_llm, resolve_llm_selection_against_catalog,
    resolve_llm_selection_allow_missing_key, validate_ironclaw_provider_catalog_contents,
};
pub use llm_admin::llm_config_service::{IronClawLlmConfigService, LlmReloadTrigger};
...
pub use deployment::{
    IronClawRuntimeProfileError, IronClawRuntimeProfileOptions,
...
pub use llm_admin::provider_admin::{
    DetectedEnvLlm, EXAMPLE_OVERLAY_PROVIDER_ID, IronClawModelRoutesState, IronClawProviderAdmin,
    IronClawProviderAdminError, IronClawProviderInfo, IronClawProviderList,
    IronClawProviderMetadata, IronClawProviderSelection, IronClawProviderStatus,
    IronClawProviderWriteOutcome, IronClawV1State, ProviderMenuEntry, ProviderProbeOutcome,
};
pub use llm_admin::provider_admin_product_command::IronClawProviderAdminProductCommandService;
...
pub use observability::trajectory_observer::IronClawTrajectoryObserver;
...
pub use product_auth::api::auth::IronClawAuthContinuationDispatcher;
pub use product_auth::api::auth::{
    IronClawAuthProductError, IronClawCredentialLifecycleError, IronClawManualTokenChallenge,
    IronClawManualTokenError, IronClawManualTokenSetupRequest, IronClawManualTokenSubmitRequest,
    IronClawManualTokenSubmitResponse, IronClawOAuthCallbackError, IronClawOAuthCallbackOutcome,
    IronClawOAuthCallbackRequest, IronClawOAuthCallbackResponse, IronClawProductAuthServicePorts,
    IronClawProductAuthServices,
};
...
// compose the IronClaw-native product-auth surface into the WebChat v2 router.
pub use product_auth::serve::{
    ProductAuthRouteMount, ProductAuthRouteState, product_auth_route_mount,
};
pub use production_runtime_policy::IronClawProductionRuntimePolicy;
pub use provider_identity::{
    ...
    IronClawUserIdentityBindingStore, IronClawUserIdentityLookup, IronClawUserIdentityLookupError,
    ProviderIdentityActorResolver, installation_scoped_provider_user_id,
};
pub use readiness::{
    IronClawFacadeReadiness, IronClawReadiness, IronClawReadinessDiagnostic,
    IronClawReadinessDiagnosticComponent, IronClawReadinessDiagnosticReason,
    IronClawReadinessDiagnosticStatus, IronClawReadinessState, IronClawWorkerReadiness,
};
...
pub use root::profile::{IronClawCompositionProfile, IronClawCompositionProfileParseError};
#[cfg(any(test, feature = "test-support"))]
pub use runtime::IronClawTurnDriveOutcome;
pub use runtime::{
    AssistantReply, ConversationId, IronClawRuntime, IronClawRuntimeError, IronClawSkillActivation,
    IronClawSkillActivationMode, IronClawSkillAsset, IronClawSkillBundle,
...
pub use runtime_input::{
    ...
    IronClawRuntimeIdentity, IronClawRuntimeInput, KeepaliveSweepSettings, PollSettings,
...
pub use runtime_input::{IronClawProviderFactory, ResolvedIronClawLlm};
pub use web_access::register_bundled_web_access_first_party_handlers;
pub use webui::facade::{IronClawWebuiBundle, build_webui_services};
...
/// Canonical IronClaw identity resolver vocabulary (issue #4381): the one
...
pub use ironclaw_identity::{
    ExternalSubjectId, IdentityKeyError, IronClawIdentityError, IronClawIdentityResolver,
    ProviderInstanceId, ProviderKind, ResolveExternalIdentity, SurfaceKind,
};

/// Test-support: build a standalone canonical IronClaw identity resolver on an
/// in-memory host filesystem under `tenant_id`.
///
/// This mirrors the production path
/// [`IronClawRuntime::open_ironclaw_identity_resolver`](crate::IronClawRuntime::open_ironclaw_identity_resolver),
...
#[cfg(any(test, feature = "test-support"))]
pub fn open_ironclaw_identity_resolver(
    tenant_id: &ironclaw_host_api::TenantId,
) -> std::sync::Arc<dyn IronClawIdentityResolver> {
    ...
    std::sync::Arc::new(ironclaw_identity::FilesystemIronClawIdentityStore::new(
        filesystem,
        tenant_id.clone(),
...
/// IronClaw model purpose slot names exposed for diagnostic callers.
///
...
pub fn ironclaw_model_slot_names() -> Vec<&'static str> {
...
pub struct IronClawRuntimeReadinessSnapshot {
    pub text_only_driver: IronClawRuntimeComponentStatus,
    pub planned_driver: IronClawRuntimeComponentStatus,
    pub subagent_planned_driver: IronClawRuntimeComponentStatus,
    pub planned_default_profile: IronClawRuntimeComponentStatus,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum IronClawRuntimeComponentStatus {
    Initialized,
    Failed(String),
}

impl IronClawRuntimeComponentStatus {
...
pub fn ironclaw_runtime_readiness_snapshot() -> IronClawRuntimeReadinessSnapshot {
    let mut registry = ironclaw_runner::driver_registry::DriverRegistry::new();
    let text_only_driver = IronClawRuntimeComponentStatus::from_result(
    ...
    let planned_driver = match &family_registry {
        Ok(family_registry) => IronClawRuntimeComponentStatus::from_result(
            ironclaw_runner::planned_driver_factory::register_default_planned_driver(
                &mut registry,
                Arc::clone(family_registry),
            ),
        ),
        Err(error) => IronClawRuntimeComponentStatus::Failed(error.to_string()),
    };
    ...
    let planned_default_profile = IronClawRuntimeComponentStatus::from_result(
        ironclaw_runner::planned_driver_factory::default_planned_run_profile_resolver(),
    );
    IronClawRuntimeReadinessSnapshot {
        text_only_driver,
        planned_driver,
...
use ironclaw_event_store::IronClawEventStoreError;
...
fn invocation_mount_view_for_segments(
    ...
    grants.push(MountGrant::new(
        ...
        // IronClaw identity store's admin user-directory needs it for the delete
    ...
    TWake: TurnRunWakeNotifier + 'static,
{
    ...
    pub event_store: IronClawEventStoreConfig,
    ...
    pub runtime_policy: IronClawProductionRuntimePolicy,
...
#[derive(Debug, Error)]
pub enum IronClawCompositionError {
    #[error("invalid IronClaw production configuration: {reason}")]
    ...
    #[error("IronClaw mount view construction failed: {0}")]
    Mount(#[from] ironclaw_host_api::HostApiError),
    #[error("IronClaw filesystem substrate failed: {0}")]
    Filesystem(#[from] ironclaw_filesystem::FilesystemError),
    #[error("IronClaw resource governor substrate failed: {0}")]
    Resource(#[from] ResourceError),
    #[error("IronClaw run-state substrate failed: {0}")]
    RunState(#[from] RunStateError),
    #[error("IronClaw capability lease substrate failed: {0}")]
    CapabilityLease(#[from] CapabilityLeaseError),
    #[error("IronClaw secret substrate failed: {0}")]
    Secret(#[from] SecretError),
    #[error("IronClaw event store substrate failed: {0}")]
    EventStore(#[from] IronClawEventStoreError),
    #[error("IronClaw turn substrate failed: {0}")]
    Turn(#[from] TurnError),
    #[error("IronClaw run-profile resolver substrate failed: {0}")]
    ...
    #[error("IronClaw production wiring failed: {report:?}")]
    ProductionWiring {
...
) -> Result<LibSqlProductionHostRuntimeServices, IronClawCompositionError>
where
    TPolicy: TrustPolicy + 'static,
    TWake: TurnRunWakeNotifier + 'static,
{
...
) -> Result<PostgresProductionHostRuntimeServices, IronClawCompositionError>
where
    TPolicy: TrustPolicy + 'static,
    TWake: TurnRunWakeNotifier + 'static,
{
...
/// Open a PostgreSQL pool for IronClaw production storage using the same
...
pub fn open_ironclaw_postgres_pool(
    url: secrecy::SecretString,
) -> Result<deadpool_postgres::Pool, IronClawCompositionError> {
    Ok(ironclaw_event_store::open_postgres_pool(url)?)
}

/// Open a PostgreSQL pool for IronClaw production storage with an explicit
/// maximum connection count.
pub fn open_ironclaw_postgres_pool_with_max_size(
    url: secrecy::SecretString,
    max_size: usize,
) -> Result<deadpool_postgres::Pool, IronClawCompositionError> {
