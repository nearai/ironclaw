//! Integration test: a production-shaped `IronClawRuntime` wires the first-class
...
//! the production store graph (`IronClawProjectService` over
...
use ironclaw_composition::{
    IronClawBuildInput, IronClawCompositionProfile, IronClawRuntimeIdentity, IronClawRuntimeInput,
    IronClawRuntimeProcessBinding, build_ironclaw_runtime, build_webui_services,
...
use ironclaw_product_workflow::{
    IronClawCreateProjectRequest, IronClawListProjectsRequest, PROJECT_CREATE_OPERATION,
    PROJECTS_VIEW, WebUiAuthenticatedCaller,
};
...
fn create_request(name: &str) -> IronClawCreateProjectRequest {
...
async fn production_runtime_wires_project_service_and_scopes_by_tenant() {
    ...
    let input = IronClawRuntimeInput::from_services(
        ...
        .with_runtime_process_binding(IronClawRuntimeProcessBinding::tenant_sandbox(Arc::new(
    ...
    let runtime = build_ironclaw_runtime(input)
        .await
    ...
    let listed = PROJECTS_VIEW
        .query_on(
            bundle.api.as_ref(),
            owner.clone(),
            IronClawListProjectsRequest { limit: None },
            None,
        )
        .await
