...
use ironclaw_composition::IronClawRuntimeProcessBinding;
use ironclaw_composition::{IronClawBuildError, IronClawCompositionProfile, IronClawServices};
use ironclaw_composition::{
    IronClawBuildInput, IronClawManualTokenSetupRequest, IronClawManualTokenSubmitRequest,
    IronClawReadinessDiagnostic, IronClawReadinessState, build_ironclaw_services,
};
use ironclaw_composition::{
    IronClawReadinessDiagnosticComponent, IronClawReadinessDiagnosticReason,
    IronClawReadinessDiagnosticStatus,
};
...
async fn assert_production_services_ready_with_first_party_runtime(services: &IronClawServices) {
    assert_eq!(
...
async fn disabled_returns_empty_services() {
    let services = build_ironclaw_services(IronClawBuildInput::disabled("test-owner"))
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        vec![IronClawReadinessDiagnostic::disabled()]
    );
}
...
fn test_sandbox_process_binding() -> IronClawRuntimeProcessBinding {
    let process_port = Arc::new(ironclaw_host_runtime::TenantSandboxProcessPort::new(
        Arc::new(ProductionReadySandboxTransport),
    ));
    IronClawRuntimeProcessBinding::tenant_sandbox(process_port)
}
...
async fn local_dev_product_auth_entrypoint_redacts_manual_token_submit() {
    ...
    let challenge = product_auth
        .request_manual_token_setup(IronClawManualTokenSetupRequest {
            scope: scope.clone(),
    ...
    let submit = IronClawManualTokenSubmitRequest::new(
        scope.clone(),
...
async fn local_dev_runtime_policy_exposes_http_capability() {
    ...
    let services = build_ironclaw_services(
        IronClawBuildInput::local_dev("test-owner", dir.path().to_path_buf())
...
async fn production_requires_configured_trust_policy() {
    ...
    let result = build_ironclaw_services(IronClawBuildInput::libsql(
    ...
    assert!(matches!(
        result,
        Err(IronClawBuildError::MissingProductionTrustPolicy)
    ));
}
...
async fn production_factory_built_product_auth_manual_token_round_trips() {
    ...
    let challenge = product_auth
        .request_manual_token_setup(IronClawManualTokenSetupRequest::new(
            scope.clone(),
            provider.clone(),
            label,
    ...
    let result = product_auth
        .submit_manual_token(IronClawManualTokenSubmitRequest::new(
            scope.clone(),
            challenge.interaction_id,
...
async fn production_rejects_empty_trust_policy() {
    ...
    assert!(matches!(
        result,
        Err(IronClawBuildError::EmptyProductionTrustPolicy)
    ));
}
...
async fn production_requires_runtime_policy() {
    ...
    assert!(matches!(
        result,
        Err(IronClawBuildError::MissingRuntimePolicy)
    ));
}
...
async fn production_rejects_local_only_runtime_policy() {
    ...
    let Err(IronClawBuildError::ProductionWiring { report }) = result else {
        panic!(
    ...
    assert!(
        diagnostics.contains(
            &IronClawReadinessDiagnostic::production_blocker(
                IronClawCompositionProfile::Production,
                IronClawReadinessDiagnosticComponent::RuntimePolicy,
    ...
    assert!(
        diagnostics
            .iter()
            .all(|diagnostic| diagnostic.status == IronClawReadinessDiagnosticStatus::Blocking)
    );
...
async fn production_libsql_resolved_secret_master_key_rejects_invalid_env_key() {
    ...
    let result = build_ironclaw_services(
        IronClawBuildInput::libsql_with_resolved_secret_master_key(
            IronClawCompositionProfile::Production,
            "test-owner",
            db,
            dir.path().join("events.db").to_string_lossy(),
...
async fn production_libsql_services_require_process_port_for_first_party_runtime() {
    ...
    let Err(IronClawBuildError::InvalidConfig { reason }) = result else {
