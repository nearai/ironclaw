...
//! `build_ironclaw_runtime` (local-dev composition); tests exercise the
...
use ironclaw_composition::{
    ...
    IronClawTurnDriveOutcome, PollSettings, build_ironclaw_runtime,
};
...
fn assert_budget_blocked_outcome(
    outcome: IronClawTurnDriveOutcome,
) -> ironclaw_resources::BudgetGateId {
    match outcome {
        IronClawTurnDriveOutcome::BlockedOnGate {
            status,
            gate_ref,
            partial_text,
            ..
        ...
        IronClawTurnDriveOutcome::Terminal(reply) => {
