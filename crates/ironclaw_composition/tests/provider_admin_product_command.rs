...
use chrono::Utc;
use ironclaw_composition::{IronClawProviderAdmin, IronClawProviderAdminProductCommandService};
...
fn workflow_for_ironclaw_home(
    ironclaw_home: &std::path::Path,
    ...
    let command_service = Arc::new(IronClawProviderAdminProductCommandService::new(admin));
...
#[tokio::test]
async fn model_provider_command_executes_through_ironclaw_provider_admin_service() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let (workflow, inbound) = workflow_for_ironclaw_home(&ironclaw_home);
...
#[tokio::test]
async fn model_set_command_executes_through_ironclaw_provider_admin_service() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
