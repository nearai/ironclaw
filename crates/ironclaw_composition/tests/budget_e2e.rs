...
//!
//! These tests drive [`build_ironclaw_runtime`] with a stub
//! [`BudgetTestGateway`] paired with a deterministic
//! [`StaticModelCostTable`], then send user messages through
//! `IronClawRuntime::send_user_message`. They assert the budget pipeline's
...
use ironclaw_composition::{
    BudgetEventObserver, IronClawBuildInput, IronClawRuntimeIdentity, IronClawRuntimeInput,
    PollSettings, build_ironclaw_runtime,
};
...
fn build_input(
    ...
    IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(format!("{tenant}-owner"), owner_root)
...
async fn f1_happy_path_records_actual_usd_in_ledger() {
    ...
    let runtime = build_ironclaw_runtime(build_input(
        "f1",
...
/// by `build_ironclaw_runtime`, plus any additional consumer that
...
/// [`IronClawRuntimeInput::with_budget_event_observer`]. The earlier fix
