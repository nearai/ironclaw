...
use ironclaw_composition::{
    IronClawAuthContinuationDispatcher, IronClawOAuthCallbackOutcome, IronClawOAuthCallbackRequest,
    IronClawOAuthCallbackResponse, IronClawProductAuthServices,
};
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for RecordingContinuationDispatcher {
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for BlockingContinuationDispatcher {
    async fn dispatch_auth_continuation(
        &self,
        _event: AuthContinuationEvent,
    ) -> Result<(), AuthProductError> {
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for FailingContinuationDispatcher {
    async fn dispatch_auth_continuation(
        &self,
        _event: AuthContinuationEvent,
    ) -> Result<(), AuthProductError> {
...
fn auth_services(dispatcher: Arc<RecordingContinuationDispatcher>) -> IronClawProductAuthServices {
...
async fn create_flow(
    services: &IronClawProductAuthServices,
...
async fn oauth_callback_handler_completes_flow_and_dispatches_typed_continuation() {
    ...
    let parsed: IronClawOAuthCallbackResponse = serde_json::from_str(&serialized).unwrap();
...
async fn oauth_callback_handler_terminalizes_and_compensates_lifecycle_activation_failure() {
    ...
    let services = IronClawProductAuthServices::from_shared(
        shared_auth.clone(),
    ...
    let dispatcher = Arc::new(RecordingContinuationDispatcher::default());
    let provider_client = Arc::new(SuccessfulCountingProviderClient::default());
    let retry_services = IronClawProductAuthServices::new(
        shared_auth.clone(),
        shared_auth.clone(),
        shared_auth.clone(),
        shared_auth.clone(),
...
async fn concurrent_lifecycle_callbacks_dispatch_once_and_never_reexchange() {
    ...
    let provider_client = Arc::new(SuccessfulCountingProviderClient::default());
    let services = Arc::new(IronClawProductAuthServices::new(
        shared_auth.clone(),
        shared_auth.clone(),
        shared_auth.clone(),
        shared_auth.clone(),
...
async fn oauth_callback_handler_rejects_wrong_pkce_without_provider_exchange_or_dispatch() {
    ...
    let flow_id = create_flow(&services, owner.clone()).await;
    let mut request = authorized_request(owner, flow_id);
    let IronClawOAuthCallbackOutcome::Authorized { provider_request } = &mut request.outcome else {
...
async fn oauth_callback_handler_returns_sanitized_failures_without_dispatch() {
    ...
    let malformed = services
        .handle_oauth_callback(IronClawOAuthCallbackRequest {
            ...
            opaque_state_hash: state_hash("state-hash"),
            outcome: IronClawOAuthCallbackOutcome::Malformed,
        })
        .await
...
async fn oauth_callback_handler_routes_exchange_failures_through_provider_boundary() {
    ...
    let parsed: ironclaw_composition::IronClawOAuthCallbackError =
