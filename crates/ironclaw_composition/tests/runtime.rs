...
use ironclaw_composition::{
    HooksActivationConfig, IronClawBuildInput, IronClawRuntimeError, IronClawRuntimeIdentity,
    IronClawRuntimeInput, IronClawSkillSourceKind, IronClawTurnDriveOutcome, PollSettings,
    TurnRunnerSettings, build_ironclaw_runtime,
};
use ironclaw_composition::{IronClawCompositionProfile, local_runtime_build_input_with_options};
...
async fn runtime_rejects_disabled_profile_before_local_substrate_lookup() {
    let input =
        IronClawRuntimeInput::from_services(IronClawBuildInput::disabled("runtime-disabled-owner"));
...
async fn runtime_rejects_migration_dry_run_before_live_traffic() {
    ...
    let input = IronClawRuntimeInput::from_services(IronClawBuildInput::libsql(
        ironclaw_composition::IronClawCompositionProfile::MigrationDryRun,
...
async fn runtime_requires_resolved_runtime_policy_for_local_dev() {
    ...
    let input = IronClawRuntimeInput::from_services(IronClawBuildInput::local_dev(
...
async fn hosted_single_tenant_volume_builds_live_runtime() {
    ...
    let runtime = build_ironclaw_runtime(input).await.unwrap();
...
async fn stub_gateway_send_cancels_recovery_required_and_releases_conversation() {
    ...
    let root = tempfile::tempdir().unwrap();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-test-owner", root.path().join("local-dev"))
...
/// `build_ironclaw_runtime` composes the durable turn-state ROW store
...
/// gracefully `shutdown()`s — which routes through `IronClawRuntime::shutdown →
...
async fn inmemory_turn_state_row_store_serves_turn_and_drains_on_shutdown() {
    ...
    let root = tempfile::tempdir().unwrap();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("wb-durable-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
...
async fn send_user_message_with_cancellation_cancels_submitted_run() {
    ...
    let root = tempfile::tempdir().unwrap();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-cancel-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert!(matches!(error, IronClawRuntimeError::OperationCancelled));
...
async fn skill_execution_adapter_prepares_filesystem_bundles_end_to_end() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-skill-execution-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    let policy_activations: Vec<_> = result
        ...
        .filter(|activation| {
            activation.name == "policy-helper"
                && activation.source == Some(IronClawSkillSourceKind::User)
        })
        .collect();
    assert_eq!(
    ...
    let policy_bundles: Vec<_> = result
        ...
        .iter()
        .filter(|bundle| {
            bundle.source == IronClawSkillSourceKind::User && bundle.skill_name == "policy-helper"
        })
        .collect();
    assert_eq!(
        policy_bundles.len(),
...
/// Drives `build_ironclaw_runtime` through the third-party hook activation wiring
...
///
/// This is the only test that exercises the `build_ironclaw_runtime` third-party
...
/// directly against a fake filesystem, and every other `build_ironclaw_runtime`
...
/// uncovered.
#[tokio::test]
async fn build_ironclaw_runtime_wires_third_party_hooks_when_enabled() {
    let _guard = runtime_composition_test_guard().await;
    let root = tempfile::tempdir().unwrap();
    let storage_root = root.path().join("local-dev");
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("runtime-hooks-owner", storage_root)
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    // hook *enforcement* through `build_ironclaw_runtime` still needs a harness
...
/// Caller-level config-wiring test: `build_ironclaw_runtime` correctly threads
/// `TurnRunnerSettings::max_concurrent_runs_per_user` into the turn-state store.
///
/// Exercises the full `build_ironclaw_runtime` → `build_ironclaw_services` →
...
/// adds the missing caller-tier assertion that `build_ironclaw_runtime` propagates the
...
/// which mirrors the exact store construction `build_ironclaw_runtime` performs.
#[tokio::test]
async fn build_ironclaw_runtime_wires_per_user_cap_from_turn_runner_settings() {
    let _guard = runtime_composition_test_guard().await;
    use std::num::NonZeroU32;

    let root = tempfile::tempdir().unwrap();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("cap-wiring-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert!(
        !matches!(reply_a, Err(IronClawRuntimeError::WorkerStopped)),
    ...
    assert!(
        !matches!(reply_b, Err(IronClawRuntimeError::WorkerStopped)),
...
///
/// `IronClawRuntime` starts N workers and returns `WorkerStopped` only when
...
/// this test contributes is the composition-level proof that `build_ironclaw_runtime`
...
async fn multi_worker_runtime_does_not_raise_worker_stopped_while_workers_are_alive() {
    ...
    let root = tempfile::tempdir().unwrap();
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev("multi-worker-guard-owner", root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
    ...
    assert!(
        !matches!(reply, Err(IronClawRuntimeError::WorkerStopped)),
...
async fn local_dev_test_support_interaction_services_use_supplied_turn_coordinator_on_resolve() {
    ...
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(format!("{tag}-owner"), root.path().join("local-dev"))
            .with_runtime_policy(local_dev_runtime_policy()),
    )
    .with_identity(IronClawRuntimeIdentity {
