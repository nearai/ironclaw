...
//! `IronClawProductAuthServices`) is exercised, not just the facade helpers.
...
use ironclaw_composition::{
    IronClawAuthContinuationDispatcher, IronClawProductAuthServices, IronClawReadiness,
    IronClawWebuiBundle,
};
...
use ironclaw_product_workflow::{
    ...
    IronClawStreamEventsRequest, IronClawStreamEventsResponse, ProductOperationRequest,
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for NoopAuthDispatcher {
...
impl ProductSurface for UnusedServices {
    async fn stream_events(
        ...
        Err(rejecting_ironclaw_services_error())
    }
...
fn build_fixture() -> AppFixture {
    ...
    let product_auth = Arc::new(IronClawProductAuthServices::from_shared(
        shared.clone(),
...
async fn challenge_for_gate_returns_oauth_url_view_for_seeded_flow() {
    ...
    let product_auth = Arc::new(
        IronClawProductAuthServices::from_shared(
            shared.clone(),
            Arc::new(NoopAuthDispatcher::default()),
        )
