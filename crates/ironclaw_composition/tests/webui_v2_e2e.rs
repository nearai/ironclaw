...
//! `IronClawRuntime`, overrides its LLM gateway with a scripted
...
use ironclaw_composition::{
    IronClawBuildInput, IronClawRuntime, IronClawRuntimeIdentity, IronClawRuntimeInput,
    OAuthClientConfig, PollSettings, build_ironclaw_runtime, build_webui_services,
};
...
fn local_dev_effective_policy() -> EffectiveRuntimePolicy {
    ...
    // `ResolvedRuntimePolicy` shape; `IronClawBuildInput::with_runtime_policy`
...
async fn build_harness_at_with_runtime_owner_auth_user_and_google_oauth_backend(
    ...
    let mut build_input =
        IronClawBuildInput::local_dev(runtime_owner_id, storage_root).with_runtime_policy(policy);
    ...
    let input = IronClawRuntimeInput::from_services(build_input)
...
async fn build_two_user_harness(
    ...
    let root = tempfile::tempdir().expect("tempdir");
    let storage_root = root.path().join("local-dev");
    let input = IronClawRuntimeInput::from_services(
        IronClawBuildInput::local_dev(USER, storage_root).with_runtime_policy(policy),
    )
    .with_identity(IronClawRuntimeIdentity {
        tenant_id: TENANT.to_string(),
        agent_id: AGENT.to_string(),
...
/// v2 API from the browser side, stream live IronClaw projections over
...
mod operator_llm_config {
    use super::*;
    use ironclaw_config::{IronClawBootConfig, IronClawHome, IronClawProfile};
    ...
    async fn build_operator_harness() -> Harness {
        ...
        let home = IronClawHome::resolve_from_env_parts(
            Some(root.path().join("ironclaw-home").into_os_string()),
            None,
            None,
        )
        .expect("valid IronClaw home");
        let boot = IronClawBootConfig::new(home, IronClawProfile::LocalDev);

        let gateway = Arc::new(ToolCallingGateway::default());
        let input = IronClawRuntimeInput::from_services(
            IronClawBuildInput::local_dev(USER, storage_root)
