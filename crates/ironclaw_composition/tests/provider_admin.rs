use ironclaw_composition::{IronClawProviderAdmin, IronClawProviderAdminError, IronClawV1State};
use ironclaw_config::{IronClawBootConfig, IronClawHome, IronClawProfile};

fn admin_for_home(ironclaw_home: &std::path::Path) -> IronClawProviderAdmin {
    ...
    IronClawProviderAdmin::new(IronClawBootConfig::new(home, IronClawProfile::LocalDev))
}

#[test]
fn list_unknown_provider_returns_known_provider_context() {
    let temp = tempfile::tempdir().expect("tempdir");
    let admin = admin_for_home(&temp.path().join("ironclaw-home"));

    let err = admin
        .list(Some("missing-provider"), false)
        .expect_err("unknown provider should reject");

    let IronClawProviderAdminError::UnknownProvider {
        provider, known, ..
    } = err
    else {
...
fn set_model_reports_active_provider_credential_metadata() {
    ...
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(
...
"#,
    ...
    .expect("write config");
    let admin = admin_for_home(&ironclaw_home);
...
fn set_provider_nearai_uses_concrete_default_model() {
    ...
    let config = std::fs::read_to_string(ironclaw_home.join("config.toml")).expect("read config");
    assert!(
...
fn provider_admin_json_omits_absolute_host_paths() {
    ...
    assert_eq!(list_json["v1_state"], IronClawV1State::NotUsed.as_str());
    ...
    assert_eq!(status_json["v1_state"], IronClawV1State::NotUsed.as_str());
}
