//! Caller-level tests for IronClaw WebUI v2 product-auth OAuth routes.
...
use ironclaw_product_workflow::{
    EXTENSIONS_VIEW, IronClawExtensionInfo, IronClawExtensionListResponse, IronClawServicesError,
    IronClawViewPage, IronClawViewQuery, LifecyclePackageKind, LifecyclePackageRef,
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for RecordingAuthDispatcher {
...
struct UnusedServices {
    installed_extensions: Vec<IronClawExtensionInfo>,
}
    ...
    fn with_installed_extensions(package_ids: &[&str]) -> Self {
        Self {
            installed_extensions: package_ids
                .iter()
                .map(|package_id| IronClawExtensionInfo {
...
fn product_auth_with_interaction_service(
...
) -> Arc<IronClawProductAuthServices> {
    let shared = Arc::new(InMemoryAuthProductServices::new());
    let flow_manager: Arc<dyn AuthFlowManager> = shared.clone();
    let credential_setup_service: Arc<dyn CredentialSetupService> = shared.clone();
    let credential_account_service: Arc<dyn CredentialAccountService> = shared.clone();
    ...
    Arc::new(IronClawProductAuthServices::new(
        flow_manager,
        interaction_service,
        credential_setup_service,
        credential_account_service,
