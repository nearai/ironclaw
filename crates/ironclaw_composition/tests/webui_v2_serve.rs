//! Caller-level tests for the IronClaw-owned WebChat v2 HTTP gateway
...
//! No TCP listener and no real IronClaw runtime are required; the v2
...
use ironclaw_composition::{IronClawReadiness, IronClawWebuiBundle, PublicRouteMount};
...
use ironclaw_product_workflow::{
    EXTENSION_SETUP_SUBMIT_CAPABILITY_ID, EXTENSION_SETUP_VIEW, IronClawCancelRunResponse,
    IronClawCreateThreadResponse, IronClawDeleteThreadRequest, IronClawGetRunStateRequest,
    IronClawGetRunStateResponse, IronClawListThreadsResponse, IronClawResolveGateResponse,
    IronClawRetryRunResponse, IronClawServicesError, IronClawServicesErrorCode,
    IronClawServicesErrorKind, IronClawSetupExtensionResponse, IronClawStreamEventsRequest,
    IronClawStreamEventsResponse, IronClawSubmitTurnResponse, IronClawTimelineResponse,
    IronClawTraceCreditsResponse, IronClawViewPage, IronClawViewQuery, LifecyclePackageKind,
    LifecyclePackageRef, ProductCapabilityInput, ProductOperationId, ProductOperationRequest,
    ProductOperationResponse, ProductSurface, THREAD_DELETE_CAPABILITY_ID, THREADS_VIEW,
    TIMELINE_VIEW, TRACE_CREDITS_VIEW, WebUiAuthenticatedCaller, WebUiCancelRunRequest,
...
fn trace_credits_response(caller: &WebUiAuthenticatedCaller) -> IronClawTraceCreditsResponse {
    let scope = ironclaw_traces::contribution::trace_scope_key(
        caller.tenant_id.as_str(),
        caller.user_id.as_str(),
    );
    let enrolled = ironclaw_traces::contribution::read_trace_policy_for_scope(Some(scope.as_str()))
...
mod openai_compat_mount_tests {
    use super::*;
    use ironclaw_composition::ProtectedRouteMount;
    ...
    impl ProductSurface for AdmissionProductSurface {
        ...
        async fn submit_turn(
            ...
            match response {
                ...
                Err(TurnError::ThreadBusy(busy)) => Ok(IronClawSubmitTurnResponse::RejectedBusy {
                    thread_id,
                    accepted_message_ref,
    ...
    fn invalid_request_error() -> IronClawServicesError {
    ...
    fn map_turn_error(error: TurnError) -> IronClawServicesError {
        match error {
            ...
            TurnError::ScopeNotFound => IronClawServicesError {
            ...
            TurnError::Unauthorized => IronClawServicesError {
            ...
            TurnError::Unavailable { .. } => IronClawServicesError {
...
impl ProductSurface for StubServices {
    async fn invoke(
        ...
        if capability.as_str() == THREAD_DELETE_CAPABILITY_ID {
            let _request: IronClawDeleteThreadRequest = serde_json::from_value(input.into_json()?)
                .map_err(IronClawServicesError::internal_from)?;
            let _ = caller;
        ...
        match query.view_id.as_str() {
            ...
            id if id == TIMELINE_VIEW.id => {
                let thread_id = query.params["thread_id"]
                    .as_str()
                    .ok_or_else(|| IronClawServicesError::internal_from("missing thread_id"))?
                    .to_string();
                Ok(IronClawViewPage {
                    payload: serde_json::to_value(IronClawTimelineResponse {
                        thread: SessionThreadRecord {
            ...
            id if id == EXTENSION_SETUP_VIEW.id => {
                let package_id = query.params["package_id"]
                    .as_str()
                    .ok_or_else(|| IronClawServicesError::internal_from("missing package_id"))?;
                let package_ref =
...
impl Drop for TraceScopeCleanup {
    fn drop(&mut self) {
        let dir =
            ironclaw_traces::contribution::trace_contribution_dir_for_scope(Some(self.0.as_str()));
...
/// in `crates/.../src` by the `ironclaw_product_api_crates_do_not_bind_http_ingress`
