...
use ironclaw_composition::{IronClawAuthContinuationDispatcher, IronClawProductAuthServices};
...
fn auth_services(services: Arc<InMemoryAuthProductServices>) -> IronClawProductAuthServices {
    IronClawProductAuthServices::from_shared(services, Arc::new(NoopContinuationDispatcher))
}
...
async fn assert_lifecycle_uninstall_denies_blocked_auth_gate(fail_flow_before_uninstall: bool) {
    ...
    let services = IronClawProductAuthServices::from_shared(auth.clone(), dispatcher);
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for RecordingCanceledContinuationDispatcher {
    async fn dispatch_auth_continuation(
        &self,
        _event: AuthContinuationEvent,
    ) -> Result<(), AuthProductError> {
...
async fn lifecycle_cleanup_dispatches_each_canceled_turn_gate_continuation_once() {
    ...
    let services = IronClawProductAuthServices::from_shared(auth.clone(), dispatcher.clone());
