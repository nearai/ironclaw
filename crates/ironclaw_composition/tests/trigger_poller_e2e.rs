...
//!
//! Drives a real `IronClawRuntime` with the trigger poller enabled, seeds a
...
async fn build_runtime_with<G: HostManagedModelGateway + 'static>(
    ...
    build_ironclaw_runtime(input).await.expect("runtime builds")
}
...
/// real IronClaw runtime — a scheduled-trigger fire resolves the dedicated
