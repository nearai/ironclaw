//! Integration test: a production-shaped `IronClawRuntime` wires the SSO / admin
...
//! before `open_ironclaw_identity_resolver` grew a production fallback it
...
use ironclaw_composition::{
    ExternalSubjectId, IronClawBuildInput, IronClawCompositionProfile, IronClawRuntimeIdentity,
    IronClawRuntimeInput, IronClawRuntimeProcessBinding, ProviderKind, ResolveExternalIdentity,
    SurfaceKind, build_ironclaw_runtime, builtin_first_party_trust_policy,
};
...
/// `open_ironclaw_identity_resolver` returned `None` here.
#[tokio::test]
async fn production_runtime_wires_identity_resolver_and_isolates_tenants() {
    ...
    let resolver = runtime
        .open_ironclaw_identity_resolver(&tenant_a)
        .await
    ...
    // (4) ADMIN USER DIRECTORY WIRING (#5013). `IronClawRuntime::ironclaw_user_directory`
    ...
    let directory = runtime
        .ironclaw_user_directory_for_tests()
