...
use ironclaw_composition::{
    IronClawCompositionError, IronClawProductionRuntimePolicy, PostgresProductionSubstrateConfig,
...
async fn postgres_substrate_builder_rejects_without_singleton_resource_governor_authority() {
    ...
    assert!(matches!(
        result,
        Err(IronClawCompositionError::InvalidConfig { reason }) if reason.contains("singleton or elected resource-governor owner")
    ));
}
