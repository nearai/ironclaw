...
use ironclaw_composition::{
    IronClawAuthContinuationDispatcher, IronClawManualTokenError, IronClawManualTokenSetupRequest,
    IronClawManualTokenSubmitRequest, IronClawManualTokenSubmitResponse,
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for NoopContinuationDispatcher {
...
#[async_trait]
impl IronClawAuthContinuationDispatcher for FailsFirstContinuationDispatcher {
    async fn dispatch_auth_continuation(
        &self,
        event: AuthContinuationEvent,
    ) -> Result<(), AuthProductError> {
...
fn auth_services() -> IronClawProductAuthServices {
...
fn setup_request(scope: AuthProductScope) -> IronClawManualTokenSetupRequest {
    IronClawManualTokenSetupRequest::new(
        scope,
        provider(),
        label(),
...
fn expired_setup_request(scope: AuthProductScope) -> IronClawManualTokenSetupRequest {
    IronClawManualTokenSetupRequest {
...
fn assert_error_safe(error: &IronClawManualTokenError) {
    let serialized = serde_json::to_string(error).unwrap();
    assert_eq!(
        serde_json::from_str::<IronClawManualTokenError>(&serialized).unwrap(),
        *error
    );
...
fn assert_response_safe(response: &IronClawManualTokenSubmitResponse) {
    let serialized = serde_json::to_string(response).unwrap();
    assert_eq!(
        serde_json::from_str::<IronClawManualTokenSubmitResponse>(&serialized).unwrap(),
        *response
    );
    assert!(!serialized.contains(RAW_TOKEN));
    assert!(!serialized.contains("manual-access-"));
...
async fn manual_token_facade_submits_secret_without_exposing_token() {
    ...
    let request = IronClawManualTokenSubmitRequest::new(
        owner.clone(),
        challenge.interaction_id,
        secret(RAW_TOKEN),
    );
...
async fn manual_token_facade_tracks_setup_and_submit_in_auth_flow() {
    ...
    let services = IronClawProductAuthServices::from_shared(shared.clone(), dispatcher.clone());
    let owner = scope("alice");
