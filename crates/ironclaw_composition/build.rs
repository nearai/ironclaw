...
fn main() -> BuildResult<()> {
    ...
    embed_ironclaw_skills(repo_root)
}

fn embed_ironclaw_skills(repo_root: &Path) -> BuildResult<()> {
    let skills_dir = repo_root.join("skills");
    println!("cargo:rerun-if-changed={}", skills_dir.display());

    let out_dir = PathBuf::from(env::var("OUT_DIR")?);
    let summaries_out_path = out_dir.join("embedded_ironclaw_skill_summaries.json");
    let bundles_out_path = out_dir.join("embedded_ironclaw_skill_bundles.json");
    ...
    for entry in entries {
        ...
        if !validate_skill_name(&dir_name) {
            return Err(build_error(format!(
                "bundled IronClaw skill directory has invalid name `{dir_name}`"
            )));
        }
        ...
        let parsed = parse_skill_md(&skill_md_content).map_err(|error| {
            build_error(format!(
                "parse bundled IronClaw skill `{dir_name}` at {}: {error}",
                skill_md.display()
            ))
        })?;
        if parsed.manifest.name != dir_name {
            return Err(build_error(format!(
                "bundled IronClaw skill `{}` manifest name `{}` must match directory name",
...
fn path_is_real_dir(path: &Path) -> BuildResult<bool> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => Err(build_error(format!(
            "bundled IronClaw skills path must not be a symlink: {}",
            path.display()
        ))),
...
fn path_is_real_file(path: &Path) -> BuildResult<bool> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => Err(build_error(format!(
            "bundled IronClaw skill file must not be a symlink: {}",
            path.display()
        ))),
...
fn non_symlink_file_type(entry: &fs::DirEntry) -> BuildResult<FileType> {
    ...
    if file_type.is_symlink() {
        return Err(build_error(format!(
            "bundled IronClaw skill entry must not be a symlink: {}",
            entry.path().display()
        )));
    }
    Ok(file_type)
