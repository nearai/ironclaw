...
use ironclaw_openai_compat::{FilesystemOpenAiCompatRefStore, OPENAI_COMPAT_ADAPTER_ID};
...
use ironclaw_product_workflow::{
    IronClawCancelRunResponse, IronClawCreateThreadResponse, IronClawServicesError,
    IronClawServicesErrorCode, IronClawServicesErrorKind, IronClawStreamEventsRequest,
    IronClawStreamEventsResponse, IronClawSubmitTurnResponse, ProductSurface,
...
struct FakeProductSurfaceState {
    ...
    stream_events: Vec<IronClawStreamEventsRequest>,
    fail_with: Option<IronClawServicesError>,
}
...
impl FakeProductSurface {
    ...
    pub(crate) fn with_error(error: IronClawServicesError) -> Self {
        Self {
            state: Mutex::new(FakeProductSurfaceState {
    ...
    pub(crate) fn force_failure(&self, error: IronClawServicesError) {
...
impl ProductSurface for FakeProductSurface {
    ...
    async fn submit_turn(
        ...
        let response = ironclaw_submit_from_ack(thread_id.clone(), outcome, rejection_param);
        if matches!(response, Ok(IronClawSubmitTurnResponse::Submitted { .. })) {
    ...
    loop {
        match ack {
            ProductInboundAck::Accepted {
                ...
                return Ok(IronClawSubmitTurnResponse::Submitted {
                    thread_id,
                    accepted_message_ref,
...
pub(crate) fn service_unavailable() -> IronClawServicesError {
...
pub(crate) fn rate_limited() -> IronClawServicesError {
    IronClawServicesError {
...
pub(crate) fn internal_error() -> IronClawServicesError {
    IronClawServicesError {
...
fn service_error_from_rejection(
    ...
    match rejection.kind {
        ProductRejectionKind::BindingRequired => IronClawServicesError {
        ...
        ProductRejectionKind::InvalidRequest => IronClawServicesError {
            code: IronClawServicesErrorCode::InvalidRequest,
            kind: IronClawServicesErrorKind::Validation,
            status_code: 400,
            retryable: false,
