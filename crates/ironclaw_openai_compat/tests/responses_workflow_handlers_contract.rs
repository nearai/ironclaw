...
async fn responses_create_submits_product_surface_and_returns_projection() {
    ...
    let reader = Arc::new(StaticResponsesReader::completed("hello from IronClaw"));
    let router = test_router(workflow.clone(), reader);

    let response = router
        .oneshot(response_create_request(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "input": "hello"}),
            Some("same-key"),
        ))
        .await
        .expect("response");
    ...
    assert_eq!(body["model"], "gpt-ironclaw");
...
async fn responses_idempotency_replays_same_id_and_conflicts_on_different_body() {
    ...
    let body = json!({"model": "gpt-ironclaw", "input": "hello"});
    ...
    let conflict = router
        .oneshot(response_create_request(
            "/v1/responses",
            json!({"model": "gpt-ironclaw", "input": "different"}),
            Some("same-key"),
        ))
        .await
        .expect("conflict");
...
async fn unsupported_responses_tools_and_unwired_stream_reject_before_product_surface() {
    ...
    let stream = router
        ...
        .oneshot(response_create_request(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "input": "hello", "stream": true}),
            None,
        ))
        .await
        .expect("stream");
    ...
    let tools = router
        ...
        .oneshot(response_create_request(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "input": "hello", "tools": [{"type": "web_search_preview"}]}),
            None,
        ))
        .await
        .expect("tools");
...
async fn responses_empty_tools_array_is_absent_equivalent() {
    ...
    let response = router
        .oneshot(response_create_request(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "input": "hello", "tools": []}),
            None,
        ))
        .await
        .expect("response");
...
async fn responses_rejects_excessive_input_items_before_product_surface() {
    ...
    let response = router
        .oneshot(response_create_request(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "input": items}),
            None,
        ))
        .await
        .expect("response");
...
async fn responses_function_call_output_without_external_tools_is_rejected() {
    ...
    let created = json_body(
        router
            ...
            .oneshot(response_create_request(
                "/api/v1/responses",
                json!({"model": "gpt-ironclaw", "input": "hi"}),
                None,
            ))
            .await
            .expect("create"),
