...
async fn responses_stream_emits_response_events_without_projection_cursor() {
    ...
    let response = router
        .oneshot(post_json(
            "/api/v1/responses",
            json!({"model": "gpt-ironclaw", "stream": true, "input": "hello"}),
        ))
        .await
        .expect("response");
...
impl OpenAiResponsesProjectionReader for StaticResponsesReader {
    ...
    ) -> Result<ironclaw_openai_compat::OpenAiResponseProjection, OpenAiCompatHttpError> {
        Ok(ironclaw_openai_compat::OpenAiResponseProjection::new(
            completed_response(request.public_id, "unused"),
        ))
    }
...
fn completed_response(public_id: OpenAiResponseId, text: &str) -> OpenAiResponseObject {
    OpenAiResponseObject {
        ...
        usage: Some(ironclaw_openai_compat::OpenAiResponseUsage {
            input_tokens: 1,
