...
//! `test_models_no_auth` behaviors onto the IronClaw route + host-injected
...
async fn models_endpoint_returns_openai_list_for_authenticated_caller() {
    for path in ["/v1/models", "/api/v1/models"] {
        let catalog = Arc::new(StaticModelCatalog {
            entries: vec![
                OpenAiCompatModelEntry::new("gpt-ironclaw"),
        ...
        assert_eq!(body["data"][0]["id"], "gpt-ironclaw", "{path}");
...
async fn models_endpoint_without_caller_returns_401_before_catalog() {
    let catalog = Arc::new(StaticModelCatalog {
        entries: vec![OpenAiCompatModelEntry::new("gpt-ironclaw")],
    });
