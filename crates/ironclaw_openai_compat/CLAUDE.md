...
IronClaw-native OpenAI-compatible API contract surface for #3283 / #4442 /
...
protected IronClaw route stack. The injected `OpenAiChatCompletionsWorkflow`
...
- The route builds a canonical projection read request from the authenticated
  ...
  product turn.
- Detached waits must remain bounded by the shared IronClaw turn-admission
...
- Client-supplied `tools` and `tool_choice` are model hints only. They are
  ...
  execute as IronClaw capabilities from this crate.
...
fields that affect IronClaw policy, such as `tools`, `tool_choice`, `stream`, and
