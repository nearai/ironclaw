...
use ironclaw_product_workflow::{IronClawServicesError, IronClawServicesErrorCode};
...
impl OpenAiCompatErrorCode {
    pub fn sanitized_message(self) -> &'static str {
        match self {
            ...
            Self::Unsupported => "This OpenAI-compatible IronClaw route is not wired yet.",
...
impl OpenAiCompatHttpError {
    ...
    pub fn from_ironclaw_services_error(error: IronClawServicesError) -> Self {
        let kind = match error.code {
            IronClawServicesErrorCode::InvalidRequest => OpenAiCompatErrorKind::Validation,
            IronClawServicesErrorCode::Unauthenticated => OpenAiCompatErrorKind::Authentication,
            IronClawServicesErrorCode::Forbidden => OpenAiCompatErrorKind::PermissionDenied,
            IronClawServicesErrorCode::NotFound => OpenAiCompatErrorKind::NotFound,
            IronClawServicesErrorCode::Conflict => OpenAiCompatErrorKind::Conflict,
            IronClawServicesErrorCode::RateLimited => OpenAiCompatErrorKind::RateLimited,
            IronClawServicesErrorCode::Unavailable => OpenAiCompatErrorKind::ServiceUnavailable,
            IronClawServicesErrorCode::Internal => OpenAiCompatErrorKind::Internal,
        };
...
impl From<IronClawServicesError> for OpenAiCompatHttpError {
    fn from(error: IronClawServicesError) -> Self {
        Self::from_ironclaw_services_error(error)
    }
}
