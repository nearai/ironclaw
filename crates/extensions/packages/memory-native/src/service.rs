...
///
/// - When the exact literal query occurs in the snippet, the preview contains
///   excerpts around successive occurrences. Each excerpt keeps
///   [`EXCERPT_PRE_BYTES`] preceding and [`EXCERPT_POST_BYTES`] following
///   bytes, and excerpts are joined by an ellipsis delimiter. A window fully
///   covered by the previous excerpt is skipped, and accumulation stops at
///   the cap. Repeated matches therefore retain useful context from across a
///   large document without returning the whole body.
...
/// in a bounded search preview. Together with the preceding window this
/// carries the matching sentence or record in typical memory documents
/// without allowing one match to consume the entire result budget.
...
mod tests {
    ...
    fn small_snippet_is_returned_unchanged() {
        ...
        assert_eq!(bound_search_snippet(snippet.clone(), QUERY), snippet);
    }

    #[test]
    fn empty_snippet_is_returned_unchanged() {
        assert_eq!(bound_search_snippet(String::new(), QUERY), String::new());
    }

    #[test]
    fn snippet_exactly_at_bound_is_returned_unchanged() {
        let snippet = "a".repeat(MAX_SEARCH_SNIPPET_BYTES);
        let bounded = bound_search_snippet(snippet.clone(), QUERY);
    ...
    fn oversized_body_with_exact_literal_query_retains_head_match() {
        ...
        assert_eq!(bound_search_snippet(body.clone(), QUERY), bounded);
    }
    ...
    fn oversized_1mib_multiple_matches_retain_each_query_context() {
        ...
        let body = mib_body_with(&[
            (100, HEAD_MATCH),
            (MAX_SEARCH_SNIPPET_BYTES + 64, MATCH_AFTER_CAP),
            (MIB / 2, MATCH_MIDDLE),
            (MIB - 256, MATCH_TAIL),
        ]);
        let bounded = bound_search_snippet(body.clone(), QUERY);
        ...
        assert_eq!(
            bound_search_snippet(body, QUERY),
            bounded,
    ...
    fn truncated_preview_is_deterministic() {
        let body = mib_body(Some(10));
        assert_eq!(
            bound_search_snippet(body.clone(), QUERY),
            bound_search_snippet(body, QUERY)
        );
    }
}
