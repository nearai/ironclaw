...
/// Build the typed guest-failure (WIT `guest-failure`, near:agent@0.4.0) for
...
fn structured_error_for(failure: RawSlackFailure) -> GuestFailure {
    match failure {
        RawSlackFailure::Transport(raw) if raw == "AuthRequired" => {
            structured_error(&raw, ErrorKind::AuthRequired)
        }
        RawSlackFailure::Transport(raw) => structured_error(&raw, ErrorKind::OperationFailed),
