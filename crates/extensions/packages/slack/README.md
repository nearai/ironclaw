...
- **Surfaces:** channel (`messages`, inbound + outbound) + 16 tools (`slack.search_messages`, `slack.send_message`, …) + `[auth.slack]` (oauth2). Every tool is a `standard_op` binding: Slack covers all 16 core standard messaging operations, so the host owns their schemas and the manifest `description` is only the Slack addendum
