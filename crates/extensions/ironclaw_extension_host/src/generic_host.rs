...
pub fn effective_resolved_for_package(
    ...
    if captures_dynamic_schemas && let Some(mcp) = resolved.mcp.as_mut() {
        mcp.dynamic_input_schemas = package
            ...
            .collect();
        mcp.tool_names = package
            .hosted_mcp_tool_names()
            .iter()
            .map(|(capability_id, wire_name)| {
                (capability_id.as_str().to_string(), wire_name.clone())
            })
            .collect();
    }
    resolved
...
pub(crate) fn rebuild_package_from_resolved(
    ...
    let package = match &resolved.root_binding {
    ...
    let Some(tool_names) = resolved
        .mcp
        .as_ref()
        .map(|mcp| &mcp.tool_names)
        .filter(|tool_names| !tool_names.is_empty())
    else {
        return Ok(package);
    };
    let tool_names = tool_names
        .iter()
        .map(|(capability_id, wire_name)| {
            ironclaw_host_api::ids::CapabilityId::new(capability_id.clone())
                .map(|capability_id| (capability_id, wire_name.clone()))
                .map_err(|error| format!("invalid persisted MCP capability id: {error}"))
        })
        .collect::<Result<std::collections::BTreeMap<_, _>, _>>()?;
    package
        .with_hosted_mcp_tool_names(tool_names)
        .map_err(|error| format!("package rebuild failed: {error}"))
}
