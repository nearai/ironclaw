...
fn collect(root: &Path, dir: &Path, out: &mut Vec<(String, PathBuf)>) {
    ...
    for entry in read_dir {
        ...
        if file_type.is_dir() {
            ...
            // (`*.test.mjs` are intentionally still embedded: the asset table
            // is reused as a fixture by the `assets.rs` caller-level JS
            // regression tests, which assert their content via `asset_text`.)
