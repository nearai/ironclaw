...
test("useChat: blank unpaired Slack chat opens pairing panel from extension state", async () => {
  ...
  assert.match(onboardingUpdate?.value?.instructions, /run \/pair in Slack/);
  assert.equal(
    onboardingUpdate?.value?.inputPlaceholder,
