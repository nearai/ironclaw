...
test("hidden jobs presenters keep empty-shell state labels and actions deterministic", () => {
  assert.deepEqual(JOB_DETAIL_TABS.map((tab) => tab.id), ["overview", "activity", "files"]);
  ...
  assert.equal(formatJobDate("not-a-date"), "Not available");
...
test("hidden routines presenters sort enabled routines first and fail closed visibly", () => {
  ...
  const sorted = sortRoutines([
    ...
    { id: "enabled-invalid", enabled: true, next_fire_at: "not-a-date" },
  ...
  assert.equal(formatRoutineDate("not-a-date"), "Not scheduled");
...
test("hidden missions presenters summarize and sort empty-shell mission state", () => {
  ...
  const sorted = sortMissions([
    ...
    { id: "active-invalid", status: "Active", updated_at: "not-a-date" },
  ...
  assert.equal(formatMissionDate("not-a-date"), "Not scheduled");
});
