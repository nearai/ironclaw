...
- Legacy import is owned by the offline `ironclaw_reborn_migration` companion,
  which writes supported documents through this crate's native service. Do not
  read, migrate, or alias legacy rows directly from this crate, and do not add a
  dependency on v1.
