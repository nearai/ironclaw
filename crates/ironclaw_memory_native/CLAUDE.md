...
- IronClaw memory is **native and isolated**. The current implementation is a single `FilesystemMemoryDocumentRepository` over `RootFilesystem` (see the last bullet); the **deferred** SQL-backed target is dedicated `ironclaw_memory_*` tables with explicit `tenant_id`, `user_id`, `agent_id`, `project_id` scope columns — not legacy `memory_documents`. Do not encode IronClaw scope into legacy `memory_documents.user_id` and do not introduce a `WorkspaceMemoryAdapter` or any other bridge over `src/workspace::Workspace`.
