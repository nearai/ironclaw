...
/// The filesystem is the single authority on whether a host process can open a path.
///
/// There were three authorities before this: the composite's mount table, `DiskFilesystem`'s own
/// `mount_local` table, and `HostProcessPort`'s separate `workdir_aliases` list, built at composition
/// from a different value. They disagreed silently — under hosted single-tenant the file tools
/// resolved `/workspace` to `<root>/tenants/<t>/users/<u>` while the shell's alias resolved the same
/// string to `<root>`, so an agent wrote a file with `write_file` and then could not run it, three
/// directories away, with no error from either side.
#[tokio::test]
async fn host_path_is_reported_for_disk_mounts_and_withheld_for_database_mounts() {
    let mut root = CompositeRootFilesystem::new();
    let (disk_backend, disk_dir) = empty_local_backend("/projects");
    // Canonical, because `mount_local` canonicalizes its host root -- on macOS a temp dir under
    // `/var` reports as `/private/var`. Canonical is what a working directory wants anyway.
    let host_root = std::fs::canonicalize(disk_dir.path()).unwrap();
    ...
    // A disk-backed path reports where it lives, whether or not it exists yet: a working directory
    // that has not been created is still correctly backed, and existence is the caller's question.
    let workspace = VirtualPath::new("/projects/workspace/scripts/egfr.py").unwrap();
    assert_eq!(
        root.host_path_for(&workspace),
        Some(host_root.join("workspace/scripts/egfr.py")),
        "a disk-backed path must report its host path so the shell and the file tools agree"
    );

    // A database-backed path has no host representation, and saying so is the whole point: a skill's
    // script lives here, and `python3 <that path>` can never work.
    assert_eq!(
        root.host_path_for(
            &VirtualPath::new("/tenants/acme/users/ada/skills/x/scripts/y.py").unwrap()
        ),
        None,
        "a database-backed path must report no host path, so callers fail loudly instead of \
         spawning against a path that does not exist"
    );

    // Longest-prefix routing, same as reads and writes.
    assert_eq!(
        root.host_path_for(&VirtualPath::new("/projects").unwrap()),
        Some(host_root.clone()),
        "the mount root itself resolves to the host root"
    );
}
