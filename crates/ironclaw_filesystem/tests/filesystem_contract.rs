...
async fn scoped_read_resolves_mount_view_and_reads_bytes() {
    ...
    let bytes = scoped
        .read_file(&ResourceScope::system(), &ScopedPath::new("/workspace/README.md").unwrap())
        .await
        .unwrap();
...
async fn scoped_write_is_denied_on_read_only_mount() {
    ...
    let err = scoped
        .write_file(&ResourceScope::system(), &ScopedPath::new("/workspace/generated.txt").unwrap(),
            b"nope",
        )
        .await
        .unwrap_err();
...
async fn scoped_append_requires_write_permission_and_appends_bytes() {
    ...
    let err = read_only
        .append_file(&ResourceScope::system(), &ScopedPath::new("/workspace/log.jsonl").unwrap(),
            b"denied\n",
        )
        .await
        .unwrap_err();
    ...
    writable
        .append_file(&ResourceScope::system(), &ScopedPath::new("/workspace/log.jsonl").unwrap(), b"two\n")
        .await
        .unwrap();
...
async fn scoped_delete_requires_delete_permission_and_removes_file() {
    ...
    let err = no_delete
        .delete(&ResourceScope::system(), &ScopedPath::new("/workspace/generated.txt").unwrap())
        .await
        .unwrap_err();
    assert!(matches!(
        err,
...
async fn scoped_create_dir_all_requires_write_permission() {
    ...
    let read_only = scoped_project_fs(storage.path(), MountPermissions::read_only());
    let err = read_only
        .create_dir_all(&ResourceScope::system(), &ScopedPath::new("/workspace/generated/deep").unwrap())
        .await
        .unwrap_err();
    assert!(matches!(
        err,
...
async fn list_requires_list_permission_through_scoped_api() {
    ...
    let err = scoped
        .list_dir(&ResourceScope::system(), &ScopedPath::new("/workspace").unwrap())
        .await
        .unwrap_err();
...
async fn unknown_scoped_alias_fails_closed_through_filesystem_api() {
    ...
    let scoped = ScopedFilesystem::with_fixed_view(Arc::new(root), MountView::new(Vec::new()).unwrap());
    let err = scoped
        .read_file(&ResourceScope::system(), &ScopedPath::new("/memory/facts.md").unwrap())
        .await
        .unwrap_err();
...
async fn artifact_write_is_confined_to_approved_virtual_mount() {
    ...
    scoped
        .write_file(&ResourceScope::system(), &ScopedPath::new("/artifacts/result.json").unwrap(), b"{}")
        .await
        .unwrap();
...
async fn local_backend_denies_symlink_escape() {
    ...
    let err = scoped
        .read_file(&ResourceScope::system(), &ScopedPath::new("/workspace/escape.txt").unwrap())
        .await
        .unwrap_err();
...
async fn read_requires_read_permission_through_scoped_api() {
    ...
    let err = scoped
        .read_file(&ResourceScope::system(), &ScopedPath::new("/workspace/secret.txt").unwrap())
        .await
        .unwrap_err();
...
async fn stat_is_allowed_by_read_or_list_and_denied_without_both() {
    ...
    let stat = read_only
        .stat(&ResourceScope::system(), &ScopedPath::new("/workspace/file.txt").unwrap())
        .await
        .unwrap();
...
async fn workspace_write_creates_parent_directories() {
    ...
    scoped
        .write_file(&ResourceScope::system(), &ScopedPath::new("/workspace/generated/deep/file.txt").unwrap(),
            b"created",
        )
        .await
        .unwrap();
...
async fn local_backend_denies_write_through_symlink_escape() {
    ...
    let scoped = scoped_project_fs(storage.path(), MountPermissions::read_write());
    let err = scoped
        .write_file(&ResourceScope::system(), &ScopedPath::new("/workspace/escape.txt").unwrap(),
            b"changed",
        )
        .await
        .unwrap_err();
...
async fn local_backend_denies_write_through_symlinked_parent_escape() {
    ...
    let scoped = scoped_project_fs(storage.path(), MountPermissions::read_write());
    let err = scoped
        .write_file(&ResourceScope::system(), &ScopedPath::new("/workspace/outside-dir/new.txt").unwrap(),
            b"escaped",
        )
        .await
        .unwrap_err();
