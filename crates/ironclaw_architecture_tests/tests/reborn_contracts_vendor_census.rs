    ...
    for src in contracts_family_src_roots(&metadata) {
        ...
        for file in files {
            ...
            for term in LLM_VENDOR_TERMS {
                let carved = TERM_COLLISION_CARVE_OUTS
                    .iter()
                    .any(|(path, carved_term, _)| *path == relative && carved_term == term);
                if carved {
                    continue;
                }
...
fn reborn_contracts_family_names_llm_vendors_only_in_censused_scopes() {
    ...
    let censused: BTreeMap<&str, &CensusScope> =
        CENSUS.iter().map(|scope| (scope.path, scope)).collect();
    assert_eq!(
        censused.len(),
        CENSUS.len(),
    ...
    let uncensused: Vec<String> = found
        .iter()
        .filter(|(path, _)| !censused.contains_key(path.as_str()))
        .map(|(path, terms)| {
            format!(
                "    {path}: {}",
                terms
    ...
    for scope in CENSUS {
        let Some(terms) = found.get(scope.path) else {
            continue;
        };
