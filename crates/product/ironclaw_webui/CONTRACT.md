...
| `product_auth_route_mount(state) -> ProductAuthRouteMount` | Product-auth route router + descriptors for OAuth start/callback/status, the unauthenticated public CIMD document at `GET /api/reborn/product-auth/oauth/{provider}/client-metadata.json`, manual-token setup/submit, account selection/recovery/refresh, and lifecycle cleanup. The CIMD document's URL is the OAuth `client_id`, so deployments must configure a public HTTPS product-auth callback base. |
