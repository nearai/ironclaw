...
//! live `// arch-exempt: large_file` waiver naming plan #5985. The map is **not**
//! that split and does not discharge that waiver — PROPOSAL §6.4.15 calls this
//! shape "module-charter work, **not a split**", and §6.9.1 asks for a
//! "module-charter map … the audited ≥11 sub-owners". Its job is to say which
//! concern a change belongs to *while the file is still one file*, so the
//! eventual split inherits a decided seam list instead of an argument.
//!
//! A map nobody checks rots faster than the file it describes: handlers get
//! added without an owner, and rows survive the handlers they name. This gate
//! pins both directions, at **item** granularity rather than line ranges —
//! owners here are conceptual, not positional (a concern may hold more than one
//! region, and `threads` does), and a line-range gate would demand exactly the
//! code movement this row is not.
//!
//! It deliberately checks coverage and existence, not prose. Whether
//! `get_attachment` is really `attachments` rather than `workspace-fs` is a
...
/// `mod` block are that item's business, not the charter's.
...
fn charted_surface() -> Vec<String> {
    ...
    let main = std::fs::read_to_string(dir.join("handlers.rs")).expect("read handlers.rs");
    let mut out = top_level_items(&main);

    let submodules = dir.join("handlers");
    let mut paths: Vec<PathBuf> = std::fs::read_dir(&submodules)
        .expect("read handlers/ submodule directory")
    ...
    for path in paths {
        ...
        for name in top_level_items(&source) {
            out.push(format!("handlers/{file}::{name}"));
        }
    }
    out
}

/// Parse the `## \`handlers.rs\` module-charter map` table into
/// `item -> [sub-owner, ...]`.
///
...
fn parse_charter_table(section: &str) -> BTreeMap<String, Vec<String>> {
    ...
    assert!(
        saw_row,
        "the '## `handlers.rs` module-charter map' section parsed to zero table \
         rows — the table shape changed and this gate silently stopped checking \
         anything"
    );
    assignments
}
...
#[test]
fn every_handler_item_has_exactly_one_sub_owner() {
    let items = charted_surface();
    let assignments = charter_assignments();

    assert!(
        items.len() > 150,
        "expected to walk the whole handler surface; found only {} top-level \
         items — the walk is broken and this gate would pass vacuously",
        items.len()
    );
    assert!(
        items.iter().any(|item| item.starts_with("handlers/")),
        "no submodule item was collected — the `handlers/` walk is broken and \
         the `run-artifact` row would go unchecked"
    );
...
fn the_map_keeps_at_least_the_audited_sub_owner_count() {
    ...
    assert!(
        ...
        "the map has collapsed to {} sub-owner(s); PROPOSAL §6.9.1 asks for at \
         least 11. Merging concerns here does not make the file smaller, it \
         only makes the eventual #5985 split re-litigate the seams: {:?}",
        owners.len(),
        owners
    );
}

/// The `large_file` waiver **stays**. It names a live pending plan (#5985), and
/// a charter map is explicitly not that split.
///
...
fn the_large_file_waiver_survives_the_charter_map() {
    let handlers =
        std::fs::read_to_string(handlers_dir().join("handlers.rs")).expect("read handlers.rs");
    assert!(
        handlers.contains("// arch-exempt: large_file"),
        "the `// arch-exempt: large_file` waiver was removed from handlers.rs. \
         The module-charter map does not discharge it — the waiver names plan \
         #5985 (the WebUI route split), which has not landed. Restore it, or \
         land the split and delete both."
    );
    assert!(
        handlers.contains("plan #5985"),
        "the waiver no longer names plan #5985. `scripts/pre-commit-safety.sh` \
         requires `// arch-exempt: <category>, <reason>, plan #NNNN`, and the \
         plan number is the only thing that makes the waiver revocable."
    );
}
...
fn an_aligned_separator_row_is_not_parsed_as_data() {
    for (label, separator) in [
        ...
        assert_eq!(
            parsed.get("cancel_run").map(Vec::as_slice),
