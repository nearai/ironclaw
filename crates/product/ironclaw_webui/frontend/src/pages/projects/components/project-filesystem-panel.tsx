...
export function ProjectFilesystemPanel({ threadId }) {
  ...
  if (!threadId) {
    return (
      <Panel className="p-4 sm:p-5">
        {header}
        <div className="mt-4 rounded-[16px] border border-dashed border-[var(--v2-panel-border)] px-4 py-8 text-ui leading-6 text-[var(--v2-text-muted)]">
  ...
  return (
    <Panel className="p-4 sm:p-5">
      ...
      <div className="mt-3 flex min-w-0 flex-wrap items-center gap-1.5 font-mono text-ui-sm text-[var(--v2-text-muted)]">
        <button
          type="button"
      ...
      {downloadError &&
      (
        <div className="mt-3 rounded-xl border border-[color-mix(in_srgb,var(--v2-danger-text)_36%,var(--v2-panel-border))] bg-[var(--v2-danger-soft)] px-3 py-2 text-ui-sm text-[var(--v2-danger-text)]">
          {downloadError}
        </div>
      )}
      {listing.error &&
      ...
      <div className="mt-3 space-y-1">
        {listing.isLoading
          ...
          ? entries.map(
              (entry) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-[12px] border border-transparent px-3 py-2 text-left hover:border-[var(--v2-panel-border)] hover:bg-[var(--v2-surface-soft)]"
                >
                  <Icon
                    name={entry.kind === "directory" ? "folder" : "file"}
                    className="h-4 w-4 shrink-0 text-[var(--v2-text-muted)]"
                  />
                  <span className="min-w-0 flex-1 truncate text-ui text-[var(--v2-text-strong)]">{entry.name}</span>
          ...
          : (
              <div className="rounded-[16px] border border-dashed border-[var(--v2-panel-border)] px-4 py-8 text-ui leading-6 text-[var(--v2-text-muted)]">
