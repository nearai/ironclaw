  ...
  return (
    <div className="rounded-[20px] border border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] p-4">
      <div className="mb-3">
        ...
        <div className="mt-1 text-title-sm font-semibold tracking-tight text-[var(--v2-text-strong)]">{widget.manifest?.name || widget.manifest?.id}</div>
      </div>
      {errorName !== undefined
        ? (<p className="rounded-xl border border-[color-mix(in_srgb,var(--v2-danger-text)_36%,var(--v2-panel-border))] bg-[var(--v2-danger-soft)] px-3 py-2 text-ui text-[var(--v2-danger-text)]">{t("projects.widgets.mountFailed", { name: errorName || t("projects.widgets.fallbackSlot") })}</p>)
        : null}
  ...
  return (
    <Panel className="p-4 sm:p-5">
      <div className="mb-4">
        ...
        <h2 className="mt-2 text-title-lg font-semibold tracking-tight text-[var(--v2-text-strong)]">{t("projects.widgets.instrumentation")}</h2>
      </div>
