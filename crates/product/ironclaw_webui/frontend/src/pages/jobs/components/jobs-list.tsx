    ...
    <div className="space-y-5">
      <Panel className="p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--v2-text-muted)]">{t("jobs.list.explorer")}</div>
            <h2 className="mt-2 text-title-lg font-semibold tracking-tight text-[var(--v2-text-strong)]">{t("jobs.list.queueTitle")}</h2>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
          <input
            ...
            className="h-11 rounded-md border border-[var(--v2-panel-border)] bg-[color-mix(in_srgb,var(--v2-canvas-strong)_88%,transparent)] px-3 text-ui text-[var(--v2-text-strong)] outline-none focus:border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))]"
          />
          <select
            value={stateFilter}
            onChange={(event) => onStateFilterChange(event.currentTarget.value)}
            className="v2-select h-11 rounded-md border border-[var(--v2-panel-border)] bg-[color-mix(in_srgb,var(--v2-canvas-strong)_88%,transparent)] px-3 text-ui text-[var(--v2-text-strong)] outline-none focus:border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))]"
          >
          ...
          <article
            ...
            className={[
              ...
              selectedJobId === job.id
                ...
                : "border-[var(--v2-panel-border)] bg-iron-800/60 hover:border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))] hover:bg-iron-800/80",
            ].join(" ")}
          >
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <button onClick={() => onSelectJob(job.id)} className="min-w-0 text-left">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="truncate text-title-sm font-semibold text-[var(--v2-text-strong)]">{job.title || t("jobs.list.untitled")}</h3>
                  <StatusPill tone={statusToneForState(job.state)} label={stateLabel(job.state)} />
                </div>
                <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--v2-text-muted)]">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="ghost" className="h-9 px-3 text-ui-sm" onClick={() => onSelectJob(job.id)}>{t("jobs.action.open")}</Button>
              </div>
            </div>
          </article>
        ))}
