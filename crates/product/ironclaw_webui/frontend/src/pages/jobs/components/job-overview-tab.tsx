...
function MetaItem({ label, value }) {
  return (
    <div className="border-t border-[var(--v2-panel-border)] py-4">
      <div className="font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--v2-text-muted)]">{label}</div>
      <div className="mt-2 text-ui leading-6 text-[var(--v2-text-strong)]">{value || "Not available"}</div>
    </div>
  );
}
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
      <Panel className="p-5 sm:p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--v2-text-muted)]">Execution context</div>
            <h3 className="mt-2 text-title font-semibold text-[var(--v2-text-strong)]">Timing, state, and runtime shape</h3>
          </div>
      ...
      <div className="space-y-5">
        <Panel className="p-5 sm:p-6">
          <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--v2-text-muted)]">Description</div>
          <h3 className="mt-2 text-title font-semibold text-[var(--v2-text-strong)]">Mission brief</h3>
          {job.description
            ? (<MarkdownRenderer content={job.description} className="mt-4 text-ui leading-7 text-[var(--v2-text-strong)]" />)
            : (<p className="mt-4 text-ui leading-6 text-[var(--v2-text-muted)]">This job did not record a long-form description.</p>)}
        </Panel>

        {transitions.length
          ? (
              <Panel className="p-5 sm:p-6">
                <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--v2-text-muted)]">Transitions</div>
                <h3 className="mt-2 text-title font-semibold text-[var(--v2-text-strong)]">State timeline</h3>
                <div className="mt-3">
