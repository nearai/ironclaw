...
export function WorkspaceDirectory({ path, entries, isLoading, filter, onOpen, onNavigate }) {
  ...
  let body;
  if (!visible.length) {
    body = (<div className="px-4 py-10 text-center text-ui text-[var(--v2-text-muted)]">{t("workspace.emptyDir")}</div>);
  } else if (!rows.length) {
    body = (<div className="px-4 py-10 text-center text-ui text-[var(--v2-text-muted)]">{t("workspace.noMatches")}</div>);
  } else {
    body = (
      <div className="divide-y divide-white/[0.06]">
        {rows.map((entry) => (
          <button
            ...
            className="flex w-full items-center gap-3 px-4 py-2.5 text-left text-ui text-[var(--v2-text-strong)] hover:bg-[var(--v2-surface-soft)] hover:text-[var(--v2-text-strong)]"
          >
            <span className={["w-4 text-center text-ui-sm", entry.is_dir ? "text-[var(--v2-accent-text)]" : "text-[var(--v2-text-muted)]"].join(" ")}>
              {entry.is_dir ? "□" : "·"}
            </span>
