...
function FileBody({ path, file }) {
  ...
  if (file.kind === "text") {
    return (
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 sm:px-6 sm:py-4">
        {isMarkdownPath(path)
          ? (<MarkdownRenderer content={file.content} className="max-w-4xl text-ui-lg leading-7" />)
          : (<pre className="overflow-x-auto whitespace-pre-wrap font-mono text-ui leading-6 text-[var(--v2-text-strong)]">{file.content}</pre>)}
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
      <p className="max-w-md text-ui text-[var(--v2-text-muted)]">{t("workspace.binaryPreviewUnavailable")}</p>
    </div>
  );
}
...
export function WorkspaceViewer({ path, file, isLoading, onNavigate }) {
  ...
  return (
    <Panel className="flex min-h-[520px] flex-col overflow-hidden p-0 xl:min-h-0">
      ...
      {parentPath(path) && (
        <div className="border-t border-[var(--v2-panel-border)] px-4 py-3 text-ui-sm text-[var(--v2-text-muted)]">
