...
export function ThreadSidebar({
    ...
    return (
      <div className="flex items-center gap-2">
        <button
          ...
          className="v2-button h-9 shrink-0 rounded-md border border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))] bg-[var(--v2-accent-soft)] px-3 text-ui-sm font-semibold text-[var(--v2-accent-text)] disabled:opacity-50"
        >
        ...
        <select
          ...
          className="v2-select h-9 min-w-0 flex-1 rounded-md border border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] px-3 text-ui text-[var(--v2-text-strong)] outline-none focus:border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))]"
        >
  ...
  return (
    <div
      className="flex h-full flex-col border-r border-[var(--v2-panel-border)] bg-[color-mix(in_srgb,var(--v2-canvas-strong)_88%,transparent)] backdrop-blur-xl"
    >
      <div
      ...
      >
        ...
        <button
          ...
          onClick={onCreate}
          disabled={createDisabled}
          className="v2-button inline-flex h-8 items-center gap-1.5 rounded-md border border-[color-mix(in_srgb,var(--v2-accent)_44%,var(--v2-panel-border))] bg-[var(--v2-accent-soft)] px-2 text-ui-sm font-medium text-[var(--v2-accent-text)] hover:bg-[var(--v2-accent-soft)] disabled:opacity-50"
        >
          {isCreating
            ? t("chat.creating")
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {threads.length === 0 &&
        (<div
          className="mx-2 mt-3 rounded-md border border-dashed border-[var(--v2-panel-border)] px-4 py-7 text-left text-ui-sm leading-5 text-[var(--v2-text-muted)]"
        >
        ...
        {threads.map((thread) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="truncate max-w-[150px] text-ui font-medium text-[var(--v2-text-strong)]">
                  {title}
                </span>
                {thread.state === "Processing" &&
                (<span
                  className="v2-breathing-dot ml-auto h-2 w-2 rounded-full bg-[var(--v2-accent)] text-[var(--v2-accent-on)]"
                />)}
              </div>
              <div
