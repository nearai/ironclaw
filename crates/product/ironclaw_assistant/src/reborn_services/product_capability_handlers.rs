...
impl ProductCapabilityHandler {
    pub(super) fn parse(capability: &CapabilityId) -> Option<Self> {
        match capability.as_str() {
            ...
            LLM_CONFIG_RESET_CAPABILITY_ID => Some(Self::LlmConfigReset),
    ...
    pub(super) const fn success_summary(self) -> &'static str {
        match self {
            ...
            Self::LlmConfigReset => "llm settings reset to defaults",
        ...
        V: RebornViewProvider + Clone + 'static,
    {
        match self {
            ...
            Self::LlmConfigReset => services.invoke_llm_config_reset(caller, input).await,
