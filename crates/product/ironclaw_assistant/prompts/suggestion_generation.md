...
For each suggestion, include one to five unique `sources` translated from the discovered extension or tool metadata that motivated it. Use concise human-readable names such as `Gmail`, `Slack`, `Telegram`, or `Web Search`; never expose internal capability IDs and do not invent sources. Choose one `icon` value from the supplied schema enum; do not invent icon values.
