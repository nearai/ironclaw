...
use chrono::Utc;
use ironclaw_approvals::{ApprovalRequestStorePort, ApprovalStatus};
...
impl ApprovalInteractionReadModel for FakeReadModel {
    ...
    ) -> Result<Vec<ApprovalGateRecord>, ironclaw_assistant::ProductSurfaceFailure> {
        Ok(self
            .gates
            .lock()
            .expect("lock")
    ...
    async fn approval_gate(
        ...
        gate_ref: &TurnGateRef,
    ) -> Result<Option<ApprovalGateRecord>, ironclaw_assistant::ProductSurfaceFailure> {
        Ok(self
            .gates
            .lock()
            .expect("lock")
...
impl ApprovalLeaseTermsProvider for FixedLeaseTermsProvider {
    ...
    ) -> Result<LeaseApproval, ironclaw_assistant::ProductSurfaceFailure> {
    ...
    async fn persistent_approval_allowed(
        &self,
        _gate: &ApprovalGateRecord,
    ) -> Result<(), ironclaw_assistant::ProductSurfaceFailure> {
        Ok(())
    }
}
...
impl ApprovalLeaseTermsProvider for RejectingPersistentLeaseTermsProvider {
    ...
    async fn persistent_approval_allowed(
        ...
        Err(
            ironclaw_assistant::ProductSurfaceFailure::ApprovalInteractionRejected {
...
fn resolver_failure() -> ironclaw_assistant::ProductSurfaceFailure {
