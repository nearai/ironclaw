...
use ironclaw_product_contracts::operator_llm::{
    ActiveModelReader, CodexLoginStart, LEARNING_SETTINGS_SET_CAPABILITY_ID, LearningSnapshot,
    LlmActiveSelection, LlmConfigService, LlmConfigServiceError, LlmConfigSnapshot,
    LlmModelsResult, LlmProbeRequest, LlmProbeResult, LlmProviderView, MemoryWritePolicy,
    NearAiLoginRequest, NearAiLoginStart, NearAiWalletLoginRequest, NearAiWalletLoginResult,
    SetActiveLlmRequest, SetLearningSettingsRequest, SetUserModelPreferenceRequest,
...
impl ApprovalInteractionService for RecordingApprovalInteractionService {
    ...
    ) -> Result<ListPendingApprovalsResponse, ironclaw_assistant::ProductSurfaceFailure> {
    ...
    ) -> Result<ResolveApprovalInteractionResponse, ironclaw_assistant::ProductSurfaceFailure> {
...
impl AuthInteractionService for RecordingAuthInteractionService {
    ...
    ) -> Result<ResolveAuthInteractionResponse, ironclaw_assistant::ProductSurfaceFailure> {
        let run_id = request.run_id_hint.expect("webui passes run_id");
...
impl LifecycleProductService for RecordingLifecycleService {
    async fn execute(
        ...
        action: ironclaw_assistant::LifecycleProductAction,
...
async fn default_invoke_uses_canonical_host_types_and_fails_closed() {
    ...
    let result: Result<Resolution, ProductSurfaceError> = services
        .invoke(
            caller(),
...
fn service_error_taxonomy_serializes_all_stable_wire_names() {
    ...
    let cases = [
        (ProductSurfaceErrorKind::Validation, "validation"),
        (ProductSurfaceErrorKind::Duplicate, "duplicate"),
        (ProductSurfaceErrorKind::Busy, "busy"),
        (
            ProductSurfaceErrorKind::ParticipantDenied,
            "participant_denied",
        ),
        (ProductSurfaceErrorKind::BlockedApproval, "blocked_approval"),
        (
            ProductSurfaceErrorKind::BlockedAuthentication,
            "blocked_authentication",
        ),
        (ProductSurfaceErrorKind::BlockedResource, "blocked_resource"),
        (
        ...
        (ProductSurfaceErrorKind::NotFound, "not_found"),
        (ProductSurfaceErrorKind::Conflict, "conflict"),
        (ProductSurfaceErrorKind::Internal, "internal"),
    ];
...
async fn submitted_replay_with_missing_or_invalid_run_id_maps_to_replay_unavailable() {
    for turn_run_id in [None, Some("not-a-uuid".to_string())] {
        ...
        assert_eq!(err.code, ProductSurfaceErrorCode::Conflict);
        assert_eq!(err.kind, ProductSurfaceErrorKind::ReplayUnavailable);
...
async fn duplicate_submit_rejects_cross_thread_reuse_maps_to_duplicate_kind() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Conflict);
    assert_eq!(err.kind, ProductSurfaceErrorKind::Duplicate);
    assert_eq!(err.status_code, 409);
    assert_eq!(coordinator.submission_count(), 1);
...
async fn delete_thread_rejects_thread_with_active_run() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Conflict);
    assert_eq!(err.kind, ProductSurfaceErrorKind::Busy);
    assert_eq!(err.status_code, 409);
...
async fn validation_errors_are_stable_and_sanitized() {
    ...
    assert_eq!(err.kind, ProductSurfaceErrorKind::Validation);
    assert_eq!(err.status_code, 400);
    assert_eq!(err.field.as_deref(), Some("content"));
    assert_eq!(
        err.validation_code,
        Some(ProductSurfaceValidationCode::MissingField)
    );
...
async fn stream_events_without_projection_stream_maps_to_replay_unavailable_taxonomy() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Unavailable);
    assert_eq!(err.kind, ProductSurfaceErrorKind::ReplayUnavailable);
...
async fn adapter_authentication_maps_to_unauthenticated() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Unauthenticated);
    assert_eq!(err.kind, ProductSurfaceErrorKind::ParticipantDenied);
...
async fn projection_egress_denied_maps_to_blocked_resource_taxonomy() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Forbidden);
    assert_eq!(err.kind, ProductSurfaceErrorKind::BlockedResource);
    assert_eq!(err.status_code, 403);
    let rendered = format!("{err:?} {}", serde_json::to_string(&err).expect("json"));
    assert!(!rendered.contains("secret-token"));
}
...
async fn timeline_backend_failure_maps_to_timeline_unavailable_taxonomy() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Unavailable);
    assert_eq!(err.kind, ProductSurfaceErrorKind::TimelineUnavailable);
    assert_eq!(err.status_code, 503);
    assert!(err.retryable);
    let rendered = format!("{err:?} {}", serde_json::to_string(&err).expect("json"));
    assert!(!rendered.contains("secret-token"));
...
async fn retry_run_maps_not_retryable_to_non_retryable_conflict() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Conflict);
    assert_eq!(err.kind, ProductSurfaceErrorKind::Conflict);
    assert_eq!(err.status_code, 409);
    assert!(!err.retryable);
...
async fn blocked_approval_run_with_stale_gate_ref_returns_conflict() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Conflict);
    assert_eq!(err.kind, ProductSurfaceErrorKind::BlockedApproval);
    assert_eq!(err.status_code, 409);
    assert_eq!(coordinator.resumption_count(), 0);
    assert_eq!(coordinator.cancellation_count(), 0);
...
async fn outbound_unwired_target_listing_fails_closed() {
    ...
    assert_eq!(list_error.code, ProductSurfaceErrorCode::Unavailable);
...
async fn setup_extension_rejects_unknown_secret_name() {
    ...
    assert_setup_validation(err, "secrets", ProductSurfaceValidationCode::InvalidValue);
...
struct SetupRecordingLlmConfigService {
    ...
    set_learning_calls: Mutex<Vec<SetLearningSettingsRequest>>,
...
impl Default for SetupRecordingLlmConfigService {
    fn default() -> Self {
        Self {
            ...
            set_learning_calls: Mutex::new(Vec::new()),
...
impl LlmConfigService for SetupRecordingLlmConfigService {
    ...
    async fn set_learning(
        ...
        self.set_learning_calls.lock().expect("lock").push(request);
        Ok(self.snapshot.lock().expect("lock").clone())
    }
...
impl OperatorConfigAutoApproveInvoker {
    ...
    async fn revoke_tool_policy(
        ...
        match self.persistent_policies.revoke(policy_key).await {
            ...
            Err(error) => Err(ProductSurfaceError::internal_from(error)),
        }
    }
}
...
fn operator_config_entry_value<'a>(
    response: &'a ironclaw_assistant::RebornOperatorConfigListResponse,
    key: &str,
) -> &'a serde_json::Value {
    &response
        .entries
...
async fn query_operator_setup<S: ProductSurface + ?Sized>(
    services: &S,
    caller: ProductSurfaceCaller,
) -> Result<ironclaw_assistant::RebornOperatorSetupResponse, ProductSurfaceError> {
    let page = query_product_surface_page(
        services,
        caller,
        RebornViewQuery {
...
async fn operator_config_reads_and_writes_auto_approve_and_tool_permissions() {
    ...
    assert_eq!(financial_error.kind, ProductSurfaceErrorKind::Validation);
    assert_eq!(
...
async fn operator_config_reports_unknown_keys_distinct_from_invalid_values() {
    ...
    assert_eq!(
        ...
        Some(ProductSurfaceValidationCode::UnknownKey)
    );
...
async fn get_operator_setup_returns_snapshot_from_llm_config() {
    ...
    assert!(response.steps.iter().any(|step| {
        step.name == "profile"
            && step.status == ironclaw_assistant::RebornOperatorSetupStepStatus::Complete
    }));
    assert!(response.steps.iter().any(|step| {
...
async fn get_operator_setup_without_llm_config_returns_service_unavailable() {
    ...
    assert_eq!(err.code, ProductSurfaceErrorCode::Unavailable);
    assert_eq!(err.kind, ProductSurfaceErrorKind::ServiceUnavailable);
    assert_eq!(err.status_code, 503);
}
...
async fn run_operator_setup_rejects_model_without_provider_id() {
    ...
    assert_setup_validation(err, "model", ProductSurfaceValidationCode::InvalidValue);
    assert_eq!(llm_config.snapshot_count(), 0);
}
...
async fn run_operator_setup_rejects_base_url_without_adapter() {
    ...
    assert_setup_validation(err, "base_url", ProductSurfaceValidationCode::InvalidValue);
    assert_eq!(llm_config.snapshot_count(), 0);
    assert_eq!(llm_config.upsert_provider_count(), 0);
    assert_eq!(llm_config.set_active_count(), 0);
}
...
async fn run_operator_setup_rejects_api_key_without_adapter() {
    ...
    assert_setup_validation(err, "api_key", ProductSurfaceValidationCode::InvalidValue);
    assert_eq!(llm_config.snapshot_count(), 0);
    assert_eq!(llm_config.upsert_provider_count(), 0);
    assert_eq!(llm_config.set_active_count(), 0);
}
...
async fn run_operator_setup_ignores_redacted_webui_access_token_sentinel() {
    ...
    assert_eq!(
        webui_step.status,
        ironclaw_assistant::RebornOperatorSetupStepStatus::Complete
    );
    assert_eq!(
        llm_config.set_active_calls.lock().expect("lock").as_slice(),
        [SetupSetActiveCall {
...
async fn run_operator_setup_propagates_llm_config_service_error() {
    ...
    assert_eq!(upsert_err.code, ProductSurfaceErrorCode::Unavailable);
    ...
    assert_eq!(set_active_err.code, ProductSurfaceErrorCode::Unavailable);
    ...
    assert_eq!(snapshot_err.code, ProductSurfaceErrorCode::Unavailable);
