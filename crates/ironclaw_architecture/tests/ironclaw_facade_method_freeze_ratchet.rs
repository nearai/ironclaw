...
//! §10 of `docs/ironclaw/2026-07-17-architecture-simplification-dto-dyn-local.md`).
//!
...
const FACADE_SOURCE: &str = "crates/ironclaw_product_workflow/src/ironclaw_services.rs";
const FACADE_TRAIT: &str = "ProductSurface";
const PRODUCT_SURFACE_TRAIT: &str = "ProductSurface";
const RETIRED_PROTO_FACADE_TRAIT: &str = "IronClawServicesApi";
...
#[test]
fn ironclaw_facade_method_allowlist_is_frozen_and_only_shrinks() {
...
#[test]
fn ironclaw_services_api_proto_facade_stays_retired() {
    let source_path = workspace_root().join(FACADE_SOURCE);
    let source = std::fs::read_to_string(&source_path)
        .unwrap_or_else(|e| panic!("failed to read facade source {source_path:?}: {e}"));
    let stripped = strip_comments_and_strings(&source);
