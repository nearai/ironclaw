...
//! (`ironclaw_localdev_typename_ratchet`, `ironclaw_deployment_mode_typename_ratchet`).
...
//!
//! `IronClawCompositionProfile` *is* such an enum. It survives as the CLI/env
...
//! that name a `IronClawCompositionProfile` variant. Set membership, not a count,
...
/// `IronClawCompositionProfile` variant, each with the reason it is still here.
///
...
const ALLOWLIST: &[(&str, &str)] = &[
    ...
    (
        ...
        "Readiness diagnostics carry a profile as an operator-facing **label** \
         on the wire (`IronClawReadinessDiagnostic::profile`), not a branch. \
...
fn collect(dir: &Path, root: &Path, found: &mut BTreeSet<String>) {
    ...
    for entry in entries {
        ...
        if !strip_comments_and_strings(&source).contains("IronClawCompositionProfile::") {
            continue;
        }
        let relative = path
            .strip_prefix(root)
...
fn deployment_mode_branching_allowlist_is_frozen_and_only_shrinks() {
    ...
    assert!(
        ...
        "new deployment-mode branching in composition: {new_debt:?}\n\
         A `IronClawCompositionProfile` variant in a production file means code is \
...
fn scanner_strips_comments_and_strings() {
    ...
    let source = r#"
        // IronClawCompositionProfile::LocalDev in a line comment
        /* IronClawCompositionProfile::Production in a block comment */
        let label = "IronClawCompositionProfile::Disabled";
    "#;
    let stripped = strip_comments_and_strings(source);
    assert!(
        !stripped.contains("IronClawCompositionProfile::"),
        "stripped source still contains a variant path: {stripped}"
    );

    let real = "match profile { IronClawCompositionProfile::LocalDev => 1, _ => 0 }";
    assert!(
        strip_comments_and_strings(real).contains("IronClawCompositionProfile::"),
    ...
    let with_char_literal = r#"
        let quote = '"';
        match profile { IronClawCompositionProfile::LocalDev => 1, _ => 0 }
    "#;
    assert!(
        strip_comments_and_strings(with_char_literal).contains("IronClawCompositionProfile::"),
