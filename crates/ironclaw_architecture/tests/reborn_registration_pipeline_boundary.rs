...
const OWNED_SCOPES: &[(&str, &str)] = &[
    ...
    ("ironclaw_extensions", "src/hosted_mcp_"),
];
...
fn ownership_follows_the_crate_inventory_at_any_depth() {
    for family in ["", "substrates/"] {
        ...
        let extensions = format!("crates/{family}ironclaw_extensions");
        ...
        assert!(
            !is_owned(
                &format!("crates/{family}ironclaw_product/src/lifecycle.rs"),
                &scopes.owned
            ),
        ...
        assert!(
            is_excluded(
                &format!("crates/{family}ironclaw_architecture/tests/gate.rs"),
                &scopes.excluded
            ),
...
fn workspace_root_search_survives_a_family_move() {
    ...
    let nested = "crates/substrates/ironclaw_architecture";
...
fn fail_closed_when_the_scan_visits_nothing() {
    ...
    write_crate(root, "crates/ironclaw_product", &["src/lifecycle.rs"]);
...
fn fail_closed_when_an_owned_scope_matches_no_files() {
    ...
    std::fs::remove_file(root.join("crates/ironclaw_extensions/src/hosted_mcp_discovery.rs"))
...
fn fail_closed_when_a_named_crate_cannot_be_resolved() {
    ...
    let root = temp.path();
    build_synthetic_workspace(root, "");
    std::fs::remove_dir_all(root.join("crates/ironclaw_extensions")).expect("remove a named crate");

    let inventory = CrateInventory::discover(root).expect("discovery");
    let error = resolve_scopes(&inventory).expect_err("an absent named crate must fail");
    assert!(
        error.contains("ironclaw_extensions"),
...
fn build_synthetic_workspace(root: &Path, family: &str) {
    ...
    write_crate(
        root,
        &format!("crates/{family}ironclaw_extensions"),
        &["src/hosted_mcp_discovery.rs"],
    );
    write_crate(
        root,
        &format!("crates/{family}ironclaw_architecture"),
        &["src/lib.rs"],
    );
}
