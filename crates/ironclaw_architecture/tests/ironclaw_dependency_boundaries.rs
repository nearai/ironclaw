...
#[test]
fn ironclaw_boundary_rules_active_crates_are_workspace_members() {
    ...
    for rule in boundary_rules() {
        let manifest = if rule.crate_name == "ironclaw" {
            root.join("crates/ironclaw_cli/Cargo.toml")
        } else {
...
#[test]
fn ironclaw_workspace_crates_declare_layers_and_follow_layer_matrix() {
    let metadata = cargo_metadata();
    let packages = metadata["packages"]
        .as_array()
        .expect("cargo metadata must include packages");
...
#[test]
fn ironclaw_virtual_roots_match_storage_placement_contract() {
    let root = workspace_root();
    ...
    let storage_contract =
        std::fs::read_to_string(root.join("docs/ironclaw/contracts/storage-placement.md"))
            .expect("storage placement contract must be readable");
    let filesystem_contract =
        std::fs::read_to_string(root.join("docs/ironclaw/contracts/filesystem.md"))
...
#[test]
fn ironclaw_crate_dependency_boundaries_hold() {
    let metadata = cargo_metadata();
    let packages = metadata["packages"]
        .as_array()
        .expect("cargo metadata must include packages");
    ...
    // Canonical IronClaw identity layer: it maps external identities to a stable
    ...
    // `ironclaw_composition` / `ironclaw_product_workflow`) or onto the v1
    // legacy enclave — the "never reach upstream" property the crate guarantees.
    let ironclaw_identity_allowed = [
        "ironclaw_identity",
        "ironclaw_host_api",
        "ironclaw_filesystem",
    ];
    assert_no_normal_workspace_deps(
        ...
        workspace_ironclaw_crates(&dependencies)
            .into_iter()
            .filter(|name| !ironclaw_identity_allowed.contains(name))
            .collect::<Vec<_>>(),
    );
...
#[test]
fn ironclaw_cli_binary_crate_stays_separate_from_v1_root() {
    let metadata = cargo_metadata();
    let packages = metadata["packages"]
        .as_array()
        .expect("cargo metadata must include packages");
    ...
    let root = workspace_root();
    let manifest_path = root.join("crates/ironclaw_cli/Cargo.toml");
    assert!(
        manifest_path.exists(),
        "IronClaw should ship as a separate binary crate at {}",
        manifest_path.display()
    );

    let manifest =
        std::fs::read_to_string(&manifest_path).expect("IronClaw CLI manifest must be readable");
    assert!(
        manifest.contains("name = \"ironclaw\""),
        "IronClaw CLI crate package name should be ironclaw"
    );
    assert!(
        manifest.contains("[[bin]]") && manifest.contains("name = \"ironclaw\""),
        "IronClaw CLI crate must declare the canonical ironclaw binary explicitly"
    );
    ...
    for path in command_module_paths {
        assert!(
            ...
            "IronClaw CLI commands should use an agent-friendly one-command-per-file layout; missing {path}"
        );
    }

    let agent_contract = std::fs::read_to_string(root.join("crates/ironclaw_cli/AGENTS.md"))
        .expect("IronClaw CLI crate-local AGENTS.md must be readable");
    for required_phrase in [
        ...
        assert!(
            ...
            "IronClaw CLI AGENTS.md should document `{required_phrase}` for future command agents"
        );
    }
    ...
    assert_workspace_deps_exactly(
        ...
        "ironclaw should enter IronClaw through ironclaw_composition (assembled-runtime and provider-admin facade), ironclaw_config (boot-config contract), ironclaw_traces (contributor-side TraceCommons client extracted from the legacy monolith), and ironclaw_webui (host-owned WebUI serve lifecycle) — plus ironclaw_extension_host (the NativeExtensionFactory contract) and concrete extension crates for the binary-assembled native factory registry (DEL-7: only the binary and tests may link concrete extension crates). Adding any other workspace crate here re-opens speculative public API access to internal IronClaw types.",
    );
    assert_workspace_deps_exactly(
    ...
    let runtime_dir = root.join("crates/ironclaw_cli/src/runtime");
    let mut cli_runtime_source = String::new();
    collect_runtime_rs(&runtime_dir, &mut cli_runtime_source);
    assert!(
        cli_runtime_source.contains("build_ironclaw_runtime"),
        "IronClaw CLI should enter the assembled runtime through ironclaw_composition::build_ironclaw_runtime"
    );
    for forbidden in [
        ...
        assert!(
            ...
            "IronClaw CLI runtime/ must not wire lower-level IronClaw runtime pieces directly via `{forbidden}`; keep REPL as a UX shell over ironclaw_composition."
        );
    }
}

#[test]
fn ironclaw_host_runtime_services_do_not_expose_lower_substrate_handles() {
    let root = workspace_root();
    ...
    let host_runtime_contract =
        std::fs::read_to_string(root.join("docs/ironclaw/contracts/host-runtime.md"))
    ...
    for export in forbidden_lib_exports {
        assert!(
            !lib.contains(export),
            "ironclaw_host_runtime must not re-export lower substrate handle `{export}`; upper IronClaw code should enter through HostRuntimeServices::host_runtime / Arc<dyn HostRuntime>"
        );
    }
    ...
    for export in forbidden_obligation_exports {
        assert!(
            ...
            "ironclaw_host_runtime must not re-export lower substrate handoff store `{export}`; upper IronClaw code should enter through HostRuntimeServices::host_runtime / Arc<dyn HostRuntime>"
        );
    }
    ...
    for required_phrase in [
        ...
        "upper IronClaw crates must not use them",
    ] {
        assert!(
            host_runtime_contract.contains(required_phrase),
            "host-runtime contract should document `{required_phrase}` so raw handoff store seams are not mistaken for upper IronClaw APIs"
        );
    }
...
#[test]
fn ironclaw_turns_public_surface_keeps_runner_api_explicit() {
    let root = workspace_root();
...
#[test]
fn ironclaw_runner_llm_wiring_is_isolated() {
    let root = workspace_root();

    let ironclaw_gateway = root.join("crates/ironclaw_runner/src/model_gateway.rs");
    assert!(
        ironclaw_gateway.exists(),
        "expected IronClaw LLM gateway wiring at {}",
        ironclaw_gateway.display()
    );
    let ironclaw_gateway_source = std::fs::read_to_string(&ironclaw_gateway)
        .expect("IronClaw model gateway source must be readable");
    assert!(
        ironclaw_gateway_source.contains("LlmProviderModelGateway"),
        "IronClaw LLM gateway wiring should expose LlmProviderModelGateway from crates/ironclaw_runner"
    );

    // IronClaw crates may reuse the extracted LLM crate, but never on its default
    // terms: `ironclaw_llm`'s defaults pull in extra provider/backend features
    // (e.g. `bedrock`) IronClaw doesn't need. `default-features = false` on every
    // edge is the durable invariant, keeping the IronClaw stack's dependency
    ...
    for manifest_path in [
        ...
        assert!(
            ...
            "{manifest_path} must depend on `ironclaw_llm` with `default-features = false`, \
             so the IronClaw stack never enables the root app's default postgres/libsql/tui \
             feature set: {llm_dep}"
        );
    }
}
...
fn provider_tool_names_stay_at_model_protocol_boundaries() {
    ...
    let allowed = BTreeSet::from([
        ...
        "crates/ironclaw_composition/src/runtime/local_dev/external_tool_capability.rs",
        "crates/ironclaw_composition/src/runtime/local_dev/synthetic_capability.rs",
        "crates/ironclaw_composition/src/observability/trace_capture.rs",
    ]);
    let violations = uses
        .into_iter()
...
/// crate outside the IronClaw family ever named any of those items** and that
...
#[test]
fn ironclaw_internal_crate_keeps_directory_of_modules_lib_rs() {
    let root = workspace_root();
    ...
    for forbidden in forbidden_reexports {
        assert!(
            ...
            "ironclaw_runner/src/lib.rs must not re-export internal items via `{forbidden}`. \
             ...
             See `ironclaw_internal_crate_keeps_directory_of_modules_lib_rs` for context."
        );
    }
    ...
    // forbid).
    let composition_runtime = root.join("crates/ironclaw_composition/src/runtime.rs");
    let composition_local_dev_runtime =
        root.join("crates/ironclaw_composition/src/runtime/local_dev.rs");
    assert!(
        composition_runtime.exists(),
        "expected IronClaw runtime assembly at {}",
...
fn composition_runtime_has_no_slack_output_policy() {
    ...
    assert!(
        violations.is_empty(),
        "IronClaw composition runtime must remain integration-neutral and must not \
         intercept model output with Slack-specific policy. Violations:\n{}",
        violations.join("\n")
    );

    let slack_policy_module =
        root.join("crates/ironclaw_composition/src/runtime/slack_output_hygiene.rs");
    assert!(
        !slack_policy_module.exists(),
        "IronClaw composition must not own the Slack-specific output policy module at {}",
        slack_policy_module.display()
    );
}

fn production_composition_sources(root: &std::path::Path) -> Vec<(PathBuf, String)> {
    let composition_src = root.join("crates/ironclaw_composition/src");
...
/// standalone `ironclaw` binary.
///
/// Three properties:
///
/// 1. `ironclaw_config` continues to expose the boot-time parser
///    (`IronClawConfigFile`) and the file-path accessors
///    (`IronClawHome::config_file_path` / `providers_file_path`). These are
...
///
/// 3. `IronClawConfigFile` rejects inline secret material at parse time.
///    The unit test in `secrets_guard` covers the patterns; this
///    boundary test asserts that the rejection path is *wired through*
///    `IronClawConfigFile::validate` (file-level grep). A regression
///    that bypasses the guard for the boot file fails here loudly
///    rather than silently round-tripping a secret through git.
#[test]
fn ironclaw_boot_config_file_layout_is_pinned() {
    let root = workspace_root();

    let config_lib = std::fs::read_to_string(root.join("crates/ironclaw_config/src/lib.rs"))
        .expect("IronClaw config lib.rs must be readable");
    for required_export in [
        "pub use config_file::",
        "IronClawConfigFile",
        "IRONCLAW_CONFIG_API_VERSION",
        "InlineSecretError",
    ] {
        assert!(
            config_lib.contains(required_export),
            "ironclaw_config/src/lib.rs must export `{required_export}`; \
             see ironclaw_boot_config_file_layout_is_pinned for context"
        );
    }

    let home_src = std::fs::read_to_string(root.join("crates/ironclaw_config/src/home.rs"))
        .expect("IronClaw config home.rs must be readable");
    for required_method in ["pub fn config_file_path", "pub fn providers_file_path"] {
        assert!(
            home_src.contains(required_method),
            "IronClawHome must expose `{required_method}` so the CLI / composition can locate \
             the boot files without hardcoding paths; see \
             ironclaw_boot_config_file_layout_is_pinned"
        );
    }
    ...
    let config_file_src =
        std::fs::read_to_string(root.join("crates/ironclaw_config/src/config_file.rs"))
            .expect("IronClaw config_file.rs must be readable");
    assert!(
        config_file_src.contains("reject_inline_secret"),
        "IronClawConfigFile::validate must call `reject_inline_secret` on operator-pasteable \
         fields. See `docs/ironclaw/contracts/secrets.md` and epic #3036's `Pitfalls & \
    ...
    // composition without forcing `ironclaw_config` to depend
    // on `ironclaw_llm` (which would violate _config's standalone
    // boundary). The composition crate is the legitimate consumer.
    let llm_catalog = root.join("crates/ironclaw_composition/src/llm_admin/llm_catalog.rs");
    assert!(
        llm_catalog.exists(),
        "composition must expose a catalog resolver at {} so the CLI can stitch \
         IronClawConfigFile + providers.json into a IronClawLlmConfig without itself \
    ...
    for required in [
        ...
        assert!(
            ...
            "composition llm_catalog must expose `{required}` so the resolver path is \
             stable; see ironclaw_boot_config_file_layout_is_pinned"
        );
    }

    // `ironclaw_llm` must expose the path-overridable loader so the
    // catalog file location is selectable per-deployment (the
    // standalone IronClaw binary points at $IRONCLAW_REBORN_HOME/providers.json,
    ...
    assert!(
        ...
        "ironclaw_llm::ProviderRegistry must expose `load_from_path` so callers can \
         ...
         and the IronClaw standalone needs its own home."
    );
}

#[test]
fn ironclaw_turns_public_surface_uses_turn_ids_not_runtime_or_process_ids() {
    let root = workspace_root();
...
#[test]
fn ironclaw_runtime_http_egress_has_single_network_boundary() {
    let forbidden = [
    ...
    assert!(
        violations.is_empty(),
        "IronClaw runtime HTTP must use the shared host egress service and ironclaw_network only:\n{}",
        violations.join("\n")
    );
}

#[test]
fn ironclaw_product_api_crates_do_not_bind_http_ingress() {
    let forbidden = [
        ForbiddenUse {
            pattern: "tokio::net::TcpListener::bind",
            reason: "IronClaw product/API crates must expose route descriptors, not bind listeners",
            exempt: None,
        },
        ForbiddenUse {
        ...
        ForbiddenUse {
            pattern: "axum::serve",
            reason: "IronClaw product/API crates must not own server lifecycle",
            exempt: None,
        },
        ForbiddenUse {
    ...
    let root = workspace_root();
    let ironclaw_product_api_src_roots = [
    ...
    let mut violations = Vec::new();
    for relative_root in ironclaw_product_api_src_roots {
        let dir = root.join(relative_root);
        if !dir.exists() {
            continue;
        }
    ...
    assert!(
        violations.is_empty(),
        "IronClaw HTTP ingress must be host-owned; product/API crates may expose descriptors or route fragments but must not bind/serve listeners:\n{}",
        violations.join("\n")
    );
}

#[test]
fn ironclaw_openai_compat_routes_do_not_depend_on_v1_gateway_or_legacy_streams() {
    let forbidden = [
        ForbiddenUse {
            pattern: "src/channels/web",
            reason: "OpenAI-compatible IronClaw routes must not route through v1 gateway handlers",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "crate::channels::web",
            reason: "OpenAI-compatible IronClaw routes must not import v1 gateway modules",
            exempt: None,
        },
        ForbiddenUse {
        ...
        ForbiddenUse {
            ...
            reason: "OpenAI-compatible IronClaw routes must not depend on v1 gateway state",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "SseManager",
            reason: "OpenAI-compatible IronClaw streaming must use projection-stream ports, not raw legacy SSE streams",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "AppEvent",
            reason: "OpenAI-compatible IronClaw streaming must translate ProductProjectionItem state, not raw legacy AppEvent streams",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "IncomingMessage",
            reason: "OpenAI-compatible IronClaw routes must enter through ProductSurface, not legacy channel ingress",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "get_or_create_assistant_conversation",
            reason: "OpenAI-compatible IronClaw retrieve/cancel must use opaque refs and projection readers, not legacy conversation reconstruction",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "ConversationManager",
            reason: "OpenAI-compatible IronClaw routes must not reconstruct legacy conversations directly",
            exempt: None,
        },
    ];

    let root = workspace_root();
    let compat_src = root.join("crates/ironclaw_openai_compat/src");
    let mut violations = Vec::new();
    collect_forbidden_uses(&compat_src, &root, &forbidden, &mut violations);

    assert!(
        violations.is_empty(),
        "IronClaw OpenAI-compatible routes must stay ProductSurface/projection-port backed and independent of v1 gateway handlers, legacy SSE/AppEvent streams, and legacy conversation reconstruction:\n{}",
        violations.join("\n")
    );
}

#[test]
fn ironclaw_product_auth_contract_stays_ironclaw_native() {
    let forbidden = [
        ForbiddenUse {
            pattern: "ironclaw::",
            reason: "IronClaw product auth must not depend on the v1 root crate",
            exempt: Some(is_ironclaw_tracing_target_line),
        },
        ForbiddenUse {
            pattern: "src/extensions",
            reason: "v1 extension paths are inventory only, not IronClaw auth implementation",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "src/channels/web",
            reason: "v1 web routes are inventory only, not IronClaw auth implementation",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "ExtensionManager",
            reason: "IronClaw product auth must not call through the v1 extension manager",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "PendingOAuth",
            reason: "IronClaw product auth must not reuse v1 pending OAuth maps",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "PendingGate",
            reason: "IronClaw product auth must not reuse v1 pending gate maps",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "SecretsStore",
            reason: "IronClaw product auth must use opaque handles, not raw v1 secrets storage",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "get_decrypted",
            reason: "IronClaw product auth must not retrieve raw secret material directly",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "auth-token",
            reason: "IronClaw manual-token setup must not fall back to v1 chat token route names",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "auth_token",
            reason: "IronClaw manual-token setup must not fall back to v1 chat token command paths",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "IncomingMessage",
            reason: "IronClaw product auth must not capture manual tokens through chat transcripts",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "ChatMessage",
        ...
        ForbiddenUse {
            pattern: "secret_name",
            reason: "IronClaw product auth must use scoped credential accounts and opaque handles, not raw v1 secret names",
            exempt: None,
        },
        ForbiddenUse {
            pattern: "SecretName",
        ...
        ForbiddenUse {
            pattern: "reqwest",
            reason: "IronClaw product auth must not own outbound HTTP transport",
            exempt: None,
        },
        ForbiddenUse {
    ...
    assert!(
        ...
        "ironclaw_auth must not depend on reqwest directly; provider transport belongs behind IronClaw-native composition"
    );

    let auth_src = root.join("crates/ironclaw_auth/src");
    assert!(
        auth_src.exists(),
        "IronClaw product auth contract crate must have a src directory at {}",
        auth_src.display()
    );

    let mut violations = Vec::new();
    collect_forbidden_uses(&auth_src, &root, &forbidden, &mut violations);
    collect_forbidden_ironclaw_auth_file_uses(
        &root.join("crates/ironclaw_composition/src/product_auth/api/auth.rs"),
        &root,
        &forbidden,
        &mut violations,
    );
    collect_forbidden_ironclaw_auth_path_uses(
        &root.join("crates/ironclaw_composition/src/product_auth/serve"),
        &root.join("crates/ironclaw_composition/src/product_auth/serve.rs"),
        &root,
        &forbidden,
        &mut violations,
    );
    ...
    assert!(
        violations.is_empty(),
        "IronClaw product auth can be behavior-compatible with v1, but implementation and composition code paths must not mingle with v1 routes, v1 extension/secrets managers, raw provider transport, or raw secret records:\n{}",
        violations.join("\n")
    );
}
...
fn boundary_rules() -> Vec<BoundaryRule> {
    vec![
        ...
        BoundaryRule {
            // Product auth is a IronClaw contract/facade vocabulary plus the
            // recipe-driven auth engine (extension-runtime workstream D). The
            // engine owns token secret storage, so the IronClaw-native
        ...
        BoundaryRule {
            // OpenAI-compatible route surface is a IronClaw product/API facade.
        ...
        BoundaryRule {
            ...
            // WASM/channel crates would bypass the IronClaw registry boundary.
        ...
        BoundaryRule {
            ...
            // internal IronClaw types (turn coordinator, session thread service,
        ...
        BoundaryRule {
            ...
            // the binary path. Reaches the rest of IronClaw through
            // ironclaw_composition's facade (Router + WebuiAuthenticator
...
fn layer_allows_dependency(crate_layer: &str, dependency_layer: &str) -> bool {
    match crate_layer {
        ...
        // The v1 package/crates may still depend on IronClaw while parity work
        // is in flight, but IronClaw layers are intentionally not allowed to
...
fn assert_no_normal_workspace_deps<'a>(
    ...
    let Some(actual) = dependencies.get(crate_name) else {
        // The landing plan introduces IronClaw crates in grouped PRs. Boundary
        ...
        // `ironclaw_boundary_rules_active_crates_are_workspace_members` covers
...
/// `ironclaw_cli_binary_crate_stays_separate_from_v1_root` to scan the
...
fn collect_runtime_rs(dir: &std::path::Path, out: &mut String) {
    for entry in std::fs::read_dir(dir).unwrap_or_else(|err| {
        panic!(
            "IronClaw CLI runtime directory must be readable at {}: {err}",
            dir.display()
        )
    }) {
        ...
        let content = std::fs::read_to_string(&path).unwrap_or_else(|err| {
            panic!(
                "IronClaw CLI runtime file {} unreadable: {err}",
                path.display()
            )
        });
...
fn collect_forbidden_ironclaw_auth_path_uses(
    ...
    collect_forbidden_ironclaw_auth_file_uses(legacy_file, root, forbidden, violations);
}

fn collect_forbidden_ironclaw_auth_file_uses(
    path: &std::path::Path,
    root: &std::path::Path,
    forbidden: &[ForbiddenUse],
    violations: &mut Vec<String>,
    ...
    let message = format!(
        "failed to read IronClaw product-auth boundary file {}",
        path.display()
    );
...
fn is_ironclaw_tracing_target_line(line: &str) -> bool {
    line.contains("target: \"ironclaw::") || line.contains("target = \"ironclaw::")
}

#[test]
fn collect_forbidden_ironclaw_auth_file_uses_detects_violation() {
    let root = std::env::temp_dir().join(format!(
        "ironclaw-auth-boundary-test-{}",
        std::process::id()
    ));
...
#[test]
fn collect_forbidden_ironclaw_auth_file_uses_allows_ironclaw_tracing_targets() {
    let root = std::env::temp_dir().join(format!(
        "ironclaw-auth-boundary-tracing-test-{}",
        std::process::id()
    ));
    let src = root.join("crates/ironclaw_composition/src");
    std::fs::create_dir_all(&src).expect("test source directory must be created");
    ...
    std::fs::write(
        &auth_rs,
        "fn allowed() { tracing::warn!(target: \"ironclaw::product_auth::oauth\"); }\n",
    )
    .expect("test auth.rs must be written");
    ...
    assert!(
        violations.is_empty(),
        "IronClaw tracing targets are log namespaces, not v1 root crate references: {:?}",
        violations
    );
}

#[test]
fn collect_forbidden_uses_allows_ironclaw_tracing_targets() {
    let root = std::env::temp_dir().join(format!(
        "ironclaw-auth-boundary-dir-tracing-test-{}",
        std::process::id()
    ));
    let src = root.join("crates/ironclaw_composition/src/product_auth_serve");
    std::fs::create_dir_all(&src).expect("test source directory must be created");
    ...
    assert!(
        violations.is_empty(),
        "Directory scanner should treat IronClaw tracing targets as log namespaces: {:?}",
        violations
    );
}
