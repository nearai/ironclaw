...
//! companion to [`ironclaw_localdev_typename_ratchet`] (§4.4 / §10 of
...
//! sibling `ironclaw_localdev_typename_ratchet` owns the `LocalDev*` shadow-runtime
...
//!   config value (the `TriggerFireAccessPolicy` on `IronClawRuntimeInput`,
//!   backed by config + the identity directory). The `IronClawLocal*`
...
/// mid-names like `IronClawLocalRuntimeProfileOptions` are inventoried too.
...
const FROZEN_OTHER_MODE_TYPES: &[&str] = &[
    ...
    //     `TriggerFireAccessPolicy` on `IronClawRuntimeInput`, backed by config
    ...
    "HookLocalId",
    //   IronClawLocal* composition family — local-dev-as-type mode names in the
    //     composition surface; shrinks with Slice B (deployment mode becomes a
    //     `DeploymentConfig` value):
    "IronClawLocalExtensionManagementPort",
    "IronClawLocalLifecycleFacade",
    "IronClawLocalRuntimeIdentity",
    "IronClawLocalServiceLifecycle",
    "IronClawLocalSkillManagementError",
...
#[test]
fn ironclaw_other_mode_typename_allowlist_is_frozen() {
    ...
    collect_type_defs(
        ...
        &is_other_mode_name,
        &[
            "ironclaw_localdev_typename_ratchet.rs",
            "ironclaw_deployment_mode_typename_ratchet.rs",
        ],
        &mut found,
    );
...
fn other_mode_predicate_self_test() {
    let sample = r##"
        ...
        pub struct IronClawLocalRuntimeServices;   // mid-name Local word -> flagged
