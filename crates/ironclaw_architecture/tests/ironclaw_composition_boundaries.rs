...
fn composition_public_api_is_facade_shaped() {
    let lib =
        std::fs::read_to_string(workspace_root().join("crates/ironclaw_composition/src/lib.rs"))
            .expect("composition lib readable");
    let input =
        std::fs::read_to_string(workspace_root().join("crates/ironclaw_composition/src/input.rs"))
            .expect("composition input readable");
    let factory = std::fs::read_to_string(
        workspace_root().join("crates/ironclaw_composition/src/factory.rs"),
    )
    .expect("composition factory readable");
    let public_surface = format!("{lib}\n{input}\n{factory}");

    assert!(
        !lib.contains("pub use input::IronClawStorageInput"),
        "composition facade API must not re-export raw storage input types"
    );
    assert!(
        !input.contains("pub enum IronClawStorageInput"),
        "IronClawStorageInput must stay crate-private"
    );
    assert!(
...
#[test]
fn ironclaw_binary_main_is_thin_bootstrap() {
    let root = workspace_root();
    let ironclaw_main = std::fs::read_to_string(root.join("crates/ironclaw_cli/src/main.rs"))
        .expect("reborn cli main.rs readable");

    assert!(
        ironclaw_main.contains("cli::run()"),
        "ironclaw main.rs should delegate to the clap command root"
    );
    for forbidden in [
        "build_ironclaw_runtime",
        ...
        assert!(
            !ironclaw_main.contains(forbidden),
            "ironclaw main.rs must stay a thin bootstrap over IronClaw-owned command \
...
fn composition_crate_installs_installed_tier_only_through_registrar() {
    let crate_src = workspace_root().join("crates/ironclaw_composition/src");
...
fn composition_src_path() -> PathBuf {
    workspace_root().join("crates/ironclaw_composition/src")
}
