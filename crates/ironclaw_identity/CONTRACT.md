...
The canonical IronClaw identity layer. It maps every external identity — WebUI
OAuth logins (`google`, `github`, …) and external channel/product actors
(`telegram`, `slack`, triggers, …) — to a stable IronClaw [`UserId`] **before**
...
separate users table elsewhere in the IronClaw stack. Any future enumeration or
...
legacy enclave. This is machine-enforced: `ironclaw_crate_dependency_boundaries_hold`
in `crates/ironclaw_architecture/tests/ironclaw_dependency_boundaries.rs` allows
exactly those two edges. The only external consumer is
`ironclaw_composition`, which re-exports a curated subset via its facade.
...
## Resolver surface (`IronClawIdentityResolver`)
...
## User directory surface (`IronClawUserDirectory`)

A **separate** trait from `IronClawIdentityResolver`, implemented by the same
`FilesystemIronClawIdentityStore`, for the operator/admin surface that enumerates
...
- **`IronClawIdentityError` carries storage paths.** `Backend(_)` wraps
