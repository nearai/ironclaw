//! Behavioral matrix for [`FilesystemIronClawIdentityStore`](super::FilesystemIronClawIdentityStore).
//!
...
use super::*;
use crate::{
    ExternalSubjectId, IronClawUserDirectory, IronClawUserProfileUpdate, IronClawUserRole,
    IronClawUserStatus, ProviderInstanceId, ProviderKind, SurfaceKind,
};
use ironclaw_filesystem::InMemoryBackend;
use ironclaw_host_api::{MountAlias, MountGrant, MountPermissions, MountView, VirtualPath};

fn store_on(root: Arc<InMemoryBackend>) -> FilesystemIronClawIdentityStore<InMemoryBackend> {
    ...
    FilesystemIronClawIdentityStore::new(
        scoped,
...
fn store() -> FilesystemIronClawIdentityStore<InMemoryBackend> {
...
fn store_pair() -> (
    FilesystemIronClawIdentityStore<InMemoryBackend>,
    FilesystemIronClawIdentityStore<InMemoryBackend>,
) {
...
async fn different_provider_instance_does_not_collide() {
    ...
    store
        .bind(
            ...
            &UserId::new("ironclaw-user-1").unwrap(),
        )
        .await
        .expect("bind");
...
async fn resolve_or_create_rejects_channel_actor() {
    ...
    let result = store
        ...
        .await;
    assert!(
        matches!(result, Err(IronClawIdentityError::ChannelActorNotMintable)),
...
async fn bind_then_lookup_returns_bound_user() {
    let store = store();
    let t = tenant("t");
    let user = UserId::new("ironclaw-user-7").expect("user");
    store
...
async fn corrupt_persisted_user_id_surfaces_invalid_user_id() {
    ...
    let via_lookup = store
        ...
        .await;
    assert!(
        matches!(via_lookup, Err(IronClawIdentityError::InvalidUserId(_))),
    ...
    let via_resolve = store
        ...
        .await;
    assert!(
        matches!(via_resolve, Err(IronClawIdentityError::InvalidUserId(_))),
...
async fn corrupt_json_body_surfaces_backend_error() {
    ...
    let result = store
        ...
        .await;
    assert!(
        matches!(result, Err(IronClawIdentityError::Backend(_))),
        "a corrupt JSON body must surface a Backend error, got {result:?}"
    );
}

// ---------------------------------------------------------------------------
// IronClawUserDirectory (admin surface)
// ---------------------------------------------------------------------------
...
async fn legacy_stored_user_json_deserializes_with_defaults() {
    ...
    let user = store
        ...
        .expect("get")
        .expect("present");
    assert_eq!(user.status, IronClawUserStatus::Active);
    assert_eq!(user.role, IronClawUserRole::Member);
...
async fn create_then_list_and_get_roundtrip() {
    ...
    let created = store
        .create_user(
            ...
            IronClawUserRole::Member,
            &admin,
        )
        .await
        .expect("create");
    assert_eq!(created.status, IronClawUserStatus::Active);
    assert_eq!(created.role, IronClawUserRole::Member);
    assert_eq!(
...
async fn list_users_paginates_by_user_id_cursor() {
    ...
    for _ in 0..5 {
        store
            .create_user(&t, None, None, IronClawUserRole::Member, &by)
            .await
            .expect("create");
    }
...
async fn update_status_role_and_profile() {
    ...
    let suspended = store
        .update_status(&user.user_id, IronClawUserStatus::Suspended)
        .await
        .expect("suspend");
    assert_eq!(suspended.status, IronClawUserStatus::Suspended);
    assert!(
    ...
    let promoted = store
        .update_role(&user.user_id, IronClawUserRole::Admin)
        .await
        .expect("promote");
    assert_eq!(promoted.role, IronClawUserRole::Admin);
    ...
    assert_eq!(profiled.status, IronClawUserStatus::Suspended);
    assert_eq!(profiled.role, IronClawUserRole::Admin);
}
...
async fn update_missing_user_surfaces_user_not_found() {
    ...
    let result = store
        ...
        .await;
    assert!(
        matches!(result, Err(IronClawIdentityError::UserNotFound(_))),
...
async fn count_active_admins_tracks_role_and_status() {
    ...
    let by = UserId::new("bootstrap").unwrap();
    let admin_a = store
        .create_user(&t, None, None, IronClawUserRole::Admin, &by)
        .await
        .expect("admin a");
    store
        .create_user(&t, None, None, IronClawUserRole::Owner, &by)
        .await
        .expect("owner");
    store
        .create_user(&t, None, None, IronClawUserRole::Member, &by)
    ...
    store
        .update_status(&admin_a.user_id, IronClawUserStatus::Suspended)
        .await
        .expect("suspend");
    assert_eq!(
        store.count_active_admins(&t).await.expect("count"),
...
async fn resolve_or_create_refuses_a_suspended_account_a_fresh_session() {
    ...
    store
        .update_status(&user, IronClawUserStatus::Suspended)
        .await
    ...
    assert!(
        matches!(err, IronClawIdentityError::UserSuspended(_)),
    ...
    store
        .update_status(&user, IronClawUserStatus::Active)
        .await
        .expect("reactivate");
...
async fn resolve_or_create_refuses_to_relink_a_user_mid_delete() {
    ...
    assert!(
        matches!(err, IronClawIdentityError::Backend(_)),
