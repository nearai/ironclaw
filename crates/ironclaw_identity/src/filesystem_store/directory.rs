//! [`IronClawUserDirectory`] implementation for [`FilesystemIronClawIdentityStore`].
//!
...
use super::{FilesystemIronClawIdentityStore, backend, to_user_id};
...
fn role_to_stored(role: IronClawUserRole) -> StoredUserRole {
    match role {
        IronClawUserRole::Owner => StoredUserRole::Owner,
        IronClawUserRole::Admin => StoredUserRole::Admin,
        IronClawUserRole::Member => StoredUserRole::Member,
    }
}

fn role_from_stored(role: StoredUserRole) -> IronClawUserRole {
    match role {
        StoredUserRole::Owner => IronClawUserRole::Owner,
        StoredUserRole::Admin => IronClawUserRole::Admin,
        StoredUserRole::Member => IronClawUserRole::Member,
    }
}

fn status_to_stored(status: IronClawUserStatus) -> StoredUserStatus {
    match status {
        IronClawUserStatus::Active => StoredUserStatus::Active,
        IronClawUserStatus::Suspended => StoredUserStatus::Suspended,
    }
}

fn status_from_stored(status: StoredUserStatus) -> IronClawUserStatus {
    match status {
        StoredUserStatus::Active => IronClawUserStatus::Active,
        StoredUserStatus::Suspended => IronClawUserStatus::Suspended,
    }
}
...
fn to_ironclaw_user(
    ...
    let tenant_id = match stored.tenant_id {
        Some(raw) => Some(TenantId::new(raw).map_err(|error| {
            IronClawIdentityError::Backend(format!("persisted tenant id is invalid: {error}"))
        })?),
        None => None,
    };
    Ok(IronClawUser {
...
fn map_cas_error(error: CasUpdateError<IronClawIdentityError>) -> IronClawIdentityError {
    match error {
        CasUpdateError::Apply(inner) => inner,
        CasUpdateError::Timeout => {
            IronClawIdentityError::Backend("user record update timed out".to_string())
        }
        CasUpdateError::RetriesExhausted => {
            IronClawIdentityError::Backend("user record update contended out".to_string())
        }
        CasUpdateError::CasUnsupported => IronClawIdentityError::Backend(
            "user record backend does not support compare-and-swap".to_string(),
        ),
        CasUpdateError::Backend(inner) => IronClawIdentityError::Backend(inner.to_string()),
    }
}
    ...
    F: RootFilesystem + 'static,
{
        ...
        M: Fn(&mut StoredUser) + Send + Sync,
    {
            ...
            |current: Option<StoredUser>| {
                ...
                async move {
                    let Some(mut user) = current else {
                        return Err(IronClawIdentityError::UserNotFound(user_id_str));
                    };
                    mutate(&mut user);
        ...
        to_ironclaw_user(user_id.as_str().to_string(), updated)
    }
...
#[async_trait]
impl<F> IronClawUserDirectory for FilesystemIronClawIdentityStore<F>
where
    F: RootFilesystem + 'static,
{
    async fn list_users(
        ...
        for (user_id, name) in candidates {
            ...
            users.push(to_ironclaw_user(user_id, stored)?);
            if users.len() >= limit {
                break;
            }
        }
    ...
    async fn get_user(
        ...
        match self.read_record::<StoredUser>(&path).await? {
            Some(stored) => Ok(Some(to_ironclaw_user(
    ...
    async fn create_user(
        ...
        .await
        .map_err(backend)?;
        to_ironclaw_user(new_user_id.as_str().to_string(), record)
    }
    ...
    async fn count_active_admins(
        ...
        let active = self
            .list_users(
                tenant_id,
                Some(IronClawUserStatus::Active),
                None,
                usize::MAX,
            )
            .await?;
