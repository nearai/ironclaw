...
//!   [`ResolveExternalIdentity`] values into [`IronClawIdentityResolver`].
//!
...
//!
//! Persistence ([`FilesystemIronClawIdentityStore`]) goes through the host
//! [`RootFilesystem`](ironclaw_filesystem::RootFilesystem) /
//! `ScopedFilesystem` abstraction — the same substrate boundary every other
//! durable IronClaw store sits behind — so substrate choice, tenant scoping,
...
pub use filesystem_store::FilesystemIronClawIdentityStore;
pub use key::{ExternalSubjectId, IdentityKeyError, ProviderInstanceId, ProviderKind};
pub use user_directory::{
    IronClawUser, IronClawUserDirectory, IronClawUserProfileUpdate, IronClawUserRole,
    IronClawUserStatus,
};
...
/// profile). Used by the link-only [`lookup`](IronClawIdentityResolver::lookup)
/// and [`bind`](IronClawIdentityResolver::bind) paths that channel actors
...
pub enum IronClawIdentityError {
    /// The persistence backend (connect / migrate / query / commit) failed.
    #[error("IronClaw identity store backend failure: {0}")]
    Backend(String),
    ...
    /// through [`lookup`](IronClawIdentityResolver::lookup) /
    /// [`bind`](IronClawIdentityResolver::bind) so an unbound actor fails closed
    /// instead of auto-provisioning a IronClaw account.
...
#[async_trait]
pub trait IronClawIdentityResolver: Send + Sync {
    ...
    /// [`ChannelActorNotMintable`](IronClawIdentityError::ChannelActorNotMintable):
    ...
    ) -> Result<UserId, IronClawIdentityError>;
    ...
    ) -> Result<Option<UserId>, IronClawIdentityError>;
    ...
    /// binding proves the actor is a known IronClaw user before binding).
    async fn bind(
        &self,
        key: ExternalIdentityKey,
        user_id: &UserId,
