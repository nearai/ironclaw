//! Filesystem-backed implementation of [`IronClawIdentityResolver`](crate::IronClawIdentityResolver).
//!
//! Identity records live behind the host [`RootFilesystem`] /
//! [`ScopedFilesystem`] abstraction — the same substrate boundary every other
//! durable IronClaw store (run-state, approvals, threads, Slack host-state) sits
...
use crate::{
    ExternalIdentityKey, IronClawIdentityError, IronClawIdentityResolver, ResolveExternalIdentity,
    SurfaceKind,
};
...
pub struct FilesystemIronClawIdentityStore<F>
where
...
impl<F> FilesystemIronClawIdentityStore<F>
where
    F: RootFilesystem + 'static,
{
    pub fn new(
    ...
    async fn read_record<T>(&self, path: &ScopedPath) -> Result<Option<T>, IronClawIdentityError>
    where
        T: DeserializeOwned,
    {
        ...
        let value = serde_json::from_slice(&versioned.entry.body)
            .map_err(|error| IronClawIdentityError::Backend(error.to_string()))?;
        Ok(Some(value))
    }
    ...
    ///     [`IronClawIdentityError::UserSuspended`] — the SSO adapter maps it to a
    ...
    async fn gate_resolved_user(
        ...
        if user.status == StoredUserStatus::Suspended {
            return Err(IronClawIdentityError::UserSuspended(
    ...
    async fn is_tombstoned(&self, user_id: &UserId) -> Result<bool, IronClawIdentityError> {
        Ok(self
            ...
            .await
        {
            ...
            Err(FilesystemError::VersionMismatch { .. }) => {
                let Some(existing) = self.read_record::<StoredExternalIdentity>(path).await? else {
                    return Err(IronClawIdentityError::Backend(
...
#[async_trait]
impl<F> IronClawIdentityResolver for FilesystemIronClawIdentityStore<F>
where
    F: RootFilesystem + 'static,
{
    async fn resolve_or_create(
        ...
        if identity.surface_kind == SurfaceKind::ChannelActor {
            return Err(IronClawIdentityError::ChannelActorNotMintable);
        }
...
fn to_user_id(raw: String) -> Result<UserId, IronClawIdentityError> {
    UserId::new(raw).map_err(|error| IronClawIdentityError::InvalidUserId(error.to_string()))
}

fn backend(error: FilesystemError) -> IronClawIdentityError {
    IronClawIdentityError::Backend(error.to_string())
}
