...
use ironclaw_composition::{IronClawIdentityResolver, PublicRouteMount};
use ironclaw_webui::{
...
/// admission adapter on top of the runtime-owned canonical IronClaw identity
...
pub(crate) async fn build_webui_auth_surface(
    ...
    identity_resolver: Option<Arc<dyn IronClawIdentityResolver>>,
    tenant_id: TenantId,
...
mod tests {
    ...
    async fn sso_configured_builds_surface_with_public_login_mount() {
        ...
        let surface = build_webui_auth_surface(
            Some(sso),
            Some(ironclaw_composition::open_ironclaw_identity_resolver(
