...
//! WebUI operator facade (`IronClawLocalServiceLifecycle` in
...
/// launchd label / systemd unit name for the canonical IronClaw service
/// identity. Shared, deliberately, with `IronClawLocalServiceLifecycle` in
/// `ironclaw_composition::observability::operator_service_lifecycle`
...
enum ServiceVerb {
    ...
    /// `<ironclaw_home>/logs/serve.std{out,err}.log`. These files are
...
impl ServiceCommand {
    pub(crate) fn execute(self, context: IronClawCliContext) -> Result<()> {
...
impl ServicePlatform {
    ...
    fn install(&self, context: &IronClawCliContext) -> Result<Vec<String>> {
...
pub(crate) fn install_and_start(context: &IronClawCliContext) -> Result<()> {
    let platform = ServicePlatform::detect()?;
...
/// service-definition file location. Distinct from the IronClaw home
...
///
/// - Not the IronClaw home itself: composition's default skill/extension
///   roots live under `<ironclaw_home>/...`, so the home is an *ancestor* of
///   them and trips `paths_overlap` (prefix match, not just equality).
/// - `<ironclaw_home>/workspace` is a leaf dir, neither ancestor nor
///   descendant of any skill root, so it never overlaps.
fn service_working_directory(ironclaw_home: &Path) -> PathBuf {
    ironclaw_home.join("workspace")
}
...
/// to list or read it.
fn ensure_service_working_directory(ironclaw_home: &Path) -> Result<PathBuf> {
    let dir = service_working_directory(ironclaw_home);
...
fn preflight_warnings(context: &IronClawCliContext) -> Result<Vec<String>> {
...
/// (`IronClawLocalServiceLifecycle`, see the module doc).
///
...
mod tests {
    ...
    fn install_with_runner_surfaces_missing_webui_token_warning_through_the_real_path() {
        ...
        let ironclaw_home = context.boot_config().home();
        std::fs::create_dir_all(ironclaw_home.path()).expect("create IronClaw home");
        std::fs::write(
            crate::commands::onboard::onboarding_marker_path(ironclaw_home),
            "{}",
        )
        .expect("write marker");
        std::fs::write(ironclaw_home.config_file_path(), "").expect("write config");
    ...
    fn service_verbs_dispatch_through_the_injectable_runner_path() {
        ...
        let context = IronClawCliContext::from_boot_config(
    ...
    /// IronClaw home nests under the same tempdir (IronClawHome falls back
    ...
    struct TempHomeGuard {
        ...
        prior_ironclaw_home: Option<std::ffi::OsString>,
    ...
    impl TempHomeGuard {
        fn set(tmp: &std::path::Path) -> Self {
            ...
            let prior_ironclaw_home = std::env::var_os("IRONCLAW_HOME");
    ...
    impl Drop for TempHomeGuard {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                match self.prior_ironclaw_home.take() {
    ...
    fn install_then_uninstall_linux_writes_and_removes_unit_file() {
        ...
        let ironclaw_home = context.boot_config().home().path().to_path_buf();
        let expected_working_directory = ironclaw_home.join("workspace");
        assert!(
            contents.contains(&format!(
                "WorkingDirectory=\"{}\"",
                expected_working_directory.display()
            )),
            "unit file must anchor cwd at <ironclaw_home>/workspace, not the IronClaw home itself \
             (the IronClaw home is an ancestor of every default skill root, so cwd=ironclaw_home \
        ...
        assert!(
            ...
            "install must create <ironclaw_home>/workspace so `serve` has somewhere to cd into"
        );
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        // facade (`IronClawLocalServiceLifecycle`), must now report that it
    ...
    fn install_then_uninstall_macos_writes_and_removes_plist() {
        ...
        assert!(
            ...
            "plist must anchor cwd at <ironclaw_home>/workspace, not the IronClaw home itself \
             (the IronClaw home is an ancestor of every default skill root, so cwd=ironclaw_home \
             still trips composition's overlap check — the crash-loop persisted after the first \
             attempt at this fix): {contents}"
        );
