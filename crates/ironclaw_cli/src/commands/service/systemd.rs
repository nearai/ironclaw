...
fn unit_content(invocation: &ServeInvocation, working_directory: &Path) -> Result<String> {
    ...
    // WorkingDirectory anchors cwd at `<ironclaw_home>/workspace`, not
    // systemd's default and not the IronClaw home itself — the home is an
...
pub(super) fn install_with_runner(
    ...
    // operator facade (`IronClawLocalServiceLifecycle`) — both surfaces
    ...
    let working_directory = super::ensure_service_working_directory(ironclaw_home)?;
...
mod tests {
    ...
    fn sample_ironclaw_home() -> PathBuf {
        PathBuf::from("/home/op/.ironclaw/reborn")
    }

    /// Resolves a `IronClawCliContext` from the currently-set `$HOME` (set by
    /// [`TempHomeGuard::set`]) — used by `install_with_runner` call sites
    /// below, which now need `context` to derive the unit's
    /// WorkingDirectory.
    fn sample_context() -> IronClawCliContext {
        IronClawCliContext::from_boot_config(
            ironclaw_config::IronClawBootConfig::resolve_from_env()
    ...
    fn unit_content_includes_service_type() {
        let unit = unit_content(&sample_invocation(), &sample_ironclaw_home()).expect("valid unit");
    ...
    fn unit_content_escapes_quotes_in_env_value() {
        ...
        let unit = unit_content(&invocation, &sample_ironclaw_home()).expect("valid unit");
    ...
    fn unit_content_cannot_inject_directives_through_newlines() {
        ...
        let unit = unit_content(&invocation, &sample_ironclaw_home()).expect("escaped unit");
