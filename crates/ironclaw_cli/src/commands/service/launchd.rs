...
/// `ironclaw_composition::observability::operator_service_lifecycle`
...
pub(super) fn install_with_runner(
    ...
    // launchd's default `/` and not the IronClaw home itself — the home is
...
mod tests {
    ...
    fn install_reloads_when_label_is_already_loaded() {
        ...
        let (_ctx_tmp, context) = IronClawCliContext::test_context();
    ...
    fn install_reports_replaced_existing_when_plist_already_present() {
        ...
        // (`IronClawLocalServiceLifecycle`) — both target the same label/
