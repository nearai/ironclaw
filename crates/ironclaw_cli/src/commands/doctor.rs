...
use ironclaw_config::{IronClawConfigFile, IronClawDoctorReport};
...
fn build_doctor_dto(context: &IronClawCliContext) -> DoctorDto {
...
fn check_config_file(path: &std::path::Path) -> DoctorCheck {
    match IronClawConfigFile::load(path) {
...
fn check_providers_file(path: &std::path::Path) -> DoctorCheck {
    match std::fs::read_to_string(path) {
        ...
        Ok(contents) => {
            match ironclaw_composition::validate_ironclaw_provider_catalog_contents(&contents) {
                Ok(()) => DoctorCheck {
                    name: "providers_file".to_string(),
                    category: CheckCategory::Core,
                    outcome: CheckOutcome::Pass,
...
fn driver_check(name: &str, status: &IronClawRuntimeComponentStatus) -> DoctorCheck {
    let (outcome, detail) = match status {
        IronClawRuntimeComponentStatus::Initialized => {
            (CheckOutcome::Pass, "initialized".to_string())
        }
        IronClawRuntimeComponentStatus::Failed(reason) => {
...
mod tests {
    ...
    fn driver_check_failed_status_produces_fail_outcome() {
        let status = IronClawRuntimeComponentStatus::Failed("timeout".to_string());
