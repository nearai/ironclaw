...
use ironclaw_composition::build_webui_services;
use ironclaw_composition::host_api::{
    AgentId, InvocationId, ProjectId, ResourceScope, SecretHandle, TenantId, UserId,
};
use ironclaw_composition::{
    IronClawBuildInput, IronClawReadiness, IronClawRuntimeIdentity, IronClawRuntimeInput,
    IronClawWebuiBundle, TriggerFireAccessPolicy, build_ironclaw_runtime,
};
use ironclaw_config::{IdentitySection, seed_default_config_file_if_missing};
use ironclaw_webui::{
    DeferredWebuiRouterHandle, EnvBearerAuthenticator, IronClawWebuiServeError,
    IronClawWebuiServeOptions, WebuiAuthenticator, WebuiServeConfig,
...
/// would silently fall through to the `<ironclaw_home>/webui-token` file
...
impl ServeCommand {
    pub(crate) fn execute(self, context: IronClawCliContext) -> anyhow::Result<()> {
        ...
        let config_file =
            ironclaw_config::IronClawConfigFile::load(&boot_config.home().config_file_path())
        ...
        // `<ironclaw_home>/webui-token` (the `onboard`-provisioned fallback a
        ...
        // sources (env var or `<ironclaw_home>/webui-token`) produced it.
        ...
        rt.block_on(async move {
            ...
            let runtime = build_ironclaw_runtime(runtime_input)
                .await
            ...
            let bundle: IronClawWebuiBundle = build_webui_services(&runtime, None)?;
            ...
            // Only SSO-enabled WebUI needs the canonical IronClaw identity
            ...
            let identity_resolver = if sso_startup.is_some() {
                match runtime.open_ironclaw_identity_resolver(&tenant_id).await {
                    Some(result) => Some(
            ...
            if let Some(ingress_parts) = runtime.services().extension_ingress_parts() {
                let ingress_mount =
                    ironclaw_composition::extension_ingress_route_mount(&ingress_parts)
            ...
            // IronClaw runtime. Protocol webhooks such as Slack can ACK a
            ...
            // Always drain the IronClaw runtime, even on serve error, so
...
struct StartupServe {
    ...
    serve_task: tokio::task::JoinHandle<Result<(), IronClawWebuiServeError>>,
}
...
fn reject_non_loopback_privileged_local_runtime(
    host: IpAddr,
    runtime_input: &IronClawRuntimeInput,
) -> anyhow::Result<()> {
...
fn with_notion_dcr_oauth_backend(
    services: IronClawBuildInput,
    callback_origin: &str,
) -> anyhow::Result<IronClawBuildInput> {
    services
...
fn reject_legacy_slack_config(
    config_file: &ironclaw_config::IronClawConfigFile,
...
fn resolve_webui_default_agent(
    ...
    runtime_identity: &IronClawRuntimeIdentity,
) -> String {
    identity_section
...
/// Resolve the owner the IronClaw runtime must run under for the WebChat v2
/// serve path.
///
...
/// through `IronClawRuntime::seed_local_dev_secret` (#5459 tenant-shared tool
...
mod tests {
    ...
    fn webui_default_agent_falls_back_to_runtime_identity() {
        let runtime_identity = IronClawRuntimeIdentity::ironclaw_cli();
    ...
    fn webui_user_id_raw_prefers_a_set_nonempty_env_var() {
        ...
        let config_file = ironclaw_config::IronClawConfigFile {
    ...
    #[test]
    fn webui_user_id_raw_defaults_to_ironclaw_cli_when_no_config_or_env() {
        let _guard = crate::runtime::test_env::lock_runtime_env();
        // SAFETY: serialized by the shared crate process-env lock.
        unsafe { std::env::remove_var(WEBUI_USER_ID_TEST_ENV) };
...
"#,
        ...
        .expect("write config");
        let config_file = ironclaw_config::IronClawConfigFile::load(&config_path)
    ...
    async fn webui_serve_wires_notion_dcr_into_runtime_services() {
        ...
        let services_input = with_notion_dcr_oauth_backend(
            IronClawBuildInput::local_dev("notion-dcr-owner", dir.path().join("local-dev")),
            "http://127.0.0.1:3000",
        )
        .expect("notion dcr wiring");
        let services = ironclaw_composition::build_ironclaw_services(services_input)
            .await
            .expect("IronClaw services build");
