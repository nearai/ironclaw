...
use ironclaw_config::{IRONCLAW_PROFILE_ENV, IronClawProfile};
...
impl ProfileListCommand {
    fn execute(self) -> anyhow::Result<()> {
        let profiles = IronClawProfile::all();

        if self.json {
            let profiles = profiles.iter().map(|profile| {
                serde_json::json!({
                    "name": profile.as_str(),
                    "default": *profile == IronClawProfile::default(),
                })
            });
            println!(
                "{}",
            ...
            for profile in profiles {
                if *profile == IronClawProfile::default() {
