...
use ironclaw_traces::client::{TraceClientHost, TraceClientScope};
...
/// `traces` subcommand surface for the standalone IronClaw CLI.
#[derive(Debug, Args)]
...
enum TracesSubcommand {
    ...
    OptIn {
        ...
        #[arg(long, default_value_t = ironclaw_traces::contribution::TRACE_UPLOAD_CLAIM_DEFAULT_TIMEOUT_MS)]
...
struct TraceQueueStatusDiagnostics {
    ...
    queue: ironclaw_traces::contribution::TraceQueueDiagnostics,
...
fn opt_out(user_scope: Option<&str>) -> anyhow::Result<()> {
    ...
    if explicit_scope {
        ...
        ironclaw_traces::contribution::opt_out_user_scope(&runtime_scope)?;
...
async fn enroll_instance(
    ...
    let consents = ironclaw_traces::onboarding::OnboardConsents {
        include_message_text,
        include_tool_payloads,
    };
    let outcome = ironclaw_traces::onboarding::onboard_instance_at_base(
        &ironclaw_traces::paths::ironclaw_base_dir(),
        invite,
        consents,
    )
    .await
...
fn show_policy_status(json: bool, user_scope: Option<&str>) -> anyhow::Result<()> {
    ...
    let queued_count = match normalized_scope.as_deref() {
        Some(scope) => {
            ironclaw_traces::contribution::queued_trace_envelope_paths_for_scope(Some(scope))?.len()
        }
...
async fn preview_recorded_trace(options: PreviewOptions) -> anyhow::Result<()> {
    ...
    let recorded_trace: ironclaw_traces::recording::TraceFile = serde_json::from_str(&raw_json)
        .map_err(|e| {
            anyhow::anyhow!(
...
fn trace_contribution_dir() -> PathBuf {
    ironclaw_traces::paths::ironclaw_base_dir().join("trace_contributions")
}
