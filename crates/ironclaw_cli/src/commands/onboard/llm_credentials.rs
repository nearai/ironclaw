...
use ironclaw_config::IronClawHome;
...
pub(crate) enum LlmCredentialProvisionOutcome {
    ...
    /// Complete LLM config detected in env (`IronClawProviderAdmin::detect_env_llm`)
...
pub(crate) trait LlmKeyStoreOpener {
    fn open(&self, home_path: &Path) -> anyhow::Result<ironclaw_composition::LlmKeyStore>;
}

/// Production [`LlmKeyStoreOpener`]: opens the real local-dev encrypted
/// secret store `serve` later reads from (see
/// `ironclaw_composition::open_local_dev_secret_store`'s doc for why
...
///   already-built [`ironclaw_composition::IronClawProviderAdmin`]
///   rather than raw construction ingredients.
pub(crate) trait LlmProbe {
    fn probe(
        &self,
        admin: &ironclaw_composition::IronClawProviderAdmin,
        provider_id: &str,
        api_key: Option<&str>,
        model: Option<&str>,
    ) -> anyhow::Result<ironclaw_composition::ProviderProbeOutcome>;
}

/// Production [`LlmProbe`]: calls `IronClawProviderAdmin::probe_candidate`,
...
///
/// Env-detect step (before the numbered menu), via `IronClawProviderAdmin::detect_env_llm`
/// (same resolution `resolve_ironclaw_runtime_llm`'s fallback and the `run`/`serve`
...
///   (`IronClawProviderAdmin::set_provider`, same machinery `models set-provider`
...
pub(crate) fn provision_llm_credentials(
    _home: &IronClawHome,
    boot: &ironclaw_config::IronClawBootConfig,
    ...
    let admin = ironclaw_composition::IronClawProviderAdmin::new(boot.clone());
...
/// (`IronClawProviderAdmin::effective_api_key_required` — not the raw
/// `providers.json` field; a `session_token`-kind provider like `nearai` is
/// overridden to `true` since IronClaw has no session-token auth wired). Not
...
mod tests {
    ...
    impl PromptSource for NonInteractivePromptSource {
        ...
        fn provider_menu(
            &mut self,
            _entries: &[ironclaw_composition::ProviderMenuEntry],
        ) -> Result<String, LlmCredentialPromptError> {
    ...
    impl LlmProbe for StubOkProbe {
        fn probe(
            &self,
            _admin: &ironclaw_composition::IronClawProviderAdmin,
            _provider_id: &str,
            _api_key: Option<&str>,
            _model: Option<&str>,
        ) -> anyhow::Result<ironclaw_composition::ProviderProbeOutcome> {
            Ok(ironclaw_composition::ProviderProbeOutcome {
                ok: true,
                models: Vec::new(),
                message: String::new(),
            })
    ...
    struct ScriptedProbe {
        outcomes: std::cell::RefCell<
            std::collections::VecDeque<ironclaw_composition::ProviderProbeOutcome>,
        >,
        calls: std::cell::RefCell<Vec<RecordedProbeCall>>,
    }

    impl ScriptedProbe {
        fn new(outcomes: Vec<ironclaw_composition::ProviderProbeOutcome>) -> Self {
            Self {
    ...
    impl LlmKeyStoreOpener for FailingLlmKeyStoreOpener {
        fn open(&self, _home_path: &Path) -> anyhow::Result<ironclaw_composition::LlmKeyStore> {
            let backend = Arc::new(
            ...
            Ok(ironclaw_composition::LlmKeyStore::new(store))
        }
    }
    ...
    fn seed_cached_master_key(home: &IronClawHome) {
        std::fs::write(
            home.path()
    ...
    fn provision_llm_credentials_writes_config_and_secret_store_through_fake_prompts() {
        ...
        std::fs::create_dir_all(home.path()).expect("create IronClaw home");
    ...
    /// `IronClawProviderAdmin::menu_entries`'s doc: IronClaw has no
    ...
    /// (base-URL-trap exclusion — see `IronClawProviderAdmin::menu_entries`'s
    ...
    /// A fresh IronClaw home, interactive session, clean environment (no LLM
    ...
    fn probe_outcome(
        ...
        models: Vec<&str>,
        message: &str,
    ) -> ironclaw_composition::ProviderProbeOutcome {
        ironclaw_composition::ProviderProbeOutcome {
            ok,
