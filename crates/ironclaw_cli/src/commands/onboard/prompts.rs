...
pub(crate) trait PromptSource {
    ...
    /// (`IronClawProviderAdmin::menu_entries`'s output — `nearai` is entry 0
...
fn menu_entry_key_note(entry: &ironclaw_composition::ProviderMenuEntry) -> &'static str {
...
#[cfg(test)]
mod tests {
    use ironclaw_composition::ProviderMenuEntry;
    ...
    fn entries() -> Vec<ProviderMenuEntry> {
        vec![
            ProviderMenuEntry {
                ...
                // Menu-level override (see `IronClawProviderAdmin::menu_entries`'s
                // doc): IronClaw has no session-token auth wired, so nearai is
