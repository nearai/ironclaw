...
///   (`ironclaw_composition::factory::resolve_local_dev_secret_master_key_with_env`)
...
pub(crate) enum MasterKeyProvisionOutcome {
    ...
    /// exists under this IronClaw home; nothing to provision.
...
pub(crate) fn provision_master_key(
    ...
    let dotfile_path = crate::runtime::local_runtime_storage_root(boot, boot.profile())
        .join(ironclaw_composition::LOCAL_DEV_SECRETS_MASTER_KEY_PATH);
    if dotfile_path.exists() {
        return Ok(MasterKeyProvisionOutcome::DotfileAlreadyPresent);
    }

    crate::runtime::block_on_cli(async move {
        let outcome = ironclaw_composition::provision_local_dev_keychain_master_key().await;
