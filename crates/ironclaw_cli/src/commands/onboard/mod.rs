...
impl OnboardCommand {
    ...
    fn finish_with_service_and_login_link(
        ...
        let config_file = ironclaw_config::IronClawConfigFile::load(&home.config_file_path())
            .ok()
            .flatten();
...
pub(crate) fn onboarding_marker_path(home: &IronClawHome) -> PathBuf {
...
mod tests {
    ...
    fn write_onboarding_marker_steps_pending_reflects_llm_configured() {
        let (_tmp, context) = crate::context::IronClawCliContext::test_context();
        let home = context.boot_config().home();
