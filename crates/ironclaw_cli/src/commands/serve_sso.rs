...
///
/// Precedence stays IronClaw-scoped: explicit IronClaw env first, then the bound
...
///
/// This keeps the cleartext-OAuth policy local to the IronClaw CLI command
...
mod tests {
    ...
    #[test]
    fn webui_oauth_base_url_prefers_explicit_ironclaw_env() {
        let _guard = crate::runtime::test_env::lock_runtime_env();
        clear_sso_env();
        // SAFETY: serialized by the shared crate process-env lock; cleaned up before the guard drops.
        unsafe {
