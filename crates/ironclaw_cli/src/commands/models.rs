...
impl ModelsListCommand {
    fn execute(self) -> anyhow::Result<()> {
        let context = IronClawCliContext::resolve_from_env()?;
        let admin = ironclaw_composition::IronClawProviderAdmin::new(context.boot_config().clone());
        let list = admin.list(
...
fn print_provider_list(list: &ironclaw_composition::IronClawProviderList, verbose: bool) {
...
fn print_provider_detail(list: &ironclaw_composition::IronClawProviderList) {
...
fn print_status(status: &ironclaw_composition::IronClawProviderStatus) {
...
fn print_write_outcome(
    ...
    outcome: &ironclaw_composition::IronClawProviderWriteOutcome,
) {
    match kind {
