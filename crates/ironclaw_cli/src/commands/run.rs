use clap::Args;
use ironclaw_composition::ironclaw_runtime_readiness_snapshot;
...
fn run_dry(context: IronClawCliContext) -> anyhow::Result<()> {
    let config = context.boot_config();
    let readiness = ironclaw_runtime_readiness_snapshot();
