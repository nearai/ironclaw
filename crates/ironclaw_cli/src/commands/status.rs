...
use ironclaw_composition::{
    IronClawRuntimeComponentStatus, ironclaw_model_slot_names, ironclaw_runtime_readiness_snapshot,
};
...
fn build_status_dto(context: &IronClawCliContext) -> anyhow::Result<StatusDto> {
...
fn build_status_dto_with_service_state(
    ...
    let model_slots = ironclaw_model_slot_names()
        .into_iter()
        .map(|s| s.to_string())
        .collect();
...
fn resolve_google_oauth_degraded(
    config: &ironclaw_config::IronClawBootConfig,
...
pub(super) fn convert_component_status(status: &IronClawRuntimeComponentStatus) -> ComponentStatus {
    match status {
        IronClawRuntimeComponentStatus::Initialized => ComponentStatus::Initialized,
        IronClawRuntimeComponentStatus::Failed(reason) => ComponentStatus::Failed {
            reason: reason.clone(),
        },
    }
}
...
mod tests {
    use super::*;
    use crate::context::IronClawCliContext;
    use ironclaw_composition::IronClawRuntimeComponentStatus;
    ...
    fn status_dto_includes_login_link_once_a_valid_webui_token_file_exists() {
        ...
        std::fs::write(
            ...
            "ironclaw-status-test-token-0123456789abcdef",
        )
        ...
        assert!(
            login_link.contains("/login?token=ironclaw-status-test-token-0123456789abcdef"),
    ...
    fn status_dto_json_excludes_the_login_link_token() {
        ...
        let home = context.boot_config().home();
        std::fs::create_dir_all(home.path()).expect("create IronClaw home");
        let token = "ironclaw-status-json-test-token-0123456789abcdef";
    ...
    fn status_dto_complete_google_config_does_not_open_secret_store() {
        ...
        std::fs::create_dir_all(config.home().path()).expect("create IronClaw home");
        std::fs::write(
...
"#,
        ...
        let db_path = ironclaw_composition::local_dev_db_path(&storage_root);
    ...
    fn convert_component_status_failed_maps_correctly() {
        let status = IronClawRuntimeComponentStatus::Failed("db connection refused".to_string());
    ...
    fn status_text_suppresses_login_link_and_shows_restart_guidance_when_service_stopped() {
        ...
        std::fs::write(
            home.path().join("webui-token"),
            "ironclaw-status-suppression-test-0123456789abcdef",
        )
        .expect("seed webui-token file");
