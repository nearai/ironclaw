...
pub(super) fn build_config_list_dto(context: &IronClawCliContext) -> anyhow::Result<ConfigListDto> {
...
fn load_config_file(
    context: &IronClawCliContext,
) -> anyhow::Result<ironclaw_config::IronClawConfigFile> {
    let config_path = context.boot_config().home().config_file_path();
    Ok(ironclaw_config::IronClawConfigFile::load(&config_path)?.unwrap_or_default())
}

fn flatten_config(
    config: &ironclaw_config::IronClawConfigFile,
) -> anyhow::Result<Vec<ConfigEntry>> {
    use ironclaw_config::IronClawConfigFile;
    ...
    // IronClawConfigFile will fail to compile here until handled — that is
    ...
    let expanded = IronClawConfigFile {
...
mod tests {
    ...
    fn flatten_empty_config_has_all_keys() {
        let config = IronClawConfigFile::default();
...
"#;
        let config =
            IronClawConfigFile::parse_text(toml, std::path::Path::new("/test/config.toml"))
                .expect("must parse");
        let entries = flatten_config(&config).expect("flatten");
...
"#;
    ...
    fn build_config_get_dto_known_key_none_value_returns_ok_none() {
        let (_tmp, context) = IronClawCliContext::test_context();
    ...
    fn build_config_get_dto_known_key_set_value_returns_ok_some() {
        let (_tmp, context) = IronClawCliContext::test_context();
        let ironclaw_home = context.boot_config().home().path();
        std::fs::create_dir_all(ironclaw_home).expect("create ironclaw_home");
        std::fs::write(
            ironclaw_home.join("config.toml"),
