...
use clap::Args;
use ironclaw_config::{IRONCLAW_CONFIG_API_VERSION, IronClawHome};
...
///   for seeding paths and `resolve_ironclaw_runtime_llm` for the env fallback
...
///   but that flow is not wired in IronClaw yet — no `SessionRenewer` attaches
...
"#,
        api_version = IRONCLAW_CONFIG_API_VERSION,
...
mod tests {
    ...
    /// `ironclaw_cli` is excluded from depending on `ironclaw_llm`
    /// directly (per `ironclaw_dependency_boundaries`), so there's no shared
    ...
    /// — so `resolve_ironclaw_runtime_llm` reaches the env fallback.
    #[test]
    fn deseeded_stub_has_no_default_llm_slot_and_reaches_env_fallback() {
        ...
        let config_file =
            ironclaw_config::IronClawConfigFile::parse_text(&config_text, &home.config_file_path())
        ...
        let resolved = ironclaw_composition::resolve_ironclaw_runtime_llm(
            context.boot_config(),
            Some(&config_file),
        );
        assert!(
            !matches!(
                &resolved,
                Err(ironclaw_composition::IronClawLlmCatalogError::MissingProviderId)
            ),
