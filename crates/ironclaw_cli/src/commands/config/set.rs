...
use clap::Args;
use ironclaw_config::{GoogleFieldUpdate, GoogleOauthConfigUpdate, IronClawHome};
...
fn write_slack_enabled(home: &IronClawHome, value: &str) -> anyhow::Result<()> {
...
trait SecretStoreOpener {
    fn open_llm_key_store(
        &self,
        home_path: &Path,
    ) -> anyhow::Result<ironclaw_composition::LlmKeyStore>;
...
impl SecretStoreOpener for LocalDevSecretStoreOpener {
    fn open_llm_key_store(
        ...
        // `config set` is a write command: create the IronClaw home directory
        ...
        crate::runtime::block_on_cli(async move {
            let store = ironclaw_composition::open_local_dev_secret_store(&home_path)
                .await
                .map_err(anyhow::Error::from)?;
            Ok::<_, anyhow::Error>(ironclaw_composition::LlmKeyStore::new(store))
        })
    }
    ...
    fn open_google_oauth_secret_store(
        ...
        // so create the IronClaw home directory before opening the store.
        std::fs::create_dir_all(home_path).map_err(|error| {
            anyhow::anyhow!("create IronClaw home {}: {error}", home_path.display())
        })?;
        let home_path = home_path.to_path_buf();
...
mod tests {
    ...
    impl SecretStoreOpener for FakeSecretStoreOpener {
        fn open_llm_key_store(
            ...
            Ok(ironclaw_composition::LlmKeyStore::new(self.store.clone()))
        }
    ...
    fn config_toml(context: &IronClawCliContext) -> String {
    ...
    /// never-onboarded home runs, before `onboard` creates the IronClaw home
    ...
    #[test]
    fn google_client_secret_write_creates_the_ironclaw_home_directory_on_a_never_onboarded_host() {
        ...
        assert!(
            !home.path().exists(),
            "sanity: test_context's IronClaw home must not exist yet, matching a \
        ...
        result.expect(
            "writing a secret-destination key must create the IronClaw home directory \
    ...
    fn llm_api_key_writes_to_secret_store_and_not_config_toml() {
        ...
        let store = opener.store.clone();
        let stored = crate::runtime::block_on_cli(async move {
            ironclaw_composition::LlmKeyStore::new(store)
                .read("openai")
                .await
        })
        .expect("read store");
