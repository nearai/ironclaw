...
use ironclaw_composition::{
    ...
    IronClawRuntimeProfileOptions, KeepaliveSweepSettings, OAuthClientConfig, OperatorLogLayer,
    PollSettings, TurnRunnerSettings, build_ironclaw_runtime,
    local_runtime_build_input_with_options, nearai_mcp_bootstrap_config_from_env,
};
use ironclaw_config::{
    IRONCLAW_PROFILE_ENV, IronClawBootConfig, IronClawProfile, LEGACY_IRONCLAW_PROFILE_ENV,
...
pub(crate) fn init_tracing() {
    ...
    let stderr_filter = ironclaw_env_filter(
        "IRONCLAW_LOG",
        "info,ironclaw_runner=info,ironclaw_composition=info",
    );
    ...
    let operator_filter = ironclaw_env_filter(
...
const IRONCLAW_NOISY_LOG_TARGETS: &[(&str, &str)] = &[
    ("tokio_postgres", "warn"),
...
fn ironclaw_env_filter(env_key: &str, default_filter: &str) -> tracing_subscriber::EnvFilter {
    let default_filter = protect_ironclaw_log_filter(default_filter);
    match std::env::var(env_key) {
        Ok(raw_filter) => {
            tracing_subscriber::EnvFilter::try_new(protect_ironclaw_log_filter(&raw_filter))
...
fn protect_ironclaw_log_filter(raw_filter: &str) -> String {
    let mut filter = if raw_filter.trim().is_empty() {
        "info".to_string()
    } else {
        raw_filter.trim().to_string()
    };
    for (target, level) in IRONCLAW_NOISY_LOG_TARGETS {
...
async fn apply_run_trigger_fire_access_policy(
    ...
    config: &IronClawBootConfig,
) -> anyhow::Result<IronClawRuntimeInput> {
    {
...
fn print_runtime_banner(config: &IronClawBootConfig) {
...
fn reply_without_text_summary(reply: &ironclaw_composition::AssistantReply) -> &'static str {
    match reply.status {
        ...
        | ironclaw_composition::TurnStatus::RecoveryRequired => {
            ironclaw_composition::ironclaw_failure_summary_for_category(
...
/// Resolve the IronClaw runtime's default LLM selection, tolerating a
...
fn resolve_ironclaw_runtime_llm_with_stored_key_fallback(
    config: &IronClawBootConfig,
    config_file: Option<&ironclaw_config::IronClawConfigFile>,
    caller: RuntimeInputCaller,
) -> anyhow::Result<Option<ironclaw_composition::ResolvedIronClawLlm>> {
    let error = match ironclaw_composition::resolve_ironclaw_runtime_llm(config, config_file) {
    ...
    let ironclaw_composition::IronClawLlmCatalogError::ApiKeyEnvUnset { ref provider, .. } = error
    else {
        return Err(error.into());
    };
    ...
    let has_stored_key = block_on_cli(async move {
        let store = ironclaw_composition::open_local_dev_secret_store(&runtime_storage_root)
            .await
            .map_err(anyhow::Error::from)?;
    ...
    ironclaw_composition::resolve_llm_selection_allow_missing_key(
        selection,
        Some(config.home().providers_file_path().as_path()),
    )
    .map(ironclaw_composition::ResolvedIronClawLlm::from_llm_config)
    .map(Some)
    .map_err(Into::into)
}
...
pub(crate) fn build_runtime_input_with_options(
    ...
    #[allow(unused_mut)]
    let mut runtime_input = IronClawRuntimeInput::from_services(services_input)
...

    {
        ...
        match resolve_ironclaw_runtime_llm_with_stored_key_fallback(
            config,
            runtime_services.config_file.as_ref(),
            caller,
        )? {
...
pub(crate) struct RuntimeServicesInput {
    pub(crate) services_input: IronClawBuildInput,
    config_file: Option<ironclaw_config::IronClawConfigFile>,
}

pub(crate) struct BuiltRuntimeInput {
    pub(crate) inner: IronClawRuntimeInput,
}
...
pub(crate) fn build_services_input_with_options(
    ...
    let mut services_input = match profile {
        ...
        IronClawProfile::HostedSingleTenant => build_hosted_single_tenant_services_input(
            profile,
            owner_id,
            config,
            config_file.as_ref(),
        )?,
        IronClawProfile::Production | IronClawProfile::MigrationDryRun => {
...
fn build_hosted_single_tenant_services_input(
    ...
    Ok(
        IronClawBuildInput::hosted_single_tenant_postgres_from_config_and_env(
            composition_profile(profile),
            owner_id,
...
fn build_production_services_input(
    ...
    IronClawBuildInput::postgres_from_config_and_env(
        composition_profile(profile),
        owner_id,
        config_file,
    )
...
pub(crate) fn resolve_google_oauth_config_from_env(
    ...
    match resolve_google_oauth_config_state_with_store_loader(env, config_google, || {
        ...
        GoogleOAuthResolution::Disabled(state) => {
            tracing::debug!(
                target = "ironclaw::cli::google_oauth",
                ?state,
...
fn google_oauth_client_secret_from_store(
    ...
    if !ironclaw_composition::local_dev_db_path(&storage_root).exists() {
        return Ok(None);
    }
    block_on_cli(async move {
        let store = ironclaw_composition::open_local_dev_secret_store(&storage_root)
            .await
            .map_err(anyhow::Error::from)?;
...
struct GoogleOAuthEnvInputs {
    ironclaw_client_id: Option<String>,
    ironclaw_redirect_uri: Option<String>,
    ironclaw_client_secret: Option<String>,
    ironclaw_hosted_domain_hint: Option<String>,
...
impl GoogleOAuthEnvInputs {
    fn read(mut lookup: impl FnMut(&str) -> Option<String>) -> Self {
        Self {
            ironclaw_client_id: lookup("IRONCLAW_GOOGLE_CLIENT_ID"),
            ironclaw_redirect_uri: lookup("IRONCLAW_GOOGLE_OAUTH_REDIRECT_URI"),
            ironclaw_client_secret: lookup("IRONCLAW_GOOGLE_CLIENT_SECRET"),
            ironclaw_hosted_domain_hint: lookup("IRONCLAW_GOOGLE_HOSTED_DOMAIN_HINT"),
    ...
    fn has_client_secret(&self) -> bool {
        self.ironclaw_client_secret.is_some() || self.legacy_client_secret.is_some()
    }
}
...
fn resolve_google_oauth_config_state_from_inputs(
    ...
    let redirect_uri = env
        ...
        .or_else(|| config_google.and_then(|section| section.redirect_uri.clone()));
    let env_client_secret = env.ironclaw_client_secret.or(env.legacy_client_secret);
    let hosted_domain_hint = env
        .ironclaw_hosted_domain_hint
...
pub(crate) fn default_owner_id(config_file: Option<&ironclaw_config::IronClawConfigFile>) -> &str {
    config_file
...
fn composition_profile(profile: IronClawProfile) -> IronClawCompositionProfile {
    match profile {
        IronClawProfile::LocalDev => IronClawCompositionProfile::LocalDev,
        IronClawProfile::LocalDevYolo => IronClawCompositionProfile::LocalDevYolo,
        IronClawProfile::HostedSingleTenant => IronClawCompositionProfile::HostedSingleTenant,
        IronClawProfile::HostedSingleTenantVolume => {
            IronClawCompositionProfile::HostedSingleTenantVolume
        }
        IronClawProfile::Production => IronClawCompositionProfile::Production,
        IronClawProfile::MigrationDryRun => IronClawCompositionProfile::MigrationDryRun,
    }
}

pub(crate) fn read_config_file(
    config: &IronClawBootConfig,
) -> anyhow::Result<Option<ironclaw_config::IronClawConfigFile>> {
    use ironclaw_config::IronClawConfigFile;
    let path = config.home().config_file_path();
    let file = IronClawConfigFile::load(&path).map_err(anyhow::Error::from)?;
...
fn runtime_identity(
    config_file: Option<&ironclaw_config::IronClawConfigFile>,
) -> IronClawRuntimeIdentity {
    let default = IronClawRuntimeIdentity::ironclaw_cli();
...
pub(crate) fn effective_profile(
    config: &IronClawBootConfig,
    config_file: Option<&ironclaw_config::IronClawConfigFile>,
) -> anyhow::Result<IronClawProfile> {
    // Env wins over file. `IronClawBootConfig` already parsed/validated env,
    // so if the variable is present we keep that value.
    if std::env::var_os(IRONCLAW_PROFILE_ENV).is_some()
        || std::env::var_os(LEGACY_IRONCLAW_PROFILE_ENV).is_some()
    {
    ...
    profile.parse::<IronClawProfile>().map_err(|error| {
...
mod tests {
    ...
    use ironclaw_composition::TriggerFireAccessPolicy;
    use ironclaw_composition::{
        IronClawCompositionProfile, KeepaliveSweepSettings, TurnStatus,
    ...
    use super::{
        ...
        build_runtime_input_with_options, no_assistant_text_message, protect_ironclaw_log_filter,
    ...
    fn parse_runner_section(toml: &str) -> ironclaw_config::IronClawConfigFile {
        ironclaw_config::IronClawConfigFile::parse_text(
            toml,
    ...
    fn build_runtime_input_env_runner_worker_count_zero_reaches_runtime_input() {
        ...
        // override propagates onto IronClawRuntimeInput.runner, not only inside
        ...
        let config = IronClawBootConfig::resolve_from_env_parts(
            Some(ironclaw_home.into_os_string()),
            None,
            None,
            None,
        )
...
"#,
    ...
    #[test]
    fn ironclaw_log_filter_suppresses_noisy_targets_for_broad_debug() {
        let filter = protect_ironclaw_log_filter("debug");
    ...
    #[test]
    fn ironclaw_log_filter_keeps_explicit_noisy_target_directives() {
        let filter = protect_ironclaw_log_filter(
    ...
    #[test]
    fn ironclaw_log_filter_blank_env_uses_info_with_noisy_target_suppression() {
        let filter = protect_ironclaw_log_filter("   ");
    ...
    fn clear_ironclaw_postgres_tls_env() -> (EnvGuard, EnvGuard) {
        (
...
"#,
    ...
    fn build_runtime_input_accepts_confirmed_local_dev_yolo_profile() {
        ...
        assert_eq!(services.profile(), IronClawCompositionProfile::LocalDevYolo);
        assert_eq!(
    ...
    fn build_runtime_input_accepts_hosted_single_tenant_volume_profile() {
        ...
        std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
        let config = IronClawBootConfig::resolve_from_env_parts(
            Some(ironclaw_home.clone().into_os_string()),
            None,
            None,
        ...
        assert_eq!(
            local_runtime_storage_root(
                &config,
                ironclaw_config::IronClawProfile::HostedSingleTenantVolume,
            ),
            ironclaw_home.join("hosted-single-tenant-volume")
        );
    }
    ...
    fn boot_config_with_config_toml(
        profile: &str,
        config_toml: &str,
    ) -> (tempfile::TempDir, IronClawBootConfig) {
        let temp = tempfile::tempdir().expect("tempdir");
        let ironclaw_home = temp.path().join("ironclaw-home");
        std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
        std::fs::write(ironclaw_home.join("config.toml"), config_toml).expect("write config");
        let config = IronClawBootConfig::resolve_from_env_parts(
            Some(ironclaw_home.into_os_string()),
            None,
            None,
...
"#,
    ...
    fn build_runtime_input_hosted_single_tenant_constructs_postgres_local_runtime_input() {
        let _lock = lock_runtime_env();
        let (_enabled, _interval) = clear_trigger_poller_env();
        let (database_sslmode, allow_cleartext) = clear_ironclaw_postgres_tls_env();
...
"#,
    ...
    fn build_runtime_input_production_preserves_whitespace_secret_master_key() {
        ...
        let _postgres_url = EnvGuard::set(
            "IRONCLAW_POSTGRES_URL",
            "postgres://localhost/ironclaw_cli_test",
        );
...
"#,
        ...
        let runtime_input =
            build_runtime_input(&config, RuntimeInputCaller::Run).expect("runtime input");
        let services = runtime_input.services.expect("services input");
        assert_eq!(services.profile(), IronClawCompositionProfile::Production);
    }
...
"#,
    ...
    fn build_runtime_input_production_rejects_remote_postgres_sslmode_disable_redacted() {
        let _lock = lock_runtime_env();
        let (_enabled, _interval) = clear_trigger_poller_env();
        let (_database_sslmode, _allow_cleartext) = clear_ironclaw_postgres_tls_env();
        let _postgres_url = EnvGuard::set(
            "IRONCLAW_POSTGRES_URL",
...
"#,
        ...
        let err = build_runtime_input(&config, RuntimeInputCaller::Run)
            .err()
            .expect("DATABASE_SSLMODE=disable must fail without the IronClaw opt-in");
        let rendered = format!("{err:#}");
...
"#,
    ...
    fn build_runtime_input_env_enables_trigger_poller_with_no_config_section() {
        ...
        let config = IronClawBootConfig::resolve_from_env_parts(
            Some(ironclaw_home.to_string_lossy().to_string().into()),
            None,
            None,
            None,
        )
...
"#,
    ...
    #[test]
    fn resolve_google_oauth_config_prefers_ironclaw_prefixed_vars() {
        let vars = HashMap::from([
            (
            ...
            ("IRONCLAW_GOOGLE_HOSTED_DOMAIN_HINT", "ironclaw.example.com"),
            (
                "GOOGLE_CLIENT_ID",
    ...
    fn google_oauth_secret_store_loader_is_lazy_for_unconfigured_partial_and_env_secret_states() {
        ...
        let partial = GoogleOAuthEnvInputs {
            ironclaw_client_id: Some("client.apps.googleusercontent.com".to_string()),
        ...
        let env_secret = GoogleOAuthEnvInputs {
            ironclaw_client_id: Some("client.apps.googleusercontent.com".to_string()),
            ironclaw_redirect_uri: Some("http://127.0.0.1:3000/oauth/google/callback".to_string()),
            ironclaw_client_secret: Some("GOCSPX-env-secret".to_string()),
            ..GoogleOAuthEnvInputs::default()
        };
        let resolution =
    ...
    fn google_oauth_client_secret_from_store_reads_back_a_stored_secret() {
        ...
        std::fs::write(
            storage_root.join(ironclaw_composition::LOCAL_DEV_SECRETS_MASTER_KEY_PATH),
    ...
    fn google_oauth_client_secret_from_store_is_read_only_on_a_pristine_home() {
        let _guard = lock_runtime_env();
        let temp = tempfile::tempdir().expect("tempdir");
        let ironclaw_home = temp.path().join("does-not-exist-yet");
        let config = IronClawBootConfig::resolve_from_env_parts(
        ...
        assert!(
            ...
            "a read-only diagnostic must not create the IronClaw home directory"
        );
    }
}
