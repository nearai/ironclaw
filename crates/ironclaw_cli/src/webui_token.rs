...
//! `<ironclaw_home>/webui-token` fallback file.
//!
...
/// relative to `<ironclaw_home>`. Shared by `onboard` (which writes it)
...
/// `<ironclaw_home>`.
pub(crate) fn webui_token_file_path(ironclaw_home: &Path) -> PathBuf {
    ironclaw_home.join(WEBUI_TOKEN_FILENAME)
}

/// Read the token file's raw contents through the shared safety checks:
/// reject a symlink (it could point outside `<ironclaw_home>` at a file the
...
fn repair_token_file_mode(path: &Path) -> anyhow::Result<()> {
    ...
    tracing::warn!(
        target: "ironclaw::cli::webui_token",
        path = %path.display(),
        mode = format!("{mode:o}"),
...
/// `Ok(true)` when a token file exists at `<ironclaw_home>/webui-token` and
...
pub(crate) fn webui_token_file_is_valid(ironclaw_home: &Path) -> anyhow::Result<bool> {
    Ok(
        read_token_file_checked(&webui_token_file_path(ironclaw_home))?
            .is_some_and(|contents| contents.trim().len() >= WEBUI_TOKEN_MIN_BYTES),
    )
}

/// Ensure `<ironclaw_home>/webui-token` holds a valid (>= entropy floor)
...
pub(crate) fn ensure_webui_token_file(ironclaw_home: &Path) -> anyhow::Result<FileWriteAction> {
    let file_path = webui_token_file_path(ironclaw_home);
    if webui_token_file_is_valid(ironclaw_home)? {
...
/// Unconditionally replace `<ironclaw_home>/webui-token` with a freshly
...
pub(crate) fn rotate_webui_token_file(ironclaw_home: &Path) -> anyhow::Result<()> {
    let file_path = webui_token_file_path(ironclaw_home);
    let token = generate_webui_token();
    write_token_file(&file_path, &token)
}
...
///    non-empty;
/// 2. `<ironclaw_home>/webui-token`, trimmed, when it exists and is
...
pub(crate) fn resolve_webui_token(
    ...
    if let Some(value) = env_value
        && !value.is_empty()
    {
        validate_token_entropy(value, env_var_name, ironclaw_home)?;
    ...
    match file_value {
        Some(token) => {
            validate_token_entropy(&token, env_var_name, ironclaw_home)?;
...
pub(crate) fn resolve_login_link_announcement(
    home: &ironclaw_config::IronClawHome,
    config_file: Option<&ironclaw_config::IronClawConfigFile>,
...
fn validate_token_entropy(
    ...
    Err(anyhow::anyhow!(
        ...
        webui_token_file_path(ironclaw_home).display(),
        value.len(),
    ))
}
...
/// one place.
pub(crate) fn login_link(home: &ironclaw_config::IronClawHome) -> anyhow::Result<Option<String>> {
...
mod tests {
    ...
    const VALID_TOKEN: &str = "ironclaw-smoke-test-token-0123456789abcdef"; // 40 bytes
