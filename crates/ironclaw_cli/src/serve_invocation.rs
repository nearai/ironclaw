...
use ironclaw_config::{
    IRONCLAW_HOME_ENV, IRONCLAW_PROFILE_ENV, IronClawBootConfig, LEGACY_IRONCLAW_PROFILE_ENV,
};
...
/// `IronClawBootConfig::profile()` (which always resolves to a default),
...
pub fn serve_invocation() -> Result<ServeInvocation> {
    ...
    let boot_config = IronClawBootConfig::resolve_from_env()
