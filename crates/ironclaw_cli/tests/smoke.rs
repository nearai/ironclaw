...
/// Shared builder for every real-binary spawn in this file: `Command::new(ironclaw_bin())`
...
fn ironclaw_command() -> Command {
    let mut command = Command::new(ironclaw_bin());
...
fn isolated_no_llm_command(workspace: &Path, ironclaw_home: &Path) -> Command {
    let mut command = ironclaw_command();
    command
...
#[cfg(unix)]
fn fake_ironclaw_bin(bin_dir: &Path) {
...
#[cfg(unix)]
fn write_ironclaw_config(ironclaw_home: &Path, profile: &str) {
    std::fs::create_dir_all(ironclaw_home).expect("IronClaw home");
...
#[cfg(unix)]
fn write_sparse_ironclaw_config(ironclaw_home: &Path) {
    std::fs::create_dir_all(ironclaw_home).expect("IronClaw home");
    std::fs::write(
        ironclaw_home.join("config.toml"),
        "api_version = \"ironclaw.runtime/v1\"\n",
...
#[test]
fn dockerfile_ironclaw_builds_without_backend_feature_flags() {
    let dockerfile =
...
#[test]
fn release_ci_compiles_ironclaw_for_all_supported_targets() {
    ...
    let compile_workflow =
        std::fs::read_to_string(root.join(".github/workflows/ironclaw-release-compile.yml"))
            .expect("IronClaw release compile workflow")
            .replace("\r\n", "\n");
    ...
    assert_eq!(
        ...
        target_runners.len(),
        "IronClaw release compile matrix must contain exactly seven targets"
    );
    for (target, runner) in target_runners {
        let matrix_entry = format!("          - target: {target}\n            runner: {runner}\n");
        assert!(
            compile_workflow.contains(&matrix_entry),
            "IronClaw release compile matrix must map {target} to {runner}"
        );
    }
    ...
    assert!(
        compile_workflow.contains("fail-fast: false")
            ...
            && !compile_workflow.contains("IRONCLAW_RELEASE_FEATURES")
            && !compile_workflow.contains("--features \"$IRONCLAW_RELEASE_FEATURES\""),
        "IronClaw release CI must fully link the shipping binary without removed backend feature flags and keep all target results"
    );
    assert_no_removed_backend_cargo_features(&compile_workflow, "IronClaw release CI");
    assert!(
    ...
    assert!(
        compile_workflow.contains("name: ironclaw-compile-${{ matrix.target }}")
    ...
    assert!(
        release_workflow.contains("\n  plan:\n")
            ...
            && !release_workflow.contains("uses: ./.github/workflows/ironclaw-release-compile.yml")
    ...
    assert!(
        ...
        "cargo-dist must select only the canonical IronClaw package and generate the supported installers without claiming the unavailable npm package name"
    );
    for target in [
    ...
    assert!(
        ...
        "the canonical IronClaw package must be cargo-dist enabled without removed backend feature flags and with WiX metadata"
    );
    ...
    assert!(
        !dist_features
            .iter()
            .any(|feature| feature == "libsql" || feature == "postgres"),
        "the canonical IronClaw package must be cargo-dist enabled without removed backend feature flags and with WiX metadata: parsed dist features={dist_features:?}\n{cli_manifest}"
    );
}
...
fn dockerfile_does_not_package_the_retired_ownership_migration() {
    ...
    let dockerfile =
        std::fs::read_to_string(workspace_root().join("Dockerfile")).expect("Dockerfile");
    assert!(
        !dockerfile.contains("ironclaw_migration")
...
#[test]
fn run_ironclaw_webui_builds_frontend_before_cargo() {
    let launcher = std::fs::read_to_string(workspace_root().join("scripts/run-ironclaw-webui.sh"))
        .expect("scripts/run-ironclaw-webui.sh");
    ...
    let cargo_run = launcher
        ...
        .expect("launcher should run the IronClaw CLI");
    assert!(
        frontend_build < cargo_run,
        "scripts/run-ironclaw-webui.sh must build frontend/dist before cargo compiles the binary: {launcher}"
    );
}

#[test]
fn docker_ironclaw_config_defaults_to_local_dev() {
    let config = std::fs::read_to_string(workspace_root().join("docker/ironclaw/config.toml"))
        .expect("docker IronClaw config");
    let parsed = ironclaw_config::IronClawConfigFile::parse_text(
        &config,
        &workspace_root().join("docker/ironclaw/config.toml"),
    )
    .expect("docker IronClaw config parses");
...
#[test]
fn docker_ironclaw_production_config_uses_postgres_storage() {
    let config =
        std::fs::read_to_string(workspace_root().join("docker/ironclaw/config.production.toml"))
            .expect("docker IronClaw production config");
    let parsed = ironclaw_config::IronClawConfigFile::parse_text(
        &config,
        &workspace_root().join("docker/ironclaw/config.production.toml"),
    )
    .expect("docker IronClaw production config parses");
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_uses_railway_volume_mount_for_home() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let volume = temp.path().join("railway-volume");
    let ironclaw_home = volume.join("ironclaw-reborn");
    write_ironclaw_config(&ironclaw_home, "local-dev");

    let output = Command::new("/bin/sh")
        .arg(workspace_root().join("docker/ironclaw/entrypoint.sh"))
        .arg("--help")
        .env_clear()
    ...
    assert!(
        stdout.contains(&format!("home={}", ironclaw_home.display())),
        "stdout: {stdout}"
    );
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_rejects_ephemeral_railway_without_volume() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_rejects_sparse_config_as_local_dev_on_railway() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_sparse_ironclaw_config(&ironclaw_home);
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_rejects_local_dev_home_outside_railway_volume() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let volume = temp.path().join("railway-volume");
    let ironclaw_home = temp.path().join("ephemeral-home");
    write_ironclaw_config(&ironclaw_home, "local-dev");
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_allows_railway_production_without_volume() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_ironclaw_config(&ironclaw_home, "production");
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_rejects_stale_local_dev_config_for_production() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let ironclaw_home = temp.path().join("ironclaw-home");
...
/// to bake in before this change (`docker/ironclaw/config.toml`,
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_migrates_a_stale_baked_llm_default_stub() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let ironclaw_home = temp.path().join("ironclaw-home");
    ...
    let config_path = ironclaw_home.join("config.toml");
    ...
    let backup_path = ironclaw_home.join("config.toml.pre-llm-migration");
...
#[cfg(unix)]
#[test]
fn docker_ironclaw_entrypoint_does_not_migrate_an_operator_modified_llm_default() {
    let temp = tempfile::tempdir().expect("tempdir");
    let bin_dir = temp.path().join("bin");
    fake_ironclaw_bin(&bin_dir);
    let ironclaw_home = temp.path().join("ironclaw-home");
    ...
    assert!(
        !ironclaw_home.join("config.toml.pre-llm-migration").exists(),
        "no backup should be written when nothing was migrated"
    );
}

#[test]
fn help_mentions_ironclaw_commands() {
    ...
    // No standalone `tui` subcommand exists (IronClaw's interactive surface
...
fn service_install_reports_error_when_service_definition_path_is_blocked() {
    ...
    let output = Command::new(ironclaw_bin())
        ...
        .env_clear()
        .env("HOME", &home)
        .env("IRONCLAW_HOME", temp.path().join("ironclaw-home"))
        .output()
        .expect("ironclaw service install should run");
...
#[test]
fn extension_search_does_not_seed_ironclaw_config() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    ...
    assert!(
        !ironclaw_home.join("config.toml").exists(),
        "extension search must not seed runtime config"
    );
}

#[test]
fn profile_list_shows_supported_profiles_without_ironclaw_home() {
    let output = ironclaw_command()
        .arg("profile")
        .arg("list")
        .output()
...
#[test]
fn profile_list_json_is_stable_and_does_not_resolve_ironclaw_home() {
    let output = ironclaw_command()
        .arg("profile")
        .arg("list")
        .arg("--json")
...
#[test]
fn skills_list_reports_ironclaw_skill_data() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let v1_home = temp.path().join("v1-home");
    write_ironclaw_skill(&ironclaw_home, "catalog-helper", "catalog helper");
...
#[test]
fn skills_list_verbose_reports_ironclaw_skill_details() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_verbose_ironclaw_skill(&ironclaw_home, "verbose-helper", "verbose helper");
...
#[test]
fn skills_list_json_reports_ironclaw_skill_data() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_ironclaw_skill(&ironclaw_home, "json-helper", "json helper");
...
#[test]
fn models_list_reports_ironclaw_provider_catalog_without_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let output = ironclaw_command()
        .arg("models")
        .arg("list")
...
#[test]
fn models_status_reads_ironclaw_default_llm_slot() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
...
#[test]
fn models_set_provider_writes_ironclaw_config_without_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let output = ironclaw_command()
        .arg("models")
    ...
    assert!(
        ...
        "IronClaw models set-provider must not write v1 bootstrap .env"
    );
}

#[test]
fn models_set_updates_ironclaw_default_model() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(
...
fn write_ironclaw_skill(ironclaw_home: &std::path::Path, name: &str, description: &str) {
    let skill_dir = ironclaw_cli_skill_root(ironclaw_home).join(name);
...
fn write_verbose_ironclaw_skill(ironclaw_home: &std::path::Path, name: &str, description: &str) {
    let skill_dir = ironclaw_cli_skill_root(ironclaw_home).join(name);
    std::fs::create_dir_all(&skill_dir).expect("skill dir");
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
fn ironclaw_cli_skill_root(ironclaw_home: &std::path::Path) -> std::path::PathBuf {
    ironclaw_home.join("local-dev/tenants/default/users/reborn-cli/skills")
}

#[test]
fn config_path_reports_ironclaw_home_without_touching_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    ...
    let output = ironclaw_command()
        .arg("config")
        .arg("path")
        .env("IRONCLAW_REBORN_HOME", &ironclaw_home)
    ...
    assert!(stdout.contains("v1_state: not-used"), "stdout: {stdout}");
    assert!(
        !ironclaw_home.exists(),
        "config path should not create IronClaw state directories"
    );
    assert!(
        !v1_base_dir.exists(),
...
#[test]
fn config_path_reports_default_ironclaw_home_without_creating_directories() {
    let temp = tempfile::tempdir().expect("tempdir");
    ...
    assert!(
        ...
        "config path should not create default IronClaw or v1 state directories"
    );
}
...
fn config_set_google_client_id_then_status_reports_partial_from_config_file() {
    ...
    let set_client_id = ironclaw_command()
        .args([
            "config",
            "set",
            "google.client_id",
...
#[test]
fn completion_generates_zsh_script_without_ironclaw_home() {
    let output = ironclaw_command()
        .arg("completion")
        .arg("--shell")
        .arg("zsh")
...
#[test]
fn completion_generates_bash_script_without_ironclaw_home() {
    let output = ironclaw_command()
        .arg("completion")
        .arg("--shell")
        .arg("bash")
...
fn serve_boots_without_user_id_env_var() {
    ...
    let mut child = ironclaw_command()
        ...
        .env(
            ...
            "ironclaw-smoke-test-token-0123456789abcdef",
        )
        .env_remove("IRONCLAW_WEBUI_USER_ID")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
/// Proves `serve` boots when launched with cwd=<ironclaw_home>/workspace, the
...
/// - Companion negative test below shows the prior cwd (ironclaw_home itself)
...
fn serve_boots_from_the_workspace_subdir_the_installed_service_now_uses_as_cwd() {
    ...
    let ironclaw_home = temp.path().join("ironclaw-home");
    let home = temp.path().join("home");
    let working_directory = ironclaw_home.join("workspace");
    std::fs::create_dir_all(&home).expect("home dir");
    ...
    let mut child = ironclaw_command()
        ...
        .env(
            "IRONCLAW_WEBUI_TOKEN",
            "ironclaw-smoke-test-cwd-workspace-token-0123456789abcdef",
        )
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
/// Companion regression pin: cwd=ironclaw_home itself (the crate's first,
/// insufficient fix attempt) still fails, because ironclaw_home is an
...
/// installer back to cwd=ironclaw_home.
#[test]
fn serve_crash_loops_with_skill_root_overlap_when_cwd_is_ironclaw_home_itself() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let home = temp.path().join("home");
    std::fs::create_dir_all(&home).expect("home dir");
    ...
    let output = ironclaw_command()
        .args(["serve", "--host", "127.0.0.1", "--port"])
        .arg(port.to_string())
        .current_dir(&ironclaw_home)
        .env("HOME", &home)
        .env("IRONCLAW_HOME", &ironclaw_home)
        .env_remove("IRONCLAW_PROFILE")
        .env(
            "IRONCLAW_WEBUI_TOKEN",
            "ironclaw-smoke-test-cwd-ironclaw-home-token-0123456789abcdef",
        )
        .output()
        .expect("ironclaw serve should run and exit");

    assert!(
        !output.status.success(),
        "serve launched with cwd=ironclaw_home must fail closed on the skill-root overlap, \
    ...
    assert!(
        ...
        "expected the exact composition overlap error this fix eliminates for \
         <ironclaw_home>/workspace: stderr={stderr}"
    );
}
...
#[test]
fn serve_with_env_auth_seeds_ironclaw_config_before_binding() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let home = temp.path().join("home");
    std::fs::create_dir_all(&home).expect("home dir");
    ...
    let config = std::fs::read_to_string(ironclaw_home.join("config.toml"))
...
#[test]
fn serve_resolves_bearer_token_from_ironclaw_home_webui_token_file() {
    ...
    // the `onboard`-provisioned `<ironclaw_home>/webui-token` fallback file.
    // Mirrors `serve_with_env_auth_seeds_ironclaw_config_before_binding` but
    // omits the env var and seeds the file instead.
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let home = temp.path().join("home");
    std::fs::create_dir_all(&home).expect("home dir");
    std::fs::create_dir_all(&ironclaw_home).expect("IronClaw home dir");
    std::fs::write(
        ironclaw_home.join("webui-token"),
...
"#,
    ...
    for (config, expected) in cases {
        ...
        let ironclaw_home = temp.path().join("ironclaw-home");
        std::fs::create_dir_all(&ironclaw_home).expect("IronClaw home");
        std::fs::write(ironclaw_home.join("config.toml"), config).expect("write config");

        let output = isolated_no_llm_command(temp.path(), &ironclaw_home)
...
fn serve_mounts_cli_login_route_without_sso() {
    ...
    let _serve_port_guard = SERVE_PORT_LOCK
        ...
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let port = unused_local_port();
    let webui_token = "ironclaw-smoke-test-token-0123456789abcdef";
    // File-sourced token, not `IRONCLAW_WEBUI_TOKEN` — this is the
    // one source `cli_login_mount` still mounts the route for.
    std::fs::create_dir_all(&ironclaw_home).expect("IronClaw home dir");
    std::fs::write(ironclaw_home.join("webui-token"), webui_token).expect("seed webui-token file");
...
fn serve_does_not_mount_cli_login_route_when_token_is_env_sourced() {
    ...
    let mut child = ironclaw_command()
        ...
        .env(
            "IRONCLAW_WEBUI_TOKEN",
            "ironclaw-smoke-test-env-token-0123456789abcdef",
        )
        .env("IRONCLAW_WEBUI_USER_ID", "test-user")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn run_reports_runtime_readiness_snapshot_without_touching_v1_state() {
    ...
    assert!(
        !ironclaw_home.exists(),
        "runtime readiness snapshot should not create IronClaw state directories"
    );
    assert!(
...
#[test]
fn doctor_uses_ironclaw_home_override_without_touching_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
...
#[test]
fn repl_exit_command_seeds_ironclaw_config() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let home_dir = temp.path().join("home");
    let v1_base_dir = temp.path().join("v1-state");
...
// mandatory dependency of the IronClaw CLI.
#[test]
...
#[test]
fn doctor_rejects_empty_ironclaw_home_override() {
    let output = ironclaw_command()
        .arg("doctor")
        .env("IRONCLAW_HOME", "")
        .output()
...
#[test]
fn doctor_rejects_relative_ironclaw_home_override() {
    let output = ironclaw_command()
        .arg("doctor")
        .env("IRONCLAW_HOME", "relative/ironclaw")
        .output()
        .expect("ironclaw doctor should run");
...
#[test]
fn doctor_rejects_missing_home_for_default_ironclaw_home() {
    let output = ironclaw_command()
        .arg("doctor")
        .output()
        .expect("ironclaw doctor should run");
...
fn config_init_writes_both_files() {
    ...
    assert!(
        ironclaw_home.join("providers.json").exists(),
    ...
    let config_text =
        std::fs::read_to_string(ironclaw_home.join("config.toml")).expect("config.toml readable");
    assert!(
...
fn config_init_refuses_to_clobber_without_force() {
    ...
    let first = Command::new(ironclaw_bin())
        .args(["config", "init"])
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
    ...
    let second = Command::new(ironclaw_bin())
        .args(["config", "init"])
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
...
fn config_init_preflights_both_targets_before_writing() {
    ...
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(ironclaw_home.join("providers.json"), "[]\n").expect("write providers");
...
fn config_init_with_force_overwrites() {
    ...
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(ironclaw_home.join("config.toml"), "partial config\n").expect("write config");
    std::fs::write(ironclaw_home.join("providers.json"), "partial providers\n")
        .expect("write providers");
    ...
    let config_text =
        std::fs::read_to_string(ironclaw_home.join("config.toml")).expect("config.toml readable");
    let providers_text = std::fs::read_to_string(ironclaw_home.join("providers.json"))
...
#[test]
fn onboard_bootstraps_ironclaw_home_without_touching_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    let v1_home = temp.path().join("v1-home");
    ...
    // onboard also provisions a `<ironclaw_home>/webui-token` fallback file
    ...
    // unset.
    let webui_token_path = ironclaw_home.join("webui-token");
    ...
    let marker_path = ironclaw_home.join(".onboard-completed.json");
...
fn onboard_is_idempotent_for_the_webui_token_file() {
    ...
    let token_path = ironclaw_home.join("webui-token");
    let first_token = std::fs::read_to_string(&token_path).expect("read webui-token");

    let second = ironclaw_command()
        .arg("onboard")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
...
///   `resolve_ironclaw_runtime_llm` returns `Ok(None)`, which is not a
...
fn onboard_then_serve_boots_in_degraded_mode_with_an_empty_environment() {
    ...
    let onboard_output = ironclaw_command()
        .arg("onboard")
        .env("HOME", &home)
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
    ...
    assert!(
        ironclaw_home.join("webui-token").exists(),
...
fn onboard_login_link_then_bearer_authorizes_a_protected_request() {
    ...
    seed_stored_llm_key(&ironclaw_home, "nearai", "nearai-smoke-test-session");
...
/// spawn `ironclaw` as a real child process and drive it over plain
...
fn spawn_chat_completion_stub() -> (String, std::sync::mpsc::Receiver<Option<String>>) {
    ...
    std::thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            // The IronClaw turn loop always drives the provider through its
...
fn patch_config_base_url(ironclaw_home: &Path, base_url: &str) {
    let config_path = ironclaw_home.join("config.toml");
...
fn stored_key_reaches_real_turn_via_webui_api() {
    ...
    let onboard_stdout = String::from_utf8_lossy(&onboard_output.stdout);
    let webui_token = std::fs::read_to_string(ironclaw_home.join("webui-token"))
        .expect("read onboard-provisioned webui-token")
        .trim()
        .to_string();
    assert!(
    ...
    let set_provider_output = ironclaw_command()
    ...
    seed_stored_llm_key_at_runtime_root(&ironclaw_home, "nearai", STORED_KEY);

    let (stub_base_url, auth_rx) = spawn_chat_completion_stub();
    patch_config_base_url(&ironclaw_home, &stub_base_url);
...
/// boots (fresh child process each time, same `ironclaw_home`, no `onboard` or
...
fn stored_key_reaches_real_turn_across_fresh_boots() {
    ...
    for boot in 1..=2 {
        let (stub_base_url, auth_rx) = spawn_chat_completion_stub();
        patch_config_base_url_replacing_previous(&ironclaw_home, &stub_base_url);
        ...
        let mut child = ironclaw_command()
            ...
            .stderr(Stdio::piped())
            .spawn()
            .unwrap_or_else(|error| panic!("boot {boot}: ironclaw serve should start: {error}"));
...
fn patch_config_base_url_replacing_previous(ironclaw_home: &Path, base_url: &str) {
    let config_path = ironclaw_home.join("config.toml");
    let original = std::fs::read_to_string(&config_path).expect("read config.toml to patch");
    if let Some(start) = original.find("base_url = \"http://127.0.0.1:") {
        ...
        patch_config_base_url(ironclaw_home, base_url);
    }
}
...
fn seed_stored_llm_key(ironclaw_home: &Path, provider_id: &str, key: &str) {
    std::fs::write(
        ironclaw_home.join(ironclaw_composition::LOCAL_DEV_SECRETS_MASTER_KEY_PATH),
    ...
    let ironclaw_home = ironclaw_home.to_path_buf();
    seed_rt.block_on(async move {
        let store = ironclaw_composition::open_local_dev_secret_store(&ironclaw_home)
            .await
...
/// reads from for a `local-dev` boot — `<ironclaw_home>/local-dev/…` (see
/// `local_runtime_storage_root` / `IronClawProfile::local_runtime_storage_
/// subdir`), NOT the bare `ironclaw_home` root [`seed_stored_llm_key`] (and
/// `onboard`'s own interactive credential prompt) write to.
///
/// This distinction matters for these real-turn tests specifically: they
/// pin the fix to `IronClawLlmReloadAdapter::reload`, which reads through
/// `IronClawRuntime`'s own `services.secret_store()` — rooted at the
...
fn seed_stored_llm_key_at_runtime_root(ironclaw_home: &Path, provider_id: &str, key: &str) {
    let runtime_root = ironclaw_home.join("local-dev");
...
/// before `resolve_ironclaw_runtime_llm` fails closed on `ApiKeyEnvUnset`.
...
fn onboard_openai_key_then_serve_boots_with_env_var_unset() {
    ...
    seed_stored_llm_key_at_runtime_root(
        &ironclaw_home,
        "openai",
        "sk-smoke-test-stored-openai-key",
    );
    ...
    let mut child = ironclaw_command()
        ...
        // `IronClawLlmReloadAdapter::reload`'s own `key_applied` trace below —
    ...
    // `resolve_ironclaw_runtime_llm` inside `IronClawLlmReloadAdapter::reload`
...
/// in the boot-time LLM reload (`IronClawLlmReloadAdapter::reload`) or its
...
/// (`local_runtime_storage_root`, i.e. `<ironclaw_home>/local-dev` —
...
/// key-presence — through `IronClawLlmReloadAdapter::reload`'s own
...
fn onboard_nearai_stored_key_then_serve_boots_with_cloud_base_url() {
    ...
    let mut child = ironclaw_command()
        ...
        // `IronClawLlmReloadAdapter::reload`, not the `ironclaw_cli`
...
fn serve_boots_with_env_api_key_set_and_empty_secret_store() {
    ...
    let config_text = std::fs::read_to_string(ironclaw_home.join("config.toml"))
        .expect("read seeded config.toml");
    assert!(
        !config_text_has_live_provider_id(&config_text),
        "config: {config_text}"
    );
...
fn onboard_prints_env_token_note_instead_of_login_link_when_env_token_is_set() {
    ...
    let onboard_output = ironclaw_command()
        ...
        .env(
            "IRONCLAW_WEBUI_TOKEN",
            "ironclaw-smoke-test-onboard-env-token-0123456789abcdef",
        )
        .output()
        .expect("ironclaw onboard should run");
    assert!(
...
fn status_prints_env_token_note_instead_of_login_link_when_env_token_is_set() {
    ...
    let status_output = ironclaw_command()
        .arg("status")
        .env("HOME", &home)
        .env("IRONCLAW_HOME", &ironclaw_home)
        .env(
            "IRONCLAW_WEBUI_TOKEN",
            "ironclaw-smoke-test-status-env-token-0123456789abcdef",
        )
        .output()
        .expect("ironclaw status should run");
    assert!(
...
fn onboard_dry_run_is_read_only() {
    ...
    assert!(
        !ironclaw_home.exists(),
        "dry-run must not create IronClaw home"
    );
}
...
fn onboard_dry_run_propagates_a_webui_token_io_error_without_mutating_home() {
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir ironclaw_home");
    std::fs::create_dir_all(ironclaw_home.join("webui-token"))
    ...
    assert!(
        !ironclaw_home.join(".onboard-completed.json").exists(),
    ...
    assert!(
        std::fs::metadata(ironclaw_home.join("webui-token"))
...
fn onboard_import_history_records_pending_step() {
    ...
    let marker_text = std::fs::read_to_string(ironclaw_home.join(".onboard-completed.json"))
        .expect("read marker");
    let marker: serde_json::Value = serde_json::from_str(&marker_text).expect("valid marker JSON");
...
fn onboard_preserves_existing_config_without_force() {
    ...
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(ironclaw_home.join("config.toml"), "custom config\n").expect("write config");
    std::fs::write(
        ironclaw_home.join(".onboard-completed.json"),
        "custom marker\n",
    )
    .expect("write marker");
    ...
    let config_text =
        std::fs::read_to_string(ironclaw_home.join("config.toml")).expect("read config");
    assert_eq!(
...
fn onboard_with_force_overwrites_existing_files_and_marker() {
    ...
    std::fs::create_dir_all(&ironclaw_home).expect("mkdir");
    std::fs::write(ironclaw_home.join("config.toml"), "custom config\n").expect("write config");
    std::fs::write(ironclaw_home.join("providers.json"), "custom providers\n")
        .expect("write providers");
    std::fs::write(
        ironclaw_home.join(".onboard-completed.json"),
        "custom marker\n",
    ...
    let config_text =
        std::fs::read_to_string(ironclaw_home.join("config.toml")).expect("read config");
    let providers_text =
        std::fs::read_to_string(ironclaw_home.join("providers.json")).expect("read providers");
    let marker_text = std::fs::read_to_string(ironclaw_home.join(".onboard-completed.json"))
        .expect("read marker");
...
/// A second `onboard` run over the same IronClaw home must not attempt the
...
/// (`<ironclaw_home>/local-dev/…`, `local_runtime_storage_root`'s subdir for
/// `IronClawProfile::LocalDev`) — the same root the real resolver
/// (`resolve_local_dev_secret_master_key_with_env`) reads/writes and
/// `serve` actually boots against — not the bare `ironclaw_home` root (PR
...
fn config_path_reports_file_presence() {
    ...
    let absent_output = Command::new(ironclaw_bin())
        .args(["config", "path"])
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
    ...
    let init_output = Command::new(ironclaw_bin())
        .args(["config", "init"])
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
    ...
    let present_output = Command::new(ironclaw_bin())
        .args(["config", "path"])
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
...
#[test]
fn status_reports_ironclaw_home_without_touching_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
...
#[test]
fn status_json_reports_ironclaw_home_without_touching_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
...
fn status_json_reports_present_config_and_providers_files() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("create IronClaw home");
    let config_path = ironclaw_home.join("config.toml");
    let providers_path = ironclaw_home.join("providers.json");
...
fn assert_config_read_rejects_malformed(args: &[&str]) {
    ...
    let ironclaw_home = temp.path().join("ironclaw-home");
    std::fs::create_dir_all(&ironclaw_home).expect("create IronClaw home");
    std::fs::write(ironclaw_home.join("config.toml"), "[boot\nprofile = broken")
...
"#;
    std::fs::write(ironclaw_home.join("config.toml"), bad_config).expect("write bad config");
...
fn run_warns_when_falling_back_to_stub_gateway() {
    ...
    let output = isolated_no_llm_command(&workspace, &ironclaw_home)
        .args(["run", "-m", "ping"])
        .output()
        .expect("ironclaw run should not crash");
    let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    assert!(
        ironclaw_home
            .join("local-dev/system/skills/code-review/SKILL.md")
            .is_file(),
        "runtime bootstrap should install bundled IronClaw skills"
    );
}
...
"#,
    ...
    .expect("write config");
    std::fs::write(ironclaw_home.join("providers.json"), "not json").expect("write providers");
...
"#,
    ...
    .expect("write config");
    std::fs::write(
        ironclaw_home.join("providers.json"),
        r#"[
  {
    "id": "empty-key-provider",
    "protocol": "open_ai_completions",
    "api_key_env": "IRONCLAW_TEST_EMPTY_KEY",
    "api_key_required": true,
    "model_env": "IRONCLAW_TEST_MODEL",
...
"#,
    ...
    let output = Command::new(ironclaw_bin())
        ...
        .env_remove("USERPROFILE")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .env("IRONCLAW_TEST_EMPTY_KEY", "")
        .output()
        .expect("ironclaw run should not crash");
    assert!(!output.status.success(), "empty API key must fail");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("IRONCLAW_TEST_EMPTY_KEY") && stderr.contains("requires API key env var"),
...
provider_id = "openai"
model = "gpt-4o-mini"
api_key_env = "IRONCLAW_TEST_UNSET_BC8F4D_KEY"
"#;
    std::fs::write(ironclaw_home.join("config.toml"), cfg).expect("write config");
    ...
    let output = Command::new(ironclaw_bin())
        ...
        .env_remove("IRONCLAW_TEST_UNSET_BC8F4D_KEY")
        .env("IRONCLAW_HOME", &ironclaw_home)
        .output()
        .expect("ironclaw run should not crash");
    assert!(
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("IRONCLAW_TEST_UNSET_BC8F4D_KEY"),
        "stderr should name the unset env var; got: {stderr}"
    );
}

#[test]
fn release_ci_publishes_ironclaw_without_enabling_legacy_or_docker_paths() {
    let root = workspace_root();
    let release_workflow =
        std::fs::read_to_string(root.join(".github/workflows/ironclaw-release.yml"))
            .expect("release workflow")
    ...
    for removed_job_name in [
        ...
        assert!(
            ...
            "IronClaw-only cargo-dist release must not define the old {removed_job_name} job"
        );
    }
    assert!(
        !release_workflow.contains("uses: ./.github/workflows/docker.yml")
            ...
            && !release_workflow.contains("ironclaw-compile-"),
        "the release workflow must consume only cargo-dist IronClaw artifacts"
    );
    assert!(
    ...
    assert!(
        workspace_manifest.contains("name = \"ironclaw_integration_tests\"")
            ...
            && cli_manifest.contains("[package]\nname = \"ironclaw\"\nversion = \"")
            && cli_manifest.contains("[package.metadata.dist]\ndist = true"),
        "cargo-dist package selection must exclude the root test-only package and enable only IronClaw"
    );
    assert!(
        ...
        "the MSI definition must install only the canonical IronClaw executable"
    );
    let cli_package_section = cli_manifest
        .split_once("[package]\n")
        .map(|(_, remainder)| remainder)
        .and_then(|remainder| remainder.split_once("\n[").map(|(section, _)| section))
        .expect("IronClaw CLI manifest must define a [package] section");
    let cli_description = cli_package_section
        ...
        .expect("IronClaw CLI manifest must define a package description");
    let wix_package_element = wix_manifest
        .split_once("<Package Id='*'")
        .map(|(_, remainder)| remainder)
        .and_then(|remainder| remainder.split_once("/>").map(|(element, _)| element))
        .expect("IronClaw WiX manifest must define a Package element");
    let wix_description = wix_package_element
        ...
        .expect("IronClaw WiX Package element must define a Description attribute");
    assert_eq!(
        wix_description, cli_description,
        "the checked-in WiX package description must match the Cargo package metadata"
    );
    let ironclaw_cli_selector = code_style_workflow
        .lines()
        .find(|line| line.contains("grep -Eq") && line.contains("crates/ironclaw_cli/"))
        .expect("code style workflow should classify IronClaw CLI changes");
    assert!(
        ironclaw_cli_selector.contains(r"\.github/dist-build-setup\.yml$")
            && ironclaw_cli_selector.contains(
                r"\.github/workflows/(code_style|ironclaw-release|docker|ironclaw-release-compile)\.yml$"
            ),
        "release workflow-only PRs must run the IronClaw CLI smoke contract"
    );
    assert!(
        code_style_workflow.contains(
            r#"if [[ "${{ needs.changes.outputs.has_ironclaw_cli }}" == "true" && "${{ needs.ironclaw-cli-smoke.result }}" != "success" ]]; then"#,
        ),
        "the required Code Style roll-up must propagate workflow-only IronClaw CLI smoke failures"
    );
}
