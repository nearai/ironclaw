...
#[test]
fn extension_search_json_reads_ironclaw_home_local_dev_packages() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_extension_fixture(&ironclaw_home, "zztest-mcp");

    let json = run_extension_json(&ironclaw_home, &["search", "zztest", "--json"]);
...
fn extension_search_json_without_query_lists_local_dev_packages() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    write_extension_fixture(&ironclaw_home, "zztest-alpha");
    write_extension_fixture(&ironclaw_home, "zztest-beta");

    let json = run_extension_json(&ironclaw_home, &["search", "--json"]);
...
#[test]
fn extension_install_json_uses_ironclaw_home_without_v1_state() {
    let temp = tempfile::tempdir().expect("tempdir");
    let ironclaw_home = temp.path().join("ironclaw-home");
    ...
    assert!(
        ironclaw_home
            ...
            .exists(),
        "extension install should operate inside IronClaw home"
    );
    assert!(
        !v1_base_dir.exists(),
...
fn extension_activate_and_remove_json_use_persisted_installation_state() {
    ...
    let install = run_extension_json(&ironclaw_home, &["install", "zztest-mcp", "--json"]);
    assert_eq!(install["phase"], "installed");

    let activate = run_extension_json(&ironclaw_home, &["activate", "zztest-mcp", "--json"]);
    ...
    let remove = run_extension_json(&ironclaw_home, &["remove", "zztest-mcp", "--json"]);
...
fn run_extension_json(ironclaw_home: &Path, args: &[&str]) -> serde_json::Value {
    let output = Command::new(ironclaw_bin())
        .arg("extension")
        .args(args)
        .env_clear()
...
fn write_extension_fixture(ironclaw_home: &Path, extension_id: &str) {
    write_extension_fixture_with_metadata(
        ironclaw_home,
        extension_id,
        "GitHub MCP",
...
fn write_extension_fixture_with_metadata(
    ...
    let extension_root = ironclaw_home
