//! IronClaw lifecycle hook traits — the shared interface between ironclaw
//! and proof_of_claw.
//!
//! Both crates depend on this crate to avoid a cyclic dependency:
//! - `ironclaw` implements the `Hook` trait to provide agent capabilities
//! - `proof_of_claw` implements `Hook` to add provable execution, TEE inference, and ZK proofs
