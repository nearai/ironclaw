...
pub fn verify_assertion(
    ...
    //
    // Ordered BEFORE the signCount policy deliberately: nothing may be
    // evaluated against an assertion whose signature has not been proven, or an
    // unauthenticated caller can distinguish "signCount regression" from "bad
    // signature" and learn about the stored counter without proving possession.
    ...
    // 9. signCount regression handling. Runs only on an assertion whose
    //    signature has already been verified (step 8).
    sign_count_policy
