...
mod tests {
    ...
    #[test]
    fn bad_signature_is_reported_before_sign_count_policy() {
        // Regression pin for the verification ORDER (#3964 review): an assertion
        // that both (a) fails the signCount policy and (b) has an invalid
        // signature must be rejected as `BadSignature`, not `SignCountPolicy`.
        //
        // If the policy were evaluated first, an unauthenticated caller could
        // distinguish "signCount regression" from "bad signature" and learn
        // about the credential's stored counter without ever proving
        // possession. Nothing may be evaluated against an unverified assertion.
        let fx = fixture(5, false);
        let pre = preimage(1);
        let commit = pre.commitment();
        let res = run(
            &fx, &pre, UP_UV, 3, // 3 < stored 5 -> would trip the signCount policy
            "webauthn.get", ORIGIN, &commit, None, true, // ...but the signature is tampered
        );
        assert!(
            matches!(res, Err(VerificationError::BadSignature)),
            "signature must be proven before the signCount policy is consulted; got {res:?}"
        );
    }
