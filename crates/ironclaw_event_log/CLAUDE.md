...
`crates/ironclaw_event_store/`. Keep storage drivers out of this
substrate crate: downstream store crates should depend on `ironclaw_event_log`,
...
Forbidden dependencies (enforced by `ironclaw_architecture_tests`): authorization,
...
`crates/ironclaw_architecture_tests/tests/reborn_dependency_boundaries.rs`; if the
