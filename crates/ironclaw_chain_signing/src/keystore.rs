...
#[cfg(test)]
mod tests {
    // Test-only seam, kept OUT of the production impl: the struct ratchet bans
    // test-support members on production types, and a child module can still
    // reach the private `keys` map and `StoredKey` / `KeyStoreKey` layout.
    impl SecretsKeyStore {
