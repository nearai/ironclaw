...
- Do not add workspace dependencies beyond `ironclaw_reborn_composition`, `ironclaw_reborn_config`, `ironclaw_reborn_traces`, `ironclaw_auth` (auth-owned contracts used by binary-assembled first-party wiring), and `ironclaw_webui` (host-owned WebUI serve lifecycle) without an architecture test update and explicit PR rationale. Provider registry/model UX should enter through the Reborn composition provider-admin facade, not a separate CLI-only path.
