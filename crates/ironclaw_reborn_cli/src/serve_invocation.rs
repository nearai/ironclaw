...
//!
//! Two call sites, both inside this crate: the `service` subcommand's
//! plist/unit generators (`commands/service/`) and the `tui` subcommand
//! entry (`commands/tui.rs`, Part B), which hands the result to
//! `ironclaw_reborn_tui`'s `ProcessInvocation` for auto-spawn. The TUI
//! crate never calls this directly — `ironclaw_reborn_cli` already
//! depends on `ironclaw_reborn_tui`, so the reverse call would be
//! circular.
