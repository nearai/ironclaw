...
fn default_dockerfile_targets_reborn_runtime() {
    ...
    assert!(
        ...
        "default Dockerfile must build the canonical Reborn CLI image: {dockerfile}"
    );
    assert!(
...
fn release_ci_compiles_reborn_for_all_supported_targets() {
    ...
    assert!(
        cli_manifest.contains("[package.metadata.dist]\ndist = true\nfeatures = [\"full\"]"),
        "cargo-dist and the explicit target matrix must build the same production feature set"
    );
}
