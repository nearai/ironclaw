...
- **Waiting on a run state MUST go through `TurnStatus::wait_class()`** (the exhaustive `RunWaitClass` classifier next to `is_terminal()` in `status.rs`). Never hand-roll a partial `is_terminal()`-based predicate in a wait/poll loop: the four `Blocked*` states are *parked* — they never self-advance. `ParkedAwaitingUser` (approval/auth/resource) is resolved only by the user, so a waiter must **surface** it, never poll-until-timeout-then-cancel (that kills a run the user was about to resolve) and never spin forever (that hangs the caller). `ParkedAwaitingRun` (`BlockedDependentRun`) is resolved when its dependent child completes, so a waiter may keep polling it like `Running`. `is_terminal()` keeps its exact meaning (`TerminalSuccess | TerminalFailure`) and a test pins the two classifiers' equivalence — extend `wait_class()`'s exhaustive match, do not add a new partial predicate. Known waiters on this contract: `wait_for_terminal` (`ironclaw_reborn_composition`), `wait_for_response_completion` (OpenAI-compat serve), and `SubagentCompletionObserver` (`ironclaw_reborn`). Follow-up: migrate `slack_delivery.rs`'s hand-rolled status predicate onto `wait_class()`.
