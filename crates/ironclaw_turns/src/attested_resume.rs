...
pub enum AttestedResumeRejection {
    ...
    EvidenceRejected,
    /// The verifier could not reach a verdict for an infrastructural reason
    /// (backend/IO failure), NOT because the claim was judged invalid.
    ///
    /// This is the one rejection that must stay retryable: a transient failure
    /// must not be cached under the caller's idempotency key, or the caller is
    /// permanently wedged on that key for a fault that has since cleared.
    Unavailable,
}
...
impl AttestedResumeRejection {
    ...
    pub fn category(self) -> &'static str {
        match self {
            ...
            Self::Unavailable => "attested_verifier_unavailable",
        }
    }

    /// Whether this verdict is **final** — i.e. a semantic judgement about the
    /// claim that will not change on retry, and may therefore be cached under
    /// the resume idempotency key.
    ///
    /// [`Self::Unavailable`] is the only non-final variant: it reports that no
    /// verdict was reached, so caching it would turn a transient outage into a
    /// permanently stuck gate.
