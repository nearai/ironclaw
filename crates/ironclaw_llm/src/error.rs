...
fn auth_guidance(provider: &str) -> String {
    ...
    let env_hint = match normalized.as_str() {
        "nearai" | "near_ai" | "near" => "Set NEARAI_API_KEY (from https://cloud.near.ai)",
        ...
        "openai_codex" | "codex" => {
            "Log in with the Codex CLI, then set LLM_USE_CODEX_AUTH=true (or set CODEX_AUTH_PATH)"
        }
        "github_copilot" => "Set GITHUB_COPILOT_TOKEN",
        _ => "Check that the required API key environment variable is set for this provider",
    };
    format!("{env_hint}.")
}
...
mod tests {
    ...
    fn auth_failed_error_includes_guidance() {
        ...
        assert!(
            ...
            "guidance must be runtime-neutral: {msg}"
        );
    }
    ...
    fn snapshot_auth_failed_nearai() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'nearai'. Set NEARAI_API_KEY (from https://cloud.near.ai)."
        );
    }
    ...
    fn snapshot_auth_failed_openai() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'openai'. Set OPENAI_API_KEY (from https://platform.openai.com/api-keys)."
        );
    }
    ...
    fn snapshot_auth_failed_anthropic() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'anthropic'. Set ANTHROPIC_API_KEY (from https://console.anthropic.com/settings/keys)."
        );
    }
    ...
    fn snapshot_auth_failed_ollama() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'ollama'. Ensure Ollama is running locally (no API key needed). Set OLLAMA_BASE_URL if not at default http://localhost:11434."
        );
    }
    ...
    fn snapshot_auth_failed_openai_compatible() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'openai_compatible'. Set LLM_API_KEY and LLM_BASE_URL for your OpenAI-compatible endpoint."
        );
    }
    ...
    fn snapshot_auth_failed_tinfoil() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'tinfoil'. Set TINFOIL_API_KEY."
        );
    }
    ...
    fn snapshot_auth_failed_bedrock() {
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'bedrock'. Configure AWS credentials (AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY or AWS_PROFILE)."
        );
    }
    ...
    fn snapshot_auth_failed_unknown_provider() {
        ...
        insta::assert_snapshot!(
            ...
            @"Authentication failed for provider 'some_future_provider'. Check that the required API key environment variable is set for this provider."
        );
    }
}
