...
fn escape_reminder_delimiters(content: &str) -> String {
    ...
    while cursor < content.len() {
        ...
        let open = lowered[cursor..].find(REMINDER_OPEN).map(|at| cursor + at);
        let close = lowered[cursor..].find(REMINDER_CLOSE).map(|at| cursor + at);
        // A `</system-reminder>` also contains no `<system-reminder>`, so the
        // two matches never overlap; take whichever comes first.
        let next = match (open, close) {
            (Some(o), Some(c)) if o <= c => Some((o, REMINDER_OPEN)),
            (Some(_), Some(c)) => Some((c, REMINDER_CLOSE)),
            (Some(o), None) => Some((o, REMINDER_OPEN)),
            (None, Some(c)) => Some((c, REMINDER_CLOSE)),
            (None, None) => None,
        };
        ...
        // Escape the ORIGINAL slice, not the constant: the payload keeps its
        // own casing so the host's text is shown to the model as written.
