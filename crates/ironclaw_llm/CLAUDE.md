...
When a terminal `[DONE]` marker arrives without any explicit finish reason, the provider infers `Stop` from non-empty text/reasoning or `ToolUse` from parsed tool calls; an explicit unrecognized finish reason remains `Unknown`.
