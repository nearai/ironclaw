...
**Interrupted text streams:** A streamed response is complete only after an SSE `[DONE]` marker or an explicit provider finish reason. If transport ends after plain assistant text but before either terminal signal, the provider makes one semantic continuation request with the partial assistant response in context and tools disabled. The continuation has a 20-second timeout so the original 60-second request plus recovery remains below the Reborn runner lease. Recovery deltas are buffered and overlap-trimmed before publication, preventing repeated prefixes in the UI. Incomplete tool-call streams are never continued as prose and remain fail-loud.
